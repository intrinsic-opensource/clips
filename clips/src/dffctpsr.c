...
/************************************************************/
globle int ParseDeffacts(void* theEnv, const char* readSource) {
  ...
  deffactsName = GetConstructNameAndComment(
      theEnv, readSource, &inputToken, "deffacts", EnvFindDeffactsInModule,
      EnvUndeffacts, "$", TRUE, TRUE, TRUE, FALSE);
  ...
  temp = BuildRHSAssert(theEnv, readSource, &inputToken, &deffactsError, FALSE,
                        FALSE, "deffacts");
  ...
  newDeffacts->header.whichModule = (struct defmoduleItemHeader*)GetModuleItem(
      theEnv, NULL, FindModuleItem(theEnv, "deffacts")->moduleIndex);
