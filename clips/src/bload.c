...
static struct FunctionDefinition* FastFindFunction(void*, const char*,
...
globle void InitializeBloadData(void* theEnv) {
  char sizeBuffer[20];
  sprintf(sizeBuffer, "%2d%2d%2d%2d%2d", (int)sizeof(void*),
          (int)sizeof(double), (int)sizeof(int), (int)sizeof(long),
  ...
  BloadData(theEnv)->BinarySizes =
      (char*)genalloc(theEnv, strlen(sizeBuffer) + 1);
...
/*    data for the bload command.               */
/************************************************/
static void DeallocateBloadData(void* theEnv) {
  ...
  genfree(theEnv, BloadData(theEnv)->BinarySizes,
          strlen(BloadData(theEnv)->BinarySizes) + 1);
}
...
/******************************/
globle int EnvBload(void* theEnv, const char* fileName) {
  ...
  GenReadBinary(theEnv, IDbuffer,
                (unsigned long)strlen(BloadData(theEnv)->BinaryPrefixID) + 1);
  ...
  GenReadBinary(theEnv, IDbuffer,
                (unsigned long)strlen(BloadData(theEnv)->BinaryVersionID) + 1);
  ...
  GenReadBinary(theEnv, sizesBuffer,
                (unsigned long)strlen(BloadData(theEnv)->BinarySizes) + 1);
  ...
  for (bfPtr = BloadData(theEnv)->BeforeBloadFunctions; bfPtr != NULL;
       bfPtr = bfPtr->next) {
  ...
  BloadData(theEnv)->FunctionArray =
  ...
  for (GenReadBinary(theEnv, constructBuffer,
                     (unsigned long)CONSTRUCT_HEADER_SIZE);
       strncmp(constructBuffer, BloadData(theEnv)->BinaryPrefixID,
               CONSTRUCT_HEADER_SIZE) != 0;
       GenReadBinary(theEnv, constructBuffer,
                     (unsigned long)CONSTRUCT_HEADER_SIZE)) {
    intBool found;
    ...
    found = FALSE;
    for (biPtr = BsaveData(theEnv)->ListOfBinaryItems; biPtr != NULL;
         biPtr = biPtr->next) {
  ...
  for (bfPtr = BloadData(theEnv)->AfterBloadFunctions; bfPtr != NULL;
       bfPtr = bfPtr->next) {
    if (bfPtr->environmentAware) {
      (*bfPtr->func)(theEnv);
    } else {
 ...
 ************************************************************/
globle void BloadandRefresh(void* theEnv, long objcnt, size_t objsz,
  ...
  oldOutOfMemoryFunction =
      EnvSetOutOfMemoryFunction(theEnv, BloadOutOfMemoryFunction);
  objsmaxread = objcnt;
  do {
...
static struct FunctionDefinition** ReadNeededFunctions(
    void* theEnv, long int* numberOfFunctions, int* error) {
  ...
  for (i = 0; i < *numberOfFunctions; i++) {
    if ((functionPtr = FastFindFunction(theEnv, namePtr, functionPtr)) ==
        NULL) {
...
static struct FunctionDefinition* FastFindFunction(
    void* theEnv, const char* functionName,
  ...
  while (strcmp(functionName, ValueToString(theFunction->callFunctionName)) !=
         0) {
...
/*************************************/
static int ClearBload(void* theEnv) {
  struct BinaryItem* biPtr;
  struct callFunctionItem* bfPtr;
  int ready, error;
  ...
  error = FALSE;
  for (bfPtr = BloadData(theEnv)->ClearBloadReadyFunctions; bfPtr != NULL;
       bfPtr = bfPtr->next) {
    if (bfPtr->environmentAware) {
...
static void AbortBload(void* theEnv) {
  ...
  for (bfPtr = BloadData(theEnv)->AbortBloadFunctions; bfPtr != NULL;
       bfPtr = bfPtr->next) {
    if (bfPtr->environmentAware) {
      (*bfPtr->func)(theEnv);
    } else {
...
/********************************************/
globle void AddBeforeBloadFunction(void* theEnv, const char* name,
                                   void (*func)(void*), int priority) {
  BloadData(theEnv)->BeforeBloadFunctions =
      AddFunctionToCallList(theEnv, name, priority, func,
                            BloadData(theEnv)->BeforeBloadFunctions, TRUE);
}
...
/*   a binary load occurs.                 */
/*******************************************/
globle void AddAfterBloadFunction(void* theEnv, const char* name,
                                  void (*func)(void*), int priority) {
  BloadData(theEnv)->AfterBloadFunctions =
      AddFunctionToCallList(theEnv, name, priority, func,
                            BloadData(theEnv)->AfterBloadFunctions, TRUE);
}
...
/**************************************************/
globle void AddClearBloadReadyFunction(void* theEnv, const char* name,
                                       int (*func)(void*), int priority) {
  BloadData(theEnv)->ClearBloadReadyFunctions =
      AddFunctionToCallList(theEnv, name, priority, (void (*)(void*))func,
...
/*   has to be aborted.                      */
/*********************************************/
globle void AddAbortBloadFunction(void* theEnv, const char* name,
                                  void (*func)(void*), int priority) {
  BloadData(theEnv)->AbortBloadFunctions =
      AddFunctionToCallList(theEnv, name, priority, func,
                            BloadData(theEnv)->AbortBloadFunctions, TRUE);
}
...
/*******************************************************
  ...
  NOTES        : None
 *******************************************************/
static int BloadOutOfMemoryFunction(void* theEnv, size_t size) {
#if MAC_XCD
...
/*****************************************************/
globle void CannotLoadWithBloadMessage(void* theEnv,
...
globle int Bload(const char* fileName) {
