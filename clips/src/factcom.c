...
static DATA_OBJECT_PTR GetSaveFactsDeftemplateNames(void*, struct expr*, int,
                                                    int*, int*);
...
globle void FactCommandDefinitions(void* theEnv) {
#if !RUN_TIME
#if DEBUGGING_FUNCTIONS
  EnvDefineFunction2(theEnv, "facts", 'v', PTIEF FactsCommand, "FactsCommand",
                     "*4iu");
#endif

  EnvDefineFunction(theEnv, "assert", 'u', PTIEF AssertCommand,
                    "AssertCommand");
  EnvDefineFunction2(theEnv, "retract", 'v', PTIEF RetractCommand,
                     "RetractCommand", "1*z");
  EnvDefineFunction2(theEnv, "assert-string", 'u', PTIEF AssertStringFunction,
                     "AssertStringFunction", "11s");
  EnvDefineFunction2(theEnv, "str-assert", 'u', PTIEF AssertStringFunction,
                     "AssertStringFunction", "11s");
  ...
  EnvDefineFunction2(theEnv, "save-facts", 'b', PTIEF SaveFactsCommand,
                     "SaveFactsCommand", "1*wk");
  EnvDefineFunction2(theEnv, "load-facts", 'b', PTIEF LoadFactsCommand,
                     "LoadFactsCommand", "11k");
  EnvDefineFunction2(theEnv, "fact-index", 'g', PTIEF FactIndexFunction,
                     "FactIndexFunction", "11y");
...
globle void AssertCommand(void* theEnv, DATA_OBJECT_PTR rv) {
  ...
  if (theDeftemplate->implied == FALSE) {
    ...
    if (theExpression->nextArg == NULL) {
      ...
      newFact->theProposition.theFields[0].value =
          CreateMultifield2(theEnv, 0L);
    }
    slotPtr = NULL;
  }
  ...
  for (theExpression = theExpression->nextArg, i = 0; theExpression != NULL;
...
globle void RetractCommand(void* theEnv) {
  ...
  for (theArgument = GetFirstArgument(), argNumber = 1; theArgument != NULL;
    ...
    if (theResult.type == INTEGER) {
      ...
      if (factIndex < 0) {
        ExpectedTypeError1(theEnv, "retract", argNumber,
    ...
    /*============================================*/
                 ...
                 ? (strcmp(ValueToString(theResult.value), "*") == 0)
                 : FALSE) {
...
globle void FactsCommand(void* theEnv) {
  ...
  if ((argumentCount = EnvArgCountCheck(theEnv, "facts", NO_MORE_THAN, 4)) ==
      -1)
    return;
  ...
  if (theValue.type == SYMBOL) {
    theModule = (struct defmodule*)EnvFindDefmodule(
        theEnv, ValueToString(theValue.value));
    if ((theModule == NULL) &&
        (strcmp(ValueToString(theValue.value), "*") != 0)) {
      SetEvaluationError(theEnv, TRUE);
      CantFindItemErrorMessage(theEnv, "defmodule",
                               ValueToString(theValue.value));
      return;
    }
  ...
  if ((end = GetFactsArgument(theEnv, 2 + argOffset, argumentCount)) == INVALID)
    return;
  if ((max = GetFactsArgument(theEnv, 3 + argOffset, argumentCount)) == INVALID)
    return;
...
/*****************************************************/
globle void EnvFacts(void* theEnv, const char* logicalName, void* vTheModule,
  ...
  if (allModules)
    ...
    factPtr = (struct fact*)GetNextFactInScope(theEnv, NULL);
  ...
  while (factPtr != NULL) {
    ...
    if (allModules)
      ...
      factPtr = (struct fact*)GetNextFactInScope(theEnv, factPtr);
  }
...
static long long GetFactsArgument(void* theEnv, int whichOne,
  ...
  if (EnvArgTypeCheck(theEnv, "facts", whichOne, INTEGER, &theValue) == FALSE)
    return (INVALID);
...
globle int SaveFactsCommand(void* theEnv) {
  ...
  if (numArgs > 1) {
    ...
    if (strcmp(argument, "local") == 0) {
      ...
      ExpectedTypeError1(theEnv, "save-facts", 2,
...
/**************************************************************/
globle intBool EnvSaveFacts(void* theEnv, const char* fileName, int saveCode) {
...
/********************************************************************/
globle intBool EnvSaveFactsDriver(void* theEnv, const char* fileName,
                                  int saveCode, struct expr* theList) {
  ...
  theDOArray =
      GetSaveFactsDeftemplateNames(theEnv, theList, saveCode, &count, &error);
...
/*******************************************************************/
static DATA_OBJECT_PTR GetSaveFactsDeftemplateNames(void* theEnv,
                                                    struct expr* theList,
                                                    int saveCode, int* count,
                                                    int* error) {
  ...
  *error = FALSE;
  ...
  for (tempList = theList, *count = 0; tempList != NULL;
  ...
  for (tempList = theList, i = 0; i < *count;
    ...
    if (saveCode == LOCAL_SAVE) {
      theDeftemplate = (struct deftemplate*)EnvFindDeftemplateInModule(
          theEnv, ValueToString(theDOArray[i].value));
      if (theDeftemplate == NULL) {
        *error = TRUE;
        ExpectedTypeError1(theEnv, "save-facts", 3 + i,
                           "local deftemplate name");
        rm3(theEnv, theDOArray, (long)sizeof(DATA_OBJECT) * *count);
        return (NULL);
      }
    } else if (saveCode == VISIBLE_SAVE) {
      theDeftemplate = (struct deftemplate*)FindImportedConstruct(
          theEnv, "deftemplate", NULL, ValueToString(theDOArray[i].value),
          &tempCount, TRUE, NULL);
      if (theDeftemplate == NULL) {
        *error = TRUE;
        ExpectedTypeError1(theEnv, "save-facts", 3 + i,
...
/**************************************************************/
globle intBool EnvLoadFacts(void* theEnv, const char* fileName) {
  FILE* filePtr;
...
/*********************************************/
globle intBool EnvLoadFactsFromString(void* theEnv, const char* theString,
                                      long theMax) {
...
/**************************************************************************/
static struct expr* StandardLoadFact(void* theEnv, const char* logicalName,
                                     struct token* theToken) {
  int error = FALSE;
  struct expr* temp;
  ...
  temp->argList = GetRHSPattern(theEnv, logicalName, theToken, &error, TRUE,
                                FALSE, TRUE, RPAREN);
...
/****************************************************************/
static struct expr* AssertParse(void* theEnv, struct expr* top,
...
globle void Facts(const char* logicalName, void* vTheModule, long long start,
                  long long end, long long max) {
  EnvFacts(GetCurrentEnvironment(), logicalName, vTheModule, start, end, max);
}

#endif /* DEBUGGING_FUNCTIONS */

globle intBool LoadFacts(const char* fileName) {
  return EnvLoadFacts(GetCurrentEnvironment(), fileName);
}

globle intBool SaveFacts(const char* fileName, int saveCode) {
  return EnvSaveFacts(GetCurrentEnvironment(), fileName, saveCode);
}

globle intBool LoadFactsFromString(const char* theString, int theMax) {
