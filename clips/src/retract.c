...
static intBool FindNextConflictingMatch(void*, struct partialMatch*,
                                        struct partialMatch*, struct joinNode*,
...
globle void PosEntryRetractAlpha(void* theEnv, struct partialMatch* alphaMatch,
  ...
  while (betaMatch != NULL) {
    ...
    if ((joinPtr->ruleToActivate != NULL) ? (betaMatch->marker != NULL)
                                          : FALSE) {
      RemoveActivation(theEnv, (struct activation*)betaMatch->marker, TRUE,
                       TRUE);
    }
...
/*************************/
static void NegEntryRetractAlpha(void* theEnv, struct partialMatch* alphaMatch,
                                 int operation) {
  ...
  while (betaMatch != NULL) {
    ...
    if ((!joinPtr->patternIsNegated) && (!joinPtr->patternIsExists) &&
...
/* NegEntryRetractBeta: */
/************************/
static void NegEntryRetractBeta(void* theEnv, struct joinNode* joinPtr,
  ...
  if (FindNextConflictingMatch(theEnv, betaMatch, alphaMatch->nextInMemory,
                               joinPtr, alphaMatch, operation)) {
    return;
  ...
  if ((operation == NETWORK_RETRACT) &&
      PartialMatchWillBeDeleted(theEnv, betaMatch)) {
    return;
  }
...
/* PosEntryRetractBeta: */
/************************/
globle void PosEntryRetractBeta(void* theEnv, struct partialMatch* parentMatch,
                                struct partialMatch* betaMatch, int operation) {
  ...
  while (betaMatch != NULL) {
    ...
    if (betaMatch->rhsMemory) {
      UnlinkNonLeftLineage(theEnv, (struct joinNode*)betaMatch->owner,
                           betaMatch, RHS);
    } else {
      UnlinkNonLeftLineage(theEnv, (struct joinNode*)betaMatch->owner,
                           betaMatch, LHS);
...
/******************************************************************/
static intBool FindNextConflictingMatch(void* theEnv,
  ...
  for (; possibleConflicts != NULL;
    ...
    /*======================================================*/
             ...
             PartialMatchWillBeDeleted(theEnv,
                                       possibleConflicts)) { /* Do Nothing */
    }
...
/*   FALSE.                                                */
/***********************************************************/
static intBool PartialMatchDefunct(void* theEnv, struct partialMatch* thePM) {
  unsigned short i;
...
/*****************************************************************/
intBool PartialMatchWillBeDeleted(void* theEnv, struct partialMatch* thePM) {
  unsigned short i;
  struct patternEntity* thePE;
...
globle void ReturnPartialMatch(void* theEnv, struct partialMatch* waste) {
  ...
  rtn_var_struct(theEnv, partialMatch,
                 (int)sizeof(struct genericMatch*) * (waste->bcount - 1),
                 waste);
}
...
/******************************************************/
static void ReturnMarkers(void* theEnv, struct multifieldMarker* waste) {
