...
globle void ProceduralFunctionDefinitions(void* theEnv) {
  AllocateEnvironmentData(theEnv, PRCDRFUN_DATA,
  ...
  EnvDefineFunction2(theEnv, "while", 'u', PTIEF WhileFunction, "WhileFunction",
                     NULL);
  EnvDefineFunction2(theEnv, "loop-for-count", 'u', PTIEF LoopForCountFunction,
                     "LoopForCountFunction", NULL);
  EnvDefineFunction2(theEnv, "(get-loop-count)", 'g', PTIEF GetLoopCount,
                     "GetLoopCount", NULL);
  EnvDefineFunction2(theEnv, "bind", 'u', PTIEF BindFunction, "BindFunction",
                     NULL);
  EnvDefineFunction2(theEnv, "progn", 'u', PTIEF PrognFunction, "PrognFunction",
                     NULL);
  EnvDefineFunction2(theEnv, "return", 'u', PTIEF ReturnFunction,
                     "ReturnFunction", NULL);
  EnvDefineFunction2(theEnv, "break", 'v', PTIEF BreakFunction, "BreakFunction",
                     NULL);
  EnvDefineFunction2(theEnv, "switch", 'u', PTIEF SwitchFunction,
                     "SwitchFunction", NULL);
...
globle void LoopForCountFunction(void* theEnv, DATA_OBJECT_PTR loopResult) {
  ...
  if (EnvArgTypeCheck(theEnv, "loop-for-count", 1, INTEGER, &arg_ptr) ==
      FALSE) {
  ...
  if (EnvArgTypeCheck(theEnv, "loop-for-count", 2, INTEGER, &arg_ptr) ==
      FALSE) {
    loopResult->type = SYMBOL;
    loopResult->value = EnvFalseSymbol(theEnv);
    ProcedureFunctionData(theEnv)->LoopCounterStack = tmpCounter->nxt;
...
globle void IfFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  if (EvaluationData(theEnv)->CurrentExpression->argList->nextArg->nextArg ==
      NULL) {
    numArgs = 2;
  } else if (EvaluationData(theEnv)
                 ->CurrentExpression->argList->nextArg->nextArg->nextArg ==
             NULL) {
    numArgs = 3;
  } else {
    EnvArgRangeCheck(theEnv, "if", 2, 3);
  ...
  EvaluateExpression(theEnv, EvaluationData(theEnv)->CurrentExpression->argList,
                     returnValue);
  ...
  if ((returnValue->value == EnvFalseSymbol(theEnv)) &&
      (returnValue->type == SYMBOL) && (numArgs == 3)) {
    theExpr =
        EvaluationData(theEnv)->CurrentExpression->argList->nextArg->nextArg;
    switch (theExpr->type) {
      case INTEGER:
      case FLOAT:
      case SYMBOL:
...
globle void BindFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  if (unbindVar == FALSE) {
    ...
    DecrementSymbolCount(theEnv,
                         (struct symbolHashNode*)theBind->supplementalInfo);
...
/*******************************************/
globle intBool GetBoundVariable(void* theEnv, DATA_OBJECT_PTR vPtr,
                                SYMBOL_HN* varName) {
...
globle void SwitchFunction(void* theEnv, DATA_OBJECT_PTR result) {
  ...
  for (theExp = GetFirstArgument()->nextArg; theExp != NULL;
    ...
    if (switch_val.type == case_val.type) {
      if ((case_val.type == MULTIFIELD)
              ? MultifieldDOsEqual(&switch_val, &case_val)
