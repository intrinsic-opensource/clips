...
static int ConstructsToC(void*, const char*, const char*, char*, long long,
                         long long);
...
globle void InitializeConstructCompilerData(void* theEnv) {
  AllocateEnvironmentData(theEnv, CONSTRUCT_COMPILER_DATA,
...
/*    data for the constructs-to-c command.                 */
/************************************************************/
static void DeallocateConstructCompilerData(void* theEnv) {
...
globle void ConstructsToCCommand(void* theEnv) {
  ...
  if ((argCount = EnvArgRangeCheck(theEnv, "constructs-to-c", 2, 4)) == -1)
    return;
  ...
  if (EnvArgTypeCheck(theEnv, "constructs-to-c", 1, SYMBOL_OR_STRING,
                      &theArg) == FALSE) {
    return;
  }
  ...
  if (EnvArgTypeCheck(theEnv, "constructs-to-c", 2, INTEGER, &theArg) ==
      FALSE) {
    return;
  }
  ...
  if (argCount == 3) {
    if (EnvArgTypeCheck(theEnv, "constructs-to-c", 3, SYMBOL_OR_STRING,
                        &theArg) == FALSE) {
      return;
    }
  ...
  if (argCount == 4) {
    if (EnvArgTypeCheck(theEnv, "constructs-to-c", 4, INTEGER, &theArg) ==
        FALSE) {
      return;
    }
  ...
  fileNameBuffer =
      (char*)genalloc(theEnv, nameLength + pathLength + EXTRA_FILE_NAME);
...
/***************************************/
static int ConstructsToC(void* theEnv, const char* fileName,
                         const char* pathName, char* fileNameBuffer,
                         long long theImageID, long long max) {
  int fileVersion;
  ...
  if ((ConstructCompilerData(theEnv)->HeaderFP =
           GenOpen(theEnv, fileNameBuffer, "w")) == NULL) {
  ...
  WriteFunctionExternDeclarations(theEnv,
  ...
  for (cgPtr = ConstructCompilerData(theEnv)->ListOfCodeGeneratorItems;
    ...
    if (cgPtr->generateFunction != NULL) {
      (*cgPtr->generateFunction)(theEnv, fileName, pathName, fileNameBuffer,
                                 fileVersion,
                                 ConstructCompilerData(theEnv)->HeaderFP,
                                 ConstructCompilerData(theEnv)->ImageID,
                                 ConstructCompilerData(theEnv)->MaxIndices);
...
/*******************************************************/
static void WriteFunctionExternDeclarations(void* theEnv, FILE* fp) {
...
/****************************************************/
static int FunctionsToCode(void* theEnv, const char* fileName,
                           const char* pathName, char* fileNameBuffer) {
  short i = 0;
  FILE* fp;
  int version = 1;
  ...
  for (fctnPtr = GetFunctionList(theEnv); fctnPtr != NULL;
  ...
  if ((fp = NewCFile(theEnv, fileName, pathName, fileNameBuffer, 2, version,
                     FALSE)) == NULL) {
    return (0);
  }
  ...
  while (fctnPtr != NULL) {
    ...
    fprintf(fp, "NULL,");
    if (fctnPtr->restrictions != NULL)
      fprintf(fp, "\"%s\",", fctnPtr->restrictions);
    else
      fprintf(fp, "NULL,");
    ...
    if ((i > ConstructCompilerData(theEnv)->MaxIndices) || (fctnPtr == NULL)) {
      ...
      version++;
      if (fctnPtr != NULL) {
        if ((fp = NewCFile(theEnv, fileName, pathName, fileNameBuffer, 2,
                           version, FALSE)) == NULL)
          return (0);
        newHeader = TRUE;
      }
...
/************************************************************/
globle void PrintFunctionReference(void* theEnv, FILE* fp,
                                   struct FunctionDefinition* funcPtr) {
  if (funcPtr == NULL)
    fprintf(fp, "NULL");
  else
    fprintf(
        fp, "&P%d_%d[%d]", ConstructCompilerData(theEnv)->ImageID,
...
/******************************************/
static int WriteInitializationFunction(void* theEnv, const char* fileName,
                                       const char* pathName,
                                       char* fileNameBuffer) {
  FILE* fp;
  struct CodeGeneratorItem* cgPtr;
  ...
  fprintf(ConstructCompilerData(theEnv)->HeaderFP,
          "   void *InitCImage_%d(void);\n",
          ConstructCompilerData(theEnv)->ImageID);
  fprintf(ConstructCompilerData(theEnv)->HeaderFP,
          "   void FixupCImage_%d(void *);\n",
          ConstructCompilerData(theEnv)->ImageID);
  ...
  fprintf(fp,
          "   theEnv = CreateRuntimeEnvironment(sht%d,fht%d,iht%d,bmht%d);\n\n",
          ConstructCompilerData(theEnv)->ImageID,
          ConstructCompilerData(theEnv)->ImageID,
          ConstructCompilerData(theEnv)->ImageID,
          ConstructCompilerData(theEnv)->ImageID);
  ...
  while (cgPtr != NULL) {
    if (cgPtr->initFunction != NULL) {
      (*cgPtr->initFunction)(theEnv, fp, ConstructCompilerData(theEnv)->ImageID,
                             ConstructCompilerData(theEnv)->MaxIndices);
      fprintf(fp, "\n");
    }
    cgPtr = cgPtr->next;
...
/**************************************************/
globle FILE* NewCFile(void* theEnv, const char* fileName, const char* pathName,
                      char* fileNameBuffer, int id, int version,
                      int reopenOldFile) {
  FILE* newFP;
...
/*   the table.                                           */
/**********************************************************/
static void HashedExpressionsToCode(void* theEnv) {
  unsigned i;
  ...
  for (i = 0; i < EXPRESSION_HASH_SIZE; i++) {
    for (exphash = ExpressionData(theEnv)->ExpressionHashTable[i];
      ...
      exphash->bsaveID = ConstructCompilerData(theEnv)->ExpressionCount +
                         (ConstructCompilerData(theEnv)->MaxIndices *
...
/*****************************************************/
globle void PrintHashedExpressionReference(void* theEnv, FILE* theFile,
  ...
  if (theExpression == NULL) {
    ...
    fprintf(theFile, "&E%d_%ld[%ld]", imageID, theIDValue / maxIndices,
...
globle int ExpressionToCode(void* theEnv, FILE* fp, struct expr* exprPtr) {
  ...
  if (ConstructCompilerData(theEnv)->ExpressionHeader == TRUE) {
    if ((ConstructCompilerData(theEnv)->ExpressionFP =
             NewCFile(theEnv, ConstructCompilerData(theEnv)->FilePrefix,
...
/**********************************************************/
static void DumpExpression(void* theEnv, struct expr* exprPtr) {
  while (exprPtr != NULL) {
    fprintf(ConstructCompilerData(theEnv)->ExpressionFP, "{");
    ...
    switch (exprPtr->type) {
      case FCALL:
        PrintFunctionReference(theEnv,
                               ConstructCompilerData(theEnv)->ExpressionFP,
                               (struct FunctionDefinition*)exprPtr->value);
        break;

      case INTEGER:
        PrintIntegerReference(theEnv,
                              ConstructCompilerData(theEnv)->ExpressionFP,
                              (INTEGER_HN*)exprPtr->value);
        break;

      case FLOAT:
        PrintFloatReference(theEnv, ConstructCompilerData(theEnv)->ExpressionFP,
                            (FLOAT_HN*)exprPtr->value);
        break;

      case PCALL:
#if DEFFUNCTION_CONSTRUCT
        PrintDeffunctionReference(theEnv,
                                  ConstructCompilerData(theEnv)->ExpressionFP,
      ...
      case GCALL:
        ...
        PrintGenericFunctionReference(
            theEnv, ConstructCompilerData(theEnv)->ExpressionFP,
            (DEFGENERIC*)exprPtr->value, ConstructCompilerData(theEnv)->ImageID,
            ConstructCompilerData(theEnv)->MaxIndices);
#else
        fprintf(ConstructCompilerData(theEnv)->ExpressionFP, "NULL");
#endif
      ...
      case DEFTEMPLATE_PTR:
        ...
        DeftemplateCConstructReference(
            theEnv, ConstructCompilerData(theEnv)->ExpressionFP, exprPtr->value,
            ConstructCompilerData(theEnv)->ImageID,
            ConstructCompilerData(theEnv)->MaxIndices);
#else
        fprintf(ConstructCompilerData(theEnv)->ExpressionFP, "NULL");
      ...
      case INSTANCE_ADDRESS:
        ...
        fprintf(ConstructCompilerData(theEnv)->ExpressionFP, "NULL");
        fprintf(
            ConstructCompilerData(theEnv)->FixupFP,
      ...
      case INSTANCE_NAME:
      case GBL_VARIABLE:
        PrintSymbolReference(theEnv,
                             ConstructCompilerData(theEnv)->ExpressionFP,
                             (SYMBOL_HN*)exprPtr->value);
        break;
      ...
      default:
        if (EvaluationData(theEnv)->PrimitivesArray[exprPtr->type] == NULL) {
          fprintf(ConstructCompilerData(theEnv)->ExpressionFP, "NULL");
                       ...
                       ->bitMap) {
          PrintBitMapReference(theEnv,
                               ConstructCompilerData(theEnv)->ExpressionFP,
                               (BITMAP_HN*)exprPtr->value);
        } else {
          fprintf(ConstructCompilerData(theEnv)->ExpressionFP, "NULL");
        }
        break;
    ...
    if (exprPtr->argList == NULL) {
      ...
      fprintf(ConstructCompilerData(theEnv)->ExpressionFP, "&E%d_%d[%ld],",
              ConstructCompilerData(theEnv)->ImageID,
              ConstructCompilerData(theEnv)->ExpressionVersion,
              ConstructCompilerData(theEnv)->ExpressionCount);
    }
    ...
    if (exprPtr->nextArg == NULL) {
      ...
      fprintf(ConstructCompilerData(theEnv)->ExpressionFP, "&E%d_%d[%ld]}",
              ...
              ConstructCompilerData(theEnv)->ExpressionVersion,
              ConstructCompilerData(theEnv)->ExpressionCount +
                  ExpressionSize(exprPtr->argList));
    }
...
globle void ConstructsToCCommandDefinition(void* theEnv) {
  EnvDefineFunction2(theEnv, "constructs-to-c", 'v', PTIEF ConstructsToCCommand,
...
globle struct CodeGeneratorItem* AddCodeGeneratorItem(
    void* theEnv, const char* name, int priority, void (*beforeFunction)(void*),
    void (*initFunction)(void*, FILE*, int, int),
    int (*generateFunction)(void*, const char*, const char*, char*, int, FILE*,
                            int, int),
    int arrayCount) {
  ...
  if (arrayCount != 0) {
    ...
    for (i = 0; i < arrayCount; i++) {
      if (ConstructCompilerData(theEnv)->CodeGeneratorCount < PRIMARY_LEN) {
        gensprintf(
            theBuffer, "%c",
            PRIMARY_CODES[ConstructCompilerData(theEnv)->CodeGeneratorCount]);
      } else {
        gensprintf(
            theBuffer, "%c_",
            SECONDARY_CODES[ConstructCompilerData(theEnv)->CodeGeneratorCount -
                            PRIMARY_LEN]);
      }
...
/*   exceeded.                                              */
/************************************************************/
globle FILE* CloseFileIfNeeded(void* theEnv, FILE* theFile, int* theCount,
                               int* arrayVersion, int maxIndices,
                               int* canBeReopened,
  ...
  if (theFile == NULL) {
    ...
    theFile = NewCFile(theEnv, codeFile->filePrefix, codeFile->pathName,
                       codeFile->fileNameBuffer, codeFile->id,
                       codeFile->version, TRUE);
    if (theFile == NULL) {
...
/*   exceeded.                                              */
/******************************************************************/
globle FILE* OpenFileIfNeeded(void* theEnv, FILE* theFile, const char* fileName,
                              const char* pathName, char* fileNameBuffer,
                              int fileID, int imageID, int* fileCount,
                              int arrayVersion, FILE* headerFP,
                              const char* structureName, char* structPrefix,
                              int reopenOldFile,
                              struct CodeGeneratorFile* codeFile) {
  char arrayName[80];
  const char* newName;
  ...
  if ((theFile = NewCFile(theEnv, newName, pathName, fileNameBuffer, newID,
...
globle void MarkConstructBsaveIDs(void* theEnv, int constructModuleIndex) {
  ...
  DoForAllConstructs(theEnv, MarkConstruct, constructModuleIndex, FALSE,
                     &theCount);
}
...
/*************************************************************/
static void MarkConstruct(void* theEnv, struct constructHeader* theConstruct,
                          void* vTheBuffer) {
...
/***********************************************************/
globle void ConstructHeaderToCode(void* theEnv, FILE* theFile,
                                  struct constructHeader* theConstruct,
                                  int imageID, int maxIndices, int moduleCount,
  ...
  fprintf(theFile, "MIHS &%s%d_%d[%d],", constructModulePrefix, imageID,
  ...
  if (theConstruct->next == NULL) {
    ...
    fprintf(theFile, "CHS &%s%d_%ld[%ld]}", constructPrefix, imageID,
...
/***********************************************************/
globle void ConstructModuleToCode(void* theEnv, FILE* theFile,
                                  struct defmodule* theModule, int imageID,
                                  int maxIndices, int constructIndex,
                                  const char* constructPrefix) {
  ...
  theModuleItem = (struct defmoduleItemHeader*)GetModuleItem(theEnv, theModule,
                                                             constructIndex);
...
/************************************/
void ConstructsToCCommand(void* theEnv) {
#if MAC_XCD
#pragma unused(theEnv)
#endif
}
