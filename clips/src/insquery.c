...
static QUERY_CLASS* DetermineQueryClasses(void*, EXPRESSION*, const char*,
                                          unsigned*);
...
static int TestForFirstInstanceInClass(void*, struct defmodule*, int, DEFCLASS*,
                                       QUERY_CLASS*, unsigned);
static void TestEntireChain(void*, QUERY_CLASS*, unsigned);
static void TestEntireClass(void*, struct defmodule*, int, DEFCLASS*,
                            QUERY_CLASS*, unsigned);
...
globle void SetupQuery(void* theEnv) {
  AllocateEnvironmentData(theEnv, INSTANCE_QUERY_DATA,
...
/* =============================================================================
   ...
   Following are the instance query functions :
     ...
     delayed-do-for-all-instances : Same as above - except that the list of
   instances which satisfy the query is formed before any actions are executed
 ...
 ******************************************************************************/
globle intBool AnyInstances(void* theEnv) {
  ...
  rm(theEnv, (void*)InstanceQueryData(theEnv)->QueryCore->solns,
     (sizeof(INSTANCE_TYPE*) * rcnt));
...
/******************************************************************************
  ...
  NOTES        : H/L Syntax : See ParseQueryNoAction()
 ******************************************************************************/
globle void QueryFindInstance(void* theEnv, DATA_OBJECT* result) {
  QUERY_CLASS* qclasses;
  unsigned rcnt, i;
  ...
  if (TestForFirstInChain(theEnv, qclasses, 0) == TRUE) {
    ...
    for (i = 1; i <= rcnt; i++) {
      ...
      SetMFValue(
          result->value, i,
          GetFullInstanceName(
              theEnv, InstanceQueryData(theEnv)->QueryCore->solns[i - 1]));
    }
  } else
    result->value = (void*)EnvCreateMultifield(theEnv, 0L);
  InstanceQueryData(theEnv)->AbortQuery = FALSE;
...
/******************************************************************************
  ...
  NOTES        : H/L Syntax : See ParseQueryNoAction()
 ******************************************************************************/
globle void QueryFindAllInstances(void* theEnv, DATA_OBJECT* result) {
  QUERY_CLASS* qclasses;
  unsigned rcnt;
  unsigned i, j;
  ...
  result->value = (void*)EnvCreateMultifield(
      theEnv, InstanceQueryData(theEnv)->QueryCore->soln_cnt * rcnt);
  while (InstanceQueryData(theEnv)->QueryCore->soln_set != NULL) {
    for (i = 0, j = (unsigned)(result->end + 2); i < rcnt; i++, j++) {
      ...
      SetMFValue(
          result->value, j,
          GetFullInstanceName(
              theEnv, InstanceQueryData(theEnv)->QueryCore->soln_set->soln[i]));
    }
    result->end = (long)j - 2;
    PopQuerySoln(theEnv);
  }
 ...
 ******************************************************************************/
globle void QueryDoForInstance(void* theEnv, DATA_OBJECT* result) {
  QUERY_CLASS* qclasses;
  unsigned i, rcnt;
  ...
  if (TestForFirstInChain(theEnv, qclasses, 0) == TRUE) {
    ...
    EvaluateExpression(theEnv, InstanceQueryData(theEnv)->QueryCore->action,
                       result);
...
/******************************************************************************
  ...
  NOTES        : H/L Syntax : See ParseQueryAction()
 ******************************************************************************/
globle void QueryDoForAllInstances(void* theEnv, DATA_OBJECT* result) {
  QUERY_CLASS* qclasses;
  unsigned rcnt;
...
/******************************************************************************
  ...
  NOTES        : H/L Syntax : See ParseQueryNoAction()
 ******************************************************************************/
globle void DelayedQueryDoForAllInstances(void* theEnv, DATA_OBJECT* result) {
  QUERY_CLASS* qclasses;
  unsigned rcnt;
  unsigned i;
  ...
  for (theSet = InstanceQueryData(theEnv)->QueryCore->soln_set; theSet != NULL;
       theSet = theSet->nxt) {
    for (i = 0; i < rcnt; i++) {
      theSet->soln[i]->busy++;
    }
...
static QUERY_CORE* FindQueryCore(void* theEnv, int depth) {
  ...
  if (depth == 0) return (InstanceQueryData(theEnv)->QueryCore);
  qptr = InstanceQueryData(theEnv)->QueryCoreStack;
  while (depth > 1) {
    qptr = qptr->nxt;
    depth--;
 ...
 **********************************************************/
static QUERY_CLASS* DetermineQueryClasses(void* theEnv, EXPRESSION* classExp,
...
/*************************************************************
  ...
  NOTES        : None
 *************************************************************/
static QUERY_CLASS* FormChain(void* theEnv, const char* func,
                              DATA_OBJECT* val) {
  DEFCLASS* cls;
...
/************************************************************
  ...
  NOTES        : None
 ************************************************************/
static int TestForFirstInChain(void* theEnv, QUERY_CLASS* qchain,
                               unsigned indx) {
  QUERY_CLASS* qptr;
  int id;
  ...
  for (qptr = qchain; qptr != NULL; qptr = qptr->chain) {
    InstanceQueryData(theEnv)->AbortQuery = FALSE;
    if ((id = GetTraversalID(theEnv)) == -1) return (FALSE);
    if (TestForFirstInstanceInClass(theEnv, qptr->theModule, id, qptr->cls,
                                    qchain, indx)) {
...
/*****************************************************************
  ...
  NOTES        : None
 *****************************************************************/
static int TestForFirstInstanceInClass(void* theEnv,
                                       struct defmodule* theModule, int id,
                                       DEFCLASS* cls, QUERY_CLASS* qchain,
                                       unsigned indx) {
  long i;
  INSTANCE_TYPE* ins;
  DATA_OBJECT temp;
  ...
  if (TestTraversalID(cls->traversalRecord, id)) return (FALSE);
  SetTraversalID(cls->traversalRecord, id);
  if (DefclassInScope(theEnv, cls, theModule) == FALSE) return (FALSE);
  ...
  while (ins != NULL) {
    ...
    if (qchain->nxt != NULL) {
      ...
      ins->busy++;
      EvaluateExpression(theEnv, InstanceQueryData(theEnv)->QueryCore->query,
                         &temp);
      ins->busy--;
    ...
    ins = ins->nxtClass;
    while ((ins != NULL) ? (ins->garbage == 1) : FALSE) ins = ins->nxtClass;
  }
...
endTest:
  ...
  for (i = 0; i < cls->directSubclasses.classCount; i++) {
    if (TestForFirstInstanceInClass(theEnv, theModule, id,
                                    cls->directSubclasses.classArray[i], qchain,
                                    indx))
      return (TRUE);
    if ((EvaluationData(theEnv)->HaltExecution == TRUE) ||
        (InstanceQueryData(theEnv)->AbortQuery == TRUE))
...
/************************************************************
  ...
  NOTES        : None
 ************************************************************/
static void TestEntireChain(void* theEnv, QUERY_CLASS* qchain, unsigned indx) {
  QUERY_CLASS* qptr;
  int id;
...
/*****************************************************************
  ...
  NOTES        : None
 *****************************************************************/
static void TestEntireClass(void* theEnv, struct defmodule* theModule, int id,
  ...
  if (TestTraversalID(cls->traversalRecord, id)) return;
  SetTraversalID(cls->traversalRecord, id);
  if (DefclassInScope(theEnv, cls, theModule) == FALSE) return;
    ...
    if (qchain->nxt != NULL) {
      ...
      if ((temp.type != SYMBOL) ? TRUE
        ...
        if (InstanceQueryData(theEnv)->QueryCore->action != NULL) {
          ...
          ValueDeinstall(theEnv, InstanceQueryData(theEnv)->QueryCore->result);
          EvaluateExpression(theEnv,
                             InstanceQueryData(theEnv)->QueryCore->action,
                             InstanceQueryData(theEnv)->QueryCore->result);
          ValueInstall(theEnv, InstanceQueryData(theEnv)->QueryCore->result);
...
endTest:
  ...
  if (ins != NULL) return;
  for (i = 0; i < cls->directSubclasses.classCount; i++) {
    TestEntireClass(theEnv, theModule, id, cls->directSubclasses.classArray[i],
                    qchain, indx);
    if ((EvaluationData(theEnv)->HaltExecution == TRUE) ||
        (InstanceQueryData(theEnv)->AbortQuery == TRUE))
      return;
...
static void AddSolution(void* theEnv) {
  ...
  new_soln->soln = (INSTANCE_TYPE**)gm2(
      theEnv, (sizeof(INSTANCE_TYPE*) *
...
static void PopQuerySoln(void* theEnv) {
  ...
  rm(theEnv, (void*)InstanceQueryData(theEnv)->QueryCore->soln_bottom->soln,
     (sizeof(INSTANCE_TYPE*) *
      InstanceQueryData(theEnv)->QueryCore->soln_size));
  rm(theEnv, (void*)InstanceQueryData(theEnv)->QueryCore->soln_bottom,
     sizeof(QUERY_SOLN));
}
