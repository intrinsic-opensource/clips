...
unsigned long HashFact(struct fact* theFact) {
  ...
  count +=
      (unsigned long)theFact->whichDeftemplate->header.name->bucket * 73981;
...
/**********************************************/
static struct fact* FactExists(void* theEnv, struct fact* theFact,
...
/************************************************************/
globle void AddHashedFact(void* theEnv, struct fact* theFact,
                          unsigned long hashValue) {
...
/****************************************************/
globle intBool FactWillBeAsserted(void* theEnv, void* theFact) {
  struct fact* tempPtr;
  unsigned long hashValue;
...
/*****************************************************/
globle unsigned long HandleFactDuplication(void* theEnv, void* theFact,
                                           intBool* duplicate) {
  struct fact* tempPtr;
  unsigned long hashValue;
  *duplicate = FALSE;
...
/*******************************************************************/
static struct factHashEntry** CreateFactHashTable(void* theEnv,
                                                  unsigned long tableSize) {
  unsigned long i;
  struct factHashEntry** theTable;

  theTable = (struct factHashEntry**)gm3(
      theEnv, sizeof(struct factHashEntry*) * tableSize);
...
/* ResizeFactHashTable: */
/*******************************************************************/
static void ResizeFactHashTable(void* theEnv) {
...
/* ResetFactHashTable: */
/*******************************************************************/
static void ResetFactHashTable(void* theEnv) {
...
/*****************************************************/
globle void ShowFactHashTable(void* theEnv) {
  int i, count;
...
globle intBool SetFactDuplication(int value) {
