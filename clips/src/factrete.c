...
/***************************************************************/
globle intBool FactPNGetVar1(void* theEnv, void* theValue,
  ...
  if (hack->allFields) {
    ...
    if (returnValue->type == MULTIFIELD) {
      ...
      SetpDOEnd(returnValue,
                ((struct multifield*)fieldPtr->value)->multifieldLength);
    }
...
/**************************************************/
globle intBool FactPNGetVar2(void* theEnv, void* theValue,
                             DATA_OBJECT_PTR returnValue) {
  struct fact* factPtr;
...
/*****************************************************************/
globle intBool FactPNGetVar3(void* theEnv, void* theValue,
                             DATA_OBJECT_PTR returnValue) {
  struct fact* factPtr;
  struct multifield* segmentPtr;
  struct field* fieldPtr;
  ...
  segmentPtr =
      (struct multifield*)factPtr->theProposition.theFields[hack->whichSlot]
          .value;
  ...
  if (hack->fromBeginning && hack->fromEnd) {
    ...
    returnValue->end =
        (long)(segmentPtr->multifieldLength - (hack->endOffset + 1));
    return (TRUE);
  }
  ...
  if (hack->fromBeginning) {
    ...
    fieldPtr =
        &segmentPtr
             ->theFields[segmentPtr->multifieldLength - (hack->endOffset + 1)];
  }
...
/******************************************************/
globle intBool FactPNConstant1(void* theEnv, void* theValue,
                               DATA_OBJECT_PTR returnValue) {
#if MAC_XCD
#pragma unused(returnValue)
  ...
  fieldPtr =
      &FactData(theEnv)
           ->CurrentPatternFact->theProposition.theFields[hack->whichSlot];
...
/****************************************************************/
globle intBool FactPNConstant2(void* theEnv, void* theValue,
                               DATA_OBJECT_PTR returnValue) {
#if MAC_XCD
#pragma unused(returnValue)
#endif
  ...
  if (fieldPtr->type == MULTIFIELD) {
    ...
    if (hack->fromBeginning) {
      ...
      fieldPtr =
          &segmentPtr
               ->theFields[segmentPtr->multifieldLength - (hack->offset + 1)];
    }
  }
...
/**************************************************************/
globle intBool FactJNGetVar1(void* theEnv, void* theValue,
                             DATA_OBJECT_PTR returnValue) {
  unsigned short theField, theSlot;
  struct fact* factPtr;
  struct field* fieldPtr;
  ...
  if (hack->lhs) {
    factPtr = (struct fact*)get_nth_pm_match(EngineData(theEnv)->GlobalLHSBinds,
                                             hack->whichPattern)
                  ->matchingItem;
    marks =
        get_nth_pm_match(EngineData(theEnv)->GlobalLHSBinds, hack->whichPattern)
            ->markers;
  } else if (hack->rhs) {
    factPtr = (struct fact*)get_nth_pm_match(EngineData(theEnv)->GlobalRHSBinds,
                                             hack->whichPattern)
                  ->matchingItem;
    marks =
        get_nth_pm_match(EngineData(theEnv)->GlobalRHSBinds, hack->whichPattern)
            ->markers;
    ...
    marks =
        get_nth_pm_match(EngineData(theEnv)->GlobalLHSBinds, hack->whichPattern)
            ->markers;
  } else if ((((unsigned short)(EngineData(theEnv)->GlobalJoin->depth - 1))) ==
             hack->whichPattern) {
    factPtr =
        (struct fact*)get_nth_pm_match(EngineData(theEnv)->GlobalRHSBinds, 0)
            ->matchingItem;
  ...
  segmentPtr =
      (struct multifield*)factPtr->theProposition.theFields[theSlot].value;
  fieldPtr = &segmentPtr->theFields[theField];
...
/*   extracted is from a single field slot.      */
/*************************************************/
globle intBool FactJNGetVar2(void* theEnv, void* theValue,
                             DATA_OBJECT_PTR returnValue) {
  struct fact* factPtr;
...
/*   slot that contains at most one multifield variable.        */
/****************************************************************/
globle intBool FactJNGetVar3(void* theEnv, void* theValue,
                             DATA_OBJECT_PTR returnValue) {
  struct fact* factPtr;
  struct multifield* segmentPtr;
  struct field* fieldPtr;
...
/****************************************************/
globle intBool FactSlotLength(void* theEnv, void* theValue,
                              DATA_OBJECT_PTR returnValue) {
  ...
  for (tempMark = FactData(theEnv)->CurrentPatternMarks; tempMark != NULL;
  ...
  segmentPtr =
      (struct multifield*)FactData(theEnv)
          ->CurrentPatternFact->theProposition.theFields[hack->whichSlot]
          .value;
  ...
  if (hack->exactly &&
      (segmentPtr->multifieldLength > (hack->minLength + extraOffset))) {
    return (FALSE);
  }
...
/************************************************************/
globle int FactJNCompVars1(void* theEnv, void* theValue,
...
#pragma unused(theResult)
  ...
  fact1 = (struct fact*)EngineData(theEnv)
              ->GlobalRHSBinds->binds[p1]
              .gm.theMatch->matchingItem;

  if (hack->p2rhs) {
    fact2 = (struct fact*)EngineData(theEnv)
                ->GlobalRHSBinds->binds[p2]
                .gm.theMatch->matchingItem;
  } else {
    fact2 = (struct fact*)EngineData(theEnv)
                ->GlobalLHSBinds->binds[p2]
                .gm.theMatch->matchingItem;
  }
...
/*****************************************************************/
globle int FactJNCompVars2(void* theEnv, void* theValue,
                           DATA_OBJECT* theResult) {
#if MAC_XCD
#pragma unused(theResult)
#endif
  ...
  if (fact1->theProposition.theFields[s1].type != MULTIFIELD) {
    ...
    if (hack->fromBeginning1) {
      ...
      fieldPtr1 =
          &segment->theFields[segment->multifieldLength - (hack->offset1 + 1)];
    }
  }
  ...
  if (fact2->theProposition.theFields[s2].type != MULTIFIELD) {
    ...
    if (hack->fromBeginning2) {
      ...
      fieldPtr2 =
          &segment->theFields[segment->multifieldLength - (hack->offset2 + 1)];
    }
  }
...
/*****************************************************/
globle int FactPNCompVars1(void* theEnv, void* theValue,
                           DATA_OBJECT* theResult) {
  int rv;
  struct field *fieldPtr1, *fieldPtr2;
  ...
  fieldPtr1 = &FactData(theEnv)
                   ->CurrentPatternFact->theProposition.theFields[hack->field1];
  fieldPtr2 = &FactData(theEnv)
                   ->CurrentPatternFact->theProposition.theFields[hack->field2];
  ...
  if (fieldPtr1->type != fieldPtr2->type)
    rv = (int)hack->fail;
  else if (fieldPtr1->value != fieldPtr2->value)
    rv = (int)hack->fail;
  else
    rv = (int)hack->pass;
...
#pragma unused(theEnv)
  ...
  for (; markList != NULL; markList = markList->next) {
    ...
    actualIndex +=
        (unsigned short)(markList->endPosition - markList->startPosition);
  }
...
/*****************************************************/
globle int FactStoreMultifield(void* theEnv, void* theValue,
                               DATA_OBJECT* theResult) {
#if MAC_XCD
