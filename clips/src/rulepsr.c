...
static struct defrule* ProcessRuleLHS(void*, struct lhsParseNode*, struct expr*,
                                      SYMBOL_HN*, int*);
...
/****************************************************/
globle int ParseDefrule(void* theEnv, const char* readSource) {
#if (!RUN_TIME) && (!BLOAD_ONLY)
  SYMBOL_HN* ruleName;
  ...
  ruleName = GetConstructNameAndComment(
      theEnv, readSource, &theToken, "defrule", EnvFindDefruleInModule,
      EnvUndefrule, "*", FALSE, TRUE, TRUE, FALSE);
...
/**************************************************************/
static struct defrule* ProcessRuleLHS(void* theEnv, struct lhsParseNode* theLHS,
                                      struct expr* actions, SYMBOL_HN* ruleName,
                                      int* error) {
  ...
  *error = FALSE;
  ...
  while ((theLHS != NULL) || (emptyLHS == TRUE)) {
    ...
    lastJoin =
        ConstructJoins(theEnv, logicalJoin, tempNode, 1, NULL, TRUE, TRUE);
...
/************************************************************************/
static struct defrule* CreateNewDisjunct(void* theEnv, SYMBOL_HN* ruleName,
                                         int localVarCnt,
                                         struct expr* theActions,
                                         int complexity, unsigned logicalJoin,
                                         struct joinNode* lastJoin) {
  ...
  newDisjunct->header.whichModule = (struct defmoduleItemHeader*)GetModuleItem(
      theEnv, NULL, FindModuleItem(theEnv, "defrule")->moduleIndex);
...
/*   duplicate commands.                                        */
/****************************************************************/
static int ReplaceRHSVariable(void* theEnv, struct expr* list, void* VtheLHS) {
  ...
  theVariable =
      FindVariable((SYMBOL_HN*)list->value, (struct lhsParseNode*)VtheLHS);
...
/*******************************************************/
static struct expr* ParseRuleRHS(void* theEnv, const char* readSource) {
  struct expr* actions;
  struct token theToken;
...
/************************************************************/
static int RuleComplexity(void* theEnv, struct lhsParseNode* theLHS) {
...
/********************************************************************/
static int ExpressionComplexity(void* theEnv, struct expr* exprPtr) {
  int complexity = 0;
      ...
      /*=========================================*/
        ...
        complexity++;
                   ...
                   ? EvaluationData(theEnv)
                         ->PrimitivesArray[exprPtr->type]
                         ->addsToRuleComplexity
                   : FALSE) {
      complexity++;
    }
...
/********************************************/
static int LogicalAnalysis(void* theEnv, struct lhsParseNode* patternList) {
  ...
  for (; patternList != NULL; patternList = patternList->bottom) {
    ...
    if (((patternList->type != PATTERN_CE) && (patternList->type != TEST_CE)) ||
...
globle struct lhsParseNode* FindVariable(SYMBOL_HN* name,
  ...
  for (; theLHS != NULL; theLHS = theLHS->bottom) {
    ...
    if ((theLHS->type != PATTERN_CE) || (theLHS->negated == TRUE) ||
...
/**********************************************************/
static void AddToDefruleList(struct defrule* rulePtr) {
  ...
  if (theModuleItem->header.lastItem == NULL) {
    ...
    tempRule =
        (struct defrule*)
            theModuleItem->header.lastItem;  // Note: Only the first disjunct
    tempRule->header.next =
        (struct constructHeader*)rulePtr;  // points to the next rule
  }
...
/************************************************************/
globle void DumpRuleAnalysis(void* theEnv, struct lhsParseNode* tempNode) {
  ...
  for (traceNode = tempNode; traceNode != NULL; traceNode = traceNode->bottom) {
    if (traceNode->userCE) {
      gensprintf(buffer, "UCE %2d (%2d %2d): ", traceNode->whichCE,
                 traceNode->beginNandDepth, traceNode->endNandDepth);
    } else {
      gensprintf(buffer, "SCE %2d (%2d %2d): ", traceNode->whichCE,
                 traceNode->beginNandDepth, traceNode->endNandDepth);
    }
