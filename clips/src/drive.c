...
/************************************************/
globle void NetworkAssert(void* theEnv, struct partialMatch* binds,
...
/*   the RHS of a join.                              */
/*****************************************************/
globle void NetworkAssertRight(void* theEnv, struct partialMatch* rhsBinds,
  ...
  while (lhsBinds != NULL) {
    ...
    if ((join->secondaryNetworkTest != NULL) && exprResult) {
      EngineData(theEnv)->GlobalLHSBinds = lhsBinds;
      exprResult =
          EvaluateJoinExpression(theEnv, join->secondaryNetworkTest, join);
      if (EvaluationData(theEnv)->EvaluationError) {
        SetEvaluationError(theEnv, FALSE);
      }
    }
...
/****************************************************/
globle void NetworkAssertLeft(void* theEnv, struct partialMatch* lhsBinds,
                              struct joinNode* join, int operation) {
  ...
  if ((operation == NETWORK_RETRACT) &&
      PartialMatchWillBeDeleted(theEnv, lhsBinds)) {
    return;
  }
  ...
  if (join->joinFromTheRight) {
    ...
    rhsBinds = GetAlphaMemory(
        theEnv, (struct patternNodeHeader*)join->rightSideEntryStructure,
        entryHashValue);
  }
  ...
  while (rhsBinds != NULL) {
    if ((operation == NETWORK_RETRACT) &&
        PartialMatchWillBeDeleted(theEnv, rhsBinds)) {
...
globle intBool EvaluateSecondaryNetworkTest(void* theEnv,
  ...
  joinExpr =
      EvaluateJoinExpression(theEnv, joinPtr->secondaryNetworkTest, joinPtr);
...
/* BetaMemoryHashValue:     */
/*******************************************************/
globle unsigned long BetaMemoryHashValue(void* theEnv, struct expr* hashExpr,
...
/*******************************************************************/
globle void PPDrive(void* theEnv, struct partialMatch* lhsBinds,
                    struct partialMatch* rhsBinds, struct joinNode* join,
                    int operation) {
  ...
  while (listOfJoins != NULL) {
    ...
    if (listOfJoins->enterDirection == LHS) {
      if (listOfJoins->join->leftHash != NULL) {
        hashValue = BetaMemoryHashValue(theEnv, listOfJoins->join->leftHash,
      ...
      if (listOfJoins->join->rightHash != NULL) {
        hashValue = BetaMemoryHashValue(theEnv, listOfJoins->join->rightHash,
                                        linker, NULL, listOfJoins->join);
      } else {
        hashValue = 0;
      }
    ...
    UpdateBetaPMLinks(theEnv, linker, lhsBinds, rhsBinds, listOfJoins->join,
                      hashValue, listOfJoins->enterDirection);
...
/***********************************************************************/
globle void EPMDrive(void* theEnv, struct partialMatch* parent,
                     struct joinNode* join, int operation) {
  struct partialMatch* linker;
  struct joinLink* listOfJoins;
  ...
  while (listOfJoins != NULL) {
    ...
    UpdateBetaPMLinks(theEnv, linker, parent, NULL, listOfJoins->join, 0,
...
/***************************************************************/
static void EmptyDrive(void* theEnv, struct joinNode* join,
  ...
  if (join->patternIsNegated ||
      (join->joinFromTheRight &&
       (!join->patternIsExists))) /* reorder to remove patternIsExists test */
  {
  ...
  while (listOfJoins != NULL) {
    ...
    if (join->patternIsExists) {
      UpdateBetaPMLinks(theEnv, linker, existsParent, NULL, listOfJoins->join,
                        hashValue, listOfJoins->enterDirection);
    } else {
      UpdateBetaPMLinks(theEnv, linker, NULL, rhsBinds, listOfJoins->join,
                        hashValue, listOfJoins->enterDirection);
    }
...
/*   was being evaluated.                                           */
/********************************************************************/
static void JoinNetErrorMessage(void* theEnv, struct joinNode* joinPtr) {
  PrintErrorID(theEnv, "DRIVE", 1, TRUE);
  EnvPrintRouter(theEnv, WERROR, "This error occurred in the join network\n");

  EnvPrintRouter(
      theEnv, WERROR,
      "   Problem resides in associated join\n"); /* TBD generate test case for
                                                     join with JFTR */
                                                  /*
                                                     sprintf(buffer,"   Problem resides in join #%d in
                                                     rule(s):\n",joinPtr->depth);                                                 EnvPrintRouter(theEnv,WERROR,buffer);
                                                  */
