...
globle void DeriveDefaultFromConstraints(void* theEnv,
  ...
  if (constraints == NULL) {
    if (multifield) {
      ...
      if (garbageMultifield)
        ...
        SetpValue(theDefault, (void*)CreateMultifield2(theEnv, 0L));
    } else {
  ...
  if (constraints->anyAllowed || constraints->symbolsAllowed) {
    theType = SYMBOL;
    theValue = FindDefaultValue(theEnv, SYMBOL, constraints,
                                EnvAddSymbol(theEnv, "nil"));
  }

  else if (constraints->stringsAllowed) {
    theType = STRING;
    theValue =
        FindDefaultValue(theEnv, STRING, constraints, EnvAddSymbol(theEnv, ""));
  }

  else if (constraints->integersAllowed) {
    theType = INTEGER;
    theValue =
        FindDefaultValue(theEnv, INTEGER, constraints, EnvAddLong(theEnv, 0LL));
  }

  else if (constraints->floatsAllowed) {
    theType = FLOAT;
    theValue =
        FindDefaultValue(theEnv, FLOAT, constraints, EnvAddDouble(theEnv, 0.0));
  }
#if OBJECT_SYSTEM
  else if (constraints->instanceNamesAllowed) {
    theType = INSTANCE_NAME;
    theValue = FindDefaultValue(theEnv, INSTANCE_NAME, constraints,
                                EnvAddSymbol(theEnv, "nil"));
  }
  ...
  if (multifield) {
    if (constraints->minFields == NULL)
      ...
      minFields = (unsigned long)ValueToLong(constraints->minFields->value);
    ...
    if (garbageMultifield)
      ...
      SetpValue(theDefault, (void*)CreateMultifield2(theEnv, minFields));
...
/*   maximum value.                                                    */
/************************************************************************/
static void* FindDefaultValue(void* theEnv, int theType,
  ...
  if (theType == INTEGER) {
    if (theConstraints->minValue->type == INTEGER) {
      ...
      return (EnvAddLong(
          theEnv, (long long)ValueToDouble(theConstraints->minValue->value)));
      ...
      return (EnvAddLong(
          theEnv, (long long)ValueToDouble(theConstraints->maxValue->value)));
    }
    ...
    if (theConstraints->minValue->type == FLOAT) {
      ...
      return (EnvAddDouble(
          theEnv, (double)ValueToLong(theConstraints->minValue->value)));
    } else if (theConstraints->maxValue->type == FLOAT) {
      return (theConstraints->maxValue->value);
    } else if (theConstraints->maxValue->type == INTEGER) {
      return (EnvAddDouble(
          theEnv, (double)ValueToLong(theConstraints->maxValue->value)));
    }
  }
...
/**********************************************/
globle struct expr* ParseDefault(void* theEnv, const char* readSource,
                                 int multifield, int dynamic, int evalStatic,
                                 int* noneSpecified, int* deriveSpecified,
                                 int* error) {
  ...
  *deriveSpecified = FALSE;
  ...
  while (theToken.type != RPAREN) {
    ...
    if ((newItem->type == SF_VARIABLE) || (newItem->type == MF_VARIABLE)) {
      ...
      if ((dynamic) || (newItem->type == MF_VARIABLE) ||
          (specialVarCode == -1) ||
          ((specialVarCode != -1) && (defaultList != NULL))) {
        if (dynamic)
          SyntaxErrorMessage(theEnv, "default-dynamic attribute");
        else
  ...
  if (multifield == FALSE) {
    ...
    if (*error) {
      ...
      EnvPrintRouter(theEnv, WERROR,
                     "The default value for a single field slot must be a "
                     "single field value\n");
      ReturnExpression(theEnv, defaultList);
      return (NULL);
    }
  ...
  while (newItem != NULL) {
    ...
    if ((theValue.type == MULTIFIELD) && (multifield == FALSE) &&
        (*error == FALSE)) {
      PrintErrorID(theEnv, "DEFAULT", 1, TRUE);
      EnvPrintRouter(theEnv, WERROR,
                     "The default value for a single field slot must be a "
