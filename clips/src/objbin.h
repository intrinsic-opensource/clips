...
#define ObjectBinaryData(theEnv) \
  ((struct objectBinaryData*)GetEnvironmentData(theEnv, OBJECTBIN_DATA))

#define DefclassPointer(i) \
  (((i) == -1L) ? NULL : (DEFCLASS*)&ObjectBinaryData(theEnv)->DefclassArray[i])
#define DefclassIndex(cls) \
  (((cls) == NULL) ? -1 : ((struct constructHeader*)cls)->bsaveID)
