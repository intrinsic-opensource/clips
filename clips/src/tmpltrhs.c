...
static struct expr* ParseAssertSlotValues(void*, const char*, struct token*,
                                          struct templateSlot*, int*, int);
static struct expr* ReorderAssertSlotValues(void*, struct templateSlot*,
                                            struct expr*, int*);
static struct expr* GetSlotAssertValues(void*, struct templateSlot*,
                                        struct expr*, int*);
static struct expr* FindAssertSlotItem(struct templateSlot*, struct expr*);
static struct templateSlot* ParseSlotLabel(void*, const char*, struct token*,
...
/******************************************************************/
globle struct expr* ParseAssertTemplate(void* theEnv, const char* readSource,
  ...
  while ((slotPtr = ParseSlotLabel(theEnv, readSource, theToken, theDeftemplate,
                                   error, endType)) != NULL) {
    /*========================================================*/
    ...
    nextSlot = ParseAssertSlotValues(theEnv, readSource, theToken, slotPtr,
                                     error, constantsOnly);
  ...
  firstArg = ReorderAssertSlotValues(theEnv, theDeftemplate->slotList,
                                     firstSlot, error);
  ReturnExpression(theEnv, firstSlot);
...
/****************************************************************/
static struct templateSlot* ParseSlotLabel(void* theEnv,
  ...
  *error = FALSE;
  ...
  if ((slotPtr = FindSlot(theDeftemplate, (SYMBOL_HN*)tempToken->value,
                          &position)) == NULL) {
...
/**************************************************************************/
static struct expr* ParseAssertSlotValues(void* theEnv, const char* inputSource,
                                          struct token* tempToken,
                                          struct templateSlot* slotPtr,
  ...
  if (slotPtr->multislot == FALSE) {
    ...
    newField = GetAssertArgument(theEnv, inputSource, tempToken, error, RPAREN,
  ...
  /*========================================*/
    ...
    SavePPBuffer(theEnv, " ");
    valueList = GetAssertArgument(theEnv, inputSource, tempToken, error, RPAREN,
                                  constantsOnly, &printError);
    if (*error) {
      if (printError) SyntaxErrorMessage(theEnv, "deftemplate pattern");
      return (NULL);
    ...
    while (lastValue != NULL) /* (tempToken->type != RPAREN) */
    {
      ...
      newField = GetAssertArgument(theEnv, inputSource, tempToken, error,
                                   RPAREN, constantsOnly, &printError);
      if (*error) {
        if (printError) SyntaxErrorMessage(theEnv, "deftemplate pattern");
...
/*************************************************************************/
static struct expr* ReorderAssertSlotValues(void* theEnv,
                                            struct templateSlot* slotPtr,
...
/***************************************************************/
static struct expr* GetSlotAssertValues(void* theEnv,
                                        struct templateSlot* slotPtr,
  ...
  if (slotPtr->multislot) {
    tempArg = GenConstant(theEnv, FACT_STORE_MULTIFIELD,
...
/*******************************************************************/
static struct expr* FindAssertSlotItem(struct templateSlot* slotPtr,
