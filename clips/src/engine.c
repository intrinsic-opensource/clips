...
globle void InitializeEngine(void* theEnv) {
  AllocateEnvironmentData(theEnv, ENGINE_DATA, sizeof(struct engineData),
                          DeallocateEngineData);

  EngineData(theEnv)->IncrementalResetFlag = TRUE;

#if DEBUGGING_FUNCTIONS
  AddWatchItem(theEnv, "statistics", 0, &EngineData(theEnv)->WatchStatistics,
               20, NULL, NULL);
  AddWatchItem(theEnv, "focus", 0, &EngineData(theEnv)->WatchFocus, 0, NULL,
               NULL);
#endif
}
...
globle long long EnvRun(void* theEnv, long long runLimit) {
  ...
  while ((theActivation != NULL) && (runLimit != 0) &&
    ...
    theBasis =
        (struct partialMatch*)EnvGetActivationBasis(theEnv, theActivation);
    EngineData(theEnv)->ExecutingRule =
        (struct defrule*)EnvGetActivationRule(theEnv, theActivation);
    ...
    if (EngineData(theEnv)->TheLogicalJoin != NULL) {
      EngineData(theEnv)->TheLogicalBind =
          FindLogicalBind(EngineData(theEnv)->TheLogicalJoin,
    ...
    EvaluateProcActions(
        theEnv,
        EngineData(theEnv)->ExecutingRule->header.whichModule->theModule,
        EngineData(theEnv)->ExecutingRule->actions,
        EngineData(theEnv)->ExecutingRule->localVarCnt, &result, NULL);
    ...
    for (theRunFunction = EngineData(theEnv)->ListOfRunFunctions;
         theRunFunction != NULL; theRunFunction = theRunFunction->next) {
    ...
    if (theActivation != NULL) {
      if (((struct defrule*)EnvGetActivationRule(theEnv, theActivation))
              ->afterBreakpoint) {
        EngineData(theEnv)->HaltRules = TRUE;
        EnvPrintRouter(theEnv, WDIALOG, "Breaking on rule ");
        EnvPrintRouter(theEnv, WDIALOG,
                       EnvGetActivationName(theEnv, theActivation));
        EnvPrintRouter(theEnv, WDIALOG, ".\n");
      }
    }
  }
  ...
  if (EngineData(theEnv)->WatchStatistics) {
    ...
    gensprintf(printSpace, "%ld mean number of facts (%ld maximum).\n",
               (long)(((double)sumFacts / (rulesFired + 1)) + 0.5), maxFacts);
  ...
  if (EngineData(theEnv)->CurrentFocus != NULL) {
    if (EngineData(theEnv)->CurrentFocus->theModule !=
        ((struct defmodule*)EnvGetCurrentModule(theEnv))) {
      EnvSetCurrentModule(theEnv,
                          (void*)EngineData(theEnv)->CurrentFocus->theModule);
    }
  }
...
/***********************************************************/
globle struct activation* NextActivationToFire(void* theEnv) {
  ...
  while ((theActivation == NULL) &&
         (EngineData(theEnv)->CurrentFocus != NULL)) {
    if (EngineData(theEnv)->CurrentFocus != NULL) EnvPopFocus(theEnv);
    if (EngineData(theEnv)->CurrentFocus != NULL)
      theActivation =
          EngineData(theEnv)->CurrentFocus->theDefruleModule->agenda;
  }
...
/***************************************************/
static struct defmodule* RemoveFocus(void* theEnv,
                                     struct defmodule* theModule) {
  ...
  if (EngineData(theEnv)->WatchFocus) {
    ...
    if ((EngineData(theEnv)->CurrentFocus != NULL) && currentFocusRemoved) {
      ...
      EnvPrintRouter(
          theEnv, WTRACE,
          ValueToString(EngineData(theEnv)->CurrentFocus->theModule->name));
    }
...
globle void* EnvPopFocus(void* theEnv) {
  if (EngineData(theEnv)->CurrentFocus == NULL) return (NULL);
  return (
      (void*)RemoveFocus(theEnv, EngineData(theEnv)->CurrentFocus->theModule));
}
...
/**************************************/
globle intBool EnvAddRunFunction(void* theEnv, const char* name,
...
/********************************************/
globle intBool EnvAddBeforeRunFunction(void* theEnv, const char* name,
                                       void (*functionPtr)(void*, void*),
                                       int priority) {
  EngineData(theEnv)->ListOfBeforeRunFunctions = AddFunctionToCallListWithArg(
...
/*****************************************/
globle intBool EnvAddRunFunctionWithContext(void* theEnv, const char* name,
                                            void (*functionPtr)(void*),
                                            int priority, void* context) {
  EngineData(theEnv)->ListOfRunFunctions = AddFunctionToCallListWithContext(
      theEnv, name, priority, functionPtr,
      EngineData(theEnv)->ListOfRunFunctions, TRUE, context);
  return (1);
}
...
globle intBool EnvAddBeforeRunFunctionWithContext(
    void* theEnv, const char* name, void (*functionPtr)(void*, void*),
    int priority, void* context) {
  EngineData(theEnv)->ListOfBeforeRunFunctions =
      AddFunctionToCallListWithArgWithContext(
          theEnv, name, priority, functionPtr,
          EngineData(theEnv)->ListOfBeforeRunFunctions, TRUE, context);
  return (1);
}
...
/********************************************/
globle intBool EnvRemoveRunFunction(void* theEnv, const char* name) {
  int found;

  EngineData(theEnv)->ListOfRunFunctions = RemoveFunctionFromCallList(
      theEnv, name, EngineData(theEnv)->ListOfRunFunctions, &found);
...
/**************************************************/
globle intBool EnvRemoveBeforeRunFunction(void* theEnv, const char* name) {
  int found;

  EngineData(theEnv)->ListOfBeforeRunFunctions =
      RemoveFunctionFromCallListWithArg(
          theEnv, name, EngineData(theEnv)->ListOfBeforeRunFunctions, &found);
...
/*****************************/
globle void EnvHalt(void* theEnv) { EngineData(theEnv)->HaltRules = TRUE; }
...
#pragma unused(theEnv)
  ...
  for (thePtr = (struct defrule*)theRule; thePtr != NULL;
...
/***********************************/
globle void EnvShowBreaks(void* theEnv, const char* logicalName,
                          void* vTheModule) {
  ListItemsDriver(theEnv, logicalName, (struct defmodule*)vTheModule, NULL,
                  NULL, EnvGetNextDefrule,
                  (const char* (*)(void*))GetConstructNameString, NULL,
                  EnvDefruleHasBreakpoint);
}
...
globle void RemoveBreakCommand(void* theEnv) {
  ...
  if ((nargs = EnvArgCountCheck(theEnv, "remove-break", NO_MORE_THAN, 1)) ==
      -1) {
    return;
  }
  ...
  if (EnvArgTypeCheck(theEnv, "remove-break", 1, SYMBOL, &argPtr) == FALSE)
    return;
...
globle void ShowBreaksCommand(void* theEnv) {
  ...
  if ((numArgs = EnvArgCountCheck(theEnv, "show-breaks", NO_MORE_THAN, 1)) ==
      -1)
    return;
...
/*   for the list-focus-stack command. */
/***************************************/
globle void EnvListFocusStack(void* theEnv, const char* logicalName) {
  struct focus* theFocus;

  for (theFocus = EngineData(theEnv)->CurrentFocus; theFocus != NULL;
       theFocus = theFocus->next) {
    EnvPrintRouter(theEnv, logicalName,
                   EnvGetDefmoduleName(theEnv, theFocus->theModule));
    EnvPrintRouter(theEnv, logicalName, "\n");
  }
}
...
globle void EnvGetFocusStack(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  for (theFocus = EngineData(theEnv)->CurrentFocus, count = 1; theFocus != NULL;
...
globle intBool AddBeforeRunFunction(const char* name,
                                    void (*functionPtr)(void*), int priority) {
  void* theEnv;
...
globle intBool AddRunFunction(const char* name, void (*functionPtr)(void),
                              int priority) {
  void* theEnv;

  theEnv = GetCurrentEnvironment();

  EngineData(theEnv)->ListOfRunFunctions = AddFunctionToCallList(
      theEnv, name, priority, (void (*)(void*))functionPtr,
      EngineData(theEnv)->ListOfRunFunctions, TRUE);
  return (1);
}

globle void ClearFocusStack() { EnvClearFocusStack(GetCurrentEnvironment()); }

globle void Focus(void* vTheModule) {
  EnvFocus(GetCurrentEnvironment(), vTheModule);
}

globle void GetFocusStack(DATA_OBJECT_PTR returnValue) {
  EnvGetFocusStack(GetCurrentEnvironment(), returnValue);
}

globle void* GetFocus() { return EnvGetFocus(GetCurrentEnvironment()); }

globle int GetFocusChanged() {
  return EnvGetFocusChanged(GetCurrentEnvironment());
}

globle void* GetNextFocus(void* theFocus) {
  return EnvGetNextFocus(GetCurrentEnvironment(), theFocus);
}

globle void Halt() { EnvHalt(GetCurrentEnvironment()); }

globle void* PopFocus() { return EnvPopFocus(GetCurrentEnvironment()); }

globle intBool RemoveRunFunction(const char* name) {
...
globle void SetFocusChanged(int value) {
  EnvSetFocusChanged(GetCurrentEnvironment(), value);
}

#if DEBUGGING_FUNCTIONS

globle void ListFocusStack(const char* logicalName) {
  EnvListFocusStack(GetCurrentEnvironment(), logicalName);
}

globle intBool DefruleHasBreakpoint(void* theRule) {
  return EnvDefruleHasBreakpoint(GetCurrentEnvironment(), theRule);
}

globle intBool RemoveBreak(void* theRule) {
  return EnvRemoveBreak(GetCurrentEnvironment(), theRule);
}

globle void SetBreak(void* theRule) {
  EnvSetBreak(GetCurrentEnvironment(), theRule);
}

globle void ShowBreaks(const char* logicalName, void* vTheModule) {
