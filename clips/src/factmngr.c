...
globle void InitializeFacts(void* theEnv) {
  struct patternEntityRecord factInfo = {
      {"FACT_ADDRESS", FACT_ADDRESS, 1, 0, 0, PrintFactIdentifier,
       PrintFactIdentifierInLongForm, EnvRetract, NULL, EnvGetNextFact,
       EnvDecrementFactCount, EnvIncrementFactCount, NULL, NULL, NULL, NULL,
       NULL},
  ...
  struct fact dummyFact = {
      {NULL, NULL, 0, 0L},        NULL, NULL, -1L, 0, 1, NULL, NULL, NULL, NULL,
      {1, 0UL, NULL, {{0, NULL}}}};

  AllocateEnvironmentData(theEnv, FACTS_DATA, sizeof(struct factsData),
                          DeallocateFactData);
...
/***********************************/
static void DeallocateFactData(void* theEnv) {
...
/**********************************************/
globle void PrintFactWithIdentifier(void* theEnv, const char* logicalName,
                                    struct fact* factPtr) {
  char printSpace[20];
...
/****************************************************/
globle void PrintFactIdentifier(void* theEnv, const char* logicalName,
                                void* factPtr) {
  char printSpace[20];
...
/********************************************/
globle void PrintFactIdentifierInLongForm(void* theEnv, const char* logicalName,
                                          void* factPtr) {
...
globle void IncrementFactBasisCount(void* theEnv, void* vFactPtr) {
  ...
  for (i = 0; i < (int)theSegment->multifieldLength; i++) {
    AtomInstall(theEnv, theSegment->theFields[i].type,
                theSegment->theFields[i].value);
  }
}
...
/* FactIsDeleted: */
/******************/
globle intBool FactIsDeleted(void* theEnv, void* theFact) {
#if MAC_XCD
...
/**************************************************/
globle void PrintFact(void* theEnv, const char* logicalName,
                      struct fact* factPtr, int seperateLines,
                      int ignoreDefaults) {
  ...
  if (factPtr->whichDeftemplate->implied == FALSE) {
    PrintTemplateFact(theEnv, logicalName, factPtr, seperateLines,
                      ignoreDefaults);
    return;
  }
  ...
  theMultifield =
      (struct multifield*)factPtr->theProposition.theFields[0].value;
  if (theMultifield->multifieldLength != 0) {
    EnvPrintRouter(theEnv, logicalName, " ");
    PrintMultifield(theEnv, logicalName, theMultifield, 0,
                    (long)(theMultifield->multifieldLength - 1), FALSE);
  }
...
globle void MatchFactFunction(void* theEnv, void* vTheFact) {
  ...
  FactPatternMatch(theEnv, theFact, theFact->whichDeftemplate->patternNetwork,
                   0, NULL, NULL);
}
...
globle intBool EnvRetract(void* theEnv, void* vTheFact) {
  ...
  if (EngineData(theEnv)->JoinOperationInProgress) {
    ...
    EnvPrintRouter(theEnv, WERROR,
                   "Facts may not be retracted during pattern-matching\n");
    return (FALSE);
  }
...
/*******************************************************************/
static void RemoveGarbageFacts(void* theEnv) {
...
globle void* EnvAssert(void* theEnv, void* vTheFact) {
  ...
  if (EngineData(theEnv)->JoinOperationInProgress) {
    ...
    EnvPrintRouter(theEnv, WERROR,
                   "Facts may not be asserted during pattern-matching\n");
    return (NULL);
  }
  ...
  if (AddLogicalDependencies(theEnv, (struct patternEntity*)theFact, FALSE) ==
      FALSE) {
    ReturnFact(theEnv, theFact);
    return (NULL);
  }
  ...
  for (theAssertFunction = FactData(theEnv)->ListOfAssertFunctions;
       theAssertFunction != NULL; theAssertFunction = theAssertFunction->next) {
...
/************************************************/
globle struct fact* EnvCreateFact(void* theEnv, void* vTheDeftemplate) {
  ...
  if (theDeftemplate->implied == FALSE) {
    ...
    for (i = 0; i < (int)theDeftemplate->numberOfSlots; i++) {
...
/******************************************/
globle intBool EnvGetFactSlot(void* theEnv, void* vTheFact,
                              const char* slotName, DATA_OBJECT* theValue) {
  struct fact* theFact = (struct fact*)vTheFact;
  ...
  if (theDeftemplate->implied) {
    ...
    SetpDOEnd(theValue,
              ((struct multifield*)theValue->value)->multifieldLength);
    return (TRUE);
  }
  ...
  if (slotName == NULL) return FALSE;
  if (FindSlot(theDeftemplate, (SYMBOL_HN*)EnvAddSymbol(theEnv, slotName),
               &whichSlot) == NULL) {
    return (FALSE);
  }
...
/***************************************/
globle intBool EnvPutFactSlot(void* theEnv, void* vTheFact,
                              const char* slotName, DATA_OBJECT* theValue) {
  struct fact* theFact = (struct fact*)vTheFact;
  struct deftemplate* theDeftemplate;
  ...
  if ((theSlot =
           FindSlot(theDeftemplate, (SYMBOL_HN*)EnvAddSymbol(theEnv, slotName),
                    &whichSlot)) == NULL) {
    return (FALSE);
  }
  ...
  if ((theSlot->constraints != NULL) &&
    ...
    if (ConstraintCheckValue(theEnv, theValue->type, theValue->value,
  ...
  if (theFact->theProposition.theFields[whichSlot - 1].type == MULTIFIELD) {
    ReturnMultifield(theEnv, (struct multifield*)theFact->theProposition
                                 .theFields[whichSlot - 1]
                                 .value);
  }
...
globle intBool EnvAssignFactSlotDefaults(void* theEnv, void* vTheFact) {
  ...
  for (i = 0, slotPtr = theDeftemplate->slotList;
       i < (int)theDeftemplate->numberOfSlots; i++, slotPtr = slotPtr->next) {
    /*===================================*/
    ...
    if (DeftemplateSlotDefault(theEnv, theDeftemplate, slotPtr, &theResult,
                               FALSE)) {
...
globle intBool DeftemplateSlotDefault(void* theEnv,
  ...
  /*==============================================*/
    ...
    if (slotPtr->multislot) {
      StoreInMultifield(theEnv, theResult, slotPtr->defaultList,
                        garbageMultifield);
    } else {
...
/***************************************************************/
globle intBool CopyFactSlotValues(void* theEnv, void* vTheDestFact,
                                  void* vTheSourceFact) {
  ...
  for (i = 0, slotPtr = theDeftemplate->slotList;
    ...
    if (theSourceFact->theProposition.theFields[i].type != MULTIFIELD) {
      ...
      theDestFact->theProposition.theFields[i].value = CopyMultifield(
          theEnv,
          (struct multifield*)theSourceFact->theProposition.theFields[i].value);
    }
  }
...
/*********************************************/
globle struct fact* CreateFactBySize(void* theEnv, unsigned size) {
  struct fact* theFact;
  unsigned newSize;
...
globle void ReturnFact(void* theEnv, struct fact* theFact) {
  ...
  if (theFact->theProposition.multifieldLength == 0)
    newSize = 1;
  else
    newSize = theFact->theProposition.multifieldLength;
...
/****************************************/
globle void EnvGetFactPPForm(void* theEnv, char* buffer, size_t bufferLength,
                             void* theFact) {
...
/***********************************************************/
static void ResetFacts(void* theEnv) {
  /*====================================*/
...
/************************************************************/
static int ClearFactsReady(void* theEnv) {
  /*======================================*/
...
/***************************************************/
globle struct fact* FindIndexedFact(void* theEnv, long long factIndexSought) {
  struct fact* theFact;

  for (theFact = (struct fact*)EnvGetNextFact(theEnv, NULL); theFact != NULL;
...
/*****************************************/
globle intBool EnvAddAssertFunction(void* theEnv, const char* name,
                                    void (*functionPtr)(void*, void*),
                                    int priority) {
  FactData(theEnv)->ListOfAssertFunctions = AddFunctionToCallListWithArg(
      theEnv, name, priority, functionPtr,
...
/********************************************/
globle intBool EnvAddAssertFunctionWithContext(void* theEnv, const char* name,
                                               void (*functionPtr)(void*,
                                                                   void*),
                                               int priority, void* context) {
  FactData(theEnv)->ListOfAssertFunctions =
      AddFunctionToCallListWithArgWithContext(
          theEnv, name, priority, functionPtr,
          FactData(theEnv)->ListOfAssertFunctions, TRUE, context);
  return (1);
}
...
/***********************************************/
globle intBool EnvRemoveAssertFunction(void* theEnv, const char* name) {
  int found;

  FactData(theEnv)->ListOfAssertFunctions = RemoveFunctionFromCallListWithArg(
      theEnv, name, FactData(theEnv)->ListOfAssertFunctions, &found);
...
/******************************************/
globle intBool EnvAddRetractFunction(void* theEnv, const char* name,
                                     void (*functionPtr)(void*, void*),
                                     int priority) {
  FactData(theEnv)->ListOfRetractFunctions = AddFunctionToCallListWithArg(
      theEnv, name, priority, functionPtr,
...
/*********************************************/
globle intBool EnvAddRetractFunctionWithContext(void* theEnv, const char* name,
                                                void (*functionPtr)(void*,
                                                                    void*),
                                                int priority, void* context) {
  FactData(theEnv)->ListOfRetractFunctions =
      AddFunctionToCallListWithArgWithContext(
          theEnv, name, priority, functionPtr,
          FactData(theEnv)->ListOfRetractFunctions, TRUE, context);
  return (1);
}
...
/************************************************/
globle intBool EnvRemoveRetractFunction(void* theEnv, const char* name) {
  int found;

  FactData(theEnv)->ListOfRetractFunctions = RemoveFunctionFromCallListWithArg(
      theEnv, name, FactData(theEnv)->ListOfRetractFunctions, &found);
...
/*****************************************/
globle intBool EnvAddModifyFunction(void* theEnv, const char* name,
                                    void (*functionPtr)(void*, void*, void*),
                                    int priority) {
  FactData(theEnv)->ListOfModifyFunctions = AddFunctionToCallListWithArg(
      theEnv, name, priority, (void (*)(void*, void*))functionPtr,
...
/********************************************/
globle intBool EnvAddModifyFunctionWithContext(void* theEnv, const char* name,
                                               void (*functionPtr)(void*, void*,
                                                                   void*),
                                               int priority, void* context) {
  FactData(theEnv)->ListOfModifyFunctions =
      AddFunctionToCallListWithArgWithContext(
          theEnv, name, priority, (void (*)(void*, void*))functionPtr,
          FactData(theEnv)->ListOfModifyFunctions, TRUE, context);
  return (1);
}
...
/***********************************************/
globle intBool EnvRemoveModifyFunction(void* theEnv, const char* name) {
  int found;

  FactData(theEnv)->ListOfModifyFunctions = RemoveFunctionFromCallListWithArg(
      theEnv, name, FactData(theEnv)->ListOfModifyFunctions, &found);
...
globle intBool AddAssertFunction(const char* name,
                                 void (*functionPtr)(void*, void*),
                                 int priority) {
  void* theEnv;
...
globle intBool AddModifyFunction(const char* name,
                                 void (*functionPtr)(void*, void*, void*),
                                 int priority) {
  void* theEnv;
...
globle intBool AddRetractFunction(const char* name,
                                  void (*functionPtr)(void*, void*),
                                  int priority) {
  void* theEnv;
...
globle void GetFactPPForm(char* buffer, unsigned bufferLength, void* theFact) {
  EnvGetFactPPForm(GetCurrentEnvironment(), buffer, bufferLength, theFact);
}

globle intBool GetFactSlot(void* vTheFact, const char* slotName,
                           DATA_OBJECT* theValue) {
  return (
      EnvGetFactSlot(GetCurrentEnvironment(), vTheFact, slotName, theValue));
}

globle void* GetNextFact(void* factPtr) {
  return EnvGetNextFact(GetCurrentEnvironment(), factPtr);
}

globle void IncrementFactCount(void* factPtr) {
  EnvIncrementFactCount(GetCurrentEnvironment(), factPtr);
}

globle intBool PutFactSlot(void* vTheFact, const char* slotName,
                           DATA_OBJECT* theValue) {
  return EnvPutFactSlot(GetCurrentEnvironment(), vTheFact, slotName, theValue);
}

globle intBool RemoveAssertFunction(const char* name) {
  return EnvRemoveAssertFunction(GetCurrentEnvironment(), name);
}

globle intBool RemoveModifyFunction(const char* name) {
  return EnvRemoveModifyFunction(GetCurrentEnvironment(), name);
}

globle intBool RemoveRetractFunction(const char* name) {
  return EnvRemoveRetractFunction(GetCurrentEnvironment(), name);
}

globle intBool Retract(void* vTheFact) {
  return EnvRetract(GetCurrentEnvironment(), vTheFact);
}

globle void SetFactListChanged(int value) {
