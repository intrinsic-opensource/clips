...
/* =========================================
   *****************************************
   ...
   =========================================
   ***************************************** */
#define ObjectPNPrefix() \
  ArbitraryPrefix(ObjectReteData(theEnv)->ObjectPatternCodeItem, 0)
#define ObjectANPrefix() \
  ArbitraryPrefix(ObjectReteData(theEnv)->ObjectPatternCodeItem, 1)
#define ObjectALPrefix() \
  ArbitraryPrefix(ObjectReteData(theEnv)->ObjectPatternCodeItem, 2)
...
static int ObjectPatternsToCode(void*, const char*, const char*, char*, int,
                                FILE*, int, int);
static void IntermediatePatternNodeReference(void*, OBJECT_PATTERN_NODE*, FILE*,
                                             int, int);
static int IntermediatePatternNodesToCode(void*, const char*, const char*,
                                          char*, int, FILE*, int, int, int);
static int AlphaPatternNodesToCode(void*, const char*, const char*, char*, int,
                                   FILE*, int, int, int);
static int ClassAlphaLinksToCode(void*, const char*, const char*, char*, int,
                                 FILE*, int, int, int);
static CLASS_ALPHA_LINK* GetNextAlphaLink(void*, struct defmodule**,
                                          DEFCLASS** theClass,
                                          CLASS_ALPHA_LINK*);
...
globle void ObjectPatternsCompilerSetup(void* theEnv) {
  ObjectReteData(theEnv)->ObjectPatternCodeItem = AddCodeGeneratorItem(
      theEnv, "object-patterns", 0, BeforeObjectPatternsToCode,
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void ObjectPatternNodeReference(void* theEnv, void* theVPattern,
                                       FILE* theFile, int imageID,
                                       int maxIndices) {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void ClassAlphaLinkReference(void* theEnv, void* theVLink, FILE* theFile,
  ...
  if (theVLink == NULL)
    ...
    fprintf(theFile, "&%s%d_%d[%d]", ObjectALPrefix(), imageID,
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void InitObjectPatternsCode(void* theEnv, FILE* initFP, int imageID,
                                   int maxIndices) {
  ...
  if (ObjectNetworkPointer(theEnv) != NULL) {
    ...
    fprintf(initFP,
            "   SetObjectNetworkTerminalPointer(theEnv,&%s%d_%d[%d]);\n",
            ObjectANPrefix(), imageID, (int)((firstAlphaNode / maxIndices) + 1),
...
/***********************************************************
  ...
  NOTES        : None
 ***********************************************************/
static int ObjectPatternsToCode(void* theEnv, const char* fileName,
  ...
  version = IntermediatePatternNodesToCode(theEnv, fileName, pathName,
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void IntermediatePatternNodeReference(void* theEnv,
                                             OBJECT_PATTERN_NODE* thePattern,
                                             FILE* theFile, int imageID,
                                             int maxIndices) {
  if (thePattern == NULL)
    ...
    fprintf(theFile, "&%s%d_%d[%d]", ObjectPNPrefix(), imageID,
            (((int)thePattern->bsaveID) / maxIndices) + 1,
            ((int)thePattern->bsaveID) % maxIndices);
  }
}
...
/*************************************************************
  ...
  NOTES        : None
 *************************************************************/
static int IntermediatePatternNodesToCode(void* theEnv, const char* fileName,
                                          const char* pathName,
                                          char* fileNameBuffer, int fileID,
                                          FILE* headerFP, int imageID,
                                          int maxIndices, int version) {
  FILE* fp;
  int arrayVersion;
  int newHeader;
  int i;
  ...
  /* ================
     Create the file.
     ================ */
  if (ObjectNetworkPointer(theEnv) == NULL) return (1);
  ...
  /* =================================
     ...
     ================================= */
  if ((fp = NewCFile(theEnv, fileName, pathName, fileNameBuffer, fileID,
                     version, FALSE)) == NULL)
    return (0);
  newHeader = TRUE;
  ...
  while (thePattern != NULL) {
    ...
    fprintf(fp, "{0,%u,%u,%u,%u,%u,0L,%u,", thePattern->multifieldNode,
            thePattern->endSlot, thePattern->selector, thePattern->whichField,
            thePattern->leaveFields, thePattern->slotNameID);

    PrintHashedExpressionReference(theEnv, fp, thePattern->networkTest, imageID,
                                   maxIndices);
    fprintf(fp, ",");
    IntermediatePatternNodeReference(theEnv, thePattern->nextLevel, fp, imageID,
                                     maxIndices);
    fprintf(fp, ",");
    IntermediatePatternNodeReference(theEnv, thePattern->lastLevel, fp, imageID,
                                     maxIndices);
    fprintf(fp, ",");
    IntermediatePatternNodeReference(theEnv, thePattern->leftNode, fp, imageID,
                                     maxIndices);
    fprintf(fp, ",");
    IntermediatePatternNodeReference(theEnv, thePattern->rightNode, fp, imageID,
                                     maxIndices);
    fprintf(fp, ",");
    ObjectPatternNodeReference(theEnv, (void*)thePattern->alphaNode, fp,
                               imageID, maxIndices);
    fprintf(fp, ",0L}");
...
/***********************************************************
  ...
  NOTES        : None
 ***********************************************************/
static int AlphaPatternNodesToCode(void* theEnv, const char* fileName,
                                   const char* pathName, char* fileNameBuffer,
                                   int fileID, FILE* headerFP, int imageID,
                                   int maxIndices, int version) {
  FILE* fp;
  ...
  /* ================
     Create the file.
     ================ */
  if (ObjectNetworkTerminalPointer(theEnv) == NULL) return (version);
  ...
  while (thePattern != NULL) {
    ...
    PatternNodeHeaderToCode(theEnv, fp, &thePattern->header, imageID,
                            maxIndices);
    ...
    fprintf(fp, ",");
    IntermediatePatternNodeReference(theEnv, thePattern->patternNode, fp,
                                     imageID, maxIndices);
    fprintf(fp, ",");
    ObjectPatternNodeReference(theEnv, thePattern->nxtInGroup, fp, imageID,
                               maxIndices);
    fprintf(fp, ",");
    ObjectPatternNodeReference(theEnv, thePattern->nxtTerminal, fp, imageID,
                               maxIndices);
    fprintf(fp, ",0L}");
...
/***********************************************************
  ...
  NOTES        : None
 ***********************************************************/
static int ClassAlphaLinksToCode(void* theEnv, const char* fileName,
                                 const char* pathName, char* fileNameBuffer,
                                 int fileID, FILE* headerFP, int imageID,
                                 int maxIndices, int version) {
  FILE* fp;
  ...
  while (theLink != NULL) {
    ...
    ObjectPatternNodeReference(theEnv, theLink->alphaNode, fp, imageID,
                               maxIndices);
...
/* GetNextAlphaLink */
/********************/
static CLASS_ALPHA_LINK* GetNextAlphaLink(void* theEnv,
                                          struct defmodule** theModule,
                                          DEFCLASS** theClass,
                                          CLASS_ALPHA_LINK* theLink) {
  while (TRUE) {
