...
/********************************************************************/
globle unsigned FindModuleSeparator(const char* theString) {
  const char* sep;
...
/*   cannot be extracted).                                         */
/*******************************************************************/
globle SYMBOL_HN* ExtractModuleName(void* theEnv, unsigned thePosition,
  ...
  genstrncpy(newString, theString, (STD_SIZE)thePosition - 1);
...
/********************************************************************/
globle SYMBOL_HN* ExtractConstructName(void* theEnv, unsigned thePosition,
                                       const char* theString) {
  size_t theLength;
  char* newString;
  SYMBOL_HN* returnValue;
...
/****************************************************/
globle const char* ExtractModuleAndConstructName(void* theEnv,
                                                 const char* theName) {
...
/************************************************************/
globle void* FindImportedConstruct(void* theEnv, const char* constructName,
                                   struct defmodule* matchModule,
                                   const char* findName, int* count,
                                   int searchCurrent,
  ...
  *count = 0;
  ...
  rv = SearchImportedConstructModules(
      theEnv, (SYMBOL_HN*)EnvAddSymbol(theEnv, constructName), matchModule,
      theModuleItem, (SYMBOL_HN*)EnvAddSymbol(theEnv, findName), count,
...
static void* SearchImportedConstructModules(
    void* theEnv, struct symbolHashNode* constructType,
    struct defmodule* matchModule, struct moduleItem* theModuleItem,
    struct symbolHashNode* findName, int* count, int searchCurrent,
    struct defmodule* notYetDefinedInModule) {
  struct defmodule* theModule;
  ...
  if ((searchCurrent) &&
    ...
    /*=========================================================*/
      ...
      if (((struct constructHeader*)rv)->whichModule->theModule ==
          currentModule) {
        (*count)++;
      }
      arv = rv;
  ...
  while (theImportList != NULL) {
    ...
    if (searchModule) {
      theModule = (struct defmodule*)EnvFindDefmodule(
          theEnv, ValueToString(theImportList->moduleName));
    ...
    if (searchModule) {
      EnvSetCurrentModule(theEnv, (void*)theModule);
      if ((rv = SearchImportedConstructModules(
               theEnv, constructType, matchModule, theModuleItem, findName,
               count, TRUE, notYetDefinedInModule)) != NULL) {
        arv = rv;
      }
    }
...
/**************************************************************/
globle intBool ConstructExported(void* theEnv, const char* constructTypeStr,
...
/*********************************************************/
globle intBool AllImportedModulesVisited(void* theEnv,
                                         struct defmodule* theModule) {
  ...
  theImportList = theModule->importList;
  while (theImportList != NULL) {
    theImportModule = (struct defmodule*)EnvFindDefmodule(
        theEnv, ValueToString(theImportList->moduleName));
...
/***************************************/
globle void ListItemsDriver(void* theEnv, const char* logicalName,
                            struct defmodule* theModule, const char* singleName,
  ...
  while (theModule != NULL) {
    ...
    while (constructPtr != NULL) {
      ...
      if (doItFunction == NULL)
        ...
        doIt = (*doItFunction)(theEnv, constructPtr);
  ...
  if (singleName != NULL)
    PrintTally(theEnv, logicalName, count, singleName, pluralName);
