...
static PARTIAL_ORDER* InitializePartialOrderTable(void*, PARTIAL_ORDER*,
                                                  PACKED_CLASS_LINKS*);
static void RecordPartialOrders(void*, PARTIAL_ORDER*, DEFCLASS*,
                                PACKED_CLASS_LINKS*, long);
...
globle PACKED_CLASS_LINKS* ParseSuperclasses(void* theEnv,
  ...
  while (GetType(DefclassData(theEnv)->ObjectParseToken) != RPAREN) {
    ...
    if (GetValue(DefclassData(theEnv)->ObjectParseToken) ==
        (void*)newClassName) {
    ...
    sclass = LookupDefclassInScope(
        theEnv,
        ValueToString(GetValue(DefclassData(theEnv)->ObjectParseToken)));
    if (sclass == NULL) {
...
/***************************************************************************
  ...
  NOTES        : WARNING!! - This routine assumes that there are no
                 ...
                 Donald Knuth.
 ***************************************************************************/
globle PACKED_CLASS_LINKS* FindPrecedenceList(void* theEnv, DEFCLASS* cls,
  ...
  /* =============================================================================
     ...
     of zero (given that there were no loops), it is first in the precedence
     list.
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void PackClassLinks(void* theEnv, PACKED_CLASS_LINKS* plinks,
                           CLASS_LINK* lptop) {
  unsigned count;
  CLASS_LINK* lp;
...
/**************************************************************************
  ...
  NOTES        : None
 **************************************************************************/
static PARTIAL_ORDER* InitializePartialOrderTable(void* theEnv,
  ...
  for (i = 0; i < supers->classCount; i++) {
    ...
    for (pop = po_table; pop != NULL; pop = pop->nxt) {
      if (pop->cls == supers->classArray[i]) break;
      poprv = pop;
    }
    if (pop == NULL) {
      pop = get_struct(theEnv, partialOrder);
      ...
      /* =============================================================
         ...
         ============================================================= */
      po_table = InitializePartialOrderTable(
          theEnv, po_table, &supers->classArray[i]->directSuperclasses);
    }
  }
  return (po_table);
}
...
/***********************************************************************************
  ...
  DESCRIPTION  : Given a predecessor class and a list of successor classes, this
                 function enters a number of partial orders into the table
 equaling the number of successor classes. INPUTS       : 1) The partial order
 table 2) The predecessor class 3) An array of successor classes 4) A starting
 index for the successor classes RETURNS      : The top of sequence table SIDE
 EFFECTS : The sequence table is built, e.g.:
  ...
  NOTES        : None
 ***********************************************************************************/
static void RecordPartialOrders(void* theEnv, PARTIAL_ORDER* po_table,
                                DEFCLASS* cls, PACKED_CLASS_LINKS* successors,
                                long starti) {
  PARTIAL_ORDER* clspo;
  SUCCESSOR* stmp;
...
static void PrintPartialOrderLoop(void* theEnv, PARTIAL_ORDER* po_table) {
  ...
  /* ====================================================
     ...
     ==================================================== */
  for (pop1 = po_table; pop1 != NULL; pop1 = pop1->nxt) pop1->pre = 0;
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void PrintClassLinks(void* theEnv, const char* logicalName,
                            const char* title, CLASS_LINK* clink) {
  if (title != NULL) EnvPrintRouter(theEnv, logicalName, title);
  while (clink != NULL) {
