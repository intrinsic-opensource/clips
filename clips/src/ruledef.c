...
globle void InitializeDefrules(void* theEnv) {
  unsigned long i;
  AllocateEnvironmentData(theEnv, DEFRULE_DATA, sizeof(struct defruleData),
  ...
  DefruleData(theEnv)->DefruleConstruct =
      AddConstruct(theEnv, "defrule", "defrules", ParseDefrule, EnvFindDefrule,
                   GetConstructNamePointer, GetConstructPPForm,
                   GetConstructModuleItem, EnvGetNextDefrule, SetNextConstruct,
                   EnvIsDefruleDeletable, EnvUndefrule, ReturnDefrule);

  DefruleData(theEnv)->AlphaMemoryTable = (ALPHA_MEMORY_HASH**)gm3(
      theEnv, sizeof(ALPHA_MEMORY_HASH*) * ALPHA_MEMORY_HASH_SIZE);
...
/**************************************************/
static void DeallocateDefruleData(void* theEnv) {
...
/********************************************************/
static void DestroyDefruleAction(void* theEnv,
...
/*****************************************************/
static void InitializeDefruleModules(void* theEnv) {
  DefruleData(theEnv)->DefruleModuleIndex =
      RegisterModuleItem(theEnv, "defrule", AllocateModule, ReturnModule,
...
globle struct defruleModule* GetDefruleModuleItem(void* theEnv,
  ...
  return ((struct defruleModule*)GetConstructModuleItemByIndex(
      theEnv, theModule, DefruleData(theEnv)->DefruleModuleIndex));
}
...
/*******************************************************************/
globle void* EnvFindDefrule(void* theEnv, const char* defruleName) {
  return (FindNamedConstructInModuleOrImports(
      theEnv, defruleName, DefruleData(theEnv)->DefruleConstruct));
}
...
/*   Returns a pointer to the defrule if found, otherwise NULL.    */
/*******************************************************************/
globle void* EnvFindDefruleInModule(void* theEnv, const char* defruleName) {
  return (FindNamedConstructInModule(theEnv, defruleName,
                                     DefruleData(theEnv)->DefruleConstruct));
}
...
globle void* EnvGetNextDefrule(void* theEnv, void* defrulePtr) {
  return ((void*)GetNextConstructItem(theEnv,
                                      (struct constructHeader*)defrulePtr,
                                      DefruleData(theEnv)->DefruleModuleIndex));
}
...
globle intBool EnvIsDefruleDeletable(void* theEnv, void* vTheDefrule) {
  ...
  for (theDefrule = (struct defrule*)vTheDefrule; theDefrule != NULL;
...
/***********************************************************/
globle long EnvGetDisjunctCount(void* theEnv, void* vTheDefrule) {
  struct defrule* theDefrule;
  long count = 0;
...
/*   to N - 1.                                            */
/**********************************************************/
globle void* EnvGetNthDisjunct(void* theEnv, void* vTheDefrule, long index) {
  struct defrule* theDefrule;
  long count = 0;
...
/******************************************/
globle void DefruleRunTimeInitialize(void* theEnv, struct joinLink* rightPrime,
  ...
  for (theModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    ...
    for (theRule = (struct defrule*)EnvGetNextDefrule(theEnv, NULL);
      ...
      for (theDisjunct = theRule; theDisjunct != NULL;
...
/******************************************/
static void AddBetaMemoriesToRule(void* theEnv, struct joinNode* theNode) {
  ...
  if (theNode->joinFromTheRight) {
    AddBetaMemoriesToRule(theEnv,
                          (struct joinNode*)theNode->rightSideEntryStructure);
  }
}
...
globle void AddBetaMemoriesToJoin(void* theEnv, struct joinNode* theNode) {
  ...
  if ((!theNode->firstJoin) || theNode->patternIsExists ||
    ...
    if (theNode->leftHash == NULL) {
      ...
      theNode->leftMemory = get_struct(theEnv, betaMemory);
      theNode->leftMemory->beta = (struct partialMatch**)genalloc(
  ...
  if (theNode->joinFromTheRight) {
    if (theNode->leftHash == NULL) {
      ...
      theNode->rightMemory = get_struct(theEnv, betaMemory);
      theNode->rightMemory->beta = (struct partialMatch**)genalloc(
          theEnv, sizeof(struct partialMatch*) * INITIAL_BETA_HASH_SIZE);
      theNode->rightMemory->last = (struct partialMatch**)genalloc(
          theEnv, sizeof(struct partialMatch*) * INITIAL_BETA_HASH_SIZE);
...
globle const char* EnvDefruleModule(void* theEnv, void* theDefrule) {
  return GetConstructModuleName((struct constructHeader*)theDefrule);
}

globle const char* EnvGetDefruleName(void* theEnv, void* theDefrule) {
  return GetConstructNameString((struct constructHeader*)theDefrule);
}

globle const char* EnvGetDefrulePPForm(void* theEnv, void* theDefrule) {
