...
globle void InitializeDeftemplates(void* theEnv) {
  globle struct entityRecord deftemplatePtrRecord = {
      "DEFTEMPLATE_PTR",
      DEFTEMPLATE_PTR,
      1,
      0,
      ...
      NULL,
      NULL};
  AllocateEnvironmentData(theEnv, DEFTEMPLATE_DATA,
                          sizeof(struct deftemplateData),
                          DeallocateDeftemplateData);

  memcpy(&DeftemplateData(theEnv)->DeftemplatePtrRecord, &deftemplatePtrRecord,
  ...
  DeftemplateData(theEnv)->DeftemplateConstruct = AddConstruct(
      theEnv, "deftemplate", "deftemplates", ParseDeftemplate,
      EnvFindDeftemplate, GetConstructNamePointer, GetConstructPPForm,
  ...
  InstallPrimitive(
      theEnv, (ENTITY_RECORD_PTR)&DeftemplateData(theEnv)->DeftemplatePtrRecord,
      DEFTEMPLATE_PTR);
}
...
/******************************************************/
static void DeallocateDeftemplateData(void* theEnv) {
#if !RUN_TIME
  ...
  DoForAllConstructs(theEnv, DestroyDeftemplateAction,
                     DeftemplateData(theEnv)->DeftemplateModuleIndex, FALSE,
                     NULL);
...
/*****************************************************/
static void DestroyDeftemplateAction(void* theEnv,
...
/*************************************************************/
static void InitializeDeftemplateModules(void* theEnv) {
  DeftemplateData(theEnv)->DeftemplateModuleIndex =
      RegisterModuleItem(theEnv, "deftemplate", AllocateModule, ReturnModule,
...
globle struct deftemplateModule* GetDeftemplateModuleItem(
  ...
  return ((struct deftemplateModule*)GetConstructModuleItemByIndex(
      theEnv, theModule, DeftemplateData(theEnv)->DeftemplateModuleIndex));
}
...
/*****************************************************/
globle void* EnvFindDeftemplate(void* theEnv, const char* deftemplateName) {
  return (FindNamedConstructInModuleOrImports(
      theEnv, deftemplateName, DeftemplateData(theEnv)->DeftemplateConstruct));
}
...
/*   the deftemplate if  found, otherwise NULL.      */
/*****************************************************/
globle void* EnvFindDeftemplateInModule(void* theEnv,
...
globle void* EnvGetNextDeftemplate(void* theEnv, void* deftemplatePtr) {
  return ((void*)GetNextConstructItem(
      theEnv, (struct constructHeader*)deftemplatePtr,
      DeftemplateData(theEnv)->DeftemplateModuleIndex));
}
...
/**************************************************************/
static void ReturnDeftemplate(void* theEnv, void* vTheConstruct) {
...
/*******************************************************************/
globle void* EnvGetNextFactInTemplate(void* theEnv, void* theTemplate,
                                      void* factPtr) {
#if MAC_XCD
#pragma unused(theEnv)
#endif
...
globle void* CreateDeftemplateScopeMap(void* theEnv,
  ...
  scopeMapSize =
      (sizeof(char) * ((GetNumberOfDefmodules(theEnv) / BITS_PER_BYTE) + 1));
  ...
  for (theModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    ...
    if (FindImportedConstruct(theEnv, "deftemplate", matchModule, templateName,
...
/**************************************************/
static void RuntimeDeftemplateAction(void* theEnv,
                                     struct constructHeader* theConstruct,
                                     void* buffer) {
#if MAC_XCD
#pragma unused(buffer)
...
/*******************************************************************/
static void SearchForHashedPatternNodes(void* theEnv,
                                        struct factPatternNode* theNode) {
  while (theNode != NULL) {
    if ((theNode->lastLevel != NULL) && (theNode->lastLevel->header.selector)) {
      AddHashedPatternNode(theEnv, theNode->lastLevel, theNode,
...
/*******************************************************************/
globle void DeftemplateRunTimeInitialize(void* theEnv) {
  DoForAllConstructs(theEnv, RuntimeDeftemplateAction,
                     DeftemplateData(theEnv)->DeftemplateModuleIndex, TRUE,
                     NULL);
}
...
globle const char* EnvDeftemplateModule(void* theEnv, void* theDeftemplate) {
  return GetConstructModuleName((struct constructHeader*)theDeftemplate);
}

globle const char* EnvGetDeftemplateName(void* theEnv, void* theDeftemplate) {
  return GetConstructNameString((struct constructHeader*)theDeftemplate);
}

globle const char* EnvGetDeftemplatePPForm(void* theEnv, void* theDeftemplate) {
...
globle void* FindDeftemplate(const char* deftemplateName) {
...
globle const char* GetDeftemplatePPForm(void* theDeftemplate) {
  return EnvGetDeftemplatePPForm(GetCurrentEnvironment(), theDeftemplate);
}

globle void* GetNextDeftemplate(void* deftemplatePtr) {
  return EnvGetNextDeftemplate(GetCurrentEnvironment(), deftemplatePtr);
}

globle intBool IsDeftemplateDeletable(void* vTheDeftemplate) {
  return EnvIsDeftemplateDeletable(GetCurrentEnvironment(), vTheDeftemplate);
}

globle void* GetNextFactInTemplate(void* theTemplate, void* factPtr) {
  return EnvGetNextFactInTemplate(GetCurrentEnvironment(), theTemplate,
                                  factPtr);
}
