...
globle void InitializeDeffacts(void* theEnv) {
  AllocateEnvironmentData(theEnv, DEFFACTS_DATA, sizeof(struct deffactsData),
  ...
  DeffactsData(theEnv)->DeffactsConstruct =
      AddConstruct(theEnv, "deffacts", "deffacts", ParseDeffacts,
                   EnvFindDeffacts, GetConstructNamePointer, GetConstructPPForm,
...
/***************************************************/
static void DeallocateDeffactsData(void* theEnv) {
#if !RUN_TIME
  ...
  DoForAllConstructs(theEnv, DestroyDeffactsAction,
                     DeffactsData(theEnv)->DeffactsModuleIndex, FALSE, NULL);
...
/*********************************************************/
static void DestroyDeffactsAction(void* theEnv,
...
/*******************************************************/
static void InitializeDeffactsModules(void* theEnv) {
  DeffactsData(theEnv)->DeffactsModuleIndex =
      RegisterModuleItem(theEnv, "deffacts", AllocateModule, ReturnModule,
...
globle struct deffactsModule* GetDeffactsModuleItem(
  ...
  return ((struct deffactsModule*)GetConstructModuleItemByIndex(
      theEnv, theModule, DeffactsData(theEnv)->DeffactsModuleIndex));
}
...
/**************************************************/
globle void* EnvFindDeffacts(void* theEnv, const char* deffactsName) {
  return (FindNamedConstructInModuleOrImports(
      theEnv, deffactsName, DeffactsData(theEnv)->DeffactsConstruct));
}
...
/*   deffact if found, otherwise NULL.            */
/**************************************************/
globle void* EnvFindDeffactsInModule(void* theEnv, const char* deffactsName) {
  return (FindNamedConstructInModule(theEnv, deffactsName,
                                     DeffactsData(theEnv)->DeffactsConstruct));
}
...
globle void* EnvGetNextDeffacts(void* theEnv, void* deffactsPtr) {
  return (
      (void*)GetNextConstructItem(theEnv, (struct constructHeader*)deffactsPtr,
                                  DeffactsData(theEnv)->DeffactsModuleIndex));
}
...
/***********************************************************/
static void ReturnDeffacts(void* theEnv, void* vTheDeffacts) {
#if (!BLOAD_ONLY) && (!RUN_TIME)
...
globle const char* EnvDeffactsModule(void* theEnv, void* theDeffacts) {
  return GetConstructModuleName((struct constructHeader*)theDeffacts);
}

globle const char* EnvGetDeffactsName(void* theEnv, void* theDeffacts) {
  return GetConstructNameString((struct constructHeader*)theDeffacts);
}

globle const char* EnvGetDeffactsPPForm(void* theEnv, void* theDeffacts) {
