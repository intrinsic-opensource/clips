...
#define SystemDependentData(theEnv)                        \
  ((struct systemDependentData*)GetEnvironmentData(theEnv, \
                                                   SYSTEM_DEPENDENT_DATA))
...
/********************************************************/
static void InitializeSystemDependentData(void* theEnv) {
  AllocateEnvironmentData(theEnv, SYSTEM_DEPENDENT_DATA,
...
globle void EnvInitializeEnvironment(
    void* vtheEnvironment, struct symbolHashNode** symbolTable,
  ...
  struct environmentData* theEnvironment =
      (struct environmentData*)vtheEnvironment;
  ...
  InitializeAtomTables(theEnvironment, symbolTable, floatTable, integerTable,
                       bitmapTable, externalAddressTable);
...
/**************************************************/
static void SystemFunctionDefinitions(void* theEnv) {
...
globle void gensystem(void* theEnv) {
  ...
  for (i = 1; i <= numa; i++) {
    ...
    if ((GetType(tempValue) != STRING) && (GetType(tempValue) != SYMBOL)) {
    ...
    commandBuffer = AppendToString(theEnv, theString, commandBuffer,
...
#if VAX_VMS
  if (SystemDependentData(theEnv)->PauseEnvFunction != NULL)
    (*SystemDependentData(theEnv)->PauseEnvFunction)(theEnv);
  VMSSystem(commandBuffer);
  putchar('\n');
  if (SystemDependentData(theEnv)->ContinueEnvFunction != NULL)
    (*SystemDependentData(theEnv)->ContinueEnvFunction)(theEnv, 1);
  if (SystemDependentData(theEnv)->RedrawScreenFunction != NULL)
    (*SystemDependentData(theEnv)->RedrawScreenFunction)(theEnv);
#endif
...
/*************************************************/
globle void VMSSystem(char* cmd) {
...
globle int gengetchar(void* theEnv) {
  /*
     ...
     return
  SystemDependentData(theEnv)->getcBuffer[SystemDependentData(theEnv)->getcPosition++];
  #else
  */
  return (getc(stdin));
  /*
...
/****************************************************/
globle void genprintfile(void* theEnv, FILE* fptr, const char* str) {
  if (fptr != stdout) {
    fprintf(fptr, "%s", str);
    fflush(fptr);
  } else {
    ...
    /*
          ...
          rv =
       MultiByteToWideChar(CP_UTF8,MB_ERR_INVALID_CHARS,str,-1,wbuffer,len+1);
...
/***********************************************************/
static void InitializeNonportableFeatures(void* theEnv) {
#if MAC_XCD
...
/**********************************************/
static void CatchCtrlC(int sgnl) {
#if ALLOW_ENVIRONMENT_GLOBALS
...
/******************************************/
char* genstrcpy(char* dest, const char* src) { return strcpy(dest, src); }
...
/******************************************/
char* genstrcat(char* dest, const char* src) { return strcat(dest, src); }
...
/******************************************************/
int genrand() { return (rand()); }

/**********************************************************************/
/* genseed: Generic function for seeding the random number generator. */
/**********************************************************************/
globle void genseed(int seed) { srand((unsigned)seed); }
...
/****************************************************/
globle int genremove(const char* fileName) {
...
/****************************************************/
globle int genrename(const char* oldFileName, const char* newFileName) {
...
/*********************************************/
globle FILE* GenOpen(void* theEnv, const char* fileName,
...
/************************************************************/
globle int GenOpenReadBinary(void* theEnv, const char* funcName,
                             const char* fileName) {
  if (SystemDependentData(theEnv)->BeforeOpenFunction != NULL) {
    (*SystemDependentData(theEnv)->BeforeOpenFunction)(theEnv);
  }
...
/*********************************************/
static void InitializeKeywords(void* theEnv) {
