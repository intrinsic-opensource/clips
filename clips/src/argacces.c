...
static void ExpectedTypeError3(void*, const char*, const char*, int,
                               const char*);
...
/********************************************************************/
globle DATA_OBJECT_PTR EnvRtnUnknown(void* theEnv, int argumentPosition,
...
/************************************************************************/
globle int EnvArgCountCheck(void* theEnv, const char* functionName,
                            int countRelation, int expectedNumber) {
...
/****************************************************************/
globle int EnvArgRangeCheck(void* theEnv, const char* functionName, int min,
                            int max) {
  int numberOfArguments;
...
/*************************************************************/
globle int EnvArgTypeCheck(void* theEnv, const char* functionName,
                           int argumentPosition, int expectedType,
                           DATA_OBJECT_PTR returnValue) {
  /*========================*/
  ...
  if ((returnValue->type == INTEGER) && (expectedType == FLOAT)) {
    ...
    returnValue->value =
        (void*)EnvAddDouble(theEnv, (double)ValueToLong(returnValue->value));
    return (TRUE);
  }
  ...
  if ((returnValue->type == FLOAT) && (expectedType == INTEGER)) {
    ...
    returnValue->value =
        (void*)EnvAddLong(theEnv, (long long)ValueToDouble(returnValue->value));
    return (TRUE);
  }
  ...
  if (expectedType == FLOAT)
    ExpectedTypeError1(theEnv, functionName, argumentPosition, "float");
  else if (expectedType == INTEGER)
    ExpectedTypeError1(theEnv, functionName, argumentPosition, "integer");
  else if (expectedType == SYMBOL)
    ExpectedTypeError1(theEnv, functionName, argumentPosition, "symbol");
  else if (expectedType == STRING)
    ExpectedTypeError1(theEnv, functionName, argumentPosition, "string");
  else if (expectedType == MULTIFIELD)
    ExpectedTypeError1(theEnv, functionName, argumentPosition, "multifield");
  else if (expectedType == INTEGER_OR_FLOAT)
    ExpectedTypeError1(theEnv, functionName, argumentPosition,
                       "integer or float");
  else if (expectedType == SYMBOL_OR_STRING)
    ExpectedTypeError1(theEnv, functionName, argumentPosition,
                       "symbol or string");
  else if (expectedType == FACT_ADDRESS)
    ExpectedTypeError1(theEnv, functionName, argumentPosition, "fact address");
#if OBJECT_SYSTEM
  else if (expectedType == INSTANCE_NAME)
    ExpectedTypeError1(theEnv, functionName, argumentPosition, "instance name");
...
/******************************************************************/
globle intBool GetNumericArgument(void* theEnv, struct expr* theArgument,
                                  const char* functionName, DATA_OBJECT* result,
                                  intBool convertToFloat, int whichArgument) {
...
globle const char* GetLogicalName(void* theEnv, int whichArgument,
  ...
  if ((GetType(result) == SYMBOL) || (GetType(result) == STRING) ||
    ...
    logicalName = ValueToString(
        EnvAddSymbol(theEnv, FloatToString(theEnv, DOToDouble(result))));
  } else if (GetType(result) == INTEGER) {
    logicalName = ValueToString(
        EnvAddSymbol(theEnv, LongIntegerToString(theEnv, DOToLong(result))));
  } else {
    logicalName = NULL;
  }
...
/******************************************************************/
globle void OpenErrorMessage(void* theEnv, const char* functionName,
                             const char* fileName) {
...
/**************************************************************************/
static void NonexistantError(void* theEnv, const char* accessFunction,
                             const char* functionName, int argumentPosition) {
...
/*********************************************************/
globle void ExpectedCountError(void* theEnv, const char* functionName,
                               int countRelation, int expectedNumber) {
...
/*************************************************************/
globle intBool CheckFunctionArgCount(void* theEnv, const char* functionName,
...
/*******************************************************************/
globle void ExpectedTypeError1(void* theEnv, const char* functionName,
                               int whichArg, const char* expectedType) {
...
/**************************************************************/
globle void ExpectedTypeError2(void* theEnv, const char* functionName,
                               int whichArg) {
...
/*   RtnDouble.                                                    */
/*******************************************************************/
static void ExpectedTypeError3(void* theEnv, const char* accessFunction,
                               const char* functionName, int argumentPosition,
                               const char* type) {
...
/***************************************************/
void* GetFactOrInstanceArgument(void* theEnv, int thePosition,
                                DATA_OBJECT* item, const char* functionName) {
  ...
  if (GetpType(item) == INSTANCE_ADDRESS) {
    if (((struct instance*)GetpValue(item))->garbage) {
      CantFindItemErrorMessage(
          theEnv, "instance",
          ((struct instance*)GetpValue(item))->name->contents);
      return NULL;
    }
  ...
  /*================================================*/
    ...
    if ((ptr = (void*)FindInstanceBySymbol(
             theEnv, (SYMBOL_HN*)GetpValue(item))) == NULL) {
      CantFindItemErrorMessage(theEnv, "instance",
                               ValueToString(GetpValue(item)));
    }
    return (ptr);
  }
#endif
...
/****************************************************/
void IllegalLogicalNameMessage(void* theEnv, const char* theFunction) {
...
globle int ArgCountCheck(const char* functionName, int countRelation,
                         int expectedNumber) {
  return EnvArgCountCheck(GetCurrentEnvironment(), functionName, countRelation,
                          expectedNumber);
}

globle int ArgRangeCheck(const char* functionName, int min, int max) {
  return EnvArgRangeCheck(GetCurrentEnvironment(), functionName, min, max);
}

globle int ArgTypeCheck(const char* functionName, int argumentPosition,
                        int expectedType, DATA_OBJECT_PTR returnValue) {
  return EnvArgTypeCheck(GetCurrentEnvironment(), functionName,
                         argumentPosition, expectedType, returnValue);
}

globle int RtnArgCount() { return EnvRtnArgCount(GetCurrentEnvironment()); }

globle double RtnDouble(int argumentPosition) {
  return EnvRtnDouble(GetCurrentEnvironment(), argumentPosition);
}

globle const char* RtnLexeme(int argumentPosition) {
  return EnvRtnLexeme(GetCurrentEnvironment(), argumentPosition);
}

globle long long RtnLong(int argumentPosition) {
  return EnvRtnLong(GetCurrentEnvironment(), argumentPosition);
}

globle DATA_OBJECT_PTR RtnUnknown(int argumentPosition,
                                  DATA_OBJECT_PTR returnValue) {
