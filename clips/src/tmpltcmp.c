...
#define SlotPrefix() \
  ArbitraryPrefix(DeftemplateData(theEnv)->DeftemplateCodeItem, 2)
...
static void DeftemplateModuleToCode(void*, FILE*, struct defmodule*, int, int,
                                    int);
static void DeftemplateToCode(void*, FILE*, struct deftemplate*, int, int, int,
                              int);
...
globle void DeftemplateCompilerSetup(void* theEnv) {
  DeftemplateData(theEnv)->DeftemplateCodeItem = AddCodeGeneratorItem(
      theEnv, "deftemplate", 0, NULL, InitDeftemplateCode, ConstructToCode, 3);
}
...
static int ConstructToCode(void* theEnv, const char* fileName,
  ...
  while (theModule != NULL) {
    ...
    moduleFile = OpenFileIfNeeded(
        ...
        &fileCount, moduleArrayVersion, headerFP, "struct deftemplateModule",
        ModulePrefix(DeftemplateData(theEnv)->DeftemplateCodeItem), FALSE,
        NULL);
    ...
    DeftemplateModuleToCode(theEnv, moduleFile, theModule, imageID, maxIndices,
                            moduleCount);
    ...
    while (theTemplate != NULL) {
      templateFile = OpenFileIfNeeded(
          theEnv, templateFile, fileName, pathName, fileNameBuffer, fileID,
          imageID, &fileCount, templateArrayVersion, headerFP,
          "struct deftemplate",
          ConstructPrefix(DeftemplateData(theEnv)->DeftemplateCodeItem), FALSE,
          NULL);
      ...
      templateArrayCount++;
      templateFile =
          CloseFileIfNeeded(theEnv, templateFile, &templateArrayCount,
                            &templateArrayVersion, maxIndices, NULL, NULL);
      ...
      while (slotPtr != NULL) {
        slotFile = OpenFileIfNeeded(
            theEnv, slotFile, fileName, pathName, fileNameBuffer, fileID,
            imageID, &fileCount, slotArrayVersion, headerFP,
        ...
        slotFile = CloseFileIfNeeded(theEnv, slotFile, &slotArrayCount,
                                     &slotArrayVersion, maxIndices, NULL, NULL);
        slotPtr = slotPtr->next;
      }

      theTemplate =
          (struct deftemplate*)EnvGetNextDeftemplate(theEnv, theTemplate);
    }
...
/************************************************************/
static void CloseDeftemplateFiles(void* theEnv, FILE* moduleFile,
                                  FILE* templateFile, FILE* slotFile,
                                  int maxIndices) {
  ...
  if (slotFile != NULL) {
    count = maxIndices;
    CloseFileIfNeeded(theEnv, slotFile, &count, &arrayVersion, maxIndices, NULL,
                      NULL);
  }

  if (templateFile != NULL) {
    count = maxIndices;
    CloseFileIfNeeded(theEnv, templateFile, &count, &arrayVersion, maxIndices,
                      NULL, NULL);
  }
...
/*************************************************************/
static void DeftemplateModuleToCode(void* theEnv, FILE* theFile,
...
/************************************************************/
static void DeftemplateToCode(void* theEnv, FILE* theFile,
                              struct deftemplate* theTemplate, int imageID,
                              int maxIndices, int moduleCount, int slotCount) {
  /*====================*/
  /* Deftemplate Header */
  /*====================*/
  ...
  ConstructHeaderToCode(
      theEnv, theFile, &theTemplate->header, imageID, maxIndices, moduleCount,
  ...
  if (theTemplate->slotList == NULL) {
    ...
    fprintf(theFile, "&%s%d_%d[%d],", SlotPrefix(), imageID,
            (slotCount / maxIndices) + 1, slotCount % maxIndices);
  }
  ...
  if (theTemplate->patternNetwork == NULL) {
    ...
    FactPatternNodeReference(theEnv, theTemplate->patternNetwork, theFile,
                             imageID, maxIndices);
  }
...
/*****************************************************/
static void SlotToCode(void* theEnv, FILE* theFile,
                       struct templateSlot* theSlot, int imageID,
                       int maxIndices, int slotCount) {
  /*===========*/
  /* Slot Name */
  /*===========*/
  ...
  PrintConstraintReference(theEnv, theFile, theSlot->constraints, imageID,
                           maxIndices);
  ...
  fprintf(theFile, ",");
  PrintHashedExpressionReference(theEnv, theFile, theSlot->defaultList, imageID,
                                 maxIndices);
  ...
  fprintf(theFile, ",");
  PrintHashedExpressionReference(theEnv, theFile, theSlot->facetList, imageID,
                                 maxIndices);
  fprintf(theFile, ",");
  ...
  if (theSlot->next == NULL) {
    ...
    fprintf(theFile, "&%s%d_%d[%d]}", SlotPrefix(), imageID,
            ((slotCount + 1) / maxIndices) + 1, (slotCount + 1) % maxIndices);
  }
}
...
/*****************************************************************/
globle void DeftemplateCModuleReference(void* theEnv, FILE* theFile, int count,
                                        int imageID, int maxIndices) {
  fprintf(theFile, "MIHS &%s%d_%d[%d]",
          ModulePrefix(DeftemplateData(theEnv)->DeftemplateCodeItem), imageID,
...
/********************************************************************/
globle void DeftemplateCConstructReference(void* theEnv, FILE* theFile,
                                           void* vTheTemplate, int imageID,
                                           int maxIndices) {
  ...
  if (theTemplate == NULL) {
    ...
    fprintf(theFile, "&%s%d_%ld[%ld]",
            ConstructPrefix(DeftemplateData(theEnv)->DeftemplateCodeItem),
            imageID, (theTemplate->header.bsaveID / maxIndices) + 1,
...
/*******************************************/
static void InitDeftemplateCode(void* theEnv, FILE* initFP, int imageID,
                                int maxIndices) {
#if MAC_XCD
