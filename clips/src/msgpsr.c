...
static SLOT_DESC* CheckSlotReference(void*, DEFCLASS*, int, void*, intBool,
                                     EXPRESSION*);
static void GenHandlerSlotReference(void*, EXPRESSION*, unsigned short,
                                    SLOT_DESC*);
...
/***********************************************************************
  ...
  NOTES        : H/L Syntax:
                 ...
                 <params> ::= <var>* | <var>* $?<name>
 ***********************************************************************/
globle int ParseDefmessageHandler(void* theEnv, const char* readSource) {
  DEFCLASS* cls;
  ...
  cname = GetConstructNameAndComment(
      theEnv, readSource, &DefclassData(theEnv)->ObjectParseToken,
      "defmessage-handler", NULL, NULL, "~", TRUE, FALSE, TRUE, FALSE);
  ...
  if (GetType(DefclassData(theEnv)->ObjectParseToken) != LPAREN) {
    ...
    if (GetType(DefclassData(theEnv)->ObjectParseToken) != STRING) {
      ...
      mtype = HandlerType(theEnv, "defmessage-handler",
                          DOToString(DefclassData(theEnv)->ObjectParseToken));
      if (mtype == MERROR) return (TRUE);
  ...
  hndParams = GenConstant(theEnv, SYMBOL,
                          (void*)MessageHandlerData(theEnv)->SELF_SYMBOL);
  hndParams = ParseProcParameters(
      theEnv, readSource, &DefclassData(theEnv)->ObjectParseToken, hndParams,
      &wildcard, &min, &max, &error, IsParameterSlotReference);
  if (error) return (TRUE);
  PPCRAndIndent(theEnv);
  ExpressionData(theEnv)->ReturnContext = TRUE;
  actions = ParseProcActions(theEnv, "message-handler", readSource,
                             &DefclassData(theEnv)->ObjectParseToken, hndParams,
                             wildcard, SlotReferenceVar, BindSlotReference,
                             &lvars, (void*)cls);
  if (actions == NULL) {
...
globle void CreateGetAndPutHandlers(void* theEnv, SLOT_DESC* sd) {
  ...
  if ((sd->createReadAccessor == 0) && (sd->createWriteAccessor == 0)) return;
  ...
  if (sd->createWriteAccessor) {
    gensprintf(buf, "%s put-%s ($?value) (bind ?self:%s ?value))", className,
               slotName, slotName);
...
/*****************************************************************
  ...
  NOTES        : None
 *****************************************************************/
static intBool IsParameterSlotReference(void* theEnv, const char* pname) {
  if ((strncmp(pname, SELF_STRING, SELF_LEN) == 0)
          ? (pname[SELF_LEN] == SELF_SLOT_REF)
          : FALSE) {
 ...
 ****************************************************************************/
static int SlotReferenceVar(void* theEnv, EXPRESSION* varexp,
                            void* userBuffer) {
  struct token itkn;
  int oldpp;
  SLOT_DESC* sd;
  ...
  if ((strncmp(ValueToString(varexp->value), SELF_STRING, SELF_LEN) == 0)
          ? (ValueToString(varexp->value)[SELF_LEN] == SELF_SLOT_REF)
          : FALSE) {
    OpenStringSource(theEnv, "hnd-var",
                     ValueToString(varexp->value) + SELF_LEN + 1, 0);
    ...
    if (itkn.type != STOP) {
      sd = CheckSlotReference(theEnv, (DEFCLASS*)userBuffer, itkn.type,
                              itkn.value, FALSE, NULL);
      if (sd == NULL) return (-1);
...
/****************************************************************************
  ...
  NOTES        : Objects are allowed to directly access their own slots
                 ...
                 violate the encapsulation principle of OOP.
 ****************************************************************************/
static int BindSlotReference(void* theEnv, EXPRESSION* bindExp,
                             void* userBuffer) {
  const char* bindName;
  struct token itkn;
  int oldpp;
  ...
  if ((strncmp(bindName, SELF_STRING, SELF_LEN) == 0)
          ? (bindName[SELF_LEN] == SELF_SLOT_REF)
          : FALSE) {
 ...
 *********************************************************/
static SLOT_DESC* CheckSlotReference(void* theEnv, DEFCLASS* theDefclass,
                                     int theType, void* theValue,
                                     intBool writeFlag,
  ...
  slotIndex =
      FindInstanceTemplateSlot(theEnv, theDefclass, (SYMBOL_HN*)theValue);
  if (slotIndex == -1) {
  ...
  if (sd->noWrite && (sd->initializeOnly == 0)) {
    SlotAccessViolationError(theEnv, ValueToString(theValue), FALSE,
                             (void*)theDefclass);
    return (NULL);
  }
  ...
  if (EnvGetStaticConstraintChecking(theEnv)) {
    vCode =
        ConstraintCheckExpressionChain(theEnv, writeExpression, sd->constraint);
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void GenHandlerSlotReference(void* theEnv, EXPRESSION* theExp,
                                    unsigned short theType, SLOT_DESC* sd) {
