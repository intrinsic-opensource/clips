...
static INSTANCE_TYPE* InstanceLocationInfo(void*, DEFCLASS*, SYMBOL_HN*,
                                           INSTANCE_TYPE**, unsigned*);
...
globle void MakeInstanceCommand(void* theEnv, DATA_OBJECT* result) {
  ...
  if ((GetType(temp) != SYMBOL) && (GetType(temp) != INSTANCE_NAME)) {
  ...
  if (GetFirstArgument()->nextArg->type == DEFCLASS_PTR)
    ...
    cls = LookupDefclassByMdlOrScope(
        theEnv, DOToString(temp));  // Module or scope is now allowed

    if (cls == NULL) {
      ClassExistError(theEnv,
  ...
  if (CoreInitializeInstance(theEnv, ins,
                             GetFirstArgument()->nextArg->nextArg) == TRUE) {
...
globle SYMBOL_HN* GetFullInstanceName(void* theEnv, INSTANCE_TYPE* ins) {
  ...
  */
  ...
  /*
     if (ins->garbage)
       return(ins->name);
     if (ins->cls->header.whichModule->theModule == ((struct defmodule *)
     EnvGetCurrentModule(theEnv))) return(ins->name); moduleName =
     EnvGetDefmoduleName(theEnv,(void *)
     ins->cls->header.whichModule->theModule); bufsz = (sizeof(char) *
     (strlen(moduleName) + strlen(ValueToString(ins->name)) + 3)); buffer =
     (char *) gm2(theEnv,bufsz);
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle INSTANCE_TYPE* BuildInstance(void* theEnv, SYMBOL_HN* iname,
                                    DEFCLASS* cls, intBool initMessage) {
  ...
  if (modulePosition) {
    moduleName =
        ExtractModuleName(theEnv, modulePosition, ValueToString(iname));
    if ((moduleName == NULL) ||
  ...
  if (ins != NULL) {
    ...
    if (ins->garbage == 0) {
      if (InstanceData(theEnv)->MkInsMsgPass)
        DirectMessage(theEnv, MessageHandlerData(theEnv)->DELETE_SYMBOL, ins,
                      NULL, NULL);
      else
        QuashInstance(theEnv, ins);
    }
  ...
  /* ==============================================
     ...
     ============================================== */
  if (AddLogicalDependencies(
          theEnv, (struct patternEntity*)InstanceData(theEnv)->CurrentInstance,
          FALSE) == FALSE) {
  ...
  if (InstanceData(theEnv)->CurrentInstance->cls->instanceList == NULL)
    ...
    InstanceData(theEnv)->CurrentInstance->cls->instanceListBottom->nxtClass =
        InstanceData(theEnv)->CurrentInstance;
  ...
  /* ==============================================================================
     Install the instance's name and slot-value symbols (prevent them from
     becoming ephemeral) - the class name and slot names are accounted for by
     the class
     ==============================================================================
   */
...
globle void InitSlotsCommand(void* theEnv, DATA_OBJECT* result) {
  ...
  if (CheckCurrentMessage(theEnv, "init-slots", TRUE) == FALSE) return;
...
globle intBool QuashInstance(void* theEnv, INSTANCE_TYPE* ins) {
...
#if DEBUGGING_FUNCTIONS
  if (ins->cls->traceInstances) PrintInstanceWatch(theEnv, UNMAKE_TRACE, ins);
#endif
  ...
  if (ins->nxtHash != NULL) ins->nxtHash->prvHash = ins->prvHash;
...
/*****************************************************************
  ...
  NOTES        : Instance names only have to be unique within
                 ...
                 of module.
 *****************************************************************/
static INSTANCE_TYPE* InstanceLocationInfo(void* theEnv, DEFCLASS* cls,
                                           SYMBOL_HN* iname,
                                           INSTANCE_TYPE** prv,
                                           unsigned* hashTableIndex) {
  INSTANCE_TYPE* ins;
...
static void InstallInstance(void* theEnv, INSTANCE_TYPE* ins, int set) {
  if (set == TRUE) {
    if (ins->installed) return;
#if DEBUGGING_FUNCTIONS
    if (ins->cls->traceInstances) PrintInstanceWatch(theEnv, MAKE_TRACE, ins);
#endif
    ins->installed = 1;
 ...
 ****************************************************************/
static void BuildDefaultSlots(void* theEnv, intBool initMessage) {
  unsigned i, j;
  unsigned scnt;
  unsigned lscnt;
...
static int InsertSlotOverrides(void* theEnv, INSTANCE_TYPE* ins,
  ...
  while (slot_exp != NULL) {
    ...
    if (InstanceData(theEnv)->MkInsMsgPass) {
      DirectMessage(theEnv, slot->desc->overrideMessage, ins, NULL,
...
static void EvaluateClassDefaults(void* theEnv, INSTANCE_TYPE* ins) {
  ...
  for (i = 0; i < ins->cls->instanceSlotCount; i++) {
    ...
    if (!slot->override) {
      ...
      } else if (((slot->desc->shared == 0) ||
                  (slot->desc->sharedCount == 1)) &&
                 (slot->desc->noDefault == 0))
        DirectPutSlotValue(theEnv, ins, slot,
                           (DATA_OBJECT*)slot->desc->defaultValue, &junk);
      else if (slot->valueRequired) {
        PrintErrorID(theEnv, "INSMNGR", 14, FALSE);
        EnvPrintRouter(theEnv, WERROR, "Override required for slot ");
        EnvPrintRouter(theEnv, WERROR,
                       ValueToString(slot->desc->slotName->name));
      ...
      if (EvaluationData(theEnv)->EvaluationError) return;
    }
    slot->override = FALSE;
  }
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void PrintInstanceWatch(void* theEnv, const char* traceString,
