...
#if !RUN_TIME
  EnvDefineFunction2(theEnv, "get-deffacts-list", 'm',
                     PTIEF GetDeffactsListFunction, "GetDeffactsListFunction",
                     "01w");
  EnvDefineFunction2(theEnv, "undeffacts", 'v', PTIEF UndeffactsCommand,
                     "UndeffactsCommand", "11w");
  EnvDefineFunction2(theEnv, "deffacts-module", 'w',
                     PTIEF DeffactsModuleFunction, "DeffactsModuleFunction",
                     "11w");

#if DEBUGGING_FUNCTIONS
  EnvDefineFunction2(theEnv, "list-deffacts", 'v', PTIEF ListDeffactsCommand,
                     "ListDeffactsCommand", "01w");
  EnvDefineFunction2(theEnv, "ppdeffacts", 'v', PTIEF PPDeffactsCommand,
                     "PPDeffactsCommand", "11w");
#endif
...
/*   deffacts constructs.                                 */
/**********************************************************/
static void ResetDeffacts(void* theEnv) {
  DoForAllConstructs(theEnv, ResetDeffactsAction,
                     DeffactsData(theEnv)->DeffactsModuleIndex, TRUE, NULL);
}
...
/*****************************************************/
static void ResetDeffactsAction(void* theEnv,
...
/**********************************************************/
static void ClearDeffacts(void* theEnv) {
  ...
  stub->argList =
      GenConstant(theEnv, DEFTEMPLATE_PTR,
                  EnvFindDeftemplateInModule(theEnv, "initial-fact"));
...
/***************************************/
static void SaveDeffacts(void* theEnv, void* theModule,
...
globle void UndeffactsCommand(void* theEnv) {
  UndefconstructCommand(theEnv, "undeffacts",
                        DeffactsData(theEnv)->DeffactsConstruct);
}
...
globle intBool EnvUndeffacts(void* theEnv, void* theDeffacts) {
  return (Undefconstruct(theEnv, theDeffacts,
                         DeffactsData(theEnv)->DeffactsConstruct));
}
...
globle void GetDeffactsListFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  GetConstructListFunction(theEnv, "get-deffacts-list", returnValue,
                           DeffactsData(theEnv)->DeffactsConstruct);
}
...
/*   for the get-deffacts-list function. */
/*****************************************/
globle void EnvGetDeffactsList(void* theEnv, DATA_OBJECT_PTR returnValue,
                               void* theModule) {
  GetConstructList(theEnv, returnValue, DeffactsData(theEnv)->DeffactsConstruct,
...
globle void* DeffactsModuleFunction(void* theEnv) {
  return (GetConstructModuleCommand(theEnv, "deffacts-module",
                                    DeffactsData(theEnv)->DeffactsConstruct));
}
...
globle void PPDeffactsCommand(void* theEnv) {
  PPConstructCommand(theEnv, "ppdeffacts",
                     DeffactsData(theEnv)->DeffactsConstruct);
}
...
/************************************/
globle int PPDeffacts(void* theEnv, const char* deffactsName,
                      const char* logicalName) {
  return (PPConstruct(theEnv, deffactsName, logicalName,
                      DeffactsData(theEnv)->DeffactsConstruct));
}
...
globle void ListDeffactsCommand(void* theEnv) {
  ListConstructCommand(theEnv, "list-deffacts",
                       DeffactsData(theEnv)->DeffactsConstruct);
}
...
/*   for the list-deffacts command.  */
/*************************************/
globle void EnvListDeffacts(void* theEnv, const char* logicalName,
                            void* theModule) {
  ListConstruct(theEnv, DeffactsData(theEnv)->DeffactsConstruct, logicalName,
                (struct defmodule*)theModule);
}
...
globle void GetDeffactsList(DATA_OBJECT_PTR returnValue, void* theModule) {
  EnvGetDeffactsList(GetCurrentEnvironment(), returnValue, theModule);
}

globle intBool Undeffacts(void* theDeffacts) {
  return EnvUndeffacts(GetCurrentEnvironment(), theDeffacts);
}

#if DEBUGGING_FUNCTIONS

globle void ListDeffacts(const char* logicalName, void* theModule) {
