...
#if !RUN_TIME
  EnvDefineFunction2(theEnv, "fact-existp", 'b', PTIEF FactExistpFunction,
                     "FactExistpFunction", "11z");
  EnvDefineFunction2(theEnv, "fact-relation", 'w', PTIEF FactRelationFunction,
                     "FactRelationFunction", "11z");
  EnvDefineFunction2(theEnv, "fact-slot-value", 'u',
                     PTIEF FactSlotValueFunction, "FactSlotValueFunction",
                     "22*zw");
  EnvDefineFunction2(theEnv, "fact-slot-names", 'u',
                     PTIEF FactSlotNamesFunction, "FactSlotNamesFunction",
                     "11z");
  EnvDefineFunction2(theEnv, "get-fact-list", 'm', PTIEF GetFactListFunction,
                     "GetFactListFunction", "01w");
  EnvDefineFunction2(theEnv, "ppfact", 'v', PTIEF PPFactFunction,
                     "PPFactFunction", "13*z");
#else
#if MAC_XCD
...
globle void* FactRelationFunction(void* theEnv) {
  ...
  if (EnvArgCountCheck(theEnv, "fact-relation", EXACTLY, 1) == -1)
...
globle void FactSlotValueFunction(void* theEnv, DATA_OBJECT* returnValue) {
  ...
  if (EnvArgTypeCheck(theEnv, "fact-slot-value", 2, SYMBOL, &theValue) ==
      FALSE) {
    return;
  }
...
/***************************************/
globle void FactSlotValue(void* theEnv, void* vTheFact, const char* theSlotName,
  ...
  if (theFact->whichDeftemplate->implied) {
    if (strcmp(theSlotName, "implied") != 0) {
      ...
      return;
                    ...
                    (SYMBOL_HN*)EnvAddSymbol(theEnv, theSlotName),
                    &position) == NULL) {
    SetEvaluationError(theEnv, TRUE);
    InvalidDeftemplateSlotMessage(
        theEnv, theSlotName,
...
/*   for the fact-slot-names function. */
/***************************************/
globle void EnvFactSlotNames(void* theEnv, void* vTheFact,
                             DATA_OBJECT* returnValue) {
  struct fact* theFact = (struct fact*)vTheFact;
  ...
  for (count = 0, theSlot = theFact->whichDeftemplate->slotList;
       theSlot != NULL; count++, theSlot = theSlot->next) { /* Do Nothing */
  }
...
globle void GetFactListFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  if ((numArgs = EnvArgCountCheck(theEnv, "get-fact-list", NO_MORE_THAN, 1)) ==
      -1) {
...
/*   for the get-fact-list function. */
/*************************************/
globle void EnvGetFactList(void* theEnv, DATA_OBJECT_PTR returnValue,
                           void* vTheModule) {
  struct fact* theFact;
  unsigned long count;
  struct multifield* theList;
...
/*   for the ppfact function.  */
/*******************************/
globle void EnvPPFact(void* theEnv, void* vTheFact, const char* logicalName,
                      int ignoreDefaults) {
#if MAC_XCD
#pragma unused(theEnv)
#endif
...
globle struct fact* GetFactAddressOrIndexArgument(void* theEnv,
  ...
  if (GetType(item) == FACT_ADDRESS) {
    if (((struct fact*)GetValue(item))->garbage) {
      ...
      return (((struct fact*)GetValue(item)));
  } else if (GetType(item) == INTEGER) {
    factIndex = ValueToLong(item.value);
    if (factIndex < 0) {
      ExpectedTypeError1(theEnv, theFunction, position,
...
globle void* FactDeftemplate(void* vTheFact) {
  return EnvFactDeftemplate(GetCurrentEnvironment(), vTheFact);
}

globle int FactExistp(void* vTheFact) {
  return EnvFactExistp(GetCurrentEnvironment(), vTheFact);
}

globle void FactSlotNames(void* vTheFact, DATA_OBJECT* returnValue) {
  EnvFactSlotNames(GetCurrentEnvironment(), vTheFact, returnValue);
}

globle void GetFactList(DATA_OBJECT_PTR returnValue, void* vTheModule) {
  EnvGetFactList(GetCurrentEnvironment(), returnValue, vTheModule);
}

globle void PPFact(void* vTheFact, const char* logicalName,
                   int ignoreDefaults) {
