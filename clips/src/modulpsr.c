...
static int ParsePortSpecifications(void*, const char*, struct token*,
                                   struct defmodule*);
static int ParseImportSpec(void*, const char*, struct token*,
                           struct defmodule*);
static int ParseExportSpec(void*, const char*, struct token*, struct defmodule*,
                           struct defmodule*);
static intBool DeleteDefmodule(void*, void*);
static int FindMultiImportConflict(void*, struct defmodule*);
static void NotExportedErrorMessage(void*, const char*, const char*,
                                    const char*);
...
/******************************************/
globle void SetNumberOfDefmodules(void* theEnv, long value) {
...
/****************************************************/
globle void AddAfterModuleDefinedFunction(void* theEnv, const char* name,
                                          void (*func)(void*), int priority) {
  DefmoduleData(theEnv)->AfterModuleDefinedFunctions = AddFunctionToCallList(
      theEnv, name, priority, func,
      DefmoduleData(theEnv)->AfterModuleDefinedFunctions, TRUE);
}
...
/******************************************************/
globle void AddPortConstructItem(void* theEnv, const char* theName,
                                 int theType) {
...
/******************************************************/
globle int ParseDefmodule(void* theEnv, const char* readSource) {
  ...
  defmoduleName = GetConstructNameAndComment(
      theEnv, readSource, &inputToken, "defmodule", EnvFindDefmodule,
      DeleteDefmodule, "+", TRUE, TRUE, FALSE, FALSE);
  ...
  if (redefiningMainModule == NULL) {
    newDefmodule = (struct defmodule*)EnvFindDefmodule(
        theEnv, ValueToString(defmoduleName));
    if (newDefmodule) {
      overwrite = TRUE;
    } else {
  ...
  for (portSpecs = newDefmodule->importList; portSpecs != NULL;
    ...
    if (portSpecs->moduleName != NULL)
      IncrementSymbolCount(portSpecs->moduleName);
    if (portSpecs->constructType != NULL)
      IncrementSymbolCount(portSpecs->constructType);
    if (portSpecs->constructName != NULL)
      IncrementSymbolCount(portSpecs->constructName);
  }
  ...
  if (redefiningMainModule != NULL) { /* Do nothing */
    ...
    newDefmodule->itemsArray = (struct defmoduleItemHeader**)gm2(
        theEnv, sizeof(void*) * DefmoduleData(theEnv)->NumberOfModuleItems);
    for (i = 0, theItem = DefmoduleData(theEnv)->ListOfModuleItems;
      ...
      if (theItem->allocateFunction == NULL) {
        ...
        newDefmodule->itemsArray[i] =
            (struct defmoduleItemHeader*)(*theItem->allocateFunction)(theEnv);
  ...
  if (redefiningMainModule == NULL) {
    if (DefmoduleData(theEnv)->LastDefmodule == NULL)
      ...
      DefmoduleData(theEnv)->LastDefmodule->next = newDefmodule;
...
/*************************************************************/
static intBool DeleteDefmodule(void* theEnv, void* theConstruct) {
...
/*********************************************************/
static int ParsePortSpecifications(void* theEnv, const char* readSource,
...
/**********************************************************/
static int ParseImportSpec(void* theEnv, const char* readSource,
                           struct token* theToken,
  ...
  if ((theModule = (struct defmodule*)EnvFindDefmodule(
           theEnv, ValueToString(theToken->value))) == NULL) {
  ...
  if (newModule->importList->constructType != NULL) {
    ...
    found = FALSE;
    for (thePort = theModule->exportList; (thePort != NULL) && (!found);
    ...
    if (!found) {
      if (newModule->importList->constructName == NULL) {
        NotExportedErrorMessage(
            theEnv, EnvGetDefmoduleName(theEnv, theModule),
            ValueToString(newModule->importList->constructType), NULL);
      } else {
        NotExportedErrorMessage(
            theEnv, EnvGetDefmoduleName(theEnv, theModule),
  ...
  for (thePort = newModule->importList; thePort != NULL;
       thePort = thePort->next) {
    ...
    theModule = (struct defmodule*)EnvFindDefmodule(
        theEnv, ValueToString(thePort->moduleName));
    EnvSetCurrentModule(theEnv, theModule);
    if (FindImportedConstruct(theEnv, ValueToString(thePort->constructType),
                              NULL, ValueToString(thePort->constructName),
...
/**********************************************************/
static int ParseExportSpec(void* theEnv, const char* readSource,
                           struct token* theToken, struct defmodule* newModule,
  ...
  if ((thePortConstruct = ValidPortConstructItem(
           theEnv, ValueToString(theConstruct))) == NULL) {
    SyntaxErrorMessage(theEnv, errorMessage);
    return (TRUE);
  }
...
/*************************************************************/
globle struct portConstructItem* ValidPortConstructItem(void* theEnv,
                                                        const char* theName) {
...
/***********************************************************/
static int FindMultiImportConflict(void* theEnv, struct defmodule* theModule) {
  ...
  for (testModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
       testModule != NULL; testModule = (struct defmodule*)EnvGetNextDefmodule(
                               theEnv, testModule)) {
    /*========================================*/
...
/******************************************************/
static void NotExportedErrorMessage(void* theEnv, const char* theModule,
  ...
  if (theConstruct == NULL)
    EnvPrintRouter(theEnv, WERROR, "any constructs");
...
/*************************************************************/
globle int FindImportExportConflict(void* theEnv, const char* constructName,
  ...
  for (theModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    ...
    FindImportedConstruct(theEnv, constructName, NULL, findName, &count, TRUE,
                          matchModule);
    if (count > 1) {
      RestoreCurrentModule(theEnv);
      return (TRUE);
...
/*   of defmodules currently defined.        */
/*********************************************/
globle long GetNumberOfDefmodules(void* theEnv) {
#if DEFMODULE_CONSTRUCT && (!RUN_TIME) && (!BLOAD_ONLY)
