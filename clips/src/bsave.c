...
globle void InitializeBsaveData(void* theEnv) {
  AllocateEnvironmentData(theEnv, BSAVE_DATA, sizeof(struct bsaveData),
                          DeallocateBsaveData);
}
...
/*    data for the bsave command.               */
/************************************************/
static void DeallocateBsaveData(void* theEnv) {
...
/*   for the bsave command.   */
/******************************/
globle intBool EnvBsave(void* theEnv, const char* fileName) {
  FILE* fp;
  ...
  GenWrite((void*)&ExpressionData(theEnv)->ExpressionCount,
           (unsigned long)sizeof(unsigned long), fp);
...
/*********************************************/
static void InitializeFunctionNeededFlags(void* theEnv) {
  struct FunctionDefinition* functionList;

  for (functionList = GetFunctionList(theEnv); functionList != NULL;
...
/**********************************************************/
static void FindNeededItems(void* theEnv) {
  struct BinaryItem* biPtr;
...
/****************************************************/
static void WriteNeededFunctions(void* theEnv, FILE* fp) {
  ...
  for (functionList = GetFunctionList(theEnv); functionList != NULL;
    ...
    if (functionList->bsaveIndex >= 0) {
      ...
      GenWrite((void*)ValueToString(functionList->callFunctionName),
               (unsigned long)length, fp);
    }
  }
}
...
/*********************************************/
static size_t FunctionBinarySize(void* theEnv) {
  size_t size = 0;
  struct FunctionDefinition* functionList;
...
globle void RestoreBloadCount(void* theEnv, long* cnt) {
  ...
  *cnt = BsaveData(theEnv)->BloadCountSaveTop->val;
  ...
  BsaveData(theEnv)->BloadCountSaveTop =
      BsaveData(theEnv)->BloadCountSaveTop->nxt;
...
/******************************************************/
static void WriteBinaryHeader(void* theEnv, FILE* fp) {
  GenWrite((void*)BloadData(theEnv)->BinaryPrefixID,
           (unsigned long)strlen(BloadData(theEnv)->BinaryPrefixID) + 1, fp);
  GenWrite((void*)BloadData(theEnv)->BinaryVersionID,
           (unsigned long)strlen(BloadData(theEnv)->BinaryVersionID) + 1, fp);
  GenWrite((void*)BloadData(theEnv)->BinarySizes,
           (unsigned long)strlen(BloadData(theEnv)->BinarySizes) + 1, fp);
}
...
/*   verification when a binary image is loaded.      */
/******************************************************/
static void WriteBinaryFooter(void* theEnv, FILE* fp) {
  char footerBuffer[CONSTRUCT_HEADER_SIZE];

  genstrncpy(footerBuffer, BloadData(theEnv)->BinaryPrefixID,
...
/*   binary file.                                         */
/**********************************************************/
globle intBool AddBinaryItem(void* theEnv, const char* name, int priority,
...
globle intBool Bsave(const char* fileName) {
