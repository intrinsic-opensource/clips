...
globle void InitializeExternalFunctionData(void* theEnv) {
  AllocateEnvironmentData(theEnv, EXTERNAL_FUNCTION_DATA,
...
/*    data for external functions.                         */
/***********************************************************/
static void DeallocateExternalFunctionData(void* theEnv) {
...
/******************************************************/
globle int EnvDefineFunction(void* theEnv, const char* name, int returnType,
                             int (*pointer)(void*), const char* actualName) {
  return (DefineFunction3(theEnv, name, returnType, pointer, actualName, NULL,
                          TRUE, NULL));
}
...
/************************************************************/
globle int EnvDefineFunctionWithContext(void* theEnv, const char* name,
                                        int returnType, int (*pointer)(void*),
                                        const char* actualName, void* context) {
  return (DefineFunction3(theEnv, name, returnType, pointer, actualName, NULL,
                          TRUE, context));
}
...
/*   external function so that the KB can access it.   */
/*******************************************************/
globle int EnvDefineFunction2(void* theEnv, const char* name, int returnType,
                              int (*pointer)(void*), const char* actualName,
                              const char* restrictions) {
  return (DefineFunction3(theEnv, name, returnType, pointer, actualName,
                          restrictions, TRUE, NULL));
}
...
/*   user external function so that the KB can access it.    */
/*************************************************************/
globle int EnvDefineFunction2WithContext(void* theEnv, const char* name,
                                         int returnType, int (*pointer)(void*),
                                         const char* actualName,
                                         const char* restrictions,
                                         void* context) {
  return (DefineFunction3(theEnv, name, returnType, pointer, actualName,
                          restrictions, TRUE, context));
}
...
/*     x - instance address                                  */
/*************************************************************/
globle int DefineFunction3(void* theEnv, const char* name, int returnType,
                           int (*pointer)(void*), const char* actualName,
                           const char* restrictions, intBool environmentAware,
                           void* context) {
  struct FunctionDefinition* newFunction;

  if ((returnType != 'a') && (returnType != 'b') && (returnType != 'c') &&
      (returnType != 'd') && (returnType != 'f') && (returnType != 'g') &&
      (returnType != 'i') && (returnType != 'j') && (returnType != 'k') &&
      (returnType != 'l') && (returnType != 'm') && (returnType != 'n') &&
#if OBJECT_SYSTEM
      (returnType != 'o') &&
#endif
      (returnType != 's') && (returnType != 'u') && (returnType != 'v') &&
#if OBJECT_SYSTEM
      (returnType != 'x') &&
#endif
...
/***********************************************/
globle int UndefineFunction(void* theEnv, const char* functionName) {
  SYMBOL_HN* findValue;
  struct FunctionDefinition *fPtr, *lastPtr = NULL;

  findValue = (SYMBOL_HN*)FindSymbolHN(theEnv, functionName);

  for (fPtr = ExternalFunctionData(theEnv)->ListOfFunctions; fPtr != NULL;
       fPtr = fPtr->next) {
...
/******************************************/
static int RemoveHashFunction(void* theEnv, struct FunctionDefinition* fdPtr) {
  struct FunctionHash *fhPtr, *lastPtr = NULL;
  unsigned hashValue;

  hashValue =
      HashSymbol(ValueToString(fdPtr->callFunctionName), SIZE_FUNCTION_HASH);
...
/*   parsing routines.                                                     */
/***************************************************************************/
globle int AddFunctionParser(void* theEnv, const char* functionName,
                             struct expr* (*fpPtr)(void*, struct expr*,
                                                   const char*)) {
...
/*********************************************************************/
globle int RemoveFunctionParser(void* theEnv, const char* functionName) {
  struct FunctionDefinition* fdPtr;
...
/*****************************************************************/
globle int FuncSeqOvlFlags(void* theEnv, const char* functionName, int seqp,
                           int ovlp) {
  struct FunctionDefinition* fdPtr;
  ...
  if (fdPtr == NULL) {
    EnvPrintRouter(theEnv, WERROR,
                   "Only existing functions can be marked as using sequence "
                   "expansion arguments/overloadable or not.\n");
    return (FALSE);
  }
...
/*********************************************************/
globle const char* GetArgumentTypeName(int theRestriction) {
...
/*********************************************************/
static void InitializeFunctionHashTable(void* theEnv) {
  int i;

  ExternalFunctionData(theEnv)->FunctionHashtable = (struct FunctionHash**)gm2(
      theEnv, (int)sizeof(struct FunctionHash*) * SIZE_FUNCTION_HASH);
...
/****************************************************************/
static void AddHashFunction(void* theEnv, struct FunctionDefinition* fdPtr) {
...
#if (!RUN_TIME)
globle int DefineFunction(const char* name, int returnType,
                          int (*pointer)(void), const char* actualName) {
  void* theEnv;

  theEnv = GetCurrentEnvironment();

  return (DefineFunction3(theEnv, name, returnType, (int (*)(void*))pointer,
                          actualName, NULL, FALSE, NULL));
}

globle int DefineFunction2(const char* name, int returnType,
                           int (*pointer)(void), const char* actualName,
                           const char* restrictions) {
  void* theEnv;
