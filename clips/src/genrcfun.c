...
globle int ClearDefmethods(void* theEnv) {
  ...
  while (gfunc != NULL) {
    if (RemoveAllExplicitMethods(theEnv, gfunc) == FALSE) success = FALSE;
...
globle int ClearDefgenerics(void* theEnv) {
  ...
  while (gfunc != NULL) {
    ...
    if (RemoveAllExplicitMethods(theEnv, gtmp) == FALSE) {
      CantDeleteItemErrorMessage(theEnv, "generic function",
                                 EnvGetDefgenericName(theEnv, gtmp));
      success = FALSE;
    } else {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void DestroyMethodInfo(void* theEnv, DEFGENERIC* gfunc,
                              DEFMETHOD* meth) {
  int j;
  RESTRICTION* rptr;
#if MAC_XCD
...
globle int MethodsExecuting(DEFGENERIC* gfunc) {
  ...
  for (i = 0; i < gfunc->mcnt; i++)
    if (gfunc->methods[i].busy > 0) return (TRUE);
  return (FALSE);
}
 ...
 **************************************************************/
globle intBool SubsumeType(int t1, int t2) {
  if ((t2 == OBJECT_TYPE_CODE) || (t2 == PRIMITIVE_TYPE_CODE)) return (TRUE);
  if ((t2 == NUMBER_TYPE_CODE) && ((t1 == INTEGER) || (t1 == FLOAT)))
    return (TRUE);
  if ((t2 == LEXEME_TYPE_CODE) && ((t1 == STRING) || (t1 == SYMBOL)))
    return (TRUE);
  if ((t2 == ADDRESS_TYPE_CODE) &&
      ((t1 == EXTERNAL_ADDRESS) || (t1 == FACT_ADDRESS) ||
       (t1 == INSTANCE_ADDRESS)))
    return (TRUE);
 ...
 ******************************************************************/
globle void PrintMethod(void* theEnv, char* buf, size_t buflen,
                        DEFMETHOD* meth) {
#if MAC_XCD
#pragma unused(theEnv)
  ...
  buf[0] = '\0';
  if (meth->system) genstrncpy(buf, "SYS", (STD_SIZE)buflen);
  gensprintf(numbuf, "%-2d ", meth->index);
  genstrncat(buf, numbuf, (STD_SIZE)buflen - 3);
  for (j = 0; j < meth->restrictionCount; j++) {
    rptr = &meth->restrictions[j];
    if ((((int)j) == meth->restrictionCount - 1) &&
    ...
    for (k = 0; k < rptr->tcnt; k++) {
#if OBJECT_SYSTEM
      genstrncat(buf, EnvGetDefclassName(theEnv, rptr->types[k]),
                 buflen - strlen(buf));
#else
      genstrncat(buf, TypeName(theEnv, ValueToInteger(rptr->types[k])),
                 buflen - strlen(buf));
#endif
    ...
    if (rptr->query != NULL) {
      if (rptr->tcnt != 0) genstrncat(buf, " ", buflen - strlen(buf));
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle DEFGENERIC* CheckGenericExists(void* theEnv, const char* fname,
                                      const char* gname) {
  DEFGENERIC* gfunc;
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle long CheckMethodExists(void* theEnv, const char* fname,
                              DEFGENERIC* gfunc, long mi) {
  long fi;
...
/*******************************************************
  ...
  NOTES        : Used only when COOL is not present
 *******************************************************/
globle const char* TypeName(void* theEnv, int tcode) {
  switch (tcode) {
    case INTEGER:
...
/******************************************************
  ...
  NOTES        : None
 ******************************************************/
globle void PrintGenericName(void* theEnv, const char* logName,
                             DEFGENERIC* gfunc) {
  if (gfunc->header.whichModule->theModule !=
      ((struct defmodule*)EnvGetCurrentModule(theEnv))) {
    EnvPrintRouter(theEnv, logName,
                   EnvGetDefmoduleName(
                       theEnv, (void*)gfunc->header.whichModule->theModule));
    EnvPrintRouter(theEnv, logName, "::");
  }
