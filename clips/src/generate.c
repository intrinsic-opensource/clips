...
static void ExtractAnds(void*, struct lhsParseNode*, int, struct expr**,
                        struct expr**, struct expr**, struct expr**,
                        struct nandFrame*);
static void ExtractFieldTest(void*, struct lhsParseNode*, int, struct expr**,
                             struct expr**, struct expr**, struct expr**,
                             struct nandFrame*);
...
static struct expr* GenPNVariableComparison(void*, struct lhsParseNode*,
                                            struct lhsParseNode*);
static int AllVariablesInPattern(struct lhsParseNode*, int);
static int AllVariablesInExpression(struct lhsParseNode*, int);

/*******************************************************/
/* FieldConversion: Generates join and pattern network */
/*   expressions for a field constraint.               */
/*******************************************************/
globle void FieldConversion(void* theEnv, struct lhsParseNode* theField,
  ...
  for (patternPtr = theField->bottom; patternPtr != NULL;
    ...
    ExtractAnds(theEnv, patternPtr, testInPatternNetwork, &patternNetTest,
                &joinNetTest, &constantSelector, &constantValue, theNandFrames);
    ...
    if (constantSelector != NULL) {
      if ((patternPtr == theField->bottom) && (patternPtr->bottom == NULL)) {
  ...
  if ((headOfPNExpression != NULL) ? (headOfPNExpression->nextArg != NULL)
                                   : FALSE) {
  ...
  if ((headOfJNExpression != NULL) ? (headOfJNExpression->nextArg != NULL)
                                   : FALSE) {
    tempExpression = GenConstant(theEnv, FCALL, ExpressionData(theEnv)->PTR_OR);
  ...
  if (((theField->type == MF_VARIABLE) || (theField->type == SF_VARIABLE)) &&
    ...
    if (theField->referringNode->pattern == theField->pattern) {
      tempExpression =
          GenPNVariableComparison(theEnv, theField, theField->referringNode);
      headOfPNExpression =
          CombineExpressions(theEnv, tempExpression, headOfPNExpression);
    }
    ...
    /*====================================*/
      ...
      headOfJNExpression =
          CombineExpressions(theEnv, tempExpression, headOfJNExpression);
      ...
      if (theField->patternType->genGetPNValueFunction != NULL) {
        ...
        thePattern->rightHash =
            AppendExpressions(tempExpression, thePattern->rightHash);
      }

      if (theField->referringNode->patternType->genGetJNValueFunction) {
        tempExpression =
            (*theField->referringNode->patternType->genGetJNValueFunction)(
                theEnv, theField->referringNode, LHS);
        thePattern->leftHash =
            AppendExpressions(tempExpression, thePattern->leftHash);
      }
    }
  }
  ...
  thePattern->networkTest =
      CombineExpressions(theEnv, thePattern->networkTest, headOfJNExpression);
}
...
/****************************************************************************/
static void ExtractAnds(void* theEnv, struct lhsParseNode* andField,
                        int testInPatternNetwork, struct expr** patternNetTest,
  ...
  *constantValue = NULL;
  ...
  for (; andField != NULL; andField = andField->right) {
    ...
    *constantSelector =
        CombineExpressions(theEnv, *constantSelector, newConstantSelector);
    *constantValue =
        CombineExpressions(theEnv, *constantValue, newConstantValue);
  }
}
...
/************************************************************************/
static void ExtractFieldTest(void* theEnv, struct lhsParseNode* theField,
  ...
  *constantValue = NULL;
  ...
  /*=====================================================================*/
    ...
    if ((testInPatternNetwork == TRUE) &&
      ...
      *joinNetTest = GenJNVariableComparison(theEnv, theField,
                                             theField->referringNode, FALSE);
      AddNandUnification(theEnv, theField, theNandFrames);
    }
  }
...
/*********************************************************/
static struct expr* GenPNConstant(void* theEnv, struct lhsParseNode* theField) {
  struct expr* top;
...
/************************************************************/
static struct expr* GenJNConstant(void* theEnv, struct lhsParseNode* theField,
                                  int isNand) {
  struct expr* top;
  ...
  if (isNand) {
    top->argList = (*theField->patternType->genGetJNValueFunction)(
        theEnv, theField, NESTED_RHS);
  } else {
    top->argList =
        (*theField->patternType->genGetJNValueFunction)(theEnv, theField, RHS);
  }
...
/******************************************************/
static struct expr* GenJNColon(void* theEnv, struct lhsParseNode* theField,
                               int isNand, struct nandFrame* theNandFrames) {
  ...
  if (isNand) {
    conversion =
        GetvarReplace(theEnv, theField->expression, TRUE, theNandFrames);
  } else {
    conversion =
        GetvarReplace(theEnv, theField->expression, FALSE, theNandFrames);
  }
...
/******************************************************/
static struct expr* GenPNColon(void* theEnv, struct lhsParseNode* theField) {
  struct expr *top, *conversion;
...
/******************************************************/
static struct expr* GenJNEq(void* theEnv, struct lhsParseNode* theField,
                            int isNand, struct nandFrame* theNandFrames) {
  struct expr *top, *conversion;
...
/*  return value field constraint (the = constraint).  */
/*******************************************************/
static struct expr* GenPNEq(void* theEnv, struct lhsParseNode* theField) {
  struct expr *top, *conversion;
...
/************************************************************************/
globle void AddNandUnification(void* theEnv, struct lhsParseNode* nodeList,
                               struct nandFrame* theNandFrames) {
  ...
  for (theFrame = theNandFrames; theFrame != NULL; theFrame = theFrame->next) {
    if (theFrame->depth >= nodeList->referringNode->beginNandDepth) {
      ...
      tempExpression = GenJNVariableComparison(theEnv, nodeList->referringNode,
                                               nodeList->referringNode, TRUE);

      theFrame->nandCE->externalNetworkTest = CombineExpressions(
          theEnv, theFrame->nandCE->externalNetworkTest, tempExpression);

      tempExpression =
          (*nodeList->referringNode->patternType->genGetJNValueFunction)(
              theEnv, nodeList->referringNode, LHS);
      theFrame->nandCE->externalRightHash = AppendExpressions(
          theFrame->nandCE->externalRightHash, tempExpression);
...
/*******************************************************************/
globle struct expr* GetvarReplace(void* theEnv, struct lhsParseNode* nodeList,
                                  int isNand, struct nandFrame* theNandFrames) {
  struct expr* newList;
  ...
  newList->nextArg =
      GetvarReplace(theEnv, nodeList->right, isNand, theNandFrames);
  newList->argList =
      GetvarReplace(theEnv, nodeList->bottom, isNand, theNandFrames);
...
/**********************************************************************/
static struct expr* GetfieldReplace(void* theEnv,
...
/**************************************************************/
static struct expr* GenJNVariableComparison(void* theEnv,
...

  {
    return (*selfNode->patternType->genCompareJNValuesFunction)(
        theEnv, selfNode, referringNode, isNand);
  }
  ...
  top->argList->nextArg = (*referringNode->patternType->genGetJNValueFunction)(
      theEnv, referringNode, LHS);
...
static struct expr* GenPNVariableComparison(
    void* theEnv, struct lhsParseNode* selfNode,
    struct lhsParseNode* referringNode) {
  if (selfNode->patternType->genComparePNValuesFunction != NULL) {
    return (*selfNode->patternType->genComparePNValuesFunction)(
        theEnv, selfNode, referringNode);
  }
...
/************************************************************/
static int AllVariablesInPattern(struct lhsParseNode* orField, int pattern) {
...
/**************************************************************************/
static int AllVariablesInExpression(struct lhsParseNode* theExpression,
                                    int pattern) {
  /*==========================================*/
