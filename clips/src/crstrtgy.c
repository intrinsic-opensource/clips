...
#define GetMatchingItem(x, i)                           \
  ((x->basis->binds[i].gm.theMatch != NULL)             \
       ? (x->basis->binds[i].gm.theMatch->matchingItem) \
       : NULL)
...
static ACTIVATION* PlaceLEXActivation(void*, ACTIVATION*,
                                      struct salienceGroup*);
static ACTIVATION* PlaceMEAActivation(void*, ACTIVATION*,
                                      struct salienceGroup*);
...
/******************************************************************/
globle void PlaceActivation(void* theEnv, ACTIVATION** whichAgenda,
...
/*******************************************************************/
static ACTIVATION* PlaceDepthActivation(ACTIVATION* newActivation,
                                        struct salienceGroup* theGroup) {
...
/*    activation should be placed at the beginning of the agenda). */
/*******************************************************************/
static ACTIVATION* PlaceBreadthActivation(ACTIVATION* newActivation,
                                          struct salienceGroup* theGroup) {
  unsigned long long timetag;
  ACTIVATION *lastAct, *actPtr;
...
/*    activation should be placed at the beginning of the agenda). */
/*******************************************************************/
static ACTIVATION* PlaceLEXActivation(void* theEnv, ACTIVATION* newActivation,
                                      struct salienceGroup* theGroup) {
  unsigned long long timetag;
  ACTIVATION *lastAct, *actPtr;
  int flag;
...
/*    activation should be placed at the beginning of the agenda). */
/*******************************************************************/
static ACTIVATION* PlaceMEAActivation(void* theEnv, ACTIVATION* newActivation,
                                      struct salienceGroup* theGroup) {
  unsigned long long timetag;
  ACTIVATION *lastAct, *actPtr;
  int flag;
...
/*********************************************************************/
static ACTIVATION* PlaceComplexityActivation(ACTIVATION* newActivation,
                                             struct salienceGroup* theGroup) {
  int complexity;
  unsigned long long timetag;
  ACTIVATION *lastAct, *actPtr;
...
/*    should be placed at the beginning of the agenda).              */
/*********************************************************************/
static ACTIVATION* PlaceSimplicityActivation(ACTIVATION* newActivation,
                                             struct salienceGroup* theGroup) {
  int complexity;
  unsigned long long timetag;
  ACTIVATION *lastAct, *actPtr;
...
/*    activation should be placed at the beginning of the agenda). */
/*******************************************************************/
static ACTIVATION* PlaceRandomActivation(ACTIVATION* newActivation,
                                         struct salienceGroup* theGroup) {
  int randomID;
  unsigned long long timetag;
  ACTIVATION *lastAct, *actPtr;
...
/*********************************************************/
static unsigned long long* SortPartialMatch(void* theEnv,
  ...
  nbinds =
      (unsigned long long*)get_mem(theEnv, sizeof(long long) * binds->bcount);
...
/**************************************************************************/
static int ComparePartialMatches(void* theEnv, ACTIVATION* actPtr,
                                 ACTIVATION* newActivation) {
...
globle void* GetStrategyCommand(void* theEnv) {
  ...
  return ((SYMBOL_HN*)EnvAddSymbol(theEnv,
                                   GetStrategyName(EnvGetStrategy(theEnv))));
}
...
globle void* SetStrategyCommand(void* theEnv) {
  ...
  if (strcmp(argument, "depth") == 0) {
    ...
    ExpectedTypeError1(theEnv, "set-strategy", 1,
                       "symbol with value depth, breadth, lex, mea, "
                       "complexity, simplicity, or random");
    return ((SYMBOL_HN*)EnvAddSymbol(theEnv,
                                     GetStrategyName(EnvGetStrategy(theEnv))));
  }
...
/**********************************************************/
static const char* GetStrategyName(int strategy) {
  const char* sname;
...
globle int SetStrategy(int value) {
  return EnvSetStrategy(GetCurrentEnvironment(), value);
}

globle int GetStrategy() { return EnvGetStrategy(GetCurrentEnvironment()); }
