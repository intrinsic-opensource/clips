...
/**************************************************/
globle void DeveloperCommands(void* theEnv) {
#if !RUN_TIME
  EnvDefineFunction2(theEnv, "primitives-info", 'v', PTIEF PrimitiveTablesInfo,
                     "PrimitiveTablesInfo", "00");
  EnvDefineFunction2(theEnv, "primitives-usage", 'v',
                     PTIEF PrimitiveTablesUsage, "PrimitiveTablesUsage", "00");

#if DEFRULE_CONSTRUCT && DEFTEMPLATE_CONSTRUCT
  EnvDefineFunction2(theEnv, "validate-fact-integrity", 'b',
                     ValidateFactIntegrity, "ValidateFactIntegrity", "00");

  EnvDefineFunction2(theEnv, "show-fpn", 'v', PTIEF ShowFactPatternNetwork,
                     "ShowFactPatternNetwork", "11w");
  EnvDefineFunction2(theEnv, "show-fht", 'v', PTIEF ShowFactHashTable,
                     "ShowFactHashTable", "00");
#endif
...
#if OBJECT_SYSTEM
  EnvDefineFunction2(theEnv, "instance-table-usage", 'v',
                     PTIEF InstanceTableUsage, "InstanceTableUsage", "00");
#endif
...
/*   symbol, float, integer, and bitmap tables.       */
/******************************************************/
globle void PrimitiveTablesUsage(void* theEnv) {
  unsigned long i;
...
/*******************************************************/
globle void ShowFactPatternNetwork(void* theEnv) {
  ...
  while (patternPtr != NULL) {
    ...
    if (patternPtr->header.singlefieldNode)
      EnvPrintRouter(theEnv, WDISPLAY, "SF   ");
    ...
    if (patternPtr->networkTest == NULL)
      ...
      PrintExpression(theEnv, WDISPLAY, patternPtr->networkTest);
    EnvPrintRouter(theEnv, WDISPLAY, " RightHash: ");
    if (patternPtr->header.rightHash == NULL)
      EnvPrintRouter(theEnv, WDISPLAY, "None");
    else
      PrintExpression(theEnv, WDISPLAY, patternPtr->header.rightHash);
    EnvPrintRouter(theEnv, WDISPLAY, "\n");
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void PrintObjectPatternNetwork(void* theEnv) {
  char indentbuf[80];
...
/**********************************************************
  ...
  NOTES        : None
 **********************************************************/
static void PrintOPNLevel(void* theEnv, OBJECT_PATTERN_NODE* pptr,
                          char* indentbuf, int ilen) {
  CLASS_BITMAP* cbmp;
  SLOT_BITMAP* sbmp;
  unsigned i;
  ...
  while (pptr != NULL) {
    ...
    if (pptr->alphaNode != NULL) EnvPrintRouter(theEnv, WDISPLAY, "+");
    EnvPrintRouter(theEnv, WDISPLAY,
                   ValueToString(FindIDSlotName(theEnv, pptr->slotNameID)));
    EnvPrintRouter(theEnv, WDISPLAY, " (");
    ...
    while (alphaPtr != NULL) {
      ...
      for (i = 0; i <= cbmp->maxid; i++)
        if (TestBitMap(cbmp->map, i)) {
          ...
          EnvPrintRouter(
              theEnv, WDISPLAY,
              EnvGetDefclassName(theEnv,
                                 (void*)DefclassData(theEnv)->ClassIDMap[i]));
        }
...
/* ExamineMemory: */
/******************/
static void ExamineMemory(void* theEnv, struct joinNode* theJoin,
...
/*************************/
static void TraverseBetaMemories(void* theEnv, struct joinNode* theJoin) {
  if (theJoin == NULL) {
    return;
  }
...
/***********************************/
static void ValidateRuleBetaMemoriesAction(void* theEnv,
...
/************************/
globle void ValidateBetaMemories(void* theEnv) {
  EnvPrintRouter(theEnv, WPROMPT, "ValidateBetaMemories");
  DoForAllConstructs(theEnv, ValidateRuleBetaMemoriesAction,
