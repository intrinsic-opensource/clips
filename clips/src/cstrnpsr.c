...
static intBool ParseRangeCardinalityAttribute(void*, const char*,
                                              CONSTRAINT_RECORD*,
...
globle intBool CheckConstraintParseConflicts(void* theEnv,
  ...
  if (constraints->anyAllowed == TRUE) { /* Do Nothing */
    ...
    for (theExp = constraints->restrictionList; theExp != NULL;
         theExp = theExp->nextArg) {
      if (ConstraintCheckValue(theEnv, theExp->type, theExp->value,
  ...
  if ((constraints->classList != NULL) && (constraints->anyAllowed == FALSE) &&
      (constraints->instanceNamesAllowed == FALSE) &&
      (constraints->instanceAddressesAllowed == FALSE)) {
...
/********************************************************/
globle void AttributeConflictErrorMessage(void* theEnv, const char* attribute1,
...
/************************************************************************/
globle intBool StandardConstraint(const char* constraintName) {
...
globle intBool ParseStandardConstraint(
    void* theEnv, const char* readSource, const char* constraintName,
    CONSTRAINT_RECORD* constraints, CONSTRAINT_PARSE_RECORD* parsedConstraints,
  ...
  if (strcmp(constraintName, "range") == 0) {
    rv = ParseRangeCardinalityAttribute(theEnv, readSource, constraints,
...
/* record.                                                 */
/***********************************************************/
globle void OverlayConstraint(void* theEnv, CONSTRAINT_PARSE_RECORD* pc,
  ...
  if (pc->allowedValues == 0) {
    if ((pc->allowedSymbols == 0) && (pc->allowedStrings == 0) &&
        (pc->allowedLexemes == 0) && (pc->allowedIntegers == 0) &&
        (pc->allowedFloats == 0) && (pc->allowedNumbers == 0) &&
...
/************************************************************/
static void AddToRestrictionList(void* theEnv, int type,
                                 CONSTRAINT_RECORD* cdst,
                                 CONSTRAINT_RECORD* csrc) {
...
static intBool ParseAllowedValuesAttribute(
  ...
  while (inputToken.type != RPAREN) {
    ...
    switch (inputToken.type) {
      case INTEGER:
        if ((expectedType != UNKNOWN_VALUE) && (expectedType != INTEGER) &&
      ...
      case FLOAT:
        if ((expectedType != UNKNOWN_VALUE) && (expectedType != FLOAT) &&
            (expectedType != INTEGER_OR_FLOAT))
          error = TRUE;
        constantParsed = TRUE;
        break;
      ...
      case STRING:
        if ((expectedType != UNKNOWN_VALUE) && (expectedType != STRING) &&
      ...
      case SYMBOL:
        if ((expectedType != UNKNOWN_VALUE) && (expectedType != SYMBOL) &&
            (expectedType != SYMBOL_OR_STRING))
          error = TRUE;
        constantParsed = TRUE;
        break;
...
#if OBJECT_SYSTEM
      case INSTANCE_NAME:
        if ((expectedType != UNKNOWN_VALUE) && (expectedType != INSTANCE_NAME))
          error = TRUE;
        constantParsed = TRUE;
        break;
#endif
...
/***********************************************************/
static void NoConjunctiveUseError(void* theEnv, const char* attribute1,
                                  const char* attribute2) {
...
/**************************************************/
static intBool ParseTypeAttribute(void* theEnv, const char* readSource,
                                  CONSTRAINT_RECORD* constraints) {
  ...
  SavePPBuffer(theEnv, " ");
  for (GetToken(theEnv, readSource, &inputToken); inputToken.type != RPAREN;
       GetToken(theEnv, readSource, &inputToken)) {
    SavePPBuffer(theEnv, " ");
...
static intBool ParseRangeCardinalityAttribute(
    void* theEnv, const char* readSource, CONSTRAINT_RECORD* constraints,
    CONSTRAINT_PARSE_RECORD* parsedConstraints, const char* constraintName,
    int multipleValuesAllowed) {
  struct token inputToken;
  int range;
  const char* tempPtr = NULL;
  ...
  if ((range == TRUE) &&
      (parsedConstraints->allowedValues || parsedConstraints->allowedNumbers ||
       parsedConstraints->allowedIntegers ||
       parsedConstraints->allowedFloats)) {
  ...
  if ((inputToken.type == INTEGER) || ((inputToken.type == FLOAT) && range)) {
    if (range) {
      ...
      constraints->minValue =
          GenConstant(theEnv, inputToken.type, inputToken.value);
    } else {
      if (ValueToLong(inputToken.value) < 0LL) {
        PrintErrorID(theEnv, "CSTRNPSR", 6, TRUE);
        EnvPrintRouter(theEnv, WERROR,
                       "Minimum cardinality value must be greater than or "
                       "equal to zero\n");
        return (FALSE);
      }
      ...
      constraints->minFields =
          GenConstant(theEnv, inputToken.type, inputToken.value);
             ...
             (strcmp(inputToken.printForm, "?VARIABLE") ==
              0)) { /* Do nothing. */
  } else {
    char tempBuffer[120];
    gensprintf(tempBuffer, "%s attribute", constraintName);
...
/******************************************************************/
static int GetConstraintTypeFromAllowedName(const char* constraintName) {
  if (strcmp(constraintName, "allowed-values") == 0)
...
/*******************************************************/
static int GetConstraintTypeFromTypeName(const char* constraintName) {
...
/**************************************************************/
static int GetAttributeParseValue(const char* constraintName,
                                  CONSTRAINT_PARSE_RECORD* parsedConstraints) {
  if (strcmp(constraintName, "type") == 0) {
...
/**********************************************************/
static void SetRestrictionFlag(int restriction, CONSTRAINT_RECORD* constraints,
                               int value) {
  switch (restriction) {
    case UNKNOWN_VALUE:
...
/********************************************************************/
static void SetParseFlag(CONSTRAINT_PARSE_RECORD* parsedConstraints,
