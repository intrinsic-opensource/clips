...
static EXPRESSION* ParseQueryRestrictions(void*, EXPRESSION*, const char*,
                                          struct token*);
...
static int ParseQueryActionExpression(void*, EXPRESSION*, const char*,
                                      EXPRESSION*, struct token*);
static intBool ReplaceInstanceVariables(void*, EXPRESSION*, EXPRESSION*, int,
                                        int);
 ...
 ***********************************************************************/
globle EXPRESSION* ParseQueryNoAction(void* theEnv, EXPRESSION* top,
  ...
  if (insQuerySetVars == NULL) return (NULL);
  ...
  if (ReplaceInstanceVariables(theEnv, insQuerySetVars, top->argList, TRUE,
                               0)) {
    ReturnExpression(theEnv, top);
    ReturnExpression(theEnv, insQuerySetVars);
    return NULL;
...
/***********************************************************************
  ...
  NOTES        : H/L Syntax :
                 ...
                 <class-2a> -> <class-2b> -> (QDS) -> ...
 ***********************************************************************/
globle EXPRESSION* ParseQueryAction(void* theEnv, EXPRESSION* top,
                                    const char* readSource) {
  EXPRESSION* insQuerySetVars;
  struct token queryInputToken;
  ...
  PPCRAndIndent(theEnv);
  if (ParseQueryActionExpression(theEnv, top, readSource, insQuerySetVars,
                                 &queryInputToken) == FALSE) {
    DecrementIndentDepth(theEnv, 3);
    ReturnExpression(theEnv, insQuerySetVars);
    return (NULL);
  }
  ...
  if (ReplaceInstanceVariables(theEnv, insQuerySetVars, top->argList->nextArg,
                               FALSE, 0)) {
    ReturnExpression(theEnv, top);
    ReturnExpression(theEnv, insQuerySetVars);
    return NULL;
 ...
 ***************************************************************/
static EXPRESSION* ParseQueryRestrictions(void* theEnv, EXPRESSION* top,
                                          const char* readSource,
                                          struct token* queryInputToken) {
  ...
  SavePPBuffer(theEnv, " ");
  GetToken(theEnv, readSource, queryInputToken);
  if (queryInputToken->type != LPAREN) goto ParseQueryRestrictionsError1;
  GetToken(theEnv, readSource, queryInputToken);
  if (queryInputToken->type != LPAREN) goto ParseQueryRestrictionsError1;
  while (queryInputToken->type == LPAREN) {
    GetToken(theEnv, readSource, queryInputToken);
    if (queryInputToken->type != SF_VARIABLE) goto ParseQueryRestrictionsError1;
    tmp = insQuerySetVars;
    while (tmp != NULL) {
      if (tmp->value == queryInputToken->value) {
        ...
        EnvPrintRouter(theEnv, WERROR,
                       ValueToString(ExpressionFunctionCallName(top)));
        EnvPrintRouter(theEnv, WERROR, ".\n");
    ...
    if (classExp == NULL) goto ParseQueryRestrictionsError1;
    ...
    PPBackup(theEnv);
    SavePPBuffer(theEnv, ")");
    tmp = GenConstant(theEnv, SYMBOL,
                      (void*)InstanceQueryData(theEnv)->QUERY_DELIMETER_SYMBOL);
    lastClassExp->nextArg = tmp;
    lastClassExp = tmp;
    if (top->argList == NULL)
  ...
  if (queryInputToken->type != RPAREN) goto ParseQueryRestrictionsError1;
  PPBackup(theEnv);
  PPBackup(theEnv);
  SavePPBuffer(theEnv, ")");
 ...
 ***************************************************/
static intBool ReplaceClassNameWithReference(void* theEnv, EXPRESSION* theExp) {
...
/*************************************************************
  ...
  NOTES        : Expects top != NULL
 *************************************************************/
static int ParseQueryTestExpression(void* theEnv, EXPRESSION* top,
                                    const char* readSource) {
  EXPRESSION* qtest;
  int error;
 ...
 *************************************************************/
static int ParseQueryActionExpression(void* theEnv, EXPRESSION* top,
                                      const char* readSource,
                                      EXPRESSION* insQuerySetVars,
                                      struct token* queryInputToken) {
  ...
  qaction =
      GroupActions(theEnv, readSource, queryInputToken, TRUE, NULL, FALSE);
  PPBackup(theEnv);
  PPBackup(theEnv);
...
/***********************************************************************************
  ...
  INPUTS       : 1) The instance-variable list
                 ...
                 3) A flag indicating whether to allow slot references of the
 type <instance-query-variable>:<slot-name> for direct slot access or not 4)
 Nesting depth of query functions RETURNS      : Nothing useful SIDE EFFECTS :
 If a SF_VARIABLE node is found and is on the list of instance variables, it is
 replaced with a query-instance function call. NOTES        : Other
 SF_VARIABLE(S) are left alone for replacement by other parsers.  This implies
 that a user may use defgeneric, defrule, and defmessage-handler variables
 within a query-function where they do not conflict with instance-variable
 names.
 ***********************************************************************************/
static intBool ReplaceInstanceVariables(void* theEnv, EXPRESSION* vlist,
                                        EXPRESSION* bexp, int sdirect,
                                        int ndepth) {
  EXPRESSION* eptr;
  ...
  while (bexp != NULL) {
    if (bexp->type == SF_VARIABLE) {
      ...
      if (eptr != NULL) {
        ...
        eptr = GenConstant(theEnv, INTEGER,
                           (void*)EnvAddLong(theEnv, (long long)ndepth));
        eptr->nextArg = GenConstant(theEnv, INTEGER,
                                    (void*)EnvAddLong(theEnv, (long long)posn));
        bexp->argList = eptr;
    ...
    if (bexp->argList != NULL) {
      if (IsQueryFunction(bexp)) {
        if (ReplaceInstanceVariables(theEnv, vlist, bexp->argList, sdirect,
                                     ndepth + 1)) {
          return TRUE;
        }
      } else {
...
/*************************************************************************
  ...
  NOTES        : None
 *************************************************************************/
static intBool ReplaceSlotReference(void* theEnv, EXPRESSION* vlist,
                                    EXPRESSION* theExp,
  ...
  for (i = len - 2; i >= 1; i--) {
    if ((str[i] == INSTANCE_SLOT_REF) ? (i >= 1) : FALSE) {
      ...
      while (eptr && ((i != strlen(ValueToString(eptr->value))) ||
                      strncmp(ValueToString(eptr->value), str, (STD_SIZE)i))) {
        eptr = eptr->nextArg;
        posn++;
      }
      if (eptr != NULL) {
        ...
        theExp->argList = GenConstant(
            theEnv, INTEGER, (void*)EnvAddLong(theEnv, (long long)ndepth));
        theExp->argList->nextArg = GenConstant(
            theEnv, INTEGER, (void*)EnvAddLong(theEnv, (long long)posn));
        theExp->argList->nextArg->nextArg =
            GenConstant(theEnv, itkn.type, itkn.value);
        theExp->argList->nextArg->nextArg->nextArg =
            GenConstant(theEnv, SYMBOL, EnvAddSymbol(theEnv, str));
        return FALSE;
      }
    }
  }
...
/********************************************************************
  ...
  NOTES        : None
 ********************************************************************/
static int IsQueryFunction(EXPRESSION* theExp) {
  int (*fptr)(void);

  if (theExp->type != FCALL) return (FALSE);
  fptr = (int (*)(void))ExpressionFunctionPointer(theExp);
  if (fptr == (int (*)(void))AnyInstances) return (TRUE);
  if (fptr == (int (*)(void))QueryFindInstance) return (TRUE);
  if (fptr == (int (*)(void))QueryFindAllInstances) return (TRUE);
  if (fptr == (int (*)(void))QueryDoForInstance) return (TRUE);
  if (fptr == (int (*)(void))QueryDoForAllInstances) return (TRUE);
  if (fptr == (int (*)(void))DelayedQueryDoForAllInstances) return (TRUE);
  return (FALSE);
}
