...
static intBool EvaluatePatternExpression(void*, struct factPatternNode*,
                                         struct expr*);
static void TraceErrorToJoin(void*, struct factPatternNode*, int);
static void ProcessFactAlphaMatch(void*, struct fact*, struct multifieldMarker*,
                                  struct factPatternNode*);
static struct factPatternNode* GetNextFactPatternNode(void*, int,
                                                      struct factPatternNode*);
static int SkipFactPatternNode(void*, struct factPatternNode*);
static void ProcessMultifieldNode(void*, struct factPatternNode*,
...
/*************************************************************************/
globle void FactPatternMatch(void* theEnv, struct fact* theFact,
                             struct factPatternNode* patternPtr, int offset,
    ...
    /*=========================================================*/
      ...
      if (patternPtr->header.endSlot &&
          ((FactData(theEnv)->CurrentPatternMarks == NULL)
               ? FALSE
               : (FactData(theEnv)
                      ->CurrentPatternMarks->where.whichSlotNumber ==
                  patternPtr->whichSlot)) &&
          (FactData(theEnv)
               ->CurrentPatternFact->theProposition
               .theFields[patternPtr->whichSlot]
               .type == MULTIFIELD)) {
        if ((patternPtr->leaveFields + theSlotField) !=
            (int)((struct multifield*)FactData(theEnv)
                      ->CurrentPatternFact->theProposition
                      .theFields[patternPtr->whichSlot]
                      .value)
                ->multifieldLength) {
      ...
      if (skipit) {
        ...
        if (EvaluatePatternExpression(theEnv, patternPtr,
                                      patternPtr->networkTest->nextArg)) {
          EvaluateExpression(theEnv, patternPtr->networkTest, &theResult);

          tempPtr = (struct factPatternNode*)FindHashedPatternNode(
              theEnv, patternPtr, theResult.type, theResult.value);
        } else {
          tempPtr = NULL;
        }
      ...
      else if (EvaluatePatternExpression(theEnv, patternPtr,
...
/**************************************************************/
static void ProcessMultifieldNode(void* theEnv,
  ...
  theSlotValue =
      (struct multifield*)FactData(theEnv)
          ->CurrentPatternFact->theProposition.theFields[thePattern->whichSlot]
          .value;
  ...
  if (thePattern->header.endSlot) {
    newMark->endPosition =
        (long)theSlotValue->multifieldLength - (thePattern->leaveFields + 1);
    ...
    if (thePattern->header.selector) {
      if (EvaluatePatternExpression(theEnv, thePattern,
                                    thePattern->networkTest->nextArg)) {
        EvaluateExpression(theEnv, thePattern->networkTest, &theResult);

        thePattern = (struct factPatternNode*)FindHashedPatternNode(
            theEnv, thePattern, theResult.type, theResult.value);
        if (thePattern != NULL) {
          success = TRUE;
        } else {
          success = FALSE;
        ...
        success = FALSE;
                   ...
                   ? TRUE
                   : (EvaluatePatternExpression(theEnv, thePattern,
    ...
    if (success) {
      ...
      if (thePattern->header.stopNode) {
        ProcessFactAlphaMatch(theEnv, FactData(theEnv)->CurrentPatternFact,
...
static struct factPatternNode* GetNextFactPatternNode(
    void* theEnv, int finishedMatching, struct factPatternNode* thePattern) {
...
/*******************************************************/
static void ProcessFactAlphaMatch(void* theEnv, struct fact* theFact,
  ...
  theMatch = CreateAlphaMatch(theEnv, theFact, theMarks,
  ...
  for (listOfJoins = thePattern->header.entryJoin; listOfJoins != NULL;
...
static int EvaluatePatternExpression(void* theEnv,
  ...
  if (theTest->value == ExpressionData(theEnv)->PTR_OR) {
    for (theTest = theTest->argList; theTest != NULL;
...
/*   be printed.                                                        */
/************************************************************************/
static void PatternNetErrorMessage(void* theEnv,
                                   struct factPatternNode* patternPtr) {
  char buffer[60];
...
/***************************************************************************/
static void TraceErrorToJoin(void* theEnv, struct factPatternNode* patternPtr,
                             int traceRight) {
  struct joinNode* joinPtr;

  while (patternPtr != NULL) {
    if (patternPtr->header.stopNode) {
      for (joinPtr = patternPtr->header.entryJoin; joinPtr != NULL;
...
/***********************************************************************/
static int SkipFactPatternNode(void* theEnv,
                               struct factPatternNode* thePattern) {
...
globle void MarkFactPatternForIncrementalReset(
    void* theEnv, struct patternNodeHeader* thePattern, int value) {
...
#pragma unused(theEnv)
  ...
  if (patternPtr->header.initialize == FALSE) {
    for (theJoin = patternPtr->header.entryJoin; theJoin != NULL;
...
globle void FactsIncrementalReset(void* theEnv) {
  ...
  for (factPtr = (struct fact*)EnvGetNextFact(theEnv, NULL); factPtr != NULL;
       factPtr = (struct fact*)EnvGetNextFact(theEnv, factPtr)) {
    EngineData(theEnv)->JoinOperationInProgress = TRUE;
    FactPatternMatch(theEnv, factPtr, factPtr->whichDeftemplate->patternNetwork,
                     0, NULL, NULL);
