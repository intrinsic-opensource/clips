...
static struct factPatternNode* FindPatternNode(struct factPatternNode*,
                                               ...
                                               unsigned, unsigned);
static struct factPatternNode* CreateNewPatternNode(void*, struct lhsParseNode*,
                                                    struct factPatternNode*,
                                                    struct factPatternNode*,
                                                    unsigned, unsigned);
...
static void FindAndSetDeftemplatePatternNetwork(void*, struct factPatternNode*,
                                                struct factPatternNode*);
#endif
...
static struct patternNodeHeader* PlaceFactPattern(
  ...
  FactData(theEnv)->CurrentDeftemplate =
      (struct deftemplate*)FindImportedConstruct(
          theEnv, "deftemplate", NULL, deftemplateName, &count, TRUE, NULL);
  ...
  while (thePattern != NULL) {
    ...
    newNode = FindPatternNode(currentLevel, thePattern, &nodeBeforeMatch,
                              endSlot, FALSE);
    ...
    if (newNode == NULL) {
      newNode = CreateNewPatternNode(theEnv, thePattern, nodeBeforeMatch,
...
static struct factPatternNode* FindPatternNode(
    struct factPatternNode* listOfNodes, struct lhsParseNode* thePattern,
    struct factPatternNode** nodeBeforeMatch, unsigned endSlot,
  ...
  *nodeBeforeMatch = NULL;
  ...
  while (listOfNodes != NULL) {
    ...
    if ((thePattern->type == SF_WILDCARD) ||
        (thePattern->type == SF_VARIABLE)) {
      if ((listOfNodes->header.singlefieldNode) &&
          ...
          IdenticalExpression(listOfNodes->header.rightHash,
                              thePattern->rightHash)) {
        return (listOfNodes);
      }
    } else if ((thePattern->type == MF_WILDCARD) ||
...
static struct lhsParseNode* RemoveUnneededSlots(
  ...
  while (tempPattern != NULL) {
    ...
    if (((tempPattern->type == SF_WILDCARD) ||
    ...
    /*=========================================================*/
      ...
      tempPattern->networkTest =
          FactGenCheckZeroLength(theEnv, tempPattern->slotNumber);
      tempPattern->multifieldSlot = FALSE;
      lastPattern = tempPattern;
      tempPattern = tempPattern->right;
    }
    ...
    /*===================================================*/
      ...
      if (tempPattern->bottom->constantSelector != NULL) {
        tempPattern->bottom->constantSelector->nextArg =
            CopyExpression(theEnv, theTest);
      }
      theTest =
          CombineExpressions(theEnv, theTest, tempPattern->bottom->networkTest);
...
static struct factPatternNode* CreateNewPatternNode(
    void* theEnv, struct lhsParseNode* thePattern,
    struct factPatternNode* nodeBeforeMatch, struct factPatternNode* upperLevel,
  ...
  newNode->header.rightHash =
      AddHashedExpression(theEnv, thePattern->rightHash);
...
/*                                                           */
/*************************************************************/
static void DetachFactPattern(void* theEnv,
...
globle void DestroyFactPatternNetwork(void* theEnv,
  ...
  while (thePattern != NULL) {
    ...
    if ((thePattern->lastLevel != NULL) &&
      ...
      RemoveHashedPatternNode(theEnv, thePattern->lastLevel, thePattern,
...
static void FindAndSetDeftemplatePatternNetwork(
    void* theEnv, struct factPatternNode* rootNode,
...
/***************************************************************/
static void ClearPatternMatches(void* theEnv,
                                struct factPatternNode* patternPtr) {
  struct fact* theFact;
