...
globle void InitializeScannerData(void* theEnv) {
  AllocateEnvironmentData(theEnv, SCANNER_DATA, sizeof(struct scannerData),
...
/*    data for scanner routines.                  */
/**************************************************/
static void DeallocateScannerData(void* theEnv) {
  if (ScannerData(theEnv)->GlobalMax != 0) {
    genfree(theEnv, ScannerData(theEnv)->GlobalString,
...
/***********************************************************************/
globle void GetToken(void* theEnv, const char* logicalName,
  ...
  /*===============================================*/
    ...
    switch (inchar) {
      ...
      case '"':
        ...
        theToken->printForm =
            StringPrintForm(theEnv, ValueToString(theToken->value));
        break;
      ...
      case '?':
...
#endif
        {
          ...
          if ((ValueToString(theToken->value)[0] == '*') &&
              ...
              (ValueToString(
                   theToken->value)[strlen(ValueToString(theToken->value)) -
                                    1] == '*')) {
            size_t count;

            theToken->type = GBL_VARIABLE;
            theToken->printForm =
                AppendStrings(theEnv, "?", ValueToString(theToken->value));
            count = strlen(ScannerData(theEnv)->GlobalString);
            ScannerData(theEnv)->GlobalString[count - 1] = EOS;
            theToken->value =
                EnvAddSymbol(theEnv, ScannerData(theEnv)->GlobalString + 1);
      ...
      case '$':
        if ((inchar = EnvGetcRouter(theEnv, logicalName)) == '?') {
          ...
          theToken->type = SYMBOL;
          ScannerData(theEnv)->GlobalString = ExpandStringWithChar(
              theEnv, '$', ScannerData(theEnv)->GlobalString,
  ...
  if (ScannerData(theEnv)->GlobalString != NULL) {
    rm(theEnv, ScannerData(theEnv)->GlobalString,
       ScannerData(theEnv)->GlobalMax);
...
/*************************************/
static void* ScanSymbol(void* theEnv, const char* logicalName, int count,
                        unsigned short* type) {
  int inchar;
#if OBJECT_SYSTEM
  void* symbol;
  ...
  while ((inchar != '<') && (inchar != '"') && (inchar != '(') &&
         ...
         (inchar != '~') && (inchar != ' ') && (inchar != ';') &&
         (isprint(inchar) || IsUTF8MultiByteStart(inchar) ||
          IsUTF8MultiByteContinuation(inchar))) {
    ScannerData(theEnv)->GlobalString = ExpandStringWithChar(
        theEnv, inchar, ScannerData(theEnv)->GlobalString,
        &ScannerData(theEnv)->GlobalPos, &ScannerData(theEnv)->GlobalMax,
        ScannerData(theEnv)->GlobalMax + 80);
...
/*************************************/
static void* ScanString(void* theEnv, const char* logicalName) {
  int inchar;
  size_t pos = 0;
  size_t max = 0;
  char* theString = NULL;
  ...
  while ((inchar != '"') && (inchar != EOF)) {
    ...
    theString =
        ExpandStringWithChar(theEnv, inchar, theString, &pos, &max, max + 80);
    inchar = EnvGetcRouter(theEnv, logicalName);
  }
...
/**************************************/
static void ScanNumber(void* theEnv, const char* logicalName,
                       struct token* theToken) {
  int count = 0;
  int inchar, phase;
  ...
  if (!digitFound) {
    theToken->type = SYMBOL;
    theToken->value =
        (void*)EnvAddSymbol(theEnv, ScannerData(theEnv)->GlobalString);
    theToken->printForm = ValueToString(theToken->value);
    return;
  }
  ...
  if (processFloat) {
    ...
    theToken->printForm =
        LongIntegerToString(theEnv, ValueToLong(theToken->value));
  }
...
/*   line count to zero.                */
/****************************************/
globle void ResetLineCount(void* theEnv) { ScannerData(theEnv)->LineCount = 0; }
...
/***********************************************/
globle long SetLineCount(void* theEnv, long value) {
  long oldValue;
