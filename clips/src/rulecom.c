...
static long long ListBetaMatches(void*, struct joinInformation*, long, long,
                                 int);
static void ListBetaJoinActivity(void*, struct joinInformation*, long, long,
                                 int, DATA_OBJECT*);
static long AlphaJoinCountDriver(void*, struct joinNode*);
static long BetaJoinCountDriver(void*, struct joinNode*);
static void AlphaJoinsDriver(void*, struct joinNode*, long,
                             struct joinInformation*);
static void BetaJoinsDriver(void*, struct joinNode*, long,
                            struct joinInformation*, struct betaMemory*,
                            struct joinNode*);
...
static const char* ActivityHeaderString(void*, struct joinInformation*, long,
                                        long);
...
globle void DefruleCommands(void* theEnv) {
  ...
  EnvDefineFunction2(theEnv, "halt", 'v', PTIEF HaltCommand, "HaltCommand",
                     "00");
  EnvDefineFunction2(theEnv, "focus", 'b', PTIEF FocusCommand, "FocusCommand",
                     "1*w");
  EnvDefineFunction2(theEnv, "clear-focus-stack", 'v',
                     PTIEF ClearFocusStackCommand, "ClearFocusStackCommand",
                     "00");
  EnvDefineFunction2(theEnv, "get-focus-stack", 'm',
                     PTIEF GetFocusStackFunction, "GetFocusStackFunction",
                     "00");
  ...
  EnvDefineFunction2(theEnv, "matches", 'u', PTIEF MatchesCommand,
                     "MatchesCommand", "12w");
  EnvDefineFunction2(theEnv, "join-activity", 'u', PTIEF JoinActivityCommand,
                     "JoinActivityCommand", "12w");
  EnvDefineFunction2(theEnv, "join-activity-reset", 'v',
                     PTIEF JoinActivityResetCommand, "JoinActivityResetCommand",
                     "00");
  EnvDefineFunction2(theEnv, "list-focus-stack", 'v',
                     PTIEF ListFocusStackCommand, "ListFocusStackCommand",
                     "00");
  ...
  EnvDefineFunction2(theEnv, "get-incremental-reset", 'b',
                     GetIncrementalResetCommand, "GetIncrementalResetCommand",
                     "00");
  EnvDefineFunction2(theEnv, "set-incremental-reset", 'b',
                     SetIncrementalResetCommand, "SetIncrementalResetCommand",
                     "11");

  EnvDefineFunction2(theEnv, "get-beta-memory-resizing", 'b',
                     GetBetaMemoryResizingCommand,
                     "GetBetaMemoryResizingCommand", "00");
  EnvDefineFunction2(theEnv, "set-beta-memory-resizing", 'b',
                     SetBetaMemoryResizingCommand,
                     "SetBetaMemoryResizingCommand", "11");

  EnvDefineFunction2(theEnv, "get-strategy", 'w', PTIEF GetStrategyCommand,
                     "GetStrategyCommand", "00");
  EnvDefineFunction2(theEnv, "set-strategy", 'w', PTIEF SetStrategyCommand,
                     "SetStrategyCommand", "11w");

#if DEVELOPER && (!BLOAD_ONLY)
  EnvDefineFunction2(theEnv, "rule-complexity", 'l',
                     PTIEF RuleComplexityCommand, "RuleComplexityCommand",
                     "11w");
  EnvDefineFunction2(theEnv, "show-joins", 'v', PTIEF ShowJoinsCommand,
                     "ShowJoinsCommand", "11w");
  EnvDefineFunction2(theEnv, "show-aht", 'v', PTIEF ShowAlphaHashTable,
                     "ShowAlphaHashTable", "00");
#if DEBUGGING_FUNCTIONS
  AddWatchItem(theEnv, "rule-analysis", 0,
               &DefruleData(theEnv)->WatchRuleAnalysis, 0, NULL, NULL);
#endif
...
/****************************************/
globle void MatchesCommand(void* theEnv, DATA_OBJECT* result) {
  ...
  if ((numberOfArguments = EnvArgRangeCheck(theEnv, "matches", 1, 2)) == -1)
    return;
  ...
  if (numberOfArguments == 2) {
    ...
    if (strcmp(argument, "verbose") == 0) {
      ...
      ExpectedTypeError1(theEnv, "matches", 2,
...
/*   for the matches command.   */
/********************************/
globle void EnvMatches(void* theEnv, void* theRule, int output,
                       DATA_OBJECT* result) {
  ...
  for (rulePtr = topDisjunct; rulePtr != NULL; rulePtr = rulePtr->disjunct) {
    ...
    for (joinIndex = 1; joinIndex < arraySize; joinIndex++) {
      betaMatchCount +=
          ListBetaMatches(theEnv, theInfo, joinIndex, arraySize, output);
  ...
  for (agendaPtr =
       ...
       agendaPtr != NULL; agendaPtr = (struct activation*)EnvGetNextActivation(
                              theEnv, agendaPtr)) {
    ...
    if (((struct activation*)agendaPtr)->theRule->header.name ==
      ...
      if (output == VERBOSE) {
        PrintPartialMatch(theEnv, WDISPLAY,
                          EnvGetActivationBasis(theEnv, agendaPtr));
...
/*   alpha joins.                                   */
/****************************************************/
static long AlphaJoinCountDriver(void* theEnv, struct joinNode* theJoin) {
  long alphaCount = 0;
  ...
  if (theJoin->joinFromTheRight) {
    return AlphaJoinCountDriver(
        theEnv, (struct joinNode*)theJoin->rightSideEntryStructure);
...
/**************************************************/
globle long EnvAlphaJoinCount(void* theEnv, void* vTheDefrule) {
...
/***************************************/
static void AlphaJoinsDriver(void* theEnv, struct joinNode* theJoin,
                             long alphaIndex, struct joinInformation* theInfo) {
  if (theJoin == NULL) {
    return;
  }

  if (theJoin->joinFromTheRight) {
    AlphaJoinsDriver(theEnv, (struct joinNode*)theJoin->rightSideEntryStructure,
                     alphaIndex, theInfo);
    return;
  } else if (theJoin->lastLevel != NULL) {
...
/********************************************/
globle void EnvAlphaJoins(void* theEnv, void* vTheDefrule, long alphaCount,
                          struct joinInformation* theInfo) {
  struct defrule* theDefrule = (struct defrule*)vTheDefrule;

  AlphaJoinsDriver(theEnv, theDefrule->lastJoin->lastLevel, alphaCount,
                   theInfo);
}
...
/*   beta joins.                                   */
/****************************************************/
static long BetaJoinCountDriver(void* theEnv, struct joinNode* theJoin) {
  long betaCount = 0;
...
/*   joins associated with the specified rule.  */
/************************************************/
globle long EnvBetaJoinCount(void* theEnv, void* vTheDefrule) {
  struct defrule* theDefrule = (struct defrule*)vTheDefrule;
...
/**************************************/
static void BetaJoinsDriver(void* theEnv, struct joinNode* theJoin,
                            long betaIndex,
  ...
  if (theJoin->joinFromTheRight) {
    BetaJoinsDriver(theEnv, (struct joinNode*)theJoin->rightSideEntryStructure,
                    betaIndex - 1, theJoinInfoArray, theJoin->rightMemory,
                    theJoin);
  } else if (theJoin->lastLevel != NULL) {
    BetaJoinsDriver(theEnv, theJoin->lastLevel, betaIndex - 1, theJoinInfoArray,
                    theJoin->leftMemory, theJoin);
  }
...
/*   associated with the specified rule.  */
/******************************************/
globle void EnvBetaJoins(void* theEnv, void* vTheDefrule, long betaArraySize,
                         struct joinInformation* theInfo) {
  struct defrule* theDefrule = (struct defrule*)vTheDefrule;

  BetaJoinsDriver(theEnv, theDefrule->lastJoin->lastLevel, betaArraySize,
...
/**************************************************/
globle struct joinInformation* EnvCreateJoinArray(void* theEnv, long size) {
  if (size == 0) return (NULL);

  return (struct joinInformation*)genalloc(
      theEnv, sizeof(struct joinInformation) * size);
}
...
/*    array of the specified size.            */
/**********************************************/
globle void EnvFreeJoinArray(void* theEnv, struct joinInformation* theArray,
                             long size) {
  if (size == 0) return;
...
/* ListAlphaMatches: */
/*********************/
static long long ListAlphaMatches(void* theEnv, struct joinInformation* theInfo,
                                  int output) {
  ...
  listOfHashNodes =
      ((struct patternNodeHeader*)theJoin->rightSideEntryStructure)->firstHash;

  for (count = 0; listOfHashNodes != NULL;
...
/* BetaHeaderString */
/********************/
static const char* BetaHeaderString(void* theEnv,
...
/* ListBetaMatches: */
/********************/
static long long ListBetaMatches(void* theEnv,
                                 struct joinInformation* infoArray,
                                 long joinIndex, long arraySize, int output) {
  long betaCount = 0;
  struct joinInformation* theInfo;
  long int count;
  ...
  count =
      PrintBetaMemory(theEnv, WDISPLAY, theInfo->theMemory, TRUE, "", output);
...
/* CountPatterns: */
/******************/
static int CountPatterns(void* theEnv, struct joinNode* theJoin,
                         int followRight) {
  int theCount = 0;
...
/*******************************************/
globle void JoinActivityCommand(void* theEnv, DATA_OBJECT* result) {
  const char *ruleName, *argument;
  void* rulePtr;
  int numberOfArguments;
  DATA_OBJECT argPtr = DATA_OBJECT_INIT;
  ...
  if ((numberOfArguments = EnvArgRangeCheck(theEnv, "join-activity", 1, 2)) ==
      -1)
    return;

  if (EnvArgTypeCheck(theEnv, "join-activity", 1, SYMBOL, &argPtr) == FALSE)
    return;
  ...
  if (numberOfArguments == 2) {
    ...
    if (strcmp(argument, "verbose") == 0) {
      ...
      ExpectedTypeError1(theEnv, "join-activity", 2,
                         "symbol with value verbose, succinct, or terse");
      return;
    }
  } else {
...
/*   for the join-activity command.  */
/*************************************/
globle void EnvJoinActivity(void* theEnv, void* theRule, int output,
                            DATA_OBJECT* result) {
  struct defrule* rulePtr;
  ...
  for (disjunctIndex = 1; disjunctIndex <= disjunctCount; disjunctIndex++) {
    rulePtr =
        (struct defrule*)EnvGetNthDisjunct(theEnv, theRule, disjunctIndex);
    ...
    for (joinIndex = 0; joinIndex < arraySize; joinIndex++) {
      ListBetaJoinActivity(theEnv, theInfo, joinIndex, arraySize, output,
                           result);
    }
...
/************************/
static const char* ActivityHeaderString(void* theEnv,
                                        struct joinInformation* infoArray,
                                        long joinIndex, long arraySize) {
  struct joinNode* theJoin;
  struct joinInformation* theInfo;
...
/*************************/
static void ListBetaJoinActivity(void* theEnv,
                                 struct joinInformation* infoArray,
                                 long joinIndex, long arraySize, int output,
                                 DATA_OBJECT* result) {
...
/*********************************************/
static void JoinActivityReset(void* theEnv,
...
/************************************************/
globle void JoinActivityResetCommand(void* theEnv) {
  DoForAllConstructs(theEnv, JoinActivityReset,
...
/***********************************************/
globle long RuleComplexityCommand(void* theEnv) {
  const char* ruleName;
  struct defrule* rulePtr;
...
/*   for the show-joins command. */
/*********************************/
static void ShowJoins(void* theEnv, void* theRule) {
  struct defrule* rulePtr;
  struct joinNode* theJoin;
  ...
  while (rulePtr != NULL) {
    ...
    while (numberOfJoins >= 0) {
      ...
      gensprintf(buffer, "%2d%c%c%c%c : ", (int)joinList[numberOfJoins]->depth,
                 (joinList[numberOfJoins]->firstJoin) ? 'f' : ' ', rhsType,
      ...
      if (joinList[numberOfJoins]->ruleToActivate != NULL) {
        ...
        EnvPrintRouter(
            theEnv, WDISPLAY,
            EnvGetDefruleName(theEnv, joinList[numberOfJoins]->ruleToActivate));
        EnvPrintRouter(theEnv, WDISPLAY, "\n");
      }

      if (joinList[numberOfJoins]->secondaryNetworkTest != NULL) {
        EnvPrintRouter(theEnv, WDISPLAY, "    SNT : ");
        PrintExpression(theEnv, WDISPLAY,
                        joinList[numberOfJoins]->secondaryNetworkTest);
        EnvPrintRouter(theEnv, WDISPLAY, "\n");
      }
      ...
      if (!joinList[numberOfJoins]->firstJoin) {
        ...
        count = PrintBetaMemory(theEnv, WDISPLAY,
                                joinList[numberOfJoins]->leftMemory, FALSE, "",
                                SUCCINCT);
        if (count == 0) {
          EnvPrintRouter(theEnv, WDISPLAY, "None\n");
        } else {
      ...
      if (joinList[numberOfJoins]->joinFromTheRight) {
        ...
        count = PrintBetaMemory(theEnv, WDISPLAY,
                                joinList[numberOfJoins]->rightMemory, FALSE, "",
                                SUCCINCT);
        if (count == 0) {
          EnvPrintRouter(theEnv, WDISPLAY, "None\n");
        } else {
...
/******************************************************/
globle void ShowAlphaHashTable(void* theEnv) {
  int i, count;
  long totalCount = 0;
  ...
  for (i = 0; i < ALPHA_MEMORY_HASH_SIZE; i++) {
    ...
    if (count != 0) {
      ...
      for (theEntry = DefruleData(theEnv)->AlphaMemoryTable[i], count = 0;
           theEntry != NULL; theEntry = theEntry->next) {
        for (theMatch = theEntry->alphaMemory; theMatch != NULL;
...
globle void Matches(void* theRule, int output, DATA_OBJECT* result) {
  EnvMatches(GetCurrentEnvironment(), theRule, output, result);
}

globle void JoinActivity(void* theRule, int output, DATA_OBJECT* result) {
...
globle intBool SetBetaMemoryResizing(int value) {
