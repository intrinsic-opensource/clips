...
#define MiscFunctionData(theEnv) \
...
static void ExpandFuncMultifield(void*, DATA_OBJECT*, EXPRESSION*, EXPRESSION**,
                                 void*);
...
globle void MiscFunctionDefinitions(void* theEnv) {
  AllocateEnvironmentData(theEnv, MISCFUN_DATA, sizeof(struct miscFunctionData),
                          NULL);
  MiscFunctionData(theEnv)->GensymNumber = 1;

#if !RUN_TIME
  EnvDefineFunction2(theEnv, "gensym", 'w', PTIEF GensymFunction,
                     "GensymFunction", "00");
  EnvDefineFunction2(theEnv, "gensym*", 'w', PTIEF GensymStarFunction,
                     "GensymStarFunction", "00");
  EnvDefineFunction2(theEnv, "setgen", 'g', PTIEF SetgenFunction,
                     "SetgenFunction", "11i");
  EnvDefineFunction2(theEnv, "system", 'v', PTIEF gensystem, "gensystem",
                     "1*k");
  EnvDefineFunction2(theEnv, "length", 'g', PTIEF LengthFunction,
                     "LengthFunction", "11q");
  EnvDefineFunction2(theEnv, "length$", 'g', PTIEF LengthFunction,
                     "LengthFunction", "11q");
  EnvDefineFunction2(theEnv, "time", 'd', PTIEF TimeFunction, "TimeFunction",
                     "00");
  EnvDefineFunction2(theEnv, "local-time", 'm', PTIEF LocalTimeFunction,
                     "LocalTimeFunction", "00");
  EnvDefineFunction2(theEnv, "gm-time", 'm', PTIEF GMTimeFunction,
                     "GMTimeFunction", "00");

  EnvDefineFunction2(theEnv, "random", 'g', PTIEF RandomFunction,
                     "RandomFunction", "02i");
  EnvDefineFunction2(theEnv, "seed", 'v', PTIEF SeedFunction, "SeedFunction",
                     "11i");
  EnvDefineFunction2(theEnv, "conserve-mem", 'v', PTIEF ConserveMemCommand,
                     "ConserveMemCommand", "11w");
  EnvDefineFunction2(theEnv, "release-mem", 'g', PTIEF ReleaseMemCommand,
                     "ReleaseMemCommand", "00");
#if DEBUGGING_FUNCTIONS
  EnvDefineFunction2(theEnv, "mem-used", 'g', PTIEF MemUsedCommand,
                     "MemUsedCommand", "00");
  EnvDefineFunction2(theEnv, "mem-requests", 'g', PTIEF MemRequestsCommand,
                     "MemRequestsCommand", "00");
#endif
  EnvDefineFunction2(theEnv, "options", 'v', PTIEF OptionsCommand,
                     "OptionsCommand", "00");
  EnvDefineFunction2(theEnv, "operating-system", 'w',
                     PTIEF OperatingSystemFunction, "OperatingSystemFunction",
                     "00");
  EnvDefineFunction2(theEnv, "(expansion-call)", 'u', PTIEF ExpandFuncCall,
                     "ExpandFuncCall", NULL);
  ...
  EnvDefineFunction2(theEnv, "(set-evaluation-error)", 'w',
                     PTIEF CauseEvaluationError, "CauseEvaluationError", NULL);
  EnvDefineFunction2(theEnv, "set-sequence-operator-recognition", 'b',
                     PTIEF SetSORCommand, "SetSORCommand", "11w");
  EnvDefineFunction2(theEnv, "get-sequence-operator-recognition", 'b',
                     PTIEF EnvGetSequenceOperatorRecognition,
                     "EnvGetSequenceOperatorRecognition", "00");
  EnvDefineFunction2(theEnv, "get-function-restrictions", 's',
                     PTIEF GetFunctionRestrictions, "GetFunctionRestrictions",
                     "11w");
  EnvDefineFunction2(theEnv, "create$", 'm', PTIEF CreateFunction,
                     "CreateFunction", NULL);
  EnvDefineFunction2(theEnv, "mv-append", 'm', PTIEF CreateFunction,
                     "CreateFunction", NULL);
  EnvDefineFunction2(theEnv, "apropos", 'v', PTIEF AproposCommand,
                     "AproposCommand", "11w");
  EnvDefineFunction2(theEnv, "get-function-list", 'm',
                     PTIEF GetFunctionListFunction, "GetFunctionListFunction",
                     "00");
  EnvDefineFunction2(theEnv, "funcall", 'u', PTIEF FuncallFunction,
                     "FuncallFunction", "1**k");
  EnvDefineFunction2(theEnv, "new", 'u', PTIEF NewFunction, "NewFunction",
                     "1*uw");
  EnvDefineFunction2(theEnv, "call", 'u', PTIEF CallFunction, "CallFunction",
                     "1*u");
...
globle long long SetgenFunction(void* theEnv) {
  ...
  if (EnvArgCountCheck(theEnv, "setgen", EXACTLY, 1) == -1)
  ...
  if (theLong < 1LL) {
    ExpectedTypeError1(theEnv, "setgen", 1,
...
globle void ConserveMemCommand(void* theEnv) {
  ...
  if (EnvArgTypeCheck(theEnv, "conserve-mem", 1, SYMBOL, &theValue) == FALSE)
    return;
  ...
  /*=====================================================*/
    ...
    ExpectedTypeError1(theEnv, "conserve-mem", 1,
                       "symbol with value on or off");
    return;
  }
...
globle void AproposCommand(void* theEnv) {
  ...
  while ((hashPtr = GetNextSymbolMatch(theEnv, argument, theLength, hashPtr,
                                       TRUE, NULL)) != NULL) {
 ...
 **********************************************************************/
static void ExpandFuncMultifield(void* theEnv, DATA_OBJECT* result,
                                 EXPRESSION* theExp, EXPRESSION** sto,
                                 void* expmult) {
  ...
  while (theExp != NULL) {
    if (theExp->value == expmult) {
      ...
      if (theExp->argList != NULL)
        ExpandFuncMultifield(theEnv, result, theExp->argList, &theExp->argList,
                             expmult);
      sto = &theExp->nextArg;
...
globle intBool SetSORCommand(void* theEnv) {
  ...
  return (EnvSetSequenceOperatorRecognition(
      theEnv, (arg.value == EnvFalseSymbol(theEnv)) ? FALSE : TRUE));
#else
  return (ExpressionData(theEnv)->SequenceOpMode);
#endif
}
...
globle void FuncallFunction(void* theEnv, DATA_OBJECT* returnValue) {
  ...
  if ((argCount = EnvArgCountCheck(theEnv, "funcall", AT_LEAST, 1)) == -1)
    return;
  ...
  if (EnvArgTypeCheck(theEnv, "funcall", 1, SYMBOL_OR_STRING, &theValue) ==
      FALSE) {
    return;
  }
  ...
  if (!GetFunctionReference(theEnv, name, &theReference)) {
    ExpectedTypeError1(theEnv, "funcall", 1,
    ...
    switch (GetType(theValue)) {
      case MULTIFIELD:
        ...
        for (j = GetDOBegin(theValue); j <= GetDOEnd(theValue); j++) {
          nextAdd = GenConstant(theEnv, GetMFType(theMultifield, j),
                                GetMFValue(theMultifield, j));
          if (multiAdd == NULL) {
  ...
  if (theReference.type == PCALL) {
    if (CheckDeffunctionCall(theEnv, theReference.value,
      ...
      EnvPrintRouter(theEnv, WERROR,
                     "Function funcall called with the wrong number of "
...
globle void NewFunction(void* theEnv, DATA_OBJECT* returnValue) {
  ...
  if ((EvaluationData(theEnv)->ExternalAddressTypes[theType] != NULL) &&
      (EvaluationData(theEnv)->ExternalAddressTypes[theType]->newFunction !=
       NULL)) {
    (*EvaluationData(theEnv)->ExternalAddressTypes[theType]->newFunction)(
        theEnv, returnValue);
  }
...
globle void CallFunction(void* theEnv, DATA_OBJECT* returnValue) {
  ...
  if ((argumentCount = EnvArgCountCheck(theEnv, "call", AT_LEAST, 1)) == -1)
    return;
  ...
  if (GetType(theValue) == SYMBOL) {
    ...
    if (theType == -1) {
      ExpectedTypeError1(theEnv, "call", 1,
    ...
    if ((EvaluationData(theEnv)->ExternalAddressTypes[theType] != NULL) &&
        (EvaluationData(theEnv)->ExternalAddressTypes[theType]->callFunction !=
         NULL)) {
      (*EvaluationData(theEnv)->ExternalAddressTypes[theType]->callFunction)(
          theEnv, &theValue, returnValue);
    }
...
/* FindLanguageType:    */
/************************************/
static int FindLanguageType(void* theEnv, const char* languageName) {
  int theType;

  for (theType = 0; theType < EvaluationData(theEnv)->numberOfAddressTypes;
       theType++) {
    if (strcmp(EvaluationData(theEnv)->ExternalAddressTypes[theType]->name,
               languageName) == 0) {
      return (theType);
    }
  }
...
/****************************************/
static void ConvertTime(void* theEnv, DATA_OBJECT_PTR returnValue,
                        struct tm* info) {
...
/*****************************************/
void LocalTimeFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  time_t rawtime;
  struct tm* info;
...
/**************************************/
void GMTimeFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  time_t rawtime;
  struct tm* info;
