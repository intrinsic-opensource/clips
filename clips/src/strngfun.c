...
#if !RUN_TIME
  EnvDefineFunction2(theEnv, "str-cat", 'k', PTIEF StrCatFunction,
                     "StrCatFunction", "1*");
  EnvDefineFunction2(theEnv, "sym-cat", 'k', PTIEF SymCatFunction,
                     "SymCatFunction", "1*");
  EnvDefineFunction2(theEnv, "str-length", 'g', PTIEF StrLengthFunction,
                     "StrLengthFunction", "11j");
  EnvDefineFunction2(theEnv, "str-compare", 'g', PTIEF StrCompareFunction,
                     "StrCompareFunction", "23*jji");
  EnvDefineFunction2(theEnv, "upcase", 'j', PTIEF UpcaseFunction,
                     "UpcaseFunction", "11j");
  EnvDefineFunction2(theEnv, "lowcase", 'j', PTIEF LowcaseFunction,
                     "LowcaseFunction", "11j");
  EnvDefineFunction2(theEnv, "sub-string", 's', PTIEF SubStringFunction,
                     "SubStringFunction", "33*iij");
  EnvDefineFunction2(theEnv, "str-index", 'u', PTIEF StrIndexFunction,
                     "StrIndexFunction", "22j");
  EnvDefineFunction2(theEnv, "eval", 'u', PTIEF EvalFunction, "EvalFunction",
                     "11k");
  EnvDefineFunction2(theEnv, "build", 'b', PTIEF BuildFunction, "BuildFunction",
                     "11k");
  EnvDefineFunction2(theEnv, "string-to-field", 'u',
                     PTIEF StringToFieldFunction, "StringToFieldFunction",
                     "11j");
#else
#if MAC_XCD
...
static void StrOrSymCatFunction(void* theEnv, DATA_OBJECT_PTR returnValue,
  ...
  for (i = 1; i <= numArgs; i++) {
    ...
    switch (GetType(theArg)) {
      ...
      case FLOAT:
        hashPtr = (SYMBOL_HN*)EnvAddSymbol(
            theEnv, FloatToString(theEnv, ValueToDouble(GetValue(theArg))));
        arrayOfStrings[i - 1] = hashPtr;
        IncrementSymbolCount(hashPtr);
        break;
      ...
      case INTEGER:
        hashPtr = (SYMBOL_HN*)EnvAddSymbol(
            theEnv, LongIntegerToString(theEnv, ValueToLong(GetValue(theArg))));
        arrayOfStrings[i - 1] = hashPtr;
        IncrementSymbolCount(hashPtr);
        break;

      default:
        ExpectedTypeError1(theEnv, functionName, i,
...
globle long long StrLengthFunction(void* theEnv) {
  ...
  if (EnvArgTypeCheck(theEnv, "str-length", 1, SYMBOL_OR_STRING, &theArg) ==
      FALSE) {
    return (-1LL);
  }
...
globle void UpcaseFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  if (EnvArgTypeCheck(theEnv, "upcase", 1, SYMBOL_OR_STRING, &theArg) ==
      FALSE) {
    SetpType(returnValue, STRING);
    SetpValue(returnValue, (void*)EnvAddSymbol(theEnv, ""));
    return;
...
globle void LowcaseFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  if (EnvArgTypeCheck(theEnv, "lowcase", 1, SYMBOL_OR_STRING, &theArg) ==
      FALSE) {
    SetpType(returnValue, STRING);
    SetpValue(returnValue, (void*)EnvAddSymbol(theEnv, ""));
    return;
...
globle long long StrCompareFunction(void* theEnv) {
  ...
  if (EnvArgTypeCheck(theEnv, "str-compare", 1, SYMBOL_OR_STRING, &arg1) ==
      FALSE) {
    return (0L);
  }

  if (EnvArgTypeCheck(theEnv, "str-compare", 2, SYMBOL_OR_STRING, &arg2) ==
      FALSE) {
    return (0L);
  }
  ...
  if (numArgs == 3) {
    ...
    returnValue = strncmp(DOToString(arg1), DOToString(arg2), (STD_SIZE)length);
  } else {
...
globle void* SubStringFunction(void* theEnv) {
  ...
  if (EnvArgTypeCheck(theEnv, "sub-string", 1, INTEGER, &theArgument) ==
      FALSE) {
    return ((void*)EnvAddSymbol(theEnv, ""));
  }
  ...
  if (CoerceToLongInteger(theArgument.type, theArgument.value) < 1) {
    ...
    start =
        (size_t)CoerceToLongInteger(theArgument.type, theArgument.value) - 1;
  }

  if (EnvArgTypeCheck(theEnv, "sub-string", 2, INTEGER, &theArgument) ==
      FALSE) {
    return ((void*)EnvAddSymbol(theEnv, ""));
  }
  ...
  if (EnvArgTypeCheck(theEnv, "sub-string", 3, SYMBOL_OR_STRING,
                      &theArgument) == FALSE) {
    return ((void*)EnvAddSymbol(theEnv, ""));
  }
  ...
  /*=============================================*/
    ...
    returnString = (char*)gm2(
        theEnv,
        (unsigned)(end - start + 2)); /* (end - start) inclusive + EOS */
...
globle void StrIndexFunction(void* theEnv, DATA_OBJECT_PTR result) {
  ...
  if (EnvArgTypeCheck(theEnv, "str-index", 1, SYMBOL_OR_STRING,
                      &theArgument1) == FALSE)
    return;

  if (EnvArgTypeCheck(theEnv, "str-index", 2, SYMBOL_OR_STRING,
                      &theArgument2) == FALSE)
    return;
  ...
  if (strlen(strg1) == 0) {
    result->type = INTEGER;
    result->value =
        (void*)EnvAddLong(theEnv, (long long)UTF8Length(strg2) + 1LL);
    return;
  }

  strg3 = strg2;
  for (i = 1; *strg2; i++, strg2++) {
    for (j = 0; *(strg1 + j) && *(strg1 + j) == *(strg2 + j);
         j++) { /* Do Nothing */
    }
    ...
    if (*(strg1 + j) == '\0') {
      result->type = INTEGER;
      result->value =
          (void*)EnvAddLong(theEnv, (long long)UTF8CharNum(strg3, i));
      return;
    }
  }
...
globle void StringToFieldFunction(void* theEnv, DATA_OBJECT* returnValue) {
  ...
  if (EnvArgTypeCheck(theEnv, "string-to-field", 1, SYMBOL_OR_STRING,
                      &theArg) == FALSE) {
    returnValue->type = STRING;
    returnValue->value = (void*)EnvAddSymbol(theEnv, "*** ERROR ***");
    return;
...
/*************************************************************/
globle void StringToField(void* theEnv, const char* theString,
...
globle int Eval(const char* theString, DATA_OBJECT_PTR returnValue) {
...
/*   for the eval function.  */
/*****************************/
globle int EnvEval(void* theEnv, const char* theString,
  ...
  if ((top->type == MF_GBL_VARIABLE) || (top->type == MF_VARIABLE)) {
    ...
    EnvPrintRouter(
        theEnv, WERROR,
        "expand$ must be used in the argument list of a function call.\n");
    SetEvaluationError(theEnv, TRUE);
    CloseStringSource(theEnv, logicalNameBuffer);
    SetpType(returnValue, SYMBOL);
    SetpValue(returnValue, EnvFalseSymbol(theEnv));
  ...
  if (ExpressionContainsVariables(top, FALSE)) {
    ...
    EnvPrintRouter(
        theEnv, WERROR,
        "Some variables could not be accessed by the eval function.\n");
    SetEvaluationError(theEnv, TRUE);
    CloseStringSource(theEnv, logicalNameBuffer);
    SetpType(returnValue, SYMBOL);
    SetpValue(returnValue, EnvFalseSymbol(theEnv));
...
globle void EvalFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  EnvPrintRouter(theEnv, WERROR,
                 "Function eval does not work in run time modules.\n");
  SetpType(returnValue, SYMBOL);
  SetpValue(returnValue, EnvFalseSymbol(theEnv));
}
...
/*****************************/
#if ALLOW_ENVIRONMENT_GLOBALS
globle int Build(const char* theString) {
...
/*   for the build function.  */
/******************************/
globle int EnvBuild(void* theEnv, const char* theString) {
