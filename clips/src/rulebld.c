...
static struct joinNode* FindShareableJoin(struct joinLink*, struct joinNode*,
                                          intBool, void*, unsigned, unsigned,
                                          unsigned, unsigned, struct expr*,
                                          struct expr*, struct expr*,
                                          struct expr*);
static int TestJoinForReuse(struct joinNode*, unsigned, unsigned, unsigned,
                            unsigned, struct expr*, struct expr*, struct expr*,
                            struct expr*);
static struct joinNode* CreateNewJoin(void*, struct expr*, struct expr*,
                                      struct joinNode*, void*, int, int, int,
                                      struct expr*, struct expr*);
...
/****************************************************************/
globle struct joinNode* ConstructJoins(void* theEnv, int logicalJoin,
                                       struct lhsParseNode* theLHS,
                                       int startDepth,
                                       struct joinNode* lastJoin,
                                       int tryToReuse, int firstJoin) {
  ...
  if (theLHS == NULL) {
    lastJoin = FindShareableJoin(DefruleData(theEnv)->RightPrimeJoins, NULL,
                                 TRUE, NULL, TRUE, FALSE, FALSE, FALSE, NULL,
                                 NULL, NULL, NULL);

    if (lastJoin == NULL) {
      lastJoin = CreateNewJoin(theEnv, NULL, NULL, NULL, NULL, FALSE, FALSE,
                               FALSE, NULL, NULL);
    }
  }
  ...
  while (theLHS != NULL) {
    ...
    if (theLHS->beginNandDepth > startDepth) {
      ...
      lastRightJoin =
          ConstructJoins(theEnv, logicalJoin, theLHS, startDepth + 1, lastJoin,
                         tryToReuse, firstJoin);
    ...
    if ((tryToReuse == TRUE) &&
        ((oldJoin = FindShareableJoin(
              theLinks, listOfJoins, useLinks, rhsStruct, firstJoin,
              theLHS->negated, isExists, isLogical, networkTest,
              secondaryNetworkTest, leftHash, rightHash)) != NULL)) {
      ...
      tryToReuse = FALSE;
      if (!joinFromTheRight) {
        lastJoin = CreateNewJoin(
            theEnv, networkTest, secondaryNetworkTest, lastJoin, lastPattern,
            FALSE, (int)theLHS->negated, isExists, leftHash, rightHash);
        lastJoin->rhsType = rhsType;
      } else {
        lastJoin = CreateNewJoin(
            theEnv, networkTest, secondaryNetworkTest, lastJoin, lastRightJoin,
            TRUE, (int)theLHS->negated, isExists, leftHash, rightHash);
        lastJoin->rhsType = rhsType;
      }
    }
  ...
  if (startDepth == 1) {
    lastJoin = CreateNewJoin(theEnv, NULL, NULL, lastJoin, NULL, FALSE, FALSE,
                             FALSE, NULL, NULL);
  }
...
#if DEBUGGING_FUNCTIONS
  if ((startDepth == 1) && (EnvGetWatchItem(theEnv, "compilations") == TRUE) &&
      GetPrintWhileLoading(theEnv)) {
...
/****************************************************************/
globle void AttachTestCEsToPatternCEs(void* theEnv,
                                      struct lhsParseNode* theLHS) {
  ...
  while (theLHS != NULL) {
    ...
    if (theLHS->beginNandDepth == theLHS->endNandDepth) {
      ...
      if (lastNode->negated) {
        lastNode->secondaryNetworkTest = CombineExpressions(
            theEnv, lastNode->secondaryNetworkTest, theLHS->networkTest);
      } else {
        lastNode->networkTest = CombineExpressions(
            theEnv, lastNode->networkTest, theLHS->networkTest);
      }
    }
    ...
    /*=========*/
      ...
      if ((lastNode->negated == FALSE) && (lastNode->existsNand == FALSE)) {
        ...
        lastNode->networkTest = CombineExpressions(
            theEnv, lastNode->networkTest, lastNode->externalNetworkTest);
        lastNode->networkTest = CombineExpressions(
            theEnv, lastNode->networkTest, theLHS->networkTest);
      ...
      else if ((lastNode->negated == TRUE) && (lastNode->exists == FALSE) &&
      ...
      else if ((lastNode->negated == TRUE) && (lastNode->exists == TRUE) &&
               (lastNode->existsNand == FALSE)) {
        lastNode->secondaryNetworkTest = CombineExpressions(
            theEnv, lastNode->secondaryNetworkTest, theLHS->networkTest);
      }
      ...
      else if ((lastNode->negated == FALSE) && (lastNode->exists == FALSE) &&
      ...
      /*======================================*/
        ...
        lastNode->secondaryNetworkTest =
            CombineExpressions(theEnv, lastNode->secondaryNetworkTest,
...
static struct joinNode* FindShareableJoin(
    struct joinLink* theLinks, struct joinNode* listOfJoins, intBool useLinks,
    void* rhsStruct, unsigned int firstJoin, unsigned int negatedRHS,
    unsigned int existsRHS, unsigned int isLogical, struct expr* joinTest,
    struct expr* secondaryJoinTest, struct expr* leftHash,
  ...
  while (listOfJoins != NULL) {
    ...
    if (listOfJoins->rightSideEntryStructure == rhsStruct) {
      if (TestJoinForReuse(listOfJoins, firstJoin, negatedRHS, existsRHS,
                           isLogical, joinTest, secondaryJoinTest, leftHash,
                           rightHash)) {
        return (listOfJoins);
      }
    }
...
/**************************************************************/
static int TestJoinForReuse(struct joinNode* testJoin, unsigned firstJoin,
                            unsigned negatedRHS, unsigned existsRHS,
                            unsigned int isLogical, struct expr* joinTest,
  ...
  if ((isLogical == TRUE) && (testJoin->logicalJoin == FALSE) &&
  ...
  if (IdenticalExpression(testJoin->secondaryNetworkTest, secondaryJoinTest) !=
      TRUE) {
    return (FALSE);
  }
...
static struct joinNode* CreateNewJoin(
    void* theEnv, struct expr* joinTest, struct expr* secondaryJoinTest,
    struct joinNode* lhsEntryStruct, void* rhsEntryStruct, int joinFromTheRight,
    int negatedRHSPattern, int existsRHSPattern, struct expr* leftHash,
    struct expr* rightHash) {
  ...
  if ((lhsEntryStruct != NULL) || existsRHSPattern || negatedRHSPattern ||
    ...
    if (leftHash == NULL) {
      ...
      newJoin->leftMemory->beta =
          (struct partialMatch**)genalloc(theEnv, sizeof(struct partialMatch*));
      ...
      newJoin->leftMemory = get_struct(theEnv, betaMemory);
      newJoin->leftMemory->beta = (struct partialMatch**)genalloc(
          theEnv, sizeof(struct partialMatch*) * INITIAL_BETA_HASH_SIZE);
  ...
  if (joinFromTheRight) {
    if (leftHash == NULL) {
      ...
      newJoin->rightMemory = get_struct(theEnv, betaMemory);
      newJoin->rightMemory->beta = (struct partialMatch**)genalloc(
          theEnv, sizeof(struct partialMatch*) * INITIAL_BETA_HASH_SIZE);
      newJoin->rightMemory->last = (struct partialMatch**)genalloc(
          theEnv, sizeof(struct partialMatch*) * INITIAL_BETA_HASH_SIZE);
  ...
  newJoin->secondaryNetworkTest =
      AddHashedExpression(theEnv, secondaryJoinTest);
