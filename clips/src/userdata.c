...
globle void InitializeUserDataData(void* theEnv) {
  AllocateEnvironmentData(theEnv, USER_DATA_DATA, sizeof(struct userDataData),
                          NULL);
}
...
/*****************************************************/
globle struct userData* FetchUserData(void* theEnv, unsigned char userDataID,
  ...
  theData = (struct userData*)(*UserDataData(theEnv)
...
/*   is not found.                                   */
/*****************************************************/
globle struct userData* TestUserData(unsigned char userDataID,
                                     struct userData* theList) {
  struct userData* theData;
...
globle void ClearUserDataList(void* theEnv, struct userData* theList) {
  ...
  while (theList != NULL) {
    ...
    (*UserDataData(theEnv)
          ->UserDataRecordArray[theList->dataID]
...
/*************************************************/
globle struct userData* DeleteUserData(void* theEnv, unsigned char userDataID,
                                       struct userData* theList) {
  ...
  for (theData = theList; theData != NULL; theData = theData->next) {
    if (theData->dataID == userDataID) {
      ...
      (*UserDataData(theEnv)->UserDataRecordArray[userDataID]->deleteUserData)(
          theEnv, theData);
      return (theList);
    }
