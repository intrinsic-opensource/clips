...
/***************************************************************/
globle void InitializeFileRouter(void* theEnv) {
  AllocateEnvironmentData(theEnv, FILE_ROUTER_DATA,
                          sizeof(struct fileRouterData),
                          DeallocateFileRouterData);

  EnvAddRouter(theEnv, "fileio", 0, FindFile, PrintFile, GetcFile, UngetcFile,
               ExitFile);
}
...
/*****************************************/
static void DeallocateFileRouterData(void* theEnv) {
...
/*****************************************/
globle FILE* FindFptr(void* theEnv, const char* logicalName) {
  ...
  while ((fptr != NULL) ? (strcmp(logicalName, fptr->logicalName) != 0)
                        : FALSE) {
    fptr = fptr->next;
  }
...
/*****************************************************/
globle int FindFile(void* theEnv, const char* logicalName) {
...
/********************************************/
static int ExitFile(void* theEnv, int num) {
#if MAC_XCD
#pragma unused(num)
#endif
#if IO_FUNCTIONS
...
/*********************************************/
static int PrintFile(void* theEnv, const char* logicalName, const char* str) {
  FILE* fptr;
...
/*******************************************/
static int GetcFile(void* theEnv, const char* logicalName) {
  FILE* fptr;
  int theChar;
...
/***********************************************/
static int UngetcFile(void* theEnv, int ch, const char* logicalName) {
  FILE* fptr;
...
/*********************************************************/
globle int OpenAFile(void* theEnv, const char* fileName, const char* accessMode,
...
/*************************************************************/
globle int CloseFile(void* theEnv, const char* fid) {
...
/**********************************************/
globle int CloseAllFiles(void* theEnv) {
  struct fileRouter *fptr, *prev;
