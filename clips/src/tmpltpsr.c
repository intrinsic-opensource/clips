...
static struct templateSlot* ParseSlot(void*, const char*, struct token*,
                                      struct templateSlot*);
static struct templateSlot* DefinedSlots(void*, const char*, SYMBOL_HN*, int,
                                         struct token*);
static intBool ParseFacetAttribute(void*, const char*, struct templateSlot*,
                                   intBool);
#endif
...
/*******************************************************/
globle int ParseDeftemplate(void* theEnv, const char* readSource) {
#if (!RUN_TIME) && (!BLOAD_ONLY)
  ...
  deftemplateName =
      GetConstructNameAndComment(theEnv, readSource, &inputToken, "deftemplate",
                                 EnvFindDeftemplateInModule, EnvUndeftemplate,
                                 "%", TRUE, TRUE, TRUE, FALSE);
  if (deftemplateName == NULL) return (TRUE);

  if (ReservedPatternSymbol(theEnv, ValueToString(deftemplateName),
                            "deftemplate")) {
  ...
  newDeftemplate->header.whichModule =
      (struct defmoduleItemHeader*)GetModuleItem(
          theEnv, NULL, DeftemplateData(theEnv)->DeftemplateModuleIndex);
...
globle void InstallDeftemplate(void* theEnv,
  ...
  for (slotPtr = theDeftemplate->slotList; slotPtr != NULL;
...
/********************************************************************/
static struct templateSlot* SlotDeclarations(void* theEnv,
...
/*****************************************************/
static struct templateSlot* ParseSlot(void* theEnv, const char* readSource,
                                      struct token* inputToken,
  ...
  while (slotList != NULL) {
    if (slotList->slotName == slotName) {
      AlreadyParsedErrorMessage(theEnv, "slot ",
                                ValueToString(slotList->slotName));
      DeftemplateData(theEnv)->DeftemplateError = TRUE;
      return (NULL);
    }
  ...
  if ((newSlot->defaultPresent) || (newSlot->defaultDynamic)) {
    rv = ConstraintCheckExpressionChain(theEnv, newSlot->defaultList,
                                        newSlot->constraints);
  } else {
    rv = NO_VIOLATION;
  }
...
/**************************************************************/
static struct templateSlot* DefinedSlots(void* theEnv, const char* readSource,
                                         SYMBOL_HN* slotName,
                                         int multifieldSlot,
                                         struct token* inputToken) {
  struct templateSlot* newSlot;
  ...
  while (inputToken->type != RPAREN) {
    ...
    if (StandardConstraint(ValueToString(inputToken->value))) {
      if (ParseStandardConstraint(theEnv, readSource,
                                  (ValueToString(inputToken->value)),
    ...
    /*=================================================*/
             ...
             (strcmp(ValueToString(inputToken->value), "default-dynamic") ==
              0)) {
      /*======================================================*/
      ...
      defaultList = ParseDefault(theEnv, readSource, multifieldSlot,
                                 (int)newSlot->defaultDynamic, TRUE,
                                 &noneSpecified, &deriveSpecified,
...
/***************************************************/
static intBool ParseFacetAttribute(void* theEnv, const char* readSource,
  ...
  if (inputToken.type != SYMBOL) {
    if (multifacet)
      SyntaxErrorMessage(theEnv, "multifacet attribute");
    else
  ...
  for (tempFacet = theSlot->facetList; tempFacet != NULL;
    ...
    if (tempFacet->value == facetName) {
      if (multifacet)
        AlreadyParsedErrorMessage(theEnv, "multifacet ",
                                  ValueToString(facetName));
      else
        AlreadyParsedErrorMessage(theEnv, "facet ", ValueToString(facetName));
      return (FALSE);
    }
  }
  ...
  if (multifacet) {
    facetPair->argList =
        GenConstant(theEnv, FCALL, (void*)FindFunction(theEnv, "create$"));
