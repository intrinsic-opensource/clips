...
globle void InitializePrettyPrintData(void* theEnv) {
  AllocateEnvironmentData(theEnv, PRETTY_PRINT_DATA,
...
/******************************************************/
static void DeallocatePrettyPrintData(void* theEnv) {
  if (PrettyPrintData(theEnv)->PrettyPrintBuffer != NULL) {
    rm(theEnv, PrettyPrintData(theEnv)->PrettyPrintBuffer,
...
/*********************************************/
globle void SavePPBuffer(void* theEnv, const char* str) {
  size_t increment;
  ...
  PrettyPrintData(theEnv)->PPBackupTwice =
      PrettyPrintData(theEnv)->PPBackupOnce;
  ...
  PrettyPrintData(theEnv)->PrettyPrintBuffer =
      AppendToString(theEnv, str, PrettyPrintData(theEnv)->PrettyPrintBuffer,
...
globle char* CopyPPBuffer(void* theEnv) {
  ...
  length = (1 + strlen(PrettyPrintData(theEnv)->PrettyPrintBuffer)) *
           (int)sizeof(char);
...
globle void PPCRAndIndent(void* theEnv) {
  ...
  if ((PrettyPrintData(theEnv)->IndentationDepth + 2) >
    ...
    buffer =
        (char*)genalloc(theEnv, PrettyPrintData(theEnv)->IndentationDepth + 2);
  } else {
    buffer = fixedBuffer;
  }
