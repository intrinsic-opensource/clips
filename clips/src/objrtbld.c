...
static struct patternNodeHeader* PlaceObjectPattern(void*,
                                                    struct lhsParseNode*);
static OBJECT_PATTERN_NODE* FindObjectPatternNode(OBJECT_PATTERN_NODE*,
                                                  struct lhsParseNode*,
                                                  OBJECT_PATTERN_NODE**,
                                                  unsigned, unsigned);
static OBJECT_PATTERN_NODE* CreateNewObjectPatternNode(void*,
                                                       struct lhsParseNode*,
                                                       OBJECT_PATTERN_NODE*,
                                                       OBJECT_PATTERN_NODE*,
                                                       unsigned, unsigned);
...
static void RemoveObjectPartialMatches(void*, INSTANCE_TYPE*,
                                       struct patternNodeHeader*);
static intBool CheckDuplicateSlots(void*, struct lhsParseNode*, SYMBOL_HN*);
static struct lhsParseNode* ParseClassRestriction(void*, const char*,
                                                  struct token*);
static struct lhsParseNode* ParseNameRestriction(void*, const char*,
                                                 struct token*);
static struct lhsParseNode* ParseSlotRestriction(void*, const char*,
                                                 struct token*,
...
static intBool ProcessClassRestriction(void*, CLASS_BITMAP*,
                                       struct lhsParseNode**, int);
static CONSTRAINT_RECORD* ProcessSlotRestriction(void*, CLASS_BITMAP*,
                                                 SYMBOL_HN*, int*);
...
static struct lhsParseNode* RemoveSlotExistenceTests(void*,
                                                     struct lhsParseNode*,
                                                     BITMAP_HN**);
...
static void MarkObjectPtnIncrementalReset(void*, struct patternNodeHeader*,
                                          int);
...
/********************************************************
  ...
  NOTES        : None
 ********************************************************/
globle void SetupObjectPatternStuff(void* theEnv) {
#if (!BLOAD_ONLY) && (!RUN_TIME)
  ...
  AddFunctionParser(theEnv, "object-pattern-match-delay",
                    ObjectMatchDelayParse);
...
static void ResetInitialObject(void* theEnv) {
  ...
  tmp->argList =
      GenConstant(theEnv, INSTANCE_NAME,
                  (void*)DefclassData(theEnv)->INITIAL_OBJECT_SYMBOL);
  tmp->argList->nextArg = GenConstant(
      theEnv, DEFCLASS_PTR,
      (void*)LookupDefclassInScope(theEnv, INITIAL_OBJECT_CLASS_NAME));
 ...
 *****************************************************/
static intBool PatternParserFind(SYMBOL_HN* value) {
...
/************************************************************************************
  ...
  NOTES        : Object Pattern Syntax:
                 (object [<class-constraint>] [<name-constraint>]
 <slot-constraint>*) <class-constraint> ::= (is-a <constraint>)
                 <name-constraint> ::= (name <constraint>)
                 <slot-constraint> ::= (<slot-name> <constraint>*)
 ************************************************************************************/
static struct lhsParseNode* ObjectLHSParse(void* theEnv, const char* readSource,
...
#pragma unused(lastToken)
  ...
  /* ========================================================
     ...
     ======================================================== */
  clsset =
      NewClassBitMap(theEnv, ((int)DefclassData(theEnv)->MaxClassID) - 1, 1);
  ...
  while (theToken.type != RPAREN) {
    ...
    if (theToken.value == (void*)DefclassData(theEnv)->ISA_SYMBOL) {
      ...
      if (tmpNode == NULL) goto ObjectLHSParseERROR;
      ...
      slotConstraints = ProcessSlotRestriction(
          theEnv, clsset, (SYMBOL_HN*)theToken.value, &multip);
      if (slotConstraints != NULL) {
        InitializeClassBitMap(theEnv, tmpset, 1);
        tmpNode = ParseSlotRestriction(theEnv, readSource, &theToken,
  ...
  firstNode->userData =
      EnvAddBitMap(theEnv, (void*)clsset, ClassBitMapSize(clsset));
...
/**************************************************************
  ...
  NOTES        : Adds a default is-a slot if one does not
                 already exist
 **************************************************************/
static intBool ReorderAndAnalyzeObjectPattern(void* theEnv,
  ...
  if ((isa_node == NULL)
    ...
    while (rexp != NULL) {
      ...
      if (cls != NULL) {
        if ((cls->id <= (unsigned)clsset->maxid)
                ? TestBitMap(clsset->map, cls->id)
                : FALSE)
  ...
  while (tmpNode != bitmap_node) {
    ...
    for (i = 0; i <= tmpset->maxid; i++)
      if (TestBitMap(tmpset->map, i)) {
        ...
        sd = cls->instanceTemplate[FindInstanceTemplateSlot(theEnv, cls,
                                                            tmpNode->slot)];
        ...
        /* =========================================
           ...
           ========================================= */
        crossConstraints =
            IntersectConstraints(theEnv, tmpNode->constraints, sd->constraint);
        ...
        if (incompatibleConstraint) {
          ...
          for (subNode = tmpNode->bottom; subNode != NULL;
            ...
            /* ========================================================
               ...
               ======================================================== */
            if ((subNode->type == MF_WILDCARD) ||
                (subNode->type == MF_VARIABLE)) {
            ...
            crossConstraints =
                IntersectConstraints(theEnv, theConstraint, sd->constraint);
...
static struct patternNodeHeader* PlaceObjectPattern(
    void* theEnv, struct lhsParseNode* thePattern) {
  ...
  do {
    ...
    newNode = FindObjectPatternNode(currentLevel, thePattern, &nodeSlotGroup,
                                    endSlot, FALSE);
    ...
    if (newNode == NULL) {
      newNode = CreateNewObjectPatternNode(theEnv, thePattern, nodeSlotGroup,
  ...
  if (newSlotBitMap != NULL) IncrementBitMapCount(newSlotBitMap);
...
static OBJECT_PATTERN_NODE* FindObjectPatternNode(
    OBJECT_PATTERN_NODE* listOfNodes, struct lhsParseNode* thePattern,
    OBJECT_PATTERN_NODE** nodeSlotGroup, unsigned endSlot,
...
static OBJECT_PATTERN_NODE* CreateNewObjectPatternNode(
    void* theEnv, struct lhsParseNode* thePattern,
    OBJECT_PATTERN_NODE* nodeSlotGroup, OBJECT_PATTERN_NODE* upperLevel,
    unsigned endSlot, unsigned constantSelector) {
  ...
  if (constantSelector) {
    newNode->networkTest =
        AddHashedExpression(theEnv, thePattern->constantValue);
  } else if (thePattern->constantSelector != NULL) {
    newNode->networkTest =
        AddHashedExpression(theEnv, thePattern->constantSelector);
  } else {
  ...
  if ((upperLevel != NULL) && (upperLevel->selector)) {
    AddHashedPatternNode(theEnv, upperLevel, newNode,
  ...
  if (nodeSlotGroup == NULL) {
    ...
    if (newNode->rightNode != NULL) newNode->rightNode->leftNode = newNode;
    return (newNode);
  }
  ...
  while ((curNode == NULL)
    ...
    if ((curNode->networkTest == NULL)
            ? FALSE
            : ((curNode->networkTest->type != OBJ_PN_CONSTANT)
                   ? FALSE
                   : ((struct ObjectCmpPNConstant*)ValueToBitMap(
                          curNode->networkTest->value))
                         ->pass))
      break;
    prvNode = curNode;
...
/********************************************************
  ...
  NOTES        : None
 ********************************************************/
static void DetachObjectPattern(void* theEnv,
  ...
  if (!ConstructData(theEnv)->ClearInProgress) {
    ...
    for (i = 0; i <= cbmp->maxid; i++) {
      if (TestBitMap(cbmp->map, i)) {
        ...
        for (lastAlpha = NULL,
            alphaLink = relevantDefclass->relevant_terminal_alpha_nodes;
             alphaLink != NULL;
  ...
  if (upperLevel->nextLevel != NULL) return;
    ...
    if ((upperLevel->leftNode == NULL) && (upperLevel->rightNode == NULL)) {
      ...
      if (upperLevel == NULL)
        ...
        if (upperLevel->selector) {
          RemoveHashedPatternNode(theEnv, upperLevel, patternPtr,
                                  patternPtr->networkTest->type,
                                  patternPtr->networkTest->value);
        }

        upperLevel->nextLevel = NULL;
        if (upperLevel->alphaNode != NULL) upperLevel = NULL;
      }
      ...
      if ((patternPtr->lastLevel != NULL) &&
        ...
        RemoveHashedPatternNode(theEnv, patternPtr->lastLevel, patternPtr,
                                patternPtr->networkTest->type,
                                patternPtr->networkTest->value);
      }
 ...
 ***************************************************/
static void ClearObjectPatternMatches(void* theEnv,
  ...
  while (ins != NULL) {
    RemoveObjectPartialMatches(theEnv, (INSTANCE_TYPE*)ins,
  ...
  while (igrb != NULL) {
    RemoveObjectPartialMatches(theEnv, (INSTANCE_TYPE*)igrb->ins,
                               (struct patternNodeHeader*)alphaPtr);
    igrb = igrb->nxt;
  }
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void RemoveObjectPartialMatches(void* theEnv, INSTANCE_TYPE* ins,
...
/******************************************************
  ...
  NOTES        : None
 ******************************************************/
static intBool CheckDuplicateSlots(void* theEnv, struct lhsParseNode* nodeList,
                                   SYMBOL_HN* slotName) {
  while (nodeList != NULL) {
...
/**********************************************************
  ...
  NOTES        : None
 **********************************************************/
static struct lhsParseNode* ParseClassRestriction(void* theEnv,
  ...
  GetToken(theEnv, readSource, theToken);
  tmpNode =
      RestrictionParse(theEnv, readSource, theToken, FALSE, rln, ISA_ID, rv, 0);
  if (tmpNode == NULL) {
    RemoveConstraint(theEnv, rv);
    return (NULL);
  }
  if ((theToken->type != RPAREN) || (tmpNode->type == MF_WILDCARD) ||
...
/**********************************************************
  ...
  NOTES        : None
 **********************************************************/
static struct lhsParseNode* ParseNameRestriction(void* theEnv,
                                                 const char* readSource,
                                                 struct token* theToken) {
  struct lhsParseNode* tmpNode;
  SYMBOL_HN* rln;
  ...
  SavePPBuffer(theEnv, " ");
  GetToken(theEnv, readSource, theToken);
  tmpNode = RestrictionParse(theEnv, readSource, theToken, FALSE, rln, NAME_ID,
                             rv, 0);
  if (tmpNode == NULL) {
    RemoveConstraint(theEnv, rv);
    return (NULL);
...
static struct lhsParseNode* ParseSlotRestriction(
    void* theEnv, const char* readSource, struct token* theToken,
    CONSTRAINT_RECORD* slotConstraints, int multip) {
  struct lhsParseNode* tmpNode;
  SYMBOL_HN* slotName;
  ...
  GetToken(theEnv, readSource, theToken);
  tmpNode =
      RestrictionParse(theEnv, readSource, theToken, multip, slotName,
                       FindSlotNameID(theEnv, slotName), slotConstraints, 1);
  if (tmpNode == NULL) {
...
/********************************************************
  ...
  NOTES        : None
 ********************************************************/
static CLASS_BITMAP* NewClassBitMap(void* theEnv, int maxid, int set) {
  CLASS_BITMAP* bmp;
  unsigned size;
...
/***********************************************************
  ...
  NOTES        : None
 ***********************************************************/
static void InitializeClassBitMap(void* theEnv, CLASS_BITMAP* bmp, int set) {
  int i, bytes;
  DEFCLASS* cls;
  ...
  if (set) {
    ...
    for (i = 0; i <= (int)bmp->maxid; i++) {
      ...
      if ((cls != NULL) ? DefclassInScope(theEnv, cls, currentModule) : FALSE) {
        if (cls->reactive && (cls->abstract == 0)) SetBitMap(bmp->map, i);
      }
    }
  }
}
...
/********************************************
  ...
  NOTES        : None
 ********************************************/
static void DeleteIntermediateClassBitMap(void* theEnv, CLASS_BITMAP* bmp) {
 ...
 ******************************************************/
static void* CopyClassBitMap(void* theEnv, void* gset) {
#if MAC_XCD
#pragma unused(theEnv)
#endif

  if (gset != NULL) IncrementBitMapCount(gset);
  return (gset);
}
...
/**********************************************************
  ...
  NOTES        : None
 **********************************************************/
static void DeleteClassBitMap(void* theEnv, void* gset) {
  if (gset == NULL) return;
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void MarkBitMapClassesBusy(void* theEnv, BITMAP_HN* bmphn, int offset) {
  CLASS_BITMAP* bmp;
  unsigned short i;
  DEFCLASS* cls;
  ...
  /* ====================================
     ...
     ==================================== */
  if (ConstructData(theEnv)->ClearInProgress) return;
...
/****************************************************
  ...
  NOTES        : None
 ****************************************************/
static intBool EmptyClassBitMap(CLASS_BITMAP* bmp) {
  unsigned short bytes;

  bytes = (unsigned short)(bmp->maxid / BITS_PER_BYTE + 1);
  while (bytes > 0) {
    if (bmp->map[bytes - 1] != (char)0) return (FALSE);
    bytes--;
  }
  return (TRUE);
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static intBool IdenticalClassBitMap(CLASS_BITMAP* cs1, CLASS_BITMAP* cs2) {
  int i;

  if (cs1->maxid != cs2->maxid) return (FALSE);
  for (i = 0; i < (int)(cs1->maxid / BITS_PER_BYTE + 1); i++)
    if (cs1->map[i] != cs2->map[i]) return (FALSE);
  return (TRUE);
}
...
/*****************************************************************
  ...
  NOTES        : None
 *****************************************************************/
static intBool ProcessClassRestriction(void* theEnv, CLASS_BITMAP* clsset,
  ...
  if (*classRestrictions == NULL) {
    if (recursiveCall) InitializeClassBitMap(theEnv, clsset, 1);
    return (TRUE);
  }
  ...
  tmpset1 =
      NewClassBitMap(theEnv, ((int)DefclassData(theEnv)->MaxClassID) - 1, 1);
  tmpset2 =
      NewClassBitMap(theEnv, ((int)DefclassData(theEnv)->MaxClassID) - 1, 0);
...
/****************************************************************
  ...
  NOTES        : None
 ****************************************************************/
static CONSTRAINT_RECORD* ProcessSlotRestriction(void* theEnv,
                                                 CLASS_BITMAP* clsset,
                                                 SYMBOL_HN* slotName,
                                                 int* multip) {
  DEFCLASS* cls;
    ...
    for (cls = DefclassData(theEnv)->ClassTable[i]; cls != NULL;
      ...
      if (TestBitMap(clsset->map, cls->id)) {
        ...
        if ((si != -1) ? cls->instanceTemplate[si]->reactive : FALSE) {
          if (cls->instanceTemplate[si]->multiple) *multip = TRUE;
          tmpConstraints = UnionConstraints(
              theEnv, cls->instanceTemplate[si]->constraint, totalConstraints);
 ...
 ****************************************************/
static void IntersectClassBitMaps(CLASS_BITMAP* cs1, CLASS_BITMAP* cs2) {
  unsigned short bytes;
...
/****************************************************
  ...
  NOTES        : Assumes the first bitmap is at least
                 as large as the second
 ****************************************************/
static void UnionClassBitMaps(CLASS_BITMAP* cs1, CLASS_BITMAP* cs2) {
  unsigned short bytes;
...
/*****************************************************
  ...
  NOTES        : None
 *****************************************************/
static CLASS_BITMAP* PackClassBitMap(void* theEnv, CLASS_BITMAP* oldset) {
  unsigned short newmaxid;
  CLASS_BITMAP* newset;

  for (newmaxid = oldset->maxid; newmaxid > 0; newmaxid--)
    if (TestBitMap(oldset->map, newmaxid)) break;
...
static struct lhsParseNode* FilterObjectPattern(
    void* theEnv, struct patternParser* selfPatternType,
    struct lhsParseNode* unfilteredSlots, struct lhsParseNode** bitmap_slot,
    struct lhsParseNode** isa_slot, struct lhsParseNode** name_slot) {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static BITMAP_HN* FormSlotBitMap(void* theEnv,
  ...
  for (node = thePattern; node != NULL; node = node->right)
    if (node->slotNumber > maxSlotID) maxSlotID = node->slotNumber;
  ...
  /* ===================================================
     ...
     store a slot bitmap
     =================================================== */
  if ((maxSlotID == ISA_ID) || (maxSlotID == NAME_ID)) return (NULL);
...
static struct lhsParseNode* RemoveSlotExistenceTests(
    void* theEnv, struct lhsParseNode* thePattern, BITMAP_HN** bmp) {
...
/***************************************************
  ...
  NOTES        : The pattern created is:
                 (object (is-a INITIAL-OBJECT)
                         (name [initial-object]))
 ***************************************************/
static struct lhsParseNode* CreateInitialObjectPattern(void* theEnv) {
  struct lhsParseNode* topNode;
  CLASS_BITMAP* clsset;
  int initialObjectClassID;

  initialObjectClassID =
      LookupDefclassInScope(theEnv, INITIAL_OBJECT_CLASS_NAME)->id;
...
/**************************************************************
  ...
  NOTES        : None
 **************************************************************/
static EXPRESSION* ObjectMatchDelayParse(void* theEnv, struct expr* top,
                                         const char* infile) {
  struct token tkn;
 ...
 ***************************************************/
static void MarkObjectPtnIncrementalReset(void* theEnv,
                                          struct patternNodeHeader* thePattern,
                                          int value) {
#if MAC_XCD
#pragma unused(theEnv)
  ...
  if (thePattern->initialize == FALSE) return;
...
/***********************************************************
  ...
  NOTES        : None
 ***********************************************************/
static void ObjectIncrementalReset(void* theEnv) {
  INSTANCE_TYPE* ins;
