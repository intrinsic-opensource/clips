...
#define ParseFunctionData(theEnv) \
  ((struct parseFunctionData*)GetEnvironmentData(theEnv, PARSEFUN_DATA))
...
globle void ParseFunctionDefinitions(void* theEnv) {
  AllocateEnvironmentData(theEnv, PARSEFUN_DATA,
                          sizeof(struct parseFunctionData), NULL);

#if !RUN_TIME
  EnvDefineFunction2(theEnv, "check-syntax", 'u', PTIEF CheckSyntaxFunction,
                     "CheckSyntaxFunction", "11s");
#endif
}
...
/*********************************/
globle int CheckSyntax(void* theEnv, const char* theString,
  ...
  if (theToken.type != SYMBOL) {
    CloseStringSource(theEnv, "check-syntax");
    SetpValue(returnValue,
              EnvAddSymbol(theEnv, "EXPECTED-SYMBOL-AFTER-LEFT-PARENTHESIS"));
    return (TRUE);
  }
  ...
  EnvAddRouter(theEnv, "error-capture", 40, FindErrorCapture, PrintErrorCapture,
               NULL, NULL, NULL);
  ...
  if (FindConstruct(theEnv, name)) {
    ...
    if (theToken.type != STOP) {
      SetpValue(
          returnValue,
          EnvAddSymbol(theEnv, "EXTRANEOUS-INPUT-AFTER-LAST-PARENTHESIS"));
      DeactivateErrorCapture(theEnv);
      return (TRUE);
    }
...
/**************************************************/
static void DeactivateErrorCapture(void* theEnv) {
  if (ParseFunctionData(theEnv)->ErrorString != NULL) {
    rm(theEnv, ParseFunctionData(theEnv)->ErrorString,
  ...
  if (ParseFunctionData(theEnv)->WarningString != NULL) {
    rm(theEnv, ParseFunctionData(theEnv)->WarningString,
...
/******************************************************************/
static void SetErrorCaptureValues(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  if (ParseFunctionData(theEnv)->ErrorString != NULL) {
    ...
    SetMFValue(theMultifield, 1,
               EnvAddSymbol(theEnv, ParseFunctionData(theEnv)->ErrorString));
  } else {
  ...
  if (ParseFunctionData(theEnv)->WarningString != NULL) {
    ...
    SetMFValue(theMultifield, 2,
               EnvAddSymbol(theEnv, ParseFunctionData(theEnv)->WarningString));
  } else {
...
/**********************************/
static int FindErrorCapture(void* theEnv, const char* logicalName) {
#if MAC_XCD
...
/*   for the check-syntax router.   */
/************************************/
static int PrintErrorCapture(void* theEnv, const char* logicalName,
                             const char* str) {
  if (strcmp(logicalName, WERROR) == 0) {
    ParseFunctionData(theEnv)->ErrorString =
        AppendToString(theEnv, str, ParseFunctionData(theEnv)->ErrorString,
                       &ParseFunctionData(theEnv)->ErrorCurrentPosition,
                       &ParseFunctionData(theEnv)->ErrorMaximumPosition);
  } else if (strcmp(logicalName, WWARNING) == 0) {
    ParseFunctionData(theEnv)->WarningString =
        AppendToString(theEnv, str, ParseFunctionData(theEnv)->WarningString,
