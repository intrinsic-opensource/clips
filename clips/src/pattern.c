...
static struct lhsParseNode* ConjuctiveRestrictionParse(void*, const char*,
                                                       struct token*, int*);
static struct lhsParseNode* LiteralRestrictionParse(void*, const char*,
                                                    struct token*, int*);
...
static struct patternNodeHashEntry** CreatePatternHashTable(void*,
                                                            unsigned long);
...
globle void InitializePatterns(void* theEnv) {
  AllocateEnvironmentData(theEnv, PATTERN_DATA, sizeof(struct patternData),
...
static struct patternNodeHashEntry** CreatePatternHashTable(
  ...
  theTable = (struct patternNodeHashEntry**)gm3(
      theEnv, sizeof(struct patternNodeHashEntry*) * tableSize);
...
/**************************************************/
static void DeallocatePatternData(void* theEnv) {
...
/******************************************************************************/
globle void AddHashedPatternNode(void* theEnv, void* parent, void* child,
                                 unsigned short keyType, void* keyValue) {
  unsigned long hashValue;
  struct patternNodeHashEntry *newhash, *temp;

  hashValue = GetAtomicHashValue(keyType, keyValue, 1) +
              HashExternalAddress(parent, 0); /* TBD mult * 30 */
...
/***************************************************/
globle intBool RemoveHashedPatternNode(void* theEnv, void* parent, void* child,
                                       unsigned short keyType, void* keyValue) {
  unsigned long hashValue;
...
/***********************************************/
globle void* FindHashedPatternNode(void* theEnv, void* parent,
                                   unsigned short keyType, void* keyValue) {
  unsigned long hashValue;
  ...
  for (hptr = PatternData(theEnv)->PatternHashTable[hashValue]; hptr != NULL;
       hptr = hptr->next) {
    if ((hptr->parent == parent) && (keyType == hptr->type) &&
        (keyValue == hptr->value)) {
      return (hptr->child);
    }
  }
...
/******************************************************************/
void AddReservedPatternSymbol(void* theEnv, const char* theSymbol,
...
/******************************************************************/
intBool ReservedPatternSymbol(void* theEnv, const char* theSymbol,
...
/********************************************************/
void ReservedPatternSymbolErrorMsg(void* theEnv, const char* theSymbol,
                                   const char* usedFor) {
...
/************************************************************/
globle void GetNextPatternEntity(void* theEnv, struct patternParser** theParser,
  ...
  /*================================================================*/
    ...
    *theEntity =
        (struct patternEntity*)(*(*theParser)
                                     ->entityType->base.getNextFunction)(
            theEnv, *theEntity);
...
/**************************************************************/
void DetachPattern(void* theEnv, int rhsType,
...
globle intBool AddPatternParser(void* theEnv, struct patternParser* newPtr) {
  ...
  while ((currentPtr != NULL) ? (newPtr->priority < currentPtr->priority)
                              : FALSE) {
    lastPtr = currentPtr;
...
/****************************************************/
globle struct patternParser* FindPatternParser(void* theEnv, const char* name) {
...
/*************************************************************/
globle void PatternNodeHeaderToCode(void* theEnv, FILE* fp,
                                    struct patternNodeHeader* theHeader,
  ...
  if (theHeader->entryJoin == NULL) {
    ...
    fprintf(fp, "&%s%d_%d[%d],", JoinPrefix(), imageID,
            (((int)theHeader->entryJoin->bsaveID) / maxIndices) + 1,
            ((int)theHeader->entryJoin->bsaveID) % maxIndices);
  }

  PrintHashedExpressionReference(theEnv, fp, theHeader->rightHash, imageID,
                                 maxIndices);

  fprintf(fp, ",%d,%d,%d,0,0,%d,%d,%d}", theHeader->singlefieldNode,
          theHeader->multifieldNode, theHeader->stopNode, theHeader->beginSlot,
          theHeader->endSlot, theHeader->selector);
}
...
struct lhsParseNode* RestrictionParse(
    ...
    int multifieldSlot, struct symbolHashNode* theSlot, short slotNumber,
    CONSTRAINT_RECORD* theConstraints, short position) {
  ...
  while (theToken->type != RPAREN) {
    ...
    if ((theToken->type == SF_WILDCARD) || (theToken->type == MF_WILDCARD)) {
      ...
      nextNode =
          ConjuctiveRestrictionParse(theEnv, readSource, theToken, &error);
      if (nextNode == NULL) {
  ...
  for (nextNode = topNode; nextNode != NULL; nextNode = nextNode->right) {
    ...
    nextNode->constraints->minFields =
        GenConstant(theEnv, SYMBOL, SymbolData(theEnv)->NegativeInfinity);
    nextNode->constraints->maxFields =
        GenConstant(theEnv, SYMBOL, SymbolData(theEnv)->PositiveInfinity);
    ...
    if (theConstraints->maxFields->value !=
      ...
      tempConstraints->maxFields = GenConstant(
          theEnv, INTEGER,
          EnvAddLong(theEnv, ValueToLong(theConstraints->maxFields->value) -
                                 numberOfSingleFields));
    }

    if ((numberOfMultifields == 1) && (theConstraints->minFields->value !=
                                       SymbolData(theEnv)->NegativeInfinity)) {
      ReturnExpression(theEnv, tempConstraints->minFields);
      tempConstraints->minFields = GenConstant(
          theEnv, INTEGER,
          EnvAddLong(theEnv, ValueToLong(theConstraints->minFields->value) -
                                 numberOfSingleFields));
    }
  }
...
/***************************************************************/
static void TallyFieldTypes(struct lhsParseNode* theRestrictions) {
  ...
  for (tempNode1 = theRestrictions; tempNode1 != NULL;
    ...
    if ((tempNode1->type == SF_VARIABLE) || (tempNode1->type == SF_WILDCARD)) {
      tempNode1->singleFieldsAfter =
          (unsigned short)(totalSingleFields - (runningSingleFields + 1));
      tempNode1->multiFieldsAfter =
          (unsigned short)(totalMultiFields - runningMultiFields);
    } else {
      tempNode1->singleFieldsAfter =
          (unsigned short)(totalSingleFields - runningSingleFields);
      tempNode1->multiFieldsAfter =
          (unsigned short)(totalMultiFields - (runningMultiFields + 1));
    }
...
/*                <single-constraint> | <connected-constraint>     */
/*******************************************************************/
static struct lhsParseNode* ConjuctiveRestrictionParse(void* theEnv,
  ...
  if (((theNode->type == SF_VARIABLE) || (theNode->type == MF_VARIABLE)) &&
      (theNode->negated == FALSE) && (theToken->type != OR_CONSTRAINT)) {
    ...
    if (theNode->type == MF_VARIABLE)
      bindNode->type = MF_WILDCARD;
    else
      bindNode->type = SF_WILDCARD;
  ...
  while ((theToken->type == OR_CONSTRAINT) ||
...
/*****************************************************/
static int CheckForVariableMixing(void* theEnv,
  ...
  if (theRestriction->type == SF_VARIABLE)
    singlefield = TRUE;
  else if (theRestriction->type == MF_VARIABLE)
    multifield = TRUE;
  ...
  for (theRestriction = theRestriction->bottom; theRestriction != NULL;
    ...
    for (tempRestriction = theRestriction; tempRestriction != NULL;
      ...
      if (tempRestriction->type == SF_VARIABLE)
        singlefield = TRUE;
      else if (tempRestriction->type == MF_VARIABLE)
        multifield = TRUE;
      else if (ConstantType(tempRestriction->type))
        constant = TRUE;
      else if (tempRestriction->type == RETURN_VALUE_CONSTRAINT) {
        theConstraint = FunctionCallToConstraintRecord(
            theEnv, tempRestriction->expression->value);
...

  {
    ...
    EnvPrintRouter(theEnv, WERROR,
                   "Single and multifield constraints cannot be mixed in a "
                   "field constraint\n");
    return (TRUE);
  }
...
/*                             =<function-call>            */
/***********************************************************/
static struct lhsParseNode* LiteralRestrictionParse(void* theEnv,
                                                    const char* readSource,
                                                    struct token* theToken,
                                                    int* error) {
  ...
  else if ((theToken->type == SF_VARIABLE) || (theToken->type == MF_VARIABLE) ||
           (theToken->type == FLOAT) || (theToken->type == INTEGER) ||
           (theToken->type == STRING) || (theToken->type == INSTANCE_NAME)) {
    topNode->value = theToken->value;
  }
