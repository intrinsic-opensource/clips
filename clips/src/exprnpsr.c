...
/***************************************************/
globle struct expr* Function0Parse(void* theEnv, const char* logicalName) {
...
/*******************************************************/
globle struct expr* Function1Parse(void* theEnv, const char* logicalName) {
  struct token theToken;
  struct expr* top;
...
/****************************************************/
globle struct expr* Function2Parse(void* theEnv, const char* logicalName,
                                   const char* name) {
  ...
  if (moduleSpecified) {
    if (ConstructExported(theEnv, "defgeneric", moduleName, constructName) ||
        EnvGetCurrentModule(theEnv) ==
            EnvFindDefmodule(theEnv, ValueToString(moduleName))) {
...
#endif
  {
    if (theFunction->parser != NULL) {
      ...
      if (ReplaceSequenceExpansionOps(theEnv, top->argList, top,
  ...
  if ((top->type == FCALL) && EnvGetStaticConstraintChecking(theEnv)) {
    if (CheckExpressionAgainstRestrictions(theEnv, top,
                                           theFunction->restrictions, name)) {
      ReturnExpression(theEnv, top);
      return (NULL);
    }
  }
    ...
    if (CheckDeffunctionCall(theEnv, top->value,
                             CountArguments(top->argList)) == FALSE) {
      ReturnExpression(theEnv, top);
      return (NULL);
    }
  }
 ...
 **********************************************************************/
globle intBool ReplaceSequenceExpansionOps(void* theEnv, EXPRESSION* actions,
                                           EXPRESSION* fcallexp, void* expcall,
                                           void* expmult) {
  EXPRESSION* theExp;
  ...
  while (actions != NULL) {
    ...
    if ((actions->type == MF_VARIABLE) || (actions->type == MF_GBL_VARIABLE) ||
      ...
      if ((fcallexp->type != FCALL)
              ? FALSE
              : (((struct FunctionDefinition*)fcallexp->value)->sequenceuseok ==
                 FALSE)) {
        PrintErrorID(theEnv, "EXPRNPSR", 4, FALSE);
        EnvPrintRouter(theEnv, WERROR,
                       "$ Sequence operator not a valid argument for ");
        EnvPrintRouter(
            theEnv, WERROR,
            ValueToString(((struct FunctionDefinition*)fcallexp->value)
                              ->callFunctionName));
        EnvPrintRouter(theEnv, WERROR, ".\n");
        return (TRUE);
      }
      ...
      if (actions->value != expmult) {
        ...
        if (actions->type == MF_GBL_VARIABLE) theExp->type = GBL_VARIABLE;
    ...
    if (actions->argList != NULL) {
      if ((actions->type == GCALL) || (actions->type == PCALL) ||
          (actions->type == FCALL))
        theExp = actions;
      else
        theExp = fcallexp;
      if (ReplaceSequenceExpansionOps(theEnv, actions->argList, theExp, expcall,
                                      expmult))
        return (TRUE);
    }
...
globle int CheckExpressionAgainstRestrictions(void* theEnv,
  ...
  for (argPtr = theExpression->argList; argPtr != NULL;
    ...
    if (CheckArgumentAgainstRestriction(theEnv, argPtr, theRestriction)) {
      ExpectedTypeError1(theEnv, functionName, j,
                         GetArgumentTypeName(theRestriction));
      return (TRUE);
    }
...
/*******************************************************/
globle struct expr* CollectArguments(void* theEnv, struct expr* top,
...
/********************************************/
globle struct expr* ArgumentParse(void* theEnv, const char* logicalName,
                                  int* errorFlag) {
  struct expr* top;
  struct token theToken;
  ...
  if ((theToken.type == SF_VARIABLE) || (theToken.type == MF_VARIABLE) ||
      ...
      (theToken.type == GBL_VARIABLE) || (theToken.type == MF_GBL_VARIABLE) ||
#endif
#if OBJECT_SYSTEM
...
/************************************************************/
globle struct expr* ParseAtomOrExpression(void* theEnv, const char* logicalName,
...
/*********************************************/
globle struct expr* GroupActions(void* theEnv, const char* logicalName,
                                 struct token* theToken, int readFirstToken,
                                 const char* endWord, int functionNameParsed) {
  ...
  while (TRUE) {
    ...
    if (functionNameParsed) {
      nextOne =
          Function2Parse(theEnv, logicalName, ValueToString(theToken->value));
...
/*******************************************/
globle EXPRESSION* ParseConstantArguments(void* theEnv, const char* argstr,
                                          int* error) {
  ...
  *error = FALSE;
  ...
  while (tkn.type != STOP) {
    if ((tkn.type != SYMBOL) && (tkn.type != STRING) && (tkn.type != FLOAT) &&
        (tkn.type != INTEGER) && (tkn.type != INSTANCE_NAME)) {
...
globle intBool SetSequenceOperatorRecognition(int value) {
