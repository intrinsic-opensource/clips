...
#if !RUN_TIME
  EnvDefineFunction2(theEnv, "get-defmodule-list", 'm',
                     PTIEF EnvGetDefmoduleList, "EnvGetDefmoduleList", "00");

#if DEBUGGING_FUNCTIONS
  EnvDefineFunction2(theEnv, "list-defmodules", 'v',
                     PTIEF ListDefmodulesCommand, "ListDefmodulesCommand",
                     "00");
  EnvDefineFunction2(theEnv, "ppdefmodule", 'v', PTIEF PPDefmoduleCommand,
                     "PPDefmoduleCommand", "11w");
#endif
#endif
#endif
...
/*********************************************************/
static void ClearDefmodules(void* theEnv) {
...
/******************************************/
static void SaveDefmodules(void* theEnv, void* theModule,
...
globle void EnvGetDefmoduleList(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  for (theConstruct = EnvGetNextDefmodule(theEnv, NULL); theConstruct != NULL;
  ...
  for (theConstruct = EnvGetNextDefmodule(theEnv, NULL), count = 1;
    ...
    SetMFValue(theList, count,
               EnvAddSymbol(theEnv, EnvGetDefmoduleName(theEnv, theConstruct)));
  }
}
...
/*************************************/
globle int PPDefmodule(void* theEnv, const char* defmoduleName,
                       const char* logicalName) {
  void* defmodulePtr;
  ...
  PrintInChunks(theEnv, logicalName,
                EnvGetDefmodulePPForm(theEnv, defmodulePtr));
  return (TRUE);
}
...
/*   for the list-defmodules command.  */
/***************************************/
globle void EnvListDefmodules(void* theEnv, const char* logicalName) {
  void* theModule;
  int count = 0;
...
globle void GetDefmoduleList(DATA_OBJECT_PTR returnValue) {
  EnvGetDefmoduleList(GetCurrentEnvironment(), returnValue);
}

#if DEBUGGING_FUNCTIONS

globle void ListDefmodules(const char* logicalName) {
