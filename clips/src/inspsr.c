...
/*************************************************************************************
  ...
  NOTES        : This function parses a initialize-instance call into
                 ...
                 (initialize-instance <instance-name> <slot-override>*)
                  ...
                  (duplicate-instance <instance> [to <new-name>]
 <slot-override>*) goes to -->
 ...
 *************************************************************************************/
globle EXPRESSION* ParseInitializeInstance(void* theEnv, EXPRESSION* top,
  ...
  if ((top->value == (void*)FindFunction(theEnv, "make-instance")) ||
    ...
    fcalltype = MAKE_TYPE;
           ...
           (top->value ==
            (void*)FindFunction(theEnv, "active-initialize-instance")))
    fcalltype = INITIALIZE_TYPE;
  else if ((top->value == (void*)FindFunction(theEnv, "modify-instance")) ||
           (top->value ==
            (void*)FindFunction(theEnv, "active-modify-instance")) ||
           (top->value ==
            (void*)FindFunction(theEnv, "message-modify-instance")) ||
           (top->value ==
            (void*)FindFunction(theEnv, "active-message-modify-instance")))
  ...
  if (fcalltype == MAKE_TYPE) {
    /* ======================================
       ...
       ====================================== */
    if ((top->argList->type != SYMBOL)
            ? FALSE
            : (strcmp(ValueToString(top->argList->value), CLASS_RLN) == 0)) {
      top->argList->nextArg = ArgumentParse(theEnv, readSource, &error);
      if (error == TRUE) goto ParseInitializeInstanceError;
      if (top->argList->nextArg == NULL) {
        SyntaxErrorMessage(theEnv, "instance class");
        goto ParseInitializeInstanceError;
      }
      if ((top->argList->nextArg->type != SYMBOL)
              ? TRUE
              : (strcmp(ValueToString(top->argList->nextArg->value),
                        CLASS_RLN) != 0)) {
    ...
    if (readclass) {
      top->argList->nextArg = ArgumentParse(theEnv, readSource, &error);
      if (error) goto ParseInitializeInstanceError;
      if (top->argList->nextArg == NULL) {
        SyntaxErrorMessage(theEnv, "instance class");
        goto ParseInitializeInstanceError;
      }
    ...
    if (fcalltype == DUPLICATE_TYPE) {
      if ((DefclassData(theEnv)->ObjectParseToken.type != SYMBOL)
        ...
        top->argList->nextArg =
            GenConstant(theEnv, FCALL, (void*)FindFunction(theEnv, "gensym*"));
      top->argList->nextArg->nextArg =
          ParseSlotOverrides(theEnv, readSource, &error);
    } else
...
/********************************************************************************
  ...
  NOTES        : <slot-override> ::= (<slot-name> <value>*)*
                 ...
                 <slot-name> --> <dummy-node> --> <slot-name> -->
 <dummy-node>...
                                       |
                                       V
 ...
 ********************************************************************************/
globle EXPRESSION* ParseSlotOverrides(void* theEnv, const char* readSource,
                                      int* error) {
 ...
 ****************************************************************************/
globle EXPRESSION* ParseSimpleInstance(void* theEnv, EXPRESSION* top,
                                       const char* readSource) {
  ...
  if ((GetType(DefclassData(theEnv)->ObjectParseToken) == SYMBOL) &&
      (strcmp(CLASS_RLN, DOToString(DefclassData(theEnv)->ObjectParseToken)) ==
       0)) {
    top->argList =
        GenConstant(theEnv, FCALL, (void*)FindFunction(theEnv, "gensym*"));
  } else {
    ...
    if ((GetType(DefclassData(theEnv)->ObjectParseToken) != SYMBOL)
            ? TRUE
            : (strcmp(CLASS_RLN,
                      DOToString(DefclassData(theEnv)->ObjectParseToken)) != 0))
      goto MakeInstanceError;
  }
  ...
  if (GetType(DefclassData(theEnv)->ObjectParseToken) != SYMBOL)
    goto MakeInstanceError;
  top->argList->nextArg = GenConstant(
      theEnv, SYMBOL, (void*)GetValue(DefclassData(theEnv)->ObjectParseToken));
  ...
  while (GetType(DefclassData(theEnv)->ObjectParseToken) == LPAREN) {
    ...
    theExp->nextArg =
        GenConstant(theEnv, SYMBOL,
                    (void*)GetValue(DefclassData(theEnv)->ObjectParseToken));
    theExp->nextArg->nextArg =
        GenConstant(theEnv, SYMBOL, EnvTrueSymbol(theEnv));
    ...
    while (GetType(DefclassData(theEnv)->ObjectParseToken) != RPAREN) {
      ...
      if (type == LPAREN) {
        ...
        if ((type != SYMBOL) && (type != STRING) && (type != FLOAT) &&
            (type != INTEGER) && (type != INSTANCE_NAME))
          goto SlotOverrideError;
        tval = GenConstant(
            theEnv, type,
            (void*)GetValue(DefclassData(theEnv)->ObjectParseToken));
...
static intBool ReplaceClassNameWithReference(void* theEnv, EXPRESSION* theExp) {
  ...
  if (theExp->type == SYMBOL) {
    ...
    theDefclass = (void*)LookupDefclassByMdlOrScope(
        theEnv, theClassName);  // Module or scope is now allowed
    if (theDefclass == NULL) {
