...
#include "setup.h"
    ...
    (BLOAD || BLOAD_ONLY || BLOAD_AND_BSAVE)
...
typedef struct bsaveObjectPatternNode {
  ...
  unsigned slotNameID;
  long networkTest, nextLevel, lastLevel, leftNode, rightNode, alphaNode;
} BSAVE_OBJECT_PATTERN_NODE;

typedef struct bsaveObjectAlphaNode {
  struct bsavePatternNodeHeader header;
  long classbmp, slotbmp, patternNode, nxtInGroup, nxtTerminal;
...
#define ObjectPatternPointer(i) \
  ((i == -1L)                   \
       ? NULL                   \
       : (OBJECT_PATTERN_NODE*)&ObjectReteBinaryData(theEnv)->PatternArray[i])
#define ObjectAlphaPointer(i) \
  ((i == -1L)                 \
       ? NULL                 \
       : (OBJECT_ALPHA_NODE*)&ObjectReteBinaryData(theEnv)->AlphaArray[i])
...
globle void SetupObjectPatternsBload(void* theEnv) {
  AllocateEnvironmentData(theEnv, OBJECTRETEBIN_DATA,
...
/***********************************************************/
static void DeallocateObjectReteBinaryData(void* theEnv) {
  ...
  for (i = 0; i < ObjectReteBinaryData(theEnv)->AlphaNodeCount; i++) {
    DestroyAlphaMemory(
        theEnv, &ObjectReteBinaryData(theEnv)->AlphaArray[i].header, FALSE);
  }

  space = ObjectReteBinaryData(theEnv)->AlphaNodeCount *
          sizeof(struct objectAlphaNode);
  if (space != 0)
    genfree(theEnv, (void*)ObjectReteBinaryData(theEnv)->AlphaArray, space);

  space = ObjectReteBinaryData(theEnv)->PatternNodeCount *
          sizeof(struct objectPatternNode);
  if (space != 0)
    genfree(theEnv, (void*)ObjectReteBinaryData(theEnv)->PatternArray, space);

  space = ObjectReteBinaryData(theEnv)->AlphaLinkCount *
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void BsaveObjectPatternsFind(void* theEnv) {
  ...
  DoForAllConstructs(theEnv, MarkDefclassItems,
                     DefclassData(theEnv)->DefclassModuleIndex, FALSE, NULL);
  ...
  while (alphaPtr != NULL) {
    ...
    if (alphaPtr->slotbmp != NULL) alphaPtr->slotbmp->neededBitMap = TRUE;
...
#pragma unused(buf)
  ...
  for (alphaLink = cls->relevant_terminal_alpha_nodes; alphaLink != NULL;
...
/****************************************************
  ...
  NOTES        : None
 ****************************************************/
static void BsaveStorageObjectPatterns(void* theEnv, FILE* fp) {
  size_t space;
 ...
 ***************************************************/
static void BsaveObjectPatterns(void* theEnv, FILE* fp) {
  size_t space;
  OBJECT_ALPHA_NODE* alphaPtr;
  OBJECT_PATTERN_NODE* patternPtr;
  ...
  DoForAllConstructs(theEnv, BsaveAlphaLinks,
  ...
  alphaPtr = ObjectNetworkTerminalPointer(theEnv);
  while (alphaPtr != NULL) {
    AssignBsavePatternHeaderValues(theEnv, &dummyAlpha.header,
                                   &alphaPtr->header);
  ...
  while (patternPtr != NULL) {
    ...
    dummyPattern.networkTest =
        HashedExpressionIndex(theEnv, patternPtr->networkTest);
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void BsaveAlphaLinks(void* theEnv, struct constructHeader* theDefclass,
                            void* buf) {
  DEFCLASS* cls = (DEFCLASS*)theDefclass;
  ...
  for (alphaLink = cls->relevant_terminal_alpha_nodes; alphaLink != NULL;
    ...
    GenWrite((void*)&dummy_alpha_link, sizeof(BSAVE_CLASS_ALPHA_LINK),
             (FILE*)buf);
  }
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void BloadStorageObjectPatterns(void* theEnv) {
  size_t space;
  long counts[3];
  ...
  if (ObjectReteBinaryData(theEnv)->AlphaNodeCount == 0L)
    ...
    ObjectReteBinaryData(theEnv)->AlphaArray =
        (OBJECT_ALPHA_NODE*)genalloc(theEnv, space);
  }
  ...
  if (ObjectReteBinaryData(theEnv)->PatternNodeCount == 0L)
    ...
    ObjectReteBinaryData(theEnv)->PatternArray =
        (OBJECT_PATTERN_NODE*)genalloc(theEnv, space);
  }
  ...
  if (ObjectReteBinaryData(theEnv)->AlphaLinkCount == 0L)
    ...
    ObjectReteBinaryData(theEnv)->AlphaLinkArray =
        (CLASS_ALPHA_LINK*)genalloc(theEnv, space);
  }
}
 ...
 ****************************************************/
static void BloadObjectPatterns(void* theEnv) {
  size_t space;
  long i;
  ...
  BloadandRefresh(theEnv, ObjectReteBinaryData(theEnv)->AlphaLinkCount,
                  sizeof(BSAVE_CLASS_ALPHA_LINK), UpdateLink);
  BloadandRefresh(theEnv, ObjectReteBinaryData(theEnv)->AlphaNodeCount,
                  sizeof(BSAVE_OBJECT_ALPHA_NODE), UpdateAlpha);
  BloadandRefresh(theEnv, ObjectReteBinaryData(theEnv)->PatternNodeCount,
  ...
  for (i = 0; i < ObjectReteBinaryData(theEnv)->PatternNodeCount; i++) {
    if ((ObjectReteBinaryData(theEnv)->PatternArray[i].lastLevel != NULL) &&
      ...
      AddHashedPatternNode(
          theEnv, ObjectReteBinaryData(theEnv)->PatternArray[i].lastLevel,
  ...
  /* =======================
     ...
     ======================= */
  SetObjectNetworkTerminalPointer(
      theEnv, (OBJECT_ALPHA_NODE*)&ObjectReteBinaryData(theEnv)->AlphaArray[0]);
  SetObjectNetworkPointer(
      theEnv,
      (OBJECT_PATTERN_NODE*)&ObjectReteBinaryData(theEnv)->PatternArray[0]);
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void UpdateAlpha(void* theEnv, void* buf, long obji) {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void UpdatePattern(void* theEnv, void* buf, long obji) {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void ClearBloadObjectPatterns(void* theEnv) {
  size_t space;
  long i;
  ...
  for (i = 0L; i < ObjectReteBinaryData(theEnv)->AlphaNodeCount; i++) {
    DecrementBitMapCount(theEnv,
                         ObjectReteBinaryData(theEnv)->AlphaArray[i].classbmp);
    if (ObjectReteBinaryData(theEnv)->AlphaArray[i].slotbmp != NULL)
      DecrementBitMapCount(theEnv,
                           ObjectReteBinaryData(theEnv)->AlphaArray[i].slotbmp);
  }
