...
static struct lhsParseNode* ReverseAndOr(void*, struct lhsParseNode*,
...
static struct lhsParseNode* ReorderDriver(void*, struct lhsParseNode*, int*,
                                          int, int);
static struct lhsParseNode* AddRemainingInitialPatterns(void*,
                                                        struct lhsParseNode*);
static struct lhsParseNode* AssignPatternIndices(struct lhsParseNode*, short,
                                                 int, short);
static void PropagateIndexSlotPatternValues(struct lhsParseNode*, short, short,
...
/*
static void                    PrintNodes(void *,const char *,struct
lhsParseNode *);
*/
...
globle struct lhsParseNode* ReorderPatterns(void* theEnv,
  ...
  if (newLHS->type == OR_CE) {
    for (tempLHS = newLHS->right, lastLHS = NULL; tempLHS != NULL;
...
/*   to accommodate KB Rete topology.     */
/******************************************/
static struct lhsParseNode* ReorderDriver(void* theEnv,
                                          struct lhsParseNode* theLHS,
                                          int* anyChange, int pass, int depth) {
  ...
  *anyChange = FALSE;
  ...
  while (change) {
    ...
    if ((theLHS->type == AND_CE) || (theLHS->type == NOT_CE) ||
        (theLHS->type == OR_CE)) {
      if (pass == 1)
        theLHS = PerformReorder1(theEnv, theLHS, &newChange, depth);
      else
        theLHS = PerformReorder2(theEnv, theLHS, &newChange, depth);
    ...
    while (argPtr != NULL) {
      ...
      if ((argPtr->type == AND_CE) || (argPtr->type == NOT_CE) ||
          (argPtr->type == OR_CE)) {
        if (before == NULL) {
          argPtr->bottom = NULL;
          theLHS->right =
              ReorderDriver(theEnv, argPtr, &newChange, pass, depth + 1);
...
/* MarkExistsNands: */
/********************/
static void MarkExistsNands(struct lhsParseNode* theLHS) {
  int currentDepth = 1;
...
globle void AddInitialPatterns(void* theEnv, struct lhsParseNode* theLHS) {
  ...
  if (theLHS->type == OR_CE) {
    for (thePattern = theLHS->right; thePattern != NULL;
...
/***********************************************************/
static struct lhsParseNode* PerformReorder1(void* theEnv,
                                            struct lhsParseNode* theLHS,
                                            int* newChange, int depth) {
...
/***********************************************************/
static struct lhsParseNode* PerformReorder2(void* theEnv,
                                            struct lhsParseNode* theLHS,
                                            int* newChange, int depth) {
  struct lhsParseNode* argPtr;
  int change;
  ...
  *newChange = FALSE;
  ...
  while (change) {
    ...
    for (argPtr = theLHS->right; argPtr != NULL;) {
      ...
      if ((theLHS->type == NOT_CE) && (argPtr->type == NOT_CE) &&
          (argPtr->right != NULL) && (argPtr->right->type == NOT_CE)) {
        change = TRUE;
        *newChange = TRUE;
      ...
      else if ((theLHS->type == NOT_CE) && (argPtr->type == NOT_CE) &&
               (argPtr->right != NULL) && (argPtr->right->type == PATTERN_CE)) {
        change = TRUE;
        *newChange = TRUE;
...
/**************************************************/
static struct lhsParseNode* ReverseAndOr(void* theEnv,
  ...
  while (orCE != NULL) {
    ...
    for (count = 1, replaceCE = copyOfCEs->right; count != orPosition;
...
/* CompressCEs: */
/****************/
static struct lhsParseNode* CompressCEs(void* theEnv,
                                        struct lhsParseNode* theLHS,
                                        int* newChange, int depth) {
  struct lhsParseNode *argPtr, *lastArg, *nextArg;
    ...
    for (argPtr = theLHS->right; argPtr != NULL;) {
      ...
      /*==============================*/
        ...
        *newChange = TRUE;
        ...
        argPtr->expression = CombineLHSParseNodes(theEnv, argPtr->expression,
      ...
      /*========================================================*/
               ...
               ((argPtr->bottom != NULL) ? argPtr->bottom->type == TEST_CE
                                         : FALSE) &&
               (argPtr->negated == FALSE) && (argPtr->exists == FALSE) &&
               (argPtr->beginNandDepth == argPtr->endNandDepth) &&
               (argPtr->endNandDepth == argPtr->bottom->beginNandDepth)) {
        int endNandDepth;
        change = TRUE;
...
/**********************************************************/
globle void CopyLHSParseNode(void* theEnv, struct lhsParseNode* dest,
                             struct lhsParseNode* src, int duplicate) {
  dest->type = src->type;
  ...
  if (duplicate) {
    ...
    dest->externalNetworkTest =
        CopyExpression(theEnv, src->externalNetworkTest);
    dest->secondaryNetworkTest =
        CopyExpression(theEnv, src->secondaryNetworkTest);
    ...
    dest->secondaryExpression =
        CopyLHSParseNodes(theEnv, src->secondaryExpression);
...
globle struct lhsParseNode* ExpressionToLHSParseNodes(
    void* theEnv, struct expr* expressionList) {
...
/*   CE.                                                    */
/************************************************************/
static void IncrementNandDepth(void* theEnv, struct lhsParseNode* theLHS,
                               int lastCE) {
  /*======================================*/
...
/***********************************************************/
static struct lhsParseNode* CreateInitialPattern(void* theEnv) {
...
static struct lhsParseNode* AddRemainingInitialPatterns(
    void* theEnv, struct lhsParseNode* theLHS) {
  struct lhsParseNode *lastNode = NULL, *thePattern, *rv = theLHS;
  int currentDepth = 1;

  while (theLHS != NULL) {
    if ((theLHS->type == TEST_CE) && (theLHS->beginNandDepth > currentDepth)) {
...
/*   d-4, and e-3.                                           */
/*************************************************************/
static struct lhsParseNode* AssignPatternIndices(struct lhsParseNode* theLHS,
                                                 short startIndex,
                                                 int nandDepth,
                                                 short joinDepth) {
  ...
  while (theLHS != NULL) {
    ...
    if (theLHS->beginNandDepth > nandDepth) {
      theLHS =
          AssignPatternIndices(theLHS, startIndex, nandDepth + 1, joinDepth);
...
/***********************************************************/
static void PropagateIndexSlotPatternValues(struct lhsParseNode* theField,
                                            short thePattern, short theIndex,
  ...
  if (theField->multifieldSlot) {
    ...
    for (tmpNode = theField->bottom; tmpNode != NULL;
  ...
  for (theField = theField->bottom; theField != NULL;
...
/*****************************************************************/
static void PropagateJoinDepth(struct lhsParseNode* theField, short joinDepth) {
  while (theField != NULL) {
...
/**************************************************************/
static void PropagateNandDepth(struct lhsParseNode* theField, int beginDepth,
                               int endDepth) {
  if (theField == NULL) return;
...
/*****************************************/
static int PropagateWhichCE(struct lhsParseNode* theField, int whichCE) {
  while (theField != NULL) {
...
/***************************************************************************/
globle struct lhsParseNode* CombineLHSParseNodes(void* theEnv,
