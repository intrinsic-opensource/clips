...
globle void SetupDefinstances(void* theEnv) {
  AllocateEnvironmentData(theEnv, DEFINSTANCES_DATA,
  ...
  DefinstancesData(theEnv)->DefinstancesConstruct = AddConstruct(
      theEnv, "definstances", "definstances",
#if (!BLOAD_ONLY) && (!RUN_TIME)
      ParseDefinstances,
#else
      NULL,
#endif
      EnvFindDefinstances, GetConstructNamePointer, GetConstructPPForm,
...
#if !BLOAD_ONLY
  EnvDefineFunction2(theEnv, "undefinstances", 'v', PTIEF UndefinstancesCommand,
  ...
  EnvDefineFunction2(theEnv, "ppdefinstances", 'v', PTIEF PPDefinstancesCommand,
                     "PPDefinstancesCommand", "11w");
  EnvDefineFunction2(theEnv, "list-definstances", 'v',
                     PTIEF ListDefinstancesCommand, "ListDefinstancesCommand",
                     "01");
#endif

  EnvDefineFunction2(theEnv, "get-definstances-list", 'm',
                     PTIEF GetDefinstancesListFunction,
                     "GetDefinstancesListFunction", "01");
  EnvDefineFunction2(theEnv, "definstances-module", 'w',
                     PTIEF GetDefinstancesModuleCommand,
                     "GetDefinstancesModuleCommand", "11w");

#endif
  EnvAddResetFunction(theEnv, "definstances",
                      (void (*)(void*))ResetDefinstances, 0);
...
static void DeallocateDefinstancesData(void* theEnv) {
  ...
  DoForAllConstructs(theEnv, DestroyDefinstancesAction,
  ...
  for (theModule = EnvGetNextDefmodule(theEnv, NULL); theModule != NULL;
    ...
    theModuleItem = (struct definstancesModule*)GetModuleItem(
...
/*****************************************************/
static void DestroyDefinstancesAction(void* theEnv,
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void* EnvFindDefinstances(void* theEnv, const char* name) {
  return (FindNamedConstructInModuleOrImports(
      theEnv, name, DefinstancesData(theEnv)->DefinstancesConstruct));
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void* EnvFindDefinstancesInModule(void* theEnv, const char* name) {
...
globle void UndefinstancesCommand(void* theEnv) {
  UndefconstructCommand(theEnv, "undefinstances",
...
/***********************************************************
  ...
  NOTES        : None
 ***********************************************************/
globle intBool EnvUndefinstances(void* theEnv, void* vptr) {
...
#endif
  if (dptr == NULL) return (RemoveAllDefinstances(theEnv));
  if (EnvIsDefinstancesDeletable(theEnv, vptr) == FALSE) return (FALSE);
...
globle void PPDefinstancesCommand(void* theEnv) {
  PPConstructCommand(theEnv, "ppdefinstances",
                     DefinstancesData(theEnv)->DefinstancesConstruct);
}
...
globle void ListDefinstancesCommand(void* theEnv) {
  ListConstructCommand(theEnv, "list-definstances",
                       DefinstancesData(theEnv)->DefinstancesConstruct);
}
...
/***************************************************
  ...
  NOTES        : C Interface
 ***************************************************/
globle void EnvListDefinstances(void* theEnv, const char* logicalName,
                                struct defmodule* theModule) {
  ListConstruct(theEnv, DefinstancesData(theEnv)->DefinstancesConstruct,
 ...
 ****************************************************************/
globle void GetDefinstancesListFunction(void* theEnv,
                                        DATA_OBJECT* returnValue) {
  GetConstructListFunction(theEnv, "get-definstances-list", returnValue,
                           DefinstancesData(theEnv)->DefinstancesConstruct);
}
 ...
 ***************************************************************/
globle void EnvGetDefinstancesList(void* theEnv, DATA_OBJECT* returnValue,
                                   struct defmodule* theModule) {
  GetConstructList(theEnv, returnValue,
                   DefinstancesData(theEnv)->DefinstancesConstruct, theModule);
}
 ...
 *********************************************************************/
static int ParseDefinstances(void* theEnv, const char* readSource) {
  SYMBOL_HN* dname;
  void* mkinsfcall;
  ...
  if (dname == NULL) return (TRUE);

  dobj = get_struct(theEnv, definstances);
  InitializeConstructHeader(theEnv, "definstances",
                            (struct constructHeader*)dobj, dname);
  dobj->busy = 0;
  ...
  if (GetType(DefclassData(theEnv)->ObjectParseToken) != RPAREN) {
    ...
    if (EnvGetConserveMemory(theEnv) == FALSE) {
      if (dobj->mkinstance != NULL) PPBackup(theEnv);
      PPBackup(theEnv);
      SavePPBuffer(theEnv, ")\n");
...
/*************************************************************
  ...
  NOTES        : Assumes "(definstances" has already
                   been scanned.
 *************************************************************/
static SYMBOL_HN* ParseDefinstancesName(void* theEnv, const char* readSource,
                                        int* active) {
  SYMBOL_HN* dname;

  *active = FALSE;
  dname = GetConstructNameAndComment(
      theEnv, readSource, &DefclassData(theEnv)->ObjectParseToken,
      "definstances", EnvFindDefinstancesInModule, EnvUndefinstances, "@", TRUE,
      FALSE, TRUE, FALSE);
  ...
  if ((GetType(DefclassData(theEnv)->ObjectParseToken) != SYMBOL)
          ...
          : (strcmp(ValueToString(
                        GetValue(DefclassData(theEnv)->ObjectParseToken)),
                    ACTIVE_RLN) == 0)) {
    PPBackup(theEnv);
    PPBackup(theEnv);
    SavePPBuffer(theEnv, " ");
...
static void RemoveDefinstances(void* theEnv, void* vdptr) {
  ...
  DecrementSymbolCount(theEnv,
                       EnvGetDefinstancesNamePointer(theEnv, (void*)dptr));
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void SaveDefinstances(void* theEnv, void* theModule,
                             const char* logName) {
...
static intBool RemoveAllDefinstances(void* theEnv) {
  ...
  while (dhead != NULL) {
    ...
    if (EnvIsDefinstancesDeletable(theEnv, (void*)dptr)) {
      ...
      DefinstancesDeleteError(theEnv,
                              EnvGetDefinstancesName(theEnv, (void*)dptr));
      success = FALSE;
    }
  }
  return (success);
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void DefinstancesDeleteError(void* theEnv, const char* dname) {
...
static void CreateInitialDefinstances(void* theEnv) {
  ...
  IncrementSymbolCount(
      EnvGetDefinstancesNamePointer(theEnv, (void*)theDefinstances));
...
static intBool ClearDefinstancesReady(void* theEnv) {
  ...
  DoForAllConstructs(theEnv, CheckDefinstancesBusy,
                     DefinstancesData(theEnv)->DefinstancesModuleIndex, FALSE,
                     (void*)&flagBuffer);
  return (flagBuffer);
}
 ...
 ***************************************************/
static void CheckDefinstancesBusy(void* theEnv,
                                  struct constructHeader* theDefinstances,
                                  void* userBuffer) {
#if MAC_XCD
#pragma unused(theEnv)
  ...
  if (((DEFINSTANCES*)theDefinstances)->busy > 0) *(int*)userBuffer = FALSE;
}
...
static void ResetDefinstances(void* theEnv) {
  DoForAllConstructs(theEnv, ResetDefinstancesAction,
                     DefinstancesData(theEnv)->DefinstancesModuleIndex, TRUE,
                     NULL);
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void ResetDefinstancesAction(void* theEnv,
...
#pragma unused(userBuffer)
  ...
  for (theExp = theDefinstances->mkinstance; theExp != NULL;
...
globle const char* EnvGetDefinstancesName(void* theEnv, void* theDefinstances) {
  return GetConstructNameString((struct constructHeader*)theDefinstances);
}

globle const char* EnvGetDefinstancesPPForm(void* theEnv,
                                            void* theDefinstances) {
  return GetConstructPPForm(theEnv, (struct constructHeader*)theDefinstances);
}

globle void EnvSetDefinstancesPPForm(void* theEnv, void* theDefinstances,
                                     const char* thePPForm) {
  SetConstructPPForm(theEnv, (struct constructHeader*)theDefinstances,
                     thePPForm);
}

globle const char* EnvDefinstancesModule(void* theEnv, void* theDefinstances) {
  return GetConstructModuleName((struct constructHeader*)theDefinstances);
}

globle SYMBOL_HN* EnvGetDefinstancesNamePointer(void* theEnv,
                                                void* theDefinstances) {
  return GetConstructNamePointer((struct constructHeader*)theDefinstances);
}

globle const char* EnvDefinstancesModuleName(void* theEnv,
                                             void* theDefinstances) {
  return GetConstructModuleName((struct constructHeader*)theDefinstances);
}
