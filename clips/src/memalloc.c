...
#if (MEM_TABLE_SIZE > 0)
  MemoryData(theEnv)->MemoryTable = (struct memoryPtr**)malloc(
...
globle int (*EnvSetOutOfMemoryFunction(
    void* theEnv, int (*functionPtr)(void*, size_t)))(void*, size_t) {
...
/******************************************************/
globle void* genrealloc(void* theEnv, void* oldaddr, size_t oldsz,
                        size_t newsz) {
  char* newaddr;
  unsigned i;
  size_t limit;
...
/***********************************/
globle long int EnvReleaseMem(void* theEnv, long int maximum) {
...
/***************************************************************/
globle unsigned long ActualPoolSize(void* theEnv) { return (PoolSize(theEnv)); }
...
globle long int MemUsed() { return EnvMemUsed(GetCurrentEnvironment()); }

globle long int ReleaseMem(long int maximum) {
  return EnvReleaseMem(GetCurrentEnvironment(), maximum);
}

globle intBool SetConserveMemory(intBool value) {
  return EnvSetConserveMemory(GetCurrentEnvironment(), value);
}

globle int (*SetOutOfMemoryFunction(int (*functionPtr)(void*,
                                                       size_t)))(void*,
                                                                 size_t) {
