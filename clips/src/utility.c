...
globle void InitializeUtilityData(void* theEnv) {
  AllocateEnvironmentData(theEnv, UTILITY_DATA, sizeof(struct utilityData),
...
/*    data for utility routines.                  */
/**************************************************/
static void DeallocateUtilityData(void* theEnv) {
...
/*****************************/
globle void CleanCurrentGarbageFrame(void* theEnv, DATA_OBJECT* returnValue) {
...
/*****************************/
globle void RestorePriorGarbageFrame(void* theEnv,
...
/*******************************/
globle intBool CurrentGarbageFrameIsDirty(void* theEnv) {
...
/*************************/
globle void CallCleanupFunctions(void* theEnv) {
...
/**************************************************/
globle void CallPeriodicTasks(void* theEnv) {
...
/***************************************************/
globle intBool AddCleanupFunction(void* theEnv, const char* name,
                                  void (*theFunction)(void*), int priority) {
  UtilityData(theEnv)->ListOfCleanupFunctions = AddFunctionToCallList(
      theEnv, name, priority, (void (*)(void*))theFunction,
...
/****************************************************/
globle intBool AddPeriodicFunction(const char* name, void (*theFunction)(void),
                                   int priority) {
  void* theEnv;

  theEnv = GetCurrentEnvironment();

  UtilityData(theEnv)->ListOfPeriodicFunctions = AddFunctionToCallList(
      theEnv, name, priority, (void (*)(void*))theFunction,
...
/*   of functions called to handle periodic tasks.     */
/*******************************************************/
globle intBool EnvAddPeriodicFunction(void* theEnv, const char* name,
...
/*   as returning free memory to the memory pool.      */
/*******************************************************/
globle intBool RemoveCleanupFunction(void* theEnv, const char* name) {
  intBool found;

  UtilityData(theEnv)->ListOfCleanupFunctions = RemoveFunctionFromCallList(
      theEnv, name, UtilityData(theEnv)->ListOfCleanupFunctions, &found);
...
/**********************************************************/
globle intBool EnvRemovePeriodicFunction(void* theEnv, const char* name) {
  intBool found;

  UtilityData(theEnv)->ListOfPeriodicFunctions = RemoveFunctionFromCallList(
      theEnv, name, UtilityData(theEnv)->ListOfPeriodicFunctions, &found);
...
/*****************************************************/
globle const char* StringPrintForm(void* theEnv, const char* str) {
  int i = 0;
  size_t pos = 0;
  size_t max = 0;
  char* theString = NULL;
  ...
  theString =
      ExpandStringWithChar(theEnv, '"', theString, &pos, &max, max + 80);
  while (str[i] != EOS) {
    if ((str[i] == '"') || (str[i] == '\\')) {
      theString =
          ExpandStringWithChar(theEnv, '\\', theString, &pos, &max, max + 80);
      theString =
          ExpandStringWithChar(theEnv, str[i], theString, &pos, &max, max + 80);
    } else {
      theString =
          ExpandStringWithChar(theEnv, str[i], theString, &pos, &max, max + 80);
    }
...
/**************************************************************/
globle char* CopyString(void* theEnv, const char* theString) {
...
/*****************************************************************/
globle void DeleteString(void* theEnv, char* theString) {
  if (theString != NULL) {
...
/***********************************************************/
globle const char* AppendStrings(void* theEnv, const char* str1,
                                 const char* str2) {
  size_t pos = 0;
  size_t max = 0;
  char* theString = NULL;
...
/******************************************************/
globle char* AppendToString(void* theEnv, const char* appendStr, char* oldStr,
                            size_t* oldPos, size_t* oldMax) {
  size_t length;
  ...
  if ((oldStr = EnlargeString(theEnv, length, oldStr, oldPos, oldMax)) ==
      NULL) {
    return (NULL);
  }
...
/*   (expanding the other string if necessary).           */
/**********************************************************/
globle char* InsertInString(void* theEnv, const char* insertStr,
                            size_t position, char* oldStr, size_t* oldPos,
                            size_t* oldMax) {
  size_t length;
...
/*******************************************************************/
globle char* EnlargeString(void* theEnv, size_t length, char* oldStr,
                           size_t* oldPos, size_t* oldMax) {
  size_t newMax;
...
/*   the string.                                       */
/*******************************************************/
globle char* AppendNToString(void* theEnv, const char* appendStr, char* oldStr,
...
/*   the string.                                       */
/*******************************************************/
globle char* ExpandStringWithChar(void* theEnv, int inchar, char* str,
                                  size_t* pos, size_t* max, size_t newSize) {
  if ((*pos + 1) >= *max) {
    if (newSize < sizeof(char*)) {
      newSize = sizeof(char*);
    }
...
globle struct callFunctionItem* AddFunctionToCallList(
    void* theEnv, const char* name, int priority, void (*func)(void*),
    struct callFunctionItem* head, intBool environmentAware) {
  return AddFunctionToCallListWithContext(theEnv, name, priority, func, head,
                                          environmentAware, NULL);
}
...
globle struct callFunctionItem* AddFunctionToCallListWithContext(
    void* theEnv, const char* name, int priority, void (*func)(void*),
    struct callFunctionItem* head, intBool environmentAware, void* context) {
...
globle struct callFunctionItem* RemoveFunctionFromCallList(
    void* theEnv, const char* name, struct callFunctionItem* head, int* found) {
...
globle struct callFunctionItemWithArg* AddFunctionToCallListWithArg(
    void* theEnv, const char* name, int priority, void (*func)(void*, void*),
    struct callFunctionItemWithArg* head, intBool environmentAware) {
  return AddFunctionToCallListWithArgWithContext(theEnv, name, priority, func,
                                                 head, environmentAware, NULL);
}
...
globle struct callFunctionItemWithArg* RemoveFunctionFromCallListWithArg(
    void* theEnv, const char* name, struct callFunctionItemWithArg* head,
    int* found) {
...
/*   (e.g. clear, reset, and bload functions).                */
/**************************************************************/
globle void DeallocateCallListWithArg(void* theEnv,
...
/*****************************************/
globle unsigned long ItemHashValue(void* theEnv, unsigned short theType,
                                   void* theValue, unsigned long theRange) {
  union {
    void* vv;
    unsigned uv;
  } fis;
...
globle void YieldTime(void* theEnv) {
  if ((UtilityData(theEnv)->YieldTimeFunction != NULL) &&
...
/*************************************************************************/
globle struct trackedMemory* AddTrackedMemory(void* theEnv, void* theMemory,
                                              size_t theSize) {
...
/******************************************/
globle size_t UTF8Length(const char* s) {
  size_t i = 0, length = 0;
...
/*********************************************/
globle void UTF8Increment(const char* s, size_t* i) {
  (void)(IsUTF8Start(s[++(*i)]) || IsUTF8Start(s[++(*i)]) ||
         IsUTF8Start(s[++(*i)]) || ++(*i));
}
...
/****************************************************/
globle size_t UTF8Offset(const char* str, size_t charnum) {
  size_t offs = 0;

  while ((charnum > 0) && (str[offs])) {
    (void)(IsUTF8Start(str[++offs]) || IsUTF8Start(str[++offs]) ||
...
/*************************************************/
globle size_t UTF8CharNum(const char* s, size_t offset) {
  size_t charnum = 0, offs = 0;

  while ((offs < offset) && (s[offs])) {
    (void)(IsUTF8Start(s[++offs]) || IsUTF8Start(s[++offs]) ||
           IsUTF8Start(s[++offs]) || ++offs);
...
globle void IncrementGCLocks() { EnvIncrementGCLocks(GetCurrentEnvironment()); }

globle void DecrementGCLocks() { EnvDecrementGCLocks(GetCurrentEnvironment()); }

globle intBool RemovePeriodicFunction(const char* name) {
