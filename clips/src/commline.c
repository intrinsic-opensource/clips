...
globle void InitializeCommandLineData(void* theEnv) {
  AllocateEnvironmentData(theEnv, COMMANDLINE_DATA,
...
/******************************************************/
static void DeallocateCommandLineData(void* theEnv) {
#if !RUN_TIME
  if (CommandLineData(theEnv)->CommandString != NULL) {
    rm(theEnv, CommandLineData(theEnv)->CommandString,
...
globle int ExpandCommandString(void* theEnv, int inchar) {
  ...
  CommandLineData(theEnv)->CommandString = ExpandStringWithChar(
      theEnv, inchar, CommandLineData(theEnv)->CommandString,
 ...
 */
/*********************************************************************************/
globle void SetCommandString(void* theEnv, const char* str) {
  size_t length;

  FlushCommandString(theEnv);
  length = strlen(str);
  CommandLineData(theEnv)->CommandString = (char*)genrealloc(
      theEnv, CommandLineData(theEnv)->CommandString,
      (unsigned)CommandLineData(theEnv)->MaximumCharacters,
...
/*************************************************************/
globle void SetNCommandString(void* theEnv, const char* str, unsigned length) {
  FlushCommandString(theEnv);
  CommandLineData(theEnv)->CommandString = (char*)genrealloc(
      theEnv, CommandLineData(theEnv)->CommandString,
      (unsigned)CommandLineData(theEnv)->MaximumCharacters,
...
/******************************************************************************/
globle void AppendCommandString(void* theEnv, const char* str) {
  CommandLineData(theEnv)->CommandString =
      AppendToString(theEnv, str, CommandLineData(theEnv)->CommandString,
                     &RouterData(theEnv)->CommandBufferInputCount,
                     &CommandLineData(theEnv)->MaximumCharacters);
}
...
/******************************************************************************/
globle void InsertCommandString(void* theEnv, const char* str,
                                unsigned int position) {
  CommandLineData(theEnv)->CommandString = InsertInString(
      theEnv, str, position, CommandLineData(theEnv)->CommandString,
      &RouterData(theEnv)->CommandBufferInputCount,
      &CommandLineData(theEnv)->MaximumCharacters);
}
...
/************************************************************/
globle void AppendNCommandString(void* theEnv, const char* str,
                                 unsigned length) {
  CommandLineData(theEnv)->CommandString =
      AppendNToString(theEnv, str, CommandLineData(theEnv)->CommandString,
...
/*   contains an error.                                                   */
/**************************************************************************/
globle int CompleteCommand(const char* mstring) {
  int i;
  char inchar;
  int depth = 0;
  int moreThanZero = 0;
...
/***********************************************************/
static int DoString(const char* str, int pos, int* complete) {
  int inchar;
...
/*************************************************************/
static int DoComment(const char* str, int pos) {
  int inchar;
...
/**************************************************************/
static int DoWhiteSpace(const char* str, int pos) {
  int inchar;
...
/********************************************************/
globle intBool RouteCommand(void* theEnv, const char* command,
                            int printResult) {
  DATA_OBJECT result;
  struct expr* top;
  ...
  if ((theToken.type == GBL_VARIABLE) || (theToken.type == SF_VARIABLE) ||
...
/*****************************************************************/
static int DefaultGetNextEvent(void* theEnv) {
  int inchar;
...
/***********************************************************/
globle const char* GetCommandCompletionString(void* theEnv,
  ...
  if (lastToken.type == SYMBOL) {
    ...
    return (ValueToString(lastToken.value));
  } else if ((lastToken.type == GBL_VARIABLE) ||
             (lastToken.type == MF_GBL_VARIABLE) ||
             (lastToken.type == INSTANCE_NAME)) {
    return (NULL);
  } else if (lastToken.type == STRING) {
    length = strlen(ValueToString(lastToken.value));
    return (GetCommandCompletionString(theEnv, ValueToString(lastToken.value),
                                       length));
