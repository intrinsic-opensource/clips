...
/**************************************************************/
globle void AtomicValuesToCode(void* theEnv, const char* fileName,
  ...
  version =
      FloatHashNodesToCode(theEnv, fileName, pathName, fileNameBuffer, version);
  version = IntegerHashNodesToCode(theEnv, fileName, pathName, fileNameBuffer,
                                   version);
  version = BitMapHashNodesToCode(theEnv, fileName, pathName, fileNameBuffer,
                                  version);
...
/*****************************************************/
static int SymbolHashNodesToCode(void* theEnv, const char* fileName,
                                 const char* pathName, char* fileNameBuffer,
                                 int version) {
  unsigned long i, j;
  ...
  if ((fp = NewCFile(theEnv, fileName, pathName, fileNameBuffer, 1, version,
                     FALSE)) == NULL)
    return (-1);
    ...
    for (hashPtr = symbolTable[i]; hashPtr != NULL; hashPtr = hashPtr->next) {
      ...
      if (hashPtr->next == NULL) {
        ...
        if ((j + 1) >=
          ...
          fprintf(fp, "{&S%d_%d[%d],", ConstructCompilerData(theEnv)->ImageID,
                  arrayVersion + 1, 0);
        } else {
          fprintf(fp, "{&S%d_%d[%ld],", ConstructCompilerData(theEnv)->ImageID,
                  arrayVersion, j + 1);
        }
      }
      ...
      if ((count == numberOfEntries) ||
          (j >= (unsigned)ConstructCompilerData(theEnv)->MaxIndices)) {
        fprintf(fp, "}};\n");
        GenClose(theEnv, fp);
        j = 0;
        arrayVersion++;
        version++;
        if (count < numberOfEntries) {
          if ((fp = NewCFile(theEnv, fileName, pathName, fileNameBuffer, 1,
                             version, FALSE)) == NULL)
            return (0);
          newHeader = TRUE;
        }
...
/*   created using the constructs-to-c function.      */
/******************************************************/
static int BitMapHashNodesToCode(void* theEnv, const char* fileName,
                                 const char* pathName, char* fileNameBuffer,
                                 int version) {
  int i, j;
  ...
  for (i = 1;
       i <= (numberOfEntries / ConstructCompilerData(theEnv)->MaxIndices) + 1;
       i++) {
    fprintf(ConstructCompilerData(theEnv)->HeaderFP,
            "extern struct bitMapHashNode B%d_%d[];\n",
            ConstructCompilerData(theEnv)->ImageID, i);
  }
    ...
    for (hashPtr = bitMapTable[i]; hashPtr != NULL; hashPtr = hashPtr->next) {
      ...
      if (hashPtr->next == NULL) {
        ...
        if ((j + 1) >= ConstructCompilerData(theEnv)->MaxIndices) {
          fprintf(fp, "{&B%d_%d[%d],", ConstructCompilerData(theEnv)->ImageID,
                  arrayVersion + 1, 0);
        } else {
          fprintf(fp, "{&B%d_%d[%d],", ConstructCompilerData(theEnv)->ImageID,
                  arrayVersion, j + 1);
      ...
      fprintf(fp, "%ld,1,0,0,%d,(char *) &L%d_%d[%d],%d", hashPtr->count + 1, i,
              ConstructCompilerData(theEnv)->ImageID, longsReqdPartition,
              longsReqdPartitionCount, hashPtr->size);
...
/*****************************************************/
static int BitMapValuesToCode(void* theEnv, const char* fileName,
                              const char* pathName, char* fileNameBuffer,
                              int version) {
  int i, j, k;
  unsigned l;
  ...
  for (i = 0; i < BITMAP_HASH_SIZE; i++) {
    for (hashPtr = bitMapTable[i]; hashPtr != NULL; hashPtr = hashPtr->next) {
      ...
      if ((hashPtr->size % sizeof(unsigned long)) != 0) longsReqd++;
...
/*   created using the constructs-to-c function.    */
/****************************************************/
static int FloatHashNodesToCode(void* theEnv, const char* fileName,
                                const char* pathName, char* fileNameBuffer,
                                int version) {
  int i, j;
  ...
  for (i = 1;
       ...
       i++) {
    fprintf(ConstructCompilerData(theEnv)->HeaderFP,
            "extern struct floatHashNode F%d_%d[];\n",
            ConstructCompilerData(theEnv)->ImageID, i);
  }
  ...
  for (i = 0; i < FLOAT_HASH_SIZE; i++) {
    for (hashPtr = floatTable[i]; hashPtr != NULL; hashPtr = hashPtr->next) {
      ...
      if (hashPtr->next == NULL) {
        ...
        if ((j + 1) >= ConstructCompilerData(theEnv)->MaxIndices) {
          fprintf(fp, "{&F%d_%d[%d],", ConstructCompilerData(theEnv)->ImageID,
                  arrayVersion + 1, 0);
        } else {
          fprintf(fp, "{&F%d_%d[%d],", ConstructCompilerData(theEnv)->ImageID,
                  arrayVersion, j + 1);
...
/*   created using the constructs-to-c function.      */
/******************************************************/
static int IntegerHashNodesToCode(void* theEnv, const char* fileName,
                                  const char* pathName, char* fileNameBuffer,
                                  int version) {
  int i, j;
  ...
  for (i = 0; i < INTEGER_HASH_SIZE; i++) {
    for (hashPtr = integerTable[i]; hashPtr != NULL; hashPtr = hashPtr->next) {
      numberOfEntries++;
    }
  }
  ...
  for (i = 1;
       ...
       i++) {
    fprintf(ConstructCompilerData(theEnv)->HeaderFP,
            "extern struct integerHashNode I%d_%d[];\n",
            ConstructCompilerData(theEnv)->ImageID, i);
  }
  ...
  for (i = 0; i < INTEGER_HASH_SIZE; i++) {
    for (hashPtr = integerTable[i]; hashPtr != NULL; hashPtr = hashPtr->next) {
      ...
      if (hashPtr->next == NULL) {
        ...
        if ((j + 1) >= ConstructCompilerData(theEnv)->MaxIndices) {
          fprintf(fp, "{&I%d_%d[%d],", ConstructCompilerData(theEnv)->ImageID,
                  arrayVersion + 1, 0);
        } else {
          fprintf(fp, "{&I%d_%d[%d],", ConstructCompilerData(theEnv)->ImageID,
                  arrayVersion, j + 1);
...
/*   created using the constructs-to-c function.                */
/****************************************************************/
static int HashTablesToCode(void* theEnv, const char* fileName,
                            const char* pathName, char* fileNameBuffer) {
  unsigned long i;
  FILE* fp;
  struct symbolHashNode** symbolTable;
  ...
  if ((fp = NewCFile(theEnv, fileName, pathName, fileNameBuffer, 1, 1,
                     FALSE)) == NULL)
    return (0);

  fprintf(ConstructCompilerData(theEnv)->HeaderFP,
          "extern struct symbolHashNode *sht%d[];\n",
  ...
  if ((fp = NewCFile(theEnv, fileName, pathName, fileNameBuffer, 1, 2,
                     FALSE)) == NULL)
    return (0);

  fprintf(ConstructCompilerData(theEnv)->HeaderFP,
          "extern struct floatHashNode *fht%d[];\n",
          ConstructCompilerData(theEnv)->ImageID);
  ...
  for (i = 0; i < FLOAT_HASH_SIZE; i++) {
    if (floatTable[i] == NULL) {
      ...
      PrintFloatReference(theEnv, fp, floatTable[i]);
  ...
  if ((fp = NewCFile(theEnv, fileName, pathName, fileNameBuffer, 1, 3,
                     FALSE)) == NULL)
    return (0);

  fprintf(ConstructCompilerData(theEnv)->HeaderFP,
          "extern struct integerHashNode *iht%d[];\n",
          ConstructCompilerData(theEnv)->ImageID);
  ...
  for (i = 0; i < INTEGER_HASH_SIZE; i++) {
    if (integerTable[i] == NULL) {
      ...
      PrintIntegerReference(theEnv, fp, integerTable[i]);
  ...
  if ((fp = NewCFile(theEnv, fileName, pathName, fileNameBuffer, 1, 4,
                     FALSE)) == NULL)
    return (0);

  fprintf(ConstructCompilerData(theEnv)->HeaderFP,
          "extern struct bitMapHashNode *bmht%d[];\n",
          ConstructCompilerData(theEnv)->ImageID);
...
/*****************************************************/
globle void PrintSymbolReference(void* theEnv, FILE* theFile,
                                 struct symbolHashNode* theSymbol) {
  if (theSymbol == NULL)
    fprintf(theFile, "NULL");
  else
    fprintf(
        theFile, "&S%d_%d[%d]", ConstructCompilerData(theEnv)->ImageID,
...
/****************************************************/
globle void PrintFloatReference(void* theEnv, FILE* theFile,
                                struct floatHashNode* theFloat) {
  fprintf(
      theFile, "&F%d_%d[%d]", ConstructCompilerData(theEnv)->ImageID,
...
/******************************************************/
globle void PrintIntegerReference(void* theEnv, FILE* theFile,
                                  struct integerHashNode* theInteger) {
  fprintf(
      theFile, "&I%d_%d[%d]", ConstructCompilerData(theEnv)->ImageID,
...
/*****************************************************/
globle void PrintBitMapReference(void* theEnv, FILE* theFile,
                                 struct bitMapHashNode* theBitMap) {
  if (theBitMap == NULL)
    fprintf(theFile, "NULL");
  else
    fprintf(
        theFile, "&B%d_%d[%d]", ConstructCompilerData(theEnv)->ImageID,
...
/*********************************************************/
static void PrintCString(FILE* theFile, const char* str) {
  unsigned i;
  size_t slen;
