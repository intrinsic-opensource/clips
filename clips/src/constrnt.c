...
static int ConstraintCompare(struct constraintRecord*,
...
globle void InitializeConstraints(void* theEnv) {
  ...
  AllocateEnvironmentData(theEnv, CONSTRAINT_DATA,
  ...
  ConstraintData(theEnv)->ConstraintHashtable = (struct constraintRecord**)gm2(
      theEnv, (int)sizeof(struct constraintRecord*) * SIZE_CONSTRAINT_HASH);
...
#if (!RUN_TIME)
  EnvDefineFunction2(theEnv, "get-dynamic-constraint-checking", 'b', GDCCommand,
                     "GDCCommand", "00");
  EnvDefineFunction2(theEnv, "set-dynamic-constraint-checking", 'b', SDCCommand,
                     "SDCCommand", "11");

  EnvDefineFunction2(theEnv, "get-static-constraint-checking", 'b', GSCCommand,
                     "GSCCommand", "00");
  EnvDefineFunction2(theEnv, "set-static-constraint-checking", 'b', SSCCommand,
                     "SSCCommand", "11");
#endif
}
...
/*****************************************************/
static void DeallocateConstraintData(void* theEnv) {
#if !RUN_TIME
...
/*************************************************************/
static void ReturnConstraintRecord(void* theEnv,
...
/***************************************************/
static void DeinstallConstraintRecord(void* theEnv,
                                      CONSTRAINT_RECORD* constraints) {
  if (constraints->bucket >= 0) {
...
globle unsigned long HashConstraint(struct constraintRecord* theConstraint) {
  ...
  count += (unsigned long)(theConstraint->anyAllowed * 17) +
  ...
  count += (unsigned long)(theConstraint->externalAddressesAllowed * 29) +
  ...
  count += (unsigned long)(theConstraint->stringRestriction * 3) +
...
/**********************************************/
static int ConstraintCompare(struct constraintRecord* constraint1,
  ...
  if ((constraint1->anyAllowed != constraint2->anyAllowed) ||
      ...
      (constraint1->instanceNamesAllowed !=
       constraint2->instanceNamesAllowed) ||
  ...
  for (tmpPtr1 = constraint1->restrictionList,
      tmpPtr2 = constraint2->restrictionList;
       (tmpPtr1 != NULL) && (tmpPtr2 != NULL);
       tmpPtr1 = tmpPtr1->nextArg, tmpPtr2 = tmpPtr2->nextArg) {
    if ((tmpPtr1->type != tmpPtr2->type) ||
        (tmpPtr1->value != tmpPtr2->value)) {
  ...
  if (((constraint1->multifield == NULL) &&
...
globle struct constraintRecord* AddConstraint(
    void* theEnv, struct constraintRecord* theConstraint) {
...
/*   types found in a constraint record.         */
/*************************************************/
static void InstallConstraintRecord(void* theEnv,
                                    CONSTRAINT_RECORD* constraints) {
...
globle int SDCCommand(void* theEnv) {
  ...
  if (EnvArgCountCheck(theEnv, "set-dynamic-constraint-checking", EXACTLY, 1) ==
      -1) {
    return (oldValue);
  }
...
globle int GDCCommand(void* theEnv) {
  ...
  if (EnvArgCountCheck(theEnv, "get-dynamic-constraint-checking", EXACTLY, 0) ==
      -1) {
    return (oldValue);
  }
...
globle int SSCCommand(void* theEnv) {
  ...
  if (EnvArgCountCheck(theEnv, "set-static-constraint-checking", EXACTLY, 1) ==
      -1) {
    return (oldValue);
  }
...
globle int GSCCommand(void* theEnv) {
  ...
  if (EnvArgCountCheck(theEnv, "get-static-constraint-checking", EXACTLY, 0) ==
      -1) {
    return (oldValue);
  }
...
globle intBool SetDynamicConstraintChecking(int value) {
...
globle intBool SetStaticConstraintChecking(int value) {
