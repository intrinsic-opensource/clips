...
#define ClassBitMapSize(bmp) \
  ((sizeof(CLASS_BITMAP) + (sizeof(char) * (bmp->maxid / BITS_PER_BYTE))))
...
#define SlotBitMapSize(bmp) \
  ((sizeof(SLOT_BITMAP) + (sizeof(char) * (bmp->maxid / BITS_PER_BYTE))))
...
struct objectAlphaNode {
  ...
  struct objectAlphaNode *nxtInGroup, *nxtTerminal;
  long bsaveID;
};
