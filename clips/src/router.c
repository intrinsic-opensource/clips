...
globle void InitializeDefaultRouters(void* theEnv) {
  AllocateEnvironmentData(theEnv, ROUTER_DATA, sizeof(struct routerData),
...
#if (!RUN_TIME)
  EnvDefineFunction2(theEnv, "exit", 'v', PTIEF ExitCommand, "ExitCommand",
                     "*1i");
#endif
...
/*************************************************/
static void DeallocateRouterData(void* theEnv) {
...
/*******************************************/
globle int EnvPrintRouter(void* theEnv, const char* logicalName,
                          const char* str) {
  ...
  while (currentPtr != NULL) {
    if ((currentPtr->printer != NULL)
            ? QueryRouter(theEnv, logicalName, currentPtr)
            : FALSE) {
      ...
      if (currentPtr->environmentAware) {
        ...
        ((int (*)(const char*, const char*))(*currentPtr->printer))(logicalName,
                                                                    str);
      }
...
/**************************************************/
globle int EnvGetcRouter(void* theEnv, const char* logicalName) {
  struct router* currentPtr;
  int inchar;
  ...
  if (((char*)RouterData(theEnv)->FastLoadFilePtr) == logicalName) {
    ...
    if (inchar == '\n') {
      if (((char*)RouterData(theEnv)->FastLoadFilePtr) ==
  ...
  if (RouterData(theEnv)->FastCharGetRouter == logicalName) {
    inchar = (unsigned char)RouterData(theEnv)
...
/******************************************************/
globle int EnvUngetcRouter(void* theEnv, int ch, const char* logicalName) {
  struct router* currentPtr;
  ...
  while (currentPtr != NULL) {
    if ((currentPtr->charunget != NULL)
      ...
      if (currentPtr->environmentAware) {
        ...
        return (((int (*)(int, const char*))(*currentPtr->charunget))(
            ch, logicalName));
      }
    }
...
globle void ExitCommand(void* theEnv) {
  ...
  if ((argCnt = EnvArgCountCheck(theEnv, "exit", NO_MORE_THAN, 1)) == -1)
    return;
  if (argCnt == 0) {
...
/************************************************************/
globle intBool EnvAddRouter(void* theEnv, const char* routerName, int priority,
                            int (*queryFunction)(void*, const char*),
                            int (*printFunction)(void*, const char*,
                                                 const char*),
                            int (*getcFunction)(void*, const char*),
                            int (*ungetcFunction)(void*, int, const char*),
                            int (*exitFunction)(void*, int)) {
  return EnvAddRouterWithContext(theEnv, routerName, priority, queryFunction,
...
/***********************************************************************/
globle intBool
EnvAddRouterWithContext(void* theEnv, const char* routerName, int priority,
                        int (*queryFunction)(void*, const char*),
                        int (*printFunction)(void*, const char*, const char*),
                        int (*getcFunction)(void*, const char*),
                        int (*ungetcFunction)(void*, int, const char*),
                        int (*exitFunction)(void*, int), void* context) {
...
/********************************************************************/
globle int EnvDeleteRouter(void* theEnv, const char* routerName) {
...
/*********************************************************************/
globle int QueryRouters(void* theEnv, const char* logicalName) {
  struct router* currentPtr;
...
/************************************************/
static int QueryRouter(void* theEnv, const char* logicalName,
                       struct router* currentPtr) {
  /*===================================================*/
...
/*******************************************************/
globle int EnvDeactivateRouter(void* theEnv, const char* routerName) {
  struct router* currentPtr;
...
/***************************************************/
globle int EnvActivateRouter(void* theEnv, const char* routerName) {
  struct router* currentPtr;
...
/*****************************************************/
globle void UnrecognizedRouterMessage(void* theEnv, const char* logicalName) {
...
/*****************************************/
globle int PrintNRouter(void* theEnv, const char* logicalName, const char* str,
...
globle int ActivateRouter(const char* routerName) {
  return EnvActivateRouter(GetCurrentEnvironment(), routerName);
}

globle intBool AddRouter(const char* routerName, int priority,
...
globle int DeactivateRouter(const char* routerName) {
  return EnvDeactivateRouter(GetCurrentEnvironment(), routerName);
}

globle int DeleteRouter(const char* routerName) {
  return EnvDeleteRouter(GetCurrentEnvironment(), routerName);
}

globle void ExitRouter(int num) { EnvExitRouter(GetCurrentEnvironment(), num); }

globle int GetcRouter(const char* logicalName) {
  return EnvGetcRouter(GetCurrentEnvironment(), logicalName);
}

globle int PrintRouter(const char* logicalName, const char* str) {
  return EnvPrintRouter(GetCurrentEnvironment(), logicalName, str);
}

globle int UngetcRouter(int ch, const char* logicalName) {
