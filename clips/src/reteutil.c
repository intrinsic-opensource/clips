...
static void TraceErrorToRuleDriver(void*, struct joinNode*, const char*, int,
                                   int);
static struct alphaMemoryHash* FindAlphaMemory(void*, struct patternNodeHeader*,
                                               unsigned long);
static unsigned long AlphaMemoryHashValue(struct patternNodeHeader*,
                                          unsigned long);
static void UnlinkAlphaMemory(void*, struct patternNodeHeader*,
...
static void TagNetworkTraverseJoins(void*, long int*, long int*,
                                    struct joinNode*);
#endif
...
/***********************************************************/
globle void PrintPartialMatch(void* theEnv, const char* logicalName,
  ...
  for (i = 0; i < list->bcount;) {
    if ((get_nth_pm_match(list, i) != NULL) &&
      ...
      (*matchingItem->theInfo->base.shortPrintFunction)(theEnv, logicalName,
                                                        matchingItem);
    } else {
...
/* InitializePMLinks: */
/**********************/
static void InitializePMLinks(struct partialMatch* theMatch) {
...
/* UpdateBetaPMLinks: */
/**********************/
globle void UpdateBetaPMLinks(void* theEnv, struct partialMatch* thePM,
                              struct partialMatch* lhsBinds,
                              struct partialMatch* rhsBinds,
                              struct joinNode* join, unsigned long hashValue,
                              int side) {
...
/***********************************************************/
globle void UnlinkBetaPMFromNodeAndLineage(void* theEnv, struct joinNode* join,
...
/***********************************************************/
globle void UnlinkNonLeftLineage(void* theEnv, struct joinNode* join,
...
globle struct partialMatch* MergePartialMatches(void* theEnv,
  ...
  static struct partialMatch mergeTemplate = {
      1}; /* betaMemory is TRUE, remainder are 0 or NULL */
...
globle struct partialMatch* CreateAlphaMatch(
    void* theEnv, void* theEntity, struct multifieldMarker* markers,
    struct patternNodeHeader* theHeader, unsigned long hashOffset) {
...
struct multifieldMarker* CopyMultifieldMarkers(
    void* theEnv, struct multifieldMarker* theMarkers) {
...
/************************************************************************/
globle void TraceErrorToRule(void* theEnv, struct joinNode* joinPtr,
...
/**************************************************************/
static void TraceErrorToRuleDriver(void* theEnv, struct joinNode* joinPtr,
                                   const char* indentSpaces,
...
/* CountPriorPatterns:            */
/**************************************************************/
static int CountPriorPatterns(struct joinNode* joinPtr) {
  int count = 0;

  while (joinPtr != NULL) {
    if (joinPtr->joinFromTheRight) {
      count += CountPriorPatterns(
...
globle void ReturnLeftMemory(void* theEnv, struct joinNode* theJoin) {
  ...
  genfree(theEnv, theJoin->leftMemory->beta,
...
globle void ReturnRightMemory(void* theEnv, struct joinNode* theJoin) {
  ...
  genfree(theEnv, theJoin->rightMemory->beta,
          sizeof(struct partialMatch*) * theJoin->rightMemory->size);
  genfree(theEnv, theJoin->rightMemory->last,
          sizeof(struct partialMatch*) * theJoin->rightMemory->size);
...
/*   deleted).                                                  */
/****************************************************************/
globle void DestroyBetaMemory(void* theEnv, struct joinNode* theJoin,
                              int side) {
  unsigned long i;
...
/*****************************************************************/
static unsigned long AlphaMemoryHashValue(struct patternNodeHeader* theHeader,
                                          unsigned long hashOffset) {
  unsigned long hashValue;
  union {
    void* vv;
...
/* UnlinkAlphaMemory:  */
/*****************************************************************/
static void UnlinkAlphaMemory(void* theEnv, struct patternNodeHeader* theHeader,
                              struct alphaMemoryHash* theAlphaMemory) {
  /*======================*/
  /* Unlink the siblings. */
  /*======================*/
...
static void UnlinkAlphaMemoryBucketSiblings(
    void* theEnv, struct alphaMemoryHash* theAlphaMemory) {
...
/* ResizeBetaMemory:                                */
/***********************************************************/
globle void ResizeBetaMemory(void* theEnv, struct betaMemory* theMemory) {
  ...
  theMemory->beta = (struct partialMatch**)genalloc(
      theEnv, sizeof(struct partialMatch*) * theMemory->size);

  lastAdd = (struct partialMatch**)genalloc(
      theEnv, sizeof(struct partialMatch*) * theMemory->size);
...
/* ResetBetaMemory:                                */
/***********************************************************/
static void ResetBetaMemory(void* theEnv, struct betaMemory* theMemory) {
...
/* PrintBetaMemory: */
/********************/
globle unsigned long PrintBetaMemory(void* theEnv, const char* logName,
                                     struct betaMemory* theMemory,
                                     int indentFirst, const char* indentString,
                                     int output) {
...
/*************************************************************/
globle void TagRuleNetwork(void* theEnv, long int* moduleCount,
                           long int* ruleCount, long int* joinCount,
                           long int* linkCount) {
  struct defmodule* modulePtr;
  struct defrule *rulePtr, *disjunctPtr;
...
/*******************************************************************/
static void TagNetworkTraverseJoins(void* theEnv, long int* joinCount,
                                    long int* linkCount,
                                    struct joinNode* joinPtr) {
  struct joinLink* theLink;
  for (; joinPtr != NULL; joinPtr = joinPtr->lastLevel) {
    ...
    if (joinPtr->joinFromTheRight) {
      TagNetworkTraverseJoins(
          theEnv, joinCount, linkCount,
          (struct joinNode*)joinPtr->rightSideEntryStructure);
    }
  }
}
