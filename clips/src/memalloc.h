...
#define get_struct(theEnv, type)                                      \
  ((MemoryData(theEnv)->MemoryTable[sizeof(struct type)] == NULL)     \
       ...
       : ((MemoryData(theEnv)->TempMemoryPtr =                        \
               MemoryData(theEnv)->MemoryTable[sizeof(struct type)]), \
          MemoryData(theEnv)->MemoryTable[sizeof(struct type)] =      \
              MemoryData(theEnv)->TempMemoryPtr->next,                \
...
#define rtn_sized_struct(theEnv, size, struct_ptr)                    \
  (MemoryData(theEnv)->TempMemoryPtr = (struct memoryPtr*)struct_ptr, \
   MemoryData(theEnv)->TempMemoryPtr->next =                          \
       MemoryData(theEnv)->MemoryTable[size],                         \
...
#define get_var_struct(theEnv, type, vsize)                                   \
  ((((sizeof(struct type) + vsize) < MEM_TABLE_SIZE)                          \
        ? (MemoryData(theEnv)->MemoryTable[sizeof(struct type) + vsize] ==    \
           NULL)                                                              \
        : 1)                                                                  \
...
#define rtn_var_struct(theEnv, type, vsize, struct_ptr)                       \
  (MemoryData(theEnv)->TempSize = sizeof(struct type) + vsize,                \
   ((MemoryData(theEnv)->TempSize < MEM_TABLE_SIZE)                           \
        ? (MemoryData(theEnv)->TempMemoryPtr = (struct memoryPtr*)struct_ptr, \
           MemoryData(theEnv)->TempMemoryPtr->next =                          \
               MemoryData(theEnv)->MemoryTable[MemoryData(theEnv)->TempSize], \
           MemoryData(theEnv)->MemoryTable[MemoryData(theEnv)->TempSize] =    \
               MemoryData(theEnv)->TempMemoryPtr)                             \
        : (genfree(theEnv, (void*)struct_ptr, MemoryData(theEnv)->TempSize),  \
           (struct memoryPtr*)struct_ptr)))

#define get_mem(theEnv, size)                                                 \
  (((size < MEM_TABLE_SIZE) ? (MemoryData(theEnv)->MemoryTable[size] == NULL) \
                            : 1)                                              \
  ...
  (MemoryData(theEnv)->TempSize = size,                                       \
   ((MemoryData(theEnv)->TempSize < MEM_TABLE_SIZE)                           \
        ? (MemoryData(theEnv)->TempMemoryPtr = (struct memoryPtr*)ptr,        \
           ...
           MemoryData(theEnv)->MemoryTable[MemoryData(theEnv)->TempSize] =    \
               MemoryData(theEnv)->TempMemoryPtr)                             \
        : (genfree(theEnv, (void*)ptr, MemoryData(theEnv)->TempSize),         \
           (struct memoryPtr*)ptr)))

#else  // MEM_TABLE_SIZE == 0
/*
 * Debug case (routes all memory management through genalloc/genfree to take
 * advantage of platform, memory debugging aids)
 */
#define get_struct(theEnv, type) \
  ((struct type*)genalloc(theEnv, sizeof(struct type)))

#define rtn_struct(theEnv, type, struct_ptr) \
  (genfree(theEnv, struct_ptr, sizeof(struct type)))

#define rtn_sized_struct(theEnv, size, struct_ptr) \
  (genfree(theEnv, struct_ptr, size))

#define get_var_struct(theEnv, type, vsize) \
  ((struct type*)genalloc(theEnv, (sizeof(struct type) + vsize)))

#define rtn_var_struct(theEnv, type, vsize, struct_ptr) \
  (genfree(theEnv, struct_ptr, sizeof(struct type) + vsize))
...
#define MemoryData(theEnv) \
  ((struct memoryData*)GetEnvironmentData(theEnv, MEMORY_DATA))

LOCALE void InitializeMemory(void*);
LOCALE void* genalloc(void*, size_t);
LOCALE int DefaultOutOfMemoryFunction(void*, size_t);
LOCALE int (*EnvSetOutOfMemoryFunction(void*, int (*)(void*, size_t)))(void*,
                                                                       size_t);
