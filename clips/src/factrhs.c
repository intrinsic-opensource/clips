...
/**********************************************************************/
globle struct expr* BuildRHSAssert(void* theEnv, const char* logicalName,
                                   struct token* theToken, int* error,
                                   int atLeastOne, int readFirstParen,
  ...
  *error = FALSE;
  ...
  while ((nextOne = GetRHSPattern(theEnv, logicalName, theToken, error, FALSE,
                                  readFirstParen, TRUE, RPAREN)) != NULL) {
...
/***************************************************************/
globle struct expr* GetRHSPattern(void* theEnv, const char* readSource,
                                  struct token* tempToken, int* error,
                                  int constantsOnly, int readFirstParen,
                                  int checkFirstParen, int endType) {
  ...
  *error = FALSE;
  ...
  if (ReservedPatternSymbol(theEnv, ValueToString(templateName), NULL)) {
    ReservedPatternSymbolErrorMsg(theEnv, ValueToString(templateName),
                                  "a relation name");
    *error = TRUE;
    return (NULL);
  }
  ...
  theDeftemplate = (struct deftemplate*)FindImportedConstruct(
      theEnv, "deftemplate", NULL, ValueToString(templateName), &count, TRUE,
      NULL);

  if (count > 1) {
    AmbiguousReferenceErrorMessage(theEnv, "deftemplate",
...
#if (!BLOAD_ONLY) && (!RUN_TIME)
  {
    ...
    if (FindImportExportConflict(
            theEnv, "deftemplate",
            ((struct defmodule*)EnvGetCurrentModule(theEnv)),
            ValueToString(templateName))) {
      ImportExportConflictMessage(theEnv, "implied deftemplate",
                                  ValueToString(templateName), NULL, NULL);
      *error = TRUE;
      return (NULL);
    }
#endif
    if (!ConstructData(theEnv)->CheckSyntaxMode) {
      theDeftemplate =
          CreateImpliedDeftemplate(theEnv, (SYMBOL_HN*)templateName, TRUE);
    }
  }
#else
  {
  ...
  if ((theDeftemplate != NULL) && (theDeftemplate->implied == FALSE)) {
    ...
    firstOne->nextArg =
        ParseAssertTemplate(theEnv, readSource, tempToken, error, endType,
  ...
  while ((nextOne = GetAssertArgument(theEnv, readSource, tempToken, error,
                                      endType, constantsOnly, &printError)) !=
         NULL) {
    if (argHead == NULL)
      argHead = nextOne;
    else
  ...
  firstOne->nextArg = GenConstant(theEnv, FACT_STORE_MULTIFIELD,
                                  EnvAddBitMap(theEnv, (void*)nullBitMap, 1));
...
/********************************************************************/
globle struct expr* GetAssertArgument(void* theEnv, const char* logicalName,
                                      struct token* theToken, int* error,
                                      int endType, int constantsOnly,
                                      int* printError) {
#if !RUN_TIME
  ...
  *printError = TRUE;
  ...
  if ((theToken->type == SYMBOL)
    ...
    if (theToken->type == LPAREN)
      nextField = Function1Parse(theEnv, logicalName);
    else
      nextField = Function0Parse(theEnv, logicalName);
    if (nextField == NULL)
#endif
    {
      *printError = FALSE;
...
/****************************************************/
globle struct fact* StringToFact(void* theEnv, const char* str) {
  ...
  assertArgs = GetRHSPattern(theEnv, "assert_str", &theToken, &error, FALSE,
                             TRUE, TRUE, RPAREN);
...
/* an assert                                             */
/*********************************************************/
static void NoSuchTemplateError(void* theEnv, const char* templateName) {
