...
static struct lhsParseNode* GetLHSSlots(void*, const char*, struct token*,
...
globle struct lhsParseNode* DeftemplateLHSParse(
    void* theEnv, const char* readSource, struct deftemplate* theDeftemplate) {
...
/******************************************/
static struct lhsParseNode* GetLHSSlots(void* theEnv, const char* readSource,
  ...
  while (tempToken->type != RPAREN) {
    ...
    if (MultiplyDefinedLHSSlots(theEnv, firstSlot,
                                (SYMBOL_HN*)tempToken->value) == TRUE) {
      *error = TRUE;
      ReturnLHSParseNodes(theEnv, firstSlot);
      return (NULL);
    }
    ...
    nextSlot = GetSingleLHSSlot(theEnv, readSource, tempToken, slotPtr, error,
                                (short)(position + 1));
    if (*error) {
      ReturnLHSParseNodes(theEnv, firstSlot);
...
/*****************************************************/
static struct lhsParseNode* GetSingleLHSSlot(void* theEnv,
                                             const char* readSource,
                                             struct token* tempToken,
                                             struct templateSlot* slotPtr,
                                             int* error, short position) {
  ...
  if (slotPtr->multislot == FALSE) {
    ...
    if ((nextSlot->type == MF_VARIABLE) || (nextSlot->type == MULTIFIELD)) {
  ...
  /*===================================*/
    ...
    nextSlot = RestrictionParse(theEnv, readSource, tempToken, TRUE, slotName,
                                (short)(position - 1), slotPtr->constraints, 1);
    if (nextSlot == NULL) {
      *error = TRUE;
      return (NULL);
    }
...
/******************************************************/
static intBool MultiplyDefinedLHSSlots(void* theEnv,
