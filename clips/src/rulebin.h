...
#define DefruleBinaryData(theEnv) \
  ((struct defruleBinaryData*)GetEnvironmentData(theEnv, RULEBIN_DATA))

#define BloadDefrulePointer(x, i) ((struct defrule*)((i == -1L) ? NULL : &x[i]))
#define BsaveJoinIndex(joinPtr) \
  ((joinPtr == NULL) ? -1L : ((struct joinNode*)joinPtr)->bsaveID)
#define BloadJoinPointer(i)             \
  ((struct joinNode*)((i == -1L) ? NULL \
                                 : &DefruleBinaryData(theEnv)->JoinArray[i]))
#define BsaveJoinLinkIndex(linkPtr) \
  ((linkPtr == NULL) ? -1L : ((struct joinLink*)linkPtr)->bsaveID)
#define BloadJoinLinkPointer(i)         \
  ((struct joinLink*)((i == -1L) ? NULL \
                                 : &DefruleBinaryData(theEnv)->LinkArray[i]))
