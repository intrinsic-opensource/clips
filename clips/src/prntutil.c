...
globle void InitializePrintUtilityData(void* theEnv) {
  AllocateEnvironmentData(theEnv, PRINT_UTILITY_DATA,
...
/***********************************************************/
globle void PrintInChunks(void* theEnv, const char* logicalName,
...
/************************************************************/
globle void PrintFloat(void* theEnv, const char* fileid, double number) {
...
/****************************************************/
globle void PrintLongInteger(void* theEnv, const char* logicalName,
                             long long number) {
  char printBuffer[32];
...
/**************************************/
globle void PrintAtom(void* theEnv, const char* logicalName, int type,
                      void* value) {
  ...
  switch (type) {
    ...
    case STRING:
      if (PrintUtilityData(theEnv)->PreserveEscapedCharacters) {
        EnvPrintRouter(theEnv, logicalName,
                       StringPrintForm(theEnv, ValueToString(value)));
      } else {
        EnvPrintRouter(theEnv, logicalName, "\"");
        EnvPrintRouter(theEnv, logicalName, ValueToString(value));
        EnvPrintRouter(theEnv, logicalName, "\"");
    ...
    case EXTERNAL_ADDRESS:
      ...
      if ((EvaluationData(theEnv)->ExternalAddressTypes[theAddress->type] !=
          ...
          (EvaluationData(theEnv)
               ...
               ->longPrintFunction != NULL)) {
        (*EvaluationData(theEnv)
              ->ExternalAddressTypes[theAddress->type]
              ->longPrintFunction)(theEnv, logicalName, value);
      } else {
        EnvPrintRouter(theEnv, logicalName, "<Pointer-");
    ...
    default:
      ...
      if (EvaluationData(theEnv)->PrimitivesArray[type]->longPrintFunction ==
          NULL) {
        EnvPrintRouter(theEnv, logicalName, "<unknown atom type>");
        break;
      }
      (*EvaluationData(theEnv)->PrimitivesArray[type]->longPrintFunction)(
          theEnv, logicalName, value);
      break;
  }
}
...
/**********************************************************/
globle void PrintTally(void* theEnv, const char* logicalName, long long count,
  ...
  if (count == 1)
    EnvPrintRouter(theEnv, logicalName, singular);
  else
    EnvPrintRouter(theEnv, logicalName, plural);
...
/********************************************/
globle void PrintErrorID(void* theEnv, const char* module, int errorID,
                         int printCR) {
...
/**********************************************/
globle void PrintWarningID(void* theEnv, const char* module, int warningID,
                           int printCR) {
#if (!RUN_TIME) && (!BLOAD_ONLY)
  FlushParsingMessages(theEnv);
...
/***************************************************/
globle void CantFindItemErrorMessage(void* theEnv, const char* itemType,
                                     const char* itemName) {
...
/*****************************************************/
globle void CantDeleteItemErrorMessage(void* theEnv, const char* itemType,
                                       const char* itemName) {
...
/****************************************************/
globle void AlreadyParsedErrorMessage(void* theEnv, const char* itemType,
                                      const char* itemName) {
...
/*********************************************************/
globle void SyntaxErrorMessage(void* theEnv, const char* location) {
...
/****************************************************/
globle void LocalVariableErrorMessage(void* theEnv, const char* byWhat) {
...
/******************************************/
globle void SystemError(void* theEnv, const char* module, int errorID) {
...
/*******************************************************/
globle void DivideByZeroErrorMessage(void* theEnv, const char* functionName) {
...
/********************************************************/
void ArgumentOverUnderflowErrorMessage(void* theEnv, const char* functionName) {
...
/*******************************************************/
globle const char* FloatToString(void* theEnv, double number) {
  char floatString[40];
  int i;
  char x;
  void* thePtr;
...
/*******************************************************************/
globle const char* LongIntegerToString(void* theEnv, long long number) {
  char buffer[50];
  void* thePtr;
...
/*******************************************************************/
globle const char* DataObjectToString(void* theEnv, DATA_OBJECT* theDO) {
  void* thePtr;
  const char* theString;
  char* newString;
  ...
  switch (GetpType(theDO)) {
    ...
    case INSTANCE_ADDRESS:
      ...
      if (((struct instance*)thePtr)->garbage) {
        ...
        prefix = "<Instance-";
        theString =
            ValueToString(GetFullInstanceName(theEnv, (INSTANCE_TYPE*)thePtr));
        postfix = ">";
      }
    ...
    case EXTERNAL_ADDRESS:
      ...
      gensprintf(buffer, "<Pointer-%d-%p>", (int)theAddress->type,
                 DOPToExternalAddress(theDO));
      thePtr = EnvAddSymbol(theEnv, buffer);
      return (ValueToString(thePtr));
...
/************************************************************/
globle void SalienceInformationError(void* theEnv, const char* constructType,
...
/****************************************************/
void FactRetractedErrorMessage(void* theEnv, void* theVFact) {
  char tempBuffer[20];
...
/*   has been retracted.                            */
/****************************************************/
void FactVarSlotErrorMessage1(void* theEnv, void* theVFact,
                              const char* varSlot) {
  char tempBuffer[20];
  struct fact* theFact = (struct fact*)theVFact;
...
/*   slot.                                          */
/****************************************************/
void FactVarSlotErrorMessage2(void* theEnv, void* theVFact,
                              const char* varSlot) {
  char tempBuffer[20];
  struct fact* theFact = (struct fact*)theVFact;
...
/*   slot.                                            */
/******************************************************/
void InvalidVarSlotErrorMessage(void* theEnv, const char* varSlot) {
...
/*******************************************************/
void InstanceVarSlotErrorMessage1(void* theEnv, void* theVInstance,
                                  const char* varSlot) {
...
/*   an invalid slot.                           */
/************************************************/
void InstanceVarSlotErrorMessage2(void* theEnv, void* theVInstance,
                                  const char* varSlot) {
  struct instance* theInstance = (struct instance*)theVInstance;
...
/***************************************************/
globle void SlotExistError(void* theEnv, const char* sname, const char* func) {
