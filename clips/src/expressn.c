...
#if (!RUN_TIME)
static long ListToPacked(struct expr*, struct expr*, long);
static EXPRESSION_HN* FindHashedExpression(void*, EXPRESSION*, unsigned*,
                                           EXPRESSION_HN**);
...
globle void InitExpressionData(void* theEnv) {
  ...
  AllocateEnvironmentData(theEnv, EXPRESSION_DATA,
  ...
  ExpressionData(theEnv)->ExpressionHashTable = (EXPRESSION_HN**)gm2(
      theEnv, (int)(sizeof(EXPRESSION_HN*) * EXPRESSION_HASH_SIZE));
...
/*****************************************/
static void DeallocateExpressionData(void* theEnv) {
#if !RUN_TIME
  int i;
...
globle struct expr* PackExpression(void* theEnv, struct expr* original) {
  ...
  packPtr = (struct expr*)gm3(
      theEnv, (long)sizeof(struct expr) * (long)ExpressionSize(original));
...
/***********************************************************/
static long ListToPacked(struct expr* original, struct expr* destination,
                         long count) {
  long i;
  ...
  while (original != NULL) {
    ...
    if (original->argList == NULL) {
      ...
      destination[i].argList = (struct expr*)&destination[(long)count];
    ...
    if (original->nextArg == NULL) {
      ...
      destination[i].nextArg = (struct expr*)&destination[(long)count];
    }
...
globle void ReturnPackedExpression(void* theEnv, struct expr* packPtr) {
  if (packPtr != NULL) {
    rm3(theEnv, (void*)packPtr,
        (long)sizeof(struct expr) * ExpressionSize(packPtr));
  }
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static EXPRESSION_HN* FindHashedExpression(void* theEnv, EXPRESSION* theExp,
                                           unsigned* hashval,
                                           EXPRESSION_HN** prv) {
  ...
  *prv = NULL;
  ...
  while (exphash != NULL) {
    if (IdenticalExpression(exphash->exp, theExp)) return (exphash);
    *prv = exphash;
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static unsigned HashExpression(EXPRESSION* theExp) {
...
globle void RemoveHashedExpression(void* theEnv, EXPRESSION* theExp) {
  ...
  if (exphash == NULL) return;
  if (--exphash->count != 0) return;
  if (prv == NULL)
