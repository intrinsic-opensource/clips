...
/*********************************************/
globle void DefglobalBinarySetup(void* theEnv) {
  AllocateEnvironmentData(theEnv, GLOBLBIN_DATA,
                          sizeof(struct defglobalBinaryData),
                          DeallocateDefglobalBloadData);
#if (BLOAD_AND_BSAVE || BLOAD)
  AddAfterBloadFunction(theEnv, "defglobal", ResetDefglobals, 50);
#endif

#if BLOAD_AND_BSAVE
  AddBinaryItem(theEnv, "defglobal", 0, BsaveFind, NULL, BsaveStorage,
                BsaveBinaryItem, BloadStorageDefglobals, BloadBinaryItem,
                ClearBload);
#endif

#if (BLOAD || BLOAD_ONLY)
  AddBinaryItem(theEnv, "defglobal", 0, NULL, NULL, NULL, NULL,
                BloadStorageDefglobals, BloadBinaryItem, ClearBload);
#endif
}
...
/*********************************************************/
static void DeallocateDefglobalBloadData(void* theEnv) {
#if (BLOAD || BLOAD_ONLY || BLOAD_AND_BSAVE) && (!RUN_TIME)
  size_t space;
  long i;

  for (i = 0; i < DefglobalBinaryData(theEnv)->NumberOfDefglobals; i++) {
    if (DefglobalBinaryData(theEnv)->DefglobalArray[i].current.type ==
        MULTIFIELD) {
      ReturnMultifield(theEnv, (struct multifield*)DefglobalBinaryData(theEnv)
                                   ->DefglobalArray[i]
                                   .current.value);
    }
  }

  space = DefglobalBinaryData(theEnv)->NumberOfDefglobals *
          sizeof(struct defglobal);
  if (space != 0) {
    genfree(theEnv, (void*)DefglobalBinaryData(theEnv)->DefglobalArray, space);
  }

  space = DefglobalBinaryData(theEnv)->NumberOfDefglobalModules *
...
static void BsaveFind(void* theEnv) {
  ...
  for (theModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    ...
    for (defglobalPtr = (struct defglobal*)EnvGetNextDefglobal(theEnv, NULL);
         defglobalPtr != NULL;
         defglobalPtr =
             (struct defglobal*)EnvGetNextDefglobal(theEnv, defglobalPtr)) {
      /*======================================================*/
...
static void BsaveStorage(void* theEnv, FILE* fp) {
  ...
  GenWrite(&DefglobalBinaryData(theEnv)->NumberOfDefglobals, sizeof(long int),
           fp);
  GenWrite(&DefglobalBinaryData(theEnv)->NumberOfDefglobalModules,
           sizeof(long int), fp);
}
...
static void BsaveBinaryItem(void* theEnv, FILE* fp) {
  ...
  for (theModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    ...
    theModuleItem = (struct defglobalModule*)GetModuleItem(
        theEnv, NULL, FindModuleItem(theEnv, "defglobal")->moduleIndex);
  ...
  for (theModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    ...
    for (theDefglobal = (struct defglobal*)EnvGetNextDefglobal(theEnv, NULL);
      ...
      newDefglobal.initial =
          HashedExpressionIndex(theEnv, theDefglobal->initial);
...
/***********************************************/
static void BloadStorageDefglobals(void* theEnv) {
  size_t space;
  ...
  GenReadBinary(theEnv, &DefglobalBinaryData(theEnv)->NumberOfDefglobals,
                sizeof(long int));
  GenReadBinary(theEnv, &DefglobalBinaryData(theEnv)->NumberOfDefglobalModules,
                sizeof(long int));
  ...
  DefglobalBinaryData(theEnv)->ModuleArray =
      (struct defglobalModule*)genalloc(theEnv, space);
  ...
  DefglobalBinaryData(theEnv)->DefglobalArray =
      (struct defglobal*)genalloc(theEnv, space);
}
...
static void BloadBinaryItem(void* theEnv) {
  ...
  BloadandRefresh(theEnv, DefglobalBinaryData(theEnv)->NumberOfDefglobalModules,
                  sizeof(struct bsaveDefglobalModule), UpdateDefglobalModule);
...
/************************************************/
static void UpdateDefglobalModule(void* theEnv, void* buf, long obji) {
  ...
  UpdateDefmoduleItemHeader(
      ...
      &DefglobalBinaryData(theEnv)->ModuleArray[obji].header,
...
/******************************************/
static void UpdateDefglobal(void* theEnv, void* buf, long obji) {
  struct bsaveDefglobal* bdp;

  bdp = (struct bsaveDefglobal*)buf;
  UpdateConstructHeader(
      theEnv, &bdp->header,
      &DefglobalBinaryData(theEnv)->DefglobalArray[obji].header,
      (int)sizeof(struct defglobalModule),
      (void*)DefglobalBinaryData(theEnv)->ModuleArray,
      (int)sizeof(struct defglobal),
      (void*)DefglobalBinaryData(theEnv)->DefglobalArray);

#if DEBUGGING_FUNCTIONS
  DefglobalBinaryData(theEnv)->DefglobalArray[obji].watch =
      DefglobalData(theEnv)->WatchGlobals;
#endif
...
static void ClearBload(void* theEnv) {
  ...
  for (i = 0; i < DefglobalBinaryData(theEnv)->NumberOfDefglobals; i++) {
    UnmarkConstructHeader(
        theEnv, &DefglobalBinaryData(theEnv)->DefglobalArray[i].header);
...
/********************************************************/
globle void* BloadDefglobalModuleReference(void* theEnv, int theIndex) {
