...
globle void InitializeFactReteFunctions(void* theEnv) {
  ...
  struct entityRecord factJNGV1Info = {"FACT_JN_VAR1",
                                       FACT_JN_VAR1,
                                       0,
                                       1,
                                       0,
  ...
  struct entityRecord factJNGV2Info = {"FACT_JN_VAR2",
                                       FACT_JN_VAR2,
                                       0,
                                       1,
                                       0,
  ...
  struct entityRecord factJNGV3Info = {"FACT_JN_VAR3",
                                       FACT_JN_VAR3,
                                       0,
                                       1,
                                       0,
  ...
  struct entityRecord factPNGV1Info = {"FACT_PN_VAR1",
                                       FACT_PN_VAR1,
                                       0,
                                       1,
                                       0,
  ...
  struct entityRecord factPNGV2Info = {"FACT_PN_VAR2",
                                       FACT_PN_VAR2,
                                       0,
                                       1,
                                       0,
  ...
  struct entityRecord factPNGV3Info = {"FACT_PN_VAR3",
                                       FACT_PN_VAR3,
                                       0,
                                       1,
                                       0,
  ...
  struct entityRecord factJNCV1Info = {"FACT_JN_CMP1",
                                       FACT_JN_CMP1,
                                       0,
                                       1,
                                       1,
  ...
  struct entityRecord factJNCV2Info = {"FACT_JN_CMP2",
                                       FACT_JN_CMP2,
                                       0,
                                       1,
                                       1,
  ...
  struct entityRecord factPNCV1Info = {"FACT_PN_CMP1",
                                       FACT_PN_CMP1,
                                       0,
                                       1,
                                       1,
  ...
  AllocateEnvironmentData(theEnv, FACTGEN_DATA, sizeof(struct factgenData),
                          NULL);
  ...
  memcpy(&FactgenData(theEnv)->FactStoreMFInfo, &factStoreMFInfo,
         sizeof(struct entityRecord));
  memcpy(&FactgenData(theEnv)->FactSlotLengthInfo, &factSlotLengthInfo,
         sizeof(struct entityRecord));
  memcpy(&FactgenData(theEnv)->FactPNConstant1Info, &factPNConstant1Info,
         sizeof(struct entityRecord));
  memcpy(&FactgenData(theEnv)->FactPNConstant2Info, &factPNConstant2Info,
         sizeof(struct entityRecord));

  InstallPrimitive(theEnv, (ENTITY_RECORD_PTR)&FactData(theEnv)->FactInfo,
                   FACT_ADDRESS);
  ...
  InstallPrimitive(theEnv, &FactgenData(theEnv)->FactStoreMFInfo,
                   FACT_STORE_MULTIFIELD);
  InstallPrimitive(theEnv, &FactgenData(theEnv)->FactSlotLengthInfo,
                   FACT_SLOT_LENGTH);
  InstallPrimitive(theEnv, &FactgenData(theEnv)->FactPNConstant1Info,
                   FACT_PN_CONSTANT1);
  InstallPrimitive(theEnv, &FactgenData(theEnv)->FactPNConstant2Info,
                   FACT_PN_CONSTANT2);
#endif
}
...
globle struct expr* FactGenPNConstant(void* theEnv,
  ...
  if (theField->withinMultifieldSlot == FALSE) {
    ...
    top = GenConstant(
        theEnv, FACT_PN_CONSTANT1,
        EnvAddBitMap(theEnv, &hack1, sizeof(struct factConstantPN1Call)));
  ...
  /*=================================================================*/
    ...
    top = GenConstant(
        theEnv, FACT_PN_CONSTANT2,
        EnvAddBitMap(theEnv, &hack2, sizeof(struct factConstantPN2Call)));
  ...
  /*===============================================================*/
    ...
    top->argList->nextArg =
        GenConstant(theEnv, theField->type, theField->value);
  }
...
/**************************************************/
globle struct expr* FactGenGetvar(void* theEnv, struct lhsParseNode* theNode,
                                  int side) {
  /*====================================================*/
  /* Generate call to retrieve single field slot value. */
  /*====================================================*/
  ...
  if ((theNode->slotNumber > 0) && (theNode->withinMultifieldSlot == FALSE)) {
    return (GenConstant(theEnv, FACT_JN_VAR2,
                        FactGetVarJN2(theEnv, theNode, side)));
  }
  ...
  if (((theNode->type == SF_WILDCARD) || (theNode->type == SF_VARIABLE)) &&
      ((theNode->multiFieldsBefore == 0) ||
       ((theNode->multiFieldsBefore == 1) &&
        (theNode->multiFieldsAfter == 0)))) {
    return (GenConstant(theEnv, FACT_JN_VAR3,
                        FactGetVarJN3(theEnv, theNode, side)));
  }
  ...
  return (
      GenConstant(theEnv, FACT_JN_VAR1, FactGetVarJN1(theEnv, theNode, side)));
}
...
globle struct expr* FactGenCheckLength(void* theEnv,
  ...
  if ((theNode->singleFieldsAfter == 0) && (theNode->type != SF_VARIABLE) &&
  ...
  if ((theNode->type != MF_VARIABLE) && (theNode->type != MF_WILDCARD) &&
      (theNode->multiFieldsAfter == 0)) {
    hack.exactly = 1;
  } else {
    hack.exactly = 0;
  ...
  return (GenConstant(
      ...
      EnvAddBitMap(theEnv, &hack, sizeof(struct factCheckLengthPNCall))));
}
...
/*********************************************************************/
globle void FactReplaceGetvar(void* theEnv, struct expr* theItem,
...
/*   network variable access functions for facts.                      */
/***********************************************************************/
globle void FactReplaceGetfield(void* theEnv, struct expr* theItem,
                                struct lhsParseNode* theNode) {
  /*====================================================*/
  /* Generate call to retrieve single field slot value. */
  /*====================================================*/
...
/*   rule.                                                   */
/*************************************************************/
static void* FactGetVarJN1(void* theEnv, struct lhsParseNode* theNode,
                           int side) {
...
/*   rule.                                                    */
/**************************************************************/
static void* FactGetVarJN2(void* theEnv, struct lhsParseNode* theNode,
                           int side) {
...
/*   rule.                                                   */
/*************************************************************/
static void* FactGetVarJN3(void* theEnv, struct lhsParseNode* theNode,
                           int side) {
...
/*   network.                                                 */
/**************************************************************/
static void* FactGetVarPN1(void* theEnv, struct lhsParseNode* theNode) {
...
/***************************************************************/
static void* FactGetVarPN2(void* theEnv, struct lhsParseNode* theNode) {
...
/*************************************************************/
static void* FactGetVarPN3(void* theEnv, struct lhsParseNode* theNode) {
...
globle struct expr* FactPNVariableComparison(
  ...
  if ((selfNode->withinMultifieldSlot == FALSE) && (selfNode->slotNumber > 0) &&
    ...
    top = GenConstant(
        theEnv, FACT_PN_CMP1,
        EnvAddBitMap(theEnv, &hack, sizeof(struct factCompVarsPN1Call)));
  }
...
globle struct expr* FactJNVariableComparison(void* theEnv,
  ...
  if ((selfNode->withinMultifieldSlot == FALSE) && (selfNode->slotNumber > 0) &&
    ...
    if (referringNode->index < 0)
      hack1.slot2 = 0;
    else
      hack1.slot2 = (unsigned short)(referringNode->slotNumber - 1);
    ...
    top = GenConstant(
        theEnv, FACT_JN_CMP1,
        EnvAddBitMap(theEnv, &hack1, sizeof(struct factCompVarsJN1Call)));
  }
  ...
  /*===============================================================*/
  ...
  else if ((selfNode->slotNumber > 0) && (selfNode->type == SF_VARIABLE) &&
    ...
    top = GenConstant(
        theEnv, FACT_JN_CMP2,
        EnvAddBitMap(theEnv, &hack2, sizeof(struct factCompVarsJN2Call)));
  }
