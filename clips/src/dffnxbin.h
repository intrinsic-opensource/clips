...
#define DeffunctionBinaryData(theEnv) \
  ((struct deffunctionBinaryData*)GetEnvironmentData(theEnv, DFFNXBIN_DATA))

#define DeffunctionPointer(i) \
  (((i) == -1L)               \
       ? NULL                 \
       : (DEFFUNCTION*)&DeffunctionBinaryData(theEnv)->DeffunctionArray[i])
