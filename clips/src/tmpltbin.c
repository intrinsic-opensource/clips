...
globle void DeftemplateBinarySetup(void* theEnv) {
  AllocateEnvironmentData(theEnv, TMPLTBIN_DATA,
                          sizeof(struct deftemplateBinaryData),
                          DeallocateDeftemplateBloadData);
#if BLOAD_AND_BSAVE
  AddBinaryItem(theEnv, "deftemplate", 0, BsaveFind, NULL, BsaveStorage,
                BsaveBinaryItem, BloadStorage, BloadBinaryItem, ClearBload);
#endif
#if (BLOAD || BLOAD_ONLY)
  AddBinaryItem(theEnv, "deftemplate", 0, NULL, NULL, NULL, NULL, BloadStorage,
...
/***********************************************************/
static void DeallocateDeftemplateBloadData(void* theEnv) {
  size_t space;

  space = DeftemplateBinaryData(theEnv)->NumberOfTemplateModules *
          sizeof(struct deftemplateModule);
  if (space != 0)
    genfree(theEnv, (void*)DeftemplateBinaryData(theEnv)->ModuleArray, space);

  space = DeftemplateBinaryData(theEnv)->NumberOfDeftemplates *
          sizeof(struct deftemplate);
  if (space != 0)
    genfree(theEnv, (void*)DeftemplateBinaryData(theEnv)->DeftemplateArray,
            space);

  space = DeftemplateBinaryData(theEnv)->NumberOfTemplateSlots *
          sizeof(struct templateSlot);
  if (space != 0)
    genfree(theEnv, (void*)DeftemplateBinaryData(theEnv)->SlotArray, space);
}
...
static void BsaveFind(void* theEnv) {
  ...
  for (theModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    ...
    for (theDeftemplate =
             (struct deftemplate*)EnvGetNextDeftemplate(theEnv, NULL);
         theDeftemplate != NULL;
         theDeftemplate = (struct deftemplate*)EnvGetNextDeftemplate(
             theEnv, theDeftemplate)) {
      /*======================================================*/
      /* Initialize the construct header for the binary save. */
      /*======================================================*/

      MarkConstructHeaderNeededItems(
      ...
      for (theSlot = theDeftemplate->slotList; theSlot != NULL;
...
static void BsaveStorage(void* theEnv, FILE* fp) {
  ...
  GenWrite(&DeftemplateBinaryData(theEnv)->NumberOfDeftemplates,
           sizeof(long int), fp);
  GenWrite(&DeftemplateBinaryData(theEnv)->NumberOfTemplateSlots,
           sizeof(long int), fp);
  GenWrite(&DeftemplateBinaryData(theEnv)->NumberOfTemplateModules,
           sizeof(long int), fp);
}
...
static void BsaveBinaryItem(void* theEnv, FILE* fp) {
  ...
  space = (DeftemplateBinaryData(theEnv)->NumberOfDeftemplates *
          ...
          (DeftemplateBinaryData(theEnv)->NumberOfTemplateModules *
           sizeof(struct bsaveDeftemplateModule));
  GenWrite(&space, sizeof(size_t), fp);
  ...
  for (theModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    ...
    theModuleItem = (struct deftemplateModule*)GetModuleItem(
        theEnv, NULL, FindModuleItem(theEnv, "deftemplate")->moduleIndex);
  ...
  for (theModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    ...
    for (theDeftemplate =
      ...
      tempDeftemplate.patternNetwork =
          BsaveFactPatternIndex(theDeftemplate->patternNetwork);

      if (theDeftemplate->slotList != NULL) {
        tempDeftemplate.slotList =
            DeftemplateBinaryData(theEnv)->NumberOfTemplateSlots;
      } else
        tempDeftemplate.slotList = -1L;
    ...
    for (theDeftemplate =
      ...
      for (theSlot = theDeftemplate->slotList; theSlot != NULL;
        ...
        tempTemplateSlot.defaultList =
            HashedExpressionIndex(theEnv, theSlot->defaultList);
        tempTemplateSlot.facetList =
            HashedExpressionIndex(theEnv, theSlot->facetList);

        if (theSlot->next != NULL)
          tempTemplateSlot.next = 0L;
        else
          tempTemplateSlot.next = -1L;
...
static void BloadStorage(void* theEnv) {
  ...
  GenReadBinary(theEnv, &DeftemplateBinaryData(theEnv)->NumberOfDeftemplates,
                sizeof(long int));
  GenReadBinary(theEnv, &DeftemplateBinaryData(theEnv)->NumberOfTemplateSlots,
                sizeof(long int));
  GenReadBinary(theEnv, &DeftemplateBinaryData(theEnv)->NumberOfTemplateModules,
                sizeof(long int));
  ...
  DeftemplateBinaryData(theEnv)->ModuleArray =
      (struct deftemplateModule*)genalloc(theEnv, space);
  ...
  DeftemplateBinaryData(theEnv)->DeftemplateArray =
      (struct deftemplate*)genalloc(theEnv, space);
  ...
  DeftemplateBinaryData(theEnv)->SlotArray =
      (struct templateSlot*)genalloc(theEnv, space);
}
...
static void BloadBinaryItem(void* theEnv) {
  ...
  BloadandRefresh(
      theEnv, DeftemplateBinaryData(theEnv)->NumberOfTemplateModules,
      sizeof(struct bsaveDeftemplateModule), UpdateDeftemplateModule);
  ...
  BloadandRefresh(theEnv, DeftemplateBinaryData(theEnv)->NumberOfDeftemplates,
  ...
  BloadandRefresh(theEnv, DeftemplateBinaryData(theEnv)->NumberOfTemplateSlots,
                  sizeof(struct bsaveTemplateSlot), UpdateDeftemplateSlot);
}
...
/**************************************************/
static void UpdateDeftemplateModule(void* theEnv, void* buf, long obji) {
  ...
  UpdateDefmoduleItemHeader(
      ...
      &DeftemplateBinaryData(theEnv)->ModuleArray[obji].header,
...
/********************************************/
static void UpdateDeftemplate(void* theEnv, void* buf, long obji) {
  struct deftemplate* theDeftemplate;
  struct bsaveDeftemplate* bdtPtr;

  bdtPtr = (struct bsaveDeftemplate*)buf;
  theDeftemplate = (struct deftemplate*)&DeftemplateBinaryData(theEnv)
                       ->DeftemplateArray[obji];

  UpdateConstructHeader(theEnv, &bdtPtr->header, &theDeftemplate->header,
                        (int)sizeof(struct deftemplateModule),
                        (void*)DeftemplateBinaryData(theEnv)->ModuleArray,
                        (int)sizeof(struct deftemplate),
                        (void*)DeftemplateBinaryData(theEnv)->DeftemplateArray);
  ...
  if (bdtPtr->patternNetwork != -1L) {
    theDeftemplate->patternNetwork =
        (struct factPatternNode*)BloadFactPatternPointer(
            bdtPtr->patternNetwork);
  } else {
...
/************************************************/
static void UpdateDeftemplateSlot(void* theEnv, void* buf, long obji) {
  struct templateSlot* theSlot;
  ...
  if (btsPtr->next != -1L) {
    theSlot->next = (struct templateSlot*)&DeftemplateBinaryData(theEnv)
                        ->SlotArray[obji + 1];
  } else {
    theSlot->next = NULL;
  }
...
static void ClearBload(void* theEnv) {
  ...
  for (i = 0; i < DeftemplateBinaryData(theEnv)->NumberOfDeftemplates; i++) {
    UnmarkConstructHeader(
        theEnv, &DeftemplateBinaryData(theEnv)->DeftemplateArray[i].header);
  }
  ...
  for (i = 0; i < DeftemplateBinaryData(theEnv)->NumberOfTemplateSlots; i++) {
    DecrementSymbolCount(theEnv,
                         DeftemplateBinaryData(theEnv)->SlotArray[i].slotName);
  }
  ...
  CreateImpliedDeftemplate(
      theEnv, (SYMBOL_HN*)EnvAddSymbol(theEnv, "initial-fact"), FALSE);
#endif
}
