...
globle void InitializeConstructData(void* theEnv) {
  AllocateEnvironmentData(theEnv, CONSTRUCT_DATA, sizeof(struct constructData),
...
/*    data for constructs.                          */
/****************************************************/
static void DeallocateConstructData(void* theEnv) {
  ...
  if (ConstructData(theEnv)->ErrorString != NULL) {
    genfree(theEnv, ConstructData(theEnv)->ErrorString,
            sizeof(ConstructData(theEnv)->ErrorString) + 1);
  }

  if (ConstructData(theEnv)->WarningString != NULL) {
    genfree(theEnv, ConstructData(theEnv)->WarningString,
...
/**************************************************/
globle void (*EnvSetParserErrorCallback(
    void* theEnv, void (*functionPtr)(void*, const char*, const char*,
                                      const char*, long)))(void*, const char*,
                                                           const char*,
                                                           const char*, long) {
...
/*************************************************/
globle struct construct* FindConstruct(void* theEnv, const char* name) {
  struct construct* currentPtr;

  for (currentPtr = ConstructData(theEnv)->ListOfConstructs; currentPtr != NULL;
...
/*   FALSE.                                                */
/***********************************************************/
globle int RemoveConstruct(void* theEnv, const char* name) {
...
/************************************************/
globle int EnvSave(void* theEnv, const char* fileName) {
  ...
  while (unvisited) {
    ...
    for (defmodulePtr = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
         defmodulePtr != NULL;
         defmodulePtr =
             (struct defmodule*)EnvGetNextDefmodule(theEnv, defmodulePtr)) {
      /*=================================================================*/
      ...
      if (defmodulePtr->visitedFlag) { /* Module has already been saved. */
        ...
        for (saveFunction = ConstructData(theEnv)->ListOfSaveFunctions;
          ...
          ((*(void (*)(void*, void*, char*))saveFunction->func))(
              theEnv, defmodulePtr, (char*)filePtr);
        }
...
/*******************************************************/
globle intBool RemoveSaveFunction(void* theEnv, const char* name) {
  int found;

  ConstructData(theEnv)->ListOfSaveFunctions = RemoveFunctionFromCallList(
      theEnv, name, ConstructData(theEnv)->ListOfSaveFunctions, &found);
...
#if (!RUN_TIME)
  EnvDefineFunction2(theEnv, "clear", 'v', PTIEF ClearCommand, "ClearCommand",
                     "00");
  EnvDefineFunction2(theEnv, "reset", 'v', PTIEF ResetCommand, "ResetCommand",
                     "00");

#if DEBUGGING_FUNCTIONS && (!BLOAD_ONLY)
  AddWatchItem(theEnv, "compilations", 0,
               &ConstructData(theEnv)->WatchCompilations, 30, NULL, NULL);
#endif
#else
#if MAC_XCD
...
/****************************************/
globle intBool EnvAddResetFunction(void* theEnv, const char* name,
                                   void (*functionPtr)(void*), int priority) {
...
/**********************************************/
globle intBool EnvRemoveResetFunction(void* theEnv, const char* name) {
  int found;

  ConstructData(theEnv)->ListOfResetFunctions = RemoveFunctionFromCallList(
      theEnv, name, ConstructData(theEnv)->ListOfResetFunctions, &found);
...
/*******************************************/
globle void EnvIncrementClearReadyLocks(void* theEnv) {
...
/*******************************************/
globle void EnvDecrementClearReadyLocks(void* theEnv) {
...
globle void EnvClear(void* theEnv) {
  ...
  if ((ConstructData(theEnv)->ClearReadyLocks > 0) ||
    ...
    EnvPrintRouter(
        theEnv, WERROR,
        "Some constructs are still in use. Clear cannot continue.\n");
#if DEBUGGING_FUNCTIONS
...
/******************************************/
globle intBool AddClearReadyFunction(void* theEnv, const char* name,
                                     int (*functionPtr)(void*), int priority) {
  ConstructData(theEnv)->ListOfClearReadyFunctions = AddFunctionToCallList(
      theEnv, name, priority, (void (*)(void*))functionPtr,
...
/************************************************/
globle intBool RemoveClearReadyFunction(void* theEnv, const char* name) {
  int found;

  ConstructData(theEnv)->ListOfClearReadyFunctions = RemoveFunctionFromCallList(
      theEnv, name, ConstructData(theEnv)->ListOfClearReadyFunctions, &found);
...
/****************************************/
globle intBool EnvAddClearFunction(void* theEnv, const char* name,
                                   void (*functionPtr)(void*), int priority) {
  ConstructData(theEnv)->ListOfClearFunctions = AddFunctionToCallList(
      theEnv, name, priority, (void (*)(void*))functionPtr,
...
/**********************************************/
globle intBool EnvRemoveClearFunction(void* theEnv, const char* name) {
  int found;

  ConstructData(theEnv)->ListOfClearFunctions = RemoveFunctionFromCallList(
      theEnv, name, ConstructData(theEnv)->ListOfClearFunctions, &found);
...
globle struct construct* AddConstruct(
    void* theEnv, const char* name, const char* pluralName,
    ...
    void (*setNextItemFunction)(struct constructHeader*,
                                struct constructHeader*),
    intBool (*isConstructDeletableFunction)(void*, void*),
    int (*deleteFunction)(void*, void*), void (*freeFunction)(void*, void*)) {
...
/************************************/
globle intBool AddSaveFunction(void* theEnv, const char* name,
                               void (*functionPtr)(void*, void*, const char*),
                               int priority) {
#if (!RUN_TIME) && (!BLOAD_ONLY)
  ConstructData(theEnv)->ListOfSaveFunctions = AddFunctionToCallList(
      theEnv, name, priority, (void (*)(void*))functionPtr,
...
globle intBool AddClearFunction(const char* name, void (*functionPtr)(void),
                                int priority) {
  void* theEnv;
...
globle intBool AddResetFunction(const char* name, void (*functionPtr)(void),
                                int priority) {
  void* theEnv;

  theEnv = GetCurrentEnvironment();

  ConstructData(theEnv)->ListOfResetFunctions = AddFunctionToCallList(
      theEnv, name, priority, (void (*)(void*))functionPtr,
      ConstructData(theEnv)->ListOfResetFunctions, FALSE);
  return (TRUE);
}

globle void Clear() { EnvClear(GetCurrentEnvironment()); }

globle intBool RemoveClearFunction(const char* name) {
  return EnvRemoveClearFunction(GetCurrentEnvironment(), name);
}

globle intBool RemoveResetFunction(const char* name) {
  return EnvRemoveResetFunction(GetCurrentEnvironment(), name);
}

globle void Reset() { EnvReset(GetCurrentEnvironment()); }

#if (!RUN_TIME) && (!BLOAD_ONLY)

globle int Save(const char* fileName) {
