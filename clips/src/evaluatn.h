...
#define EnvSetpDOBegin(theEnv, target, val) \
  ((target)->begin = (long)((val) - 1))
#define EnvSetpDOEnd(theEnv, target, val) ((target)->end = (long)((val) - 1))

#define DOPToString(target) \
  (((struct symbolHashNode*)((target)->value))->contents)
#define DOPToDouble(target) \
  (((struct floatHashNode*)((target)->value))->contents)
#define DOPToFloat(target) \
  ((float)(((struct floatHashNode*)((target)->value))->contents))
#define DOPToLong(target) \
  (((struct integerHashNode*)((target)->value))->contents)
#define DOPToInteger(target) \
  ((int)(((struct integerHashNode*)((target)->value))->contents))
#define DOPToPointer(target) ((target)->value)
#define DOPToExternalAddress(target) \
  (((struct externalAddressHashNode*)((target)->value))->externalAddress)

#define EnvDOPToString(theEnv, target) \
  (((struct symbolHashNode*)((target)->value))->contents)
#define EnvDOPToDouble(theEnv, target) \
  (((struct floatHashNode*)((target)->value))->contents)
#define EnvDOPToFloat(theEnv, target) \
  ((float)(((struct floatHashNode*)((target)->value))->contents))
#define EnvDOPToLong(theEnv, target) \
  (((struct integerHashNode*)((target)->value))->contents)
#define EnvDOPToInteger(theEnv, target) \
  ((int)(((struct integerHashNode*)((target)->value))->contents))
#define EnvDOPToPointer(theEnv, target) ((target)->value)
#define EnvDOPToExternalAddress(target) \
  (((struct externalAddressHashNode*)((target)->value))->externalAddress)

#define DOToString(target) \
  (((struct symbolHashNode*)((target).value))->contents)
#define DOToDouble(target) (((struct floatHashNode*)((target).value))->contents)
#define DOToFloat(target) \
  ((float)(((struct floatHashNode*)((target).value))->contents))
#define DOToLong(target) (((struct integerHashNode*)((target).value))->contents)
#define DOToInteger(target) \
  ((int)(((struct integerHashNode*)((target).value))->contents))
#define DOToPointer(target) ((target).value)
#define DOToExternalAddress(target) \
  (((struct externalAddressHashNode*)((target).value))->externalAddress)

#define EnvDOToString(theEnv, target) \
  (((struct symbolHashNode*)((target).value))->contents)
#define EnvDOToDouble(theEnv, target) \
  (((struct floatHashNode*)((target).value))->contents)
#define EnvDOToFloat(theEnv, target) \
  ((float)(((struct floatHashNode*)((target).value))->contents))
#define EnvDOToLong(theEnv, target) \
  (((struct integerHashNode*)((target).value))->contents)
#define EnvDOToInteger(theEnv, target) \
  ((int)(((struct integerHashNode*)((target).value))->contents))
#define EnvDOToPointer(theEnv, target) ((target).value)
#define EnvDOToExternalAddress(target) \
  (((struct externalAddressHashNode*)((target).value))->externalAddress)

#define CoerceToLongInteger(t, v) \
  ((t == INTEGER) ? ValueToLong(v) : (long int)ValueToDouble(v))
#define CoerceToInteger(t, v) \
  ((t == INTEGER) ? (int)ValueToLong(v) : (int)ValueToDouble(v))
#define CoerceToDouble(t, v) \
...
#define TestBitMap(map, id) \
  BitwiseTest(map[(id) / BITS_PER_BYTE], (id) % BITS_PER_BYTE)
#define SetBitMap(map, id) \
  BitwiseSet(map[(id) / BITS_PER_BYTE], (id) % BITS_PER_BYTE)
#define ClearBitMap(map, id) \
  BitwiseClear(map[(id) / BITS_PER_BYTE], (id) % BITS_PER_BYTE)
...
#define EvaluationData(theEnv) \
  ((struct evaluationData*)GetEnvironmentData(theEnv, EVALUATION_DATA))
