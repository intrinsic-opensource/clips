...
static void ObjectPatternMatch(void*, int, OBJECT_PATTERN_NODE*,
                               struct multifieldMarker*);
static void ProcessPatternNode(void*, int, OBJECT_PATTERN_NODE*,
                               struct multifieldMarker*);
static void CreateObjectAlphaMatch(void*, OBJECT_ALPHA_NODE*);
static intBool EvaluateObjectPatternTest(void*, int, struct multifieldMarker*,
...
/************************************************************************
  ...
  NOTES        : None
 ************************************************************************/
globle void ObjectNetworkAction(void* theEnv, int type, INSTANCE_TYPE* ins,
                                int slotNameID) {
  SLOT_BITMAP* tmpMap;

  if (EngineData(theEnv)->JoinOperationInProgress) return;
...
globle void ResetObjectMatchTimeTags(void* theEnv) {
  ...
  while (alphaPtr != NULL) {
    ...
    while (lastLevel != NULL) {
      if (lastLevel->matchTimeTag == 0L) break;
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void QueueObjectMatchAction(void* theEnv, int type, INSTANCE_TYPE* ins,
                                   int slotNameID) {
  ...
  newMatch->slotNameIDs = (type != OBJECT_MODIFY)
                              ? NULL
...
/****************************************************
  ...
  NOTES        : If the bitmap must be (re)allocated,
                 ...
                 to allow for growth.
 ****************************************************/
static SLOT_BITMAP* QueueModifySlotMap(void* theEnv, SLOT_BITMAP* oldMap,
                                       int slotNameID) {
  SLOT_BITMAP* newMap;
  ...
  if ((oldMap == NULL) ? TRUE : (slotNameID > oldMap->maxid)) {
    ...
    newsz = sizeof(SLOT_BITMAP) + (sizeof(char) * (newmaxid / BITS_PER_BYTE));
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void ReturnObjectMatchAction(void* theEnv, OBJECT_MATCH_ACTION* omaPtr) {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void ProcessObjectMatchQueue(void* theEnv) {
...
/******************************************************
  ...
  NOTES        : Incremental reset status is also
                 checked here
 ******************************************************/
static void MarkObjectPatternNetwork(void* theEnv, SLOT_BITMAP* slotNameIDs) {
  OBJECT_ALPHA_NODE* alphaPtr;
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static intBool CompareSlotBitMaps(SLOT_BITMAP* smap1, SLOT_BITMAP* smap2) {
  unsigned short i, maxByte;

  maxByte = (unsigned short)(((smap1->maxid < smap2->maxid) ? smap1->maxid
                                                            : smap2->maxid) /
                             BITS_PER_BYTE);
  for (i = 0; i <= maxByte; i++)
    if (smap1->map[i] & smap2->map[i]) return (TRUE);
  return (FALSE);
}
 ...
 **********************************************************************************/
static void ObjectPatternMatch(void* theEnv, int offset,
...
/**********************************************************************************
  ...
  NOTES        : Several globals are used to keep track of the current
                 ...
                 CurrentObjectSlotLength - the cardinality of the slot value
 **********************************************************************************/
static void ProcessPatternNode(void* theEnv, int offset,
  ...
  if (ObjectReteData(theEnv)->CurrentPatternObjectSlot == NULL) {
    ...
    {
      if (EvaluateObjectPatternTest(theEnv, objectSlotField, NULL,
                                    patternNode->networkTest->nextArg,
                                    patternNode)) {
        EvaluateExpression(theEnv, patternNode->networkTest, &theResult);

        tempPtr = (OBJECT_PATTERN_NODE*)FindHashedPatternNode(
            theEnv, patternNode, theResult.type, theResult.value);
    ...
    } else if ((patternNode->networkTest == NULL)
                   ? TRUE
                   : (EvaluateObjectPatternTest(
  ...
  newMark->where.whichSlot =
      (void*)ObjectReteData(theEnv)
          ->CurrentPatternObjectSlot->desc->slotName->name;
  ...
  if (patternNode->endSlot == FALSE) {
    ...
    while (repeatCount > 0) {
      if (patternNode->selector) {
        if (EvaluateObjectPatternTest(theEnv, objectSlotField, newMark,
                                      patternNode->networkTest->nextArg,
                                      patternNode)) {
          EvaluateExpression(theEnv, patternNode->networkTest, &theResult);
          ...
          if (tempPtr != NULL) {
            ...
            ObjectPatternMatch(
                theEnv,
                (int)(offset + (newMark->endPosition - objectSlotField)),
            ...
            ObjectReteData(theEnv)->CurrentPatternObjectSlot = objectSlot;
                     ...
                     : EvaluateObjectPatternTest(
                           ...
                           (EXPRESSION*)patternNode->networkTest,
                           patternNode)) {
        if (patternNode->alphaNode != NULL)
          CreateObjectAlphaMatch(theEnv, patternNode->alphaNode);
        ObjectPatternMatch(
            theEnv, (int)(offset + (newMark->endPosition - objectSlotField)),
    ...
    newMark->endPosition =
        (long)ObjectReteData(theEnv)->CurrentObjectSlotLength -
        patternNode->leaveFields;  // Bug fix: added leaveFields
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void CreateObjectAlphaMatch(void* theEnv, OBJECT_ALPHA_NODE* alphaPtr) {
  ...
  while (alphaPtr != NULL) {
    if (alphaPtr->matchTimeTag ==
      ...
      theMatch = CreateAlphaMatch(
          theEnv, (void*)ObjectReteData(theEnv)->CurrentPatternObject,
      ...
      newMatch->next = (struct patternMatch*)ObjectReteData(theEnv)
...
static intBool EvaluateObjectPatternTest(
  ...
  if (networkTest->type == OBJ_PN_CONSTANT) {
    ...
    if (rv) {
      if (((struct ObjectCmpPNConstant*)ValueToBitMap(networkTest->value))
              ->pass)
  ...
  if (networkTest->value == ExpressionData(theEnv)->PTR_OR) {
    ...
    while (networkTest != NULL) {
      if (EvaluateObjectPatternTest(theEnv, objectSlotField, selfSlotMarker,
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void ObjectAssertAction(void* theEnv, INSTANCE_TYPE* ins) {
...
/**********************************************************************
  ...
  NOTES        : None
 **********************************************************************/
static void ObjectModifyAction(void* theEnv, INSTANCE_TYPE* ins,
...
/****************************************************
  ...
  NOTES        : None
 ****************************************************/
static void ObjectRetractAction(void* theEnv, INSTANCE_TYPE* ins,
                                SLOT_BITMAP* slotNameIDs) {
    ...
    while (tmpMatch != NULL) {
      ...
      if (alphaPtr->slotbmp != NULL) {
        if (CompareSlotBitMaps(
                slotNameIDs, (SLOT_BITMAP*)ValueToBitMap(alphaPtr->slotbmp))) {
          ins->busy--;
          if (prvMatch == NULL)
...
/*****************************************************
  ...
  NOTES        : None
 *****************************************************/
static void ObjectPatternNetErrorMessage(void* theEnv,
  ...
  EnvPrintRouter(
      theEnv, WERROR,
      ValueToString(ObjectReteData(theEnv)->CurrentPatternObject->name));
  EnvPrintRouter(theEnv, WERROR, "]\n");
  EnvPrintRouter(theEnv, WERROR, "   Problem resides in slot ");
  EnvPrintRouter(theEnv, WERROR,
                 ValueToString(FindIDSlotName(theEnv, patternPtr->slotNameID)));
...
/*********************************************************
  ...
  NOTES        : None
 *********************************************************/
static void TraceErrorToObjectPattern(void* theEnv, int errorNode,
                                      OBJECT_PATTERN_NODE* patternPtr) {
