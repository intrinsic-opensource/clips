...
globle void* CreateMultifield2(void* theEnv, long size) {
  ...
  theSegment = get_var_struct(theEnv, multifield,
                              (long)sizeof(struct field) * (newSize - 1L));
...
globle void ReturnMultifield(void* theEnv, struct multifield* theSegment) {
  ...
  if (theSegment->multifieldLength == 0)
    ...
    newSize = theSegment->multifieldLength;

  rtn_var_struct(theEnv, multifield, sizeof(struct field) * (newSize - 1),
                 theSegment);
}
...
globle struct multifield* StringToMultifield(void* theEnv,
  ...
  while (theToken.type != STOP) {
    if ((theToken.type == SYMBOL) || (theToken.type == STRING) ||
      ...
      theAtom =
          GenConstant(theEnv, STRING, EnvAddSymbol(theEnv, theToken.printForm));
    }
...
globle void* DOToMultifield(void* theEnv, DATA_OBJECT* theValue) {
  ...
  dst = (struct multifield*)CreateMultifield2(
      theEnv, (unsigned long)GetpDOLength(theValue));

  src = (struct multifield*)theValue->value;
  GenCopyMemory(struct field, dst->multifieldLength, &(dst->theFields[0]),
                &(src->theFields[GetpDOBegin(theValue) - 1]));
  ...
  while (theSegment != NULL) {
    ...
    if (theSegment->busyCount == 0) {
      ...
      rtn_var_struct(theEnv, multifield, sizeof(struct field) * (newSize - 1),
                     theSegment);
      if (lastPtr == NULL)
        UtilityData(theEnv)->CurrentGarbageFrame->ListOfMultifields = nextPtr;
      else
...
/************************************************************************/
globle void DuplicateMultifield(void* theEnv, DATA_OBJECT_PTR dst,
                                DATA_OBJECT_PTR src) {
  dst->type = MULTIFIELD;
  dst->begin = 0;
  dst->end = src->end - src->begin;
  dst->value = (void*)CreateMultifield2(theEnv, (unsigned long)dst->end + 1);
  GenCopyMemory(struct field, dst->end + 1,
                &((struct multifield*)dst->value)->theFields[0],
...
globle void* CopyMultifield(void* theEnv, struct multifield* src) {
  ...
  GenCopyMemory(struct field, src->multifieldLength, &(dst->theFields[0]),
                &(src->theFields[0]));
  return ((void*)dst);
}
...
/*********************************************/
globle void PrintMultifield(void* theEnv, const char* fileid,
                            struct multifield* segment, long begin, long end,
                            int printParens) {
  struct field* theMultifield;
  int i;

  theMultifield = segment->theFields;
  if (printParens) EnvPrintRouter(theEnv, fileid, "(");
  i = begin;
  while (i <= end) {
...
globle void StoreInMultifield(void* theEnv, DATA_OBJECT* returnValue,
  ...
  if (argCount == 0) {
    ...
    if (garbageSegment)
      ...
      theMultifield = (struct multifield*)CreateMultifield2(theEnv, 0L);
    ...
    if (garbageSegment) {
      ...
      theMultifield = (struct multifield*)CreateMultifield2(theEnv, seg_size);
...
globle unsigned long HashMultifield(struct multifield* theSegment,
  ...
  for (i = 0; i < length; i++) {
    switch (fieldPtr[i].type) {
      case MULTIFIELD:
        count +=
            HashMultifield((struct multifield*)fieldPtr[i].value, theRange);
        break;
      ...
      case INSTANCE_NAME:
#endif
        tvalue = (unsigned long)HashSymbol(ValueToString(fieldPtr[i].value),
                                           theRange);
...
globle void* ImplodeMultifield(void* theEnv, DATA_OBJECT* value) {
  ...
  for (i = GetpDOBegin(value); i <= GetpDOEnd(value); i++) {
    if (GetMFType(theMultifield, i) == FLOAT) {
      tmp_str =
          FloatToString(theEnv, ValueToDouble(GetMFValue(theMultifield, i)));
      strsize += strlen(tmp_str) + 1;
    } else if (GetMFType(theMultifield, i) == INTEGER) {
      tmp_str = LongIntegerToString(theEnv,
                                    ValueToLong(GetMFValue(theMultifield, i)));
      strsize += strlen(tmp_str) + 1;
      ...
      strsize += strlen(ValueToString(
                     ((INSTANCE_TYPE*)GetMFValue(theMultifield, i))->name)) +
                 3;
    }
#endif
  ...
  for (j = 0, i = GetpDOBegin(value); i <= GetpDOEnd(value); i++) {
    ...
    /*=======================================*/
      ...
      *(ret_str + j++) = ']';
      ...
      tmp_str =
          ValueToString(((INSTANCE_TYPE*)GetMFValue(theMultifield, i))->name);
      *(ret_str + j++) = '[';
      while (*tmp_str) {
        *(ret_str + j) = *tmp_str;
        j++;
...
globle void* CreateMultifield(long size) {
