...
static SLOT_DESC* CheckSlotExists(void*, const char*, DEFCLASS**, intBool,
                                  intBool);
...
static void DisplaySlotBasicInfo(void*, const char*, const char*, const char*,
                                 char*, DEFCLASS*);
static intBool PrintSlotSources(void*, const char*, SYMBOL_HN*,
                                PACKED_CLASS_LINKS*, long, int);
static void DisplaySlotConstraintInfo(void*, const char*, const char*, char*,
                                      unsigned, DEFCLASS*);
...
/****************************************************************
  ...
  NOTES        : None
 ****************************************************************/
globle void EnvBrowseClasses(void* theEnv, const char* logicalName,
                             void* clsptr) {
...
/******************************************************
  ...
  NOTES        : None
 ******************************************************/
globle void EnvDescribeClass(void* theEnv, const char* logicalName,
                             void* clsptr) {
  DEFCLASS* cls;
  char buf[83], slotNamePrintFormat[12], overrideMessagePrintFormat[12];
  int messageBanner;
  long i;
  ...
  if (cls->abstract)
    ...
    if (cls->reactive)
      ...
      EnvPrintRouter(theEnv, logicalName,
                     "Non-reactive: direct instances of this class cannot "
                     "match defrule patterns.\n\n");
#else
    EnvPrintRouter(theEnv, logicalName, "\n");
#endif
  }
  PrintPackedClassLinks(theEnv, logicalName,
                        "Direct Superclasses:", &cls->directSuperclasses);
  PrintPackedClassLinks(theEnv, logicalName,
                        "Inheritance Precedence:", &cls->allSuperclasses);
  PrintPackedClassLinks(theEnv, logicalName,
                        "Direct Subclasses:", &cls->directSubclasses);
  if (cls->instanceTemplate != NULL) {
    ...
    for (i = 0; i < cls->instanceSlotCount; i++) {
      slotNameLength =
          strlen(ValueToString(cls->instanceTemplate[i]->slotName->name));
    ...
    if (maxSlotNameLength > 16) maxSlotNameLength = 16;
    if (maxOverrideMessageLength > 12) maxOverrideMessageLength = 12;
#if WIN_MVC
    gensprintf(slotNamePrintFormat, "%%-%Id.%Ids : ", maxSlotNameLength,
               maxSlotNameLength);
    gensprintf(overrideMessagePrintFormat, "%%-%Id.%Ids ",
               maxOverrideMessageLength, maxOverrideMessageLength);
#elif WIN_GCC
    gensprintf(slotNamePrintFormat, "%%-%ld.%lds : ", (long)maxSlotNameLength,
               (long)maxSlotNameLength);
    gensprintf(overrideMessagePrintFormat, "%%-%ld.%lds ",
               (long)maxOverrideMessageLength, (long)maxOverrideMessageLength);
#else
    gensprintf(slotNamePrintFormat, "%%-%zd.%zds : ", maxSlotNameLength,
               maxSlotNameLength);
    gensprintf(overrideMessagePrintFormat, "%%-%zd.%zds ",
               maxOverrideMessageLength, maxOverrideMessageLength);
#endif

    DisplaySlotBasicInfo(theEnv, logicalName, slotNamePrintFormat,
                         overrideMessagePrintFormat, buf, cls);
    EnvPrintRouter(theEnv, logicalName,
                   "\nConstraint information for slots:\n\n");
    DisplaySlotConstraintInfo(theEnv, logicalName, slotNamePrintFormat, buf, 82,
                              cls);
  }
 ...
 **********************************************************/
globle const char* GetCreateAccessorString(void* vsd) {
  SLOT_DESC* sd = (SLOT_DESC*)vsd;

  if (sd->createReadAccessor && sd->createWriteAccessor) return ("RW");
...
globle void* GetDefclassModuleCommand(void* theEnv) {
  return (GetConstructModuleCommand(theEnv, "defclass-module",
...
globle intBool SuperclassPCommand(void* theEnv) {
  ...
  if (CheckTwoClasses(theEnv, "superclassp", &c1, &c2) == FALSE) return (FALSE);
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle intBool EnvSuperclassP(void* theEnv, void* firstClass,
                              void* secondClass) {
#if MAC_XCD
...
globle intBool SubclassPCommand(void* theEnv) {
  ...
  if (CheckTwoClasses(theEnv, "subclassp", &c1, &c2) == FALSE) return (FALSE);
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle intBool EnvSlotExistP(void* theEnv, void* theDefclass,
                             const char* slotName, intBool inheritFlag) {
  return ((LookupSlot(theEnv, (DEFCLASS*)theDefclass, slotName, inheritFlag) !=
           NULL)
              ? TRUE
              : FALSE);
}
...
globle int MessageHandlerExistPCommand(void* theEnv) {
  ...
  if (EnvArgTypeCheck(theEnv, "message-handler-existp", 2, SYMBOL, &temp) ==
      FALSE)
    return (FALSE);
  mname = (SYMBOL_HN*)GetValue(temp);
  if (EnvRtnArgCount(theEnv) == 3) {
    if (EnvArgTypeCheck(theEnv, "message-handler-existp", 3, SYMBOL, &temp) ==
        FALSE)
      return (FALSE);
  ...
  if (FindHandlerByAddress(cls, mname, mtype) != NULL) return (TRUE);
  return (FALSE);
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle intBool EnvSlotWritableP(void* theEnv, void* theDefclass,
                                const char* slotName) {
  SLOT_DESC* sd;
...
globle intBool EnvSlotPublicP(void* theEnv, void* theDefclass,
  ...
  if ((sd = LookupSlot(theEnv, (DEFCLASS*)theDefclass, slotName, FALSE)) ==
      NULL)
    return (FALSE);
  return (sd->publicVisibility ? TRUE : FALSE);
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle int EnvSlotDefaultP(void* theEnv, void* theDefclass,
                           const char* slotName) {
  SLOT_DESC* sd;
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle intBool EnvSlotDirectAccessP(void* theEnv, void* theDefclass,
                                    const char* slotName) {
  SLOT_DESC* sd;
...
globle void SlotDefaultValueCommand(void* theEnv, DATA_OBJECT_PTR theValue) {
  ...
  if (sd->dynamicDefault)
    EvaluateAndStoreInDataObject(theEnv, (int)sd->multiple,
                                 (EXPRESSION*)sd->defaultValue, theValue, TRUE);
  else
...
/*********************************************************
  ...
  NOTES        : None
 *********************************************************/
globle intBool EnvSlotDefaultValue(void* theEnv, void* theDefclass,
                                   const char* slotName,
...
globle intBool ClassExistPCommand(void* theEnv) {
  ...
  if (EnvArgTypeCheck(theEnv, "class-existp", 1, SYMBOL, &temp) == FALSE)
    return (FALSE);
  return ((LookupDefclassByMdlOrScope(theEnv, DOToString(temp)) != NULL)
              ? TRUE
              : FALSE);
}
...
static int CheckTwoClasses(void* theEnv, const char* func, DEFCLASS** c1,
  ...
  *c1 = LookupDefclassByMdlOrScope(theEnv, DOToString(temp));
  ...
  if (EnvArgTypeCheck(theEnv, func, 2, SYMBOL, &temp) == FALSE) return (FALSE);
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static SLOT_DESC* CheckSlotExists(void* theEnv, const char* func,
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static SLOT_DESC* LookupSlot(void* theEnv, DEFCLASS* theDefclass,
                             const char* slotName, intBool inheritFlag) {
...
/*****************************************************
  ...
  NOTES        : None
 ******************************************************/
static DEFCLASS* CheckClass(void* theEnv, const char* func, const char* cname) {
  DEFCLASS* cls;

  cls = LookupDefclassByMdlOrScope(theEnv, cname);
  if (cls == NULL) ClassExistError(theEnv, func, cname);
  return (cls);
}
...
/****************************************************************
  ...
  NOTES        : None
 ****************************************************************/
static void PrintClassBrowse(void* theEnv, const char* logicalName,
                             DEFCLASS* cls, long depth) {
  long i;

  for (i = 0; i < depth; i++) EnvPrintRouter(theEnv, logicalName, "  ");
  EnvPrintRouter(theEnv, logicalName, EnvGetDefclassName(theEnv, (void*)cls));
  if (cls->directSuperclasses.classCount > 1)
    EnvPrintRouter(theEnv, logicalName, " *");
  EnvPrintRouter(theEnv, logicalName, "\n");
  for (i = 0; i < cls->directSubclasses.classCount; i++)
    PrintClassBrowse(theEnv, logicalName, cls->directSubclasses.classArray[i],
                     depth + 1);
}
...
/*********************************************************
  ...
  NOTES        : None
 *********************************************************/
static void DisplaySeparator(void* theEnv, const char* logicalName, char* buf,
                             int maxlen, int sepchar) {
  int i;

  for (i = 0; i < maxlen - 2; i++) buf[i] = (char)sepchar;
  buf[i++] = '\n';
  buf[i] = '\0';
...
/*************************************************************
  ...
  NOTES        : None
 *************************************************************/
static void DisplaySlotBasicInfo(void* theEnv, const char* logicalName,
  ...
  for (i = 0; i < cls->instanceSlotCount; i++) {
    ...
    if (createString[1] == '\0') genstrcat(buf, " ");
    ...
    EnvPrintRouter(theEnv, logicalName, buf);
    PrintSlotSources(theEnv, logicalName, sp->slotName->name,
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static intBool PrintSlotSources(void* theEnv, const char* logicalName,
                                SYMBOL_HN* sname, PACKED_CLASS_LINKS* sprec,
                                long theIndex, int inhp) {
  SLOT_DESC* csp;

  if (theIndex == sprec->classCount) return (FALSE);
  csp = FindClassSlot(sprec->classArray[theIndex], sname);
  if ((csp != NULL) ? ((csp->noInherit == 0) || inhp) : FALSE) {
    if (csp->composite) {
      if (PrintSlotSources(theEnv, logicalName, sname, sprec, theIndex + 1,
                           FALSE))
        EnvPrintRouter(theEnv, logicalName, " ");
    }
    ...
    return (PrintSlotSources(theEnv, logicalName, sname, sprec, theIndex + 1,
                             FALSE));
}
...
/*********************************************************
  ...
  NOTES        : None
 *********************************************************/
static void DisplaySlotConstraintInfo(void* theEnv, const char* logicalName,
                                      const char* slotNamePrintFormat,
  ...
  for (i = 0; i < cls->instanceSlotCount; i++) {
    ...
    gensprintf(buf, slotNamePrintFormat,
               ValueToString(cls->instanceTemplate[i]->slotName->name));
    if (cr != NULL) {
      ...
      genstrcat(buf,
                ConstraintCode(cr, (unsigned)cr->externalAddressesAllowed, 0));
      ...
      OpenStringDestination(theEnv, strdest, buf + strlen(buf),
                            (maxlen - strlen(buf) - 1));
 ...
 ******************************************************/
static const char* ConstraintCode(CONSTRAINT_RECORD* cr, unsigned allow,
...
globle void BrowseClasses(const char* logicalName, void* clsptr) {
  EnvBrowseClasses(GetCurrentEnvironment(), logicalName, clsptr);
}

globle void DescribeClass(const char* logicalName, void* clsptr) {
  EnvDescribeClass(GetCurrentEnvironment(), logicalName, clsptr);
}

#endif

globle intBool SlotDirectAccessP(void* theDefclass, const char* slotName) {
  return EnvSlotDirectAccessP(GetCurrentEnvironment(), theDefclass, slotName);
}

globle intBool SlotExistP(void* theDefclass, const char* slotName,
                          intBool inheritFlag) {
  return EnvSlotExistP(GetCurrentEnvironment(), theDefclass, slotName,
                       inheritFlag);
}

globle intBool SlotInitableP(void* theDefclass, const char* slotName) {
  return EnvSlotInitableP(GetCurrentEnvironment(), theDefclass, slotName);
}

globle intBool SlotPublicP(void* theDefclass, const char* slotName) {
  return EnvSlotPublicP(GetCurrentEnvironment(), theDefclass, slotName);
}

globle int SlotDefaultP(void* theDefclass, const char* slotName) {
  return EnvSlotDefaultP(GetCurrentEnvironment(), theDefclass, slotName);
}

globle intBool SlotWritableP(void* theDefclass, const char* slotName) {
  return EnvSlotWritableP(GetCurrentEnvironment(), theDefclass, slotName);
}

globle intBool SubclassP(void* firstClass, void* secondClass) {
  return EnvSubclassP(GetCurrentEnvironment(), firstClass, secondClass);
}

globle intBool SuperclassP(void* firstClass, void* secondClass) {
  return EnvSuperclassP(GetCurrentEnvironment(), firstClass, secondClass);
}

globle intBool SlotDefaultValue(void* theDefclass, const char* slotName,
                                DATA_OBJECT_PTR theValue) {
  return EnvSlotDefaultValue(GetCurrentEnvironment(), theDefclass, slotName,
                             theValue);
}
