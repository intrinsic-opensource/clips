...
static intBool OutputProfileInfo(void*, const char*,
                                 struct constructProfileInfo*, const char*,
...
globle void ConstructProfilingFunctionDefinitions(void* theEnv) {
  struct userDataRecord profileDataInfo = {0, CreateProfileData,
                                           DeleteProfileData};

  AllocateEnvironmentData(theEnv, PROFLFUN_DATA,
                          sizeof(struct profileFunctionData), NULL);

  memcpy(&ProfileFunctionData(theEnv)->ProfileDataInfo, &profileDataInfo,
         sizeof(struct userDataRecord));
...
#if !RUN_TIME
  EnvDefineFunction2(theEnv, "profile", 'v', PTIEF ProfileCommand,
                     "ProfileCommand", "11w");
  EnvDefineFunction2(theEnv, "profile-info", 'v', PTIEF ProfileInfoCommand,
                     "ProfileInfoCommand", "01w");
  EnvDefineFunction2(theEnv, "profile-reset", 'v', PTIEF ProfileResetCommand,
                     "ProfileResetCommand", "00");
  ...
  ProfileFunctionData(theEnv)->ProfileDataID = InstallUserDataRecord(
      theEnv, &ProfileFunctionData(theEnv)->ProfileDataInfo);
...
globle void* CreateProfileData(void* theEnv) {
  ...
  theInfo = (struct constructProfileInfo*)genalloc(
      theEnv, sizeof(struct constructProfileInfo));
...
globle void ProfileCommand(void* theEnv) {
  ...
  if (!Profile(theEnv, argument)) {
    ExpectedTypeError1(theEnv, "profile", 1,
...
/*   for the profile command. */
/******************************/
globle intBool Profile(void* theEnv, const char* argument) {
  /*======================================================*/
  ...
  /*======================================================*/
    ...
    ProfileFunctionData(theEnv)->ProfileTotalTime +=
        (ProfileFunctionData(theEnv)->ProfileEndTime -
...
globle void ProfileInfoCommand(void* theEnv) {
  ...
  if ((argCount = EnvArgCountCheck(theEnv, "profile", NO_MORE_THAN, 1)) == -1)
    return;
  ...
  if (argCount == 1) {
    if (EnvArgTypeCheck(theEnv, "profile", 1, SYMBOL, &theValue) == FALSE)
      return;
  }
  ...
  if (ProfileFunctionData(theEnv)->ProfileUserFunctions ||
  ...
  if (ProfileFunctionData(theEnv)->LastProfileInfo != NO_PROFILE) {
    ...
    if (ProfileFunctionData(theEnv)->LastProfileInfo == USER_FUNCTIONS) {
      EnvPrintRouter(theEnv, WDISPLAY,
                     "Function Name                            ");
    } else if (ProfileFunctionData(theEnv)->LastProfileInfo ==
               CONSTRUCTS_CODE) {
      EnvPrintRouter(theEnv, WDISPLAY,
                     "Construct Name                           ");
    }
  ...
  if (ProfileFunctionData(theEnv)->LastProfileInfo == USER_FUNCTIONS)
    OutputUserFunctionsInfo(theEnv);
  if (ProfileFunctionData(theEnv)->LastProfileInfo == CONSTRUCTS_CODE)
...
/**********************************************/
globle void StartProfile(void* theEnv, struct profileFrameInfo* theFrame,
                         struct userData** theList, intBool checkFlag) {
  ...
  profileInfo = (struct constructProfileInfo*)FetchUserData(
      theEnv, ProfileFunctionData(theEnv)->ProfileDataID, theList);
  ...
  if (ProfileFunctionData(theEnv)->ActiveProfileFrame != NULL) {
    addTime =
        startTime - ProfileFunctionData(theEnv)->ActiveProfileFrame->startTime;
...
globle void EndProfile(void* theEnv, struct profileFrameInfo* theFrame) {
  ...
  if (theFrame->parentCall) {
    ...
    ProfileFunctionData(theEnv)->ActiveProfileFrame->totalWithChildrenTime +=
        addTime;
    ProfileFunctionData(theEnv)->ActiveProfileFrame->childCall = FALSE;
  }

  ProfileFunctionData(theEnv)->ActiveProfileFrame->totalSelfTime +=
      (endTime - ProfileFunctionData(theEnv)->ActiveProfileFrame->startTime);
...
/******************************************/
static intBool OutputProfileInfo(void* theEnv, const char* itemName,
                                 struct constructProfileInfo* profileInfo,
  ...
  if (ProfileFunctionData(theEnv)->ProfileTotalTime != 0.0) {
    percent = (profileInfo->totalSelfTime * 100.0) /
              ProfileFunctionData(theEnv)->ProfileTotalTime;
    if (percent < 0.005) percent = 0.0;
    percentWithKids = (profileInfo->totalWithChildrenTime * 100.0) /
                      ProfileFunctionData(theEnv)->ProfileTotalTime;
  ...
  gensprintf(buffer, ProfileFunctionData(theEnv)->OutputString, itemName,
             (long)profileInfo->numberOfEntries,

             (double)profileInfo->totalSelfTime, (double)percent,
...
globle void ProfileResetCommand(void* theEnv) {
  ...
  for (theFunction = GetFunctionList(theEnv); theFunction != NULL;
       theFunction = theFunction->next) {
    ResetProfileInfo((struct constructProfileInfo*)TestUserData(
        ProfileFunctionData(theEnv)->ProfileDataID, theFunction->usrData));
  }

  for (i = 0; i < MAXIMUM_PRIMITIVES; i++) {
    if (EvaluationData(theEnv)->PrimitivesArray[i] != NULL) {
      ResetProfileInfo((struct constructProfileInfo*)TestUserData(
          ProfileFunctionData(theEnv)->ProfileDataID,
  ...
  for (theDeffunction = (DEFFUNCTION*)EnvGetNextDeffunction(theEnv, NULL);
       theDeffunction != NULL;
       theDeffunction =
           (DEFFUNCTION*)EnvGetNextDeffunction(theEnv, theDeffunction)) {
    ResetProfileInfo((struct constructProfileInfo*)TestUserData(
        ProfileFunctionData(theEnv)->ProfileDataID,
  ...
  for (theDefgeneric = (DEFGENERIC*)EnvGetNextDefgeneric(theEnv, NULL);
       theDefgeneric != NULL; theDefgeneric = (DEFGENERIC*)EnvGetNextDefgeneric(
                                  theEnv, theDefgeneric)) {
    ResetProfileInfo((struct constructProfileInfo*)TestUserData(
        ProfileFunctionData(theEnv)->ProfileDataID,
        theDefgeneric->header.usrData));

    for (methodIndex = EnvGetNextDefmethod(theEnv, theDefgeneric, 0);
         methodIndex != 0; methodIndex = EnvGetNextDefmethod(
                               theEnv, theDefgeneric, methodIndex)) {
      theMethod = GetDefmethodPointer(theDefgeneric, methodIndex);
      ResetProfileInfo((struct constructProfileInfo*)TestUserData(
          ProfileFunctionData(theEnv)->ProfileDataID, theMethod->usrData));
    }
  }
#endif
  ...
  for (theDefclass = (DEFCLASS*)EnvGetNextDefclass(theEnv, NULL);
    ...
    for (handlerIndex = EnvGetNextDefmessageHandler(theEnv, theDefclass, 0);
         handlerIndex != 0; handlerIndex = EnvGetNextDefmessageHandler(
                                theEnv, theDefclass, handlerIndex)) {
      theHandler = GetDefmessageHandlerPointer(theDefclass, handlerIndex);
      ResetProfileInfo((struct constructProfileInfo*)TestUserData(
          ProfileFunctionData(theEnv)->ProfileDataID, theHandler->usrData));
    }
  }
#endif
}
...
/*************************************************/
static void OutputUserFunctionsInfo(void* theEnv) {
  struct FunctionDefinition* theFunction;
  int i;
  ...
  for (theFunction = GetFunctionList(theEnv); theFunction != NULL;
       theFunction = theFunction->next) {
    OutputProfileInfo(
        theEnv, ValueToString(theFunction->callFunctionName),
        (struct constructProfileInfo*)TestUserData(
            ProfileFunctionData(theEnv)->ProfileDataID, theFunction->usrData),
        NULL, NULL, NULL, NULL);
  }
  ...
  for (i = 0; i < MAXIMUM_PRIMITIVES; i++) {
    if (EvaluationData(theEnv)->PrimitivesArray[i] != NULL) {
      OutputProfileInfo(
          theEnv, EvaluationData(theEnv)->PrimitivesArray[i]->name,
          (struct constructProfileInfo*)TestUserData(
              ProfileFunctionData(theEnv)->ProfileDataID,
              EvaluationData(theEnv)->PrimitivesArray[i]->usrData),
          NULL, NULL, NULL, NULL);
...
/*************************************************/
static void OutputConstructsCodeInfo(void* theEnv) {
...
#pragma unused(theEnv)
  ...
  for (theDefgeneric = (DEFGENERIC*)EnvGetNextDefgeneric(theEnv, NULL);
    ...
    for (methodIndex = EnvGetNextDefmethod(theEnv, theDefgeneric, 0);
      ...
      EnvGetDefmethodDescription(theEnv, methodBuffer, 510, theDefgeneric,
                                 methodIndex);
  ...
  for (theDefclass = (DEFCLASS*)EnvGetNextDefclass(theEnv, NULL);
    ...
    for (handlerIndex = EnvGetNextDefmessageHandler(theEnv, theDefclass, 0);
         handlerIndex != 0; handlerIndex = EnvGetNextDefmessageHandler(
                                theEnv, theDefclass, handlerIndex)) {
      theHandler = GetDefmessageHandlerPointer(theDefclass, handlerIndex);
      if (OutputProfileInfo(
              theEnv,
              EnvGetDefmessageHandlerName(theEnv, theDefclass, handlerIndex),
              (struct constructProfileInfo*)TestUserData(
                  ProfileFunctionData(theEnv)->ProfileDataID,
                  theHandler->usrData),
              prefixBefore, prefix, prefixAfter, &banner)) {
...
globle double SetProfilePercentThresholdCommand(void* theEnv) {
  ...
  if (EnvArgTypeCheck(theEnv, "set-profile-percent-threshold", 1,
                      INTEGER_OR_FLOAT, &theValue) == FALSE) {
    return (ProfileFunctionData(theEnv)->PercentThreshold);
  }
...
/**********************************************************/
globle const char* SetProfileOutputString(void* theEnv, const char* value) {
...
/******************************************************************/
static void ProfileClearFunction(void* theEnv) {
  struct FunctionDefinition* theFunction;
  int i;
  ...
  for (theFunction = GetFunctionList(theEnv); theFunction != NULL;
       theFunction = theFunction->next) {
    theFunction->usrData =
        DeleteUserData(theEnv, ProfileFunctionData(theEnv)->ProfileDataID,
                       theFunction->usrData);
  }
