...
static INSTANCE_TYPE* FindImportedInstance(void*, struct defmodule*,
                                           struct defmodule*, INSTANCE_TYPE*);
...
globle void InitializeInstanceTable(void* theEnv) {
  ...
  InstanceData(theEnv)->InstanceTable = (INSTANCE_TYPE**)gm2(
      theEnv, (int)(sizeof(INSTANCE_TYPE*) * INSTANCE_TABLE_HASH_SIZE));
...
globle void CleanupInstances(void* theEnv) {
  ...
  if (InstanceData(theEnv)->MaintainGarbageInstances) return;
  gprv = NULL;
  gtmp = InstanceData(theEnv)->InstanceGarbageList;
  while (gtmp != NULL) {
#if DEFRULE_CONSTRUCT
    if ((gtmp->ins->busy == 0) && (gtmp->ins->header.busyCount == 0))
#else
    if (gtmp->ins->busy == 0)
#endif
    {
...
globle void DestroyAllInstances(void* theEnv) {
  ...
  while (iptr != NULL) {
    EnvSetCurrentModule(theEnv,
                        (void*)iptr->cls->header.whichModule->theModule);
    DirectMessage(theEnv, MessageHandlerData(theEnv)->DELETE_SYMBOL, iptr, NULL,
                  NULL);
    iptr = iptr->nxtList;
    while ((iptr != NULL) ? iptr->garbage : FALSE) iptr = iptr->nxtList;
  }
...
globle void RemoveInstanceData(void* theEnv, INSTANCE_TYPE* ins) {
  ...
  for (i = 0; i < ins->cls->instanceSlotCount; i++) {
    ...
    if ((sp == &sp->desc->sharedValue) ? (--sp->desc->sharedCount == 0)
                                       : TRUE) {
      if (sp->desc->multiple) {
...
globle INSTANCE_TYPE* FindInstanceBySymbol(void* theEnv,
  ...
  if (modulePosition == FALSE) {
    ...
    */
    ...
    ins = InstanceData(theEnv)
              ->InstanceTable[HashInstance(moduleAndInstanceName)];
    while (ins != NULL) {
  ...
  /* =========================================
    ...
    theModule = currentModule;
    instanceName = ExtractConstructName(theEnv, modulePosition,
                                        ValueToString(moduleAndInstanceName));
    searchImports = TRUE;
  }
  ...
  /* =============================================
    ...
    moduleName = ExtractModuleName(theEnv, modulePosition,
                                   ValueToString(moduleAndInstanceName));
    theModule =
        (struct defmodule*)EnvFindDefmodule(theEnv, ValueToString(moduleName));
    instanceName = ExtractConstructName(theEnv, modulePosition,
                                        ValueToString(moduleAndInstanceName));
    if (theModule == NULL) return (NULL);
    searchImports = FALSE;
  }
  return (FindInstanceInModule(theEnv, instanceName, theModule, currentModule,
                               searchImports));
}
...
globle INSTANCE_TYPE* FindInstanceInModule(void* theEnv,
  ...
  /* ===============================
     ...
     =============================== */
  startInstance =
      InstanceData(theEnv)->InstanceTable[HashInstance(instanceName)];
  while (startInstance != NULL) {
    if (startInstance->name == instanceName) break;
    startInstance = startInstance->nxtHash;
  }

  if (startInstance == NULL) return (NULL);
  ...
  for (ins = startInstance;
    ...
    if (ins->cls->header.whichModule->theModule == theModule) return (ins);

  /* ================================
     For ::<name> formats, we need to
     search imported modules too
     ================================ */
  if (searchImports == FALSE) return (NULL);
  MarkModulesAsUnvisited(theEnv);
  return (
      FindImportedInstance(theEnv, theModule, currentModule, startInstance));
}
...
/********************************************************************
  ...
  NOTES        : None
 ********************************************************************/
globle INSTANCE_SLOT* FindInstanceSlot(void* theEnv, INSTANCE_TYPE* ins,
                                       SYMBOL_HN* sname) {
  int i;
...
/********************************************************************
  ...
  NOTES        : The slot's unique id is used as index into
                 the slot map array.
 ********************************************************************/
globle int FindInstanceTemplateSlot(void* theEnv, DEFCLASS* cls,
                                    SYMBOL_HN* sname) {
  int sid;

  sid = FindSlotNameID(theEnv, sname);
  if (sid == -1) return (-1);
  if (sid > (int)cls->maxSlotNameID) return (-1);
...
/*******************************************************
  ...
  NOTES        : None
 *******************************************************/
globle int PutSlotValue(void* theEnv, INSTANCE_TYPE* ins, INSTANCE_SLOT* sp,
                        DATA_OBJECT* val, DATA_OBJECT* setVal,
...
/*******************************************************
  ...
  NOTES        : None
 *******************************************************/
globle int DirectPutSlotValue(void* theEnv, INSTANCE_TYPE* ins,
                              INSTANCE_SLOT* sp, DATA_OBJECT* val,
                              DATA_OBJECT* setVal) {
  ...
  if (val == NULL) {
    ...
    if (sp->desc->dynamicDefault) {
      ...
      if (!EvaluateAndStoreInDataObject(theEnv, sp->desc->multiple,
                                        (EXPRESSION*)sp->desc->defaultValue,
                                        val, TRUE))
        return (FALSE);
  ...
  if (ins->basisSlots != NULL) {
    spaddr =
        &ins->slotAddresses[ins->cls->slotNameMap[sp->desc->slotName->id] - 1];
  ...
  if (ins->cls->reactive && sp->desc->reactive) {
    ...
    if (sp->desc->shared) {
      ...
      if (sharedTraversalID != -1) {
        NetworkModifyForSharedSlot(theEnv, sharedTraversalID, sp->desc->cls,
                                   sp->desc);
        ...
        EnvPrintRouter(theEnv, WERROR,
                       EnvGetDefclassName(theEnv, (void*)sp->desc->cls));
        EnvPrintRouter(theEnv, WERROR, ".\n");
      }
    } else
      ObjectNetworkAction(theEnv, OBJECT_MODIFY, (INSTANCE_TYPE*)ins,
 ...
 *******************************************************************/
globle int ValidSlotValue(void* theEnv, DATA_OBJECT* val, SLOT_DESC* sd,
                          INSTANCE_TYPE* ins, const char* theCommand) {
  int violationCode;
...
globle INSTANCE_TYPE* CheckInstance(void* theEnv, const char* func) {
  ...
  } else if ((temp.type == INSTANCE_NAME) || (temp.type == SYMBOL)) {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void NoInstanceError(void* theEnv, const char* iname, const char* func) {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void StaleInstanceAddress(void* theEnv, const char* func, int whichArg) {
...
/*******************************************************************
  ...
  NOTES        : None
 *******************************************************************/
globle void PrintSlot(void* theEnv, const char* logName, SLOT_DESC* sd,
                      INSTANCE_TYPE* ins, const char* theCommand) {
  ...
  if (theCommand != NULL)
    ...
    PrintHandler(theEnv, logName, MessageHandlerData(theEnv)->CurrentCore->hnd,
                 FALSE);
}
...
/*****************************************************
  ...
  NOTES        : None
 *****************************************************/
globle void PrintInstanceNameAndClass(void* theEnv, const char* logicalName,
                                      INSTANCE_TYPE* theInstance,
                                      intBool linefeedFlag) {
  EnvPrintRouter(theEnv, logicalName, "[");
  EnvPrintRouter(theEnv, logicalName,
                 EnvGetInstanceName(theEnv, (void*)theInstance));
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void PrintInstanceName(void* theEnv, const char* logName, void* vins) {
  INSTANCE_TYPE* ins;
  ...
  if (ins->garbage) {
    ...
    EnvPrintRouter(theEnv, logName, "[");
    EnvPrintRouter(theEnv, logName,
                   ValueToString(GetFullInstanceName(theEnv, ins)));
    EnvPrintRouter(theEnv, logName, "]");
  }
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void PrintInstanceLongForm(void* theEnv, const char* logName,
                                  void* vins) {
...
globle void DecrementObjectBasisCount(void* theEnv, void* vins) {
  ...
  if (ins->header.busyCount == 0) {
    if (ins->garbage) RemoveInstanceData(theEnv, ins);
    if (ins->cls->instanceSlotCount != 0) {
      for (i = 0; i < ins->cls->instanceSlotCount; i++)
        if (ins->basisSlots[i].value != NULL) {
          if (ins->basisSlots[i].desc->multiple)
            MultifieldDeinstall(theEnv,
                                (struct multifield*)ins->basisSlots[i].value);
          else
...
globle void IncrementObjectBasisCount(void* theEnv, void* vins) {
  ...
  if (ins->header.busyCount == 0) {
    if (ins->cls->instanceSlotCount != 0) {
      ins->basisSlots = (INSTANCE_SLOT*)gm2(
          theEnv, (sizeof(INSTANCE_SLOT) * ins->cls->instanceSlotCount));
      for (i = 0; i < ins->cls->instanceSlotCount; i++) {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle intBool InstanceIsDeleted(void* theEnv, void* vins) {
#if MAC_XCD
#pragma unused(theEnv)
#endif
...
/*****************************************************
  ...
  NOTES        : None
 *****************************************************/
static INSTANCE_TYPE* FindImportedInstance(void* theEnv,
                                           struct defmodule* theModule,
                                           struct defmodule* currentModule,
  ...
  if (theModule->visitedFlag) return (NULL);
  theModule->visitedFlag = TRUE;
  importList = theModule->importList;
  while (importList != NULL) {
    theModule = (struct defmodule*)EnvFindDefmodule(
        theEnv, ValueToString(importList->moduleName));
    for (ins = startInstance;
         (ins != NULL) ? (ins->name == startInstance->name) : FALSE;
         ins = ins->nxtHash)
      if ((ins->cls->header.whichModule->theModule == theModule) &&
...
/*****************************************************
  ...
  NOTES        : Assumes traversal id has been
                 established
 *****************************************************/
static void NetworkModifyForSharedSlot(void* theEnv, int sharedTraversalID,
                                       DEFCLASS* cls, SLOT_DESC* sd) {
  INSTANCE_TYPE* ins;
  long i;
  ...
  /* ================================================
     ...
     ================================================ */
  if (TestTraversalID(cls->traversalRecord, sharedTraversalID)) return;
  ...
  /* ===========================================
     ...
     =========================================== */
  if ((sd->slotName->id > cls->maxSlotNameID)
          ? FALSE
          : ((cls->slotNameMap[sd->slotName->id] == 0)
                 ? FALSE
                 : (cls->instanceTemplate[cls->slotNameMap[sd->slotName->id] -
                                          1] == sd))) {
...
globle void DecrementInstanceCount(void* vptr) {
...
globle void IncrementInstanceCount(void* vptr) {
  EnvIncrementInstanceCount(GetCurrentEnvironment(), vptr);
}

globle void SetInstancesChanged(int changed) {
