  ...
  while (waste != NULL) {
    ...
    if (first) {
      ...
      if (waste->header.ppForm != NULL) {
        rm(theEnv, (void*)waste->header.ppForm,
           strlen(waste->header.ppForm) + 1);
        ...
        for (tmpPtr = waste->disjunct; tmpPtr != NULL;
  ...
  while (theDefrule != NULL) {
    ...
    if (first) {
      ...
      if (theDefrule->header.ppForm != NULL) {
        ...
        rm(theEnv, (void*)theDefrule->header.ppForm,
           strlen(theDefrule->header.ppForm) + 1);
...
/* DetachJoinsDriver:                           */
/**********************************************************************/
static void DetachJoinsDriver(void* theEnv, struct defrule* theRule,
                              intBool destroy) {
...
/**********************************************************************/
static void DetachJoins(void* theEnv, struct joinNode* join, intBool destroy) {
  ...
  while (join != NULL) {
    ...
    if (join->firstJoin && (join->patternIsNegated || join->joinFromTheRight) &&
...
/***********************************************************************/
static void RemoveIntranetworkLink(void* theEnv, struct joinNode* join) {
