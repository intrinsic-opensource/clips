...
static DEFFUNCTION* AddDeffunction(void*, SYMBOL_HN*, EXPRESSION*, int, int,
                                   int, int);
...
/***************************************************************************
  ...
  NOTES        : H/L Syntax :
                 (deffunction <name> [<comment>]
                    ...
                    <action>*)
 ***************************************************************************/
globle intBool ParseDeffunction(void* theEnv, const char* readSource) {
  ...
  /* =====================================================
     ...
     ===================================================== */
  deffunctionName = GetConstructNameAndComment(
      theEnv, readSource, &DeffunctionData(theEnv)->DFInputToken, "deffunction",
      EnvFindDeffunctionInModule, NULL, "!", TRUE, TRUE, TRUE, FALSE);
  ...
  /*==========================*/
  parameterList = ParseProcParameters(
      theEnv, readSource, &DeffunctionData(theEnv)->DFInputToken, NULL,
      &wildcard, &min, &max, &DeffunctionError, NULL);
  ...
  if (ConstructData(theEnv)->CheckSyntaxMode) {
    dptr = (DEFFUNCTION*)EnvFindDeffunctionInModule(
        theEnv, ValueToString(deffunctionName));
    if (dptr == NULL) {
  ...
  actions = ParseProcActions(theEnv, "deffunction", readSource,
                             &DeffunctionData(theEnv)->DFInputToken,
                             parameterList, wildcard, NULL, NULL, &lvars, NULL);
...
/************************************************************
  ...
  NOTES        : GetConstructNameAndComment() (called before
                 ...
                 another module
 ************************************************************/
static intBool ValidDeffunctionName(void* theEnv,
  ...
  /* ============================================
     ...
     ============================================ */
  theDefgeneric = (struct constructHeader*)LookupDefgenericInScope(
      theEnv, theDeffunctionName);
  if (theDefgeneric != NULL) {
    ...
    if (theModule != ((struct defmodule*)EnvGetCurrentModule(theEnv))) {
      ...
      EnvPrintRouter(theEnv, WERROR,
                     EnvGetDefgenericName(theEnv, (void*)theDefgeneric));
  ...
  theDeffunction = (struct constructHeader*)EnvFindDeffunctionInModule(
      theEnv, theDeffunctionName);
...
static DEFFUNCTION* AddDeffunction(void* theEnv, SYMBOL_HN* name,
                                   EXPRESSION* actions, int min, int max,
                                   int lvars, int headerp) {
...
#pragma unused(headerp)
  ...
  /*===============================================================*/
  dfuncPtr =
      (DEFFUNCTION*)EnvFindDeffunctionInModule(theEnv, ValueToString(name));
  if (dfuncPtr == NULL) {
    dfuncPtr = get_struct(theEnv, deffunctionStruct);
    InitializeConstructHeader(theEnv, "deffunction",
                              (struct constructHeader*)dfuncPtr, name);
...
#if DEBUGGING_FUNCTIONS
  EnvSetDeffunctionWatch(
      theEnv, DFHadWatch ? TRUE : DeffunctionData(theEnv)->WatchDeffunctions,
      (void*)dfuncPtr);
