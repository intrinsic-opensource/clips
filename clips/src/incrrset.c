...
static void MarkPatternForIncrementalReset(void*, int,
                                           struct patternNodeHeader*, int);
#endif
...
globle void IncrementalReset(void* theEnv, struct defrule* tempRule) {
  ...
  for (theParser = PatternData(theEnv)->ListOfPatternParsers; theParser != NULL;
...
/*   incremental reset.                                               */
/**********************************************************************/
static void MarkNetworkForIncrementalReset(void* theEnv,
...
/*   incremental reset.                                               */
/**********************************************************************/
static void MarkJoinsForIncrementalReset(void* theEnv, struct joinNode* joinPtr,
                                         int value) {
  ...
  for (; joinPtr != NULL; joinPtr = GetPreviousJoin(joinPtr)) {
    ...
    //   { MarkJoinsForIncrementalReset(theEnv,(struct joinNode *)
    ...
    if (joinPtr->initialize) {
      ...
      if (joinPtr->joinFromTheRight == FALSE) {
        ...
        if (patternPtr != NULL) {
          MarkPatternForIncrementalReset(theEnv, (int)joinPtr->rhsType,
                                         patternPtr, value);
        }
      }
    }
...
/*******************************************************************************/
static void CheckForPrimableJoins(void* theEnv, struct defrule* tempRule,
  ...
  for (; joinPtr != NULL; joinPtr = GetPreviousJoin(joinPtr)) {
    ...
    // if (joinPtr->joinFromTheRight)
    //   { CheckForPrimableJoins(theEnv,tempRule,(struct joinNode *)
...
/****************************************************************************/
static void PrimeJoinFromLeftMemory(void* theEnv, struct joinNode* joinPtr) {
  ...
  if (joinPtr->firstJoin == TRUE) {
    if (joinPtr->rightSideEntryStructure == NULL) {
      ...
      for (listOfHashNodes =
           ...
           listOfHashNodes != NULL;
           listOfHashNodes = listOfHashNodes->nextHash) {
        for (theList = listOfHashNodes->alphaMemory; theList != NULL;
  ...
  while (tempLink != NULL) {
    if ((tempLink->join != joinPtr) && (tempLink->join->initialize == FALSE)) {
      break;
    }
  ...
  for (b = 0; b < theMemory->size; b++) {
    for (theList = theMemory->beta[b]; theList != NULL;
         theList = theList->nextInMemory) {
      linker = CopyPartialMatch(theEnv, theList);

      if (joinPtr->leftHash != NULL) {
        hashValue = BetaMemoryHashValue(theEnv, joinPtr->leftHash, linker, NULL,
                                        joinPtr);
      } else {
        hashValue = 0;
      }

      UpdateBetaPMLinks(theEnv, linker, theList->leftParent,
                        theList->rightParent, joinPtr, hashValue, LHS);
...
/*   entry pattern node has not been marked for initialization.             */
/****************************************************************************/
static void PrimeJoinFromRightMemory(void* theEnv, struct joinNode* joinPtr) {
  struct partialMatch *theList, *linker;
  unsigned long b;
  struct betaMemory* theMemory;
  unsigned long hashValue;
  ...
  if (tempLink == NULL) {
    if (joinPtr->firstJoin && (joinPtr->rightMemory->beta[0] == NULL) &&
  ...
  for (b = 0; b < theMemory->size; b++) {
    for (theList = theMemory->beta[b]; theList != NULL;
      ...
      if (joinPtr->rightHash != NULL) {
        hashValue = BetaMemoryHashValue(theEnv, joinPtr->rightHash, linker,
                                        NULL, joinPtr);
      } else {
        hashValue = 0;
      }
      ...
      UpdateBetaPMLinks(theEnv, linker, theList->leftParent,
                        theList->rightParent, joinPtr, hashValue, RHS);
...
/*********************************************************************/
static void MarkPatternForIncrementalReset(void* theEnv, int rhsType,
  ...
  for (theModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    ...
    if (EnvGetNextDefrule(theEnv, NULL) != NULL) {
      ...
      EnvPrintRouter(theEnv, WERROR,
                     "The incremental reset behavior cannot be changed with "
                     "rules loaded.\n");
...
globle intBool SetIncrementalReset(int value) {
