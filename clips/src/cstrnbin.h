...
#define ConstraintIndex(theConstraint)                                     \
  (((!EnvGetDynamicConstraintChecking(theEnv)) || (theConstraint == NULL)) \
       ? -1L                                                               \
       : ((long)theConstraint->bsaveIndex))
#define ConstraintPointer(i) \
  (((i) == -1L)              \
       ? NULL                \
       : (CONSTRAINT_RECORD*)&ConstraintData(theEnv)->ConstraintArray[i])
