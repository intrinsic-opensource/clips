...
globle void AssignBsaveConstructHeaderVals(
  ...
  theBsaveConstruct->whichModule =
      theConstruct->whichModule->theModule->bsaveID;
  if (theConstruct->next != NULL)
    theBsaveConstruct->next =
        ((struct constructHeader*)theConstruct->next)->bsaveID;
  else
...
LOCALE void UpdateConstructHeader(
    void* theEnv, struct bsaveConstructHeader* theBsaveConstruct,
    struct constructHeader* theConstruct, int itemModuleSize,
    void* itemModuleArray, int itemSize, void* itemArray) {
  ...
  if (theBsaveConstruct->next != -1L) {
    ...
    theConstruct->next =
        (struct constructHeader*)&((char*)itemArray)[itemOffset];
  } else
