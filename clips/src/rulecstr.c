...
static struct lhsParseNode* UnionVariableConstraints(void*,
                                                     struct lhsParseNode*,
                                                     struct lhsParseNode*);
static struct lhsParseNode* AddToVariableConstraints(void*,
                                                     struct lhsParseNode*,
                                                     struct lhsParseNode*);
static void ConstraintConflictMessage(void*, struct symbolHashNode*, int, int,
                                      struct symbolHashNode*);
static intBool CheckArgumentForConstraintError(void*, struct expr*,
                                               struct expr*, int,
...
/***********************************************************/
static intBool CheckForUnmatchableConstraints(void* theEnv,
...
/******************************************************/
static void ConstraintConflictMessage(void* theEnv,
                                      struct symbolHashNode* variableName,
                                      int thePattern, int theField,
...
/***************************************************************/
static intBool MultifieldCardinalityViolation(void* theEnv,
                                              struct lhsParseNode* theNode) {
  ...
  for (tmpNode = theNode->bottom; tmpNode != NULL; tmpNode = tmpNode->right) {
    ...
    if ((tmpNode->type == SF_VARIABLE) || (tmpNode->type == SF_WILDCARD)) {
      minFields++;
      maxFields++;
    }
  ...
  if (theNode->constraints == NULL)
    tempConstraint = GetConstraintRecord(theEnv);
  else
    tempConstraint = CopyConstraintRecord(theEnv, theNode->constraints);
  ReturnExpression(theEnv, tempConstraint->minFields);
  ReturnExpression(theEnv, tempConstraint->maxFields);
  tempConstraint->minFields =
      GenConstant(theEnv, INTEGER, EnvAddLong(theEnv, (long long)minFields));
  if (posInfinity)
    tempConstraint->maxFields =
        GenConstant(theEnv, SYMBOL, SymbolData(theEnv)->PositiveInfinity);
  else
    tempConstraint->maxFields =
        GenConstant(theEnv, INTEGER, EnvAddLong(theEnv, (long long)maxFields));
  ...
  newConstraint =
      IntersectConstraints(theEnv, theNode->constraints, tempConstraint);
  if (theNode->derivedConstraints)
    RemoveConstraint(theEnv, theNode->constraints);
...
globle intBool ProcessConnectedConstraints(
    void* theEnv, struct lhsParseNode* theNode,
    struct lhsParseNode* multifieldHeader, struct lhsParseNode* patternHead) {
  ...
  for (orNode = theNode->bottom; orNode != NULL; orNode = orNode->bottom) {
      ...
      if (!andNode->negated) {
        if (andNode->type == RETURN_VALUE_CONSTRAINT) {
          if (andNode->expression->type == FCALL) {
            rvConstraints = FunctionCallToConstraintRecord(
                theEnv, andNode->expression->value);
            tmpConstraints = andConstraints;
            andConstraints =
                IntersectConstraints(theEnv, andConstraints, rvConstraints);
          ...
          tmpConstraints = andConstraints;
          andConstraints = IntersectConstraints(theEnv, andConstraints,
                                                andNode->constraints);
          RemoveConstraint(theEnv, tmpConstraints);
        }
      }
    ...
    tmpConstraints = andConstraints;
    andConstraints =
        IntersectConstraints(theEnv, andConstraints, theNode->constraints);
    RemoveConstraint(theEnv, tmpConstraints);
    ...
    for (andNode = orNode; andNode != NULL; andNode = andNode->right) {
      if ((andNode->negated) && ConstantType(andNode->type)) {
        RemoveConstantFromConstraint(theEnv, andNode->type, andNode->value,
                                     andConstraints);
      }
    }
  ...
  if (CheckForUnmatchableConstraints(theEnv, theNode,
  ...
  if ((multifieldHeader != NULL) && (theNode->right == NULL)) {
    if (MultifieldCardinalityViolation(theEnv, multifieldHeader)) {
      ConstraintViolationErrorMessage(
          theEnv, "The group of restrictions", NULL, FALSE,
          (int)patternHead->whichCE, multifieldHeader->slot,
          multifieldHeader->index, CARDINALITY_VIOLATION,
...
static struct lhsParseNode* AddToVariableConstraints(
    void* theEnv, struct lhsParseNode* oldList, struct lhsParseNode* newItems) {
  ...
  while (list1 != NULL) {
    ...
    for (trace = list2; trace != NULL; trace = trace->right) {
      ...
      if (list1->value == trace->value) {
        ...
        temp->constraints =
            UnionConstraints(theEnv, list1->constraints, trace->constraints);
        temp->right = list3;
        list3 = temp;
        break;
      }
...
globle struct lhsParseNode* GetExpressionVarConstraints(
    void* theEnv, struct lhsParseNode* theExpression) {
  ...
  for (; theExpression != NULL; theExpression = theExpression->bottom) {
    ...
    if (theExpression->type == SF_VARIABLE) {
      ...
      list2->constraints =
          CopyConstraintRecord(theEnv, theExpression->constraints);
      list1 = AddToVariableConstraints(theEnv, list2, list1);
    }
  }
...
globle intBool CheckRHSForConstraintErrors(void* theEnv,
  ...
  for (checkList = expressionList; checkList != NULL;
...
static intBool CheckArgumentForConstraintError(
    void* theEnv, struct expr* expressionList, struct expr* lastOne, int i,
    struct FunctionDefinition* theFunction, struct lhsParseNode* theLHS) {
  int theRestriction;
  ...
  if (UnmatchableConstraint(constraint4) &&
    ...
    EnvPrintRouter(theEnv, WERROR, "Previous variable bindings of ?");
    EnvPrintRouter(theEnv, WERROR,
                   ValueToString((SYMBOL_HN*)expressionList->value));
    EnvPrintRouter(theEnv, WERROR, " caused the type restrictions");
    EnvPrintRouter(theEnv, WERROR, "\nfor argument #");
