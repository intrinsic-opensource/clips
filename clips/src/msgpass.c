...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void EnvSend(void* theEnv, DATA_OBJECT* idata, const char* msg,
...
globle void SendCommand(void* theEnv, DATA_OBJECT* result) {
  ...
  result->type = SYMBOL;
  result->value = EnvFalseSymbol(theEnv);
  if (EnvArgTypeCheck(theEnv, "send", 2, SYMBOL, &temp) == FALSE) return;
...
globle void CallNextHandler(void* theEnv, DATA_OBJECT* result) {
  ...
  if (EvaluationData(theEnv)->CurrentExpression->value ==
      (void*)FindFunction(theEnv, "override-next-handler")) {
    overridep = 1;
    args.type = ProceduralPrimitiveData(theEnv)->ProcParamArray[0].type;
    if (args.type != MULTIFIELD)
      args.value =
          (void*)ProceduralPrimitiveData(theEnv)->ProcParamArray[0].value;
    else
      args.value = (void*)&ProceduralPrimitiveData(theEnv)->ProcParamArray[0];
    args.nextArg = GetFirstArgument();
    args.argList = NULL;
    PushProcParameters(
        theEnv, &args, CountArguments(&args),
        ValueToString(MessageHandlerData(theEnv)->CurrentMessageName),
        "message", UnboundHandlerErr);
  ...
  if (MessageHandlerData(theEnv)->CurrentCore->hnd->type == MAROUND) {
    if (MessageHandlerData(theEnv)->NextInCore->hnd->type == MAROUND) {
      ...
      if (MessageHandlerData(theEnv)->CurrentCore->hnd->trace)
        WatchHandler(theEnv, WTRACE, MessageHandlerData(theEnv)->CurrentCore,
                     BEGIN_TRACE);
#endif
      if (CheckHandlerArgCount(theEnv)) {
        ...
        EvaluateProcActions(
            ...
            MessageHandlerData(theEnv)
                ->CurrentCore->hnd->cls->header.whichModule->theModule,
            MessageHandlerData(theEnv)->CurrentCore->hnd->actions,
            MessageHandlerData(theEnv)->CurrentCore->hnd->localVarCount, result,
            UnboundHandlerErr);
#if PROFILING_FUNCTIONS
  ...
  if (overridep) PopProcParameters(theEnv);
  ProcedureFunctionData(theEnv)->ReturnFlag = FALSE;
}
...
/*************************************************************************
  ...
  NOTES        : None
 *************************************************************************/
globle void FindApplicableOfName(void* theEnv, DEFCLASS* cls,
                                 HANDLER_LINK* tops[4], HANDLER_LINK* bots[4],
                                 SYMBOL_HN* mname) {
  int i;
  int e;
  HANDLER* hnd;
...
/*************************************************************************
  ...
  NOTES        : None
 *************************************************************************/
globle HANDLER_LINK* JoinHandlerLinks(void* theEnv, HANDLER_LINK* tops[4],
                                      HANDLER_LINK* bots[4], SYMBOL_HN* mname) {
  int i;
  HANDLER_LINK* mlink;

  if (tops[MPRIMARY] == NULL) {
    PrintNoHandlerError(theEnv, ValueToString(mname));
    for (i = MAROUND; i <= MAFTER; i++) DestroyHandlerLinks(theEnv, tops[i]);
    SetEvaluationError(theEnv, TRUE);
    return (NULL);
  }
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void PrintHandlerSlotGetFunction(void* theEnv, const char* logicalName,
                                        void* theValue) {
#if DEVELOPER
  ...
  sd = theDefclass
           ->instanceTemplate[theDefclass->slotNameMap[theReference->slotID] -
                              1];
 ...
 ***************************************************/
globle intBool HandlerSlotGetFunction(void* theEnv, void* theValue,
  ...
  theReference = (HANDLER_SLOT_REFERENCE*)ValueToBitMap(theValue);
  theInstance =
      (INSTANCE_TYPE*)ProceduralPrimitiveData(theEnv)->ProcParamArray[0].value;
  theDefclass = DefclassData(theEnv)->ClassIDMap[theReference->classID];
  ...
  if (theInstance->cls == theDefclass) {
    ...
    instanceSlotIndex = theInstance->cls->slotNameMap[theReference->slotID];
    if (instanceSlotIndex == 0) goto HandlerGetError;
    instanceSlotIndex--;
    sp = theInstance->slotAddresses[instanceSlotIndex];
    if (sp->desc->cls != theDefclass) goto HandlerGetError;
  }
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void PrintHandlerSlotPutFunction(void* theEnv, const char* logicalName,
                                        void* theValue) {
#if DEVELOPER
  HANDLER_SLOT_REFERENCE* theReference;
  DEFCLASS* theDefclass;
...
/***************************************************
  ...
  NOTES        : It is possible for a handler
                 ...
                 (e.g., an instance of a subclass
                  ...
                  overrode the original slot)
 ***************************************************/
globle intBool HandlerSlotPutFunction(void* theEnv, void* theValue,
                                      DATA_OBJECT* theResult) {
  HANDLER_SLOT_REFERENCE* theReference;
  DEFCLASS* theDefclass;
  INSTANCE_TYPE* theInstance;
  ...
  if (theInstance->cls == theDefclass) {
    ...
    instanceSlotIndex = theInstance->cls->slotNameMap[theReference->slotID];
    if (instanceSlotIndex == 0) goto HandlerPutError;
    instanceSlotIndex--;
    sp = theInstance->slotAddresses[instanceSlotIndex];
    if (sp->desc->cls != theDefclass) goto HandlerPutError;
  }
  ...
  if (PutSlotValue(theEnv, theInstance, sp, &theSetVal, theResult, NULL) ==
      FALSE)
    goto HandlerPutError2;
  return (TRUE);
...
globle void DynamicHandlerGetSlot(void* theEnv, DATA_OBJECT* result) {
  ...
  result->type = SYMBOL;
  result->value = EnvFalseSymbol(theEnv);
  if (CheckCurrentMessage(theEnv, "dynamic-get", TRUE) == FALSE) return;
  ...
  if ((sp->desc->publicVisibility == 0) &&
    ...
    SlotVisibilityViolationError(
        theEnv, sp->desc, MessageHandlerData(theEnv)->CurrentCore->hnd->cls);
    SetEvaluationError(theEnv, TRUE);
    return;
  }
...
globle void DynamicHandlerPutSlot(void* theEnv, DATA_OBJECT* theResult) {
  ...
  theResult->type = SYMBOL;
  theResult->value = EnvFalseSymbol(theEnv);
  if (CheckCurrentMessage(theEnv, "dynamic-put", TRUE) == FALSE) return;
  EvaluateExpression(theEnv, GetFirstArgument(), &temp);
  if (temp.type != SYMBOL) {
 ...
 *****************************************************/
static intBool PerformMessage(void* theEnv, DATA_OBJECT* result,
                              EXPRESSION* args, SYMBOL_HN* mname) {
  int oldce;
  ...
  result->value = EnvFalseSymbol(theEnv);
  EvaluationData(theEnv)->EvaluationError = FALSE;
  if (EvaluationData(theEnv)->HaltExecution) return FALSE;
  ...
  if (ProceduralPrimitiveData(theEnv)->ProcParamArray->type ==
      INSTANCE_ADDRESS) {
    ins =
        (INSTANCE_TYPE*)ProceduralPrimitiveData(theEnv)->ProcParamArray->value;
    if (ins->garbage == 1) {
      StaleInstanceAddress(theEnv, "send", 0);
      SetEvaluationError(theEnv, TRUE);
    }
    // else if (DefclassInScope(theEnv,ins->cls,(struct defmodule *)
    // EnvGetCurrentModule(theEnv)) == FALSE)
    //   NoInstanceError(theEnv,ValueToString(ins->name),"send");
    else {
      cls = ins->cls;
      ins->busy++;
    }
  } else if (ProceduralPrimitiveData(theEnv)->ProcParamArray->type ==
             INSTANCE_NAME) {
    ins = FindInstanceBySymbol(
        theEnv,
        (SYMBOL_HN*)ProceduralPrimitiveData(theEnv)->ProcParamArray->value);
    if (ins == NULL) {
      PrintErrorID(theEnv, "MSGPASS", 2, FALSE);
      EnvPrintRouter(theEnv, WERROR, "No such instance ");
      EnvPrintRouter(theEnv, WERROR,
                     ValueToString((SYMBOL_HN*)ProceduralPrimitiveData(theEnv)
                                       ->ProcParamArray->value));
  ...
  } else if ((cls = DefclassData(theEnv)
                        ->PrimitiveClassMap[ProceduralPrimitiveData(theEnv)
                                                ->ProcParamArray->type]) ==
             NULL) {
 ...
 *****************************************************************************/
static HANDLER_LINK* FindApplicableHandlers(void* theEnv, DEFCLASS* cls,
                                            SYMBOL_HN* mname) {
  int i;
  HANDLER_LINK *tops[4], *bots[4];

  for (i = MAROUND; i <= MAFTER; i++) tops[i] = bots[i] = NULL;

  for (i = 0; i < cls->allSuperclasses.classCount; i++)
    FindApplicableOfName(theEnv, cls->allSuperclasses.classArray[i], tops, bots,
                         mname);
    ...
    if (CheckHandlerArgCount(theEnv)) {
      ...
      EvaluateProcActions(
          ...
          MessageHandlerData(theEnv)
              ->CurrentCore->hnd->cls->header.whichModule->theModule,
          MessageHandlerData(theEnv)->CurrentCore->hnd->actions,
          MessageHandlerData(theEnv)->CurrentCore->hnd->localVarCount, &temp,
          UnboundHandlerErr);
...
/********************************************************
  ...
  NOTES        : None
 ********************************************************/
static void EarlySlotBindError(void* theEnv, INSTANCE_TYPE* theInstance,
...
globle void Send(DATA_OBJECT* idata, const char* msg, const char* args,
                 DATA_OBJECT* result) {
