...
/*****************************************************/
globle void DefruleBinarySetup(void* theEnv) {
  AllocateEnvironmentData(theEnv, RULEBIN_DATA,
                          sizeof(struct defruleBinaryData),
                          DeallocateDefruleBloadData);

#if BLOAD_AND_BSAVE
  AddBinaryItem(theEnv, "defrule", 20, BsaveFind, BsaveExpressions,
                BsaveStorage, BsaveBinaryItem, BloadStorage, BloadBinaryItem,
                ClearBload);
#endif
#if BLOAD || BLOAD_ONLY
  AddBinaryItem(theEnv, "defrule", 20, NULL, NULL, NULL, NULL, BloadStorage,
                BloadBinaryItem, ClearBload);
#endif
}
...
/*******************************************************/
static void DeallocateDefruleBloadData(void* theEnv) {
  ...
  space = DefruleBinaryData(theEnv)->NumberOfDefruleModules *
          sizeof(struct defruleModule);
  if (space != 0)
    genfree(theEnv, (void*)DefruleBinaryData(theEnv)->ModuleArray, space);

  space = DefruleBinaryData(theEnv)->NumberOfDefrules * sizeof(struct defrule);
  if (space != 0)
    genfree(theEnv, (void*)DefruleBinaryData(theEnv)->DefruleArray, space);

  space = DefruleBinaryData(theEnv)->NumberOfJoins * sizeof(struct joinNode);
  if (space != 0)
    genfree(theEnv, (void*)DefruleBinaryData(theEnv)->JoinArray, space);

  space = DefruleBinaryData(theEnv)->NumberOfLinks * sizeof(struct joinLink);
  if (space != 0)
    genfree(theEnv, (void*)DefruleBinaryData(theEnv)->LinkArray, space);

  if (Bloaded(theEnv)) {
    rm3(theEnv, DefruleData(theEnv)->AlphaMemoryTable,
...
/*************************************************************/
static void BsaveFind(void* theEnv) {
  ...
  for (theModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    ...
    for (theDefrule = (struct defrule*)EnvGetNextDefrule(theEnv, NULL);
      ...
      MarkConstructHeaderNeededItems(&theDefrule->header,
      ...
      ExpressionData(theEnv)->ExpressionCount +=
          ExpressionSize(theDefrule->dynamicSalience);
      ...
      for (theDisjunct = theDefrule; theDisjunct != NULL;
        ...
        ExpressionData(theEnv)->ExpressionCount +=
            ExpressionSize(theDisjunct->actions);
...
/************************************************/
static void BsaveExpressions(void* theEnv, FILE* fp) {
  struct defrule *theDefrule, *theDisjunct;
  struct defmodule* theModule;
...
/*****************************************************/
static void BsaveStorage(void* theEnv, FILE* fp) {
  size_t space;
  long int value;

  space = sizeof(long) * 5;
  GenWrite(&space, sizeof(size_t), fp);
  GenWrite(&DefruleBinaryData(theEnv)->NumberOfDefruleModules, sizeof(long int),
           fp);
...
/*******************************************/
static void BsaveBinaryItem(void* theEnv, FILE* fp) {
  size_t space;
  ...
  space = (DefruleBinaryData(theEnv)->NumberOfDefrules *
           sizeof(struct bsaveDefrule)) +
          (DefruleBinaryData(theEnv)->NumberOfJoins *
           sizeof(struct bsaveJoinNode)) +
          (DefruleBinaryData(theEnv)->NumberOfLinks *
           sizeof(struct bsaveJoinLink)) +
          (DefruleBinaryData(theEnv)->NumberOfDefruleModules *
           sizeof(struct bsaveDefruleModule));
  GenWrite(&space, sizeof(size_t), fp);
  ...
  for (theModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    ...
    theModuleItem = (struct defruleModule*)GetModuleItem(
        theEnv, NULL, FindModuleItem(theEnv, "defrule")->moduleIndex);
...
/************************************************************/
static void BsaveDisjuncts(void* theEnv, FILE* fp, struct defrule* theDefrule) {
  ...
  for (theDisjunct = theDefrule, first = TRUE; theDisjunct != NULL;
    ...
    AssignBsaveConstructHeaderVals(&tempDefrule.header, &theDisjunct->header);
    ...
    if (theDisjunct->dynamicSalience != NULL) {
      if (first) {
        ...
        ExpressionData(theEnv)->ExpressionCount +=
            ExpressionSize(theDisjunct->dynamicSalience);
      } else {
...
/********************************************/
static void BsaveJoins(void* theEnv, FILE* fp) {
...
/********************************************/
static void BsaveJoin(void* theEnv, FILE* fp, struct joinNode* joinPtr) {
  ...
  if (joinPtr->joinFromTheRight) {
    tempJoin.rightSideEntryStructure =
        BsaveJoinIndex(joinPtr->rightSideEntryStructure);
  } else {
  ...
  tempJoin.secondaryNetworkTest =
      HashedExpressionIndex(theEnv, joinPtr->secondaryNetworkTest);
...
/*   data structures to the binary file.    */
/********************************************/
static void BsaveLinks(void* theEnv, FILE* fp) {
  struct defrule *rulePtr, *disjunctPtr;
  struct defmodule* theModule;
  struct joinLink* theLink;

  for (theLink = DefruleData(theEnv)->LeftPrimeJoins; theLink != NULL;
       theLink = theLink->next) {
    BsaveLink(fp, theLink);
  }

  for (theLink = DefruleData(theEnv)->RightPrimeJoins; theLink != NULL;
       theLink = theLink->next) {
    BsaveLink(fp, theLink);
  }
...
/**************************************************/
static void BsaveTraverseLinks(void* theEnv, FILE* fp,
                               struct joinNode* joinPtr) {
  struct joinLink* theLink;

  for (; joinPtr != NULL; joinPtr = joinPtr->lastLevel) {
    if (joinPtr->marked) {
      for (theLink = joinPtr->nextLinks; theLink != NULL;
           theLink = theLink->next) {
        BsaveLink(fp, theLink);
      }
...
/*   data structure to the binary file.     */
/********************************************/
static void BsaveLink(FILE* fp, struct joinLink* linkPtr) {
...
globle void AssignBsavePatternHeaderValues(
    void* theEnv, struct bsavePatternNodeHeader* theBsaveHeader,
    struct patternNodeHeader* theHeader) {
  theBsaveHeader->multifieldNode = theHeader->multifieldNode;
  theBsaveHeader->entryJoin = BsaveJoinIndex(theHeader->entryJoin);
  theBsaveHeader->rightHash =
      HashedExpressionIndex(theEnv, theHeader->rightHash);
...
/************************************************/
static void BloadStorage(void* theEnv) {
  size_t space;
  ...
  GenReadBinary(theEnv, &DefruleBinaryData(theEnv)->NumberOfDefruleModules,
                sizeof(long int));
  GenReadBinary(theEnv, &DefruleBinaryData(theEnv)->NumberOfDefrules,
                sizeof(long int));
  GenReadBinary(theEnv, &DefruleBinaryData(theEnv)->NumberOfJoins,
                sizeof(long int));
  GenReadBinary(theEnv, &DefruleBinaryData(theEnv)->NumberOfLinks,
                sizeof(long int));
  GenReadBinary(theEnv, &DefruleBinaryData(theEnv)->RightPrimeIndex,
                sizeof(long int));
  GenReadBinary(theEnv, &DefruleBinaryData(theEnv)->LeftPrimeIndex,
                sizeof(long int));
  ...
  DefruleBinaryData(theEnv)->ModuleArray =
      (struct defruleModule*)genalloc(theEnv, space);
  ...
  DefruleBinaryData(theEnv)->DefruleArray =
      (struct defrule*)genalloc(theEnv, space);
  ...
  DefruleBinaryData(theEnv)->JoinArray =
      (struct joinNode*)genalloc(theEnv, space);
  ...
  DefruleBinaryData(theEnv)->LinkArray =
      (struct joinLink*)genalloc(theEnv, space);
}
...
/****************************************************/
static void BloadBinaryItem(void* theEnv) {
  size_t space;
  ...
  DefruleData(theEnv)->RightPrimeJoins =
      BloadJoinLinkPointer(DefruleBinaryData(theEnv)->RightPrimeIndex);
  DefruleData(theEnv)->LeftPrimeJoins =
      BloadJoinLinkPointer(DefruleBinaryData(theEnv)->LeftPrimeIndex);
}
...
/**********************************************/
static void UpdateDefruleModule(void* theEnv, void* buf, long obji) {
  struct bsaveDefruleModule* bdmPtr;

  bdmPtr = (struct bsaveDefruleModule*)buf;
  UpdateDefmoduleItemHeader(
      theEnv, &bdmPtr->header,
      &DefruleBinaryData(theEnv)->ModuleArray[obji].header,
...
/****************************************/
static void UpdateDefrule(void* theEnv, void* buf, long obji) {
  struct bsaveDefrule* br;

  br = (struct bsaveDefrule*)buf;
  UpdateConstructHeader(theEnv, &br->header,
                        &DefruleBinaryData(theEnv)->DefruleArray[obji].header,
                        (int)sizeof(struct defruleModule),
  ...
  DefruleBinaryData(theEnv)->DefruleArray[obji].logicalJoin =
      BloadJoinPointer(br->logicalJoin);
  DefruleBinaryData(theEnv)->DefruleArray[obji].lastJoin =
      BloadJoinPointer(br->lastJoin);
  DefruleBinaryData(theEnv)->DefruleArray[obji].disjunct = BloadDefrulePointer(
      DefruleBinaryData(theEnv)->DefruleArray, br->disjunct);
  ...
  DefruleBinaryData(theEnv)->DefruleArray[obji].watchActivation =
      AgendaData(theEnv)->WatchActivations;
  DefruleBinaryData(theEnv)->DefruleArray[obji].watchFiring =
      DefruleData(theEnv)->WatchRules;
#endif
}
...
/*************************************/
static void UpdateJoin(void* theEnv, void* buf, long obji) {
  ...
  DefruleBinaryData(theEnv)->JoinArray[obji].rightMatchNode =
      BloadJoinPointer(bj->rightMatchNode);
  DefruleBinaryData(theEnv)->JoinArray[obji].ruleToActivate =
      BloadDefrulePointer(DefruleBinaryData(theEnv)->DefruleArray,
                          bj->ruleToActivate);
...
globle void UpdatePatternNodeHeader(
    void* theEnv, struct patternNodeHeader* theHeader,
...
/**************************************/
static void ClearBload(void* theEnv) {
  size_t space;
  long i;
  ...
  for (i = 0; i < DefruleBinaryData(theEnv)->NumberOfDefrules; i++) {
    UnmarkConstructHeader(theEnv,
                          &DefruleBinaryData(theEnv)->DefruleArray[i].header);
  }
...
/*******************************************************/
globle void* BloadDefruleModuleReference(void* theEnv, int theIndex) {
