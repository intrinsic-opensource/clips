...
static void SlotInfoSupportFunction(void*, DATA_OBJECT*, const char*,
...
static SLOT_DESC* SlotInfoSlot(void*, DATA_OBJECT*, DEFCLASS*, const char*,
                               const char*);
...
globle void* ClassInfoFnxArgs(void* theEnv, const char* fnx, int* inhp) {
  ...
  *inhp = 0;
  ...
  if (EnvArgTypeCheck(theEnv, fnx, 1, SYMBOL, &tmp) == FALSE) return (NULL);
  ...
  if (EnvRtnArgCount(theEnv) == 2) {
    if (EnvArgTypeCheck(theEnv, fnx, 2, SYMBOL, &tmp) == FALSE) return (NULL);
...
globle void SlotAllowedValuesCommand(void* theEnv, DATA_OBJECT* result) {
  SlotInfoSupportFunction(theEnv, result, "slot-allowed-values",
                          EnvSlotAllowedValues);
}

globle void SlotAllowedClassesCommand(void* theEnv, DATA_OBJECT* result) {
  SlotInfoSupportFunction(theEnv, result, "slot-allowed-classes",
...
globle void SlotCardinalityCommand(void* theEnv, DATA_OBJECT* result) {
  SlotInfoSupportFunction(theEnv, result, "slot-cardinality",
                          EnvSlotCardinality);
}
...
/********************************************************************
  ...
  NOTES        : None
 ********************************************************************/
globle void EnvClassSlots(void* theEnv, void* clsptr, DATA_OBJECT* result,
                          int inhp) {
  long size;
  DEFCLASS* cls;
  long i;
  ...
  if (inhp) {
    for (i = 0; i < cls->instanceSlotCount; i++) {
      ...
      SetMFValue(result->value, i + 1,
                 cls->instanceTemplate[i]->slotName->name);
    }
  } else {
...
/************************************************************************
  ...
  NOTES        : None
 ************************************************************************/
globle void EnvGetDefmessageHandlerList(void* theEnv, void* clsptr,
                                        DATA_OBJECT* result, int inhp) {
  ...
  for (cls = svcls, sublen = 0; cls != NULL;
       cls = (DEFCLASS*)EnvGetNextDefclass(theEnv, (void*)cls)) {
    classiLimit = inhp ? cls->allSuperclasses.classCount : 1;
    for (classi = 0; classi < classiLimit; classi++) {
      ...
      for (j = 0; j < supcls->handlerCount; j++) {
        ...
        SetMFValue(
            result->value, i++,
            EnvAddSymbol(theEnv, MessageHandlerData(theEnv)
                                     ->hndquals[supcls->handlers[j].type]));
      }
      sublen += supcls->handlerCount * 3;
    }
  }
  if (svcls != NULL) SetNextDefclass((void*)svcls, (void*)svnxt);
}
...
/***************************************************************************
  ...
  NOTES        : None
 ***************************************************************************/
globle void EnvClassSuperclasses(void* theEnv, void* clsptr,
                                 DATA_OBJECT* result, int inhp) {
  ...
  for (i = offset, j = 1; i < plinks->classCount; i++, j++) {
    ...
    SetMFValue(result->value, j,
               GetDefclassNamePointer((void*)plinks->classArray[i]));
  }
}
...
/**************************************************************************
  ...
  NOTES        : None
 **************************************************************************/
globle void EnvClassSubclasses(void* theEnv, void* clsptr, DATA_OBJECT* result,
                               int inhp) {
  int i;  // Bug fix 2014-07-18: Previously unsigned and SetpDOEnd decremented
          // to -1.
  int id;
...
/**************************************************************************
  ...
  NOTES        : None
 **************************************************************************/
globle void ClassSubclassAddresses(void* theEnv, void* clsptr,
                                   DATA_OBJECT* result, int inhp) {
  int i;  // Bug fix 2014-07-18: Previously unsigned and SetpDOEnd decremented
          // to -1.
  int id;
...
globle void EnvSlotFacets(void* theEnv, void* clsptr, const char* sname,
                          DATA_OBJECT* result) {
  int i;
  SLOT_DESC* sp;

  if ((sp = SlotInfoSlot(theEnv, result, (DEFCLASS*)clsptr, sname,
                         "slot-facets")) == NULL)
    return;
#if DEFRULE_CONSTRUCT
  result->end = 9;
  result->value = (void*)EnvCreateMultifield(theEnv, 10L);
  for (i = 1; i <= 10; i++) SetMFType(result->value, i, SYMBOL);
#else
  result->end = 8;
  result->value = (void*)EnvCreateMultifield(theEnv, 9L);
  for (i = 1; i <= 9; i++) SetMFType(result->value, i, SYMBOL);
#endif
  if (sp->multiple)
  ...
  SetMFValue(result->value, 9,
             EnvAddSymbol(theEnv, GetCreateAccessorString((void*)sp)));
  SetMFValue(
      result->value, 10,
      sp->noWrite ? EnvAddSymbol(theEnv, "NIL") : (void*)sp->overrideMessage);
#else
  if (sp->composite)
...
globle void EnvSlotSources(void* theEnv, void* clsptr, const char* sname,
                           DATA_OBJECT* result) {
  unsigned i;
  int classi;
  SLOT_DESC *sp, *csp;
...
globle void EnvSlotTypes(void* theEnv, void* clsptr, const char* sname,
                         DATA_OBJECT* result) {
  unsigned i, j;
  SLOT_DESC* sp;
  char typemap[2];
  ...
  while (i <= msize) {
    if (TestBitMap(typemap, j)) {
      ...
      SetMFValue(result->value, i,
                 (void*)GetDefclassNamePointer(
                     (void*)DefclassData(theEnv)->PrimitiveClassMap[j]));
      i++;
    }
    j++;
  }
...
globle void EnvSlotAllowedValues(void* theEnv, void* clsptr, const char* sname,
                                 DATA_OBJECT* result) {
  int i;
  SLOT_DESC* sp;
  EXPRESSION* theExp;
  ...
  if ((sp = SlotInfoSlot(theEnv, result, (DEFCLASS*)clsptr, sname,
    ...
    return;
  if ((sp->constraint != NULL) ? (sp->constraint->restrictionList == NULL)
                               : TRUE) {
    result->type = SYMBOL;
...
globle void EnvSlotAllowedClasses(void* theEnv, void* clsptr, const char* sname,
                                  DATA_OBJECT* result) {
  int i;
  SLOT_DESC* sp;
  EXPRESSION* theExp;
...
globle void EnvSlotRange(void* theEnv, void* clsptr, const char* sname,
                         DATA_OBJECT* result) {
  SLOT_DESC* sp;
...
globle void EnvSlotCardinality(void* theEnv, void* clsptr, const char* sname,
                               DATA_OBJECT* result) {
  SLOT_DESC* sp;
...
/*****************************************************
  ...
  NOTES        : None
 *****************************************************/
static void SlotInfoSupportFunction(void* theEnv, DATA_OBJECT* result,
                                    const char* fnxname,
                                    void (*fnx)(void*, void*, const char*,
                                                DATA_OBJECT*)) {
  SYMBOL_HN* ssym;
  DEFCLASS* cls;
 ...
 *********************************************************************/
static unsigned StoreSubclasses(void* mfval, unsigned si, DEFCLASS* cls,
                                int inhp, int tvid, short storeName) {
  long i, classi;
  DEFCLASS* subcls;
...
/*********************************************************
  ...
  NOTES        : None
 *********************************************************/
static SLOT_DESC* SlotInfoSlot(void* theEnv, DATA_OBJECT* result, DEFCLASS* cls,
                               const char* sname, const char* fnxname) {
  SYMBOL_HN* ssym;
  int i;
...
#if DEFRULE_CONSTRUCT
globle intBool ClassReactiveP(void* clsptr) {
  return EnvClassReactiveP(GetCurrentEnvironment(), clsptr);
}
#endif

globle void ClassSlots(void* clsptr, DATA_OBJECT* result, int inhp) {
  EnvClassSlots(GetCurrentEnvironment(), clsptr, result, inhp);
}

globle void ClassSubclasses(void* clsptr, DATA_OBJECT* result, int inhp) {
  EnvClassSubclasses(GetCurrentEnvironment(), clsptr, result, inhp);
}

globle void ClassSuperclasses(void* clsptr, DATA_OBJECT* result, int inhp) {
  EnvClassSuperclasses(GetCurrentEnvironment(), clsptr, result, inhp);
}

globle void SlotAllowedValues(void* clsptr, const char* sname,
                              DATA_OBJECT* result) {
  EnvSlotAllowedValues(GetCurrentEnvironment(), clsptr, sname, result);
}

globle void SlotAllowedClasses(void* clsptr, const char* sname,
                               DATA_OBJECT* result) {
  EnvSlotAllowedClasses(GetCurrentEnvironment(), clsptr, sname, result);
}

globle void SlotCardinality(void* clsptr, const char* sname,
                            DATA_OBJECT* result) {
  EnvSlotCardinality(GetCurrentEnvironment(), clsptr, sname, result);
}

globle void SlotFacets(void* clsptr, const char* sname, DATA_OBJECT* result) {
  EnvSlotFacets(GetCurrentEnvironment(), clsptr, sname, result);
}

globle void SlotRange(void* clsptr, const char* sname, DATA_OBJECT* result) {
  EnvSlotRange(GetCurrentEnvironment(), clsptr, sname, result);
}

globle void SlotSources(void* clsptr, const char* sname, DATA_OBJECT* result) {
  EnvSlotSources(GetCurrentEnvironment(), clsptr, sname, result);
}

globle void SlotTypes(void* clsptr, const char* sname, DATA_OBJECT* result) {
  EnvSlotTypes(GetCurrentEnvironment(), clsptr, sname, result);
}

globle void GetDefmessageHandlerList(void* clsptr, DATA_OBJECT* result,
                                     int inhp) {
