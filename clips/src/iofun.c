...
#define IOFunctionData(theEnv) \
  ((struct IOFunctionData*)GetEnvironmentData(theEnv, IO_FUNCTION_DATA))
...
globle void IOFunctionDefinitions(void* theEnv) {
  AllocateEnvironmentData(theEnv, IO_FUNCTION_DATA,
                          sizeof(struct IOFunctionData), NULL);

#if IO_FUNCTIONS
  IOFunctionData(theEnv)->useFullCRLF = FALSE;
  IOFunctionData(theEnv)->locale =
      (SYMBOL_HN*)EnvAddSymbol(theEnv, setlocale(LC_ALL, NULL));
  IncrementSymbolCount(IOFunctionData(theEnv)->locale);
#endif

#if !RUN_TIME
#if IO_FUNCTIONS
  EnvDefineFunction2(theEnv, "printout", 'v', PTIEF PrintoutFunction,
                     "PrintoutFunction", "1*");
  EnvDefineFunction2(theEnv, "read", 'u', PTIEF ReadFunction, "ReadFunction",
                     "*1");
  EnvDefineFunction2(theEnv, "open", 'b', OpenFunction, "OpenFunction", "23*k");
  EnvDefineFunction2(theEnv, "close", 'b', CloseFunction, "CloseFunction",
                     "*1");
  EnvDefineFunction2(theEnv, "get-char", 'i', GetCharFunction,
                     "GetCharFunction", "*1");
  EnvDefineFunction2(theEnv, "put-char", 'v', PTIEF PutCharFunction,
                     "PutCharFunction", "12");
  EnvDefineFunction2(theEnv, "remove", 'b', RemoveFunction, "RemoveFunction",
                     "11k");
  EnvDefineFunction2(theEnv, "rename", 'b', RenameFunction, "RenameFunction",
                     "22k");
  EnvDefineFunction2(theEnv, "format", 's', PTIEF FormatFunction,
                     "FormatFunction", "2**us");
  EnvDefineFunction2(theEnv, "readline", 'k', PTIEF ReadlineFunction,
                     "ReadlineFunction", "*1");
  EnvDefineFunction2(theEnv, "set-locale", 'u', PTIEF SetLocaleFunction,
                     "SetLocaleFunction", "*1");
  EnvDefineFunction2(theEnv, "read-number", 'u', PTIEF ReadNumberFunction,
                     "ReadNumberFunction", "*1");
#endif
#else
#if MAC_XCD
...
globle void PrintoutFunction(void* theEnv) {
  ...
  if ((argCount = EnvArgCountCheck(theEnv, "printout", AT_LEAST, 1)) == -1)
    return;
...
globle void ReadFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  if ((numberOfArguments = EnvArgCountCheck(theEnv, "read", NO_MORE_THAN, 1)) ==
      -1) {
...
/********************************************************/
static void ReadTokenFromStdin(void* theEnv, struct token* theToken) {
  char* inputString;
  ...
  while (theToken->type == STOP) {
    ...
    while ((inchar != '\n') && (inchar != '\r') && (inchar != EOF) &&
      ...
      inputString =
          ExpandStringWithChar(theEnv, inchar, inputString,
...
globle int OpenFunction(void* theEnv) {
  ...
  if ((strcmp(accessMode, "r") != 0) && (strcmp(accessMode, "w") != 0) &&
      (strcmp(accessMode, "a") != 0) && (strcmp(accessMode, "rb") != 0) &&
      (strcmp(accessMode, "wb") != 0) && (strcmp(accessMode, "ab") != 0)) {
    SetHaltExecution(theEnv, TRUE);
    SetEvaluationError(theEnv, TRUE);
    ExpectedTypeError1(
        theEnv, "open", 3,
        "string with value \"r\", \"w\", \"a\", \"rb\", \"wb\", or \"ab\"");
    return (0);
  }
...
globle int CloseFunction(void* theEnv) {
  ...
  if ((numberOfArguments =
           EnvArgCountCheck(theEnv, "close", NO_MORE_THAN, 1)) == -1)
    return (0);
...
globle int GetCharFunction(void* theEnv) {
  ...
  if ((numberOfArguments =
           EnvArgCountCheck(theEnv, "get-char", NO_MORE_THAN, 1)) == -1) {
    return (-1);
  }
...
globle void PutCharFunction(void* theEnv) {
  ...
  if (numberOfArguments == 1) {
    if (EnvArgTypeCheck(theEnv, "put-char", 1, INTEGER, &theValue) == FALSE)
      return;
  } else {
    if (EnvArgTypeCheck(theEnv, "put-char", 2, INTEGER, &theValue) == FALSE)
      return;
  }
...
globle void* FormatFunction(void* theEnv) {
  ...
  while (formatString[form_pos] != '\0') {
    if (formatString[form_pos] != '%') {
      ...
      fstr = AppendNToString(theEnv, &formatString[start_pos], fstr,
                             form_pos - start_pos, &fpos, &fmaxm);
    } else {
      form_pos++;
      formatFlagType =
          FindFormatFlag(formatString, &form_pos, percentBuffer, FLAG_MAX);
      if (formatFlagType != ' ') {
        if ((theString = PrintFormatFlag(theEnv, percentBuffer, f_cur_arg,
                                         formatFlagType)) == NULL) {
  ...
  if (fstr != NULL) {
    ...
    if (strcmp(logicalName, "nil") != 0)
      EnvPrintRouter(theEnv, logicalName, fstr);
...
static const char* ControlStringCheck(void* theEnv, int argCount) {
  ...
  if (EnvArgTypeCheck(theEnv, "format", 2, STRING, &t_ptr) == FALSE)
    return (NULL);
...
/***********************************************/
static char FindFormatFlag(const char* formatString, size_t* a,
                           char* formatBuffer, size_t bufferMax) {
  ...
  while ((formatString[*a] != '%') && (formatString[*a] != '\0') &&
         (copy_pos < (bufferMax - 5))) {
    inchar = formatString[*a];
    (*a)++;

    if ((inchar == 'd') || (inchar == 'o') || (inchar == 'x') ||
        (inchar == 'u')) {
      ...
      formatBuffer[copy_pos] = '\0';
      return (formatFlagType);
    } else if ((inchar == 'c') || (inchar == 's') || (inchar == 'e') ||
               (inchar == 'f') || (inchar == 'g')) {
      formatBuffer[copy_pos++] = inchar;
      formatBuffer[copy_pos] = '\0';
      formatFlagType = inchar;
...
static const char* PrintFormatFlag(void* theEnv, const char* formatString,
  ...
  switch (formatType) {
    case 's':
      if (EnvArgTypeCheck(theEnv, "format", whichArg, SYMBOL_OR_STRING,
                          &theResult) == FALSE)
        return (NULL);
      theLength =
          strlen(formatString) + strlen(ValueToString(theResult.value)) + 200;
    ...
    case 'c':
      ...
      if ((GetType(theResult) == STRING) || (GetType(theResult) == SYMBOL)) {
        theLength = strlen(formatString) + 200;
        printBuffer = (char*)gm2(theEnv, (sizeof(char) * theLength));
        gensprintf(printBuffer, formatString,
                   (ValueToString(theResult.value))[0]);
      } else if (GetType(theResult) == INTEGER) {
        theLength = strlen(formatString) + 200;
        printBuffer = (char*)gm2(theEnv, (sizeof(char) * theLength));
        gensprintf(printBuffer, formatString, (char)DOToLong(theResult));
      } else {
        ExpectedTypeError1(theEnv, "format", whichArg,
    ...
    case 'o':
    case 'u':
      if (EnvArgTypeCheck(theEnv, "format", whichArg, INTEGER_OR_FLOAT,
                          &theResult) == FALSE)
        return (NULL);
      theLength = strlen(formatString) + 200;
      printBuffer = (char*)gm2(theEnv, (sizeof(char) * theLength));
      ...
      if (GetType(theResult) == FLOAT) {
        gensprintf(printBuffer, formatString,
                   (long long)ValueToDouble(theResult.value));
      } else {
        gensprintf(printBuffer, formatString,
                   (long long)ValueToLong(theResult.value));
      }
    ...
    default:
      EnvPrintRouter(theEnv, WERROR,
                     " Error in format, the conversion character");
...
globle void ReadlineFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  if ((numberOfArguments =
           EnvArgCountCheck(theEnv, "readline", NO_MORE_THAN, 1)) == -1) {
    returnValue->value = (void*)EnvAddSymbol(theEnv, "*** READ ERROR ***");
    return;
  }
  ...
  RouterData(theEnv)->CommandBufferInputCount = 0;
  RouterData(theEnv)->AwaitingInput = TRUE;
  buffer = FillBuffer(theEnv, logicalName,
...
/*************************************************************/
static char* FillBuffer(void* theEnv, const char* logicalName,
  ...
  while ((c != '\n') && (c != '\r') && (c != EOF) &&
         (!GetHaltExecution(theEnv))) {
    buf = ExpandStringWithChar(theEnv, c, buf, currentPosition, maximumSize,
                               *maximumSize + 80);
    c = EnvGetcRouter(theEnv, logicalName);
  }
  ...
  buf = ExpandStringWithChar(theEnv, EOS, buf, currentPosition, maximumSize,
                             *maximumSize + 80);
  return (buf);
}
...
globle void SetLocaleFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  if ((numArgs = EnvArgCountCheck(theEnv, "set-locale", NO_MORE_THAN, 1)) ==
      -1) {
    returnValue->type = SYMBOL;
  ...
  DecrementSymbolCount(theEnv,
                       (struct symbolHashNode*)IOFunctionData(theEnv)->locale);
...
/********************************************/
static void ReadNumber(void* theEnv, const char* logicalName,
                       struct token* theToken, int isStdin) {
  char* inputString;
  char* charPtr = NULL;
  size_t inputStringSize;
  int inchar;
  ...
  while (isspace(inchar) && (inchar != EOF) && (!GetHaltExecution(theEnv))) {
    inchar = EnvGetcRouter(theEnv, logicalName);
  }
  ...
  while ((((!isStdin) && (!isspace(inchar))) ||
          (isStdin && (inchar != '\n') && (inchar != '\r'))) &&
         (inchar != EOF) && (!GetHaltExecution(theEnv))) {
    inputString =
        ExpandStringWithChar(theEnv, inchar, inputString,
                             &RouterData(theEnv)->CommandBufferInputCount,
                             &inputStringSize, inputStringSize + 80);
