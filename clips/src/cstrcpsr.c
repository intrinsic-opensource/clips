...
globle int Load(const char* fileName) {
...
/*   while loading.                                         */
/************************************************************/
globle int EnvLoad(void* theEnv, const char* fileName) {
  FILE* theFile;
...
/*******************************************************/
globle void EnvSetParsingFileName(void* theEnv, const char* fileName) {
  ...
  if (ConstructData(theEnv)->ParsingFileName != NULL) {
    genfree(theEnv, ConstructData(theEnv)->ParsingFileName,
...
/*   being parsed by the load/batch command.              */
/**********************************************************/
globle char* EnvGetParsingFileName(void* theEnv) {
...
/**********************************************/
globle void EnvSetErrorFileName(void* theEnv, const char* fileName) {
  char* fileNameCopy = NULL;
  ...
  if (ConstructData(theEnv)->ErrorFileName != NULL) {
    genfree(theEnv, ConstructData(theEnv)->ErrorFileName,
...
/*   associated with the last error detected. */
/**********************************************/
globle char* EnvGetErrorFileName(void* theEnv) {
...
/************************************************/
globle void EnvSetWarningFileName(void* theEnv, const char* fileName) {
  char* fileNameCopy = NULL;
  ...
  if (ConstructData(theEnv)->WarningFileName != NULL) {
    genfree(theEnv, ConstructData(theEnv)->WarningFileName,
...
/*   associated with the last warning detected. */
/************************************************/
globle char* EnvGetWarningFileName(void* theEnv) {
...
/*****************************************************************/
globle int LoadConstructsFromLogicalName(void* theEnv, const char* readSource) {
  int constructFlag;
  ...
  foundConstruct =
      FindConstructBeginning(theEnv, readSource, &theToken, FALSE, &noErrors);
  ...
  while ((foundConstruct == TRUE) && (GetHaltExecution(theEnv) == FALSE)) {
    ...
    constructFlag =
        ParseConstruct(theEnv, ValueToString(theToken.value), readSource);
    ...
    if (constructFlag == 1) {
      ...
      noErrors = FALSE;
      GetToken(theEnv, readSource, &theToken);
      foundConstruct = FindConstructBeginning(theEnv, readSource, &theToken,
                                              TRUE, &noErrors);
    }
...
/********************************************************************/
static int FindConstructBeginning(void* theEnv, const char* readSource,
                                  struct token* theToken, int errorCorrection,
                                  int* noErrors) {
  ...
  while (theToken->type != STOP) {
    ...
    /*=================================================================*/
      ...
      if (FindConstruct(theEnv, ValueToString(theToken->value)) != NULL)
        return (TRUE);
...
/*************************************************/
static int FindError(void* theEnv, const char* logicalName) {
#if MAC_XCD
...
/***************************************************/
static int PrintError(void* theEnv, const char* logicalName, const char* str) {
  if (strcmp(logicalName, WERROR) == 0) {
    ConstructData(theEnv)->ErrorString = AppendToString(
        theEnv, str, ConstructData(theEnv)->ErrorString,
        &ConstructData(theEnv)->CurErrPos, &ConstructData(theEnv)->MaxErrChars);
  } else if (strcmp(logicalName, WWARNING) == 0) {
    ConstructData(theEnv)->WarningString = AppendToString(
        theEnv, str, ConstructData(theEnv)->WarningString,
        &ConstructData(theEnv)->CurWrnPos, &ConstructData(theEnv)->MaxWrnChars);
  }
...
/***********************************************/
globle void CreateErrorCaptureRouter(void* theEnv) {
  /*===========================================================*/
  ...
  if (ConstructData(theEnv)->errorCaptureRouterCount == 0) {
    EnvAddRouter(theEnv, "error-capture", 40, FindError, PrintError, NULL, NULL,
                 NULL);
  }
...
/***********************************************/
globle void DeleteErrorCaptureRouter(void* theEnv) {
  /*===========================================================*/
...
/*******************************************************/
globle void FlushParsingMessages(void* theEnv) {
  /*===========================================================*/
  ...
  if (ConstructData(theEnv)->ErrorString != NULL) {
    (*ConstructData(theEnv)->ParserErrorCallback)(
        theEnv, EnvGetErrorFileName(theEnv), NULL,
  ...
  if (ConstructData(theEnv)->WarningString != NULL) {
    (*ConstructData(theEnv)->ParserErrorCallback)(
        theEnv, EnvGetWarningFileName(theEnv),
...
/***********************************************************/
globle int ParseConstruct(void* theEnv, const char* name,
...
globle SYMBOL_HN* GetConstructNameAndComment(
    void* theEnv, const char* readSource, struct token* inputToken,
    const char* constructName, void* (*findFunction)(void*, const char*),
    int (*deleteFunction)(void*, void*), const char* constructSymbol,
    int fullMessageCR, int getComment, int moduleNameAllowed,
...
#pragma unused(fullMessageCR)
  ...
  if (separatorPosition) {
    ...
    moduleName =
        ExtractModuleName(theEnv, separatorPosition, ValueToString(name));
    if (moduleName == NULL) {
  ...
  if (FindImportExportConflict(theEnv, constructName, theModule,
                               ValueToString(name))) {
    ImportExportConflictMessage(theEnv, constructName, ValueToString(name),
                                NULL, NULL);
    return (NULL);
  }
#endif
...
/******************************************************/
globle void ImportExportConflictMessage(void* theEnv, const char* constructName,
                                        const char* itemName,
