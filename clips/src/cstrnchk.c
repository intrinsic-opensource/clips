...
static intBool CheckRangeAgainstCardinalityConstraint(void*, int, int,
                                                      CONSTRAINT_RECORD*);
...
/******************************************************/
static int CheckFunctionReturnType(int functionReturnType,
  ...
  switch (functionReturnType) {
    ...
    case 'j':
      if ((constraints->symbolsAllowed) || (constraints->stringsAllowed) ||
          (constraints->instanceNamesAllowed))
        return (TRUE);
      else
        return (FALSE);
    ...
    case 'n':
      if ((constraints->integersAllowed) || (constraints->floatsAllowed))
        return (TRUE);
      else
        return (FALSE);
...
/****************************************************/
static intBool CheckTypeConstraint(int type, CONSTRAINT_RECORD* constraints) {
...
/********************************************************/
globle intBool CheckCardinalityConstraint(void* theEnv, long number,
                                          CONSTRAINT_RECORD* constraints) {
  /*=========================================*/
...
static intBool CheckRangeAgainstCardinalityConstraint(
    void* theEnv, int min, int max, CONSTRAINT_RECORD* constraints) {
  /*=========================================*/
  /* If the constraint record is NULL, there */
  /* are no cardinality restrictions.        */
  /*=========================================*/
...
/*   FALSE is returned.                                               */
/**********************************************************************/
globle intBool CheckAllowedValuesConstraint(int type, void* vPtr,
                                            CONSTRAINT_RECORD* constraints) {
  struct expr* tmpPtr;
  ...
  for (tmpPtr = constraints->restrictionList; tmpPtr != NULL;
...
/*   FALSE is returned.                                               */
/**********************************************************************/
globle intBool CheckAllowedClassesConstraint(void* theEnv, int type, void* vPtr,
                                             CONSTRAINT_RECORD* constraints) {
#if OBJECT_SYSTEM
  struct expr* tmpPtr;
  INSTANCE_TYPE* ins;
  ...
  for (tmpPtr = constraints->classList; tmpPtr != NULL;
       tmpPtr = tmpPtr->nextArg) {
    // cmpClass = (DEFCLASS *)
    // EnvFindDefclass(theEnv,ValueToString(tmpPtr->value));
    cmpClass = (DEFCLASS*)LookupDefclassByMdlOrScope(
        theEnv, ValueToString(tmpPtr->value));
...
/*************************************************************/
static intBool CheckRangeConstraint(void* theEnv, int type, void* vPtr,
                                    CONSTRAINT_RECORD* constraints) {
  ...
  while (minList != NULL) {
    if (CompareNumbers(theEnv, type, vPtr, minList->type, minList->value) ==
        LESS_THAN) {
      minList = minList->nextArg;
      maxList = maxList->nextArg;
    } else if (CompareNumbers(theEnv, type, vPtr, maxList->type,
...
globle void ConstraintViolationErrorMessage(
    void* theEnv, const char* theWhat, const char* thePlace, int command,
    int thePattern, struct symbolHashNode* theSlot, int theField,
    int violationType, CONSTRAINT_RECORD* theConstraint, int printPrelude) {
  /*======================================================*/
...
/********************************************************************/
static void PrintRange(void* theEnv, const char* logicalName,
                       CONSTRAINT_RECORD* theConstraint) {
  if (theConstraint->minValue->value == SymbolData(theEnv)->NegativeInfinity) {
    EnvPrintRouter(theEnv, logicalName,
                   ValueToString(SymbolData(theEnv)->NegativeInfinity));
  } else
    PrintExpression(theEnv, logicalName, theConstraint->minValue);
  EnvPrintRouter(theEnv, logicalName, " to ");
  if (theConstraint->maxValue->value == SymbolData(theEnv)->PositiveInfinity) {
    EnvPrintRouter(theEnv, logicalName,
                   ValueToString(SymbolData(theEnv)->PositiveInfinity));
  } else
    PrintExpression(theEnv, logicalName, theConstraint->maxValue);
}
...
/*************************************************************/
globle int ConstraintCheckDataObject(void* theEnv, DATA_OBJECT* theData,
  ...
  if (theData->type == MULTIFIELD) {
    ...
    for (i = theData->begin; i <= theData->end; i++) {
      if ((rv = ConstraintCheckValue(theEnv, theMultifield[i].type,
                                     theMultifield[i].value, theConstraints)) !=
          NO_VIOLATION) {
        return (rv);
      }
    }
  ...
  return (ConstraintCheckValue(theEnv, theData->type, theData->value,
                               theConstraints));
}
...
/****************************************************************/
globle int ConstraintCheckValue(void* theEnv, int theType, void* theValue,
                                CONSTRAINT_RECORD* theConstraints) {
  if (CheckTypeConstraint(theType, theConstraints) == FALSE) {
    return (TYPE_VIOLATION);
  }

  else if (CheckAllowedValuesConstraint(theType, theValue, theConstraints) ==
           FALSE) {
    return (ALLOWED_VALUES_VIOLATION);
  }

  else if (CheckAllowedClassesConstraint(theEnv, theType, theValue,
                                         theConstraints) == FALSE) {
    ...
    if (CheckFunctionReturnType((int)ValueFunctionType(theValue),
                                theConstraints) == FALSE) {
...
globle int ConstraintCheckExpressionChain(void* theEnv,
  ...
  if (max == 0) max = min;
  if (CheckRangeAgainstCardinalityConstraint(theEnv, min, max,
                                             theConstraints) == FALSE) {
    return (CARDINALITY_VIOLATION);
  }
  ...
  for (theExp = theExpression; theExp != NULL; theExp = theExp->nextArg) {
    vCode = ConstraintCheckValue(theEnv, theExp->type, theExp->value,
                                 theConstraints);
    if (vCode != NO_VIOLATION) return (vCode);
  }
...
/***************************************************/
globle int ConstraintCheckExpression(void* theEnv, struct expr* theExpression,
                                     CONSTRAINT_RECORD* theConstraints) {
  int rv = NO_VIOLATION;
  ...
  while (theExpression != NULL) {
    rv = ConstraintCheckValue(theEnv, theExpression->type, theExpression->value,
                              theConstraints);
    if (rv != NO_VIOLATION) return (rv);
    rv = ConstraintCheckExpression(theEnv, theExpression->argList,
                                   theConstraints);
    if (rv != NO_VIOLATION) return (rv);
...
globle intBool UnmatchableConstraint(CONSTRAINT_RECORD* theConstraint) {
  ...
  if ((!theConstraint->anyAllowed) && (!theConstraint->symbolsAllowed) &&
