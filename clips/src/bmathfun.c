...
#define BasicMathFunctionData(theEnv) \
  ((struct basicMathFunctionData*)GetEnvironmentData(theEnv, BMATHFUN_DATA))

/***************************************************************/
/* BasicMathFunctionDefinitions: Defines basic math functions. */
/***************************************************************/
globle void BasicMathFunctionDefinitions(void* theEnv) {
  AllocateEnvironmentData(theEnv, BMATHFUN_DATA,
                          sizeof(struct basicMathFunctionData), NULL);

  BasicMathFunctionData(theEnv)->AutoFloatDividend = TRUE;

#if !RUN_TIME
  EnvDefineFunction2(theEnv, "+", 'n', PTIEF AdditionFunction,
                     "AdditionFunction", "2*n");
  EnvDefineFunction2(theEnv, "*", 'n', PTIEF MultiplicationFunction,
                     "MultiplicationFunction", "2*n");
  EnvDefineFunction2(theEnv, "-", 'n', PTIEF SubtractionFunction,
                     "SubtractionFunction", "2*n");

  EnvDefineFunction2(theEnv, "/", 'n', PTIEF DivisionFunction,
                     "DivisionFunction", "2*n");
  EnvDefineFunction2(theEnv, "div", 'g', PTIEF DivFunction, "DivFunction",
                     "2*n");
  ...
  EnvDefineFunction2(theEnv, "integer", 'g', PTIEF IntegerFunction,
                     "IntegerFunction", "11n");
  EnvDefineFunction2(theEnv, "float", 'd', PTIEF FloatFunction, "FloatFunction",
                     "11n");
  EnvDefineFunction2(theEnv, "abs", 'n', PTIEF AbsFunction, "AbsFunction",
                     "11n");
  EnvDefineFunction2(theEnv, "min", 'n', PTIEF MinFunction, "MinFunction",
                     "2*n");
  EnvDefineFunction2(theEnv, "max", 'n', PTIEF MaxFunction, "MaxFunction",
                     "2*n");
#endif
}
...
globle void AdditionFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  while (theExpression != NULL) {
    if (!GetNumericArgument(theEnv, theExpression, "+", &theArgument,
                            useFloatTotal, pos))
      theExpression = NULL;
    else
...
globle void DivisionFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  while (theExpression != NULL) {
    ...
    if ((theArgument.type == INTEGER)
            ? (ValueToLong(theArgument.value) == 0L)
            : ((theArgument.type == FLOAT)
                   ? ValueToDouble(theArgument.value) == 0.0
                   : FALSE)) {
...
globle long long DivFunction(void* theEnv) {
  ...
  theExpression = GetFirstArgument();
  if (theExpression != NULL) {
    if (!GetNumericArgument(theEnv, theExpression, "div", &theArgument, FALSE,
                            pos))
      theExpression = NULL;
    else
      theExpression = GetNextArgument(theExpression);
  ...
  while (theExpression != NULL) {
    ...
    if (theArgument.type == INTEGER)
      theNumber = ValueToLong(theArgument.value);
    else if (theArgument.type == FLOAT)
      theNumber = (long long)ValueToDouble(theArgument.value);
    else
      theNumber = 1;
...
globle int SetAutoFloatDividendCommand(void* theEnv) {
  ...
  if ((theArgument.value == EnvFalseSymbol(theEnv)) &&
      (theArgument.type == SYMBOL)) {
...
globle long long IntegerFunction(void* theEnv) {
  ...
  if (EnvArgTypeCheck(theEnv, "integer", 1, INTEGER, &valstruct) == FALSE)
    return (0LL);
...
globle double FloatFunction(void* theEnv) {
  ...
  if (EnvArgTypeCheck(theEnv, "float", 1, FLOAT, &valstruct) == FALSE)
    return (0.0);
...
globle void AbsFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  if (EnvArgTypeCheck(theEnv, "abs", 1, INTEGER_OR_FLOAT, returnValue) ==
      FALSE) {
    returnValue->type = INTEGER;
    returnValue->value = (void*)EnvAddLong(theEnv, 0L);
    return;
  ...
  if (returnValue->type == INTEGER) {
    if (ValueToLong(returnValue->value) < 0L) {
      returnValue->value =
          (void*)EnvAddLong(theEnv, -ValueToLong(returnValue->value));
    }
  } else if (ValueToDouble(returnValue->value) < 0.0) {
    returnValue->value =
        (void*)EnvAddDouble(theEnv, -ValueToDouble(returnValue->value));
  }
}
...
globle void MinFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  if ((numberOfArguments = EnvArgCountCheck(theEnv, "min", AT_LEAST, 1)) ==
      -1) {
    returnValue->type = INTEGER;
    returnValue->value = (void*)EnvAddLong(theEnv, 0L);
    return;
  ...
  if (EnvArgTypeCheck(theEnv, "min", 1, INTEGER_OR_FLOAT, returnValue) ==
      FALSE) {
    returnValue->type = INTEGER;
    returnValue->value = (void*)EnvAddLong(theEnv, 0L);
    return;
  ...
  for (i = 2; i <= numberOfArguments; i++) {
    if (EnvArgTypeCheck(theEnv, "min", i, INTEGER_OR_FLOAT, &argValue) == FALSE)
      return;
...
globle void MaxFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  if ((numberOfArguments = EnvArgCountCheck(theEnv, "max", AT_LEAST, 1)) ==
      -1) {
    returnValue->type = INTEGER;
    returnValue->value = (void*)EnvAddLong(theEnv, 0L);
    return;
  ...
  if (EnvArgTypeCheck(theEnv, "max", 1, INTEGER_OR_FLOAT, returnValue) ==
      FALSE) {
    returnValue->type = INTEGER;
    returnValue->value = (void*)EnvAddLong(theEnv, 0L);
    return;
  ...
  for (i = 2; i <= numberOfArguments; i++) {
    if (EnvArgTypeCheck(theEnv, "max", i, INTEGER_OR_FLOAT, &argValue) == FALSE)
      return;
...
globle intBool SetAutoFloatDividend(int value) {
