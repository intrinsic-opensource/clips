...
#if OBJECT_SYSTEM
        (testPtr->type != INSTANCE_NAME) &&
        (testPtr->type != INSTANCE_ADDRESS) &&
#endif
...
/*****************************************************************************/
globle intBool IdenticalExpression(struct expr* firstList,
  ...
  for (; (firstList != NULL) && (secondList != NULL);
...
globle intBool ExpressionContainsVariables(struct expr* theExpression,
  ...
  while (theExpression != NULL) {
    if (theExpression->argList != NULL) {
      if (ExpressionContainsVariables(theExpression->argList,
                                      globalsAreVariables)) {
        return (TRUE);
      }
    }
...
/************************************************/
globle struct expr* GenConstant(void* theEnv, unsigned short type,
                                void* value) {
  struct expr* top;
...
/*************************************************/
globle void PrintExpression(void* theEnv, const char* fileid,
                            struct expr* theExpression) {
  ...
  while (theExpression != NULL) {
    switch (theExpression->type) {
      ...
      case FCALL:
        ...
        EnvPrintRouter(
            theEnv, fileid,
            ValueToString(ExpressionFunctionCallName(theExpression)));
        if (theExpression->argList != NULL) {
          EnvPrintRouter(theEnv, fileid, " ");
        }
...
/*************************************************************************/
globle struct expr* CombineExpressions(void* theEnv, struct expr* expr1,
                                       struct expr* expr2) {
  struct expr* tempPtr;
...
/* NegateExpression: */
/*********************/
globle struct expr* NegateExpression(void* theEnv, struct expr* theExpression) {
  struct expr* tempPtr;
