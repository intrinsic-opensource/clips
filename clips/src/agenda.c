...
static struct salienceGroup* ReuseOrCreateSalienceGroup(void*,
                                                        struct defruleModule*,
                                                        int);
static struct salienceGroup* FindSalienceGroup(struct defruleModule*, int);
static void RemoveActivationFromGroup(void*, struct activation*,
                                      struct defruleModule*);
...
globle void InitializeAgenda(void* theEnv) {
  ...
  AddWatchItem(theEnv, "activations", 1, &AgendaData(theEnv)->WatchActivations,
               40, DefruleWatchAccess, DefruleWatchPrint);
#endif
#if !RUN_TIME
  EnvDefineFunction2(theEnv, "refresh", 'v', PTIEF RefreshCommand,
                     "RefreshCommand", "11w");

  EnvDefineFunction2(theEnv, "refresh-agenda", 'v', PTIEF RefreshAgendaCommand,
                     "RefreshAgendaCommand", "01w");
  ...
  EnvDefineFunction2(theEnv, "set-salience-evaluation", 'w',
                     ...
                     "SetSalienceEvaluationCommand", "11w");

#if DEBUGGING_FUNCTIONS
  EnvDefineFunction2(theEnv, "agenda", 'v', PTIEF AgendaCommand,
                     "AgendaCommand", "01w");
#endif
#endif
}
...
globle void AddActivation(void* theEnv, void* vTheRule, void* vBinds) {
  ...
  theGroup = ReuseOrCreateSalienceGroup(theEnv, theModuleItem,
...
static struct salienceGroup* ReuseOrCreateSalienceGroup(
    void* theEnv, struct defruleModule* theRuleModule, int salience) {
  struct salienceGroup *theGroup, *lastGroup, *newGroup;

  for (lastGroup = NULL, theGroup = theRuleModule->groupings; theGroup != NULL;
...
static struct salienceGroup* FindSalienceGroup(
  ...
  for (theGroup = theRuleModule->groupings; theGroup != NULL;
...
globle void ClearRuleFromAgenda(void* theEnv, void* vTheRule) {
  ...
  while (agendaPtr != NULL) {
    ...
    for (tempRule = theRule; tempRule != NULL; tempRule = tempRule->disjunct) {
...
globle void* EnvGetNextActivation(void* theEnv, void* actPtr) {
  ...
  if (actPtr == NULL) {
    theModuleItem = (struct defruleModule*)GetModuleItem(
        theEnv, NULL, DefruleData(theEnv)->DefruleModuleIndex);
...
/***********************************************/
globle struct partialMatch* EnvGetActivationBasis(void* theEnv, void* actPtr) {
#if MAC_XCD
...
/*   the rule associated with an activation. */
/*********************************************/
globle const char* EnvGetActivationName(void* theEnv, void* actPtr) {
#if MAC_XCD
#pragma unused(theEnv)
#endif
...
/******************************************/
globle struct defrule* EnvGetActivationRule(void* theEnv, void* actPtr) {
#if MAC_XCD
#pragma unused(theEnv)
#endif
...
/**************************************************/
globle int EnvGetActivationSalience(void* theEnv, void* actPtr) {
#if MAC_XCD
#pragma unused(theEnv)
#endif
...
/**********************************************/
globle void EnvGetActivationPPForm(void* theEnv, char* buffer,
                                   size_t bufferLength, void* theActivation) {
...
/****************************************************/
globle void EnvGetActivationBasisPPForm(void* theEnv, char* buffer,
                                        size_t bufferLength,
                                        void* vTheActivation) {
...
globle intBool EnvDeleteActivation(void* theEnv, void* theActivation) {
  if (theActivation == NULL)
    ...
    RemoveActivation(theEnv, (struct activation*)theActivation, TRUE, TRUE);
...
/****************************************************************************/
static void PrintActivation(void* theEnv, const char* logicalName,
                            void* vTheActivation) {
  struct activation* theActivation = (struct activation*)vTheActivation;
  char printSpace[20];
  ...
  EnvPrintRouter(theEnv, logicalName,
                 ValueToString(theActivation->theRule->header.name));
  EnvPrintRouter(theEnv, logicalName, ": ");
...
/*******************************/
globle void EnvAgenda(void* theEnv, const char* logicalName, void* vTheModule) {
...
/*******************************************************************/
globle void RemoveActivation(void* theEnv, void* vTheActivation,
                             int updateAgenda, int updateLinks) {
  struct defruleModule* theModuleItem;
  struct activation* theActivation = (struct activation*)vTheActivation;
...
/**************************************************************/
static void RemoveActivationFromGroup(void* theEnv,
...
/**************************************************************/
static void AgendaClearFunction(void* theEnv) {
...
globle intBool EnvRefresh(void* theEnv, void* theRule) {
  ...
  for (rulePtr = (struct defrule*)theRule; rulePtr != NULL;
    ...
    for (b = 0; b < rulePtr->lastJoin->leftMemory->size; b++) {
      for (listOfMatches = rulePtr->lastJoin->leftMemory->beta[b];
           listOfMatches != NULL; listOfMatches = listOfMatches->nextInMemory) {
        /*=======================================================*/
...
globle void RefreshAgendaCommand(void* theEnv) {
  ...
  if ((numArgs = EnvArgCountCheck(theEnv, "refresh-agenda", NO_MORE_THAN, 1)) ==
      -1)
    return;
...
globle void EnvRefreshAgenda(void* theEnv, void* vTheModule) {
  ...
  for (; theModule != NULL;
    ...
    for (theActivation = (struct activation*)EnvGetNextActivation(theEnv, NULL);
         theActivation != NULL;
         theActivation =
             (struct activation*)EnvGetNextActivation(theEnv, theActivation)) {
      theActivation->salience =
          EvaluateSalience(theEnv, theActivation->theRule);
    }
...
globle void* SetSalienceEvaluationCommand(void* theEnv) {
  ...
  if (EnvArgTypeCheck(theEnv, "set-salience-evaluation", 1, SYMBOL, &argPtr) ==
      FALSE) {
    return ((SYMBOL_HN*)EnvAddSymbol(theEnv, oldValue));
  }
  ...
  if (strcmp(argument, "when-defined") == 0) {
    ...
    ExpectedTypeError1(
        theEnv, "set-salience-evaluation", 1,
...
globle void* GetSalienceEvaluationCommand(void* theEnv) {
  ...
  return ((SYMBOL_HN*)EnvAddSymbol(
      theEnv, SalienceEvaluationName(EnvGetSalienceEvaluation(theEnv))));
}
...
/*****************************************************************/
static const char* SalienceEvaluationName(int strategy) {
  const char* sname;
...
static int EvaluateSalience(void* theEnv, void* vPtr) {
  ...
  if (EvaluateExpression(theEnv, rPtr->dynamicSalience, &salienceValue)) {
    SalienceInformationError(theEnv, "defrule",
                             ValueToString(rPtr->header.name));
    return (rPtr->salience);
  }
  ...
  if ((salience > MAX_DEFRULE_SALIENCE) || (salience < MIN_DEFRULE_SALIENCE)) {
    ...
    SalienceInformationError(
        theEnv, "defrule", ValueToString(((struct defrule*)rPtr)->header.name));
    return (rPtr->salience);
  }
...
globle void AgendaCommand(void* theEnv) {
  ...
  if ((numArgs = EnvArgCountCheck(theEnv, "agenda", NO_MORE_THAN, 1)) == -1)
    return;
...
globle void Agenda(const char* logicalName, void* vTheModule) {
  EnvAgenda(GetCurrentEnvironment(), logicalName, vTheModule);
}

globle intBool DeleteActivation(void* theActivation) {
  return EnvDeleteActivation(GetCurrentEnvironment(), theActivation);
}

globle struct partialMatch* GetActivationBasis(void* actPtr) {
  return EnvGetActivationBasis(GetCurrentEnvironment(), actPtr);
}

globle const char* GetActivationName(void* actPtr) {
  return EnvGetActivationName(GetCurrentEnvironment(), actPtr);
}

globle void GetActivationPPForm(char* buffer, unsigned bufferLength,
                                void* theActivation) {
  EnvGetActivationPPForm(GetCurrentEnvironment(), buffer, bufferLength,
                         theActivation);
}

globle struct defrule* GetActivationRule(void* actPtr) {
  return EnvGetActivationRule(GetCurrentEnvironment(), actPtr);
}

globle int GetActivationSalience(void* actPtr) {
...
globle void* GetNextActivation(void* actPtr) {
  return EnvGetNextActivation(GetCurrentEnvironment(), actPtr);
}

globle intBool Refresh(void* theRule) {
  return EnvRefresh(GetCurrentEnvironment(), theRule);
}

globle void RefreshAgenda(void* vTheModule) {
  EnvRefreshAgenda(GetCurrentEnvironment(), vTheModule);
}

globle void ReorderAgenda(void* vTheModule) {
  EnvReorderAgenda(GetCurrentEnvironment(), vTheModule);
}

globle void SetAgendaChanged(int value) {
  EnvSetAgendaChanged(GetCurrentEnvironment(), value);
}

globle int SetActivationSalience(void* actPtr, int value) {
...
globle intBool SetSalienceEvaluation(int value) {
