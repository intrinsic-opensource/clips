...
globle void AllocateExpressions(void* theEnv) {
  ...
  GenReadBinary(theEnv, (void*)&ExpressionData(theEnv)->NumberOfExpressions,
                sizeof(long));
  if (ExpressionData(theEnv)->NumberOfExpressions == 0L)
    ...
    ExpressionData(theEnv)->ExpressionArray =
        (struct expr*)genalloc(theEnv, space);
  }
}
...
/*********************************************************
  ...
  NOTES        : None
 *********************************************************/
static void UpdateExpression(void* theEnv, void* buf, long obji) {
  ...
  switch (bexp->type) {
    case FCALL:
      ExpressionData(theEnv)->ExpressionArray[obji].value =
          (void*)BloadData(theEnv)->FunctionArray[bexp->value];
      break;

    case GCALL:
#if DEFGENERIC_CONSTRUCT
      ExpressionData(theEnv)->ExpressionArray[obji].value =
          (void*)GenericPointer(bexp->value);
#else
...
#if DEFFUNCTION_CONSTRUCT
      ExpressionData(theEnv)->ExpressionArray[obji].value =
          (void*)DeffunctionPointer(bexp->value);
#else
      ExpressionData(theEnv)->ExpressionArray[obji].value = NULL;
#endif
      break;
...
#if DEFTEMPLATE_CONSTRUCT
      ExpressionData(theEnv)->ExpressionArray[obji].value =
          (void*)DeftemplatePointer(bexp->value);
#else
      ExpressionData(theEnv)->ExpressionArray[obji].value = NULL;
#endif
      break;
...
#if OBJECT_SYSTEM
      ExpressionData(theEnv)->ExpressionArray[obji].value =
          (void*)DefclassPointer(bexp->value);
#else
      ExpressionData(theEnv)->ExpressionArray[obji].value = NULL;
#endif
      break;
...
#if DEFGLOBAL_CONSTRUCT
      ExpressionData(theEnv)->ExpressionArray[obji].value =
          (void*)DefglobalPointer(bexp->value);
#else
      ExpressionData(theEnv)->ExpressionArray[obji].value = NULL;
#endif
      break;
    ...
    case INTEGER:
      ExpressionData(theEnv)->ExpressionArray[obji].value =
          (void*)SymbolData(theEnv)->IntegerArray[bexp->value];
      IncrementIntegerCount(
          (INTEGER_HN*)ExpressionData(theEnv)->ExpressionArray[obji].value);
      break;

    case FLOAT:
      ExpressionData(theEnv)->ExpressionArray[obji].value =
          (void*)SymbolData(theEnv)->FloatArray[bexp->value];
      IncrementFloatCount(
          (FLOAT_HN*)ExpressionData(theEnv)->ExpressionArray[obji].value);
      break;
    ...
    case STRING:
      ExpressionData(theEnv)->ExpressionArray[obji].value =
          (void*)SymbolData(theEnv)->SymbolArray[bexp->value];
      IncrementSymbolCount(
          (SYMBOL_HN*)ExpressionData(theEnv)->ExpressionArray[obji].value);
      break;

#if DEFTEMPLATE_CONSTRUCT
    case FACT_ADDRESS:
      ExpressionData(theEnv)->ExpressionArray[obji].value =
          (void*)&FactData(theEnv)->DummyFact;
      EnvIncrementFactCount(
          theEnv, ExpressionData(theEnv)->ExpressionArray[obji].value);
      break;
#endif
    ...
    case INSTANCE_ADDRESS:
      ExpressionData(theEnv)->ExpressionArray[obji].value =
          (void*)&InstanceData(theEnv)->DummyInstance;
    ...
    default:
      ...
      if (EvaluationData(theEnv)->PrimitivesArray[bexp->type]->bitMap) {
        ExpressionData(theEnv)->ExpressionArray[obji].value =
            (void*)SymbolData(theEnv)->BitMapArray[bexp->value];
        IncrementBitMapCount(
            (BITMAP_HN*)ExpressionData(theEnv)->ExpressionArray[obji].value);
      }
      break;
  }
...
globle void ClearBloadedExpressions(void* theEnv) {
  ...
  for (i = 0; i < (unsigned long)ExpressionData(theEnv)->NumberOfExpressions;
       i++) {
    switch (ExpressionData(theEnv)->ExpressionArray[i].type) {
      ...
      case GBL_VARIABLE:
        DecrementSymbolCount(
            theEnv,
            (SYMBOL_HN*)ExpressionData(theEnv)->ExpressionArray[i].value);
        break;
      case FLOAT:
        DecrementFloatCount(
            theEnv,
            (FLOAT_HN*)ExpressionData(theEnv)->ExpressionArray[i].value);
        break;
      case INTEGER:
        DecrementIntegerCount(
            theEnv,
            (INTEGER_HN*)ExpressionData(theEnv)->ExpressionArray[i].value);
        break;

#if DEFTEMPLATE_CONSTRUCT
      case FACT_ADDRESS:
        EnvDecrementFactCount(theEnv,
      ...
      case INSTANCE_ADDRESS:
        EnvDecrementInstanceCount(
            theEnv, ExpressionData(theEnv)->ExpressionArray[i].value);
        break;
#endif
      ...
      default:
        if (EvaluationData(theEnv)->PrimitivesArray
                [ExpressionData(theEnv)->ExpressionArray[i].type] == NULL)
          break;
        if (EvaluationData(theEnv)
                ->PrimitivesArray
                    [ExpressionData(theEnv)->ExpressionArray[i].type]
                ->bitMap) {
          DecrementBitMapCount(
              theEnv,
              (BITMAP_HN*)ExpressionData(theEnv)->ExpressionArray[i].value);
        }
        break;
    }
  }
  ...
  space = ExpressionData(theEnv)->NumberOfExpressions * sizeof(struct expr);
  if (space != 0)
    genfree(theEnv, (void*)ExpressionData(theEnv)->ExpressionArray, space);
...
globle void BsaveExpression(void* theEnv, struct expr* testPtr, FILE* fp) {
  ...
  while (testPtr != NULL) {
    ...
    if (testPtr->nextArg == NULL) {
      ...
      newIndex = ExpressionData(theEnv)->ExpressionCount +
                 ExpressionSize(testPtr->argList);
    ...
    switch (testPtr->type) {
      ...
      case FCALL:
        newTest.value =
            (long)((struct FunctionDefinition*)testPtr->value)->bsaveIndex;
        break;
      ...
      default:
        if (EvaluationData(theEnv)->PrimitivesArray[testPtr->type] == NULL)
          break;
