...
globle void* EnvFindDefclass(  // TBD Needs to look in imported
    void* theEnv, const char* classAndModuleName) {
  ...
  if (className != NULL) {
    classSymbol = FindSymbolHN(
        theEnv, ExtractModuleAndConstructName(theEnv, classAndModuleName));
...
/*******************************************************************
  ...
  NOTES        : None
 ******************************************************************/
globle void* EnvFindDefclassInModule(void* theEnv,
...
globle DEFCLASS* LookupDefclassByMdlOrScope(void* theEnv,
  ...
  if ((classSymbol = FindSymbolHN(theEnv, className)) == NULL) return (NULL);
  cls = DefclassData(theEnv)->ClassTable[HashClass(classSymbol)];
  while (cls != NULL) {
    if ((cls->header.name == classSymbol) &&
...
/****************************************************
  ...
  NOTES        : Assumes no two classes of the same
                 name are ever in the same scope
 ****************************************************/
globle DEFCLASS* LookupDefclassInScope(void* theEnv, const char* className) {
  DEFCLASS* cls;
  SYMBOL_HN* classSymbol;
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle intBool DefclassInScope(void* theEnv, DEFCLASS* theDefclass,
                               struct defmodule* theModule) {
...
globle void* EnvGetNextDefclass(void* theEnv, void* ptr) {
  return (
      (void*)GetNextConstructItem(theEnv, (struct constructHeader*)ptr,
...
globle void UndefclassCommand(void* theEnv) {
  UndefconstructCommand(theEnv, "undefclass",
...
#endif
  if (cls == NULL) return (RemoveAllUserClasses(theEnv));
...
globle void PPDefclassCommand(void* theEnv) {
  PPConstructCommand(theEnv, "ppdefclass",
                     DefclassData(theEnv)->DefclassConstruct);
}
...
globle void ListDefclassesCommand(void* theEnv) {
  ListConstructCommand(theEnv, "list-defclasses",
                       DefclassData(theEnv)->DefclassConstruct);
}
...
/***************************************************
  ...
  NOTES        : C Interface
 ***************************************************/
globle void EnvListDefclasses(void* theEnv, const char* logicalName,
                              struct defmodule* theModule) {
  ListConstruct(theEnv, DefclassData(theEnv)->DefclassConstruct, logicalName,
                theModule);
}
...
/*********************************************************
  ...
  NOTES        : None
 *********************************************************/
globle void EnvSetDefclassWatchInstances(void* theEnv, unsigned newState,
                                         void* theClass) {
#if MAC_XCD
#pragma unused(theEnv)
#endif

  if (((DEFCLASS*)theClass)->abstract) return;
...
/**********************************************************
  ...
  NOTES        : None
 **********************************************************/
globle void EnvSetDefclassWatchSlots(void* theEnv, unsigned newState,
                                     void* theClass) {
#if MAC_XCD
#pragma unused(theEnv)
#endif
 ...
 ******************************************************************/
globle unsigned DefclassWatchAccess(void* theEnv, int code, unsigned newState,
                                    EXPRESSION* argExprs) {
  if (code)
    return (ConstructSetWatchAccess(
        theEnv, DefclassData(theEnv)->DefclassConstruct, newState, argExprs,
...
/***********************************************************************
  ...
  NOTES        : Accessory function for AddWatchItem()
 ***********************************************************************/
globle unsigned DefclassWatchPrint(void* theEnv, const char* logName, int code,
                                   EXPRESSION* argExprs) {
  if (code)
    return (ConstructPrintWatchAccess(
        theEnv, DefclassData(theEnv)->DefclassConstruct, logName, argExprs,
        EnvGetDefclassWatchSlots, EnvSetDefclassWatchSlots));
  else
    return (ConstructPrintWatchAccess(
        theEnv, DefclassData(theEnv)->DefclassConstruct, logName, argExprs,
...
globle void GetDefclassListFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  GetConstructListFunction(theEnv, "get-defclass-list", returnValue,
                           DefclassData(theEnv)->DefclassConstruct);
}
 ...
 ***************************************************************/
globle void EnvGetDefclassList(void* theEnv, DATA_OBJECT* returnValue,
                               struct defmodule* theModule) {
  GetConstructList(theEnv, returnValue, DefclassData(theEnv)->DefclassConstruct,
                   theModule);
}
...
globle int HasSuperclass(DEFCLASS* c1, DEFCLASS* c2) {
  ...
  for (i = 1; i < c1->allSuperclasses.classCount; i++)
    if (c1->allSuperclasses.classArray[i] == c2) return (TRUE);
  return (FALSE);
}
...
globle SYMBOL_HN* CheckClassAndSlot(void* theEnv, const char* func,
  ...
  *cls = LookupDefclassByMdlOrScope(theEnv, DOToString(temp));
  ...
  if (EnvArgTypeCheck(theEnv, func, 2, SYMBOL, &temp) == FALSE) return (NULL);
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void SaveDefclasses(void* theEnv, void* theModule, const char* logName) {
#if DEBUGGING_FUNCTIONS
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void SaveDefclass(void* theEnv, struct constructHeader* theDefclass,
                         void* userBuffer) {
...
globle void* GetClassDefaultsModeCommand(void* theEnv) {
  ...
  return ((SYMBOL_HN*)EnvAddSymbol(
      theEnv, GetClassDefaultsModeName(EnvGetClassDefaultsMode(theEnv))));
}
...
globle void* SetClassDefaultsModeCommand(void* theEnv) {
  ...
  if (EnvArgTypeCheck(theEnv, "set-class-defaults-mode", 1, SYMBOL, &argPtr) ==
      FALSE) {
    return ((SYMBOL_HN*)EnvAddSymbol(
        theEnv, GetClassDefaultsModeName(EnvGetClassDefaultsMode(theEnv))));
  }
...
/*******************************************************************/
static const char* GetClassDefaultsModeName(unsigned short mode) {
  const char* sname;
...
globle SYMBOL_HN* GetDefclassNamePointer(void* theClass) {
  return GetConstructNamePointer((struct constructHeader*)theClass);
}

globle void SetNextDefclass(void* theClass, void* targetClass) {
...
globle const char* EnvGetDefclassName(void* theEnv, void* theClass) {
  return EnvGetConstructNameString(theEnv, (struct constructHeader*)theClass);
}

globle const char* EnvGetDefclassPPForm(void* theEnv, void* theClass) {
  return GetConstructPPForm(theEnv, (struct constructHeader*)theClass);
}

globle struct defmoduleItemHeader* EnvGetDefclassModule(void* theEnv,
                                                        void* theClass) {
  return GetConstructModuleItem((struct constructHeader*)theClass);
}

globle const char* EnvDefclassModule(void* theEnv, void* theClass) {
  return GetConstructModuleName((struct constructHeader*)theClass);
}

globle void EnvSetDefclassPPForm(void* theEnv, void* theClass,
                                 char* thePPForm) {
...
globle void* FindDefclass(const char* classAndModuleName) {
  return EnvFindDefclass(GetCurrentEnvironment(), classAndModuleName);
}

globle void GetDefclassList(DATA_OBJECT* returnValue,
                            struct defmodule* theModule) {
  EnvGetDefclassList(GetCurrentEnvironment(), returnValue, theModule);
}

globle void* GetNextDefclass(void* ptr) {
  return EnvGetNextDefclass(GetCurrentEnvironment(), ptr);
}

globle intBool IsDefclassDeletable(void* ptr) {
  return EnvIsDefclassDeletable(GetCurrentEnvironment(), ptr);
}

globle intBool Undefclass(void* theDefclass) {
  return EnvUndefclass(GetCurrentEnvironment(), theDefclass);
}

globle unsigned short SetClassDefaultsMode(unsigned short value) {
...
globle const char* GetDefclassName(void* theClass) {
  return EnvGetDefclassName(GetCurrentEnvironment(), theClass);
}

globle const char* GetDefclassPPForm(void* theClass) {
  return EnvGetDefclassPPForm(GetCurrentEnvironment(), theClass);
}

globle struct defmoduleItemHeader* GetDefclassModule(void* theClass) {
  return EnvGetDefclassModule(GetCurrentEnvironment(), theClass);
}

globle const char* DefclassModule(void* theClass) {
  return EnvDefclassModule(GetCurrentEnvironment(), theClass);
}

#if DEBUGGING_FUNCTIONS

globle unsigned GetDefclassWatchInstances(void* theClass) {
  return EnvGetDefclassWatchInstances(GetCurrentEnvironment(), theClass);
}

globle unsigned GetDefclassWatchSlots(void* theClass) {
  return EnvGetDefclassWatchSlots(GetCurrentEnvironment(), theClass);
}

globle void ListDefclasses(const char* logicalName,
                           struct defmodule* theModule) {
  EnvListDefclasses(GetCurrentEnvironment(), logicalName, theModule);
}

globle void SetDefclassWatchInstances(unsigned newState, void* theClass) {
  EnvSetDefclassWatchInstances(GetCurrentEnvironment(), newState, theClass);
}

globle void SetDefclassWatchSlots(unsigned newState, void* theClass) {
