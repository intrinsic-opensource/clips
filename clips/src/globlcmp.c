...
static int ConstructToCode(void*, const char*, const char*, char*, int, FILE*,
                           int, int);
static void DefglobalToCode(void*, FILE*, struct defglobal*, int, int, int);
static void DefglobalModuleToCode(void*, FILE*, struct defmodule*, int, int,
                                  int);
...
/**************************************************************/
static void BeforeDefglobalsToCode(void* theEnv) {
...
/*************************************************/
static void InitDefglobalsCode(void* theEnv, FILE* initFP, int imageID,
                               int maxIndices) {
#if MAC_XCD
...
static int ConstructToCode(void* theEnv, const char* fileName,
  ...
  for (theModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    ...
    moduleFile = OpenFileIfNeeded(
        ...
        &fileCount, moduleArrayVersion, headerFP, "struct defglobalModule",
        ModulePrefix(DefglobalData(theEnv)->DefglobalCodeItem), FALSE, NULL);
    ...
    DefglobalModuleToCode(theEnv, moduleFile, theModule, imageID, maxIndices,
                          moduleCount);
    ...
    for (theDefglobal = (struct defglobal*)EnvGetNextDefglobal(theEnv, NULL);
      ...
      defglobalFile = OpenFileIfNeeded(
          theEnv, defglobalFile, fileName, pathName, fileNameBuffer, fileID,
          imageID, &fileCount, defglobalArrayVersion, headerFP,
          "struct defglobal",
          ConstructPrefix(DefglobalData(theEnv)->DefglobalCodeItem), FALSE,
          NULL);
      ...
      DefglobalToCode(theEnv, defglobalFile, theDefglobal, imageID, maxIndices,
                      moduleCount);
      defglobalArrayCount++;
      defglobalFile =
          CloseFileIfNeeded(theEnv, defglobalFile, &defglobalArrayCount,
...
/**********************************************************/
static void CloseDefglobalFiles(void* theEnv, FILE* moduleFile,
                                FILE* defglobalFile, int maxIndices) {
  int count = maxIndices;
  int arrayVersion = 0;

  if (defglobalFile != NULL) {
    count = maxIndices;
    CloseFileIfNeeded(theEnv, defglobalFile, &count, &arrayVersion, maxIndices,
                      NULL, NULL);
  }
...
/***********************************************************/
static void DefglobalModuleToCode(void* theEnv, FILE* theFile,
...
/**********************************************************/
static void DefglobalToCode(void* theEnv, FILE* theFile,
                            struct defglobal* theDefglobal, int imageID,
                            int maxIndices, int moduleCount) {
  /*==================*/
  /* Defglobal Header */
  /*==================*/
  ...
  ConstructHeaderToCode(
      theEnv, theFile, &theDefglobal->header, imageID, maxIndices, moduleCount,
  ...
  fprintf(theFile, ",");
  PrintHashedExpressionReference(theEnv, theFile, theDefglobal->initial,
                                 imageID, maxIndices);
...
/***************************************************************/
globle void DefglobalCModuleReference(void* theEnv, FILE* theFile, int count,
                                      int imageID, int maxIndices) {
  fprintf(theFile, "MIHS &%s%d_%d[%d]",
          ModulePrefix(DefglobalData(theEnv)->DefglobalCodeItem), imageID,
...
/******************************************************************/
globle void DefglobalCConstructReference(void* theEnv, FILE* theFile,
                                         void* vTheGlobal, int imageID,
                                         int maxIndices) {
  ...
  if (theGlobal == NULL) {
    ...
    fprintf(theFile, "&%s%d_%ld[%ld]",
            ConstructPrefix(DefglobalData(theEnv)->DefglobalCodeItem), imageID,
