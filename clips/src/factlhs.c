...
globle struct lhsParseNode* SequenceRestrictionParse(void* theEnv,
  ...
  nextField =
      RestrictionParse(theEnv, readSource, theToken, TRUE, NULL, 1, NULL, 1);
  if (nextField == NULL) {
    ReturnLHSParseNodes(theEnv, topNode);
    return (NULL);
  }
...
/****************************************************************/
globle struct lhsParseNode* CreateInitialFactPattern(void* theEnv) {
  struct lhsParseNode* topNode;
  ...
  theDeftemplate = (struct deftemplate*)FindImportedConstruct(
      theEnv, "deftemplate", NULL, "initial-fact", &count, TRUE, NULL);
  if (theDeftemplate == NULL) {
    PrintWarningID(theEnv, "FACTLHS", 1, FALSE);
    EnvPrintRouter(theEnv, WWARNING,
                   "Creating implied initial-fact deftemplate in module ");
    EnvPrintRouter(theEnv, WWARNING,
                   EnvGetDefmoduleName(theEnv, EnvGetCurrentModule(theEnv)));
    EnvPrintRouter(theEnv, WWARNING, ".\n");
    EnvPrintRouter(theEnv, WWARNING,
                   "  You probably want to import this deftemplate from the "
                   "MAIN module.\n");
...
globle struct lhsParseNode* FactPatternParse(void* theEnv,
  ...
  theDeftemplate = (struct deftemplate*)FindImportedConstruct(
      theEnv, "deftemplate", NULL, ValueToString(theToken->value), &count, TRUE,
      NULL);
  ...
  if (theDeftemplate == NULL) {
    ...
    if (FindImportExportConflict(
      ...
      ImportExportConflictMessage(theEnv, "implied deftemplate",
                                  ValueToString(theToken->value), NULL, NULL);
      return (NULL);
    }
    ...
    if (!ConstructData(theEnv)->CheckSyntaxMode) {
      theDeftemplate =
          CreateImpliedDeftemplate(theEnv, (SYMBOL_HN*)theToken->value, TRUE);
    } else {
      theDeftemplate = NULL;
    }
  }
...
#endif /* DEFTEMPLATE_CONSTRUCT && DEFRULE_CONSTRUCT && (! RUN_TIME) && (! \
          BLOAD_ONLY) */
