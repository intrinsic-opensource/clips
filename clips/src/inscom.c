...
static long ListInstancesInModule(void*, int, const char*, const char*, intBool,
                                  intBool);
static long TabulateInstances(void*, int, const char*, DEFCLASS*, intBool,
                              intBool);
#endif
...
globle void SetupInstances(void* theEnv) {
  struct patternEntityRecord instanceInfo = {
      {"INSTANCE_ADDRESS", INSTANCE_ADDRESS, 0, 0, 0, PrintInstanceName,
       PrintInstanceLongForm, EnvUnmakeInstance, NULL, EnvGetNextInstance,
       EnvDecrementInstanceCount, EnvIncrementInstanceCount, NULL, NULL, NULL,
       NULL, NULL},
  ...
  AllocateEnvironmentData(theEnv, INSTANCE_DATA, sizeof(struct instanceData),
  ...
  memcpy(&InstanceData(theEnv)->DummyInstance, &dummyInstance,
         sizeof(INSTANCE_TYPE));

  InitializeInstanceTable(theEnv);
  InstallPrimitive(theEnv,
                   (struct entityRecord*)&InstanceData(theEnv)->InstanceInfo,
                   INSTANCE_ADDRESS);
  ...
  EnvDefineFunction2(theEnv, "initialize-instance", 'u',
                     PTIEF InactiveInitializeInstance,
                     "InactiveInitializeInstance", NULL);
  EnvDefineFunction2(theEnv, "active-initialize-instance", 'u',
                     PTIEF InitializeInstanceCommand,
                     "InitializeInstanceCommand", NULL);
  AddFunctionParser(theEnv, "active-initialize-instance",
                    ParseInitializeInstance);

  EnvDefineFunction2(theEnv, "make-instance", 'u', PTIEF InactiveMakeInstance,
                     "InactiveMakeInstance", NULL);
  EnvDefineFunction2(theEnv, "active-make-instance", 'u',
                     PTIEF MakeInstanceCommand, "MakeInstanceCommand", NULL);
  ...
  EnvDefineFunction2(theEnv, "initialize-instance", 'u',
                     PTIEF InitializeInstanceCommand,
                     "InitializeInstanceCommand", NULL);
  EnvDefineFunction2(theEnv, "make-instance", 'u', PTIEF MakeInstanceCommand,
                     "MakeInstanceCommand", NULL);
#endif
  ...
  EnvDefineFunction2(theEnv, "init-slots", 'u', PTIEF InitSlotsCommand,
                     "InitSlotsCommand", "00");

  EnvDefineFunction2(theEnv, "delete-instance", 'b',
                     PTIEF DeleteInstanceCommand, "DeleteInstanceCommand",
                     "00");
  EnvDefineFunction2(theEnv, "(create-instance)", 'b',
                     PTIEF CreateInstanceHandler, "CreateInstanceHandler",
                     "00");
  EnvDefineFunction2(theEnv, "unmake-instance", 'b',
                     PTIEF UnmakeInstanceCommand, "UnmakeInstanceCommand",
                     "1*e");

#if DEBUGGING_FUNCTIONS
  EnvDefineFunction2(theEnv, "instances", 'v', PTIEF InstancesCommand,
                     "InstancesCommand", "*3w");
  EnvDefineFunction2(theEnv, "ppinstance", 'v', PTIEF PPInstanceCommand,
                     "PPInstanceCommand", "00");
#endif
  ...
  EnvDefineFunction2(theEnv, "instance-address", 'u',
                     PTIEF InstanceAddressCommand, "InstanceAddressCommand",
                     "12eep");
  EnvDefineFunction2(theEnv, "instance-addressp", 'b',
                     PTIEF InstanceAddressPCommand, "InstanceAddressPCommand",
                     "11");
  ...
  EnvDefineFunction2(theEnv, "instancep", 'b', PTIEF InstancePCommand,
                     "InstancePCommand", "11");
  EnvDefineFunction2(theEnv, "instance-existp", 'b',
                     PTIEF InstanceExistPCommand, "InstanceExistPCommand",
                     "11e");
  EnvDefineFunction2(theEnv, "class", 'u', PTIEF ClassCommand, "ClassCommand",
                     "11");
...
globle intBool EnvDeleteInstance(void* theEnv, void* iptr) {
  ...
  if (iptr != NULL) return (QuashInstance(theEnv, (INSTANCE_TYPE*)iptr));
  ins = InstanceData(theEnv)->InstanceList;
  while (ins != NULL) {
    itmp = ins;
    ins = ins->nxtList;
    if (QuashInstance(theEnv, (INSTANCE_TYPE*)itmp) == 0) success = 0;
  }
...
globle intBool EnvUnmakeInstance(void* theEnv, void* iptr) {
  ...
  if (ins != NULL) {
    ...
    while (ins != NULL) {
      ...
      if (ins->garbage == 0) success = 0;
      ins = ins->nxtList;
      while ((ins != NULL) ? ins->garbage : FALSE) ins = ins->nxtList;
    }
  }
...
globle void InstancesCommand(void* theEnv) {
  ...
  if (argno > 0) {
    if (EnvArgTypeCheck(theEnv, "instances", 1, SYMBOL, &temp) == FALSE) return;
    theDefmodule = EnvFindDefmodule(theEnv, DOToString(temp));
    if ((theDefmodule != NULL) ? FALSE : (strcmp(DOToString(temp), "*") != 0)) {
    ...
    if (argno > 1) {
      ...
      if (LookupDefclassAnywhere(theEnv, (struct defmodule*)theDefmodule,
                                 className) == NULL) {
...
globle void PPInstanceCommand(void* theEnv) {
  ...
  if (CheckCurrentMessage(theEnv, "ppinstance", TRUE) == FALSE) return;
...
/***************************************************************
  ...
  NOTES        : None
 **************************************************************/
globle void EnvInstances(void* theEnv, const char* logicalName,
                         void* theVModule, const char* className,
                         int inheritFlag) {
  int id;
  ...
  if (theVModule == NULL) {
    ...
    while (theModule != NULL) {
      ...
      count += ListInstancesInModule(theEnv, id, logicalName, className,
                                     inheritFlag, TRUE);
      theModule =
...
/*********************************************************
  ...
  NOTES        : None
 *********************************************************/
globle void* EnvMakeInstance(void* theEnv, const char* mkstr) {
  ...
  if (tkn.type == LPAREN) {
    ...
    top = GenConstant(theEnv, FCALL,
                      (void*)FindFunction(theEnv, "make-instance"));
...
/***************************************************************
  ...
  NOTES        : None
 ***************************************************************/
globle void* EnvCreateRawInstance(void* theEnv, void* cptr, const char* iname) {
  return ((void*)BuildInstance(theEnv, (SYMBOL_HN*)EnvAddSymbol(theEnv, iname),
                               (DEFCLASS*)cptr, FALSE));
}
...
/***************************************************************************
  ...
  NOTES        : None
 ***************************************************************************/
globle void* EnvFindInstance(void* theEnv, void* theModule, const char* iname,
                             unsigned searchImports) {
  SYMBOL_HN* isym;

  isym = FindSymbolHN(theEnv, iname);
  if (isym == NULL) return (NULL);
  if (theModule == NULL) theModule = (void*)EnvGetCurrentModule(theEnv);
  return ((void*)FindInstanceInModule(
      theEnv, isym, (struct defmodule*)theModule,
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void EnvDirectGetSlot(void* theEnv, void* ins, const char* sname,
                             DATA_OBJECT* result) {
  INSTANCE_SLOT* sp;
...
globle int EnvDirectPutSlot(void* theEnv, void* ins, const char* sname,
  ...
  if (PutSlotValue(theEnv, (INSTANCE_TYPE*)ins, sp, val, &junk,
                   "external put")) {
    if ((UtilityData(theEnv)->CurrentGarbageFrame->topLevel) &&
        (!CommandLineData(theEnv)->EvaluatingTopLevelCommand) &&
        (EvaluationData(theEnv)->CurrentExpression == NULL) &&
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle const char* EnvGetInstanceName(void* theEnv, void* iptr) {
#if MAC_XCD
#pragma unused(theEnv)
#endif

  if (((INSTANCE_TYPE*)iptr)->garbage == 1) return (NULL);
...
globle void* EnvGetNextInstance(void* theEnv, void* iptr) {
  if (iptr == NULL) return ((void*)InstanceData(theEnv)->InstanceList);
  if (((INSTANCE_TYPE*)iptr)->garbage == 1) return (NULL);
...
globle void* GetNextInstanceInScope(void* theEnv, void* iptr) {
  ...
  if (ins == NULL)
    ...
    ins = ins->nxtList;
  while (ins != NULL) {
    if (DefclassInScope(theEnv, ins->cls, NULL)) return ((void*)ins);
    ins = ins->nxtList;
  }
  return (NULL);
}
...
#pragma unused(theEnv)
  ...
  if (iptr == NULL) return ((void*)((DEFCLASS*)cptr)->instanceList);
  if (((INSTANCE_TYPE*)iptr)->garbage == 1) return (NULL);
...
globle void* EnvGetNextInstanceInClassAndSubclasses(
    void* theEnv, void** cptr, void* iptr, DATA_OBJECT* iterationInfo) {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void EnvGetInstancePPForm(void* theEnv, char* buf, size_t buflen,
                                 void* iptr) {
  const char* pbuf = "***InstancePPForm***";

  if (((INSTANCE_TYPE*)iptr)->garbage == 1) return;
  if (OpenStringDestination(theEnv, pbuf, buf, buflen + 1) == 0) return;
...
globle void ClassCommand(void* theEnv, DATA_OBJECT* result) {
  ...
  if (temp.type == INSTANCE_ADDRESS) {
    ...
    switch (temp.type) {
      ...
      case FACT_ADDRESS:
        result->value = (void*)GetDefclassNamePointer(
            (void*)DefclassData(theEnv)->PrimitiveClassMap[temp.type]);
        return;
...
globle intBool UnmakeInstanceCommand(void* theEnv) {
  ...
  while (theArgument != NULL) {
    ...
    if ((theResult.type == INSTANCE_NAME) || (theResult.type == SYMBOL)) {
      ...
      ExpectedTypeError1(theEnv, "unmake-instance", argNumber,
                         "instance-address, instance-name, or the symbol *");
      SetEvaluationError(theEnv, TRUE);
      return (FALSE);
    }
    if (EnvUnmakeInstance(theEnv, ins) == FALSE) rtn = FALSE;
    if (ins == NULL) return (rtn);
    argNumber++;
...
globle void SymbolToInstanceName(void* theEnv, DATA_OBJECT* result) {
  if (EnvArgTypeCheck(theEnv, "symbol-to-instance-name", 1, SYMBOL, result) ==
      FALSE) {
...
globle void InstanceAddressCommand(void* theEnv, DATA_OBJECT* result) {
  ...
  } else if (EnvArgTypeCheck(theEnv, "instance-address", 1,
                             INSTANCE_OR_INSTANCE_NAME, &temp)) {
    if (temp.type == INSTANCE_ADDRESS) {
      ins = (INSTANCE_TYPE*)temp.value;
      if (ins->garbage == 0) {
        result->type = INSTANCE_ADDRESS;
...
globle void InstanceNameCommand(void* theEnv, DATA_OBJECT* result) {
  ...
  result->type = SYMBOL;
  result->value = EnvFalseSymbol(theEnv);
  if (EnvArgTypeCheck(theEnv, "instance-name", 1, INSTANCE_OR_INSTANCE_NAME,
                      &temp) == FALSE)
    return;
  if (temp.type == INSTANCE_ADDRESS) {
    ins = (INSTANCE_TYPE*)temp.value;
...
globle intBool InstanceExistPCommand(void* theEnv) {
  ...
  if ((temp.type == INSTANCE_NAME) || (temp.type == SYMBOL))
    return ((FindInstanceBySymbol(theEnv, (SYMBOL_HN*)temp.value) != NULL)
                ? TRUE
                : FALSE);
  ExpectedTypeError1(theEnv, "instance-existp", 1,
...
/***************************************************
  ...
  NOTES        : Assumes defclass scope flags
                 are up to date
 ***************************************************/
static long ListInstancesInModule(void* theEnv, int id, const char* logicalName,
                                  const char* className, intBool inheritFlag,
  ...
  if (className == NULL) {
    /* ==============================================
       ...
       ============================================== */
    if (allModulesFlag) {
      for (theDefclass = EnvGetNextDefclass(theEnv, NULL); theDefclass != NULL;
           theDefclass = EnvGetNextDefclass(theEnv, theDefclass))
        count +=
            TabulateInstances(theEnv, id, logicalName, (DEFCLASS*)theDefclass,
                              FALSE, allModulesFlag);
    }
    ...
    /* ===================================================
      ...
      while (theInstance != NULL) {
        ...
        count++;
        PrintInstanceNameAndClass(theEnv, logicalName,
                                  (INSTANCE_TYPE*)theInstance, TRUE);
  ...
  /* ===================================
    ...
    theDefclass = (void*)LookupDefclassAnywhere(
        theEnv, ((struct defmodule*)EnvGetCurrentModule(theEnv)), className);
    if (theDefclass != NULL) {
      count +=
          TabulateInstances(theEnv, id, logicalName, (DEFCLASS*)theDefclass,
...
/******************************************************
  ...
  NOTES        : None
 ******************************************************/
static long TabulateInstances(void* theEnv, int id, const char* logicalName,
                              DEFCLASS* cls, intBool inheritFlag,
                              intBool allModulesFlag) {
  INSTANCE_TYPE* ins;
  long i;
  long count = 0;

  if (TestTraversalID(cls->traversalRecord, id)) return (0L);
  SetTraversalID(cls->traversalRecord, id);
  for (ins = cls->instanceList; ins != NULL; ins = ins->nxtClass) {
    if (EvaluationData(theEnv)->HaltExecution) return (count);
    if (allModulesFlag) EnvPrintRouter(theEnv, logicalName, "   ");
 ...
 ***************************************************/
static void PrintInstance(void* theEnv, const char* logicalName,
                          INSTANCE_TYPE* ins, const char* separator) {
  long i;
  INSTANCE_SLOT* sp;
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static INSTANCE_SLOT* FindISlotByName(void* theEnv, INSTANCE_TYPE* ins,
                                      const char* sname) {
  SYMBOL_HN* ssym;
...
globle const char* GetInstanceName(void* iptr) {
  return EnvGetInstanceName(GetCurrentEnvironment(), iptr);
}

globle void* CreateRawInstance(void* cptr, const char* iname) {
  return EnvCreateRawInstance(GetCurrentEnvironment(), cptr, iname);
}

globle intBool DeleteInstance(void* iptr) {
  return EnvDeleteInstance(GetCurrentEnvironment(), iptr);
}

globle void DirectGetSlot(void* ins, const char* sname, DATA_OBJECT* result) {
  EnvDirectGetSlot(GetCurrentEnvironment(), ins, sname, result);
}

globle int DirectPutSlot(void* ins, const char* sname, DATA_OBJECT* val) {
  return EnvDirectPutSlot(GetCurrentEnvironment(), ins, sname, val);
}

globle void* FindInstance(void* theModule, const char* iname,
                          unsigned searchImports) {
  return EnvFindInstance(GetCurrentEnvironment(), theModule, iname,
                         searchImports);
}

globle void* GetInstanceClass(void* iptr) {
  return EnvGetInstanceClass(GetCurrentEnvironment(), iptr);
}

globle void GetInstancePPForm(char* buf, unsigned buflen, void* iptr) {
  EnvGetInstancePPForm(GetCurrentEnvironment(), buf, buflen, iptr);
}

globle void* GetNextInstance(void* iptr) {
  return EnvGetNextInstance(GetCurrentEnvironment(), iptr);
}

globle void* GetNextInstanceInClass(void* cptr, void* iptr) {
  return EnvGetNextInstanceInClass(GetCurrentEnvironment(), cptr, iptr);
}

globle void* GetNextInstanceInClassAndSubclasses(void** cptr, void* iptr,
                                                 DATA_OBJECT* iterationInfo) {
  return EnvGetNextInstanceInClassAndSubclasses(GetCurrentEnvironment(), cptr,
                                                iptr, iterationInfo);
}

#if DEBUGGING_FUNCTIONS
globle void Instances(const char* logicalName, void* theVModule,
                      const char* className, int inheritFlag) {
  EnvInstances(GetCurrentEnvironment(), logicalName, theVModule, className,
               inheritFlag);
}
#endif

globle void* MakeInstance(const char* mkstr) {
  return EnvMakeInstance(GetCurrentEnvironment(), mkstr);
}

globle intBool UnmakeInstance(void* iptr) {
  return EnvUnmakeInstance(GetCurrentEnvironment(), iptr);
}

globle int ValidInstanceAddress(void* iptr) {
