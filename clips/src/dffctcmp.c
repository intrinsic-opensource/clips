...
static int ConstructToCode(void*, const char*, const char*, char*, int, FILE*,
                           int, int);
static void DeffactsToCode(void*, FILE*, struct deffacts*, int, int, int);
static void DeffactsModuleToCode(void*, FILE*, struct defmodule*, int, int,
                                 int);
...
globle void DeffactsCompilerSetup(void* theEnv) {
  DeffactsData(theEnv)->DeffactsCodeItem = AddCodeGeneratorItem(
      theEnv, "deffacts", 0, BeforeDeffactsToCode, NULL, ConstructToCode, 2);
}
...
/*************************************************************/
static void BeforeDeffactsToCode(void* theEnv) {
...
static int ConstructToCode(void* theEnv, const char* fileName,
  ...
  for (theModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    ...
    moduleFile = OpenFileIfNeeded(
        ...
        &fileCount, moduleArrayVersion, headerFP, "struct deffactsModule",
        ModulePrefix(DeffactsData(theEnv)->DeffactsCodeItem), FALSE, NULL);
    ...
    DeffactsModuleToCode(theEnv, moduleFile, theModule, imageID, maxIndices,
                         moduleCount);
    ...
    for (theDeffacts = (struct deffacts*)EnvGetNextDeffacts(theEnv, NULL);
      ...
      deffactsFile = OpenFileIfNeeded(
          theEnv, deffactsFile, fileName, pathName, fileNameBuffer, fileID,
          imageID, &fileCount, deffactsArrayVersion, headerFP,
          "struct deffacts",
          ConstructPrefix(DeffactsData(theEnv)->DeffactsCodeItem), FALSE, NULL);
      if (deffactsFile == NULL) {
        CloseDeffactsFiles(theEnv, moduleFile, deffactsFile, maxIndices);
        return (0);
      }

      DeffactsToCode(theEnv, deffactsFile, theDeffacts, imageID, maxIndices,
                     moduleCount);
      deffactsArrayCount++;
      deffactsFile =
          CloseFileIfNeeded(theEnv, deffactsFile, &deffactsArrayCount,
...
/*********************************************************/
static void CloseDeffactsFiles(void* theEnv, FILE* moduleFile,
                               FILE* deffactsFile, int maxIndices) {
  int count = maxIndices;
  int arrayVersion = 0;

  if (deffactsFile != NULL) {
    count = maxIndices;
    CloseFileIfNeeded(theEnv, deffactsFile, &count, &arrayVersion, maxIndices,
                      NULL, NULL);
  }
...
/**********************************************************/
static void DeffactsModuleToCode(void* theEnv, FILE* theFile,
...
/*********************************************************/
static void DeffactsToCode(void* theEnv, FILE* theFile,
                           struct deffacts* theDeffacts, int imageID,
                           int maxIndices, int moduleCount) {
  /*=================*/
  /* Deffacts Header */
  /*=================*/
  ...
  ConstructHeaderToCode(
      theEnv, theFile, &theDeffacts->header, imageID, maxIndices, moduleCount,
...
/**************************************************************/
globle void DeffactsCModuleReference(void* theEnv, FILE* theFile, int count,
                                     int imageID, int maxIndices) {
  fprintf(theFile, "MIHS &%s%d_%d[%d]",
          ModulePrefix(DeffactsData(theEnv)->DeffactsCodeItem), imageID,
