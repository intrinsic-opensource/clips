...
globle void InitializeDefglobals(void* theEnv) {
  struct entityRecord globalInfo = {"GBL_VARIABLE",
                                    GBL_VARIABLE,
                                    0,
                                    0,
                                    0,
  ...
  struct entityRecord defglobalPtrRecord = {"DEFGLOBAL_PTR",
                                            DEFGLOBAL_PTR,
                                            0,
                                            0,
                                            0,
  ...
  AllocateEnvironmentData(theEnv, DEFGLOBAL_DATA, sizeof(struct defglobalData),
  ...
  InstallPrimitive(theEnv, &DefglobalData(theEnv)->DefglobalPtrRecord,
                   DEFGLOBAL_PTR);
  ...
  DefglobalData(theEnv)->DefglobalConstruct = AddConstruct(
      theEnv, "defglobal", "defglobals", ParseDefglobal, EnvFindDefglobal,
      GetConstructNamePointer, GetConstructPPForm, GetConstructModuleItem,
      EnvGetNextDefglobal, SetNextConstruct, EnvIsDefglobalDeletable,
      EnvUndefglobal, ReturnDefglobal);
}
...
/****************************************************/
static void DeallocateDefglobalData(void* theEnv) {
#if !RUN_TIME
  ...
  for (theModule = EnvGetNextDefmodule(theEnv, NULL); theModule != NULL;
    ...
    theModuleItem = (struct defglobalModule*)GetModuleItem(
        theEnv, (struct defmodule*)theModule,
...
/***************************************************/
static void DestroyDefglobalAction(void* theEnv,
...
/*********************************************************/
static void InitializeDefglobalModules(void* theEnv) {
  DefglobalData(theEnv)->DefglobalModuleIndex =
      RegisterModuleItem(theEnv, "defglobal", AllocateModule, ReturnModule,
...
static void ReturnModule(void* theEnv, void* theItem) {
  FreeConstructHeaderModule(theEnv, (struct defmoduleItemHeader*)theItem,
...
globle struct defglobalModule* GetDefglobalModuleItem(
    void* theEnv, struct defmodule* theModule) {
  return ((struct defglobalModule*)GetConstructModuleItemByIndex(
      theEnv, theModule, DefglobalData(theEnv)->DefglobalModuleIndex));
}
...
/*****************************************************/
globle void* EnvFindDefglobal(void* theEnv, const char* defglobalName) {
  return (FindNamedConstructInModuleOrImports(
      theEnv, defglobalName, DefglobalData(theEnv)->DefglobalConstruct));
}
...
/*   defglobal if found, otherwise NULL.             */
/*****************************************************/
globle void* EnvFindDefglobalInModule(void* theEnv, const char* defglobalName) {
  return (FindNamedConstructInModule(
      theEnv, defglobalName, DefglobalData(theEnv)->DefglobalConstruct));
}
...
globle void* EnvGetNextDefglobal(void* theEnv, void* defglobalPtr) {
  return (
      (void*)GetNextConstructItem(theEnv, (struct constructHeader*)defglobalPtr,
                                  DefglobalData(theEnv)->DefglobalModuleIndex));
}
...
/************************************************************/
static void ReturnDefglobal(void* theEnv, void* vTheDefglobal) {
...
/************************************************************/
#if (!BLOAD_ONLY)
static void DestroyDefglobal(void* theEnv, void* vTheDefglobal) {
  struct defglobal* theDefglobal = (struct defglobal*)vTheDefglobal;
...
/************************************************/
globle void QSetDefglobalValue(void* theEnv, struct defglobal* theGlobal,
                               DATA_OBJECT_PTR vPtr, int resetVar) {
  /*====================================================*/
  ...
  if (vPtr->type != MULTIFIELD)
    ...
    DuplicateMultifield(theEnv, &theGlobal->current, vPtr);
...
/*   otherwise NULL.                                          */
/**************************************************************/
globle struct defglobal* QFindDefglobal(void* theEnv,
  ...
  for (theDefglobal = (struct defglobal*)EnvGetNextDefglobal(theEnv, NULL);
       theDefglobal != NULL;
       theDefglobal =
           (struct defglobal*)EnvGetNextDefglobal(theEnv, theDefglobal)) {
...
/*   be returned.                                                    */
/*********************************************************************/
globle void EnvGetDefglobalValueForm(void* theEnv, char* buffer,
                                     size_t bufferLength, void* vTheGlobal) {
  ...
  EnvPrintRouter(theEnv, "GlobalValueForm",
                 ValueToString(theGlobal->header.name));
  EnvPrintRouter(theEnv, "GlobalValueForm", "* = ");
...
/**********************************************************/
static intBool GetDefglobalValue2(void* theEnv, void* theValue,
  ...
  theGlobal = (struct defglobal*)FindImportedConstruct(
      theEnv, "defglobal", NULL, ValueToString(theValue), &count, TRUE, NULL);
  ...
  if (count > 1) {
    AmbiguousReferenceErrorMessage(theEnv, "defglobal",
...
/***************************************************************/
globle int QGetDefglobalValue(void* theEnv, void* vTheGlobal,
                              DATA_OBJECT_PTR vPtr) {
  struct defglobal* theGlobal = (struct defglobal*)vTheGlobal;
  ...
  if (vPtr->type == MULTIFIELD) {
    ...
    GenCopyMemory(struct field, vPtr->end + 1,
                  ...
                  &((struct multifield*)theGlobal->current.value)
                       ->theFields[theGlobal->current.begin]);
  }
...
/*   global variable in the supplied DATA_OBJECT.           */
/************************************************************/
globle intBool EnvGetDefglobalValue(void* theEnv, const char* variableName,
                                    DATA_OBJECT_PTR vPtr) {
  struct defglobal* theDefglobal;

  if ((theDefglobal =
           (struct defglobal*)EnvFindDefglobal(theEnv, variableName)) == NULL) {
    return (FALSE);
  }
...
/****************************************************************/
globle intBool EnvSetDefglobalValue(void* theEnv, const char* variableName,
                                    DATA_OBJECT_PTR vPtr) {
  struct defglobal* theGlobal;

  if ((theGlobal = QFindDefglobal(
           theEnv, (SYMBOL_HN*)EnvAddSymbol(theEnv, variableName))) == NULL) {
    return (FALSE);
  }
...
/**********************************************************/
static void DecrementDefglobalBusyCount(void* theEnv, void* vTheGlobal) {
  struct defglobal* theGlobal = (struct defglobal*)vTheGlobal;
...
/*   of a defglobal data structure.                       */
/**********************************************************/
static void IncrementDefglobalBusyCount(void* theEnv, void* vTheGlobal) {
  struct defglobal* theGlobal = (struct defglobal*)vTheGlobal;
#if MAC_XCD
...
globle void* GetNextDefglobalInScope(void* theEnv, void* vTheGlobal) {
  ...
  if (theGlobal == NULL) {
    ...
    DefglobalData(theEnv)->TheDefmodule =
        (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    theItem = (struct defmoduleItemHeader*)GetModuleItem(
  ...
  while (DefglobalData(theEnv)->TheDefmodule != NULL) {
    ...
    for (; theGlobal != NULL;
         theGlobal =
             (struct defglobal*)EnvGetNextDefglobal(theEnv, theGlobal)) {
    ...
    DefglobalData(theEnv)->TheDefmodule =
        (struct defmodule*)EnvGetNextDefmodule(
            theEnv, DefglobalData(theEnv)->TheDefmodule);
    theItem = (struct defmoduleItemHeader*)GetModuleItem(
        theEnv, DefglobalData(theEnv)->TheDefmodule,
        DefglobalData(theEnv)->DefglobalModuleIndex);
...
globle const char* EnvDefglobalModule(void* theEnv, void* theDefglobal) {
  return GetConstructModuleName((struct constructHeader*)theDefglobal);
}

globle const char* EnvGetDefglobalName(void* theEnv, void* theDefglobal) {
  return GetConstructNameString((struct constructHeader*)theDefglobal);
}

globle const char* EnvGetDefglobalPPForm(void* theEnv, void* theDefglobal) {
...
globle const char* DefglobalModule(void* theDefglobal) {
  return EnvDefglobalModule(GetCurrentEnvironment(), theDefglobal);
}

globle void* FindDefglobal(const char* defglobalName) {
  return EnvFindDefglobal(GetCurrentEnvironment(), defglobalName);
}

globle const char* GetDefglobalName(void* theDefglobal) {
  return EnvGetDefglobalName(GetCurrentEnvironment(), theDefglobal);
}

globle const char* GetDefglobalPPForm(void* theDefglobal) {
  return EnvGetDefglobalPPForm(GetCurrentEnvironment(), theDefglobal);
}

globle intBool GetDefglobalValue(const char* variableName,
                                 DATA_OBJECT_PTR vPtr) {
  return EnvGetDefglobalValue(GetCurrentEnvironment(), variableName, vPtr);
}

globle void GetDefglobalValueForm(char* buffer, unsigned bufferLength,
                                  void* vTheGlobal) {
  EnvGetDefglobalValueForm(GetCurrentEnvironment(), buffer, bufferLength,
                           vTheGlobal);
}
...
globle void* GetNextDefglobal(void* defglobalPtr) {
  return EnvGetNextDefglobal(GetCurrentEnvironment(), defglobalPtr);
}

globle intBool IsDefglobalDeletable(void* ptr) {
  return EnvIsDefglobalDeletable(GetCurrentEnvironment(), ptr);
}

globle intBool SetDefglobalValue(const char* variableName,
                                 DATA_OBJECT_PTR vPtr) {
  return EnvSetDefglobalValue(GetCurrentEnvironment(), variableName, vPtr);
}

globle void SetGlobalsChanged(int value) {
