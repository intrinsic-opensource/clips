...
#define TextProcessingData(theEnv) \
...
static struct entries* AllocateEntryNode(void*, FILE*, const char*, const char*,
                                         int);
static int AttachLeaf(void*, struct lists*, struct entries*, FILE*, const char*,
                      int);
...
/****************************************************************************/
globle int TextLookupFetch(void* theEnv, const char* file) {
  ...
  if (entries_ct == 0) TextLookupToss(theEnv, file);
  return (entries_ct);
}
...
/*          failure or success.                                               */
/******************************************************************************/
globle int TextLookupToss(void* theEnv, const char* file) {
...
/*                                                                            */
/******************************************************************************/
static FILE* GetEntries(void* theEnv, const char* file, char** menu, char* name,
                        int* code) {
...
/******************************************************************************/
static FILE* GetCurrentMenu(void* theEnv, const char* file, int* status) {
...
/******************************************************************************/
static char* grab_string(void* theEnv, FILE* fp, char* buf, int bufsize) {
...
/**************************************************************************/
static int findstr(const char* s, const char* t) {
  int i, j, k;
...
/******************************************************************************/
static struct lists* NewFetchFile(void* theEnv, const char* file) {
  struct lists *lptr = NULL, *lnode;

  if (TextProcessingData(theEnv)->headings != NULL) {
    lptr = TextProcessingData(theEnv)->headings;
    while (lptr->next != NULL) {
      if (strcmp(lptr->file, file) == 0) return (NULL);
      lptr = lptr->next;
    }
    if (strcmp(lptr->file, file) == 0) return (NULL);
  }
...
/*address.                                                                    */
/******************************************************************************/
static struct entries* AllocateEntryNode(void* theEnv, FILE* fp,
...
/******************************************************************************/
static int AttachLeaf(void* theEnv, struct lists* lnode, struct entries* enode,
                      FILE* fp, const char* file, int line_ct) {
  int p_flag; /*Used in searching the tree for a parent*/

  /*====================*/
  /*First topic for file*/
  /*====================*/
  if (lnode->topics == NULL) lnode->topics = enode;
...
/******************************************************************************/
static long int LookupEntry(void* theEnv, const char* file, char** menu,
                            char* name, int* code) {
  ...
  if (lptr->curr_menu != NULL) *menu = lptr->curr_menu->name;
  return (eptr->offset);
}
...
/******************************************************************************/
static void TossFunction(void* theEnv, struct entries* eptr) {
  struct entries* prev;

  while (eptr != NULL) {
    if (eptr->child != NULL) TossFunction(theEnv, eptr->child);
    prev = eptr;
    eptr = eptr->next;
...
static FILE* FindTopicInEntries(void*, const char*, struct topics*, char**,
                                int*);
...
/* Input : Via the argument "stack", logical name for the output, the name of
 * the */
...
globle int PrintRegionCommand(void* theEnv) {
  ...
  fp = FindTopicInEntries(theEnv, params->next->name, params->next->next, menu,
                          &status);
  if ((status != NO_FILE) && (status != NO_TOPIC) && (status != EXIT)) {
    if (strcmp(params->name, "t") == 0) genstrcpy(params->name, STDOUT);
...
globle void* GetRegionCommand(void* theEnv) {
  ...
  if (theString == NULL) {
    ...
    if ((sLength > 0) && (((theString[sLength - 1] == '\r') &&
                           (theString[sLength - 2] == '\n')) ||
                          ((theString[sLength - 1] == '\n') &&
...
/******************************************************************************/
static struct topics* GetCommandLineTopics(void* theEnv) {
  ...
  for (theIndex = 1; theIndex <= topic_num; theIndex++) {
    ...
    if ((GetType(val) == SYMBOL) || (GetType(val) == STRING))
      ...
      genstrncpy(tnode->name, FloatToString(theEnv, DOToDouble(val)),
                 NAMESIZE - 1);
    else if (GetType(val) == INTEGER)
      genstrncpy(tnode->name, LongIntegerToString(theEnv, DOToLong(val)),
                 NAMESIZE - 1);
    else
...
/******************************************************************************/
static FILE* FindTopicInEntries(void* theEnv, const char* file,
                                struct topics* main_topic, char** menu,
                                int* status) {
  ...
  if (tptr != end_list) do {
      ...
      if ((*status == NO_FILE) || (*status == NO_TOPIC)) break;
      tptr = tptr->next;
    } while (tptr != end_list);
  else
    /*==================================================================*/
...
globle void HelpFunctionDefinitions(void* theEnv) {
  AllocateEnvironmentData(theEnv, TEXTPRO_DATA,
                          sizeof(struct textProcessingData),
                          DeallocateTextProcessingData);
#if !RUN_TIME
#if TEXTPRO_FUNCTIONS
  EnvDefineFunction2(theEnv, "fetch", 'u', PTIEF FetchCommand, "FetchCommand",
                     "11k");
  EnvDefineFunction2(theEnv, "toss", 'b', PTIEF TossCommand, "TossCommand",
                     "11k");
  EnvDefineFunction2(theEnv, "print-region", 'b', PTIEF PrintRegionCommand,
                     "PrintRegionCommand", "2**wk");
  EnvDefineFunction2(theEnv, "get-region", 's', PTIEF GetRegionCommand,
                     "GetRegionCommand", "1**k");
#endif
#endif
}
