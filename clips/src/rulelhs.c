...
static struct lhsParseNode* RuleBodyParse(void*, const char*, struct token*,
                                          const char*, int*);
static void DeclarationParse(void*, const char*, const char*, int*);
static struct lhsParseNode* LHSPattern(void*, const char*, int, const char*,
                                       int*, int, struct token*, const char*);
static struct lhsParseNode* ConnectedPatternParse(void*, const char*,
                                                  struct token*, int*);
static struct lhsParseNode* GroupPatterns(void*, const char*, int, const char*,
                                          int*);
static struct lhsParseNode* TestPattern(void*, const char*, int*);
static struct lhsParseNode* AssignmentParse(void*, const char*, SYMBOL_HN*,
                                            int*);
static void TagLHSLogicalNodes(struct lhsParseNode*);
static struct lhsParseNode* SimplePatternParse(void*, const char*,
                                               struct token*, int*);
...
/*******************************************************************/
globle struct lhsParseNode* ParseRuleLHS(void* theEnv, const char* readSource,
...
/*                 =>                                    */
/*********************************************************/
static struct lhsParseNode* RuleBodyParse(void* theEnv, const char* readSource,
                                          struct token* theToken,
                                          const char* ruleName, int* error) {
...
/********************************************************/
static void DeclarationParse(void* theEnv, const char* readSource,
                             const char* ruleName, int* error) {
  struct token theToken;
  struct expr* packPtr;
  int notDone = TRUE;
...
static void ParseSalience(void* theEnv, const char* readSource,
  ...
  PatternData(theEnv)->SalienceExpression =
      ParseAtomOrExpression(theEnv, readSource, NULL);
  ...
  if (EvaluateExpression(theEnv, PatternData(theEnv)->SalienceExpression,
                         &salienceValue)) {
...
/**************************************************************/
static void ParseAutoFocus(void* theEnv, const char* readSource, int* error) {
  struct token theToken;
...
/*                           <forall-CE> | <exists-CE>           */
/*****************************************************************/
static struct lhsParseNode* LHSPattern(void* theEnv, const char* readSource,
                                       int terminator,
                                       const char* terminatorString, int* error,
  ...
  /*=======================================*/
    ...
    theNode =
        AssignmentParse(theEnv, readSource, (SYMBOL_HN*)theToken.value, error);
  }
  ...
  /*=================================================*/
               ...
               ? (strcmp(theToken.printForm, terminatorString) == 0)
               : FALSE) {
    return (NULL);
  }
...
/*                           <conditional-element>+)                 */
/*********************************************************************/
static struct lhsParseNode* ConnectedPatternParse(void* theEnv,
  ...
  if ((connectorValue == NOT_CE) || (connectorValue == EXISTS_CE) ||
      (connectorValue == FORALL_CE)) {
...
/***********************************************/
static struct lhsParseNode* GroupPatterns(void* theEnv, const char* readSource,
                                          int terminator,
...
/**************************************************************/
static struct lhsParseNode* TestPattern(void* theEnv, const char* readSource,
                                        int* error) {
  struct lhsParseNode* theNode;
  struct token theToken;
...
/****************************************************************/
static struct lhsParseNode* AssignmentParse(void* theEnv,
                                            const char* readSource,
...
/************************************************************/
static void TagLHSLogicalNodes(struct lhsParseNode* nodePtr) {
  while (nodePtr != NULL) {
    nodePtr->logical = TRUE;
    if ((nodePtr->type == AND_CE) || (nodePtr->type == OR_CE) ||
        (nodePtr->type == NOT_CE)) {
...
/*                  <template-pattern-CE>                  */
/***********************************************************/
static struct lhsParseNode* SimplePatternParse(void* theEnv,
                                               const char* readSource,
                                               struct token* theToken,
                                               int* error) {
  struct lhsParseNode* theNode;
...
globle void PropagatePatternType(struct lhsParseNode* theLHS,
  ...
  while (theLHS != NULL) {
    ...
    if (theLHS->expression != NULL)
      PropagatePatternType(theLHS->expression, theParser);
