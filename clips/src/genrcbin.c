...
#define MethodPointer(i) \
  (((i) == -1L) ? NULL   \
                : (DEFMETHOD*)&DefgenericBinaryData(theEnv)->MethodArray[i])
#define RestrictionPointer(i) \
  (((i) == -1L)               \
       ? NULL                 \
       : (RESTRICTION*)&DefgenericBinaryData(theEnv)->RestrictionArray[i])
#define TypePointer(i) \
  (((i) == -1L) ? NULL : (void**)&DefgenericBinaryData(theEnv)->TypeArray[i])
...
typedef struct bsaveMethod {
  short index;
  short restrictionCount, minRestrictions, maxRestrictions, localVarCount;
  int system;
...
globle void SetupGenericsBload(void* theEnv) {
  AllocateEnvironmentData(theEnv, GENRCBIN_DATA,
                          sizeof(struct defgenericBinaryData),
                          DeallocateDefgenericBinaryData);
#if BLOAD_AND_BSAVE
  AddBinaryItem(theEnv, "generic functions", 0, BsaveGenericsFind,
                BsaveGenericsExpressions, BsaveStorageGenerics, BsaveGenerics,
                BloadStorageGenerics, BloadGenerics, ClearBloadGenerics);
#endif
#if BLOAD || BLOAD_ONLY
...
/***********************************************************/
static void DeallocateDefgenericBinaryData(void* theEnv) {
#if (BLOAD || BLOAD_ONLY || BLOAD_AND_BSAVE) && (!RUN_TIME)
  size_t space;

  space =
      DefgenericBinaryData(theEnv)->GenericCount * sizeof(struct defgeneric);
  if (space != 0)
    genfree(theEnv, (void*)DefgenericBinaryData(theEnv)->DefgenericArray,
            space);
  ...
  space = DefgenericBinaryData(theEnv)->RestrictionCount *
          sizeof(struct restriction);
  if (space != 0)
    genfree(theEnv, (void*)DefgenericBinaryData(theEnv)->RestrictionArray,
            space);
  ...
  space = DefgenericBinaryData(theEnv)->ModuleCount *
 ...
 ***************************************************************************/
static void BsaveGenericsFind(void* theEnv) {
  ...
  DefgenericBinaryData(theEnv)->ModuleCount = DoForAllConstructs(
      theEnv, MarkDefgenericItems,
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void MarkDefgenericItems(void* theEnv,
...
#pragma unused(userBuffer)
  ...
  MarkConstructHeaderNeededItems(&gfunc->header,
                                 DefgenericBinaryData(theEnv)->GenericCount++);
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void BsaveGenericsExpressions(void* theEnv, FILE* fp) {
  /* ================================================================
     Important to save all expressions for methods before any
     expressions for restrictions, since methods will be stored first
     ================================================================ */
  DoForAllConstructs(theEnv, BsaveMethodExpressions,
                     DefgenericData(theEnv)->DefgenericModuleIndex, FALSE,
                     (void*)fp);

  DoForAllConstructs(theEnv, BsaveRestrictionExpressions,
                     DefgenericData(theEnv)->DefgenericModuleIndex, FALSE,
                     (void*)fp);
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void BsaveMethodExpressions(void* theEnv,
                                   struct constructHeader* theDefgeneric,
                                   void* userBuffer) {
  DEFGENERIC* gfunc = (DEFGENERIC*)theDefgeneric;
  long i;
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void BsaveRestrictionExpressions(void* theEnv,
                                        struct constructHeader* theDefgeneric,
                                        void* userBuffer) {
  DEFGENERIC* gfunc = (DEFGENERIC*)theDefgeneric;
  long i, j;
...
/***********************************************************
  ...
  NOTES        : None
 ***********************************************************/
static void BsaveStorageGenerics(void* theEnv, FILE* fp) {
  size_t space;

  space = sizeof(long) * 5;
  GenWrite((void*)&space, sizeof(size_t), fp);
  GenWrite((void*)&DefgenericBinaryData(theEnv)->ModuleCount, sizeof(long), fp);
  GenWrite((void*)&DefgenericBinaryData(theEnv)->GenericCount, sizeof(long),
           fp);
  GenWrite((void*)&DefgenericBinaryData(theEnv)->MethodCount, sizeof(long), fp);
  GenWrite((void*)&DefgenericBinaryData(theEnv)->RestrictionCount, sizeof(long),
           fp);
...
/****************************************************************************************
  ...
  DESCRIPTION  : Writes out generic function in binary format
                 ...
                 All generic modules (sizeof(DEFGENERIC_MODULE) * Number of
 generic modules) All generic headers (sizeof(DEFGENERIC) * Number of generics)
                 All methods (sizeof(DEFMETHOD) * Number of methods)
                 All method restrictions (sizeof(RESTRICTION) * Number of
 restrictions) All restriction type arrays (sizeof(void *) * # of types) INPUTS
 : File pointer of binary file RETURNS      : Nothing useful SIDE EFFECTS :
 Binary file adjusted NOTES        : None
 ****************************************************************************************/
static void BsaveGenerics(void* theEnv, FILE* fp) {
  ...
  /* =====================================================================
     ...
     ===================================================================== */
  space = ((unsigned long)DefgenericBinaryData(theEnv)->ModuleCount *
           sizeof(BSAVE_DEFGENERIC_MODULE)) +
          ((unsigned long)DefgenericBinaryData(theEnv)->GenericCount *
           sizeof(BSAVE_GENERIC)) +
          ((unsigned long)DefgenericBinaryData(theEnv)->MethodCount *
           sizeof(BSAVE_METHOD)) +
          ((unsigned long)DefgenericBinaryData(theEnv)->RestrictionCount *
           sizeof(BSAVE_RESTRICTION)) +
          ((unsigned long)DefgenericBinaryData(theEnv)->TypeCount *
  ...
  while (theModule != NULL) {
    theModuleItem = (DEFGENERIC_MODULE*)GetModuleItem(
        theEnv, theModule, FindModuleItem(theEnv, "defgeneric")->moduleIndex);
    AssignBsaveDefmdlItemHdrVals(&dummy_generic_module.header,
                                 &theModuleItem->header);
    GenWrite((void*)&dummy_generic_module, sizeof(BSAVE_DEFGENERIC_MODULE), fp);
    theModule =
  ...
  /* ======================================
     ...
     ====================================== */
  DefgenericBinaryData(theEnv)->MethodCount = 0L;
  DoForAllConstructs(theEnv, BsaveDefgenericHeader,
                     DefgenericData(theEnv)->DefgenericModuleIndex, FALSE,
                     (void*)fp);
  ...
  /* =====================
     ...
     ===================== */
  DefgenericBinaryData(theEnv)->RestrictionCount = 0L;
  DoForAllConstructs(theEnv, BsaveMethods,
                     DefgenericData(theEnv)->DefgenericModuleIndex, FALSE,
                     (void*)fp);
  ...
  /* =================================
     ...
     ================================= */
  DefgenericBinaryData(theEnv)->TypeCount = 0L;
  DoForAllConstructs(theEnv, BsaveMethodRestrictions,
                     DefgenericData(theEnv)->DefgenericModuleIndex, FALSE,
                     (void*)fp);
  ...
  /* =============================================================
     ...
     ============================================================= */
  DoForAllConstructs(theEnv, BsaveRestrictionTypes,
                     DefgenericData(theEnv)->DefgenericModuleIndex, FALSE,
                     (void*)fp);
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void BsaveDefgenericHeader(void* theEnv,
                                  struct constructHeader* theDefgeneric,
                                  void* userBuffer) {
  DEFGENERIC* gfunc = (DEFGENERIC*)theDefgeneric;
  ...
  GenWrite((void*)&dummy_generic, (unsigned long)sizeof(BSAVE_GENERIC),
           (FILE*)userBuffer);
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void BsaveMethods(void* theEnv, struct constructHeader* theDefgeneric,
                         void* userBuffer) {
  DEFGENERIC* gfunc = (DEFGENERIC*)theDefgeneric;
  DEFMETHOD* meth;
...
/******************************************************
  ...
  NOTES        : None
 ******************************************************/
static void BsaveMethodRestrictions(void* theEnv,
                                    struct constructHeader* theDefgeneric,
                                    void* userBuffer) {
  DEFGENERIC* gfunc = (DEFGENERIC*)theDefgeneric;
  ...
  for (i = 0; i < gfunc->mcnt; i++) {
    for (j = 0; j < gfunc->methods[i].restrictionCount; j++) {
      ...
      GenWrite((void*)&dummy_restriction, sizeof(BSAVE_RESTRICTION),
               (FILE*)userBuffer);
    }
  }
}
...
/*************************************************************
  ...
  NOTES        : None
 *************************************************************/
static void BsaveRestrictionTypes(void* theEnv,
                                  struct constructHeader* theDefgeneric,
                                  void* userBuffer) {
  DEFGENERIC* gfunc = (DEFGENERIC*)theDefgeneric;
  long dummy_type;
 ...
 ***********************************************************************/
static void BloadStorageGenerics(void* theEnv) {
  size_t space;
  long counts[5];
  ...
  if (DefgenericBinaryData(theEnv)->ModuleCount != 0L) {
    ...
    DefgenericBinaryData(theEnv)->ModuleArray =
        (DEFGENERIC_MODULE*)genalloc(theEnv, space);
  } else
    return;
  if (DefgenericBinaryData(theEnv)->GenericCount != 0L) {
    ...
    DefgenericBinaryData(theEnv)->DefgenericArray =
        (DEFGENERIC*)genalloc(theEnv, space);
  } else
    return;
  if (DefgenericBinaryData(theEnv)->MethodCount != 0L) {
    ...
    DefgenericBinaryData(theEnv)->MethodArray =
        (DEFMETHOD*)genalloc(theEnv, space);
  } else
    return;
  if (DefgenericBinaryData(theEnv)->RestrictionCount != 0L) {
    ...
    DefgenericBinaryData(theEnv)->RestrictionArray =
        (RESTRICTION*)genalloc(theEnv, space);
  } else
    return;
 ...
 ********************************************************************/
static void BloadGenerics(void* theEnv) {
  size_t space;

  GenReadBinary(theEnv, (void*)&space, sizeof(size_t));
  if (DefgenericBinaryData(theEnv)->ModuleCount == 0L) return;
  BloadandRefresh(theEnv, DefgenericBinaryData(theEnv)->ModuleCount,
                  sizeof(BSAVE_DEFGENERIC_MODULE), UpdateGenericModule);
  if (DefgenericBinaryData(theEnv)->GenericCount == 0L) return;
  BloadandRefresh(theEnv, DefgenericBinaryData(theEnv)->GenericCount,
                  sizeof(BSAVE_GENERIC), UpdateGeneric);
  BloadandRefresh(theEnv, DefgenericBinaryData(theEnv)->MethodCount,
                  sizeof(BSAVE_METHOD), UpdateMethod);
  BloadandRefresh(theEnv, DefgenericBinaryData(theEnv)->RestrictionCount,
                  sizeof(BSAVE_RESTRICTION), UpdateRestriction);
  BloadandRefresh(theEnv, DefgenericBinaryData(theEnv)->TypeCount, sizeof(long),
                  UpdateType);
}
 ...
 *********************************************/
static void UpdateGenericModule(void* theEnv, void* buf, long obji) {
  BSAVE_DEFGENERIC_MODULE* bdptr;

  bdptr = (BSAVE_DEFGENERIC_MODULE*)buf;
  UpdateDefmoduleItemHeader(
      theEnv, &bdptr->header,
      &DefgenericBinaryData(theEnv)->ModuleArray[obji].header,
      (int)sizeof(DEFGENERIC),
      (void*)DefgenericBinaryData(theEnv)->DefgenericArray);
}

static void UpdateGeneric(void* theEnv, void* buf, long obji) {
  BSAVE_GENERIC* bgp;
  DEFGENERIC* gp;
  ...
  UpdateConstructHeader(
      theEnv, &bgp->header, &gp->header, (int)sizeof(DEFGENERIC_MODULE),
      (void*)DefgenericBinaryData(theEnv)->ModuleArray, (int)sizeof(DEFGENERIC),
      (void*)DefgenericBinaryData(theEnv)->DefgenericArray);
  DefgenericBinaryData(theEnv)->DefgenericArray[obji].busy = 0;
#if DEBUGGING_FUNCTIONS
  DefgenericBinaryData(theEnv)->DefgenericArray[obji].trace =
      DefgenericData(theEnv)->WatchGenerics;
#endif
...
static void UpdateMethod(void* theEnv, void* buf, long obji) {
  BSAVE_METHOD* bmth;
  ...
  DefgenericBinaryData(theEnv)->MethodArray[obji].trace =
      DefgenericData(theEnv)->WatchMethods;
#endif
...
static void UpdateRestriction(void* theEnv, void* buf, long obji) {
...
static void UpdateType(void* theEnv, void* buf, long obji) {
#if OBJECT_SYSTEM
  DefgenericBinaryData(theEnv)->TypeArray[obji] =
      (void*)DefclassPointer(*(long*)buf);
#else
  if ((*(long*)buf) > (long)INSTANCE_TYPE_CODE) {
    ...
    DefgenericBinaryData(theEnv)->TypeArray[obji] =
        (void*)EnvAddLong(theEnv, (long long)OBJECT_TYPE_CODE);
  } else
    DefgenericBinaryData(theEnv)->TypeArray[obji] =
        (void*)EnvAddLong(theEnv, *(long*)buf);
  IncrementIntegerCount(
      (INTEGER_HN*)DefgenericBinaryData(theEnv)->TypeArray[obji]);
#endif
}
 ...
 ***************************************************************/
static void ClearBloadGenerics(void* theEnv) {
  long i;
  size_t space;
  ...
  for (i = 0; i < DefgenericBinaryData(theEnv)->GenericCount; i++)
    UnmarkConstructHeader(
        theEnv, &DefgenericBinaryData(theEnv)->DefgenericArray[i].header);
  ...
  for (i = 0; i < DefgenericBinaryData(theEnv)->TypeCount; i++)
    DecrementIntegerCount(
        theEnv, (INTEGER_HN*)DefgenericBinaryData(theEnv)->TypeArray[i]);
#endif
  space = (sizeof(void*) * DefgenericBinaryData(theEnv)->TypeCount);
  if (space == 0L) return;
  genfree(theEnv, (void*)DefgenericBinaryData(theEnv)->TypeArray, space);
