...
static EXPRESSION* GenerateSlotComparisonTest(void*, int, int,
                                              struct lhsParseNode*,
                                              struct lhsParseNode*);
 ...
 **********************************************/
globle void ReplaceGetJNObjectValue(void* theEnv, EXPRESSION* theItem,
...
globle EXPRESSION* ObjectJNVariableComparison(
    ...
    struct lhsParseNode* referringNode, int isNand) {
  return (GenerateSlotComparisonTest(theEnv, TRUE, isNand, selfNode,
                                     referringNode));
}
...
globle EXPRESSION* GenObjectPNConstantCompare(void* theEnv,
  ...
  if (((theNode->withinMultifieldSlot == FALSE) ||
       (theNode->multiFieldsAfter == 0) || (theNode->multiFieldsBefore == 0)) &&
    ...
    theExp = GenConstant(theEnv, OBJ_PN_CONSTANT,
                         EnvAddBitMap(theEnv, (void*)&hack,
    ...
    theExp->argList->nextArg =
        GenConstant(theEnv, theNode->type, theNode->value);
  }
  return (theExp);
}

globle void ReplaceGetPNObjectValue(void* theEnv, EXPRESSION* theItem,
                                    struct lhsParseNode* theNode) {
...
globle EXPRESSION* ObjectPNVariableComparison(
  ...
  return (GenerateSlotComparisonTest(theEnv, FALSE, FALSE, selfNode,
                                     referringNode));
}
...
globle void GenObjectLengthTest(void* theEnv, struct lhsParseNode* theNode) {
  ...
  theNode->networkTest =
      CombineExpressions(theEnv, theTest, theNode->networkTest);
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void GenObjectGetVar(void* theEnv, int joinReference,
                            EXPRESSION* theItem, struct lhsParseNode* theNode,
                            int side) {
  ...
  if (theNode->slotNumber < 0) {
    ...
    theItem->value = EnvAddBitMap(theEnv, (void*)&hack1,
                                  (int)sizeof(struct ObjectMatchVar1));
    return;
  }
  ...
  /* ======================================
     ...
     ====================================== */
  if ((theNode->singleFieldsBefore == 0) && (theNode->singleFieldsAfter == 0) &&
      (theNode->multiFieldsBefore == 0) && (theNode->multiFieldsAfter == 0) &&
      ((theNode->withinMultifieldSlot == FALSE) ||
       (theNode->type == MF_VARIABLE) || (theNode->type == MF_WILDCARD))) {
    hack1.allFields = 1;
    hack1.whichSlot = (unsigned short)theNode->slotNumber;
    theItem->type = (unsigned short)(joinReference ? OBJ_GET_SLOT_JNVAR1
                                                   : OBJ_GET_SLOT_PNVAR1);
    theItem->value = EnvAddBitMap(theEnv, (void*)&hack1,
                                  (int)sizeof(struct ObjectMatchVar1));
    return;
  ...
  if (((theNode->type == SF_WILDCARD) || (theNode->type == SF_VARIABLE) ||
    ...
    theItem->type = (unsigned short)(joinReference ? OBJ_GET_SLOT_JNVAR2
                                                   : OBJ_GET_SLOT_PNVAR2);
    theItem->value =
        EnvAddBitMap(theEnv, (void*)&hack2, sizeof(struct ObjectMatchVar2));
    return;
  }
  ...
  theItem->type = (unsigned short)(joinReference ? OBJ_GET_SLOT_JNVAR1
                                                 : OBJ_GET_SLOT_PNVAR1);
  theItem->value =
      EnvAddBitMap(theEnv, (void*)&hack1, sizeof(struct ObjectMatchVar1));
}
...
static intBool IsSimpleSlotVariable(struct lhsParseNode* node) {
  if ((node->type == MF_WILDCARD) || (node->type == MF_VARIABLE))
    return (FALSE);
  if ((node->slotNumber < 0) || (node->slotNumber == ISA_ID) ||
      (node->slotNumber == NAME_ID))
    return (FALSE);
  if (node->withinMultifieldSlot == FALSE) return (TRUE);
  if (node->multifieldSlot == TRUE) return (FALSE);
...
static EXPRESSION* GenerateSlotComparisonTest(
    void* theEnv, int joinTest, int isNand, struct lhsParseNode* selfNode,
    struct lhsParseNode* referringNode) {
  EXPRESSION* theExp;
  ...
  if (IsSimpleSlotVariable(firstNode) && IsSimpleSlotVariable(referringNode)) {
    ...
    if ((firstNode->withinMultifieldSlot == FALSE) &&
      ...
      ClearBitString((void*)&phack1,
                     (int)sizeof(struct ObjectCmpPNSingleSlotVars1));
      ClearBitString((void*)&jhack1,
      ...
      phack1.firstSlot = jhack1.firstSlot =
          (unsigned short)firstNode->slotNumber;
      phack1.secondSlot = jhack1.secondSlot =
          (unsigned short)referringNode->slotNumber;
      if (joinTest) {
        if (isNand) {
        ...
        theExp = GenConstant(
            theEnv, OBJ_JN_CMP1,
            EnvAddBitMap(theEnv, (void*)&jhack1,
                         (int)sizeof(struct ObjectCmpJoinSingleSlotVars1)));
      } else
        theExp = GenConstant(
            theEnv, OBJ_PN_CMP1,
            EnvAddBitMap(theEnv, (void*)&phack1,
                         (int)sizeof(struct ObjectCmpPNSingleSlotVars1)));
    }
    /* ============================================
       ...
       ============================================ */
             ...
             (referringNode->withinMultifieldSlot == FALSE)) {
      ClearBitString((void*)&phack2,
                     (int)sizeof(struct ObjectCmpPNSingleSlotVars2));
      ClearBitString((void*)&jhack2,
      ...
      if (firstNode->withinMultifieldSlot == TRUE) {
        phack2.firstSlot = jhack2.firstSlot =
            (unsigned short)firstNode->slotNumber;
      ...
      if (joinTest)
        theExp = GenConstant(
            theEnv, OBJ_JN_CMP2,
            EnvAddBitMap(theEnv, (void*)&jhack2,
                         (int)sizeof(struct ObjectCmpJoinSingleSlotVars2)));
      else
        theExp = GenConstant(
            theEnv, OBJ_PN_CMP2,
            EnvAddBitMap(theEnv, (void*)&phack2,
                         (int)sizeof(struct ObjectCmpPNSingleSlotVars2)));
    }
    ...
    /* ===================================
      ...
      ClearBitString((void*)&phack3,
                     (int)sizeof(struct ObjectCmpPNSingleSlotVars3));
      ClearBitString((void*)&jhack3,
      ...
      phack3.firstSlot = jhack3.firstSlot =
          (unsigned short)firstNode->slotNumber;
      phack3.secondSlot = jhack3.secondSlot =
          (unsigned short)referringNode->slotNumber;
      ...
      if (joinTest) {
        ...
        theExp = GenConstant(
            theEnv, OBJ_JN_CMP3,
            EnvAddBitMap(theEnv, (void*)&jhack3,
                         (int)sizeof(struct ObjectCmpJoinSingleSlotVars3)));
      } else
        theExp = GenConstant(
            theEnv, OBJ_PN_CMP3,
            EnvAddBitMap(theEnv, (void*)&phack3,
                         (int)sizeof(struct ObjectCmpPNSingleSlotVars3)));
    }
  }
  ...
  /* ==================================================
    ...
    theExp = GenConstant(theEnv, FCALL,
