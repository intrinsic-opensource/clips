...
/*********************************************/
globle void DefmoduleBinarySetup(void* theEnv) {
  AddBeforeBloadFunction(theEnv, "defmodule", RemoveAllDefmodules, 2000);

#if BLOAD_AND_BSAVE
  AddBinaryItem(theEnv, "defmodule", 0, BsaveFind, NULL, BsaveStorage,
...
#if (BLOAD || BLOAD_ONLY)
  AddBinaryItem(theEnv, "defmodule", 0, NULL, NULL, NULL, NULL, BloadStorage,
...
globle void UpdateDefmoduleItemHeader(
    void* theEnv, struct bsaveDefmoduleItemHeader* theBsaveHeader,
    struct defmoduleItemHeader* theHeader, int itemSize, void* itemArray) {
...
static void BsaveFind(void* theEnv) {
  ...
  for (defmodulePtr = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    ...
    for (theList = defmodulePtr->importList; theList != NULL;
    ...
    for (theList = defmodulePtr->exportList; theList != NULL;
         theList = theList->next) {
      DefmoduleData(theEnv)->NumberOfPortItems++;
      if (theList->moduleName != NULL) {
        theList->moduleName->neededSymbol = TRUE;
...
static void BsaveBinaryItem(void* theEnv, FILE* fp) {
  ...
  space = DefmoduleData(theEnv)->BNumberOfDefmodules *
  ...
  while (defmodulePtr != NULL) {
    for (theList = defmodulePtr->importList; theList != NULL;
      ...
      DefmoduleData(theEnv)->NumberOfPortItems++;
      if (theList->moduleName == NULL)
        newPortItem.moduleName = -1L;
      else
        newPortItem.moduleName = (long)theList->moduleName->bucket;

      if (theList->constructType == NULL)
        newPortItem.constructType = -1L;
      else
        newPortItem.constructType = (long)theList->constructType->bucket;

      if (theList->constructName == NULL)
        newPortItem.constructName = -1L;
      else
        newPortItem.constructName = (long)theList->constructName->bucket;
...
static void BloadStorage(void* theEnv) {
  ...
  GenReadBinary(theEnv, &DefmoduleData(theEnv)->BNumberOfDefmodules,
                sizeof(long int));
  GenReadBinary(theEnv, &DefmoduleData(theEnv)->NumberOfPortItems,
                sizeof(long int));
  ...
  space =
      (DefmoduleData(theEnv)->BNumberOfDefmodules * sizeof(struct defmodule));
  DefmoduleData(theEnv)->DefmoduleArray =
      (struct defmodule*)genalloc(theEnv, space);
  ...
  DefmoduleData(theEnv)->PortItemArray =
      (struct portItem*)genalloc(theEnv, space);
}
...
static void BloadBinaryItem(void* theEnv) {
  ...
  BloadandRefresh(theEnv, DefmoduleData(theEnv)->BNumberOfDefmodules,
                  sizeof(struct bsaveDefmodule), UpdateDefmodule);
  BloadandRefresh(theEnv, DefmoduleData(theEnv)->NumberOfPortItems,
...
/******************************************/
static void UpdateDefmodule(void* theEnv, void* buf, long obji) {
  ...
  if (bdp->next != -1L) {
    DefmoduleData(theEnv)->DefmoduleArray[obji].next =
        (struct defmodule*)&DefmoduleData(theEnv)->DefmoduleArray[bdp->next];
  } else {
  ...
  if (GetNumberOfModuleItems(theEnv) == 0) {
    ...
    DefmoduleData(theEnv)->DefmoduleArray[obji].itemsArray =
        (struct defmoduleItemHeader**)gm2(
            theEnv, sizeof(void*) * GetNumberOfModuleItems(theEnv));
  }
  ...
  for (i = 0, theItem = GetListOfModuleItems(theEnv);
    ...
    if (theItem->bloadModuleReference == NULL) {
      ...
      DefmoduleData(theEnv)->DefmoduleArray[obji].itemsArray[i] =
          (struct defmoduleItemHeader*)(*theItem->bloadModuleReference)(theEnv,
                                                                        obji);
    }
  }
...
/*****************************************/
static void UpdatePortItem(void* theEnv, void* buf, long obji) {
...
static void ClearBload(void* theEnv) {
  ...
  for (i = 0; i < DefmoduleData(theEnv)->BNumberOfDefmodules; i++) {
    ...
    for (theList = DefmoduleData(theEnv)->DefmoduleArray[i].importList;
      ...
      if (theList->moduleName != NULL)
        DecrementSymbolCount(theEnv, theList->moduleName);
      if (theList->constructType != NULL)
        DecrementSymbolCount(theEnv, theList->constructType);
      if (theList->constructName != NULL)
        DecrementSymbolCount(theEnv, theList->constructName);
    }
  ...
  space = DefmoduleData(theEnv)->NumberOfPortItems * sizeof(struct portItem);
  if (space != 0)
    genfree(theEnv, (void*)DefmoduleData(theEnv)->PortItemArray, space);
  DefmoduleData(theEnv)->NumberOfPortItems = 0;
