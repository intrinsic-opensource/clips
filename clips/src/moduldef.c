...
globle void AllocateDefmoduleGlobals(void* theEnv) {
  AllocateEnvironmentData(theEnv, DEFMODULE_DATA, sizeof(struct defmoduleData),
                          NULL);
  AddEnvironmentCleanupFunction(theEnv, "defmodules", DeallocateDefmoduleData,
                                -1000);
...
/****************************************************/
static void DeallocateDefmoduleData(void* theEnv) {
...
globle void InitializeDefmodules(void* theEnv) {
  ...
  AddConstruct(theEnv, "defmodule", "defmodules", ParseDefmodule, NULL, NULL,
               NULL, NULL, NULL, NULL, NULL, NULL, NULL);
#endif

#if (!RUN_TIME) && DEFMODULE_CONSTRUCT
  EnvDefineFunction2(theEnv, "get-current-module", 'w',
                     PTIEF GetCurrentModuleCommand, "GetCurrentModuleCommand",
                     "00");

  EnvDefineFunction2(theEnv, "set-current-module", 'w',
                     PTIEF SetCurrentModuleCommand, "SetCurrentModuleCommand",
                     "11w");
#endif
}
...
globle int RegisterModuleItem(
    void* theEnv, const char* theItem, void* (*allocateFunction)(void*),
...
/********************************************************/
globle struct moduleItem* FindModuleItem(void* theEnv, const char* theName) {
...
/**************************************************************/
globle void* EnvSetCurrentModule(void* theEnv, void* vNewValue) {
...
/*   is returned.                                            */
/*************************************************************/
globle void* GetModuleItem(void* theEnv, struct defmodule* theModule,
                           int moduleItemIndex) {
  if (theModule == NULL) {
...
/*   is returned.                                           */
/************************************************************/
globle void SetModuleItem(void* theEnv, struct defmodule* theModule,
                          int moduleItemIndex, void* newValue) {
  if (theModule == NULL) {
...
globle void SetListOfDefmodules(void* theEnv, void* defmodulePtr) {
  ...
  while (DefmoduleData(theEnv)->LastDefmodule->next != NULL)
    DefmoduleData(theEnv)->LastDefmodule =
        DefmoduleData(theEnv)->LastDefmodule->next;
}
...
/*****************************************/
globle const char* EnvGetDefmoduleName(void* theEnv, void* defmodulePtr) {
#if MAC_XCD
...
/***************************************************/
globle const char* EnvGetDefmodulePPForm(void* theEnv, void* defmodulePtr) {
#if MAC_XCD
#pragma unused(theEnv)
#endif
...
/************************************************************/
static void ReturnDefmodule(void* theEnv, struct defmodule* theDefmodule,
  ...
  if (theDefmodule->itemsArray != NULL) {
    ...
    rm(theEnv, theDefmodule->itemsArray,
  ...
  while (theSpec != NULL) {
    ...
    if (!environmentClear) {
      if (theSpec->moduleName != NULL)
        DecrementSymbolCount(theEnv, theSpec->moduleName);
      if (theSpec->constructType != NULL)
        DecrementSymbolCount(theEnv, theSpec->constructType);
      if (theSpec->constructName != NULL)
        DecrementSymbolCount(theEnv, theSpec->constructName);
    }
...
/**********************************************************************/
globle void* EnvFindDefmodule(void* theEnv, const char* defmoduleName) {
  struct defmodule* defmodulePtr;
  SYMBOL_HN* findValue;

  if ((findValue = (SYMBOL_HN*)FindSymbolHN(theEnv, defmoduleName)) == NULL)
    return (NULL);
...
globle void* SetCurrentModuleCommand(void* theEnv) {
  ...
  defaultReturn = (SYMBOL_HN*)EnvAddSymbol(
      theEnv,
      ValueToString(((struct defmodule*)EnvGetCurrentModule(theEnv))->name));

  if (EnvArgCountCheck(theEnv, "set-current-module", EXACTLY, 1) == -1) {
    return (defaultReturn);
  }

  if (EnvArgTypeCheck(theEnv, "set-current-module", 1, SYMBOL, &argPtr) ==
      FALSE) {
    return (defaultReturn);
  }
...
/*************************************************/
globle void AddAfterModuleChangeFunction(void* theEnv, const char* name,
                                         void (*func)(void*), int priority) {
  DefmoduleData(theEnv)->AfterModuleChangeFunctions = AddFunctionToCallList(
      theEnv, name, priority, func,
      DefmoduleData(theEnv)->AfterModuleChangeFunctions, TRUE);
}
...
globle void* FindDefmodule(const char* defmoduleName) {
...
globle const char* GetDefmoduleName(void* defmodulePtr) {
  return EnvGetDefmoduleName(GetCurrentEnvironment(), defmodulePtr);
}

globle const char* GetDefmodulePPForm(void* defmodulePtr) {
  return EnvGetDefmodulePPForm(GetCurrentEnvironment(), defmodulePtr);
}

globle void* GetNextDefmodule(void* defmodulePtr) {
  return EnvGetNextDefmodule(GetCurrentEnvironment(), defmodulePtr);
}

globle void* SetCurrentModule(void* vNewValue) {
