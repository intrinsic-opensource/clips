...
globle void InitializeEvaluationData(void* theEnv) {
  struct externalAddressType cPointer = {"C",  PrintCAddress, PrintCAddress,
                                         NULL, NewCAddress,   NULL};

  AllocateEnvironmentData(theEnv, EVALUATION_DATA,
                          sizeof(struct evaluationData),
                          DeallocateEvaluationData);
...
/*****************************************************/
static void DeallocateEvaluationData(void* theEnv) {
  int i;

  for (i = 0; i < EvaluationData(theEnv)->numberOfAddressTypes; i++) {
    rtn_struct(theEnv, externalAddressType,
               EvaluationData(theEnv)->ExternalAddressTypes[i]);
  }
}
...
/**************************************************************/
globle int EvaluateExpression(void* theEnv, struct expr* problem,
  ...
  switch (problem->type) {
...
#if PROFILING_FUNCTIONS
      StartProfile(theEnv, &profileFrame, &fptr->usrData,
      ...
      switch (fptr->returnValueType) {
        ...
        case 'a':
          ...
          if (fptr->environmentAware) {
            ...
            returnValue->value = (*(void* (*)(void))fptr->functionPointer)();
          }
          break;
        case 'g':
          returnValue->type = INTEGER;
          if (fptr->environmentAware) {
            returnValue->value = (void*)EnvAddLong(
                theEnv, (*(long long (*)(void*))fptr->functionPointer)(theEnv));
          } else {
            returnValue->value = (void*)EnvAddLong(
                theEnv, (*(long long (*)(void))fptr->functionPointer)());
          }
          break;
        case 'i':
          returnValue->type = INTEGER;
          if (fptr->environmentAware) {
            returnValue->value = (void*)EnvAddLong(
                theEnv,
                (long long)(*(int (*)(void*))fptr->functionPointer)(theEnv));
          } else {
            returnValue->value = (void*)EnvAddLong(
                theEnv, (long long)(*(int (*)(void))fptr->functionPointer)());
          }
          break;
        case 'l':
          returnValue->type = INTEGER;
          if (fptr->environmentAware) {
            returnValue->value = (void*)EnvAddLong(
                theEnv, (long long)(*(long int (*)(
                            void*))fptr->functionPointer)(theEnv));
          } else {
            returnValue->value = (void*)EnvAddLong(
                theEnv,
                (long long)(*(long int (*)(void))fptr->functionPointer)());
          }
          break;
        case 'f':
          returnValue->type = FLOAT;
          if (fptr->environmentAware) {
            returnValue->value = (void*)EnvAddDouble(
                theEnv,
                (double)(*(float (*)(void*))fptr->functionPointer)(theEnv));
          } else {
            returnValue->value = (void*)EnvAddDouble(
                theEnv, (double)(*(float (*)(void))fptr->functionPointer)());
          }
          break;
        case 'd':
          returnValue->type = FLOAT;
          if (fptr->environmentAware) {
            returnValue->value = (void*)EnvAddDouble(
                theEnv, (*(double (*)(void*))fptr->functionPointer)(theEnv));
          } else {
            returnValue->value = (void*)EnvAddDouble(
                theEnv, (*(double (*)(void))fptr->functionPointer)());
          }
          break;
        case 's':
          returnValue->type = STRING;
          if (fptr->environmentAware) {
            returnValue->value = (void*)(*(SYMBOL_HN * (*)(void*))
                                              fptr->functionPointer)(theEnv);
          } else {
            returnValue->value =
                (void*)(*(SYMBOL_HN * (*)(void)) fptr->functionPointer)();
          }
          break;
        case 'w':
          returnValue->type = SYMBOL;
        ...
        case 'u':
          if (fptr->environmentAware) {
            (*(void (*)(void*, DATA_OBJECT_PTR))fptr->functionPointer)(
                theEnv, returnValue);
          } else {
    ...
    case MF_VARIABLE:
    case SF_VARIABLE:
      if (GetBoundVariable(theEnv, returnValue, (SYMBOL_HN*)problem->value) ==
          FALSE) {
    ...
    default:
      ...
      if (EvaluationData(theEnv)
              ->PrimitivesArray[problem->type]
              ->copyToEvaluate) {
        returnValue->type = problem->type;
        returnValue->value = problem->value;
        break;
...
/******************************************/
globle void InstallPrimitive(void* theEnv, struct entityRecord* thePrimitive,
                             int whichPosition) {
...
globle int InstallExternalAddressType(
  ...
  if (EvaluationData(theEnv)->numberOfAddressTypes ==
      MAXIMUM_EXTERNAL_ADDRESS_TYPES) {
    SystemError(theEnv, "EVALUATN", 6);
    EnvExitRouter(theEnv, EXIT_FAILURE);
  }

  copyEAT = (struct externalAddressType*)genalloc(
      theEnv, sizeof(struct externalAddressType));
  memcpy(copyEAT, theAddressType, sizeof(struct externalAddressType));
  EvaluationData(theEnv)
      ->ExternalAddressTypes[EvaluationData(theEnv)->numberOfAddressTypes++] =
      copyEAT;
...
/******************************************************/
globle void ReturnValues(void* theEnv, DATA_OBJECT_PTR garbagePtr,
  ...
  while (garbagePtr != NULL) {
    ...
    if ((garbagePtr->supplementalInfo != NULL) && decrementSupplementalInfo) {
      DecrementSymbolCount(
          theEnv, (struct symbolHashNode*)garbagePtr->supplementalInfo);
    }
...
/***************************************************/
globle void PrintDataObject(void* theEnv, const char* fileid,
  ...
  switch (argPtr->type) {
    ...
    default:
      if (EvaluationData(theEnv)->PrimitivesArray[argPtr->type] != NULL) {
        if (EvaluationData(theEnv)
                ->PrimitivesArray[argPtr->type]
                ->longPrintFunction) {
          (*EvaluationData(theEnv)
                ->PrimitivesArray[argPtr->type]
                ->longPrintFunction)(theEnv, fileid, argPtr->value);
          break;
        } else if (EvaluationData(theEnv)
                       ->PrimitivesArray[argPtr->type]
                       ->shortPrintFunction) {
          (*EvaluationData(theEnv)
                ->PrimitivesArray[argPtr->type]
                ->shortPrintFunction)(theEnv, fileid, argPtr->value);
          break;
        }
      }
...
globle void ValueInstall(void* theEnv, DATA_OBJECT* vPtr) {
  if (vPtr->type == MULTIFIELD)
    MultifieldInstall(theEnv, (struct multifield*)vPtr->value);
  else
    AtomInstall(theEnv, vPtr->type, vPtr->value);
}
...
globle void ValueDeinstall(void* theEnv, DATA_OBJECT* vPtr) {
  if (vPtr->type == MULTIFIELD)
    MultifieldDeinstall(theEnv, (struct multifield*)vPtr->value);
  else
    AtomDeinstall(theEnv, vPtr->type, vPtr->value);
}
...
globle void AtomInstall(void* theEnv, int type, void* vPtr) {
  switch (type) {
    ...
    default:
      ...
      if (EvaluationData(theEnv)->PrimitivesArray[type]->bitMap)
        IncrementBitMapCount(vPtr);
      else if (EvaluationData(theEnv)
                   ->PrimitivesArray[type]
                   ->incrementBusyCount) {
        (*EvaluationData(theEnv)->PrimitivesArray[type]->incrementBusyCount)(
            theEnv, vPtr);
      }
      break;
  }
...
globle void AtomDeinstall(void* theEnv, int type, void* vPtr) {
  switch (type) {
    ...
    default:
      if (EvaluationData(theEnv)->PrimitivesArray[type] == NULL) break;
      if (EvaluationData(theEnv)->PrimitivesArray[type]->bitMap)
        DecrementBitMapCount(theEnv, (BITMAP_HN*)vPtr);
      else if (EvaluationData(theEnv)
                   ->PrimitivesArray[type]
                   ->decrementBusyCount) {
        (*EvaluationData(theEnv)->PrimitivesArray[type]->decrementBusyCount)(
            theEnv, vPtr);
      }
  }
}
...
/********************************************/
globle int EnvFunctionCall(void* theEnv, const char* name, const char* args,
                           DATA_OBJECT* result) {
  ...
  EnvPrintRouter(theEnv, WERROR,
                 "No function, generic function or deffunction of name ");
...
/*   Allows only constants as arguments.    */
/********************************************/
globle int FunctionCall2(void* theEnv, FUNCTION_REFERENCE* theReference,
                         const char* args, DATA_OBJECT* result) {
  EXPRESSION* argexps;
  int error = FALSE;
  ...
  if (theReference->type == PCALL) {
    if (CheckDeffunctionCall(theEnv, theReference->value,
                             CountArguments(theReference->argList)) == FALSE) {
      PrintErrorID(theEnv, "MISCFUN", 4, FALSE);
      EnvPrintRouter(theEnv, WERROR,
                     "EnvFunctionCall called with the wrong number of "
                     "arguments for deffunction ");
      EnvPrintRouter(theEnv, WERROR,
                     EnvGetDeffunctionName(theEnv, theReference->value));
      EnvPrintRouter(theEnv, WERROR, "\n");
...
/***************************************************/
globle void CopyDataObject(void* theEnv, DATA_OBJECT* dst, DATA_OBJECT* src,
...
/****************************************/
unsigned long GetAtomicHashValue(unsigned short type, void* value,
                                 int position) {
...
/*   function.                                                    */
/******************************************************************/
globle intBool GetFunctionReference(void* theEnv, const char* name,
                                    FUNCTION_REFERENCE* theReference) {
#if DEFGENERIC_CONSTRUCT
  void* gfunc;
#endif
...
/***********************************************************
  ...
  NOTES        : None
 ***********************************************************/
globle int EvaluateAndStoreInDataObject(void* theEnv, int mfp,
                                        EXPRESSION* theExp, DATA_OBJECT* val,
                                        int garbageSegment) {
  val->type = MULTIFIELD;
  val->begin = 0;
  val->end = -1;
  ...
  if (theExp == NULL) {
    if (garbageSegment)
      ...
      val->value = CreateMultifield2(theEnv, 0L);
...
/* PrintCAddress:  */
/*******************************************************/
static void PrintCAddress(void* theEnv, const char* logicalName,
                          void* theValue) {
  char buffer[20];
...
/* NewCAddress: */
/****************/
static void NewCAddress(void* theEnv, DATA_OBJECT* rv) {
  ...
  if (numberOfArguments != 1) {
    ...
    EnvPrintRouter(theEnv, WERROR,
                   "Function new expected no additional arguments for the C "
                   "external language type.\n");
    SetEvaluationError(theEnv, TRUE);
    return;
  }
...
globle void SetMultifieldErrorValue(DATA_OBJECT_PTR returnValue) {
  EnvSetMultifieldErrorValue(GetCurrentEnvironment(), returnValue);
}

globle int FunctionCall(const char* name, const char* args,
                        DATA_OBJECT* result) {
