...
static intBool GetVariableDefinition(void*, const char*, int*, int,
                                     struct token*);
...
/*********************************************************************/
globle intBool ParseDefglobal(void* theEnv, const char* readSource) {
  ...
  if (theToken.type == SYMBOL) {
    ...
    theModule = (struct defmodule*)EnvFindDefmodule(
        theEnv, ValueToString(theToken.value));
    if (theModule == NULL) {
  ...
  /*===========================================*/
    ...
    SavePPBuffer(theEnv,
                 EnvGetDefmoduleName(
                     theEnv, ((struct defmodule*)EnvGetCurrentModule(theEnv))));
    SavePPBuffer(theEnv, " ");
  ...
  while (GetVariableDefinition(theEnv, readSource, &defglobalError, tokenRead,
                               &theToken)) {
    tokenRead = FALSE;
...
/***************************************************************/
static intBool GetVariableDefinition(void* theEnv, const char* readSource,
                                     int* defglobalError, int tokenRead,
                                     struct token* theToken) {
  ...
  if ((EnvGetWatchItem(theEnv, "compilations") == ON) &&
    ...
    if (QFindDefglobal(theEnv, variableName) != NULL) {
      ...
      EnvPrintRouter(theEnv, outRouter, "Defining defglobal: ");
  ...
  if (FindImportExportConflict(theEnv, "defglobal",
                               ((struct defmodule*)EnvGetCurrentModule(theEnv)),
                               ValueToString(variableName))) {
    ImportExportConflictMessage(theEnv, "defglobal",
                                ValueToString(variableName), NULL, NULL);
    *defglobalError = TRUE;
    return (FALSE);
  }
#endif
...
/*********************************************************/
static void AddDefglobal(void* theEnv, SYMBOL_HN* name, DATA_OBJECT_PTR vPtr,
                         struct expr* ePtr) {
  ...
  if (vPtr->type != MULTIFIELD)
    ...
    DuplicateMultifield(theEnv, &defglobalPtr->current, vPtr);
...
#if DEBUGGING_FUNCTIONS
  defglobalPtr->watch =
      GlobalHadWatch ? TRUE : DefglobalData(theEnv)->WatchGlobals;
#endif
  ...
  defglobalPtr->header.whichModule = (struct defmoduleItemHeader*)GetModuleItem(
...
globle intBool ReplaceGlobalVariable(void* theEnv, struct expr* ePtr) {
  ...
  theGlobal = (struct defglobal*)FindImportedConstruct(
      theEnv, "defglobal", NULL, ValueToString(ePtr->value), &count, TRUE,
      NULL);
...
/*****************************************************************/
globle void GlobalReferenceErrorMessage(void* theEnv,
