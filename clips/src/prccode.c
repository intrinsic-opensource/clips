...
static void EvaluateProcParameters(void*, EXPRESSION*, int, const char*,
                                   const char*);
...
globle void InstallProcedurePrimitives(void* theEnv) {
  ENTITY_RECORD procParameterInfo =
                    {"PROC_PARAM", PROC_PARAM,   0,    1,    0,    NULL, NULL,
                     NULL,         RtnProcParam, NULL, NULL, NULL, NULL, NULL,
                     NULL,         NULL,         NULL},
                procWildInfo = {"PROC_WILD_PARAM",
                                PROC_WILD_PARAM,
                                0,
                                1,
                                0,
                ...
                procGetInfo = {"PROC_GET_BIND",
                               ...
                               NULL,
                               NULL},
                procBindInfo = {"PROC_BIND", PROC_BIND, 0,    1,           0,
                                NULL,        NULL,      NULL, PutProcBind, NULL,
                                NULL,        NULL,      NULL, NULL,        NULL,
                                NULL,        NULL};
#if !DEFFUNCTION_CONSTRUCT
  ENTITY_RECORD deffunctionEntityRecord = {
      "PCALL", PCALL, 0,    0,    1,    NULL, NULL, NULL, EvaluateBadCall,
      NULL,    NULL,  NULL, NULL, NULL, NULL, NULL, NULL};
#endif
#if !DEFGENERIC_CONSTRUCT
  ENTITY_RECORD genericEntityRecord = {
      "GCALL", GCALL, 0,    0,    1,    NULL, NULL, NULL, EvaluateBadCall,
      NULL,    NULL,  NULL, NULL, NULL, NULL, NULL, NULL};
#endif

  AllocateEnvironmentData(theEnv, PROCEDURAL_PRIMITIVE_DATA,
  ...
  InstallPrimitive(theEnv, &ProceduralPrimitiveData(theEnv)->ProcParameterInfo,
                   PROC_PARAM);
  InstallPrimitive(theEnv, &ProceduralPrimitiveData(theEnv)->ProcWildInfo,
                   PROC_WILD_PARAM);
  InstallPrimitive(theEnv, &ProceduralPrimitiveData(theEnv)->ProcGetInfo,
                   PROC_GET_BIND);
  InstallPrimitive(theEnv, &ProceduralPrimitiveData(theEnv)->ProcBindInfo,
                   PROC_BIND);
  ...
  InstallPrimitive(
      theEnv, &ProceduralPrimitiveData(theEnv)->DeffunctionEntityRecord, PCALL);
#endif
  ...
  InstallPrimitive(
      theEnv, &ProceduralPrimitiveData(theEnv)->GenericEntityRecord, GCALL);
#endif
  ...
  MultifieldInstall(
      theEnv, (MULTIFIELD_PTR)ProceduralPrimitiveData(theEnv)->NoParamValue);
}
...
/**************************************************************/
static void DeallocateProceduralPrimitiveData(void* theEnv) {
  ReturnMultifield(
      theEnv,
      (struct multifield*)ProceduralPrimitiveData(theEnv)->NoParamValue);
...
/************************************************************
  ...
  NOTES        : None
 ************************************************************/
globle EXPRESSION* ParseProcParameters(void* theEnv, const char* readSource,
                                       struct token* tkn,
                                       EXPRESSION* parameterList,
                                       SYMBOL_HN** wildcard, int* min, int* max,
                                       int* error,
  ...
  *error = TRUE;
  ...
  while ((tkn->type == SF_VARIABLE) || (tkn->type == MF_VARIABLE)) {
    ...
    if ((checkfunc != NULL) ? (*checkfunc)(theEnv, ValueToString(tkn->value))
                            : FALSE) {
      ReturnExpression(theEnv, parameterList);
      return (NULL);
    }
...
globle EXPRESSION* ParseProcActions(
    void* theEnv, const char* bodytype, const char* readSource,
    struct token* tkn, EXPRESSION* params, SYMBOL_HN* wildcard,
    int (*altvarfunc)(void*, EXPRESSION*, void*),
    int (*altbindfunc)(void*, EXPRESSION*, void*), int* lvarcnt,
    void* userBuffer) {
  ...
  *lvarcnt = CountParsedBindNames(theEnv);
  if (ReplaceProcVars(theEnv, bodytype, actions, params, wildcard, altvarfunc,
                      userBuffer)) {
    ClearParsedBindNames(theEnv);
    ReturnExpression(theEnv, actions);
    return (NULL);
 ...
 *************************************************************************/
globle int ReplaceProcVars(void* theEnv, const char* bodytype,
                           EXPRESSION* actions, EXPRESSION* parameterList,
                           SYMBOL_HN* wildcard,
                           int (*altvarfunc)(void*, EXPRESSION*, void*),
                           void* specdata) {
  ...
  while (actions != NULL) {
    if (actions->type == SF_VARIABLE) {
      ...
      if ((position == 0) && (boundPosn == 0)) {
        ...
        /*================================================================*/
        if ((altvarfunc != NULL)
                ? ((*altvarfunc)(theEnv, actions, specdata) != 1)
                : TRUE) {
      ...
      /*===================================================*/
        ...
        actions->type =
            (unsigned short)((bindName != wildcard) ? PROC_PARAM
                                                    : PROC_WILD_PARAM);
        actions->value =
            EnvAddBitMap(theEnv, (void*)&position, (int)sizeof(int));
      }
      ...
      /*=========================================================*/
        ...
        actions->value =
            EnvAddBitMap(theEnv, (void*)&pvar, (int)sizeof(PACKED_PROC_VAR));
      ...
      if (ReplaceGlobalVariable(theEnv, actions) == FALSE) return (-1);
    }
#endif
    if ((altvarfunc != NULL) ? ((*altvarfunc)(theEnv, actions, specdata) == -1)
                             : FALSE)
      return (TRUE);
    if (actions->argList != NULL) {
      ...
      if ((actions->value == (void*)FindFunction(theEnv, "bind")) &&
        ...
        boundPosn =
            SearchParsedBindNames(theEnv, (SYMBOL_HN*)actions->argList->value);
        actions->value =
            EnvAddBitMap(theEnv, (void*)&boundPosn, (int)sizeof(intBool));
...
globle EXPRESSION* GenProcWildcardReference(void* theEnv, int theIndex) {
  return (
      GenConstant(theEnv, PROC_WILD_PARAM,
                  EnvAddBitMap(theEnv, (void*)&theIndex, (int)sizeof(int))));
}
 ...
 *******************************************************************/
globle void PushProcParameters(void* theEnv, EXPRESSION* parameterList,
                               int numberOfParameters, const char* pname,
                               const char* bodytype,
  ...
  EvaluateProcParameters(theEnv, parameterList, numberOfParameters, pname,
                         bodytype);
...
globle void PopProcParameters(void* theEnv) {
  ...
  if (ProceduralPrimitiveData(theEnv)->ProcParamArray != NULL)
    rm(theEnv, (void*)ProceduralPrimitiveData(theEnv)->ProcParamArray,
       (sizeof(DATA_OBJECT) *
        ProceduralPrimitiveData(theEnv)->ProcParamArraySize));

#if DEFGENERIC_CONSTRUCT
  if (ProceduralPrimitiveData(theEnv)->ProcParamExpressions != NULL)
    rm(theEnv, (void*)ProceduralPrimitiveData(theEnv)->ProcParamExpressions,
       (sizeof(EXPRESSION) *
        ProceduralPrimitiveData(theEnv)->ProcParamArraySize));
#endif
  ...
  if (ProceduralPrimitiveData(theEnv)->WildcardValue != NULL) {
    MultifieldDeinstall(
        theEnv,
        (MULTIFIELD_PTR)ProceduralPrimitiveData(theEnv)->WildcardValue->value);
    if (ProceduralPrimitiveData(theEnv)->WildcardValue->value !=
        ProceduralPrimitiveData(theEnv)->NoParamValue)
      AddToMultifieldList(theEnv,
                          (MULTIFIELD_PTR)ProceduralPrimitiveData(theEnv)
                              ->WildcardValue->value);
...
/******************************************************************
  ...
  NOTES        : Assumes pstack != NULL
 ******************************************************************/
static void ReleaseProcParameters(void* theEnv) {
  ...
  if (ProceduralPrimitiveData(theEnv)->WildcardValue != NULL) {
    if (ProceduralPrimitiveData(theEnv)->WildcardValue->value !=
      ...
      ReturnMultifield(theEnv,
                       (struct multifield*)ProceduralPrimitiveData(theEnv)
                           ->WildcardValue->value);
    }
  ...
  while (ptmp != NULL) {
    ...
    if (ptmp->ParamArray != NULL) {
      rm(theEnv, (void*)ptmp->ParamArray,
         (sizeof(DATA_OBJECT) * ptmp->ParamArraySize));
    }

#if DEFGENERIC_CONSTRUCT
    if (ptmp->ParamExpressions != NULL) {
      rm(theEnv, (void*)ptmp->ParamExpressions,
    ...
    if (ptmp->WildcardValue != NULL) {
      if (ptmp->WildcardValue->value !=
          ProceduralPrimitiveData(theEnv)->NoParamValue) {
        ReturnMultifield(theEnv,
                         (struct multifield*)ptmp->WildcardValue->value);
      }
...
globle EXPRESSION* GetProcParamExpressions(void* theEnv) {
  ...
  ProceduralPrimitiveData(theEnv)->ProcParamExpressions = (EXPRESSION*)gm2(
      theEnv, (sizeof(EXPRESSION) *
               ProceduralPrimitiveData(theEnv)->ProcParamArraySize));
  for (i = 0; i < ProceduralPrimitiveData(theEnv)->ProcParamArraySize; i++) {
    ProceduralPrimitiveData(theEnv)->ProcParamExpressions[i].type =
        ProceduralPrimitiveData(theEnv)->ProcParamArray[i].type;
    if (ProceduralPrimitiveData(theEnv)->ProcParamArray[i].type != MULTIFIELD)
      ...
      ProceduralPrimitiveData(theEnv)->ProcParamExpressions[i].value =
          (void*)&ProceduralPrimitiveData(theEnv)->ProcParamArray[i];
    ...
    ProceduralPrimitiveData(theEnv)->ProcParamExpressions[i].nextArg =
        ((i + 1) != ProceduralPrimitiveData(theEnv)->ProcParamArraySize)
            ? &ProceduralPrimitiveData(theEnv)->ProcParamExpressions[i + 1]
            : NULL;
  }
  return (ProceduralPrimitiveData(theEnv)->ProcParamExpressions);
}
...
/***********************************************************
  ...
  NOTES        : None
 ***********************************************************/
globle void EvaluateProcActions(void* theEnv, struct defmodule* theModule,
                                EXPRESSION* actions, int lvarcnt,
                                DATA_OBJECT* result, void (*crtproc)(void*)) {
  ...
  if (lvarcnt != 0) {
    theTM =
        AddTrackedMemory(theEnv, ProceduralPrimitiveData(theEnv)->LocalVarArray,
                         sizeof(DATA_OBJECT) * lvarcnt);
  } else {
    theTM = NULL;
  }
  ...
  if (oldModule != theModule) EnvSetCurrentModule(theEnv, (void*)theModule);
  ...
  if (lvarcnt != 0) {
    ...
    for (i = 0; i < lvarcnt; i++)
      if (ProceduralPrimitiveData(theEnv)->LocalVarArray[i].supplementalInfo ==
          EnvTrueSymbol(theEnv))
        ValueDeinstall(theEnv,
                       &ProceduralPrimitiveData(theEnv)->LocalVarArray[i]);
    rm(theEnv, (void*)ProceduralPrimitiveData(theEnv)->LocalVarArray,
       (sizeof(DATA_OBJECT) * lvarcnt));
  }
...
/****************************************************
  ...
  NOTES        : None
 ****************************************************/
globle void PrintProcParamArray(void* theEnv, const char* logName) {
  int i;

  EnvPrintRouter(theEnv, logName, " (");
  for (i = 0; i < ProceduralPrimitiveData(theEnv)->ProcParamArraySize; i++) {
    PrintDataObject(theEnv, logName,
...
globle void GrabProcWildargs(void* theEnv, DATA_OBJECT* result, int theIndex) {
  ...
  for (i = theIndex - 1;
    ...
    if (ProceduralPrimitiveData(theEnv)->ProcParamArray[i].type == MULTIFIELD)
      size += ProceduralPrimitiveData(theEnv)->ProcParamArray[i].end -
              ProceduralPrimitiveData(theEnv)->ProcParamArray[i].begin;
  }
  result->end = ProceduralPrimitiveData(theEnv)->WildcardValue->end = size - 1;
  result->value = ProceduralPrimitiveData(theEnv)->WildcardValue->value =
      (void*)CreateMultifield2(theEnv, (unsigned long)size);
  for (i = theIndex - 1, j = 1;
       i < ProceduralPrimitiveData(theEnv)->ProcParamArraySize; i++) {
    if (ProceduralPrimitiveData(theEnv)->ProcParamArray[i].type != MULTIFIELD) {
      SetMFType(result->value, j,
                (short)ProceduralPrimitiveData(theEnv)->ProcParamArray[i].type);
      SetMFValue(result->value, j,
                 ProceduralPrimitiveData(theEnv)->ProcParamArray[i].value);
      j++;
    } else {
...
/*******************************************************************
  ...
  NOTES        : EvaluationError set on errors
 *******************************************************************/
static void EvaluateProcParameters(void* theEnv, EXPRESSION* parameterList,
                                   int numberOfParameters, const char* pname,
                                   const char* bodytype) {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static intBool RtnProcParam(void* theEnv, void* value, DATA_OBJECT* result) {
  DATA_OBJECT* src;

  src = &ProceduralPrimitiveData(theEnv)
             ->ProcParamArray[*((int*)ValueToBitMap(value)) - 1];
...
/**************************************************************
  ...
  NOTES        : None
 **************************************************************/
static intBool GetProcBind(void* theEnv, void* value, DATA_OBJECT* result) {
  DATA_OBJECT* src;
  PACKED_PROC_VAR* pvar;
...
/**************************************************************
  ...
  NOTES        : None
 **************************************************************/
static intBool PutProcBind(void* theEnv, void* value, DATA_OBJECT* result) {
  DATA_OBJECT* dst;

  dst = &ProceduralPrimitiveData(theEnv)
             ->LocalVarArray[*((int*)ValueToBitMap(value)) - 1];
...
/****************************************************************
  ...
  NOTES        : Multi-field is NOT on list of ephemeral segments
 ****************************************************************/
static intBool RtnProcWild(void* theEnv, void* value, DATA_OBJECT* result) {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static int FindProcParameter(SYMBOL_HN* name, EXPRESSION* parameterList,
                             SYMBOL_HN* wildcard) {
  int i = 1;
...
/*************************************************************************
  ...
  NOTES        : Local variable binds are replaced in ReplaceProcVars
                 (after this routine has had a chance to replace all
                  ...
                  bind list)
 *************************************************************************/
static int ReplaceProcBinds(void* theEnv, EXPRESSION* actions,
  ...
  while (actions != NULL) {
    if (actions->argList != NULL) {
      ...
      if ((actions->value == (void*)FindFunction(theEnv, "bind")) &&
        ...
        if (bcode == -1) return (TRUE);
        if (bcode == 1) RemoveParsedBindName(theEnv, bname);
      }
    }
    actions = actions->nextArg;
  }
 ...
 *****************************************************/
static EXPRESSION* CompactActions(void* theEnv, EXPRESSION* actions) {
  struct expr* tmp;
 ...
 ******************************************************/
static intBool EvaluateBadCall(void* theEnv, void* value, DATA_OBJECT* result) {
#if MAC_XCD
#pragma unused(value)
#endif
