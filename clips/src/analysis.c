...
static int PropagateVariableToNodes(void*, struct lhsParseNode*, int,
                                    struct symbolHashNode*,
                                    struct lhsParseNode*, int, int, int);
static struct lhsParseNode* CheckExpression(void*, struct lhsParseNode*,
                                            struct lhsParseNode*, int,
                                            struct symbolHashNode*, int);
static void VariableReferenceErrorMessage(void*, struct symbolHashNode*,
                                          struct lhsParseNode*, int,
                                          struct symbolHashNode*, int);
static int ProcessField(void* theEnv, struct lhsParseNode*,
                        struct lhsParseNode*, struct lhsParseNode*, int,
                        struct nandFrame*);
static int ProcessVariable(void*, struct lhsParseNode*, struct lhsParseNode*,
                           struct lhsParseNode*, int, struct nandFrame*);
static void VariableMixingErrorMessage(void*, struct symbolHashNode*);
static int PropagateVariableDriver(void*, struct lhsParseNode*,
                                   struct lhsParseNode*, struct lhsParseNode*,
                                   int, struct symbolHashNode*,
                                   struct lhsParseNode*, int, int);
static int TestCEAnalysis(void*, struct lhsParseNode*, struct lhsParseNode*,
                          int, int*, struct nandFrame*);
...
globle int VariableAnalysis(void* theEnv, struct lhsParseNode* patternPtr) {
  ...
  while (patternPtr != NULL) {
    ...
    if (patternPtr->type == PATTERN_CE) {
      ...
      if ((patternPtr->value != NULL) && (patternPtr->referringNode != NULL)) {
        errorFlag = TRUE;
      ...
      if (TestCEAnalysis(theEnv, patternPtr, patternPtr->expression, FALSE,
                         &errorFlag, theNandFrames) == TRUE) {
        ReleaseNandFrames(theEnv, theNandFrames);
        return TRUE;
      }

      if (TestCEAnalysis(theEnv, patternPtr, patternPtr->secondaryExpression,
                         TRUE, &errorFlag, theNandFrames) == TRUE) {
        ReleaseNandFrames(theEnv, theNandFrames);
        return TRUE;
      }
    }
...
/******************************************************/
static void ReleaseNandFrames(void* theEnv, struct nandFrame* theFrames) {
...
/*******************************************************************/
static int TestCEAnalysis(void* theEnv, struct lhsParseNode* patternPtr,
                          struct lhsParseNode* theExpression, int secondary,
                          int* errorFlag, struct nandFrame* theNandFrames) {
  ...
  rv = CheckExpression(theEnv, theExpression, NULL, (int)patternPtr->whichCE,
                       NULL, 0);
  ...
  for (tempList = theList; tempList != NULL; tempList = tempList->right) {
    if (PropagateVariableDriver(theEnv, patternPtr, patternPtr, NULL,
                                SF_VARIABLE, (SYMBOL_HN*)tempList->value,
                                tempList, FALSE, TEST_CE)) {
  ...
  if (rv != NULL) {
    *errorFlag = TRUE;
    ...
    patternPtr->secondaryNetworkTest = CombineExpressions(
        theEnv, patternPtr->secondaryNetworkTest,
        GetvarReplace(theEnv, theExpression, FALSE, theNandFrames));
  } else {
    patternPtr->networkTest = CombineExpressions(
        theEnv, patternPtr->networkTest,
        GetvarReplace(theEnv, theExpression, FALSE, theNandFrames));
  }
...
/****************************************************************/
static int GetVariables(void* theEnv, struct lhsParseNode* thePattern,
                        int patternHeadType, struct nandFrame* theNandFrames) {
...
/******************************************************/
static int ProcessVariable(void* theEnv, struct lhsParseNode* thePattern,
  ...
  if (thePattern->type == PATTERN_CE) {
    ...
    if (thePattern->derivedConstraints)
      RemoveConstraint(theEnv, thePattern->constraints);
  ...
  if (thePattern->type != PATTERN_CE) {
    PropagateVariableToNodes(theEnv, thePattern->bottom, theType, theVariable,
                             thePattern, patternHead->beginNandDepth, TRUE,
                             FALSE);
  ...
  return (PropagateVariableDriver(theEnv, patternHead, thePattern,
                                  multifieldHeader, theType, theVariable,
                                  thePattern, TRUE, patternHeadType));
}
...
/*******************************************/
static int PropagateVariableDriver(void* theEnv,
                                   struct lhsParseNode* patternHead,
                                   ...
                                   int assignReference, int patternHeadType) {
  /*===================================================*/
  /* Propagate the variable location to any additional */
  ...
  if (multifieldHeader != NULL) {
    if (PropagateVariableToNodes(theEnv, multifieldHeader->right, theType,
                                 variableName, theReference,
  ...
  if (PropagateVariableToNodes(theEnv, theNode->right, theType, variableName,
                               theReference, patternHead->beginNandDepth,
                               assignReference, FALSE)) {
    VariableMixingErrorMessage(theEnv, variableName);
    return (TRUE);
  }
  ...
  if (((patternHead->type == PATTERN_CE) || (patternHead->type == TEST_CE)) &&
    ...
    if (PropagateVariableToNodes(theEnv, patternHead->bottom, theType,
                                 variableName, theReference,
                                 patternHead->beginNandDepth, assignReference,
...
/********************************************************/
static int ProcessField(void* theEnv, struct lhsParseNode* thePattern,
                        struct lhsParseNode* multifieldHeader,
                        struct lhsParseNode* patternHead, int patternHeadType,
                        struct nandFrame* theNandFrames) {
  ...
  for (tempList = theList; tempList != NULL; tempList = tempList->right) {
    if (PropagateVariableDriver(
            theEnv, patternHead, thePattern, multifieldHeader, tempList->type,
  ...
  if (UnboundVariablesInPattern(theEnv, thePattern,
  ...
  if (ProcessConnectedConstraints(theEnv, thePattern, multifieldHeader,
                                  patternHead)) {
    return (TRUE);
  }
...
/*************************************************************/
static int PropagateVariableToNodes(void* theEnv, struct lhsParseNode* theNode,
                                    int theType,
                                    struct symbolHashNode* variableName,
                                    struct lhsParseNode* theReference,
                                    int startDepth, int assignReference,
  ...
  while (theNode != NULL) {
    ...
    if (theNode->secondaryExpression != NULL) {
      PropagateVariableToNodes(theEnv, theNode->secondaryExpression, theType,
                               variableName, theReference, startDepth,
                               assignReference, TRUE);
    }
    ...
    /*======================================================*/
    ...
    else if (((theNode->type == SF_VARIABLE) ||
      ...
      if ((theReference->constraints != NULL) && (!theNode->negated)) {
        ...
        theNode->constraints = IntersectConstraints(
            theEnv, theReference->constraints, tempConstraints);
    ...
    if (theNode->right != NULL) {
      if (PropagateVariableToNodes(theEnv, theNode->right, theType,
                                   variableName, theReference, startDepth,
...
/*   binding occurrence).                                    */
/*************************************************************/
static intBool UnboundVariablesInPattern(void* theEnv,
  ...
  for (orField = theSlot->bottom; orField != NULL; orField = orField->bottom) {
    ...
    for (andField = orField; andField != NULL; andField = andField->right) {
      ...
      if (((andField->type == SF_VARIABLE) ||
           (andField->type == MF_VARIABLE)) &&
          (andField->referringNode == NULL)) {
        VariableReferenceErrorMessage(theEnv, (SYMBOL_HN*)andField->value, NULL,
                                      pattern, slotName, theField);
        return (TRUE);
      }
      ...
      /*==============================================*/
        ...
        rv = CheckExpression(theEnv, andField->expression, NULL, pattern,
                             slotName, theField);
      ...
      /*========================================================*/
        ...
        result = ConstraintCheckValue(theEnv, andField->type, andField->value,
                                      theConstraints);
        if (result != NO_VIOLATION) {
          ConstraintViolationErrorMessage(
              theEnv, "A literal restriction value", NULL, FALSE, pattern,
              slotName, theField, result, theConstraints, TRUE);
          return (TRUE);
        }
      }
    }
...
static struct lhsParseNode* CheckExpression(
    void* theEnv, struct lhsParseNode* exprPtr, struct lhsParseNode* lastOne,
    int whichCE, struct symbolHashNode* slotName, int theField) {
  struct lhsParseNode* rv;
  int i = 1;
  ...
  while (exprPtr != NULL) {
    ...
    if (exprPtr->type == SF_VARIABLE) {
      if (exprPtr->referringNode == NULL) {
        VariableReferenceErrorMessage(theEnv, (SYMBOL_HN*)exprPtr->value,
                                      lastOne, whichCE, slotName, theField);
        return (exprPtr);
      } else if ((UnmatchableConstraint(exprPtr->constraints)) &&
                 EnvGetStaticConstraintChecking(theEnv)) {
        ConstraintReferenceErrorMessage(theEnv, (SYMBOL_HN*)exprPtr->value,
                                        lastOne, i, whichCE, slotName,
                                        theField);
        return (exprPtr);
      }
    }
    ...
    else if ((exprPtr->type == MF_VARIABLE) &&
    ...
    /*=====================================================*/
      ...
      if (FindImportedConstruct(theEnv, "defglobal", NULL,
                                ValueToString(exprPtr->value), &count, TRUE,
                                NULL) == NULL) {
        VariableReferenceErrorMessage(theEnv, (SYMBOL_HN*)exprPtr->value,
                                      lastOne, whichCE, slotName, theField);
        return (exprPtr);
    ...
    /*============================================*/
             ...
             (exprPtr->bottom != NULL)) {
      if ((rv = CheckExpression(theEnv, exprPtr->bottom, exprPtr, whichCE,
...
/********************************************************/
static void VariableReferenceErrorMessage(void* theEnv,
                                          struct symbolHashNode* theVariable,
...
/************************************************************/
static void VariableMixingErrorMessage(void* theEnv,
                                       struct symbolHashNode* theVariable) {
