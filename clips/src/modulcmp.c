...
#define ItemPrefix() \
  ArbitraryPrefix(DefmoduleData(theEnv)->DefmoduleCodeItem, 0)
#define DefmodulePrefix() \
  ArbitraryPrefix(DefmoduleData(theEnv)->DefmoduleCodeItem, 1)
#define PortPrefix() \
  ArbitraryPrefix(DefmoduleData(theEnv)->DefmoduleCodeItem, 2)
...
static struct portItem* GetNextPortItem(void*, struct defmodule**,
                                        struct portItem**, int*, int*);
static int PortItemsToCode(void*, const char*, const char*, char*, int, FILE*,
                           int, int, int*);
...
/***************************************************************/
globle void DefmoduleCompilerSetup(void* theEnv) {
...
/***********************************************************/
static void BeforeDefmodulesToCode(void* theEnv) {
  int value = 0;
...
/*************************************************************/
globle void PrintDefmoduleReference(void* theEnv, FILE* theFile,
                                    struct defmodule* theModule) {
  if (theModule == NULL)
    fprintf(theFile, "NULL");
  else
    fprintf(
        theFile, "&%s%d_%ld[%ld]", DefmodulePrefix(),
        ConstructCompilerData(theEnv)->ImageID,
        (long)((theModule->bsaveID /
...
/************************************************/
static void InitDefmoduleCode(void* theEnv, FILE* initFP, int imageID,
                              int maxIndices) {
#if MAC_XCD
#pragma unused(maxIndices)
  ...
  fprintf(initFP,
          "   EnvSetCurrentModule(theEnv,(void *) "
          "EnvGetNextDefmodule(theEnv,NULL));\n");
}
...
static int ConstructToCode(void* theEnv, const char* fileName,
  ...
  if ((itemsFile = NewCFile(theEnv, fileName, pathName, fileNameBuffer, fileID,
                            1, FALSE)) == NULL) {
    return (FALSE);
  }
  ...
  for (theConstruct = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
       theConstruct != NULL;
       theConstruct =
           (struct defmodule*)EnvGetNextDefmodule(theEnv, theConstruct)) {
    /*===========================================*/
    ...
    moduleFile = OpenFileIfNeeded(
        ...
        &fileCount, moduleArrayVersion, headerFP, "struct defmodule",
        DefmodulePrefix(), FALSE, NULL);

    if (moduleFile == NULL) {
      moduleCount = maxIndices;
      CloseFileIfNeeded(theEnv, moduleFile, &moduleCount, &moduleArrayVersion,
    ...
    for (j = 0, theItem = GetListOfModuleItems(theEnv);
      ...
      if (theItem->constructsToCModuleReference == NULL) {
        ...
        (*theItem->constructsToCModuleReference)(
            theEnv, itemsFile, (int)theConstruct->bsaveID, imageID, maxIndices);
      }

      if ((j + 1) < GetNumberOfModuleItems(theEnv))
        fprintf(itemsFile, ",");
      else if (theConstruct->next != NULL)
        fprintf(itemsFile, ",\n");
    }
    ...
    if (theConstruct->importList == NULL) {
      ...
      fprintf(moduleFile, "&%s%d_%d[%d],", PortPrefix(), imageID,
              (portItemCount / maxIndices) + 1, portItemCount % maxIndices);
      for (portItemPtr = theConstruct->importList; portItemPtr != NULL;
    ...
    if (theConstruct->exportList == NULL) {
      ...
      fprintf(moduleFile, "&%s%d_%d[%d],", PortPrefix(), imageID,
              (portItemCount / maxIndices) + 1, portItemCount % maxIndices);
      for (portItemPtr = theConstruct->exportList; portItemPtr != NULL;
           portItemPtr = portItemPtr->next) {
        portItemCount++;
      }
    }
    ...
    if (theConstruct->next == NULL) {
      ...
      fprintf(moduleFile, "&%s%d_%d[%d]}",
              ConstructPrefix(DefmoduleData(theEnv)->DefmoduleCodeItem),
              imageID, (int)(theConstruct->next->bsaveID / maxIndices) + 1,
    ...
    moduleCount++;
    moduleFile = CloseFileIfNeeded(theEnv, moduleFile, &moduleCount,
...
/************************************************************/
static int PortItemsToCode(void* theEnv, const char* fileName,
                           const char* pathName, char* fileNameBuffer,
                           int fileID, FILE* headerFP, int imageID,
  ...
  for (thePortItem = GetNextPortItem(theEnv, &theDefmodule, &thePortItem,
                                     &importChecked, &exportChecked);
       thePortItem != NULL;
       thePortItem = GetNextPortItem(theEnv, &theDefmodule, &thePortItem,
                                     &importChecked, &exportChecked)) {
    /*===========================================*/
    /* Open a new file to write to if necessary. */
    /*===========================================*/

    portItemsFile = OpenFileIfNeeded(
        theEnv, portItemsFile, fileName, pathName, fileNameBuffer, fileID,
        imageID, fileCount, portItemArrayVersion, headerFP, "struct portItem",
        PortPrefix(), FALSE, NULL);
...
/*********************************************************************/
static struct portItem* GetNextPortItem(void* theEnv,
  ...
  while (*theDefmodule != NULL) {
    ...
    *theDefmodule =
        (struct defmodule*)EnvGetNextDefmodule(theEnv, *theDefmodule);
    *importChecked = FALSE;
    *exportChecked = FALSE;
  }
