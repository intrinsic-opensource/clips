...
/****************************************************/
globle intBool DeleteNamedConstruct(void* theEnv, const char* constructName,
...
globle void* FindNamedConstructInModuleOrImports(
    void* theEnv, const char* constructName, struct construct* constructClass) {
  void* theConstruct;
  int count;
  ...
  theConstruct =
      FindNamedConstructInModule(theEnv, constructName, constructClass);
  ...
  theConstruct = FindImportedConstruct(theEnv, constructClass->constructName,
                                       NULL, constructName, &count, TRUE, NULL);

  if (count > 1) {
    AmbiguousReferenceErrorMessage(theEnv, constructClass->constructName,
                                   constructName);
    return (NULL);
  }
...
/*   for searching for a specified construct.  */
/***********************************************/
globle void* FindNamedConstructInModule(void* theEnv, const char* constructName,
                                        struct construct* constructClass) {
  void* theConstruct;
  SYMBOL_HN* findValue;
  ...
  if ((constructName == NULL) ? TRUE
                              : ((findValue = (SYMBOL_HN*)FindSymbolHN(
                                      theEnv, constructName)) == NULL)) {
  ...
  for (theConstruct = (*constructClass->getNextItemFunction)(theEnv, NULL);
    ...
    if (findValue == (*constructClass->getConstructNameFunction)(
...
/*****************************************/
globle void UndefconstructCommand(void* theEnv, const char* command,
                                  struct construct* constructClass) {
  ...
  if (((*constructClass->findFunction)(theEnv, constructName) == NULL) &&
    ...
    CantFindItemErrorMessage(theEnv, constructClass->constructName,
                             constructName);
    return;
  }
...
/******************************************/
globle void PPConstructCommand(void* theEnv, const char* command,
                               struct construct* constructClass) {
  const char* constructName;
  char buffer[80];
...
/***********************************/
globle int PPConstruct(void* theEnv, const char* constructName,
  ...
  if ((*constructClass->getPPFormFunction)(
          theEnv, (struct constructHeader*)constructPtr) == NULL) {
    return (TRUE);
  }
  ...
  PrintInChunks(theEnv, logicalName,
                (*constructClass->getPPFormFunction)(
                    theEnv, (struct constructHeader*)constructPtr));
...
/*********************************************/
globle SYMBOL_HN* GetConstructModuleCommand(void* theEnv, const char* command,
                                            struct construct* constructClass) {
  const char* constructName;
  char buffer[80];
...
globle struct defmodule* GetConstructModule(void* theEnv,
  ...
  if ((position = FindModuleSeparator(constructName)) != FALSE) {
    ...
    if (theName != NULL) {
      return (
          (struct defmodule*)EnvFindDefmodule(theEnv, ValueToString(theName)));
    }
  }
  ...
  constructPtr = (struct constructHeader*)FindImportedConstruct(
      theEnv, constructClass->constructName, NULL, constructName, &count, TRUE,
      NULL);
...
/*************************************/
globle intBool Undefconstruct(void* theEnv, void* theConstruct,
                              struct construct* constructClass) {
#if BLOAD_ONLY || RUN_TIME
#if MAC_XCD
...
#pragma unused(theEnv)
  ...
  if (theConstruct == NULL) {
    ...
    while (currentConstruct != NULL) {
      ...
      if ((*constructClass->isConstructDeletableFunction)(theEnv,
                                                          currentConstruct)) {
        RemoveConstructFromModule(theEnv,
                                  (struct constructHeader*)currentConstruct);
        (*constructClass->freeFunction)(theEnv, currentConstruct);
      } else {
        CantDeleteItemErrorMessage(
            theEnv, constructClass->constructName,
            ValueToString((*constructClass->getConstructNameFunction)(
                (struct constructHeader*)currentConstruct)));
        success = FALSE;
      }
  ...
  if ((*constructClass->isConstructDeletableFunction)(theEnv, theConstruct) ==
      FALSE) {
    return (FALSE);
  }
...
/***********************************/
globle void SaveConstruct(void* theEnv, void* theModule,
                          const char* logicalName,
                          struct construct* constructClass) {
  const char* ppform;
  struct constructHeader* theConstruct;
  ...
  for (theConstruct =
           (struct constructHeader*)(*constructClass->getNextItemFunction)(
               theEnv, NULL);
       theConstruct != NULL;
       theConstruct =
           (struct constructHeader*)(*constructClass->getNextItemFunction)(
...
globle const char* GetConstructModuleName(
    struct constructHeader* theConstruct) {
  return (
      EnvGetDefmoduleName(NULL, (void*)theConstruct->whichModule->theModule));
}
...
globle const char* EnvGetConstructNameString(
    void* theEnv, struct constructHeader* theConstruct) {
#if MAC_XCD
#pragma unused(theEnv)
#endif
...
/************************************************/
globle void GetConstructListFunction(void* theEnv, const char* functionName,
  ...
  if ((numArgs = EnvArgCountCheck(theEnv, functionName, NO_MORE_THAN, 1)) ==
      -1) {
  ...
  if (numArgs == 1) {
    ...
    if ((theModule = (struct defmodule*)EnvFindDefmodule(
             theEnv, DOToString(result))) == NULL) {
...
/********************************************/
globle void GetConstructList(void* theEnv, DATA_OBJECT_PTR returnValue,
                             struct construct* constructClass,
                             struct defmodule* theModule) {
  void* theConstruct;
  ...
  while (loopModule != NULL) {
    ...
    while ((theConstruct = (*constructClass->getNextItemFunction)(
                theEnv, theConstruct)) != NULL) {
      /*================================*/
      ...
      tempSize =
          strlen(ValueToString((*constructClass->getConstructNameFunction)(
              (struct constructHeader*)theConstruct)));
    ...
    if (allModules)
      loopModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, loopModule);
    else
      loopModule = NULL;
  }
  ...
  while (loopModule != NULL) {
    ...
    while ((theConstruct = (*constructClass->getNextItemFunction)(
                theEnv, theConstruct)) != NULL) {
      theName = (*constructClass->getConstructNameFunction)(
          (struct constructHeader*)theConstruct);
      ...
      if (allModules) {
        ...
        SetMFValue(theList, count,
                   EnvAddSymbol(theEnv, ValueToString(theName)));
      }
      count++;
    }
...
/*   listing the constructs in a module. */
/*****************************************/
globle void ListConstruct(void* theEnv, struct construct* constructClass,
                          const char* logicalName,
                          struct defmodule* theModule) {
  void* constructPtr;
  ...
  while (theModule != NULL) {
    ...
    for (constructPtr = (*constructClass->getNextItemFunction)(theEnv, NULL);
      ...
      constructName = (*constructClass->getConstructNameFunction)(
          (struct constructHeader*)constructPtr);
...
/*************************************************/
globle const char* GetConstructPPForm(void* theEnv,
                                      struct constructHeader* theConstruct) {
#if MAC_XCD
#pragma unused(theEnv)
#endif
...
globle struct constructHeader* GetNextConstructItem(
    void* theEnv, struct constructHeader* theConstruct, int moduleIndex) {
  struct defmoduleItemHeader* theModuleItem;

  if (theConstruct == NULL) {
    theModuleItem =
        (struct defmoduleItemHeader*)GetModuleItem(theEnv, NULL, moduleIndex);
...
globle struct defmoduleItemHeader* GetConstructModuleItemByIndex(
    void* theEnv, struct defmodule* theModule, int moduleIndex) {
  if (theModule != NULL) {
    return ((struct defmoduleItemHeader*)GetModuleItem(theEnv, theModule,
                                                       moduleIndex));
  }

  return ((struct defmoduleItemHeader*)GetModuleItem(
...
globle long DoForAllConstructs(
    void* theEnv, void (*actionFunction)(void*, struct constructHeader*, void*),
    int moduleItemIndex, int interruptable, void* userBuffer) {
  ...
  for (theModule = EnvGetNextDefmodule(theEnv, NULL); theModule != NULL;
    ...
    for (theConstruct = theModuleItem->firstItem; theConstruct != NULL;
         theConstruct = next) {
      /*==========================================*/
...
/*****************************************************/
globle void InitializeConstructHeader(void* theEnv, const char* constructType,
                                      struct constructHeader* theConstruct,
  ...
  theItemHeader = (struct defmoduleItemHeader*)GetModuleItem(
      theEnv, NULL, theModuleItem->moduleIndex);
...
globle unsigned ConstructPrintWatchAccess(
    void* theEnv, struct construct* constructClass, const char* logName,
    EXPRESSION* argExprs, unsigned (*getWatchFunc)(void*, void*),
    void (*setWatchFunc)(void*, unsigned, void*)) {
  return (ConstructWatchSupport(theEnv, constructClass, "list-watch-items",
                                logName, argExprs, FALSE, FALSE, getWatchFunc,
                                setWatchFunc));
}
...
globle unsigned ConstructSetWatchAccess(
    void* theEnv, struct construct* constructClass, unsigned newState,
    EXPRESSION* argExprs, unsigned (*getWatchFunc)(void*, void*),
    void (*setWatchFunc)(void*, unsigned, void*)) {
  return (ConstructWatchSupport(theEnv, constructClass, "watch", WERROR,
                                argExprs, TRUE, newState, getWatchFunc,
                                setWatchFunc));
}
...
static unsigned ConstructWatchSupport(
    void* theEnv, struct construct* constructClass, const char* funcName,
    const char* logName, EXPRESSION* argExprs, intBool setFlag,
    unsigned newState, unsigned (*getWatchFunc)(void*, void*),
    void (*setWatchFunc)(void*, unsigned, void*)) {
  struct defmodule* theModule;
  void* theConstruct;
  ...
  if (argExprs == NULL) {
    ...
    for (theModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
         theModule != NULL; theModule = (struct defmodule*)EnvGetNextDefmodule(
                                theEnv, (void*)theModule)) {
      /*============================*/
      /* Set the current module to  */
      /* the module being examined. */
      /*============================*/
      ...
      for (theConstruct = (*constructClass->getNextItemFunction)(theEnv, NULL);
        ...
        if (setFlag) {
          ...
          EnvPrintRouter(theEnv, logName, "   ");
          ConstructPrintWatch(theEnv, logName, constructClass, theConstruct,
                              getWatchFunc);
        }
      }
    }
  ...
  while (argExprs != NULL) {
    ...
    if ((constructName.type != SYMBOL)
            ? TRUE
            : ((theConstruct = LookupConstruct(
                    theEnv, constructClass, DOToString(constructName), TRUE)) ==
               NULL)) {
      ExpectedTypeError1(theEnv, funcName, argIndex,
...
/*************************************************/
static void ConstructPrintWatch(void* theEnv, const char* logName,
                                struct construct* constructClass,
                                void* theConstruct,
...
/*****************************************************/
globle void* LookupConstruct(void* theEnv, struct construct* constructClass,
                             const char* constructName,
  ...
  theConstruct = FindImportedConstruct(theEnv, constructType, NULL,
                                       constructName, &moduleCount, TRUE, NULL);
