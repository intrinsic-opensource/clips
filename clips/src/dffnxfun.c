...
globle void SetupDeffunctions(void* theEnv) {
  ENTITY_RECORD deffunctionEntityRecord = {"PCALL",
                                           PCALL,
                                           0,
                                           0,
                                           1,
                                           ...
                                           PrintDeffunctionCall,
                                           NULL,
                                           EvaluateDeffunctionCall,
                                           NULL,
                                           DecrementDeffunctionBusyCount,
                                           IncrementDeffunctionBusyCount,
                                           NULL,
                                           NULL,
                                           NULL,
                                           NULL,
  ...
  AllocateEnvironmentData(theEnv, DEFFUNCTION_DATA,
  ...
  InstallPrimitive(theEnv, &DeffunctionData(theEnv)->DeffunctionEntityRecord,
                   PCALL);
  ...
  DeffunctionData(theEnv)->DeffunctionConstruct = AddConstruct(
      theEnv, "deffunction", "deffunctions",
#if (!BLOAD_ONLY) && (!RUN_TIME)
      ParseDeffunction,
#else
      NULL,
#endif
      EnvFindDeffunction, GetConstructNamePointer, GetConstructPPForm,
      GetConstructModuleItem, EnvGetNextDeffunction, SetNextConstruct,
  ...
  EnvDefineFunction2(theEnv, "undeffunction", 'v', PTIEF UndeffunctionCommand,
                     "UndeffunctionCommand", "11w");
#endif

#if DEBUGGING_FUNCTIONS
  EnvDefineFunction2(theEnv, "list-deffunctions", 'v',
                     PTIEF ListDeffunctionsCommand, "ListDeffunctionsCommand",
                     "01");
  EnvDefineFunction2(theEnv, "ppdeffunction", 'v', PTIEF PPDeffunctionCommand,
                     "PPDeffunctionCommand", "11w");
#endif

  EnvDefineFunction2(theEnv, "get-deffunction-list", 'm',
                     PTIEF GetDeffunctionListFunction,
                     "GetDeffunctionListFunction", "01");

  EnvDefineFunction2(theEnv, "deffunction-module", 'w',
                     PTIEF GetDeffunctionModuleCommand,
...
#if DEBUGGING_FUNCTIONS
  AddWatchItem(theEnv, "deffunctions", 0,
               &DeffunctionData(theEnv)->WatchDeffunctions, 32,
...
/******************************************************/
static void DeallocateDeffunctionData(void* theEnv) {
#if !RUN_TIME
...
/*****************************************************/
static void DestroyDeffunctionAction(void* theEnv,
...
#endif
#if (!BLOAD_ONLY) && (!RUN_TIME)
  struct deffunctionStruct* theDeffunction =
      (struct deffunctionStruct*)theConstruct;
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void* EnvFindDeffunction(void* theEnv, const char* dfnxModuleAndName) {
  return (FindNamedConstructInModuleOrImports(
      theEnv, dfnxModuleAndName,
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void* EnvFindDeffunctionInModule(void* theEnv,
...
globle DEFFUNCTION* LookupDeffunctionByMdlOrScope(void* theEnv,
  ...
  return ((DEFFUNCTION*)LookupConstruct(
      theEnv, DeffunctionData(theEnv)->DeffunctionConstruct, deffunctionName,
      TRUE));
}
...
#endif
  if (vptr == NULL) return (RemoveAllDeffunctions(theEnv));
  if (EnvIsDeffunctionDeletable(theEnv, vptr) == FALSE) return (FALSE);
...
globle void RemoveDeffunction(void* theEnv, void* vdptr) {
  ...
  DecrementSymbolCount(theEnv,
                       EnvGetDeffunctionNamePointer(theEnv, (void*)dptr));
...
globle void UndeffunctionCommand(void* theEnv) {
  UndefconstructCommand(theEnv, "undeffunction",
                        DeffunctionData(theEnv)->DeffunctionConstruct);
}
...
globle void* GetDeffunctionModuleCommand(void* theEnv) {
  return (
      GetConstructModuleCommand(theEnv, "deffunction-module",
                                DeffunctionData(theEnv)->DeffunctionConstruct));
}
...
globle void PPDeffunctionCommand(void* theEnv) {
  PPConstructCommand(theEnv, "ppdeffunction",
                     DeffunctionData(theEnv)->DeffunctionConstruct);
}
...
globle void ListDeffunctionsCommand(void* theEnv) {
  ListConstructCommand(theEnv, "list-deffunctions",
                       DeffunctionData(theEnv)->DeffunctionConstruct);
}
...
/***************************************************
  ...
  NOTES        : C Interface
 ***************************************************/
globle void EnvListDeffunctions(void* theEnv, const char* logicalName,
                                struct defmodule* theModule) {
  ListConstruct(theEnv, DeffunctionData(theEnv)->DeffunctionConstruct,
...
globle void GetDeffunctionListFunction(void* theEnv, DATA_OBJECT* returnValue) {
  GetConstructListFunction(theEnv, "get-deffunction-list", returnValue,
                           DeffunctionData(theEnv)->DeffunctionConstruct);
}
 ...
 ***************************************************************/
globle void EnvGetDeffunctionList(void* theEnv, DATA_OBJECT* returnValue,
                                  struct defmodule* theModule) {
  GetConstructList(theEnv, returnValue,
                   DeffunctionData(theEnv)->DeffunctionConstruct, theModule);
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void PrintDeffunctionCall(void* theEnv, const char* logName,
                                 void* value) {
#if DEVELOPER
...
/*******************************************************
  ...
  NOTES        : None
 *******************************************************/
static intBool EvaluateDeffunctionCall(void* theEnv, void* value,
                                       DATA_OBJECT* result) {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void DecrementDeffunctionBusyCount(void* theEnv, void* value) {
  /* ==============================================
     ...
     ============================================== */
  if (!ConstructData(theEnv)->ClearInProgress) ((DEFFUNCTION*)value)->busy--;
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void IncrementDeffunctionBusyCount(void* theEnv, void* value) {
#if MAC_XCD
#pragma unused(theEnv)
#endif
 ...
 ***************************************************/
static intBool ClearDeffunctionsReady(void* theEnv) {
  return ((DeffunctionData(theEnv)->ExecutingDeffunction != NULL) ? FALSE
                                                                  : TRUE);
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static intBool RemoveAllDeffunctions(void* theEnv) {
  ...
  while (dptr != NULL) {
    if (dptr->executing > 0) {
      DeffunctionDeleteError(theEnv,
                             EnvGetDeffunctionName(theEnv, (void*)dptr));
      success = FALSE;
    } else {
      oldbusy = dptr->busy;
      ExpressionDeinstall(theEnv, dptr->code);
    ...
    if (dtmp->executing == 0) {
      if (dtmp->busy > 0) {
        ...
        EnvPrintRouter(theEnv, WWARNING,
                       EnvGetDeffunctionName(theEnv, (void*)dtmp));
        EnvPrintRouter(
            theEnv, WWARNING,
...
/****************************************************
  ...
  NOTES        : None
 ****************************************************/
static void DeffunctionDeleteError(void* theEnv, const char* dfnxName) {
...
/***************************************************
  ...
  NOTES        : Used for deffunctions which are
                 ...
                 constructs
 ***************************************************/
static void SaveDeffunctionHeaders(void* theEnv, void* theModule,
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void SaveDeffunctionHeader(void* theEnv,
  ...
  if (EnvGetDeffunctionPPForm(theEnv, (void*)dfnxPtr) != NULL) {
    ...
    EnvPrintRouter(theEnv, logicalName,
                   EnvDeffunctionModule(theEnv, (void*)dfnxPtr));
    EnvPrintRouter(theEnv, logicalName, "::");
    EnvPrintRouter(theEnv, logicalName,
                   EnvGetDeffunctionName(theEnv, (void*)dfnxPtr));
    EnvPrintRouter(theEnv, logicalName, " (");
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void SaveDeffunctions(void* theEnv, void* theModule,
                             const char* logicalName) {
  SaveConstruct(theEnv, theModule, logicalName,
                DeffunctionData(theEnv)->DeffunctionConstruct);
}
 ...
 ******************************************************************/
static unsigned DeffunctionWatchAccess(void* theEnv, int code,
                                       unsigned newState,
                                       EXPRESSION* argExprs) {
#if MAC_XCD
#pragma unused(code)
  ...
  return (ConstructSetWatchAccess(
      theEnv, DeffunctionData(theEnv)->DeffunctionConstruct, newState, argExprs,
...
/***********************************************************************
  ...
  NOTES        : Accessory function for AddWatchItem()
 ***********************************************************************/
static unsigned DeffunctionWatchPrint(void* theEnv, const char* logName,
...
#pragma unused(code)
  ...
  return (ConstructPrintWatchAccess(
      theEnv, DeffunctionData(theEnv)->DeffunctionConstruct, logName, argExprs,
      EnvGetDeffunctionWatch, EnvSetDeffunctionWatch));
}
...
/*********************************************************
  ...
  NOTES        : None
 *********************************************************/
globle void EnvSetDeffunctionWatch(void* theEnv, unsigned newState,
                                   void* dptr) {
#if MAC_XCD
#pragma unused(theEnv)
#endif
...
globle const char* EnvDeffunctionModule(void* theEnv, void* theDeffunction) {
  return GetConstructModuleName((struct constructHeader*)theDeffunction);
}

globle const char* EnvGetDeffunctionName(void* theEnv, void* theDeffunction) {
  return GetConstructNameString((struct constructHeader*)theDeffunction);
}

globle const char* EnvGetDeffunctionPPForm(void* theEnv, void* theDeffunction) {
  return GetConstructPPForm(theEnv, (struct constructHeader*)theDeffunction);
}

globle SYMBOL_HN* EnvGetDeffunctionNamePointer(void* theEnv,
                                               void* theDeffunction) {
  return GetConstructNamePointer((struct constructHeader*)theDeffunction);
}

globle void EnvSetDeffunctionPPForm(void* theEnv, void* theDeffunction,
                                    const char* thePPForm) {
  SetConstructPPForm(theEnv, (struct constructHeader*)theDeffunction,
                     thePPForm);
}
...
globle void ListDeffunctions(const char* logicalName,
                             struct defmodule* theModule) {
  EnvListDeffunctions(GetCurrentEnvironment(), logicalName, theModule);
}

globle unsigned GetDeffunctionWatch(void* dptr) {
  return EnvGetDeffunctionWatch(GetCurrentEnvironment(), dptr);
}

globle void SetDeffunctionWatch(unsigned newState, void* dptr) {
