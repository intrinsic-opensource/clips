...
globle struct constraintRecord* GetConstraintRecord(void* theEnv) {
  ...
  constraints->minFields =
      GenConstant(theEnv, INTEGER, SymbolData(theEnv)->Zero);
...
globle struct constraintRecord* CopyConstraintRecord(
    void* theEnv, CONSTRAINT_RECORD* sourceConstraint) {
  ...
  theConstraint->classList =
      CopyExpression(theEnv, sourceConstraint->classList);
  theConstraint->restrictionList =
      CopyExpression(theEnv, sourceConstraint->restrictionList);
  ...
  theConstraint->minFields =
      CopyExpression(theEnv, sourceConstraint->minFields);
  theConstraint->maxFields =
      CopyExpression(theEnv, sourceConstraint->maxFields);
  theConstraint->bucket = -1;
  theConstraint->count = 0;
  theConstraint->multifield =
      CopyConstraintRecord(theEnv, sourceConstraint->multifield);
...
/*************************************************************/
globle int CompareNumbers(void* theEnv, int type1, void* vptr1, int type2,
                          void* vptr2) {
  /*============================================*/
...
globle CONSTRAINT_RECORD* ExpressionToConstraintRecord(
    void* theEnv, struct expr* theExpression) {
  ...
  if (rv->floatsAllowed || rv->integersAllowed || rv->symbolsAllowed ||
    ...
    rv->restrictionList =
        GenConstant(theEnv, theExpression->type, theExpression->value);
  }
