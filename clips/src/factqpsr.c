 ...
 ***********************************************************************/
globle EXPRESSION* FactParseQueryNoAction(void* theEnv, EXPRESSION* top,
...
/***********************************************************************
  ...
  NOTES        : H/L Syntax :
                 ...
                 <template-2a> -> <template-2b> -> (QDS) -> ...
 ***********************************************************************/
globle EXPRESSION* FactParseQueryAction(void* theEnv, EXPRESSION* top,
                                        const char* readSource) {
  EXPRESSION* factQuerySetVars;
  struct token queryInputToken;
  ...
  if (ParseQueryActionExpression(theEnv, top, readSource, factQuerySetVars,
  ...
  if (ReplaceFactVariables(theEnv, factQuerySetVars, top->argList->nextArg,
                           FALSE, 0)) {
    ReturnExpression(theEnv, top);
    ReturnExpression(theEnv, factQuerySetVars);
    return NULL;
...
static EXPRESSION* ParseQueryRestrictions(void* theEnv, EXPRESSION* top,
  ...
  while (queryInputToken->type == LPAREN) {
    ...
    tmp = GenConstant(theEnv, SYMBOL,
                      (void*)FactQueryData(theEnv)->QUERY_DELIMETER_SYMBOL);
 ...
 ***************************************************/
static intBool ReplaceTemplateNameWithReference(void* theEnv,
                                                EXPRESSION* theExp) {
  ...
  if (theExp->type == SYMBOL) {
    ...
    theDeftemplate = (struct deftemplate*)FindImportedConstruct(
        theEnv, "deftemplate", NULL, theTemplateName, &count, TRUE, NULL);
...
/***********************************************************************************
 ...
 type <fact-query-variable>:<slot-name> for direct slot access or not 4) Nesting
 depth of query functions RETURNS      : Nothing useful SIDE EFFECTS : If a
 SF_VARIABLE node is found and is on the list of fact variables, it is replaced
 with a query-fact function call. NOTES        : Other SF_VARIABLE(S) are left
 alone for replacement by other parsers.  This implies that a user may use
 defgeneric, defrule, and defmessage-handler variables within a query-function
                   where they do not conflict with fact-variable names.
 ***********************************************************************************/
static intBool ReplaceFactVariables(void* theEnv, EXPRESSION* vlist,
                                    EXPRESSION* bexp, int sdirect, int ndepth) {
  EXPRESSION* eptr;
  ...
  while (bexp != NULL) {
    ...
    if (bexp->argList != NULL) {
      if (IsQueryFunction(bexp)) {
        if (ReplaceFactVariables(theEnv, vlist, bexp->argList, sdirect,
                                 ndepth + 1)) {
          return TRUE;
        }
      } else {
...
static int IsQueryFunction(EXPRESSION* theExp) {
  ...
  if (fptr == (int (*)(void))AnyFacts) return (TRUE);
  if (fptr == (int (*)(void))QueryFindFact) return (TRUE);
  if (fptr == (int (*)(void))QueryFindAllFacts) return (TRUE);
  if (fptr == (int (*)(void))QueryDoForFact) return (TRUE);
  if (fptr == (int (*)(void))QueryDoForAllFacts) return (TRUE);
  if (fptr == (int (*)(void))DelayedQueryDoForAllFacts) return (TRUE);
