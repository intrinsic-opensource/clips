...
static void AddEphemeralHashNode(void*, GENERIC_HN*, struct ephemeron**, int,
                                 int, int);
static void RemoveEphemeralHashNodes(void*, struct ephemeron**, GENERIC_HN**,
                                     int, int, int);
...
globle void InitializeAtomTables(
    void* theEnv, struct symbolHashNode** symbolTable,
    struct floatHashNode** floatTable, struct integerHashNode** integerTable,
...
#pragma unused(externalAddressTable)
  ...
  AllocateEnvironmentData(theEnv, SYMBOL_DATA, sizeof(struct symbolData),
  ...
  SymbolData(theEnv)->SymbolTable =
      (SYMBOL_HN**)gm3(theEnv, sizeof(SYMBOL_HN*) * SYMBOL_HASH_SIZE);

  SymbolData(theEnv)->FloatTable =
      (FLOAT_HN**)gm2(theEnv, (int)sizeof(FLOAT_HN*) * FLOAT_HASH_SIZE);

  SymbolData(theEnv)->IntegerTable =
      (INTEGER_HN**)gm2(theEnv, (int)sizeof(INTEGER_HN*) * INTEGER_HASH_SIZE);

  SymbolData(theEnv)->BitMapTable =
      (BITMAP_HN**)gm2(theEnv, (int)sizeof(BITMAP_HN*) * BITMAP_HASH_SIZE);

  SymbolData(theEnv)->ExternalAddressTable = (EXTERNAL_ADDRESS_HN**)gm2(
      theEnv, (int)sizeof(EXTERNAL_ADDRESS_HN*) * EXTERNAL_ADDRESS_HASH_SIZE);
...
/*    data for symbols.                          */
/*************************************************/
static void DeallocateSymbolData(void* theEnv) {
  int i;
...
#if !RUN_TIME
  rm3(theEnv, SymbolData(theEnv)->SymbolTable,
      sizeof(SYMBOL_HN*) * SYMBOL_HASH_SIZE);

  genfree(theEnv, SymbolData(theEnv)->FloatTable,
          (int)sizeof(FLOAT_HN*) * FLOAT_HASH_SIZE);

  genfree(theEnv, SymbolData(theEnv)->IntegerTable,
          (int)sizeof(INTEGER_HN*) * INTEGER_HASH_SIZE);

  genfree(theEnv, SymbolData(theEnv)->BitMapTable,
          (int)sizeof(BITMAP_HN*) * BITMAP_HASH_SIZE);
#endif

  genfree(theEnv, SymbolData(theEnv)->ExternalAddressTable,
          (int)sizeof(EXTERNAL_ADDRESS_HN*) * EXTERNAL_ADDRESS_HASH_SIZE);
  ...
  if (SymbolData(theEnv)->SymbolArray != NULL)
    rm3(theEnv, (void*)SymbolData(theEnv)->SymbolArray,
        (long)sizeof(SYMBOL_HN*) * SymbolData(theEnv)->NumberOfSymbols);
  if (SymbolData(theEnv)->FloatArray != NULL)
    rm3(theEnv, (void*)SymbolData(theEnv)->FloatArray,
        (long)sizeof(FLOAT_HN*) * SymbolData(theEnv)->NumberOfFloats);
  if (SymbolData(theEnv)->IntegerArray != NULL)
    rm3(theEnv, (void*)SymbolData(theEnv)->IntegerArray,
        (long)sizeof(INTEGER_HN*) * SymbolData(theEnv)->NumberOfIntegers);
  if (SymbolData(theEnv)->BitMapArray != NULL)
    rm3(theEnv, (void*)SymbolData(theEnv)->BitMapArray,
        (long)sizeof(BITMAP_HN*) * SymbolData(theEnv)->NumberOfBitMaps);
#endif
}
...
/*********************************************************************/
globle void* EnvAddSymbol(void* theEnv, const char* str) {
  unsigned long tally;
  size_t length;
  ...
  AddEphemeralHashNode(
      theEnv, (GENERIC_HN*)peek,
...
/*****************************************************************/
globle SYMBOL_HN* FindSymbolHN(void* theEnv, const char* str) {
  unsigned long tally;
  SYMBOL_HN* peek;

  tally = HashSymbol(str, SYMBOL_HASH_SIZE);

  for (peek = SymbolData(theEnv)->SymbolTable[tally]; peek != NULL;
       peek = peek->next) {
    if (strcmp(str, peek->contents) == 0) {
      return (peek);
    }
...
globle INTEGER_HN* FindLongHN(void* theEnv, long long theLong) {
  ...
  for (peek = SymbolData(theEnv)->IntegerTable[tally]; peek != NULL;
       peek = peek->next) {
...
globle void* EnvAddBitMap(void* theEnv, void* vTheBitMap, unsigned size) {
  ...
  AddEphemeralHashNode(
      theEnv, (GENERIC_HN*)peek,
      &UtilityData(theEnv)->CurrentGarbageFrame->ephemeralBitMapList,
...
/*******************************************************************/
globle void* EnvAddExternalAddress(void* theEnv, void* theExternalAddress,
                                   unsigned theType) {
  unsigned long tally;
...
/***************************************************/
globle unsigned long HashSymbol(const char* word, unsigned long range) {
  int i;
...
/***************************************************/
globle unsigned long HashBitMap(const char* word, unsigned long range,
                                unsigned length) {
  unsigned k, j, i;
  unsigned long tally;
  unsigned longLength;
  ...
  for (i = 0, j = 0; i < longLength; i++) {
    for (k = 0; k < sizeof(unsigned long); k++, j++) tmpPtr[k] = word[j];
    count += tmpLong;
  }
...
/************************************************/
static void RemoveHashNode(void* theEnv, GENERIC_HN* theValue,
  ...
  if (type == SYMBOL) {
    ...
    if ((EvaluationData(theEnv)->ExternalAddressTypes[theAddress->type] !=
         NULL) &&
        (EvaluationData(theEnv)
             ->ExternalAddressTypes[theAddress->type]
             ->discardFunction != NULL)) {
      (*EvaluationData(theEnv)
            ->ExternalAddressTypes[theAddress->type]
            ->discardFunction)(theEnv, theAddress->externalAddress);
    }
  }
...
/***********************************************************/
static void AddEphemeralHashNode(void* theEnv, GENERIC_HN* theHashNode,
                                 struct ephemeron** theEphemeralList,
                                 int hashNodeSize, int averageContentsSize,
                                 int checkCount) {
...
globle void RemoveEphemeralAtoms(void* theEnv) {
  ...
  RemoveEphemeralHashNodes(
      theEnv, &theGarbageFrame->ephemeralExternalAddressList,
...
/**********************************************************/
globle void EphemerateMultifield(void* theEnv, struct multifield* theSegment) {
...
/***********************************************/
globle void EphemerateValue(void* theEnv, int theType, void* theValue) {
  SYMBOL_HN* theSymbol;
  FLOAT_HN* theFloat;
...
/****************************************************************/
static void RemoveEphemeralHashNodes(void* theEnv,
                                     struct ephemeron** theEphemeralList,
                                     GENERIC_HN** theTable, int hashNodeSize,
                                     int hashNodeType,
  ...
  while (edPtr != NULL) {
    ...
    if (edPtr->associatedValue->count == 0) {
      RemoveHashNode(theEnv, edPtr->associatedValue, theTable, hashNodeSize,
                     hashNodeType);
...
globle void RefreshSpecialSymbols(void* theEnv) {
  ...
  SymbolData(theEnv)->PositiveInfinity =
      (void*)FindSymbolHN(theEnv, POSITIVE_INFINITY_STRING);
  SymbolData(theEnv)->NegativeInfinity =
      (void*)FindSymbolHN(theEnv, NEGATIVE_INFINITY_STRING);
...
globle struct symbolMatch* FindSymbolMatches(void* theEnv,
  ...
  *numberOfMatches = 0;
  ...
  while ((hashPtr = GetNextSymbolMatch(theEnv, searchString, searchLength,
                                       hashPtr, FALSE, commonPrefixLength)) !=
         NULL) {
...
/*****************************************************************/
globle SYMBOL_HN* GetNextSymbolMatch(void* theEnv, const char* searchString,
                                     size_t searchLength, SYMBOL_HN* prevSymbol,
                                     int anywhere, size_t* commonPrefixLength) {
  unsigned long i;
  SYMBOL_HN* hashPtr;
  int flag = TRUE;
  size_t prefixLength;
  ...
  if (anywhere && (commonPrefixLength != NULL)) *commonPrefixLength = 0;
  ...
  while (flag) {
    ...
    for (; hashPtr != NULL; hashPtr = hashPtr->next) {
      ...
      if (!anywhere) {
        ...
        if (prevSymbol != NULL)
          prefixLength =
              CommonPrefixLength(prevSymbol->contents, hashPtr->contents);
        else
...
/**********************************************/
static const char* StringWithinString(const char* cs, const char* ct) {
  unsigned i, j, k;

  for (i = 0; cs[i] != '\0'; i++) {
    for (j = i, k = 0; ct[k] != '\0' && cs[j] == ct[k]; j++, k++);
    if ((ct[k] == '\0') && (k != 0)) return (cs + i);
  }
  return (NULL);
}
...
/************************************************/
static size_t CommonPrefixLength(const char* cs, const char* ct) {
  unsigned i;
...
globle void* EnvFalseSymbol(void* theEnv) {
  return SymbolData(theEnv)->FalseSymbolHN;
}

globle void* EnvTrueSymbol(void* theEnv) {
...
globle void* AddSymbol(const char* str) {
  return EnvAddSymbol(GetCurrentEnvironment(), str);
}

globle void* AddLong(long long number) {
  return EnvAddLong(GetCurrentEnvironment(), number);
}

globle void* AddDouble(double number) {
