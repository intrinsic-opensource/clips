...
globle void InitializeWatchData(void* theEnv) {
  AllocateEnvironmentData(theEnv, WATCH_DATA, sizeof(struct watchData),
                          DeallocateWatchData);
}
...
/*    data for watch items.                     */
/************************************************/
static void DeallocateWatchData(void* theEnv) {
...
globle intBool AddWatchItem(
    void* theEnv, const char* name, int code, unsigned* flag, int priority,
...
/*****************************************************/
globle intBool EnvWatch(void* theEnv, const char* itemName) {
...
/*********************************************************/
globle intBool EnvUnwatch(void* theEnv, const char* itemName) {
...
/***********************************************************************/
globle int EnvSetWatchItem(void* theEnv, const char* itemName,
                           unsigned newState, struct expr* argExprs) {
  ...
  if (strcmp(itemName, "all") == 0) {
    for (wPtr = WatchData(theEnv)->ListOfWatchItems; wPtr != NULL;
         wPtr = wPtr->next) {
      /*==============================================*/
...
/******************************************************************/
globle int EnvGetWatchItem(void* theEnv, const char* itemName) {
  struct watchItem* wPtr;
...
/****************************************************************/
static struct watchItem* ValidWatchItem(void* theEnv, const char* itemName,
                                        int* recognized) {
  struct watchItem* wPtr;

  *recognized = TRUE;
  if (strcmp(itemName, "all") == 0) return (NULL);
...
/*************************************************************/
globle const char* GetNthWatchName(void* theEnv, int whichItem) {
  int i;
  struct watchItem* wPtr;

  for (wPtr = WatchData(theEnv)->ListOfWatchItems, i = 1; wPtr != NULL;
       wPtr = wPtr->next, i++) {
...
globle void ListWatchItemsCommand(void* theEnv) {
  ...
  if (EnvArgTypeCheck(theEnv, "list-watch-items", 1, SYMBOL, &theValue) ==
      FALSE)
    return;
...
globle int GetWatchItemCommand(void* theEnv) {
  ...
  if (EnvArgTypeCheck(theEnv, "get-watch-item", 1, SYMBOL, &theValue) ==
      FALSE) {
    return (FALSE);
  }
...
#if !RUN_TIME
  EnvDefineFunction2(theEnv, "watch", 'v', PTIEF WatchCommand, "WatchCommand",
                     "1**w");
  EnvDefineFunction2(theEnv, "unwatch", 'v', PTIEF UnwatchCommand,
                     "UnwatchCommand", "1**w");
  EnvDefineFunction2(theEnv, "get-watch-item", 'b', PTIEF GetWatchItemCommand,
                     "GetWatchItemCommand", "11w");
  EnvDefineFunction2(theEnv, "list-watch-items", 'v',
                     PTIEF ListWatchItemsCommand, "ListWatchItemsCommand",
                     "0**w");
#endif
...
/**************************************************/
static intBool RecognizeWatchRouters(void* theEnv, const char* logName) {
#if MAC_XCD
...
/**************************************************/
static int CaptureWatchPrints(void* theEnv, const char* logName,
                              const char* str) {
#if MAC_XCD
...
globle intBool Watch(const char* itemName) {
  return (EnvWatch(GetCurrentEnvironment(), itemName));
}

globle intBool Unwatch(const char* itemName) {
  return (EnvUnwatch(GetCurrentEnvironment(), itemName));
}

globle int GetWatchItem(const char* itemName) {
  return EnvGetWatchItem(GetCurrentEnvironment(), itemName);
}

globle int SetWatchItem(const char* itemName, unsigned newState,
