...
/***********************************************************************************
  ...
  SIDE EFFECTS : Any side-effects of evaluating the generic function arguments
                 Any side-effects of evaluating query functions on method
 parameter restrictions when determining the core (see warning #1) Any
 side-effects of actual execution of methods (see warning #2) Caller's buffer
 set to the result of the generic function call
  ...
  NOTES        : WARNING #1: Query functions on method parameter restrictions
                    should not have side-effects, for they might be evaluated
 even for methods that aren't applicable to the generic function call. WARNING
 #2: Side-effects of method execution should not always rely on only being
 executed once per generic function call.  Every time a method calls
 (shadow-call) the same next-most-specific method is executed.  Thus, it is
 possible for a method to be executed multiple times per generic function call.
 ***********************************************************************************/
globle void GenericDispatch(void* theEnv, DEFGENERIC* gfunc,
                            DEFMETHOD* prevmeth, DEFMETHOD* meth,
                            EXPRESSION* params, DATA_OBJECT* result) {
  ...
  if (DefgenericData(theEnv)->CurrentMethod != NULL) {
    ...
    if (DefgenericData(theEnv)->CurrentMethod->system) {
      ...
      EvaluateProcActions(
          ...
          DefgenericData(theEnv)->CurrentMethod->localVarCount, result,
          UnboundMethodErr);
...
globle void UnboundMethodErr(void* theEnv) {
  EnvPrintRouter(theEnv, WERROR, "generic function ");
  EnvPrintRouter(theEnv, WERROR,
                 EnvGetDefgenericName(
                     theEnv, (void*)DefgenericData(theEnv)->CurrentGeneric));
  EnvPrintRouter(theEnv, WERROR, " method #");
  PrintLongInteger(theEnv, WERROR,
                   (long long)DefgenericData(theEnv)->CurrentMethod->index);
  EnvPrintRouter(theEnv, WERROR, ".\n");
}
...
globle intBool IsMethodApplicable(void* theEnv, DEFMETHOD* meth) {
  ...
  if ((ProceduralPrimitiveData(theEnv)->ProcParamArraySize <
       meth->minRestrictions) ||
  ...
  for (i = 0, k = 0; i < ProceduralPrimitiveData(theEnv)->ProcParamArraySize;
    ...
    if (rp->tcnt != 0) {
#if OBJECT_SYSTEM
      type = (void*)DetermineRestrictionClass(
          theEnv, &ProceduralPrimitiveData(theEnv)->ProcParamArray[i]);
      if (type == NULL) return (FALSE);
      for (j = 0; j < rp->tcnt; j++) {
        if (type == rp->types[j]) break;
        if (HasSuperclass((DEFCLASS*)type, (DEFCLASS*)rp->types[j])) break;
        if (rp->types[j] ==
            (void*)DefclassData(theEnv)->PrimitiveClassMap[INSTANCE_ADDRESS]) {
          if (ProceduralPrimitiveData(theEnv)->ProcParamArray[i].type ==
              INSTANCE_ADDRESS)
            break;
        } else if (rp->types[j] == (void*)DefclassData(theEnv)
      ...
      for (j = 0; j < rp->tcnt; j++) {
        if (type == ValueToInteger(rp->types[j])) break;
        if (SubsumeType(type, ValueToInteger(rp->types[j]))) break;
      }
#endif
      if (j == rp->tcnt) return (FALSE);
    }
    if (rp->query != NULL) {
    ...
    if (((int)k) != meth->restrictionCount - 1) k++;
  }
  return (TRUE);
}
...
globle int NextMethodP(void* theEnv) {
  ...
  meth = FindApplicableMethod(theEnv, DefgenericData(theEnv)->CurrentGeneric,
                              DefgenericData(theEnv)->CurrentMethod);
  if (meth != NULL) {
    meth->busy--;
    return (TRUE);
...
globle void CallSpecificMethod(void* theEnv, DATA_OBJECT* result) {
  ...
  result->type = SYMBOL;
  result->value = EnvFalseSymbol(theEnv);
  if (EnvArgTypeCheck(theEnv, "call-specific-method", 1, SYMBOL, &temp) ==
      FALSE)
    return;
  gfunc = CheckGenericExists(theEnv, "call-specific-method", DOToString(temp));
  if (gfunc == NULL) return;
  if (EnvArgTypeCheck(theEnv, "call-specific-method", 2, INTEGER, &temp) ==
      FALSE)
    return;
  mi = CheckMethodExists(theEnv, "call-specific-method", gfunc,
                         (long)DOToLong(temp));
  if (mi == -1) return;
...
globle void OverrideNextMethod(void* theEnv, DATA_OBJECT* result) {
  ...
  GenericDispatch(theEnv, DefgenericData(theEnv)->CurrentGeneric,
                  DefgenericData(theEnv)->CurrentMethod, NULL,
...
/************************************************************
  ...
  NOTES        : None
 ************************************************************/
static DEFMETHOD* FindApplicableMethod(void* theEnv, DEFGENERIC* gfunc,
                                       DEFMETHOD* meth) {
  if (meth != NULL)
    meth++;
  else
  ...
  for (; meth < &gfunc->methods[gfunc->mcnt]; meth++) {
    meth->busy++;
    if (IsMethodApplicable(theEnv, meth)) return (meth);
    meth->busy--;
  }
  return (NULL);
}
 ...
 **********************************************************************/
static void WatchGeneric(void* theEnv, const char* tstring) {
  EnvPrintRouter(theEnv, WTRACE, "GNC ");
  EnvPrintRouter(theEnv, WTRACE, tstring);
  EnvPrintRouter(theEnv, WTRACE, " ");
  if (DefgenericData(theEnv)->CurrentGeneric->header.whichModule->theModule !=
      ((struct defmodule*)EnvGetCurrentModule(theEnv))) {
    EnvPrintRouter(
        theEnv, WTRACE,
        EnvGetDefmoduleName(
            theEnv, (void*)DefgenericData(theEnv)
  ...
  EnvPrintRouter(
      theEnv, WTRACE,
      ValueToString(
          (void*)DefgenericData(theEnv)->CurrentGeneric->header.name));
  EnvPrintRouter(theEnv, WTRACE, " ");
  EnvPrintRouter(theEnv, WTRACE, " ED:");
  PrintLongInteger(theEnv, WTRACE,
...
/**********************************************************************
  ...
  NOTES        : Uses the globals CurrentGeneric, CurrentMethod,
                   ...
                   other trace info
 **********************************************************************/
static void WatchMethod(void* theEnv, const char* tstring) {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static DEFCLASS* DetermineRestrictionClass(void* theEnv, DATA_OBJECT* dobj) {
  INSTANCE_TYPE* ins;
  DEFCLASS* cls;
