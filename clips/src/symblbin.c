...
/**********************************************/
globle void WriteNeededAtomicValues(void* theEnv, FILE* fp) {
...
/********************************************************/
globle void InitAtomicValueNeededFlags(void* theEnv) {
  unsigned long i;
...
/*****************************************************************/
globle void WriteNeededSymbols(void* theEnv, FILE* fp) {
  unsigned long i;
  size_t length;
  ...
  GenWrite((void*)&numberOfUsedSymbols,
           (unsigned long)sizeof(unsigned long int), fp);
...
/*   table needed for this binary image in the binary save file. */
/*****************************************************************/
globle void WriteNeededFloats(void* theEnv, FILE* fp) {
  int i;
  FLOAT_HN** floatArray;
  FLOAT_HN* floatPtr;
  ...
  for (i = 0; i < FLOAT_HASH_SIZE; i++) {
    for (floatPtr = floatArray[i]; floatPtr != NULL;
      ...
      if (floatPtr->neededFloat) {
        GenWrite(&floatPtr->contents, (unsigned long)sizeof(floatPtr->contents),
                 fp);
      }
    }
  }
...
/*   table needed for this binary image in the binary save file.  */
/******************************************************************/
globle void WriteNeededIntegers(void* theEnv, FILE* fp) {
  int i;
...
/*   table needed for this binary image in the binary save file. */
/*****************************************************************/
static void WriteNeededBitMaps(void* theEnv, FILE* fp) {
  int i;
  ...
  GenWrite((void*)&numberOfUsedBitMaps,
           (unsigned long)sizeof(unsigned long int), fp);
  GenWrite((void*)&size, (unsigned long)sizeof(unsigned long int), fp);
  ...
  for (i = 0; i < BITMAP_HASH_SIZE; i++) {
    for (bitMapPtr = bitMapArray[i]; bitMapPtr != NULL;
      ...
      if (bitMapPtr->neededBitMap) {
        ...
        GenWrite((void*)bitMapPtr->contents, (unsigned long)bitMapPtr->size,
                 fp);
      }
    }
  }
...
/*********************************************/
globle void ReadNeededAtomicValues(void* theEnv) {
...
/*******************************************/
globle void ReadNeededSymbols(void* theEnv) {
  ...
  GenReadBinary(theEnv, (void*)&SymbolData(theEnv)->NumberOfSymbols,
  ...
  SymbolData(theEnv)->SymbolArray = (SYMBOL_HN**)gm3(
      theEnv, (long)sizeof(SYMBOL_HN*) * SymbolData(theEnv)->NumberOfSymbols);
  namePtr = symbolNames;
  for (i = 0; i < SymbolData(theEnv)->NumberOfSymbols; i++) {
    SymbolData(theEnv)->SymbolArray[i] =
        (SYMBOL_HN*)EnvAddSymbol(theEnv, namePtr);
...
/*   used by the binary image.           */
/*****************************************/
globle void ReadNeededFloats(void* theEnv) {
  double* floatValues;
  long i;
  ...
  GenReadBinary(theEnv, &SymbolData(theEnv)->NumberOfFloats,
                (unsigned long)sizeof(long int));
  ...
  floatValues = (double*)gm3(
      theEnv, (long)sizeof(double) * SymbolData(theEnv)->NumberOfFloats);
  GenReadBinary(
      theEnv, (void*)floatValues,
      (unsigned long)(sizeof(double) * SymbolData(theEnv)->NumberOfFloats));

  /*======================================*/
  /* Store the floats in the float array. */
  /*======================================*/

  SymbolData(theEnv)->FloatArray = (FLOAT_HN**)gm3(
      theEnv, (long)sizeof(FLOAT_HN*) * SymbolData(theEnv)->NumberOfFloats);
  for (i = 0; i < SymbolData(theEnv)->NumberOfFloats; i++) {
    SymbolData(theEnv)->FloatArray[i] =
        (FLOAT_HN*)EnvAddDouble(theEnv, floatValues[i]);
  }
  ...
  rm3(theEnv, (void*)floatValues,
      (long)(sizeof(double) * SymbolData(theEnv)->NumberOfFloats));
}
...
/*   used by the binary image.               */
/*********************************************/
globle void ReadNeededIntegers(void* theEnv) {
  ...
  GenReadBinary(theEnv, &SymbolData(theEnv)->NumberOfIntegers,
  ...
  integerValues = (long long*)gm3(
      theEnv, (long)(sizeof(long long) * SymbolData(theEnv)->NumberOfIntegers));
  ...
  SymbolData(theEnv)->IntegerArray = (INTEGER_HN**)gm3(
      theEnv,
  ...
  for (i = 0; i < SymbolData(theEnv)->NumberOfIntegers; i++) {
    SymbolData(theEnv)->IntegerArray[i] =
        (INTEGER_HN*)EnvAddLong(theEnv, integerValues[i]);
  }
...
/*   used by the binary image.             */
/*******************************************/
static void ReadNeededBitMaps(void* theEnv) {
  ...
  GenReadBinary(theEnv, (void*)&SymbolData(theEnv)->NumberOfBitMaps,
                (unsigned long)sizeof(long int));
  GenReadBinary(theEnv, &space, (unsigned long)sizeof(unsigned long int));
  ...
  SymbolData(theEnv)->BitMapArray = (BITMAP_HN**)gm3(
      theEnv, (long)sizeof(BITMAP_HN*) * SymbolData(theEnv)->NumberOfBitMaps);
  bitMapPtr = bitMapStorage;
  for (i = 0; i < SymbolData(theEnv)->NumberOfBitMaps; i++) {
    tempSize = (unsigned short*)bitMapPtr;
    SymbolData(theEnv)->BitMapArray[i] = (BITMAP_HN*)EnvAddBitMap(
        theEnv, bitMapPtr + sizeof(unsigned short), *tempSize);
...
/**********************************************************/
globle void FreeAtomicValueStorage(void* theEnv) {
...
#endif /* BLOAD || BLOAD_ONLY || BLOAD_AND_BSAVE || BLOAD_INSTANCES || \
          BSAVE_INSTANCES */
