...
#define SortFunctionData(theEnv) \
  ((struct sortFunctionData*)GetEnvironmentData(theEnv, SORTFUN_DATA))
...
globle void SortFunctionDefinitions(void* theEnv) {
  AllocateEnvironmentData(theEnv, SORTFUN_DATA, sizeof(struct sortFunctionData),
                          DeallocateSortFunctionData);
#if !RUN_TIME
  EnvDefineFunction2(theEnv, "sort", 'u', PTIEF SortFunction, "SortFunction",
                     "1**w");
#endif
}
...
/*******************************************************/
static void DeallocateSortFunctionData(void* theEnv) {
...
static int DefaultCompareSwapFunction(void* theEnv, DATA_OBJECT* item1,
  ...
  SortFunctionData(theEnv)->SortComparisonFunction->argList->nextArg =
      GenConstant(theEnv, item2->type, item2->value);
  ExpressionInstall(theEnv, SortFunctionData(theEnv)->SortComparisonFunction);
  EvaluateExpression(theEnv, SortFunctionData(theEnv)->SortComparisonFunction,
                     &returnValue);
...
globle void SortFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  if (functionReference == NULL) {
    ExpectedTypeError1(theEnv, "sort", 1,
  ...
  if (functionReference->type == FCALL) {
    ...
    if ((GetMinimumArgs(fptr) > 2) || (GetMaximumArgs(fptr) == 0) ||
  ...
  theArguments =
      (DATA_OBJECT*)genalloc(theEnv, (argumentCount - 1) * sizeof(DATA_OBJECT));
  ...
  if (argumentSize == 0) {
    genfree(theEnv, theArguments,
            (argumentCount - 1) * sizeof(DATA_OBJECT)); /* Bug Fix */
    EnvSetMultifieldErrorValue(theEnv, returnValue);
    ReturnExpression(theEnv, functionReference);
    return;
  }
  ...
  theArguments2 =
      (DATA_OBJECT*)genalloc(theEnv, argumentSize * sizeof(DATA_OBJECT));

  for (i = 2; i <= argumentCount; i++) {
    if (GetType(theArguments[i - 2]) == MULTIFIELD) {
      tempMultifield = (struct multifield*)GetValue(theArguments[i - 2]);
      for (j = GetDOBegin(theArguments[i - 2]);
           j <= GetDOEnd(theArguments[i - 2]); j++, k++) {
  ...
  MergeSort(theEnv, (unsigned long)argumentSize, theArguments2,
            DefaultCompareSwapFunction);
  ...
  SortFunctionData(theEnv)->SortComparisonFunction =
      SortFunctionData(theEnv)->SortComparisonFunction->nextArg;
  functionReference->nextArg = NULL;
  ReturnExpression(theEnv, functionReference);

  theMultifield = (struct multifield*)EnvCreateMultifield(
      theEnv, (unsigned long)argumentSize);
...
/*******************************************/
void MergeSort(void* theEnv, unsigned long listSize, DATA_OBJECT* theList,
  ...
  middle = (listSize + 1) / 2;
  DoMergeSort(theEnv, theList, tempList, 0, middle - 1, middle, listSize - 1,
              swapFunction);
...
/******************************************************/
static void DoMergeSort(void* theEnv, DATA_OBJECT* theList,
                        DATA_OBJECT* tempList, unsigned long s1,
  ...
  if (s1 == e1) { /* List doesn't need to be merged. */
    ...
    size = ((e1 - s1) + 1);
    middle = s1 + ((size + 1) / 2);
    DoMergeSort(theEnv, theList, tempList, s1, middle - 1, middle, e1,
                swapFunction);
  }
  ...
  if (s2 == e2) { /* List doesn't need to be merged. */
    ...
    size = ((e2 - s2) + 1);
    middle = s2 + ((size + 1) / 2);
    DoMergeSort(theEnv, theList, tempList, s2, middle - 1, middle, e2,
                swapFunction);
  }
