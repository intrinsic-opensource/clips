...
#define ProcedureParserData(theEnv) \
  ((struct procedureParserData*)GetEnvironmentData(theEnv, PRCDRPSR_DATA))
#endif
...
globle void ProceduralFunctionParsers(void* theEnv) {
  AllocateEnvironmentData(theEnv, PRCDRPSR_DATA,
...
/*************************************************************/
static void DeallocateProceduralFunctionData(void* theEnv) {
...
/* GetParsedBindNames:                                      */
/********************************************************/
globle struct BindInfo* GetParsedBindNames(void* theEnv) {
...
globle void ClearParsedBindNames(void* theEnv) {
  ...
  while (ProcedureParserData(theEnv)->ListOfParsedBindNames != NULL) {
    ...
    RemoveConstraint(
        theEnv,
        ProcedureParserData(theEnv)->ListOfParsedBindNames->constraints);
    rtn_struct(theEnv, BindInfo,
               ProcedureParserData(theEnv)->ListOfParsedBindNames);
    ProcedureParserData(theEnv)->ListOfParsedBindNames = temp_bind;
  }
...
/*********************************************************/
static struct expr* WhileParse(void* theEnv, struct expr* parse,
                               const char* infile) {
  ...
  if ((theToken.type == SYMBOL) &&
      (strcmp(ValueToString(theToken.value), "do") == 0)) {
  ...
  parse->argList->nextArg =
      GroupActions(theEnv, infile, &theToken, read_first_token, NULL, FALSE);
...
/*           <range> ::= (<sf-var> [<start-integer-expression>]
 * <end-integer-expression>) */
/******************************************************************************************/
static struct expr* LoopForCountParse(void* theEnv, struct expr* parse,
                                      const char* infile) {
  struct token theToken;
  ...
  if (theToken.type != LPAREN) {
    ...
    if (theToken.type != SF_VARIABLE) {
      if (theToken.type != SYMBOL) goto LoopForCountParseError;
      parse->argList = GenConstant(theEnv, INTEGER, EnvAddLong(theEnv, 1LL));
      parse->argList->nextArg =
          Function2Parse(theEnv, infile, ValueToString(theToken.value));
      if (parse->argList->nextArg == NULL) {
        ReturnExpression(theEnv, parse);
        return (NULL);
      }
    ...
    /* =============================================================
      ...
      if (theToken.type == RPAREN) {
        ...
        parse->argList->nextArg =
            ParseAtomOrExpression(theEnv, infile, &theToken);
        if (parse->argList->nextArg == NULL) {
          ReturnExpression(theEnv, parse);
          return (NULL);
        }
  ...
  if (CheckArgumentAgainstRestriction(theEnv, parse->argList->nextArg,
                                      (int)'i'))
    goto LoopForCountParseError;
...
/***************************************************/
static void ReplaceLoopCountVars(void* theEnv, SYMBOL_HN* loopVar,
                                 EXPRESSION* theExp, int depth) {
  while (theExp != NULL) {
    if ((theExp->type != SF_VARIABLE) ? FALSE
                                      : (strcmp(ValueToString(theExp->value),
                                                ValueToString(loopVar)) == 0)) {
      theExp->type = FCALL;
      ...
      if ((theExp->type != FCALL)
              ? FALSE
              : (theExp->value ==
                 (void*)FindFunction(theEnv, "loop-for-count")))
...
/*               [ else <action>+ ] )                    */
/*********************************************************/
static struct expr* IfParse(void* theEnv, struct expr* top,
                            const char* infile) {
  struct token theToken;
  ...
  if (ExpressionData(theEnv)->svContexts->brk == TRUE)
    ExpressionData(theEnv)->BreakContext = TRUE;
  top->argList->nextArg =
      GroupActions(theEnv, infile, &theToken, TRUE, "else", FALSE);
  ...
  if ((theToken.type != SYMBOL) ||
      (strcmp(ValueToString(theToken.value), "else") != 0)) {
    SyntaxErrorMessage(theEnv, "if function");
    ReturnExpression(theEnv, top);
    return (NULL);
  }
  ...
  top->argList->nextArg->nextArg =
      GroupActions(theEnv, infile, &theToken, TRUE, NULL, FALSE);
  ...
  top->argList->nextArg->nextArg =
      RemoveUnneededProgn(theEnv, top->argList->nextArg->nextArg);
...
/********************************************************/
static struct expr* PrognParse(void* theEnv, struct expr* top,
                               const char* infile) {
  struct token tkn;
  struct expr* tmp;
...
/***********************************************************/
static struct expr* BindParse(void* theEnv, struct expr* top,
                              const char* infile) {
  struct token theToken;
  ...
  if ((theToken.type == GBL_VARIABLE)
          ? ((theGlobal = (struct defglobal*)FindImportedConstruct(
                  theEnv, "defglobal", NULL, ValueToString(variableName),
...
/********************************************/
static struct expr* ReturnParse(void* theEnv, struct expr* top,
                                const char* infile) {
...
/* BreakParse:                                */
/**********************************************/
static struct expr* BreakParse(void* theEnv, struct expr* top,
                               const char* infile) {
  struct token theToken;
...
/* SwitchParse:                               */
/**********************************************/
static struct expr* SwitchParse(void* theEnv, struct expr* top,
                                const char* infile) {
  struct token theToken;
  ...
  if (theExp == NULL) goto SwitchParseError;
  ...
  while (theToken.type != RPAREN) {
    ...
    PPCRAndIndent(theEnv);
    SavePPBuffer(theEnv, theToken.printForm);
    if (theToken.type != LPAREN) goto SwitchParseErrorAndMessage;
    GetToken(theEnv, infile, &theToken);
    SavePPBuffer(theEnv, " ");
    if ((theToken.type == SYMBOL) &&
        (strcmp(ValueToString(theToken.value), "case") == 0)) {
      if (default_count != 0) goto SwitchParseErrorAndMessage;
      theExp->nextArg = ParseAtomOrExpression(theEnv, infile, NULL);
      SavePPBuffer(theEnv, " ");
      if (theExp->nextArg == NULL) goto SwitchParseError;
      ...
      if (default_count) goto SwitchParseErrorAndMessage;
...
/****************************************************************/
static int AddBindName(void* theEnv, SYMBOL_HN* variableName,
    ...
    if (currentBind->name == variableName) {
      if (theConstraint != NULL) {
        ...
        currentBind->constraints =
            UnionConstraints(theEnv, theConstraint, currentBind->constraints);
