...
globle void FileCommandDefinitions(void* theEnv) {
  AllocateEnvironmentData(theEnv, FILECOM_DATA, sizeof(struct fileCommandData),
                          DeallocateFileCommandData);

#if !RUN_TIME
#if DEBUGGING_FUNCTIONS
  EnvDefineFunction2(theEnv, "batch", 'b', PTIEF BatchCommand, "BatchCommand",
                     "11k");
  EnvDefineFunction2(theEnv, "batch*", 'b', PTIEF BatchStarCommand,
                     "BatchStarCommand", "11k");
  EnvDefineFunction2(theEnv, "dribble-on", 'b', PTIEF DribbleOnCommand,
                     "DribbleOnCommand", "11k");
  EnvDefineFunction2(theEnv, "dribble-off", 'b', PTIEF DribbleOffCommand,
                     "DribbleOffCommand", "00");
  EnvDefineFunction2(theEnv, "save", 'b', PTIEF SaveCommand, "SaveCommand",
                     "11k");
#endif
  EnvDefineFunction2(theEnv, "load", 'b', PTIEF LoadCommand, "LoadCommand",
                     "11k");
  EnvDefineFunction2(theEnv, "load*", 'b', PTIEF LoadStarCommand,
                     "LoadStarCommand", "11k");
#if BLOAD_AND_BSAVE
  EnvDefineFunction2(theEnv, "bsave", 'b', PTIEF BsaveCommand, "BsaveCommand",
                     "11k");
#endif
#if BLOAD || BLOAD_ONLY || BLOAD_AND_BSAVE
  InitializeBsaveData(theEnv);
  InitializeBloadData(theEnv);
  EnvDefineFunction2(theEnv, "bload", 'b', PTIEF BloadCommand, "BloadCommand",
                     "11k");
#endif
#endif
}
...
/******************************************************/
static void DeallocateFileCommandData(void* theEnv) {
  ...
  while (theEntry != NULL) {
    ...
    if (theEntry->batchType == FILE_BATCH) {
      GenClose(theEnv,
               (FILE*)FileCommandData(theEnv)->TopOfBatchList->inputSource);
    } else {
  ...
  if (FileCommandData(theEnv)->BatchBuffer != NULL) {
    rm(theEnv, FileCommandData(theEnv)->BatchBuffer,
  ...
  if (FileCommandData(theEnv)->DribbleBuffer != NULL) {
    rm(theEnv, FileCommandData(theEnv)->DribbleBuffer,
...
/*****************************************************/
static int FindDribble(void* theEnv, const char* logicalName) {
#if MAC_XCD
#pragma unused(theEnv)
#endif

  if ((strcmp(logicalName, STDOUT) == 0) || (strcmp(logicalName, STDIN) == 0) ||
...
/* AppendDribble: */
/******************/
void AppendDribble(void* theEnv, const char* str) {
  int i;
...
/*******************************************************/
static int PrintDribble(void* theEnv, const char* logicalName,
                        const char* str) {
  int i;
...
/*****************************************************/
static int GetcDribble(void* theEnv, const char* logicalName) {
  int rv;
...
/***********************************************************/
static void PutcDribbleBuffer(void* theEnv, int rv) {
  /*===================================================*/
  ...
  /*=====================================================*/
    ...
    FileCommandData(theEnv)->DribbleBuffer = ExpandStringWithChar(
        theEnv, rv, FileCommandData(theEnv)->DribbleBuffer,
...
/*********************************************************/
static int UngetcDribble(void* theEnv, int ch, const char* logicalName) {
  int rv;
...
/*****************************************************/
static int ExitDribble(void* theEnv, int num) {
#if MAC_XCD
#pragma unused(num)
#endif
...
/*   for the dribble-on command.  */
/**********************************/
globle intBool EnvDribbleOn(void* theEnv, const char* fileName) {
  /*==============================*/
  ...
  EnvAddRouter(theEnv, "dribble", 40, FindDribble, PrintDribble, GetcDribble,
               UngetcDribble, ExitDribble);
...
/*************************************************/
static int FindBatch(void* theEnv, const char* logicalName) {
#if MAC_XCD
#pragma unused(theEnv)
#endif
...
/*************************************************/
static int GetcBatch(void* theEnv, const char* logicalName) {
...
/***************************************************/
globle int LLGetcBatch(void* theEnv, const char* logicalName, int returnOnEOF) {
  int rv = EOF, flag = 1;
    ...
    if (rv == EOF) {
      if (FileCommandData(theEnv)->BatchCurrentPosition > 0)
        EnvPrintRouter(theEnv, STDOUT,
                       (char*)FileCommandData(theEnv)->BatchBuffer);
  ...
  FileCommandData(theEnv)->BatchBuffer = ExpandStringWithChar(
      theEnv, (char)rv, FileCommandData(theEnv)->BatchBuffer,
...
/*****************************************************/
static int UngetcBatch(void* theEnv, int ch, const char* logicalName) {
#if MAC_XCD
#pragma unused(logicalName)
  ...
  if (FileCommandData(theEnv)->BatchBuffer != NULL)
    FileCommandData(theEnv)
        ->BatchBuffer[FileCommandData(theEnv)->BatchCurrentPosition] = EOS;
  if (FileCommandData(theEnv)->BatchType == FILE_BATCH) {
    return (ungetc(ch, (FILE*)FileCommandData(theEnv)->BatchSource));
  }

  return (
      EnvUngetcRouter(theEnv, ch, (char*)FileCommandData(theEnv)->BatchSource));
}

/*************************************************/
/* ExitBatch: Exit routine for the batch router. */
/*************************************************/
static int ExitBatch(void* theEnv, int num) {
#if MAC_XCD
#pragma unused(num)
#endif
...
/**************************************************/
globle int Batch(void* theEnv, const char* fileName) {
  return (OpenBatch(theEnv, fileName, FALSE));
}
...
/***********************************************/
globle int OpenBatch(void* theEnv, const char* fileName, int placeAtEnd) {
  FILE* theFile;
  ...
  if (FileCommandData(theEnv)->TopOfBatchList == NULL) {
    EnvAddRouter(theEnv, "batch", 20, FindBatch, NULL, GetcBatch, UngetcBatch,
                 ExitBatch);
  }
  ...
  if (FileCommandData(theEnv)->TopOfBatchList == NULL) {
    FileCommandData(theEnv)->batchPriorParsingFile =
        CopyString(theEnv, EnvGetParsingFileName(theEnv));
  }
...
/*****************************************************************/
globle int OpenStringBatch(void* theEnv, const char* stringName,
                           const char* theString, int placeAtEnd) {
...
/*******************************************************/
static void AddBatch(void* theEnv, int placeAtEnd, void* theSource, int type,
                     const char* theString, const char* theFileName) {
...
globle int RemoveBatch(void* theEnv) {
  ...
  if (FileCommandData(theEnv)->TopOfBatchList->batchType == FILE_BATCH) {
    ...
    CloseStringSource(
        theEnv, (char*)FileCommandData(theEnv)->TopOfBatchList->inputSource);
  ...
  DeleteString(theEnv,
               (char*)FileCommandData(theEnv)->TopOfBatchList->fileName);
  bptr = FileCommandData(theEnv)->TopOfBatchList;
  FileCommandData(theEnv)->TopOfBatchList =
      FileCommandData(theEnv)->TopOfBatchList->next;
  ...
  /*===========================================*/
    ...
    FileCommandData(theEnv)->BatchType =
        FileCommandData(theEnv)->TopOfBatchList->batchType;
    FileCommandData(theEnv)->BatchSource =
        FileCommandData(theEnv)->TopOfBatchList->inputSource;
    FileCommandData(theEnv)->BatchCurrentPosition = 0;
    rv = 1;
#if (!RUN_TIME) && (!BLOAD_ONLY)
    if (FileCommandData(theEnv)->TopOfBatchList->batchType == FILE_BATCH) {
      EnvSetParsingFileName(theEnv,
                            FileCommandData(theEnv)->TopOfBatchList->fileName);
    }
...
/**********************************************************/
globle int EnvBatchStar(void* theEnv, const char* fileName) {
  int inchar;
  FILE* theFile;
  char* theString = NULL;
  size_t position = 0;
...
globle int LoadCommand(void* theEnv) {
  ...
  EnvPrintRouter(theEnv, WDIALOG,
                 "Load is not available in this environment\n");
  return (FALSE);
#endif
}
...
#else
  EnvPrintRouter(theEnv, WDIALOG,
                 "Save is not available in this environment\n");
  return (FALSE);
#endif
}
#endif
...
globle intBool DribbleOn(const char* fileName) {
  return EnvDribbleOn(GetCurrentEnvironment(), fileName);
}

globle intBool DribbleOff() { return EnvDribbleOff(GetCurrentEnvironment()); }

#endif /* DEBUGGING_FUNCTIONS */

globle int BatchStar(const char* fileName) {
