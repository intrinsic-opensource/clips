...
#if BLOAD_AND_BSAVE
static void CopyToBsaveConstraintRecord(void*, CONSTRAINT_RECORD*,
...
static void CopyToBsaveConstraintRecord(
    void* theEnv, CONSTRAINT_RECORD* constraints,
    BSAVE_CONSTRAINT_RECORD* bsaveConstraints) {
  ...
  bsaveConstraints->restrictionList =
      HashedExpressionIndex(theEnv, constraints->restrictionList);
  bsaveConstraints->classList =
      HashedExpressionIndex(theEnv, constraints->classList);
  bsaveConstraints->minValue =
      HashedExpressionIndex(theEnv, constraints->minValue);
  bsaveConstraints->maxValue =
      HashedExpressionIndex(theEnv, constraints->maxValue);
  bsaveConstraints->minFields =
      HashedExpressionIndex(theEnv, constraints->minFields);
  bsaveConstraints->maxFields =
      HashedExpressionIndex(theEnv, constraints->maxFields);
}
...
globle void ReadNeededConstraints(void* theEnv) {
  GenReadBinary(theEnv, (void*)&ConstraintData(theEnv)->NumberOfConstraints,
                sizeof(unsigned long int));
  if (ConstraintData(theEnv)->NumberOfConstraints == 0) return;

  ConstraintData(theEnv)->ConstraintArray = (CONSTRAINT_RECORD*)genalloc(
      theEnv, (sizeof(CONSTRAINT_RECORD) *
               ConstraintData(theEnv)->NumberOfConstraints));

  BloadandRefresh(theEnv, ConstraintData(theEnv)->NumberOfConstraints,
...
/*****************************************************/
static void CopyFromBsaveConstraintRecord(void* theEnv, void* buf,
                                          long theIndex) {
  BSAVE_CONSTRAINT_RECORD* bsaveConstraints;
  ...
  constraints =
      (CONSTRAINT_RECORD*)&ConstraintData(theEnv)->ConstraintArray[theIndex];
