...
globle void InitializeClasses(void* theEnv) {
  ...
  DefclassData(theEnv)->SlotNameTable = (SLOT_NAME**)gm2(
      theEnv, (int)(sizeof(SLOT_NAME*) * SLOT_NAME_TABLE_HASH_SIZE));
...
globle SLOT_DESC* FindClassSlot(DEFCLASS* cls, SYMBOL_HN* sname) {
  ...
  for (i = 0; i < cls->slotCount; i++) {
    if (cls->slots[i].slotName->name == sname) return (&cls->slots[i]);
  }
  return (NULL);
}
...
/***************************************************************
  ...
  NOTES        : None
 ***************************************************************/
globle void ClassExistError(void* theEnv, const char* func, const char* cname) {
...
/******************************************************
  ...
  NOTES        : None
 ******************************************************/
globle void PrintClassName(void* theEnv, const char* logicalName,
                           DEFCLASS* theDefclass, intBool linefeedFlag) {
  if ((theDefclass->header.whichModule->theModule !=
       ((struct defmodule*)EnvGetCurrentModule(theEnv))) &&
      (theDefclass->system == 0)) {
    EnvPrintRouter(theEnv, logicalName,
                   EnvGetDefmoduleName(
                       theEnv, theDefclass->header.whichModule->theModule));
    EnvPrintRouter(theEnv, logicalName, "::");
  }
  EnvPrintRouter(theEnv, logicalName, ValueToString(theDefclass->header.name));
  if (linefeedFlag) EnvPrintRouter(theEnv, logicalName, "\n");
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void PrintPackedClassLinks(void* theEnv, const char* logicalName,
                                  const char* title,
...
/***************************************************
  ...
  NOTES        : Assumes the pack structure belongs
                 ...
                 be deallocated
 ***************************************************/
globle void AddClassLink(void* theEnv, PACKED_CLASS_LINKS* src, DEFCLASS* cls,
                         int posn) {
  PACKED_CLASS_LINKS dst;

  dst.classArray =
      (DEFCLASS**)gm2(theEnv, (sizeof(DEFCLASS*) * (src->classCount + 1)));
  ...
  if (posn == -1) {
    ...
    GenCopyMemory(DEFCLASS*, src->classCount - posn, dst.classArray + posn + 1,
                  src->classArray + posn);
...
globle void DeleteSubclassLink(void* theEnv, DEFCLASS* sclass, DEFCLASS* cls) {
  ...
  for (deletedIndex = 0; deletedIndex < src->classCount; deletedIndex++)
    if (src->classArray[deletedIndex] == cls) break;
  if (deletedIndex == src->classCount) return;
  if (src->classCount > 1) {
    dst.classArray =
        (DEFCLASS**)gm2(theEnv, (sizeof(DEFCLASS*) * (src->classCount - 1)));
    if (deletedIndex != 0)
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void DeleteSuperclassLink(void* theEnv, DEFCLASS* sclass,
                                 DEFCLASS* cls) {
  long deletedIndex;
  PACKED_CLASS_LINKS *src, dst;
...
globle DEFCLASS* NewClass(void* theEnv, SYMBOL_HN* className) {
  ...
  InitializeConstructHeader(theEnv, "defclass", (struct constructHeader*)cls,
                            className);
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void DeletePackedClassLinks(void* theEnv, PACKED_CLASS_LINKS* plp,
                                   int deleteTop) {
  if (plp->classCount > 0) {
  ...
  if (deleteTop) rtn_struct(theEnv, packedClassLinks, plp);
}
...
globle void AssignClassID(void* theEnv, DEFCLASS* cls) {
  ...
  if ((DefclassData(theEnv)->MaxClassID % CLASS_ID_MAP_CHUNK) == 0) {
    DefclassData(theEnv)->ClassIDMap = (DEFCLASS**)genrealloc(
        theEnv, (void*)DefclassData(theEnv)->ClassIDMap,
        (unsigned)(DefclassData(theEnv)->MaxClassID * sizeof(DEFCLASS*)),
        (unsigned)((DefclassData(theEnv)->MaxClassID + CLASS_ID_MAP_CHUNK) *
                   sizeof(DEFCLASS*)));
    DefclassData(theEnv)->AvailClassID += (unsigned short)CLASS_ID_MAP_CHUNK;

    for (i = DefclassData(theEnv)->MaxClassID;
         i < (unsigned)(DefclassData(theEnv)->MaxClassID + CLASS_ID_MAP_CHUNK);
         i++)
...
/*********************************************************
  ...
  NOTES        : None
 *********************************************************/
globle SLOT_NAME* AddSlotName(void* theEnv, SYMBOL_HN* slotName, int newid,
                              int usenewid) {
  SLOT_NAME* snp;
...
LOCALE void RemoveDefclass(void* theEnv, void* vcls) {
  ...
  for (i = 0; i < cls->handlerCount; i++) {
    hnd = &cls->handlers[i];
    if (hnd->actions != NULL) ReturnPackedExpression(theEnv, hnd->actions);
    if (hnd->ppForm != NULL)
...
globle void InstallClass(void* theEnv, DEFCLASS* cls, int set) {
  ...
  if (set == FALSE) {
    ...
    for (i = 0; i < cls->handlerCount; i++) {
      ...
      if (hnd->actions != NULL) ExpressionDeinstall(theEnv, hnd->actions);
    }
  } else {
    cls->installed = 1;
...
globle int IsClassBeingUsed(DEFCLASS* cls) {
  ...
  if (cls->busy > 0) return (TRUE);
  for (i = 0; i < cls->directSubclasses.classCount; i++)
    if (IsClassBeingUsed(cls->directSubclasses.classArray[i])) return (TRUE);
  return (FALSE);
}
...
globle int RemoveAllUserClasses(void* theEnv) {
  ...
  while (userClasses != NULL) {
    if (((DEFCLASS*)userClasses)->system == 0) break;
  ...
  while (userClasses != NULL) {
    ...
    if (EnvIsDefclassDeletable(theEnv, ctmp)) {
      ...
      success = FALSE;
      CantDeleteItemErrorMessage(theEnv, "defclass",
                                 EnvGetDefclassName(theEnv, ctmp));
    }
  }
  return (success);
}
...
globle int DeleteClassUAG(void* theEnv, DEFCLASS* cls) {
  ...
  while (cls->directSubclasses.classCount != 0) {
    ...
    if (cls->directSubclasses.classCount == subCount) return (FALSE);
  }
 ...
 ***************************************************/
globle void ReleaseTraversalID(void* theEnv) { DefclassData(theEnv)->CTID--; }
...
/*******************************************************
  ...
  NOTES        : Counts on the fact that the symbol
                 ...
                 multiplied by a prime for a new hash
 *******************************************************/
static unsigned HashSlotName(SYMBOL_HN* sname) {
  unsigned long tally;
...
static void DeassignClassID(void* theEnv, unsigned id) {
  ...
  for (i = id + 1; i < DefclassData(theEnv)->MaxClassID; i++)
    if (DefclassData(theEnv)->ClassIDMap[i] != NULL) return;
  reallocReqd = FALSE;
  while (DefclassData(theEnv)->ClassIDMap[id] == NULL) {
    ...
    if ((DefclassData(theEnv)->MaxClassID % CLASS_ID_MAP_CHUNK) == 0) {
      ...
      if (reallocReqd == FALSE) {
        oldChunk = (unsigned short)(DefclassData(theEnv)->MaxClassID +
                                    CLASS_ID_MAP_CHUNK);
        reallocReqd = TRUE;
      }
    }
  ...
  if (reallocReqd) {
    DefclassData(theEnv)->ClassIDMap =
        (DEFCLASS**)genrealloc(theEnv, (void*)DefclassData(theEnv)->ClassIDMap,
