...
static unsigned DefmessageHandlerWatchPrint(void*, const char*, int,
                                            EXPRESSION*);
static unsigned DefmessageHandlerWatchSupport(
    void*, const char*, const char*, int,
    void (*)(void*, const char*, void*, int), void (*)(void*, int, void*, int),
    EXPRESSION*);
static unsigned WatchClassHandlers(void*, void*, const char*, int, const char*,
                                   int, int,
...
globle void SetupMessageHandlers(void* theEnv) {
  ENTITY_RECORD handlerGetInfo = {"HANDLER_GET",
                                  HANDLER_GET,
                                  0,
                                  1,
                                  1,
  ...
  AllocateEnvironmentData(theEnv, MESSAGE_HANDLER_DATA,
  ...
  InstallPrimitive(theEnv, &MessageHandlerData(theEnv)->HandlerGetInfo,
                   HANDLER_GET);
  InstallPrimitive(theEnv, &MessageHandlerData(theEnv)->HandlerPutInfo,
                   HANDLER_PUT);

#if !RUN_TIME
  MessageHandlerData(theEnv)->INIT_SYMBOL =
      (SYMBOL_HN*)EnvAddSymbol(theEnv, INIT_STRING);
  IncrementSymbolCount(MessageHandlerData(theEnv)->INIT_SYMBOL);

  MessageHandlerData(theEnv)->DELETE_SYMBOL =
      (SYMBOL_HN*)EnvAddSymbol(theEnv, DELETE_STRING);
  IncrementSymbolCount(MessageHandlerData(theEnv)->DELETE_SYMBOL);

  MessageHandlerData(theEnv)->CREATE_SYMBOL =
      (SYMBOL_HN*)EnvAddSymbol(theEnv, CREATE_STRING);
  ...
  MessageHandlerData(theEnv)->SELF_SYMBOL =
      (SYMBOL_HN*)EnvAddSymbol(theEnv, SELF_STRING);
  IncrementSymbolCount(MessageHandlerData(theEnv)->SELF_SYMBOL);

  AddConstruct(theEnv, "defmessage-handler", "defmessage-handlers",
               ParseDefmessageHandler, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
               NULL, NULL);
  EnvDefineFunction2(theEnv, "undefmessage-handler", 'v',
                     PTIEF UndefmessageHandlerCommand,
                     "UndefmessageHandlerCommand", "23w");

#endif

  EnvDefineFunction2(theEnv, "send", 'u', PTIEF SendCommand, "SendCommand",
                     "2*uuw");

#if DEBUGGING_FUNCTIONS
  EnvDefineFunction2(theEnv, "preview-send", 'v', PTIEF PreviewSendCommand,
                     "PreviewSendCommand", "22w");

  EnvDefineFunction2(theEnv, "ppdefmessage-handler", 'v',
                     PTIEF PPDefmessageHandlerCommand,
                     "PPDefmessageHandlerCommand", "23w");
  EnvDefineFunction2(theEnv, "list-defmessage-handlers", 'v',
                     PTIEF ListDefmessageHandlersCommand,
                     "ListDefmessageHandlersCommand", "02w");
#endif

  EnvDefineFunction2(theEnv, "next-handlerp", 'b', PTIEF NextHandlerAvailable,
                     "NextHandlerAvailable", "00");
  FuncSeqOvlFlags(theEnv, "next-handlerp", TRUE, FALSE);
  EnvDefineFunction2(theEnv, "call-next-handler", 'u', PTIEF CallNextHandler,
                     "CallNextHandler", "00");
  ...
  EnvDefineFunction2(theEnv, "dynamic-get", 'u', PTIEF DynamicHandlerGetSlot,
                     "DynamicHandlerGetSlot", "11w");
  EnvDefineFunction2(theEnv, "dynamic-put", 'u', PTIEF DynamicHandlerPutSlot,
                     "DynamicHandlerPutSlot", "1**w");
  EnvDefineFunction2(theEnv, "get", 'u', PTIEF DynamicHandlerGetSlot,
                     "DynamicHandlerGetSlot", "11w");
  EnvDefineFunction2(theEnv, "put", 'u', PTIEF DynamicHandlerPutSlot,
                     "DynamicHandlerPutSlot", "1**w");
#endif

#if DEBUGGING_FUNCTIONS
  AddWatchItem(theEnv, "messages", 0,
               &MessageHandlerData(theEnv)->WatchMessages, 36, NULL, NULL);
  AddWatchItem(theEnv, "message-handlers", 0,
               &MessageHandlerData(theEnv)->WatchHandlers, 35,
...
/*****************************************************
  ...
  NOTES        : None
 *****************************************************/
globle const char* EnvGetDefmessageHandlerName(void* theEnv, void* ptr,
                                               int theIndex) {
#if MAC_XCD
...
/*****************************************************
  ...
  NOTES        : None
 *****************************************************/
globle const char* EnvGetDefmessageHandlerType(void* theEnv, void* ptr,
                                               int theIndex) {
  return (MessageHandlerData(theEnv)
              ->hndquals[((DEFCLASS*)ptr)->handlers[theIndex - 1].type]);
}
...
#pragma unused(theEnv)
  ...
  cls = (DEFCLASS*)ptr;
  if (theIndex == 0) return ((cls->handlers != NULL) ? 1 : 0);
  if (theIndex == cls->handlerCount) return (0);
  return (theIndex + 1);
}
...
/*********************************************************
  ...
  NOTES        : None
 *********************************************************/
globle unsigned EnvGetDefmessageHandlerWatch(void* theEnv, void* theClass,
                                             int theIndex) {
#if MAC_XCD
#pragma unused(theEnv)
#endif
...
/*********************************************************
  ...
  NOTES        : None
 *********************************************************/
globle void EnvSetDefmessageHandlerWatch(void* theEnv, int newState,
                                         void* theClass, int theIndex) {
#if MAC_XCD
#pragma unused(theEnv)
#endif
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle unsigned EnvFindDefmessageHandler(void* theEnv, void* ptr,
                                         const char* hname,
                                         const char* htypestr) {
  unsigned htype;
  SYMBOL_HN* hsym;
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle int EnvIsDefmessageHandlerDeletable(void* theEnv, void* ptr,
                                           int theIndex) {
  DEFCLASS* cls;
  ...
  cls = (DEFCLASS*)ptr;
  if (cls->handlers[theIndex - 1].system == 1) return (FALSE);
...
globle void UndefmessageHandlerCommand(void* theEnv) {
  ...
  if (EnvRtnArgCount(theEnv) == 3) {
    if (EnvArgTypeCheck(theEnv, "undefmessage-handler", 3, SYMBOL, &tmp) ==
        FALSE)
      return;
    tname = DOToString(tmp);
    if (strcmp(tname, "*") == 0) tname = NULL;
  } else
...
globle int EnvUndefmessageHandler(void* theEnv, void* vptr, int mhi) {
  ...
  if (mhi == 0) return (WildDeleteHandler(theEnv, (DEFCLASS*)vptr, NULL, NULL));
  cls = (DEFCLASS*)vptr;
...
globle void PPDefmessageHandlerCommand(void* theEnv) {
  ...
  if (EnvArgTypeCheck(theEnv, "ppdefmessage-handler", 1, SYMBOL, &temp) ==
      FALSE)
    return;
  csym = FindSymbolHN(theEnv, DOToString(temp));
  if (EnvArgTypeCheck(theEnv, "ppdefmessage-handler", 2, SYMBOL, &temp) ==
      FALSE)
    return;
  msym = FindSymbolHN(theEnv, DOToString(temp));
  if (EnvRtnArgCount(theEnv) == 3) {
    if (EnvArgTypeCheck(theEnv, "ppdefmessage-handler", 3, SYMBOL, &temp) ==
        FALSE)
      return;
  ...
  if (((cls == NULL) || (msym == NULL))
          ? TRUE
          : ((hnd = FindHandlerByAddress(cls, msym, (unsigned)mtype)) ==
             NULL)) {
  ...
  if (hnd->ppForm != NULL) PrintInChunks(theEnv, WDISPLAY, hnd->ppForm);
}
...
/********************************************************
  ...
  NOTES        : None
 ********************************************************/
globle const char* EnvGetDefmessageHandlerPPForm(void* theEnv, void* ptr,
                                                 int theIndex) {
#if MAC_XCD
#pragma unused(theEnv)
#endif
...
/*******************************************************************
  ...
  NOTES        : None
 *******************************************************************/
globle void EnvListDefmessageHandlers(void* theEnv, const char* logName,
                                      void* vptr, int inhp) {
  DEFCLASS* cls;
  long cnt;
  ...
  if (vptr != NULL) {
    ...
    plinks.classCount = 1;
    cnt = 0L;
    for (cls = (DEFCLASS*)EnvGetNextDefclass(theEnv, NULL); cls != NULL;
...
/********************************************************************
  ...
  NOTES        : None
 ********************************************************************/
globle void EnvPreviewSend(void* theEnv, const char* logicalName, void* clsptr,
                           const char* msgname) {
  HANDLER_LINK* core;
  SYMBOL_HN* msym;
 ...
 ****************************************************/
globle long DisplayHandlersInLinks(void* theEnv, const char* logName,
                                   PACKED_CLASS_LINKS* plinks, int theIndex) {
  long i;
  long cnt;
  ...
  for (i = 0; i < plinks->classArray[theIndex]->handlerCount; i++)
    PrintHandler(theEnv, logName, &plinks->classArray[theIndex]->handlers[i],
                 TRUE);
  return (cnt);
}
...
static void CreateSystemHandlers(void* theEnv) {
  ...
  NewSystemHandler(theEnv, USER_TYPE_NAME, CREATE_STRING, "(create-instance)",
                   0);
  ...
  NewSystemHandler(theEnv, USER_TYPE_NAME, DIRECT_MODIFY_STRING,
                   "(direct-modify)", 1);
  NewSystemHandler(theEnv, USER_TYPE_NAME, MSG_MODIFY_STRING,
                   "(message-modify)", 1);
  NewSystemHandler(theEnv, USER_TYPE_NAME, DIRECT_DUPLICATE_STRING,
                   "(direct-duplicate)", 2);
  NewSystemHandler(theEnv, USER_TYPE_NAME, MSG_DUPLICATE_STRING,
                   "(message-duplicate)", 2);
}
...
/************************************************************
  ...
  NOTES        : None
 ************************************************************/
static int WildDeleteHandler(void* theEnv, DEFCLASS* cls, SYMBOL_HN* msym,
                             const char* tname) {
  int mtype;

  if (msym == NULL) msym = (SYMBOL_HN*)EnvAddSymbol(theEnv, "*");
  if (tname != NULL) {
    mtype = (int)HandlerType(theEnv, "undefmessage-handler", tname);
    if (mtype == MERROR) return (0);
  } else
    mtype = -1;
  if (cls == NULL) {
    int success = 1;
    ...
    for (cls = (DEFCLASS*)EnvGetNextDefclass(theEnv, NULL); cls != NULL;
         cls = (DEFCLASS*)EnvGetNextDefclass(theEnv, (void*)cls))
      if (DeleteHandler(theEnv, cls, msym, mtype, FALSE) == 0) success = 0;
    return (success);
  }
 ...
 ******************************************************************/
static unsigned DefmessageHandlerWatchAccess(void* theEnv, int code,
                                             unsigned newState,
                                             EXPRESSION* argExprs) {
#if MAC_XCD
#pragma unused(code)
#endif
  if (newState)
    return (DefmessageHandlerWatchSupport(theEnv, "watch", NULL, newState, NULL,
                                          EnvSetDefmessageHandlerWatch,
                                          argExprs));
  else
    return (DefmessageHandlerWatchSupport(theEnv, "unwatch", NULL, newState,
                                          NULL, EnvSetDefmessageHandlerWatch,
                                          argExprs));
}
...
/***********************************************************************
  ...
  NOTES        : Accessory function for AddWatchItem()
 ***********************************************************************/
static unsigned DefmessageHandlerWatchPrint(void* theEnv, const char* logName,
 ...
 *******************************************************/
static unsigned DefmessageHandlerWatchSupport(
    void* theEnv, const char* funcName, const char* logName, int newState,
    void (*printFunc)(void*, const char*, void*, int),
    void (*traceFunc)(void*, int, void*, int), EXPRESSION* argExprs) {
    ...
    while (theModule != NULL) {
      ...
      if (traceFunc == NULL) {
        EnvPrintRouter(theEnv, logName,
                       EnvGetDefmoduleName(theEnv, (void*)theModule));
        EnvPrintRouter(theEnv, logName, ":\n");
      }
  ...
  /* ================================================
     ...
     ================================================ */
  while (argExprs != NULL) {
    if (EvaluateExpression(theEnv, argExprs, &tmpData)) return (FALSE);
    ...
    if (GetNextArgument(argExprs) != NULL) {
      ...
      if (GetNextArgument(argExprs) != NULL) {
        ...
        if ((theType = (int)HandlerType(theEnv, funcName,
                                        DOToString(tmpData))) == MERROR)
          return (FALSE);
      } else
        theType = -1;
    } else {
...
static unsigned WatchClassHandlers(
    void* theEnv, void* theClass, const char* theHandlerStr, int theType,
    const char* logName, int newState, int indentp,
    void (*printFunc)(void*, const char*, void*, int),
  ...
  while (theHandler != 0) {
    if ((theType == -1)
            ? TRUE
            : (theType ==
               (int)((DEFCLASS*)theClass)->handlers[theHandler - 1].type)) {
      if ((theHandlerStr == NULL)
              ? TRUE
              : (strcmp(theHandlerStr, EnvGetDefmessageHandlerName(
                                           theEnv, theClass, theHandler)) ==
                 0)) {
        if (traceFunc != NULL)
          (*traceFunc)(theEnv, newState, theClass, theHandler);
        else {
          if (indentp) EnvPrintRouter(theEnv, logName, "   ");
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void PrintHandlerWatchFlag(void* theEnv, const char* logName,
                                  void* theClass, int theHandler) {
  EnvPrintRouter(theEnv, logName, EnvGetDefclassName(theEnv, theClass));
  EnvPrintRouter(theEnv, logName, " ");
  EnvPrintRouter(theEnv, logName,
                 EnvGetDefmessageHandlerName(theEnv, theClass, theHandler));
  EnvPrintRouter(theEnv, logName, " ");
  EnvPrintRouter(theEnv, logName,
                 EnvGetDefmessageHandlerType(theEnv, theClass, theHandler));
...
globle unsigned FindDefmessageHandler(void* ptr, const char* hname,
                                      const char* htypestr) {
  return EnvFindDefmessageHandler(GetCurrentEnvironment(), ptr, hname,
                                  htypestr);
}

globle const char* GetDefmessageHandlerName(void* ptr, int theIndex) {
  return EnvGetDefmessageHandlerName(GetCurrentEnvironment(), ptr, theIndex);
}

globle const char* GetDefmessageHandlerType(void* ptr, int theIndex) {
  return EnvGetDefmessageHandlerType(GetCurrentEnvironment(), ptr, theIndex);
}

globle int GetNextDefmessageHandler(void* ptr, int theIndex) {
  return EnvGetNextDefmessageHandler(GetCurrentEnvironment(), ptr, theIndex);
}

globle int IsDefmessageHandlerDeletable(void* ptr, int theIndex) {
  return EnvIsDefmessageHandlerDeletable(GetCurrentEnvironment(), ptr,
                                         theIndex);
}

globle int UndefmessageHandler(void* vptr, int mhi) {
  return EnvUndefmessageHandler(GetCurrentEnvironment(), vptr, mhi);
}

#if DEBUGGING_FUNCTIONS

globle const char* GetDefmessageHandlerPPForm(void* ptr, int theIndex) {
  return EnvGetDefmessageHandlerPPForm(GetCurrentEnvironment(), ptr, theIndex);
}

globle unsigned GetDefmessageHandlerWatch(void* theClass, int theIndex) {
  return EnvGetDefmessageHandlerWatch(GetCurrentEnvironment(), theClass,
                                      theIndex);
}

globle void ListDefmessageHandlers(const char* logName, void* vptr, int inhp) {
  EnvListDefmessageHandlers(GetCurrentEnvironment(), logName, vptr, inhp);
}

globle void PreviewSend(const char* logicalName, void* clsptr,
                        const char* msgname) {
  EnvPreviewSend(GetCurrentEnvironment(), logicalName, clsptr, msgname);
}

globle void SetDefmessageHandlerWatch(int newState, void* theClass,
                                      int theIndex) {
  EnvSetDefmessageHandlerWatch(GetCurrentEnvironment(), newState, theClass,
                               theIndex);
}
