...
static int ParseSimpleFacet(void*, const char*, char*, const char*, int,
                            const char*, const char*, const char*, const char*,
                            SYMBOL_HN**);
static intBool ParseDefaultFacet(void*, const char*, char*, SLOT_DESC*);
static void BuildCompositeFacets(void*, SLOT_DESC*, PACKED_CLASS_LINKS*,
                                 const char*, CONSTRAINT_PARSE_RECORD*);
static intBool CheckForFacetConflicts(void*, SLOT_DESC*,
 ...
 ************************************************************/
globle TEMP_SLOT_LINK* ParseSlot(void* theEnv, const char* readSource,
                                 TEMP_SLOT_LINK* slist,
                                 PACKED_CLASS_LINKS* preclist, int multiSlot,
                                 int fieldSpecified) {
  SLOT_DESC* slot;
  ...
  slot = NewSlot(theEnv,
                 (SYMBOL_HN*)GetValue(DefclassData(theEnv)->ObjectParseToken));
  slist = InsertSlot(theEnv, slist, slot);
  if (slist == NULL) return (NULL);
  if (multiSlot) slot->multiple = TRUE;
  if (fieldSpecified) SetBitMap(specbits, FIELD_BIT);
  GetToken(theEnv, readSource, &DefclassData(theEnv)->ObjectParseToken);
  ...
  while (GetType(DefclassData(theEnv)->ObjectParseToken) == LPAREN) {
    ...
    if (GetType(DefclassData(theEnv)->ObjectParseToken) != SYMBOL) {
      SyntaxErrorMessage(theEnv, "defclass slot");
      goto ParseSlotError;
    } else if (strcmp(DOToString(DefclassData(theEnv)->ObjectParseToken),
                      DEFAULT_FACET) == 0) {
      ...
      if (ParseDefaultFacet(theEnv, readSource, specbits, slot) == FALSE)
        goto ParseSlotError;
                      ...
                      ACCESS_FACET) == 0) {
      rtnCode = ParseSimpleFacet(theEnv, readSource, specbits, ACCESS_FACET,
                                 ACCESS_BIT, SLOT_RDWRT_RLN, SLOT_RDONLY_RLN,
                                 SLOT_INIT_RLN, NULL, NULL);
      if (rtnCode == -1)
        goto ParseSlotError;
      else if (rtnCode == 1)
        slot->noWrite = 1;
        ...
        slot->initializeOnly = 1;
                      ...
                      STORAGE_FACET) == 0) {
      rtnCode = ParseSimpleFacet(theEnv, readSource, specbits, STORAGE_FACET,
                                 STORAGE_BIT, SLOT_LOCAL_RLN, SLOT_SHARE_RLN,
                                 NULL, NULL, NULL);
      if (rtnCode == -1) goto ParseSlotError;
      slot->shared = rtnCode;
    } else if (strcmp(DOToString(DefclassData(theEnv)->ObjectParseToken),
                      PROPAGATION_FACET) == 0) {
      rtnCode = ParseSimpleFacet(
          theEnv, readSource, specbits, PROPAGATION_FACET, PROPAGATION_BIT,
          SLOT_INH_RLN, SLOT_NO_INH_RLN, NULL, NULL, NULL);
      if (rtnCode == -1) goto ParseSlotError;
      slot->noInherit = rtnCode;
    } else if (strcmp(DOToString(DefclassData(theEnv)->ObjectParseToken),
                      SOURCE_FACET) == 0) {
      rtnCode = ParseSimpleFacet(theEnv, readSource, specbits, SOURCE_FACET,
                                 SOURCE_BIT, SLOT_EXCLUSIVE_RLN,
      ...
      slot->composite = rtnCode;
                    ...
                    MATCH_FACET) == 0) {
      rtnCode = ParseSimpleFacet(theEnv, readSource, specbits, MATCH_FACET,
      ...
      slot->reactive = rtnCode;
                    ...
                    VISIBILITY_FACET) == 0) {
      rtnCode = ParseSimpleFacet(theEnv, readSource, specbits, VISIBILITY_FACET,
                                 VISIBILITY_BIT, SLOT_PRIVATE_RLN,
                                 SLOT_PUBLIC_RLN, NULL, NULL, NULL);
      if (rtnCode == -1) goto ParseSlotError;
      slot->publicVisibility = rtnCode;
    } else if (strcmp(DOToString(DefclassData(theEnv)->ObjectParseToken),
                      CREATE_ACCESSOR_FACET) == 0) {
      rtnCode =
          ParseSimpleFacet(theEnv, readSource, specbits, CREATE_ACCESSOR_FACET,
                           CREATE_ACCESSOR_BIT, SLOT_READ_RLN, SLOT_WRITE_RLN,
                           SLOT_RDWRT_RLN, SLOT_NONE_RLN, NULL);
      if (rtnCode == -1) goto ParseSlotError;
      if ((rtnCode == 0) || (rtnCode == 2)) slot->createReadAccessor = TRUE;
      if ((rtnCode == 1) || (rtnCode == 2)) slot->createWriteAccessor = TRUE;
    } else if (strcmp(DOToString(DefclassData(theEnv)->ObjectParseToken),
                      OVERRIDE_MSG_FACET) == 0) {
      rtnCode = ParseSimpleFacet(theEnv, readSource, specbits,
    ...
    } else if (StandardConstraint(
                   DOToString(DefclassData(theEnv)->ObjectParseToken))) {
      if (ParseStandardConstraint(
              theEnv, readSource,
              DOToString(DefclassData(theEnv)->ObjectParseToken),
 ...
 **************************************************************/
static SLOT_DESC* NewSlot(void* theEnv, SYMBOL_HN* name) {
  SLOT_DESC* slot;
...
/**********************************************************
  ...
  NOTES        : None
 **********************************************************/
static TEMP_SLOT_LINK* InsertSlot(void* theEnv, TEMP_SLOT_LINK* slist,
                                  SLOT_DESC* slot) {
...
/****************************************************************
  ...
  NOTES        : None
 *****************************************************************/
static int ParseSimpleFacet(void* theEnv, const char* readSource,
                            char* specbits, const char* facetName, int testBit,
                            const char* clearRelation, const char* setRelation,
  ...
  if (DefclassData(theEnv)->ObjectParseToken.type == SF_VARIABLE) {
    if ((varRelation == NULL)
            ? FALSE
            : (strcmp(DOToString(DefclassData(theEnv)->ObjectParseToken),
                      varRelation) == 0))
      rtnCode = 3;
    else
    ...
    if (facetSymbolicValue == NULL) {
      if (strcmp(DOToString(DefclassData(theEnv)->ObjectParseToken),
                 clearRelation) == 0)
        rtnCode = 0;
      else if (strcmp(DOToString(DefclassData(theEnv)->ObjectParseToken),
                      setRelation) == 0)
        rtnCode = 1;
      else if ((alternateRelation == NULL)
                   ? FALSE
                   : (strcmp(DOToString(DefclassData(theEnv)->ObjectParseToken),
                             alternateRelation) == 0))
        rtnCode = 2;
      ...
      rtnCode = 4;
      *facetSymbolicValue =
          (SYMBOL_HN*)DefclassData(theEnv)->ObjectParseToken.value;
    }
  }
  GetToken(theEnv, readSource, &DefclassData(theEnv)->ObjectParseToken);
 ...
 *************************************************************/
static intBool ParseDefaultFacet(void* theEnv, const char* readSource,
                                 char* specbits, SLOT_DESC* slot) {
  EXPRESSION* tmp;
  ...
  error = FALSE;
  tmp = ParseDefault(theEnv, readSource, 1,
                     (int)TestBitMap(specbits, DEFAULT_DYNAMIC_BIT), 0,
 ...
 *************************************************************************/
static void BuildCompositeFacets(void* theEnv, SLOT_DESC* sd,
  ...
  for (i = 1; i < preclist->classCount; i++) {
    ...
    if ((compslot != NULL) ? (compslot->noInherit == 0) : FALSE) break;
  }
  if (compslot != NULL) {
    if ((sd->defaultSpecified == 0) && (compslot->defaultSpecified == 1)) {
      ...
      if (compslot->defaultValue != NULL) {
        if (sd->dynamicDefault) {
          sd->defaultValue = (void*)PackExpression(
              theEnv, (EXPRESSION*)compslot->defaultValue);
          ...
          if (oldValue->type == MULTIFIELD) {
            newValue->value =
                CopyMultifield(theEnv, (struct multifield*)oldValue->value);
          }
          ValueInstall(theEnv, newValue);
        }
      }
    }
    if (TestBitMap(specbits, FIELD_BIT) == 0) sd->multiple = compslot->multiple;
    if (TestBitMap(specbits, STORAGE_BIT) == 0) sd->shared = compslot->shared;
    if (TestBitMap(specbits, ACCESS_BIT) == 0) {
      sd->noWrite = compslot->noWrite;
      sd->initializeOnly = compslot->initializeOnly;
    }
#if DEFRULE_CONSTRUCT
    if (TestBitMap(specbits, MATCH_BIT) == 0) sd->reactive = compslot->reactive;
#endif
    ...
    OverlayConstraint(theEnv, parsedConstraint, sd->constraint,
                      compslot->constraint);
  }
}
...
static intBool CheckForFacetConflicts(
  ...
  if (sd->noDefault && sd->noWrite) {
    ...
    EnvPrintRouter(theEnv, WERROR,
                   "read-only slots must have a default value\n");
    return (FALSE);
  }
  if (sd->noWrite &&
    ...
    EnvPrintRouter(theEnv, WERROR,
                   "read-only slots cannot have a write accessor\n");
    return (FALSE);
  }
...
static intBool EvaluateSlotDefaultValue(void* theEnv, SLOT_DESC* sd,
  ...
  /* ===================================================================
     ...
     =================================================================== */
  if (TestBitMap(specbits, DEFAULT_DYNAMIC_BIT) == 0) sd->dynamicDefault = 0;

  if (sd->noDefault) return (TRUE);

  if (sd->dynamicDefault == 0) {
    if (TestBitMap(specbits, DEFAULT_BIT)) {
      oldce = ExecutingConstruct(theEnv);
      SetExecutingConstruct(theEnv, TRUE);
      olddcc = EnvSetDynamicConstraintChecking(
          theEnv, EnvGetStaticConstraintChecking(theEnv));
      ...
      if (vCode) {
        ...
        if (temp.type == MULTIFIELD) {
          newValue->value =
              CopyMultifield(theEnv, (struct multifield*)temp.value);
        }
      ...
      DeriveDefaultFromConstraints(theEnv, sd->constraint,
                                   (DATA_OBJECT*)sd->defaultValue,
                                   (int)sd->multiple, FALSE);
      ValueInstall(theEnv, (DATA_OBJECT*)sd->defaultValue);
    }
  } else if (EnvGetStaticConstraintChecking(theEnv)) {
    vCode = ConstraintCheckExpressionChain(
        theEnv, (EXPRESSION*)sd->defaultValue, sd->constraint);
    if (vCode != NO_VIOLATION) {
      ...
      ConstraintViolationErrorMessage(theEnv, NULL, NULL, 0, 0, NULL, 0, vCode,
                                      sd->constraint, FALSE);
      return (FALSE);
    }
  }
