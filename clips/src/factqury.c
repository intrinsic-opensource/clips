...
static QUERY_TEMPLATE* DetermineQueryTemplates(void*, EXPRESSION*, const char*,
                                               unsigned*);
...
static int TestForFirstFactInTemplate(void*, struct deftemplate*,
                                      QUERY_TEMPLATE*, unsigned);
static void TestEntireChain(void*, QUERY_TEMPLATE*, unsigned);
static void TestEntireTemplate(void*, struct deftemplate*, QUERY_TEMPLATE*,
                               unsigned);
...
globle void SetupFactQuery(void* theEnv) {
  AllocateEnvironmentData(theEnv, FACT_QUERY_DATA, sizeof(struct factQueryData),
                          NULL);

#if RUN_TIME
  FactQueryData(theEnv)->QUERY_DELIMETER_SYMBOL =
      FindSymbolHN(theEnv, QUERY_DELIMETER_STRING);
#endif

#if !RUN_TIME
  FactQueryData(theEnv)->QUERY_DELIMETER_SYMBOL =
      (SYMBOL_HN*)EnvAddSymbol(theEnv, QUERY_DELIMETER_STRING);
  IncrementSymbolCount(FactQueryData(theEnv)->QUERY_DELIMETER_SYMBOL);

  EnvDefineFunction2(theEnv, "(query-fact)", 'u', PTIEF GetQueryFact,
                     "GetQueryFact", NULL);

  EnvDefineFunction2(theEnv, "(query-fact-slot)", 'u', PTIEF GetQueryFactSlot,
                     "GetQueryFactSlot", NULL);

  EnvDefineFunction2(theEnv, "any-factp", 'b', PTIEF AnyFacts, "AnyFacts",
                     NULL);
  AddFunctionParser(theEnv, "any-factp", FactParseQueryNoAction);

  EnvDefineFunction2(theEnv, "find-fact", 'm', PTIEF QueryFindFact,
                     "QueryFindFact", NULL);
  AddFunctionParser(theEnv, "find-fact", FactParseQueryNoAction);

  EnvDefineFunction2(theEnv, "find-all-facts", 'm', PTIEF QueryFindAllFacts,
                     "QueryFindAllFacts", NULL);
  AddFunctionParser(theEnv, "find-all-facts", FactParseQueryNoAction);

  EnvDefineFunction2(theEnv, "do-for-fact", 'u', PTIEF QueryDoForFact,
                     "QueryDoForFact", NULL);
  AddFunctionParser(theEnv, "do-for-fact", FactParseQueryAction);

  EnvDefineFunction2(theEnv, "do-for-all-facts", 'u', PTIEF QueryDoForAllFacts,
                     "QueryDoForAllFacts", NULL);
...
globle void GetQueryFact(void* theEnv, DATA_OBJECT* result) {
  ...
  result->value =
      core->solns[ValueToInteger(GetpValue(GetFirstArgument()->nextArg))];
}
...
globle void GetQueryFactSlot(void* theEnv, DATA_OBJECT* result) {
  ...
  varSlot = ((SYMBOL_HN*)GetFirstArgument()->nextArg->nextArg->nextArg->value)
                ->contents;
  ...
  if (TestForFirstInChain(theEnv, qtemplates, 0) == TRUE) {
    ...
    for (i = 1; i <= rcnt; i++) {
      ...
      SetMFValue(result->value, i,
                 FactQueryData(theEnv)->QueryCore->solns[i - 1]);
    }
  } else
    result->value = (void*)EnvCreateMultifield(theEnv, 0L);
  FactQueryData(theEnv)->AbortQuery = FALSE;
...
globle void QueryFindAllFacts(void* theEnv, DATA_OBJECT* result) {
  ...
  FactQueryData(theEnv)->AbortQuery = FALSE;
  result->value = (void*)EnvCreateMultifield(
      theEnv, FactQueryData(theEnv)->QueryCore->soln_cnt * rcnt);
...
globle void QueryDoForFact(void* theEnv, DATA_OBJECT* result) {
  ...
  result->type = SYMBOL;
  result->value = EnvFalseSymbol(theEnv);
  qtemplates = DetermineQueryTemplates(
      theEnv, GetFirstArgument()->nextArg->nextArg, "do-for-fact", &rcnt);
  ...
  if (TestForFirstInChain(theEnv, qtemplates, 0) == TRUE) {
    ...
    EvaluateExpression(theEnv, FactQueryData(theEnv)->QueryCore->action,
                       result);
...
globle void QueryDoForAllFacts(void* theEnv, DATA_OBJECT* result) {
  ...
  qtemplates = DetermineQueryTemplates(
      theEnv, GetFirstArgument()->nextArg->nextArg, "do-for-all-facts", &rcnt);
  if (qtemplates == NULL) return;
...
globle void DelayedQueryDoForAllFacts(void* theEnv, DATA_OBJECT* result) {
  ...
  result->value = EnvFalseSymbol(theEnv);
  qtemplates =
      DetermineQueryTemplates(theEnv, GetFirstArgument()->nextArg->nextArg,
...
static QUERY_CORE* FindQueryCore(void* theEnv, int depth) {
  ...
  if (depth == 0) return (FactQueryData(theEnv)->QueryCore);
  qptr = FactQueryData(theEnv)->QueryCoreStack;
  while (depth > 1) {
    qptr = qptr->nxt;
    depth--;
 ...
 **********************************************************/
static QUERY_TEMPLATE* DetermineQueryTemplates(void* theEnv,
  ...
  *rcnt = 0;
  while (templateExp != NULL) {
    ...
    if ((temp.type == SYMBOL) &&
        (temp.value == (void*)FactQueryData(theEnv)->QUERY_DELIMETER_SYMBOL)) {
      new_list = TRUE;
      (*rcnt)++;
...
/*************************************************************
  ...
  NOTES        : None
 *************************************************************/
static QUERY_TEMPLATE* FormChain(void* theEnv, const char* func,
                                 DATA_OBJECT* val) {
  ...
  if (val->type == SYMBOL) {
    ...
    templatePtr = (struct deftemplate*)FindImportedConstruct(
        theEnv, "deftemplate", NULL, DOPToString(val), &count, TRUE, NULL);
    if (templatePtr == NULL) {
      CantFindItemInFunctionErrorMessage(theEnv, "deftemplate",
                                         DOPToString(val), func);
      return (NULL);
    }
  ...
  if (val->type == MULTIFIELD) {
    ...
    for (i = GetpDOBegin(val); i <= end; i++) {
      if (GetMFType(val->value, i) == SYMBOL) {
        ...
        templatePtr = (struct deftemplate*)FindImportedConstruct(
            theEnv, "deftemplate", NULL, templateName, &count, TRUE, NULL);
...
/******************************************************
  ...
  NOTES        : None
 ******************************************************/
static void DeleteQueryTemplates(void* theEnv, QUERY_TEMPLATE* qlist) {
  QUERY_TEMPLATE* tmp;
...
/************************************************************
  ...
  NOTES        : None
 ************************************************************/
static int TestForFirstInChain(void* theEnv, QUERY_TEMPLATE* qchain,
                               unsigned indx) {
  QUERY_TEMPLATE* qptr;
...
/*****************************************************************
  ...
  NOTES        : None
 *****************************************************************/
static int TestForFirstFactInTemplate(void* theEnv,
                                      struct deftemplate* templatePtr,
  ...
  while (theFact != NULL) {
    ...
    if (qchain->nxt != NULL) {
      ...
      theFact->factHeader.busyCount++;
      EvaluateExpression(theEnv, FactQueryData(theEnv)->QueryCore->query,
                         &temp);
      ...
      if ((temp.type != SYMBOL) ? TRUE : (temp.value != EnvFalseSymbol(theEnv)))
        break;
    }
...
endTest:
  ...
  if (theFact != NULL)
    return (((EvaluationData(theEnv)->HaltExecution == TRUE) ||
             (FactQueryData(theEnv)->AbortQuery == TRUE))
                ? FALSE
                : TRUE);
...
/************************************************************
  ...
  NOTES        : None
 ************************************************************/
static void TestEntireChain(void* theEnv, QUERY_TEMPLATE* qchain,
                            unsigned indx) {
  QUERY_TEMPLATE* qptr;
...
static void PopQuerySoln(void* theEnv) {
  ...
  rm(theEnv, (void*)FactQueryData(theEnv)->QueryCore->soln_bottom->soln,
     (sizeof(struct fact*) * FactQueryData(theEnv)->QueryCore->soln_size));
  rm(theEnv, (void*)FactQueryData(theEnv)->QueryCore->soln_bottom,
     sizeof(QUERY_SOLN));
}
