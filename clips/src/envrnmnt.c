...
static void* CreateEnvironmentDriver(struct symbolHashNode**,
...
globle intBool AllocateEnvironmentData(void* vtheEnvironment,
  ...
  if (size <= 0) {
    printf(
        "\n[ENVRNMNT1] Environment data position %d allocated with size of 0 "
        "or less.\n",
        position);
    return (FALSE);
  }
  ...
  if (position >= MAXIMUM_ENVIRONMENT_POSITIONS) {
    printf(
        "\n[ENVRNMNT2] Environment data position %d exceeds the maximum "
        "allowed.\n",
        position);
    return (FALSE);
  }
...
globle intBool DeallocateEnvironmentData() {
  ...
  for (i = 0; i < SIZE_ENVIRONMENT_HASH; i++) {
    for (theEnvironment = EnvironmentHashTable[i]; theEnvironment != NULL;) {
...
static void InitializeEnvironmentHashTable() {
  ...
  EnvironmentHashTable = (struct environmentData**)malloc(
      sizeof(struct environmentData*) * SIZE_ENVIRONMENT_HASH);
...
/*********************************************/
static void AddHashedEnvironment(struct environmentData* theEnvironment) {
...
/***************************************************/
static intBool RemoveHashedEnvironment(struct environmentData* theEnvironment) {
  unsigned long hashValue;
  struct environmentData *hptr, *prev;

  hashValue = theEnvironment->environmentIndex % SIZE_ENVIRONMENT_HASH;

  for (hptr = EnvironmentHashTable[hashValue], prev = NULL; hptr != NULL;
       hptr = hptr->next) {
...
/**********************************************************/
static struct environmentData* FindEnvironment(unsigned long environmentIndex) {
  struct environmentData* theEnvironment;
  unsigned long hashValue;

  hashValue = environmentIndex % SIZE_ENVIRONMENT_HASH;

  for (theEnvironment = EnvironmentHashTable[hashValue]; theEnvironment != NULL;
...
globle void* CreateRuntimeEnvironment(struct symbolHashNode** symbolTable,
  ...
  return CreateEnvironmentDriver(symbolTable, floatTable, integerTable,
                                 bitmapTable, NULL);
}
...
globle void* CreateEnvironmentDriver(
    struct symbolHashNode** symbolTable, struct floatHashNode** floatTable,
    struct integerHashNode** integerTable, struct bitMapHashNode** bitmapTable,
  ...
  theData = malloc(sizeof(void (*)(struct environmentData*)) *
  ...
  EnvInitializeEnvironment(theEnvironment, symbolTable, floatTable,
...
/********************************************/
globle void* GetCurrentEnvironment() { return (CurrentEnvironment); }
...
globle void* SetEnvironmentFunctionContext(void* theEnvironment,
  ...
  oldFunctionContext =
      ((struct environmentData*)theEnvironment)->functionContext;
...
globle void* SetEnvironmentCallbackContext(void* theEnvironment,
  ...
  oldCallbackContext =
      ((struct environmentData*)theEnvironment)->callbackContext;
...
/**************************************************/
globle intBool AddEnvironmentCleanupFunction(void* vtheEnv, const char* name,
  ...
  newPtr = (struct environmentCleanupFunction*)malloc(
...
/**************************************************/
static void RemoveEnvironmentCleanupFunctions(struct environmentData* theEnv) {
