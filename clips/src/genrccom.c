...
globle void SetupGenericFunctions(void* theEnv) {
  ENTITY_RECORD genericEntityRecord = {"GCALL",
                                       GCALL,
                                       0,
                                       0,
                                       1,
                                       ...
                                       EvaluateGenericCall,
                                       NULL,
                                       DecrementGenericBusyCount,
                                       IncrementGenericBusyCount,
                                       NULL,
                                       NULL,
                                       NULL,
                                       NULL,
  ...
  AllocateEnvironmentData(theEnv, DEFGENERIC_DATA,
  ...
  DefgenericData(theEnv)->DefgenericConstruct = AddConstruct(
      theEnv, "defgeneric", "defgenerics",
#if (!BLOAD_ONLY) && (!RUN_TIME)
      ParseDefgeneric,
#else
      NULL,
#endif
      EnvFindDefgeneric, GetConstructNamePointer, GetConstructPPForm,
      GetConstructModuleItem, EnvGetNextDefgeneric, SetNextConstruct,
      EnvIsDefgenericDeletable, EnvUndefgeneric,
#if (!BLOAD_ONLY) && (!RUN_TIME)
      RemoveDefgeneric
#else
      NULL
...
#endif
  AddConstruct(theEnv, "defmethod", "defmethods", ParseDefmethod, NULL, NULL,
  ...
  EnvDefineFunction2(theEnv, "undefgeneric", 'v', PTIEF UndefgenericCommand,
                     "UndefgenericCommand", "11w");
  EnvDefineFunction2(theEnv, "undefmethod", 'v', PTIEF UndefmethodCommand,
                     "UndefmethodCommand", "22*wg");
#endif

  EnvDefineFunction2(theEnv, "call-next-method", 'u', PTIEF CallNextMethod,
                     "CallNextMethod", "00");
  FuncSeqOvlFlags(theEnv, "call-next-method", TRUE, FALSE);
  EnvDefineFunction2(theEnv, "call-specific-method", 'u',
                     PTIEF CallSpecificMethod, "CallSpecificMethod", "2**wi");
  FuncSeqOvlFlags(theEnv, "call-specific-method", TRUE, FALSE);
  EnvDefineFunction2(theEnv, "override-next-method", 'u',
                     PTIEF OverrideNextMethod, "OverrideNextMethod", NULL);
  FuncSeqOvlFlags(theEnv, "override-next-method", TRUE, FALSE);
  EnvDefineFunction2(theEnv, "next-methodp", 'b', PTIEF NextMethodP,
                     "NextMethodP", "00");
  FuncSeqOvlFlags(theEnv, "next-methodp", TRUE, FALSE);

  EnvDefineFunction2(theEnv, "(gnrc-current-arg)", 'u',
                     PTIEF GetGenericCurrentArgument,
                     "GetGenericCurrentArgument", NULL);

#if DEBUGGING_FUNCTIONS
  EnvDefineFunction2(theEnv, "ppdefgeneric", 'v', PTIEF PPDefgenericCommand,
                     "PPDefgenericCommand", "11w");
  EnvDefineFunction2(theEnv, "list-defgenerics", 'v',
                     PTIEF ListDefgenericsCommand, "ListDefgenericsCommand",
                     "01");
  EnvDefineFunction2(theEnv, "ppdefmethod", 'v', PTIEF PPDefmethodCommand,
                     "PPDefmethodCommand", "22*wi");
  EnvDefineFunction2(theEnv, "list-defmethods", 'v',
                     PTIEF ListDefmethodsCommand, "ListDefmethodsCommand",
                     "01w");
  EnvDefineFunction2(theEnv, "preview-generic", 'v', PTIEF PreviewGeneric,
                     "PreviewGeneric", "1**w");
#endif

  EnvDefineFunction2(theEnv, "get-defgeneric-list", 'm',
                     PTIEF GetDefgenericListFunction,
                     "GetDefgenericListFunction", "01");
  EnvDefineFunction2(theEnv, "get-defmethod-list", 'm',
                     PTIEF GetDefmethodListCommand, "GetDefmethodListCommand",
                     "01w");
  EnvDefineFunction2(theEnv, "get-method-restrictions", 'm',
                     PTIEF GetMethodRestrictionsCommand,
                     "GetMethodRestrictionsCommand", "22iw");
  EnvDefineFunction2(theEnv, "defgeneric-module", 'w',
                     PTIEF GetDefgenericModuleCommand,
                     "GetDefgenericModuleCommand", "11w");

#if OBJECT_SYSTEM
  EnvDefineFunction2(theEnv, "type", 'u', PTIEF ClassCommand, "ClassCommand",
                     "11u");
#else
  EnvDefineFunction2(theEnv, "type", 'u', PTIEF TypeCommand, "TypeCommand",
                     "11u");
#endif
...
#if DEBUGGING_FUNCTIONS
  AddWatchItem(theEnv, "generic-functions", 0,
               &DefgenericData(theEnv)->WatchGenerics, 34,
...
/****************************************************/
static void DestroyDefgenericAction(void* theEnv,
...
#pragma unused(buffer)
  ...
  if (theDefgeneric->mcnt != 0)
    rm(theEnv, (void*)theDefgeneric->methods,
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void* EnvFindDefgeneric(void* theEnv, const char* genericModuleAndName) {
  return (FindNamedConstructInModuleOrImports(
      theEnv, genericModuleAndName,
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void* EnvFindDefgenericInModule(void* theEnv,
                                       const char* genericModuleAndName) {
  return (
      FindNamedConstructInModule(theEnv, genericModuleAndName,
                                 DefgenericData(theEnv)->DefgenericConstruct));
}
...
globle DEFGENERIC* LookupDefgenericByMdlOrScope(void* theEnv,
  ...
  return ((DEFGENERIC*)LookupConstruct(
      theEnv, DefgenericData(theEnv)->DefgenericConstruct, defgenericName,
      TRUE));
}
...
globle void* EnvGetNextDefgeneric(void* theEnv, void* ptr) {
  return ((void*)GetNextConstructItem(
      theEnv, (struct constructHeader*)ptr,
      DefgenericData(theEnv)->DefgenericModuleIndex));
}
...
#pragma unused(theEnv)
  ...
  if (theIndex == 0) {
    if (gfunc->methods != NULL) return (gfunc->methods[0].index);
    return (0);
  }
  mi = FindMethodByIndex(gfunc, theIndex);
  if ((mi + 1) == gfunc->mcnt) return (0);
...
globle int EnvIsDefmethodDeletable(void* theEnv, void* ptr, long theIndex) {
  ...
  if (((DEFGENERIC*)ptr)
          ->methods[FindMethodByIndex((DEFGENERIC*)ptr, theIndex)]
          .system)
    return (FALSE);
...
globle void UndefgenericCommand(void* theEnv) {
  UndefconstructCommand(theEnv, "undefgeneric",
                        DefgenericData(theEnv)->DefgenericConstruct);
}
...
globle void* GetDefgenericModuleCommand(void* theEnv) {
  return (
      GetConstructModuleCommand(theEnv, "defgeneric-module",
                                DefgenericData(theEnv)->DefgenericConstruct));
}
...
globle void UndefmethodCommand(void* theEnv) {
  ...
  if (EnvArgTypeCheck(theEnv, "undefmethod", 1, SYMBOL, &temp) == FALSE) return;
...
globle intBool EnvUndefgeneric(void* theEnv, void* vptr) {
  ...
  if (gfunc == NULL) {
    if (ClearDefmethods(theEnv) == FALSE) success = FALSE;
    if (ClearDefgenerics(theEnv) == FALSE) success = FALSE;
    return (success);
  }
  if (EnvIsDefgenericDeletable(theEnv, vptr) == FALSE) return (FALSE);
...
/*****************************************************
  ...
  NOTES        : Terminating '\n' not written
 *****************************************************/
globle void EnvGetDefmethodDescription(void* theEnv, char* buf, size_t buflen,
                                       void* ptr, long theIndex) {
  DEFGENERIC* gfunc;
  long mi;
#if MAC_XCD
#pragma unused(theEnv)
...
/*********************************************************
  ...
  NOTES        : None
 *********************************************************/
globle void EnvSetDefgenericWatch(void* theEnv, unsigned newState,
                                  void* theGeneric) {
#if MAC_XCD
#pragma unused(theEnv)
#endif
...
/*********************************************************
  ...
  NOTES        : None
 *********************************************************/
globle unsigned EnvGetDefmethodWatch(void* theEnv, void* theGeneric,
                                     long theIndex) {
  DEFGENERIC* gfunc;
  long mi;
#if MAC_XCD
...
/*********************************************************
  ...
  NOTES        : None
 *********************************************************/
globle void EnvSetDefmethodWatch(void* theEnv, unsigned newState,
                                 void* theGeneric, long theIndex) {
  DEFGENERIC* gfunc;
  long mi;
#if MAC_XCD
#pragma unused(theEnv)
...
globle void PPDefgenericCommand(void* theEnv) {
  PPConstructCommand(theEnv, "ppdefgeneric",
                     DefgenericData(theEnv)->DefgenericConstruct);
}
...
globle void PPDefmethodCommand(void* theEnv) {
  ...
  if (EnvArgTypeCheck(theEnv, "ppdefmethod", 1, SYMBOL, &temp) == FALSE) return;
...
globle void ListDefmethodsCommand(void* theEnv) {
  ...
  if (EnvRtnArgCount(theEnv) == 0)
    ...
    if (gfunc != NULL) EnvListDefmethods(theEnv, WDISPLAY, (void*)gfunc);
  }
}
...
/***************************************************************
  ...
  NOTES        : None
 ***************************************************************/
globle const char* EnvGetDefmethodPPForm(void* theEnv, void* ptr,
                                         long theIndex) {
  DEFGENERIC* gfunc;
  int mi;
#if MAC_XCD
...
globle void ListDefgenericsCommand(void* theEnv) {
  ListConstructCommand(theEnv, "list-defgenerics",
                       DefgenericData(theEnv)->DefgenericConstruct);
}
...
/***************************************************
  ...
  NOTES        : C Interface
 ***************************************************/
globle void EnvListDefgenerics(void* theEnv, const char* logicalName,
                               struct defmodule* theModule) {
  ListConstruct(theEnv, DefgenericData(theEnv)->DefgenericConstruct,
...
/******************************************************
  ...
  NOTES        : None
 ******************************************************/
globle void EnvListDefmethods(void* theEnv, const char* logicalName,
                              void* vptr) {
  DEFGENERIC* gfunc;
  long count;
  if (vptr != NULL)
    ...
    count = 0L;
    for (gfunc = (DEFGENERIC*)EnvGetNextDefgeneric(theEnv, NULL); gfunc != NULL;
...
globle void GetDefgenericListFunction(void* theEnv, DATA_OBJECT* returnValue) {
  GetConstructListFunction(theEnv, "get-defgeneric-list", returnValue,
                           DefgenericData(theEnv)->DefgenericConstruct);
}
 ...
 ***************************************************************/
globle void EnvGetDefgenericList(void* theEnv, DATA_OBJECT* returnValue,
                                 struct defmodule* theModule) {
  GetConstructList(theEnv, returnValue,
                   DefgenericData(theEnv)->DefgenericConstruct, theModule);
}
...
globle void GetDefmethodListCommand(void* theEnv, DATA_OBJECT_PTR returnValue) {
  ...
  if (EnvRtnArgCount(theEnv) == 0)
    ...
    if (EnvArgTypeCheck(theEnv, "get-defmethod-list", 1, SYMBOL, &temp) ==
        FALSE) {
...
/***********************************************************
  ...
  NOTES        : None
 ***********************************************************/
globle void EnvGetDefmethodList(void* theEnv, void* vgfunc,
  ...
  if (vgfunc != NULL) {
    ...
    svnxt = (gfunc != NULL)
                ? (DEFGENERIC*)EnvGetNextDefgeneric(theEnv, (void*)gfunc)
                : NULL;
  }
  count = 0;
  for (svg = gfunc; gfunc != NULL;
       gfunc = (DEFGENERIC*)EnvGetNextDefgeneric(theEnv, (void*)gfunc))
  ...
  for (gfunc = svg, i = 1; gfunc != NULL;
    ...
    for (j = 0; j < gfunc->mcnt; j++) {
      ...
      SetMFValue(theList, i++,
                 EnvAddLong(theEnv, (long long)gfunc->methods[j].index));
    }
  }
  if (svg != NULL) SetNextDefgeneric((void*)svg, (void*)svnxt);
}
...
/***********************************************************************************
  ...
  NOTES        : Syntax: (get-method-restrictions <generic-function>
 <method-index>)
 ***********************************************************************************/
globle void GetMethodRestrictionsCommand(void* theEnv, DATA_OBJECT* result) {
  ...
  gfunc =
      CheckGenericExists(theEnv, "get-method-restrictions", DOToString(temp));
  if (gfunc == NULL) {
    EnvSetMultifieldErrorValue(theEnv, result);
    return;
  }
  if (EnvArgTypeCheck(theEnv, "get-method-restrictions", 2, INTEGER, &temp) ==
      FALSE) {
    EnvSetMultifieldErrorValue(theEnv, result);
    return;
  }
  if (CheckMethodExists(theEnv, "get-method-restrictions", gfunc,
                        (long)DOToLong(temp)) == -1) {
    EnvSetMultifieldErrorValue(theEnv, result);
    return;
  }
  EnvGetMethodRestrictions(theEnv, (void*)gfunc, (unsigned)DOToLong(temp),
                           result);
}
 ...
 ***********************************************************************/
globle void EnvGetMethodRestrictions(void* theEnv, void* vgfunc, long mi,
                                     DATA_OBJECT* result) {
  short i, j;
  DEFMETHOD* meth;
  RESTRICTION* rptr;
  ...
  meth = ((DEFGENERIC*)vgfunc)->methods +
         FindMethodByIndex((DEFGENERIC*)vgfunc, mi);
  count = 3;
  ...
  for (i = 0; i < meth->restrictionCount; i++) {
    ...
    SetMFValue(
        theList, roffset++,
        (rptr->query != NULL) ? EnvTrueSymbol(theEnv) : EnvFalseSymbol(theEnv));
    ...
    for (j = 0; j < rptr->tcnt; j++) {
      ...
      SetMFValue(
          theList, roffset++,
          EnvAddSymbol(theEnv, EnvGetDefclassName(theEnv, rptr->types[j])));
#else
      SetMFValue(theList, roffset++,
                 EnvAddSymbol(
                     theEnv, TypeName(theEnv, ValueToInteger(rptr->types[j]))));
#endif
    }
  }
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void PrintGenericCall(void* theEnv, const char* logName, void* value) {
#if DEVELOPER
...
/*******************************************************
  ...
  NOTES        : None
 *******************************************************/
static intBool EvaluateGenericCall(void* theEnv, void* value,
                                   DATA_OBJECT* result) {
  GenericDispatch(theEnv, (DEFGENERIC*)value, NULL, NULL, GetFirstArgument(),
                  result);
...
static void DecrementGenericBusyCount(void* theEnv, void* value) {
  /* ==============================================
     ...
     ============================================== */
  if (!ConstructData(theEnv)->ClearInProgress) ((DEFGENERIC*)value)->busy--;
}
...
/**********************************************************************
  ...
  NOTES        : None
 **********************************************************************/
static void SaveDefgenerics(void* theEnv, void* theModule,
                            const char* logName) {
  SaveConstruct(theEnv, theModule, logName,
                DefgenericData(theEnv)->DefgenericConstruct);
}
...
/**********************************************************************
  ...
  NOTES        : None
 **********************************************************************/
static void SaveDefmethods(void* theEnv, void* theModule, const char* logName) {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void SaveDefmethodsForDefgeneric(void* theEnv,
...
/******************************************************
  ...
  NOTES        : None
 ******************************************************/
static long ListMethodsForGeneric(void* theEnv, const char* logicalName,
                                  DEFGENERIC* gfunc) {
  long gi;
  char buf[256];
 ...
 ******************************************************************/
static unsigned DefgenericWatchAccess(void* theEnv, int code, unsigned newState,
                                      EXPRESSION* argExprs) {
#if MAC_XCD
#pragma unused(code)
#endif

  return (ConstructSetWatchAccess(
      theEnv, DefgenericData(theEnv)->DefgenericConstruct, newState, argExprs,
...
/***********************************************************************
  ...
  NOTES        : Accessory function for AddWatchItem()
 ***********************************************************************/
static unsigned DefgenericWatchPrint(void* theEnv, const char* logName,
                                     int code, EXPRESSION* argExprs) {
#if MAC_XCD
#pragma unused(code)
#endif

  return (ConstructPrintWatchAccess(
      theEnv, DefgenericData(theEnv)->DefgenericConstruct, logName, argExprs,
      EnvGetDefgenericWatch, EnvSetDefgenericWatch));
}
...
/******************************************************************
  ...
  NOTES        : Accessory function for AddWatchItem()
 ******************************************************************/
static unsigned DefmethodWatchAccess(void* theEnv, int code, unsigned newState,
                                     EXPRESSION* argExprs) {
#if MAC_XCD
#pragma unused(code)
#endif
  if (newState)
    return (DefmethodWatchSupport(theEnv, "watch", NULL, newState, NULL,
                                  EnvSetDefmethodWatch, argExprs));
  else
    return (DefmethodWatchSupport(theEnv, "unwatch", NULL, newState, NULL,
                                  EnvSetDefmethodWatch, argExprs));
}
...
/***********************************************************************
  ...
  NOTES        : Accessory function for AddWatchItem()
 ***********************************************************************/
static unsigned DefmethodWatchPrint(void* theEnv, const char* logName, int code,
                                    EXPRESSION* argExprs) {
#if MAC_XCD
#pragma unused(code)
#endif
...
static unsigned DefmethodWatchSupport(
    void* theEnv, const char* funcName, const char* logName, unsigned newState,
    void (*printFunc)(void*, const char*, void*, long),
    void (*traceFunc)(void*, unsigned, void*, long), EXPRESSION* argExprs) {
  void* theGeneric;
  ...
  /* =========================================
     ...
     ========================================= */
  while (argExprs != NULL) {
    if (EvaluateExpression(theEnv, argExprs, &genericName)) return (FALSE);
    if ((genericName.type != SYMBOL)
            ? TRUE
            : ((theGeneric = (void*)LookupDefgenericByMdlOrScope(
                    theEnv, DOToString(genericName))) == NULL)) {
    ...
    if (GetNextArgument(argExprs) == NULL)
      ...
      argIndex++;
      if (EvaluateExpression(theEnv, argExprs, &methodIndex)) return (FALSE);
      if ((methodIndex.type != INTEGER)
              ? FALSE
              : ((DOToLong(methodIndex) <= 0)
                     ? FALSE
                     : (FindMethodByIndex((DEFGENERIC*)theGeneric, theMethod) !=
                        -1)))
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void PrintMethodWatchFlag(void* theEnv, const char* logName,
                                 void* theGeneric, long theMethod) {
  char buf[60];
 ...
 ***************************************************/
globle void TypeCommand(void* theEnv, DATA_OBJECT* result) {
...
globle SYMBOL_HN* GetDefgenericNamePointer(void* theDefgeneric) {
  return GetConstructNamePointer((struct constructHeader*)theDefgeneric);
}

globle void SetNextDefgeneric(void* theDefgeneric, void* targetDefgeneric) {
...
globle const char* EnvDefgenericModule(void* theEnv, void* theDefgeneric) {
  return GetConstructModuleName((struct constructHeader*)theDefgeneric);
}

globle const char* EnvGetDefgenericName(void* theEnv, void* theDefgeneric) {
  return GetConstructNameString((struct constructHeader*)theDefgeneric);
}

globle const char* EnvGetDefgenericPPForm(void* theEnv, void* theDefgeneric) {
  return GetConstructPPForm(theEnv, (struct constructHeader*)theDefgeneric);
}

globle SYMBOL_HN* EnvGetDefgenericNamePointer(void* theEnv,
                                              void* theDefgeneric) {
  return GetConstructNamePointer((struct constructHeader*)theDefgeneric);
}

globle void EnvSetDefgenericPPForm(void* theEnv, void* theDefgeneric,
...
globle void GetDefmethodDescription(char* buf, int buflen, void* ptr,
                                    long theIndex) {
  EnvGetDefmethodDescription(GetCurrentEnvironment(), buf, buflen, ptr,
                             theIndex);
}
