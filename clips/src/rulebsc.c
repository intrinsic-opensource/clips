...
globle void DefruleBasicCommands(void* theEnv) {
  ...
  AddWatchItem(theEnv, "rules", 0, &DefruleData(theEnv)->WatchRules, 70,
               DefruleWatchAccess, DefruleWatchPrint);
#endif

#if !RUN_TIME
  EnvDefineFunction2(theEnv, "get-defrule-list", 'm',
                     PTIEF GetDefruleListFunction, "GetDefruleListFunction",
                     "01w");
  EnvDefineFunction2(theEnv, "undefrule", 'v', PTIEF UndefruleCommand,
                     "UndefruleCommand", "11w");
  EnvDefineFunction2(theEnv, "defrule-module", 'w', PTIEF DefruleModuleFunction,
                     "DefruleModuleFunction", "11w");

#if DEBUGGING_FUNCTIONS
  EnvDefineFunction2(theEnv, "rules", 'v', PTIEF ListDefrulesCommand,
                     "ListDefrulesCommand", "01w");
  EnvDefineFunction2(theEnv, "list-defrules", 'v', PTIEF ListDefrulesCommand,
                     "ListDefrulesCommand", "01w");
  EnvDefineFunction2(theEnv, "ppdefrule", 'v', PTIEF PPDefruleCommand,
                     "PPDefruleCommand", "11w");
#endif
...
/*   cleared.                                        */
/*****************************************************/
static void ResetDefrules(void* theEnv) {
...
/* ResetDefrulesPrime:                      */
/*****************************************************/
static void ResetDefrulesPrime(void* theEnv) {
  struct joinLink* theLink;
  struct partialMatch* notParent;
  ...
  for (theLink = DefruleData(theEnv)->LeftPrimeJoins; theLink != NULL;
    ...
    if ((theLink->join->patternIsNegated || theLink->join->joinFromTheRight) &&
      ...
      if (theLink->join->secondaryNetworkTest != NULL) {
        if (EvaluateSecondaryNetworkTest(theEnv, notParent, theLink->join) ==
            FALSE) {
          continue;
        }
      }
...
/******************************************************************/
static int ClearDefrulesReady(void* theEnv) {
...
/***************************************************************/
static void ClearDefrules(void* theEnv) {
  struct defmodule* theModule;
...
/**************************************/
static void SaveDefrules(void* theEnv, void* theModule,
...
globle void UndefruleCommand(void* theEnv) {
  UndefconstructCommand(theEnv, "undefrule",
                        DefruleData(theEnv)->DefruleConstruct);
}
...
globle intBool EnvUndefrule(void* theEnv, void* theDefrule) {
  return (Undefconstruct(theEnv, theDefrule,
                         DefruleData(theEnv)->DefruleConstruct));
}
...
globle void GetDefruleListFunction(void* theEnv, DATA_OBJECT_PTR returnValue) {
  GetConstructListFunction(theEnv, "get-defrule-list", returnValue,
                           DefruleData(theEnv)->DefruleConstruct);
}
...
/*   for the get-defrule-list function. */
/****************************************/
globle void EnvGetDefruleList(void* theEnv, DATA_OBJECT_PTR returnValue,
                              void* theModule) {
  GetConstructList(theEnv, returnValue, DefruleData(theEnv)->DefruleConstruct,
                   (struct defmodule*)theModule);
}
...
globle void* DefruleModuleFunction(void* theEnv) {
  return (GetConstructModuleCommand(theEnv, "defrule-module",
                                    DefruleData(theEnv)->DefruleConstruct));
}
...
globle void PPDefruleCommand(void* theEnv) {
  PPConstructCommand(theEnv, "ppdefrule",
                     DefruleData(theEnv)->DefruleConstruct);
}
...
/***********************************/
globle int PPDefrule(void* theEnv, const char* defruleName,
                     const char* logicalName) {
  return (PPConstruct(theEnv, defruleName, logicalName,
                      DefruleData(theEnv)->DefruleConstruct));
}
...
globle void ListDefrulesCommand(void* theEnv) {
  ListConstructCommand(theEnv, "list-defrules",
                       DefruleData(theEnv)->DefruleConstruct);
}
...
/*   for the list-defrules command.  */
/*************************************/
globle void EnvListDefrules(void* theEnv, const char* logicalName,
                            void* theModule) {
  ListConstruct(theEnv, DefruleData(theEnv)->DefruleConstruct, logicalName,
                (struct defmodule*)theModule);
}
...
#pragma unused(theEnv)
  ...
  for (thePtr = (struct defrule*)rulePtr; thePtr != NULL;
...
/***************************************************/
globle void EnvSetDefruleWatchActivations(void* theEnv, unsigned newState,
                                          void* rulePtr) {
  struct defrule* thePtr;
#if MAC_XCD
#pragma unused(theEnv)
...
/*   firings.                                       */
/****************************************************/
globle void EnvSetDefruleWatchFirings(void* theEnv, unsigned newState,
                                      void* rulePtr) {
  struct defrule* thePtr;
#if MAC_XCD
#pragma unused(theEnv)
...
/*******************************************************************/
globle unsigned DefruleWatchAccess(void* theEnv, int code, unsigned newState,
                                   struct expr* argExprs) {
  if (code)
    return (ConstructSetWatchAccess(
        theEnv, DefruleData(theEnv)->DefruleConstruct, newState, argExprs,
...
/*****************************************************************/
globle unsigned DefruleWatchPrint(void* theEnv, const char* logName, int code,
                                  struct expr* argExprs) {
  if (code)
    return (ConstructPrintWatchAccess(
        theEnv, DefruleData(theEnv)->DefruleConstruct, logName, argExprs,
        EnvGetDefruleWatchActivations, EnvSetDefruleWatchActivations));
  else
    return (ConstructPrintWatchAccess(
        theEnv, DefruleData(theEnv)->DefruleConstruct, logName, argExprs,
...
globle void GetDefruleList(DATA_OBJECT_PTR returnValue, void* theModule) {
  EnvGetDefruleList(GetCurrentEnvironment(), returnValue, theModule);
}

#if DEBUGGING_FUNCTIONS

globle unsigned GetDefruleWatchActivations(void* rulePtr) {
  return EnvGetDefruleWatchActivations(GetCurrentEnvironment(), rulePtr);
}

globle unsigned GetDefruleWatchFirings(void* rulePtr) {
  return EnvGetDefruleWatchFirings(GetCurrentEnvironment(), rulePtr);
}

globle void ListDefrules(const char* logicalName, void* theModule) {
  EnvListDefrules(GetCurrentEnvironment(), logicalName, theModule);
}

globle void SetDefruleWatchActivations(unsigned newState, void* rulePtr) {
  EnvSetDefruleWatchActivations(GetCurrentEnvironment(), newState, rulePtr);
}

globle void SetDefruleWatchFirings(unsigned newState, void* rulePtr) {
  EnvSetDefruleWatchFirings(GetCurrentEnvironment(), newState, rulePtr);
}

#endif

globle intBool Undefrule(void* theDefrule) {
