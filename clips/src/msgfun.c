...
static void PrintPreviewHandler(void*, const char*, HANDLER_LINK*, int,
                                const char*);
#endif
...
globle void UnboundHandlerErr(void* theEnv) {
  ...
  PrintHandler(theEnv, WERROR, MessageHandlerData(theEnv)->CurrentCore->hnd,
               TRUE);
}
...
/*****************************************************************
  ...
  NOTES        : None
 *****************************************************************/
globle void PrintNoHandlerError(void* theEnv, const char* msg) {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void SlotAccessViolationError(void* theEnv, const char* slotName,
  ...
  if (instanceFlag)
    PrintInstanceNameAndClass(theEnv, WERROR,
                              (INSTANCE_TYPE*)theInstanceOrClass, FALSE);
  else {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void SlotVisibilityViolationError(void* theEnv, SLOT_DESC* sd,
 ...
 *******************************************************************************/
globle void NewSystemHandler(void* theEnv, const char* cname, const char* mname,
                             const char* fname, int extraargs) {
  DEFCLASS* cls;
  HANDLER* hnd;

  cls = LookupDefclassInScope(theEnv, cname);
  hnd = InsertHandlerHeader(theEnv, cls,
                            (SYMBOL_HN*)EnvAddSymbol(theEnv, mname), MPRIMARY);
 ...
 ***************************************************/
globle HANDLER* InsertHandlerHeader(void* theEnv, DEFCLASS* cls,
                                    SYMBOL_HN* mname, int mtype) {
  HANDLER *nhnd, *hnd;
  unsigned *narr, *arr;
  long i;
  long j, ni = -1;
  ...
  for (i = 0, j = 0; i < cls->handlerCount; i++, j++) {
    if (ni == -1) {
      if ((hnd[arr[i]].name->bucket > mname->bucket)
              ? TRUE
  ...
  if (ni == -1) ni = (int)cls->handlerCount;
...
globle int HandlersExecuting(DEFCLASS* cls) {
  ...
  for (i = 0; i < cls->handlerCount; i++)
    if (cls->handlers[i].busy > 0) return (TRUE);
  return (FALSE);
}
...
/*********************************************************************
  ...
  NOTES        : If any handlers for the class are
                   ...
                   will fail
 **********************************************************************/
globle int DeleteHandler(void* theEnv, DEFCLASS* cls, SYMBOL_HN* mname,
                         int mtype, int indicate_missing) {
  long i;
  HANDLER* hnd;
  int found, success = 1;
  ...
  if (mtype == -1) {
    ...
    if ((found == FALSE) ? (strcmp(ValueToString(mname), "*") == 0) : FALSE) {
      for (i = 0; i < cls->handlerCount; i++)
        if (cls->handlers[i].system == 0) cls->handlers[i].mark = 1;
    }
  } else {
...
globle void DeallocateMarkedHandlers(void* theEnv, DEFCLASS* cls) {
  ...
  if (count == cls->handlerCount) {
    ...
    rm(theEnv, (void*)cls->handlerOrderMap,
...
/*****************************************************
  ...
  NOTES        : None
 *****************************************************/
globle unsigned HandlerType(void* theEnv, const char* func, const char* str) {
  unsigned i;
...
/*****************************************************************
  ...
  NOTES        : None
 *****************************************************************/
globle int CheckCurrentMessage(void* theEnv, const char* func, int ins_reqd) {
  ...
  if ((activeMsgArg->type == INSTANCE_ADDRESS)
          ? (((INSTANCE_TYPE*)activeMsgArg->value)->garbage == 1)
          : FALSE) {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void PrintHandler(void* theEnv, const char* logName, HANDLER* theHandler,
                         int crtn) {
 ...
 ***********************************************************/
globle HANDLER* FindHandlerByAddress(DEFCLASS* cls, SYMBOL_HN* name,
                                     unsigned type) {
  int b;
  long i;
  HANDLER* hnd;
  ...
  if ((b = FindHandlerNameGroup(cls, name)) == -1) return (NULL);
  arr = cls->handlerOrderMap;
  hnd = cls->handlers;
  for (i = (unsigned)b; i < cls->handlerCount; i++) {
    if (hnd[arr[i]].name != name) return (NULL);
    if (hnd[arr[i]].type == type) return (&hnd[arr[i]]);
  }
  return (NULL);
}
...
globle int FindHandlerByIndex(DEFCLASS* cls, SYMBOL_HN* name, unsigned type) {
  ...
  if ((b = FindHandlerNameGroup(cls, name)) == -1) return (-1);
  arr = cls->handlerOrderMap;
  hnd = cls->handlers;
  for (i = (unsigned)b; i < cls->handlerCount; i++) {
    if (hnd[arr[i]].name != name) return (-1);
    if (hnd[arr[i]].type == type) return ((int)arr[i]);
  }
  return (-1);
}
...
globle int FindHandlerNameGroup(DEFCLASS* cls, SYMBOL_HN* name) {
  ...
  if (cls->handlerCount == 0) return (-1);
  hnd = cls->handlers;
  arr = cls->handlerOrderMap;
  b = 0;
  ...
  do {
    ...
    if (name->bucket == hnd[arr[i]].name->bucket) {
      for (j = i; j >= b; j--) {
        if (hnd[arr[j]].name == name) start = j;
        if (hnd[arr[j]].name->bucket != name->bucket) break;
      }
      if (start != -1) return (start);
      for (j = i + 1; j <= e; j++) {
        if (hnd[arr[j]].name == name) return (j);
        if (hnd[arr[j]].name->bucket != name->bucket) return (-1);
      }
      return (-1);
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle void HandlerDeleteError(void* theEnv, const char* cname) {
 ...
 ********************************************************************/
globle void DisplayCore(void* theEnv, const char* logicalName,
                        HANDLER_LINK* core, int sdepth) {
...
/*******************************************************************
  ...
  NOTES        : None
 ******************************************************************/
globle HANDLER_LINK* FindPreviewApplicableHandlers(void* theEnv, DEFCLASS* cls,
                                                   SYMBOL_HN* mname) {
  int i;
 ...
 ***********************************************************/
globle void WatchMessage(void* theEnv, const char* logName,
                         const char* tstring) {
  ...
  PrintLongInteger(theEnv, logName,
                   (long long)EvaluationData(theEnv)->CurrentEvaluationDepth);
...
/***********************************************************
  ...
  NOTES        : Uses the global variables ProcParamArray
                   and CurrentMessageName
 ***********************************************************/
globle void WatchHandler(void* theEnv, const char* logName, HANDLER_LINK* hndl,
                         const char* tstring) {
  HANDLER* hnd;
...
/********************************************************************
  ...
  NOTES        : Expects that the core was created in PREVIEW mode,
                 ...
                 Assumes (partial) core is not empty
 ********************************************************************/
static HANDLER_LINK* DisplayPrimaryCore(void* theEnv, const char* logicalName,
                                        HANDLER_LINK* core, int pdepth) {
  HANDLER_LINK* rtn;
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void PrintPreviewHandler(void* theEnv, const char* logicalName,
                                HANDLER_LINK* cptr, int sdepth,
                                const char* tstr) {
  int i;

  for (i = 0; i < sdepth; i++) EnvPrintRouter(theEnv, logicalName, "| ");
