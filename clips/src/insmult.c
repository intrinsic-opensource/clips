...
static INSTANCE_SLOT* CheckMultifieldSlotModify(void*, int, const char*,
                                                INSTANCE_TYPE*, EXPRESSION*,
                                                long*, long*, DATA_OBJECT*);
...
globle void SetupInstanceMultifieldCommands(void* theEnv) {
  /* ===================================
     ...
     =================================== */
  EnvDefineFunction2(theEnv, "direct-mv-replace", 'b',
                     PTIEF DirectMVReplaceCommand, "DirectMVReplaceCommand",
                     "4**wii");
  EnvDefineFunction2(theEnv, "direct-mv-insert", 'b',
                     PTIEF DirectMVInsertCommand, "DirectMVInsertCommand",
                     "3**wi");
  EnvDefineFunction2(theEnv, "direct-mv-delete", 'b',
                     PTIEF DirectMVDeleteCommand, "DirectMVDeleteCommand",
                     "33iw");
  ...
  /* =====================
     ...
     ===================== */
  EnvDefineFunction2(theEnv, "slot-direct-replace$", 'b',
                     PTIEF DirectMVReplaceCommand, "DirectMVReplaceCommand",
                     "4**wii");
  EnvDefineFunction2(theEnv, "slot-direct-insert$", 'b',
                     PTIEF DirectMVInsertCommand, "DirectMVInsertCommand",
                     "3**wi");
  EnvDefineFunction2(theEnv, "slot-direct-delete$", 'b',
                     PTIEF DirectMVDeleteCommand, "DirectMVDeleteCommand",
                     "33iw");
...
globle void MVSlotReplaceCommand(void* theEnv, DATA_OBJECT* result) {
  ...
  if (ins == NULL) return;
  sp =
      CheckMultifieldSlotModify(theEnv, REPLACE, "slot-replace$", ins,
                                GetFirstArgument()->nextArg, &rb, &re, &newval);
  if (sp == NULL) return;
  AssignSlotToDataObject(&oldseg, sp);
  if (ReplaceMultiValueField(theEnv, &newseg, &oldseg, rb, re, &newval,
                             "slot-replace$") == FALSE)
    return;
  arg.type = MULTIFIELD;
...
globle void MVSlotInsertCommand(void* theEnv, DATA_OBJECT* result) {
  ...
  sp = CheckMultifieldSlotModify(theEnv, INSERT, "slot-insert$", ins,
                                 GetFirstArgument()->nextArg, &theIndex, NULL,
                                 &newval);
  if (sp == NULL) return;
  AssignSlotToDataObject(&oldseg, sp);
  if (InsertMultiValueField(theEnv, &newseg, &oldseg, theIndex, &newval,
                            "slot-insert$") == FALSE)
    return;
  arg.type = MULTIFIELD;
  arg.value = (void*)&newseg;
...
globle void MVSlotDeleteCommand(void* theEnv, DATA_OBJECT* result) {
  ...
  if (sp == NULL) return;
  AssignSlotToDataObject(&oldseg, sp);
  if (DeleteMultiValueField(theEnv, &newseg, &oldseg, rb, re, "slot-delete$") ==
      FALSE)
    return;
  arg.type = MULTIFIELD;
  arg.value = (void*)&newseg;
...
globle intBool DirectMVReplaceCommand(void* theEnv) {
  ...
  AssignSlotToDataObject(&oldseg, sp);
  if (ReplaceMultiValueField(theEnv, &newseg, &oldseg, rb, re, &newval,
                             "direct-slot-replace$") == FALSE)
    return (FALSE);
  if (PutSlotValue(theEnv, ins, sp, &newseg, &newval,
                   "function direct-slot-replace$"))
    return (TRUE);
  return (FALSE);
}
...
globle intBool DirectMVInsertCommand(void* theEnv) {
  ...
  AssignSlotToDataObject(&oldseg, sp);
  if (InsertMultiValueField(theEnv, &newseg, &oldseg, theIndex, &newval,
                            "direct-slot-insert$") == FALSE)
    return (FALSE);
  if (PutSlotValue(theEnv, ins, sp, &newseg, &newval,
                   "function direct-slot-insert$"))
    return (TRUE);
  return (FALSE);
}
...
globle intBool DirectMVDeleteCommand(void* theEnv) {
  ...
  if (sp == NULL) return (FALSE);
  AssignSlotToDataObject(&oldseg, sp);
  if (DeleteMultiValueField(theEnv, &newseg, &oldseg, rb, re,
                            "direct-slot-delete$") == FALSE)
    return (FALSE);
  if (PutSlotValue(theEnv, ins, sp, &newseg, &oldseg,
                   "function direct-slot-delete$"))
    return (TRUE);
  return (FALSE);
}
...
static INSTANCE_TYPE* CheckMultifieldSlotInstance(void* theEnv,
  ...
  if (temp.type == INSTANCE_ADDRESS) {
    ...
    if (ins == NULL) NoInstanceError(theEnv, ValueToString(temp.value), func);
  }
  return (ins);
}
 ...
 *********************************************************************/
static INSTANCE_SLOT* CheckMultifieldSlotModify(void* theEnv, int code,
                                                const char* func,
                                                INSTANCE_TYPE* ins,
 ...
 ***************************************************/
static void AssignSlotToDataObject(DATA_OBJECT* theDataObject,
