...
#define LinkPointer(i) \
  (((i) == -1L) ? NULL : (DEFCLASS**)&ObjectBinaryData(theEnv)->LinkArray[i])
#define SlotPointer(i) \
  (((i) == -1L) ? NULL : (SLOT_DESC*)&ObjectBinaryData(theEnv)->SlotArray[i])
#define TemplateSlotPointer(i) \
  (((i) == -1L) ? NULL         \
                : (SLOT_DESC**)&ObjectBinaryData(theEnv)->TmpslotArray[i])
#define OrderedSlotPointer(i) \
  (((i) == -1L) ? NULL : (unsigned*)&ObjectBinaryData(theEnv)->MapslotArray[i])
#define SlotNamePointer(i) \
  ((SLOT_NAME*)&ObjectBinaryData(theEnv)->SlotNameArray[i])
#define HandlerPointer(i) \
  (((i) == -1L) ? NULL : (HANDLER*)&ObjectBinaryData(theEnv)->HandlerArray[i])
#define OrderedHandlerPointer(i) \
  (((i) == -1L) ? NULL           \
                : (unsigned*)&ObjectBinaryData(theEnv)->MaphandlerArray[i])
...
typedef struct bsaveDefclass {
  ...
  unsigned system : 1;
  unsigned id;
  BSAVE_PACKED_CLASS_LINKS directSuperclasses, directSubclasses,
      allSuperclasses;
  short slotCount, localInstanceSlotCount, instanceSlotCount, maxSlotNameID;
  short handlerCount;
  long slots, instanceTemplate, slotNameMap, handlers, scopeMap;
#if DEFRULE_CONSTRUCT
...
typedef struct bsaveSlotDescriptor {
  ...
  long cls, slotName, defaultValue, constraint, overrideMessage;
} BSAVE_SLOT_DESC;

typedef struct bsaveMessageHandler {
  unsigned system : 1;
  unsigned type : 2;
  short minParams, maxParams, localVarCount;
  long name, cls, actions;
} BSAVE_HANDLER;
...
static void BsaveHandlerActionExpressions(void*, struct constructHeader*,
                                          void*);
...
globle void SetupObjectsBload(void* theEnv) {
  AllocateEnvironmentData(theEnv, OBJECTBIN_DATA,
  ...
  AddBinaryItem(theEnv, "defclass", 0, BsaveObjectsFind,
                BsaveObjectsExpressions, BsaveStorageObjects, BsaveObjects,
                BloadStorageObjects, BloadObjects, ClearBloadObjects);
#endif
#if BLOAD || BLOAD_ONLY
...
/*******************************************************/
static void DeallocateObjectBinaryData(void* theEnv) {
  size_t space;
  long i;
  ...
  if (ObjectBinaryData(theEnv)->ClassCount != 0) {
    if (DefclassData(theEnv)->ClassIDMap != NULL) {
      rm(theEnv, (void*)DefclassData(theEnv)->ClassIDMap,
    ...
    for (i = 0L; i < ObjectBinaryData(theEnv)->SlotCount; i++) {
      if ((ObjectBinaryData(theEnv)->SlotArray[i].defaultValue != NULL) &&
        ...
        DATA_OBJECT* theValue =
            (DATA_OBJECT*)ObjectBinaryData(theEnv)->SlotArray[i].defaultValue;
        if (theValue->type == MULTIFIELD) {
          ReturnMultifield(theEnv, (struct multifield*)theValue->value);
        }
        rtn_struct(theEnv, dataObject,
                   ObjectBinaryData(theEnv)->SlotArray[i].defaultValue);
      }
    }
 ...
 ***************************************************************************/
static void BsaveObjectsFind(void* theEnv) {
  unsigned i;
  SLOT_NAME* snp;
  ...
  /* ==============================================
     ...
     ============================================== */
  ObjectBinaryData(theEnv)->ModuleCount = DoForAllConstructs(
      theEnv, MarkDefclassItems, DefclassData(theEnv)->DefclassModuleIndex,
      FALSE, NULL);
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void MarkDefclassItems(void* theEnv, struct constructHeader* theDefclass,
                              void* buf) {
#if MAC_XCD
#pragma unused(buf)
#endif
  ...
  MarkConstructHeaderNeededItems(&cls->header,
    ...
    if (cls->slots[i].defaultValue != NULL) {
      if (cls->slots[i].dynamicDefault) {
        ...
        /* =================================================
           ...
           ================================================= */
        tmpexp = ConvertValueToExpression(
            theEnv, (DATA_OBJECT*)cls->slots[i].defaultValue);
  ...
  for (i = 0; i < cls->handlerCount; i++) {
    ...
    ExpressionData(theEnv)->ExpressionCount +=
        ExpressionSize(cls->handlers[i].actions);
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void BsaveObjectsExpressions(void* theEnv, FILE* fp) {
  ...
  /* ================================================
     ...
     ================================================ */
  DoForAllConstructs(theEnv, BsaveDefaultSlotExpressions,
  ...
  /* ==============================================
     ...
     ============================================== */
  DoForAllConstructs(theEnv, BsaveHandlerActionExpressions,
                     DefclassData(theEnv)->DefclassModuleIndex, FALSE,
                     (void*)fp);
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void BsaveDefaultSlotExpressions(void* theEnv,
  ...
  for (i = 0; i < cls->slotCount; i++) {
    if (cls->slots[i].defaultValue != NULL) {
      if (cls->slots[i].dynamicDefault)
        BsaveExpression(theEnv, (EXPRESSION*)cls->slots[i].defaultValue,
                        (FILE*)buf);
      else {
        /* =================================================
           Static default values are stotred as data objects
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void BsaveHandlerActionExpressions(void* theEnv,
                                          struct constructHeader* theDefclass,
                                          void* buf) {
  DEFCLASS* cls = (DEFCLASS*)theDefclass;
  long i;
...
/*************************************************************************************
  ...
  NOTES        : None
 *************************************************************************************/
static void BsaveStorageObjects(void* theEnv, FILE* fp) {
  size_t space;
  long maxClassID;
  ...
  space = sizeof(long) * 9;
  GenWrite((void*)&space, sizeof(size_t),
           fp);  // 64-bit issue changed long to size_t
  ...
  GenWrite((void*)&ObjectBinaryData(theEnv)->TemplateSlotCount, sizeof(long),
           fp);
  GenWrite((void*)&ObjectBinaryData(theEnv)->SlotNameMapCount, sizeof(long),
           fp);
...
/*************************************************************************************
  ...
  NOTES        : None
 *************************************************************************************/
static void BsaveObjects(void* theEnv, FILE* fp) {
  size_t space;
  ...
  space =
      (ObjectBinaryData(theEnv)->ModuleCount * sizeof(BSAVE_DEFCLASS_MODULE)) +
  ...
  while (theModule != NULL) {
    theModuleItem = (DEFCLASS_MODULE*)GetModuleItem(
        theEnv, theModule, FindModuleItem(theEnv, "defclass")->moduleIndex);
    ...
    theModule =
        (struct defmodule*)EnvGetNextDefmodule(theEnv, (void*)theModule);
  }

  /* =====================
     Write out the classes
     ===================== */
  DoForAllConstructs(theEnv, BsaveDefclass,
                     DefclassData(theEnv)->DefclassModuleIndex, FALSE,
                     (void*)fp);
  ...
  /* =========================
     ...
     ========================= */
  ObjectBinaryData(theEnv)->LinkCount = 0L;
  DoForAllConstructs(theEnv, BsaveClassLinks,
                     DefclassData(theEnv)->DefclassModuleIndex, FALSE,
                     (void*)fp);
  ...
  /* ===================
     Write out the slots
     =================== */
  DoForAllConstructs(theEnv, BsaveSlots,
                     DefclassData(theEnv)->DefclassModuleIndex, FALSE,
                     (void*)fp);
  ...
  /* =====================================
     ...
     ===================================== */
  DoForAllConstructs(theEnv, BsaveTemplateSlots,
                     DefclassData(theEnv)->DefclassModuleIndex, FALSE,
                     (void*)fp);
  ...
  /* =============================================
     ...
     ============================================= */
  DoForAllConstructs(theEnv, BsaveSlotMap,
                     DefclassData(theEnv)->DefclassModuleIndex, FALSE,
                     (void*)fp);
  ...
  /* ==============================
     ...
     ============================== */
  DoForAllConstructs(theEnv, BsaveHandlers,
                     DefclassData(theEnv)->DefclassModuleIndex, FALSE,
                     (void*)fp);
  ...
  /* ==========================================
     ...
     ========================================== */
  DoForAllConstructs(theEnv, BsaveHandlerMap,
                     DefclassData(theEnv)->DefclassModuleIndex, FALSE,
                     (void*)fp);
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void BsaveDefclass(void* theEnv, struct constructHeader* theDefclass,
                          void* buf) {
  DEFCLASS* cls = (DEFCLASS*)theDefclass;
  ...
  if (cls->relevant_terminal_alpha_nodes != NULL) {
    dummy_class.relevant_terminal_alpha_nodes =
        cls->relevant_terminal_alpha_nodes->bsaveID;
  } else
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void BsaveClassLinks(void* theEnv, struct constructHeader* theDefclass,
                            void* buf) {
  DEFCLASS* cls = (DEFCLASS*)theDefclass;
  long i;
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void BsaveSlots(void* theEnv, struct constructHeader* theDefclass,
                       void* buf) {
  DEFCLASS* cls = (DEFCLASS*)theDefclass;
  long i;
  ...
  for (i = 0; i < cls->slotCount; i++) {
    ...
    if (sp->defaultValue != NULL) {
      ...
      if (sp->dynamicDefault)
        ExpressionData(theEnv)->ExpressionCount +=
            ExpressionSize((EXPRESSION*)sp->defaultValue);
      else {
        tmpexp =
            ConvertValueToExpression(theEnv, (DATA_OBJECT*)sp->defaultValue);
        ExpressionData(theEnv)->ExpressionCount += ExpressionSize(tmpexp);
        ReturnExpression(theEnv, tmpexp);
      }
    } else
...
/**************************************************************
  ...
  NOTES        : None
 **************************************************************/
static void BsaveTemplateSlots(void* theEnv,
...
/***************************************************************
  ...
  NOTES        : None
 ***************************************************************/
static void BsaveSlotMap(void* theEnv, struct constructHeader* theDefclass,
                         void* buf) {
  DEFCLASS* cls = (DEFCLASS*)theDefclass;
#if MAC_XCD
#pragma unused(theEnv)
...
/************************************************************
  ...
  NOTES        : None
 ************************************************************/
static void BsaveHandlers(void* theEnv, struct constructHeader* theDefclass,
                          void* buf) {
  DEFCLASS* cls = (DEFCLASS*)theDefclass;
  long i;
...
/****************************************************************
  ...
  NOTES        : None
 ****************************************************************/
static void BsaveHandlerMap(void* theEnv, struct constructHeader* theDefclass,
                            void* buf) {
  DEFCLASS* cls = (DEFCLASS*)theDefclass;
#if MAC_XCD
#pragma unused(theEnv)
  ...
  GenWrite((void*)cls->handlerOrderMap, (sizeof(unsigned) * cls->handlerCount),
           (FILE*)buf);
}
 ...
 ***********************************************************************/
static void BloadStorageObjects(void* theEnv) {
  size_t space;
  long counts[9];
  ...
  if (ObjectBinaryData(theEnv)->ModuleCount != 0L) {
    ...
    ObjectBinaryData(theEnv)->ModuleArray =
        (DEFCLASS_MODULE*)genalloc(theEnv, space);
  }
  if (ObjectBinaryData(theEnv)->ClassCount != 0L) {
    space = (sizeof(DEFCLASS) * ObjectBinaryData(theEnv)->ClassCount);
    ObjectBinaryData(theEnv)->DefclassArray =
        (DEFCLASS*)genalloc(theEnv, space);
    DefclassData(theEnv)->ClassIDMap = (DEFCLASS**)gm2(
        theEnv, (sizeof(DEFCLASS*) * DefclassData(theEnv)->MaxClassID));
  }
  ...
  if (ObjectBinaryData(theEnv)->HandlerCount != 0L) {
    ...
    ObjectBinaryData(theEnv)->MaphandlerArray =
        (unsigned*)genalloc(theEnv, space);
  }
}
 ...
 **************************************************************/
static void BloadObjects(void* theEnv) {
  size_t space;

  GenReadBinary(theEnv, (void*)&space, sizeof(size_t));
  if (space == 0L) return;
  if (ObjectBinaryData(theEnv)->ModuleCount != 0L)
    BloadandRefresh(theEnv, ObjectBinaryData(theEnv)->ModuleCount,
                    sizeof(BSAVE_DEFCLASS_MODULE), UpdateDefclassModule);
  if (ObjectBinaryData(theEnv)->ClassCount != 0L) {
    BloadandRefresh(theEnv, ObjectBinaryData(theEnv)->ClassCount,
                    sizeof(BSAVE_DEFCLASS), UpdateDefclass);
    BloadandRefresh(
        theEnv, ObjectBinaryData(theEnv)->LinkCount, sizeof(long),
        UpdateLink);  // 64-bit bug fix: DEFCLASS * replaced with long
    BloadandRefresh(theEnv, ObjectBinaryData(theEnv)->SlotNameCount,
                    sizeof(BSAVE_SLOT_NAME), UpdateSlotName);
    BloadandRefresh(theEnv, ObjectBinaryData(theEnv)->SlotCount,
                    sizeof(BSAVE_SLOT_DESC), UpdateSlot);
    if (ObjectBinaryData(theEnv)->TemplateSlotCount != 0L)
      BloadandRefresh(theEnv, ObjectBinaryData(theEnv)->TemplateSlotCount,
                      sizeof(long), UpdateTemplateSlot);
    if (ObjectBinaryData(theEnv)->SlotNameMapCount != 0L) {
      space = (sizeof(unsigned) * ObjectBinaryData(theEnv)->SlotNameMapCount);
      GenReadBinary(theEnv, (void*)ObjectBinaryData(theEnv)->MapslotArray,
                    space);
    }
    if (ObjectBinaryData(theEnv)->HandlerCount != 0L) {
      BloadandRefresh(theEnv, ObjectBinaryData(theEnv)->HandlerCount,
                      sizeof(BSAVE_HANDLER), UpdateHandler);
      space = (sizeof(unsigned) * ObjectBinaryData(theEnv)->HandlerCount);
      GenReadBinary(theEnv, (void*)ObjectBinaryData(theEnv)->MaphandlerArray,
                    space);
    }
 ...
 ***************************************************/
static void UpdatePrimitiveClassesMap(void* theEnv) {
  unsigned i;
 ...
 *********************************************************/
static void UpdateDefclassModule(void* theEnv, void* buf, long obji) {
  BSAVE_DEFCLASS_MODULE* bdptr;

  bdptr = (BSAVE_DEFCLASS_MODULE*)buf;
  UpdateDefmoduleItemHeader(theEnv, &bdptr->header,
                            &ObjectBinaryData(theEnv)->ModuleArray[obji].header,
                            (int)sizeof(DEFCLASS),
                            (void*)ObjectBinaryData(theEnv)->DefclassArray);
}

static void UpdateDefclass(void* theEnv, void* buf, long obji) {
  BSAVE_DEFCLASS* bcls;
  DEFCLASS* cls;
  ...
  UpdateConstructHeader(
      theEnv, &bcls->header, &cls->header, (int)sizeof(DEFCLASS_MODULE),
      (void*)ObjectBinaryData(theEnv)->ModuleArray, (int)sizeof(DEFCLASS),
      (void*)ObjectBinaryData(theEnv)->DefclassArray);
  ...
  cls->relevant_terminal_alpha_nodes =
      ClassAlphaPointer(bcls->relevant_terminal_alpha_nodes);
#if DEFMODULE_CONSTRUCT
...
static void UpdateLink(void* theEnv, void* buf, long obji) {
  long* blink;
...
static void UpdateSlot(void* theEnv, void* buf, long obji) {
  SLOT_DESC* sp;
  BSAVE_SLOT_DESC* bsp;
  ...
  if (bsp->defaultValue != -1L) {
    if (sp->dynamicDefault)
      ...
      EvaluateAndStoreInDataObject(theEnv, (int)sp->multiple,
                                   ExpressionPointer(bsp->defaultValue),
...
static void UpdateSlotName(void* theEnv, void* buf, long obji) {
  SLOT_NAME* snp;
...
static void UpdateTemplateSlot(void* theEnv, void* buf, long obji) {
  ObjectBinaryData(theEnv)->TmpslotArray[obji] = SlotPointer(*(long*)buf);
}

static void UpdateHandler(void* theEnv, void* buf, long obji) {
  HANDLER* hnd;
  BSAVE_HANDLER* bhnd;
...
/***************************************************************
  ...
  NOTES        : None
 ***************************************************************/
static void ClearBloadObjects(void* theEnv) {
  long i;
  size_t space;
  ...
  if (ObjectBinaryData(theEnv)->ClassCount != 0L) {
    ...
    for (i = 0L; i < ObjectBinaryData(theEnv)->ClassCount; i++) {
      UnmarkConstructHeader(theEnv,
                            &ObjectBinaryData(theEnv)->DefclassArray[i].header);
#if DEFMODULE_CONSTRUCT
      DecrementBitMapCount(theEnv,
                           ObjectBinaryData(theEnv)->DefclassArray[i].scopeMap);
#endif
      RemoveClassFromTable(
          theEnv, (DEFCLASS*)&ObjectBinaryData(theEnv)->DefclassArray[i]);
    }
    for (i = 0L; i < ObjectBinaryData(theEnv)->SlotCount; i++) {
      DecrementSymbolCount(
          theEnv, ObjectBinaryData(theEnv)->SlotArray[i].overrideMessage);
      if ((ObjectBinaryData(theEnv)->SlotArray[i].defaultValue != NULL) &&
          (ObjectBinaryData(theEnv)->SlotArray[i].dynamicDefault == 0)) {
        DATA_OBJECT* theValue =
            (DATA_OBJECT*)ObjectBinaryData(theEnv)->SlotArray[i].defaultValue;
    ...
    for (i = 0L; i < ObjectBinaryData(theEnv)->SlotNameCount; i++) {
      DefclassData(theEnv)->SlotNameTable
          [ObjectBinaryData(theEnv)->SlotNameArray[i].hashTableIndex] = NULL;
      DecrementSymbolCount(theEnv,
                           ObjectBinaryData(theEnv)->SlotNameArray[i].name);
      DecrementSymbolCount(
          theEnv, ObjectBinaryData(theEnv)->SlotNameArray[i].putHandlerName);
    }
  ...
  if (ObjectBinaryData(theEnv)->HandlerCount != 0L) {
    for (i = 0L; i < ObjectBinaryData(theEnv)->HandlerCount; i++)
      DecrementSymbolCount(theEnv,
                           ObjectBinaryData(theEnv)->HandlerArray[i].name);
