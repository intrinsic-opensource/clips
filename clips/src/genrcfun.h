...
#define DefgenericData(theEnv) \
  ((struct defgenericData*)GetEnvironmentData(theEnv, DEFGENERIC_DATA))
#define SaveBusyCount(gfunc) \
  (DefgenericData(theEnv)->OldGenericBusySave = gfunc->busy)
#define RestoreBusyCount(gfunc) \
  (gfunc->busy = DefgenericData(theEnv)->OldGenericBusySave)
