...
static intBool ParseSimpleQualifier(void*, const char*, const char*,
                                    const char*, const char*, intBool*,
                                    intBool*);
...
static TEMP_SLOT_LINK* MergeSlots(void*, TEMP_SLOT_LINK*, DEFCLASS*, short*,
                                  int);
...
/***************************************************************************************
  ...
  NOTES        : H/L Syntax :
                 ...
                 <slot-descriptor>  :== (default <default-expression>) |
                                        ...
                                        (access
  read-only|read-write|initialize-only) | (propagation no-inherit|inherit) |
  ...
  ***************************************************************************************/
globle int ParseDefclass(void* theEnv, const char* readSource) {
  SYMBOL_HN* cname;
  DEFCLASS* cls;
  PACKED_CLASS_LINKS *sclasses, *preclist;
  TEMP_SLOT_LINK* slots = NULL;
  int roleSpecified = FALSE, abstract = FALSE, parseError;
#if DEFRULE_CONSTRUCT
  int patternMatchSpecified = FALSE, reactive = TRUE;
#endif
  ...
  cname = GetConstructNameAndComment(
      theEnv, readSource, &DefclassData(theEnv)->ObjectParseToken, "defclass",
      EnvFindDefclassInModule, NULL, "#", TRUE, TRUE, TRUE, FALSE);
  ...
  while (GetType(DefclassData(theEnv)->ObjectParseToken) != RPAREN) {
    ...
    if (strcmp(DOToString(DefclassData(theEnv)->ObjectParseToken), ROLE_RLN) ==
        0) {
      if (ParseSimpleQualifier(theEnv, readSource, ROLE_RLN, CONCRETE_RLN,
                               ABSTRACT_RLN, &roleSpecified,
                               &abstract) == FALSE) {
        parseError = TRUE;
        break;
      }
                    ...
                    MATCH_RLN) == 0) {
      if (ParseSimpleQualifier(theEnv, readSource, MATCH_RLN, NONREACTIVE_RLN,
                               REACTIVE_RLN, &patternMatchSpecified,
                               &reactive) == FALSE) {
        parseError = TRUE;
        break;
      }
      ...
      if (slots == NULL) {
        ...
        break;
                      ...
                      HANDLER_DECL) == 0) {
      if (ReadUntilClosingParen(theEnv, readSource,
  ...
  if (roleSpecified == FALSE) {
    if (preclist->classArray[1]->system && /* Change to cause         */
        (DefclassData(theEnv)->ClassDefaultsMode ==
         CONVENIENCE_MODE)) /* default role of         */
    {
      abstract = FALSE;
  ...
  if (patternMatchSpecified == FALSE) {
    if ((preclist->classArray[1]->system) && /* Change to cause       */
        ...
        (DefclassData(theEnv)->ClassDefaultsMode ==
         CONVENIENCE_MODE)) /* of classes to be      */
    {
      reactive = TRUE;
    } /* reactive.             */
    else {
  ...
  /* =================================
     ...
     ================================= */
  if (slots != NULL) PackSlots(theEnv, cls, slots);
  AddClass(theEnv, cls);
...
/***********************************************************
  ...
  NOTES        : GetConstructNameAndComment() (called before
                 ...
                 another module
 ***********************************************************/
static intBool ValidClassName(void* theEnv, const char* theClassName,
  ...
  *theDefclass = (DEFCLASS*)EnvFindDefclassInModule(theEnv, theClassName);
  if (*theDefclass != NULL) {
    ...
    if ((EnvIsDefclassDeletable(theEnv, (void*)*theDefclass) == FALSE) &&
      ...
      EnvPrintRouter(theEnv, WERROR,
                     EnvGetDefclassName(theEnv, (void*)*theDefclass));
...
/***************************************************************
  ...
  NOTES        : None
 ***************************************************************/
static intBool ParseSimpleQualifier(void* theEnv, const char* readSource,
  ...
  if (strcmp(DOToString(DefclassData(theEnv)->ObjectParseToken), setRelation) ==
      0)
    *binaryFlag = TRUE;
  else if (strcmp(DOToString(DefclassData(theEnv)->ObjectParseToken),
                  clearRelation) == 0)
 ...
 ***************************************************/
static intBool ReadUntilClosingParen(void* theEnv, const char* readSource,
                                     struct token* inputToken) {
  int cnt = 1, lparen_read = FALSE;

  do {
    if (lparen_read == FALSE) SavePPBuffer(theEnv, " ");
...
static void AddClass(void* theEnv, DEFCLASS* cls) {
  ...
  int oldTraceInstances = FALSE, oldTraceSlots = FALSE;
#endif
  ...
  ctmp = (DEFCLASS*)EnvFindDefclassInModule(
      theEnv, EnvGetDefclassName(theEnv, (void*)cls));
  ...
  if (cls->abstract) {
    ...
    if (oldTraceInstances) cls->traceInstances = TRUE;
    if (oldTraceSlots) cls->traceSlots = TRUE;
  }
#endif
...
static void BuildSubclassLinks(void* theEnv, DEFCLASS* cls) {
  ...
  for (i = 0; i < cls->directSuperclasses.classCount; i++)
    AddClassLink(theEnv,
                 &cls->directSuperclasses.classArray[i]->directSubclasses, cls,
                 -1);
}
...
static void FormInstanceTemplate(void* theEnv, DEFCLASS* cls) {
  ...
  for (i = 1; i < cls->allSuperclasses.classCount; i++)
    islots = MergeSlots(theEnv, islots, cls->allSuperclasses.classArray[i],
                        &scnt, INHERIT);
  ...
  if (scnt > 0)
    cls->instanceTemplate =
        (SLOT_DESC**)gm2(theEnv, (scnt * sizeof(SLOT_DESC*)));
  for (i = 0; i < scnt; i++) {
    stmp = islots;
    islots = islots->nxt;
    cls->instanceTemplate[i] = stmp->desc;
    if (stmp->desc->shared == 0) cls->localInstanceSlotCount++;
...
static void FormSlotNameMap(void* theEnv, DEFCLASS* cls) {
  ...
  if (cls->instanceSlotCount == 0) return;
  for (i = 0; i < cls->instanceSlotCount; i++)
    if (cls->instanceTemplate[i]->slotName->id > cls->maxSlotNameID)
      cls->maxSlotNameID = cls->instanceTemplate[i]->slotName->id;
  cls->slotNameMap =
      (unsigned*)gm2(theEnv, (sizeof(unsigned) * (cls->maxSlotNameID + 1)));
  for (i = 0; i <= cls->maxSlotNameID; i++) cls->slotNameMap[i] = 0;
  for (i = 0; i < cls->instanceSlotCount; i++)
 ...
 *******************************************************************/
static TEMP_SLOT_LINK* MergeSlots(void* theEnv, TEMP_SLOT_LINK* old,
                                  DEFCLASS* cls, short* scnt, int src) {
...
globle void* CreateClassScopeMap(void* theEnv, DEFCLASS* theDefclass) {
  ...
  for (theModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    ...
    if (FindImportedConstruct(theEnv, "defclass", matchModule, className,
...
/*****************************************************************************
  ...
  NOTES        : None
 ******************************************************************************/
static void CreatePublicSlotMessageHandlers(void* theEnv,
                                            DEFCLASS* theDefclass) {
  long i;
  SLOT_DESC* sd;
