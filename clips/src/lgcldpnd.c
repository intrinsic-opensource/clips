...
static struct dependency* DetachAssociatedDependencies(void*,
                                                       struct dependency*,
                                                       void*);
...
static struct dependency* DetachAssociatedDependencies(
    void* theEnv, struct dependency* theList, void* theEntity) {
...
globle void ForceLogicalRetractions(void* theEnv) {
  ...
  while (EngineData(theEnv)->UnsupportedDataEntities != NULL) {
    ...
    theEntity = (struct patternEntity*)EngineData(theEnv)
...
globle void Dependencies(void* theEnv, struct patternEntity* theEntity) {
  ...
  for (fdPtr = (struct dependency*)theEntity->dependents; fdPtr != NULL;
       fdPtr = fdPtr->next) {
...
globle void Dependents(void* theEnv, struct patternEntity* theEntity) {
  ...
  for (GetNextPatternEntity(theEnv, &theParser, &entityPtr); entityPtr != NULL;
    ...
    for (fdPtr = (struct dependency*)entityPtr->dependents; fdPtr != NULL;
         fdPtr = fdPtr->next) {
      if (GetHaltExecution(theEnv) == TRUE) return;
      ...
      if (FindEntityInPartialMatch(theEntity, theBinds) == TRUE) {
        ...
        (*entityPtr->theInfo->base.shortPrintFunction)(theEnv, WDISPLAY,
                                                       entityPtr);
        found = TRUE;
        break;
      }
