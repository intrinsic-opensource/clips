...
static long InstancesSaveCommandParser(void*, const char*,
                                       long (*)(void*, const char*, int,
                                                EXPRESSION*, intBool));
static DATA_OBJECT* ProcessSaveClassList(void*, const char*, EXPRESSION*, int,
                                         intBool);
...
static long SaveOrMarkInstancesOfClass(void*, void*, struct defmodule*, int,
                                       DEFCLASS*, intBool, int,
                                       void (*)(void*, void*, INSTANCE_TYPE*));
...
static void CreateSlotValue(void*, DATA_OBJECT*, struct bsaveSlotValueAtom*,
                            unsigned long);
...
globle void SetupInstanceFileCommands(void* theEnv) {
  ...
  AllocateEnvironmentData(theEnv, INSTANCE_FILE_DATA,
  ...
  EnvDefineFunction2(theEnv, "restore-instances", 'l',
                     PTIEF RestoreInstancesCommand, "RestoreInstancesCommand",
                     "11k");

#if BSAVE_INSTANCES
  EnvDefineFunction2(theEnv, "bsave-instances", 'l',
                     PTIEF BinarySaveInstancesCommand,
                     "BinarySaveInstancesCommand", "1*wk");
#endif
#if BLOAD_INSTANCES
  EnvDefineFunction2(theEnv, "bload-instances", 'l',
                     PTIEF BinaryLoadInstancesCommand,
...
globle long SaveInstancesCommand(void* theEnv) {
  return (InstancesSaveCommandParser(theEnv, "save-instances",
                                     EnvSaveInstancesDriver));
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle long EnvLoadInstances(void* theEnv, const char* file) {
 ...
 ***************************************************/
globle long EnvLoadInstancesFromString(void* theEnv, const char* theString,
                                       int theMax) {
  long theCount;
  const char* theStrRouter = "*** load-instances-from-string ***";

  if ((theMax == -1) ? (!OpenStringSource(theEnv, theStrRouter, theString, 0))
                     : (!OpenTextSource(theEnv, theStrRouter, theString, 0,
                                        (unsigned)theMax)))
    return (-1L);
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle long EnvRestoreInstances(void* theEnv, const char* file) {
...
/***************************************************
  ...
  NOTES        : Uses string routers
 ***************************************************/
globle long EnvRestoreInstancesFromString(void* theEnv, const char* theString,
                                          int theMax) {
  long theCount;
  const char* theStrRouter = "*** load-instances-from-string ***";
...
/****************************************************
  ...
  NOTES        : None
 ****************************************************/
globle long EnvBinaryLoadInstances(void* theEnv, const char* theFile) {
  ...
  GenReadBinary(theEnv,
                (void*)&InstanceFileData(theEnv)->BinaryInstanceFileSize,
...
/*******************************************************
  ...
  NOTES        : None
 *******************************************************/
globle long EnvSaveInstances(void* theEnv, const char* file, int saveCode) {
...
/*******************************************************
  ...
  NOTES        : None
 *******************************************************/
globle long EnvSaveInstancesDriver(void* theEnv, const char* file, int saveCode,
  ...
  classList = ProcessSaveClassList(theEnv, "save-instances",
                                   classExpressionList, saveCode, inheritFlag);
  if ((classList == NULL) && (classExpressionList != NULL)) return (0L);

  SaveOrMarkInstances(theEnv, (void*)sfile, saveCode, classList, inheritFlag,
                      TRUE, NULL);
...
globle long BinarySaveInstancesCommand(void* theEnv) {
  return (InstancesSaveCommandParser(theEnv, "bsave-instances",
                                     EnvBinarySaveInstancesDriver));
}
...
/*******************************************************
  ...
  NOTES        : None
 *******************************************************/
globle long EnvBinarySaveInstances(void* theEnv, const char* file,
                                   int saveCode) {
...
/*******************************************************
  ...
  NOTES        : None
 *******************************************************/
globle long EnvBinarySaveInstancesDriver(void* theEnv, const char* file,
                                         int saveCode,
                                         EXPRESSION* classExpressionList,
                                         intBool inheritFlag) {
  DATA_OBJECT* classList;
  ...
  classList = ProcessSaveClassList(theEnv, "bsave-instances",
                                   classExpressionList, saveCode, inheritFlag);
  if ((classList == NULL) && (classExpressionList != NULL)) return (0L);

  InstanceFileData(theEnv)->BinaryInstanceFileSize = 0L;
  InitAtomicValueNeededFlags(theEnv);
  instanceCount = SaveOrMarkInstances(theEnv, NULL, saveCode, classList,
                                      inheritFlag, FALSE, MarkSingleInstance);
  ...
  fwrite((void*)&InstanceFileData(theEnv)->BinaryInstanceFileSize,
         sizeof(unsigned long), 1, bsaveFP);
  fwrite((void*)&instanceCount, sizeof(long), 1, bsaveFP);

  SetAtomicValueIndices(theEnv, FALSE);
  SaveOrMarkInstances(theEnv, (void*)bsaveFP, saveCode, classList, inheritFlag,
                      FALSE, SaveSingleInstanceBinary);
...
/******************************************************
  ...
  NOTES        : None
 ******************************************************/
static long InstancesSaveCommandParser(void* theEnv, const char* functionName,
                                       long (*saveFunction)(void*, const char*,
                                                            int, EXPRESSION*,
                                                            intBool)) {
  const char* fileFound;
  DATA_OBJECT temp = DATA_OBJECT_INIT;
  ...
  if (EnvArgTypeCheck(theEnv, functionName, 1, SYMBOL_OR_STRING, &temp) ==
      FALSE)
    return (0L);
  fileFound = DOToString(temp);
  ...
  if (argCount > 1) {
    if (EnvArgTypeCheck(theEnv, functionName, 2, SYMBOL, &temp) == FALSE) {
      ExpectedTypeError1(theEnv, functionName, 2,
...
static DATA_OBJECT* ProcessSaveClassList(void* theEnv, const char* functionName,
  ...
  while (classExps != NULL) {
    if (EvaluateExpression(theEnv, classExps, &tmp)) goto ProcessClassListError;
    if (tmp.type != SYMBOL) goto ProcessClassListError;
    if (saveCode == LOCAL_SAVE)
      theDefclass =
          LookupDefclassAnywhere(theEnv, currentModule, DOToString(tmp));
    else
...
/****************************************************
  ...
  NOTES        : None
 ****************************************************/
static void ReturnSaveClassList(void* theEnv, DATA_OBJECT* classList) {
  DATA_OBJECT* tmp;
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static long SaveOrMarkInstances(void* theEnv, void* theOutput, int saveCode,
                                DATA_OBJECT* classList, intBool inheritFlag,
                                intBool interruptOK,
                                void (*saveInstanceFunc)(void*, void*,
                                                         INSTANCE_TYPE*)) {
  struct defmodule* currentModule;
  int traversalID;
  DATA_OBJECT* tmp;
  ...
  if (classList != NULL) {
    ...
    if (traversalID != -1) {
      for (tmp = classList;
           (!((tmp == NULL) ||
              (EvaluationData(theEnv)->HaltExecution && interruptOK)));
           tmp = tmp->next)
        instanceCount += SaveOrMarkInstancesOfClass(
            theEnv, theOutput, currentModule, saveCode, (DEFCLASS*)tmp->value,
            inheritFlag, traversalID, saveInstanceFunc);
...
static long SaveOrMarkInstancesOfClass(
    void* theEnv, void* theOutput, struct defmodule* currentModule,
    int saveCode, DEFCLASS* theDefclass, intBool inheritFlag, int traversalID,
  ...
  if (((saveCode == LOCAL_SAVE) &&
    ...
    for (theInstance = (INSTANCE_TYPE*)EnvGetNextInstanceInClass(
             theEnv, (void*)theDefclass, NULL);
         theInstance != NULL;
         theInstance = (INSTANCE_TYPE*)EnvGetNextInstanceInClass(
             theEnv, (void*)theDefclass, (void*)theInstance)) {
      if (saveInstanceFunc != NULL)
  ...
  if (inheritFlag) {
    for (i = 0; i < theDefclass->directSubclasses.classCount; i++) {
      ...
      instanceCount += SaveOrMarkInstancesOfClass(
          theEnv, theOutput, currentModule, saveCode, subclass, TRUE,
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void SaveSingleInstanceText(void* theEnv, void* vLogicalName,
                                   INSTANCE_TYPE* theInstance) {
  long i;
  INSTANCE_SLOT* sp;
  ...
  EnvPrintRouter(theEnv, logicalName,
                 ValueToString(theInstance->cls->header.name));
  for (i = 0; i < theInstance->cls->instanceSlotCount; i++) {
    sp = theInstance->slotAddresses[i];
    EnvPrintRouter(theEnv, logicalName, "\n   (");
    EnvPrintRouter(theEnv, logicalName,
                   ValueToString(sp->desc->slotName->name));
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void WriteBinaryHeader(void* theEnv, FILE* bsaveFP) {
  fwrite(
      (void*)InstanceFileData(theEnv)->InstanceBinaryPrefixID,
      (STD_SIZE)(strlen(InstanceFileData(theEnv)->InstanceBinaryPrefixID) + 1),
      1, bsaveFP);
  fwrite(
      (void*)InstanceFileData(theEnv)->InstanceBinaryVersionID,
      (STD_SIZE)(strlen(InstanceFileData(theEnv)->InstanceBinaryVersionID) + 1),
      1, bsaveFP);
}
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void MarkSingleInstance(void* theEnv, void* theOutput,
                               INSTANCE_TYPE* theInstance) {
#if MAC_XCD
  ...
  for (i = 0; i < theInstance->cls->instanceSlotCount; i++) {
    ...
    if (sp->desc->multiple) {
      for (j = 1; j <= GetInstanceSlotLength(sp); j++)
        MarkNeededAtom(theEnv, GetMFType(sp->value, j),
                       GetMFValue(sp->value, j));
    } else
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void MarkNeededAtom(void* theEnv, int type, void* value) {
  InstanceFileData(theEnv)->BinaryInstanceFileSize +=
      (unsigned long)sizeof(struct bsaveSlotValueAtom);
...
/****************************************************
  ...
  NOTES        : None
 ****************************************************/
static void SaveSingleInstanceBinary(void* theEnv, void* vBsaveFP,
                                     INSTANCE_TYPE* theInstance) {
  long nameIndex;
  long i, j;
  INSTANCE_SLOT* sp;
  ...
  /* ======================================
     ...
     ====================================== */
  fwrite((void*)&theInstance->cls->instanceSlotCount, (int)sizeof(short), 1,
         bsaveFP);
  ...
  /* ==================================
     ...
     ================================== */
  if (theInstance->cls->instanceSlotCount !=
      0)  // (totalValueCount != 0L) : Bug fix if any slots, write out count
  ...
  for (i = 0; i < theInstance->cls->instanceSlotCount; i++) {
    ...
    if (sp->desc->multiple) {
      for (j = 1; j <= slotLen; j++)
        SaveAtomBinary(theEnv, GetMFType(sp->value, j),
                       GetMFValue(sp->value, j), bsaveFP);
    } else
...
/***************************************************
  ...
  NOTES        :
 ***************************************************/
static void SaveAtomBinary(void* theEnv, unsigned short type, void* value,
                           FILE* bsaveFP) {
  ...
  switch (type) {
    ...
    case INSTANCE_ADDRESS:
      ...
      bsa.value =
          (long)GetFullInstanceName(theEnv, (INSTANCE_TYPE*)value)->bucket;
      break;
    default:
      bsa.value = -1L;
  }
...
static long LoadOrRestoreInstances(void* theEnv, const char* file, int usemsgs,
  ...
  while ((GetType(DefclassData(theEnv)->ObjectParseToken) != STOP) &&
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void ProcessFileErrorMessage(void* theEnv, const char* functionName,
                                    const char* fileName) {
...
/*******************************************************
  ...
  NOTES        : Assumes file already open with
                 GenOpenReadBinary
 *******************************************************/
static intBool VerifyBinaryHeader(void* theEnv, const char* theFile) {
  char buf[20];

  GenReadBinary(
      theEnv, (void*)buf,
      (unsigned long)(strlen(InstanceFileData(theEnv)->InstanceBinaryPrefixID) +
                      1));
  ...
  if (strcmp(buf, InstanceFileData(theEnv)->InstanceBinaryVersionID) != 0) {
    ...
    EnvPrintRouter(theEnv, WERROR, theFile);
    EnvPrintRouter(theEnv, WERROR,
                   " file is not a compatible binary instances file.\n");
    return (FALSE);
  }
  return (TRUE);
}
 ...
 ***************************************************/
static intBool LoadSingleBinaryInstance(void* theEnv) {
  SYMBOL_HN *instanceName, *className;
  short slotCount;
  DEFCLASS* theDefclass;
  ...
  /* ====================================
     ...
     ==================================== */
  bsArray = (struct bsaveSlotValue*)gm2(
      theEnv, (sizeof(struct bsaveSlotValue) * slotCount));
  BufferedRead(theEnv, (void*)bsArray,
               (unsigned long)(sizeof(struct bsaveSlotValue) * slotCount));

  BufferedRead(theEnv, (void*)&totalValueCount,
               (unsigned long)sizeof(unsigned long));

  if (totalValueCount != 0L) {
    bsaArray = (struct bsaveSlotValueAtom*)gm3(
        theEnv, (long)(totalValueCount * sizeof(struct bsaveSlotValueAtom)));
    BufferedRead(
  ...
  for (i = 0, j = 0L; i < slotCount; i++) {
    ...
    if (PutSlotValue(theEnv, newInstance, sp, &slotValue, &junkValue,
                     "bload-instances") == FALSE)
      goto LoadError;
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void BinaryLoadInstanceError(void* theEnv, SYMBOL_HN* instanceName,
                                    DEFCLASS* theDefclass) {
  PrintErrorID(theEnv, "INSFILE", 4, FALSE);
  EnvPrintRouter(theEnv, WERROR,
                 "Function bload-instances unable to load instance [");
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void CreateSlotValue(void* theEnv, DATA_OBJECT* result,
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void* GetBinaryAtomValue(void* theEnv, struct bsaveSlotValueAtom* ba) {
  switch (ba->type) {
    case SYMBOL:
    case STRING:
    case INSTANCE_NAME:
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static void BufferedRead(void* theEnv, void* buf, unsigned long bufsz) {
  unsigned long i, amountLeftToRead;

  if (InstanceFileData(theEnv)->CurrentReadBuffer != NULL) {
    amountLeftToRead = InstanceFileData(theEnv)->CurrentReadBufferSize -
    ...
    if (bufsz > MAX_BLOCK_SIZE) {
      ...
      if (bufsz > (InstanceFileData(theEnv)->BinaryInstanceFileSize -
    ...
    InstanceFileData(theEnv)->CurrentReadBuffer = (char*)genalloc(
        theEnv, InstanceFileData(theEnv)->CurrentReadBufferSize);
    GenReadBinary(theEnv, (void*)InstanceFileData(theEnv)->CurrentReadBuffer,
                  InstanceFileData(theEnv)->CurrentReadBufferSize);
    for (i = 0L; i < bufsz; i++)
...
/*****************************************************
  ...
  NOTES        : None
 *****************************************************/
static void FreeReadBuffer(void* theEnv) {
  if (InstanceFileData(theEnv)->CurrentReadBufferSize != 0L) {
    genfree(theEnv, (void*)InstanceFileData(theEnv)->CurrentReadBuffer,
            InstanceFileData(theEnv)->CurrentReadBufferSize);
...
#if BLOAD_INSTANCES
globle long BinaryLoadInstances(const char* theFile) {
  return EnvBinaryLoadInstances(GetCurrentEnvironment(), theFile);
}
#endif

#if BSAVE_INSTANCES
globle long BinarySaveInstances(const char* file, int saveCode) {
  return EnvBinarySaveInstances(GetCurrentEnvironment(), file, saveCode);
}
#endif

globle long LoadInstances(const char* file) {
  return EnvLoadInstances(GetCurrentEnvironment(), file);
}

globle long LoadInstancesFromString(const char* theString, int theMax) {
  return EnvLoadInstancesFromString(GetCurrentEnvironment(), theString, theMax);
}

globle long RestoreInstances(const char* file) {
  return EnvRestoreInstances(GetCurrentEnvironment(), file);
}

globle long RestoreInstancesFromString(const char* theString, int theMax) {
  return EnvRestoreInstancesFromString(GetCurrentEnvironment(), theString,
                                       theMax);
}

globle long SaveInstances(const char* file, int saveCode) {
