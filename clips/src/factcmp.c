...
#if DEFRULE_CONSTRUCT && (!RUN_TIME) && DEFTEMPLATE_CONSTRUCT && \
    CONSTRUCT_COMPILER
...
static int PatternNetworkToCode(void*, const char*, const char*, char*, int,
                                FILE*, int, int);
...
globle void FactPatternsCompilerSetup(void* theEnv) {
  FactData(theEnv)->FactCodeItem =
      AddCodeGeneratorItem(theEnv, "facts", 0, BeforePatternNetworkToCode, NULL,
                           PatternNetworkToCode, 1);
}
...
/****************************************************************/
static void BeforePatternNetworkToCode(void* theEnv) {
  int whichPattern = 0;
  ...
  for (theModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    ...
    for (theDeftemplate =
      ...
      for (thePattern = theDeftemplate->patternNetwork; thePattern != NULL;
...
/********************************************************************/
static int PatternNetworkToCode(void* theEnv, const char* fileName,
  ...
  for (theModule = (struct defmodule*)EnvGetNextDefmodule(theEnv, NULL);
    ...
    for (theTemplate = (struct deftemplate*)EnvGetNextDeftemplate(theEnv, NULL);
      ...
      for (thePatternNode = theTemplate->patternNetwork; thePatternNode != NULL;
           thePatternNode = GetNextPatternNode(thePatternNode)) {
        networkFile = OpenFileIfNeeded(
            theEnv, networkFile, fileName, pathName, fileNameBuffer, fileID,
            imageID, &fileCount, networkArrayVersion, headerFP,
        ...
        networkArrayCount++;
        networkFile =
            CloseFileIfNeeded(theEnv, networkFile, &networkArrayCount,
...
/****************************************************************/
static void CloseNetworkFiles(void* theEnv, FILE* networkFile, int maxIndices) {
  int count = maxIndices;
  int arrayVersion = 0;

  if (networkFile != NULL) {
    CloseFileIfNeeded(theEnv, networkFile, &count, &arrayVersion, maxIndices,
                      NULL, NULL);
  }
}
...
/************************************************************/
static void PatternNodeToCode(void* theEnv, FILE* theFile,
                              struct factPatternNode* thePatternNode,
  ...
  PatternNodeHeaderToCode(theEnv, theFile, &thePatternNode->header, imageID,
                          maxIndices);
  ...
  PrintHashedExpressionReference(theEnv, theFile, thePatternNode->networkTest,
                                 imageID, maxIndices);
  ...
  if (thePatternNode->nextLevel == NULL) {
    ...
    fprintf(theFile, ",&%s%d_%ld[%ld],", FactPrefix(), imageID,
            (thePatternNode->nextLevel->bsaveID / maxIndices) + 1,
  ...
  if (thePatternNode->lastLevel == NULL) {
    ...
    fprintf(theFile, "&%s%d_%ld[%ld],", FactPrefix(), imageID,
            (thePatternNode->lastLevel->bsaveID / maxIndices) + 1,
  ...
  if (thePatternNode->leftNode == NULL) {
    ...
    fprintf(theFile, "&%s%d_%ld[%ld],", FactPrefix(), imageID,
            (thePatternNode->leftNode->bsaveID / maxIndices) + 1,
  ...
  if (thePatternNode->rightNode == NULL) {
    ...
    fprintf(theFile, "&%s%d_%ld[%ld]}", FactPrefix(), imageID,
            (thePatternNode->rightNode->bsaveID / maxIndices) + 1,
...
/**********************************************************/
globle void FactPatternNodeReference(void* theEnv, void* theVPattern,
...
#endif /* DEFRULE_CONSTRUCT && (! RUN_TIME) && DEFTEMPLATE_CONSTRUCT && \
          CONSTRUCT_COMPILER */
