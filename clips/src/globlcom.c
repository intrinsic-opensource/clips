...
#if !RUN_TIME
  EnvDefineFunction2(theEnv, "set-reset-globals", 'b', SetResetGlobalsCommand,
                     "SetResetGlobalsCommand", "11");
  EnvDefineFunction2(theEnv, "get-reset-globals", 'b', GetResetGlobalsCommand,
                     "GetResetGlobalsCommand", "00");

#if DEBUGGING_FUNCTIONS
  EnvDefineFunction2(theEnv, "show-defglobals", 'v',
                     PTIEF ShowDefglobalsCommand, "ShowDefglobalsCommand",
                     "01w");
#endif
...
globle void ShowDefglobalsCommand(void* theEnv) {
  ...
  if ((numArgs =
           EnvArgCountCheck(theEnv, "show-defglobals", NO_MORE_THAN, 1)) == -1)
    return;
...
/*   for the show-defglobals command.  */
/***************************************/
globle void EnvShowDefglobals(void* theEnv, const char* logicalName,
                              void* vTheModule) {
  ...
  for (; theModule != NULL;
    ...
    for (constructPtr = theModuleItem->firstItem; constructPtr != NULL;
...
/*   ?*x* = 3                                        */
/*****************************************************/
static void PrintDefglobalValueForm(void* theEnv, const char* logicalName,
                                    void* vTheGlobal) {
...
globle intBool SetResetGlobals(int value) {
  return EnvSetResetGlobals(GetCurrentEnvironment(), value);
}

#if DEBUGGING_FUNCTIONS

globle void ShowDefglobals(const char* logicalName, void* vTheModule) {
