...
/********************************************************/
globle void InvalidDeftemplateSlotMessage(void* theEnv, const char* slotName,
...
globle void MultiIntoSingleFieldSlotError(void* theEnv,
  ...
  if (theSlot != NULL)
    ...
    EnvPrintRouter(theEnv, WERROR, "<<unknown>>");
...
globle void CheckTemplateFact(void* theEnv, struct fact* theFact) {
  ...
  for (slotPtr = theDeftemplate->slotList; slotPtr != NULL;
    ...
    if (slotPtr->multislot == FALSE) {
      ...
      SetDOEnd(theData,
               ((struct multifield*)sublist[i].value)->multifieldLength);
      i++;
    }
    ...
    if (rv != NO_VIOLATION) {
      ...
      EnvPrintRouter(theEnv, WERROR, " ");
      ConstraintViolationErrorMessage(theEnv, NULL, thePlace, FALSE, 0,
...
/***********************************************************************/
globle intBool CheckRHSSlotTypes(void* theEnv, struct expr* rhsSlots,
                                 struct templateSlot* slotPtr,
                                 const char* thePlace) {
  int rv;
  const char* theName;
...
globle int FindSlotPosition(struct deftemplate* theDeftemplate,
  ...
  for (slotPtr = theDeftemplate->slotList, position = 1; slotPtr != NULL;
...
/*   otherwise FALSE.                                        */
/*******************************************************************/
globle void PrintTemplateFact(void* theEnv, const char* logicalName,
                              struct fact* theFact, int seperateLines,
                              int ignoreDefaults) {
  struct field* sublist;
  int i;
  struct deftemplate* theDeftemplate;
  ...
  while (slotPtr != NULL) {
    ...
    /*==========================================================*/
      ...
      if (theSegment->multifieldLength > 0) {
        EnvPrintRouter(theEnv, logicalName, " ");
        PrintMultifield(theEnv, logicalName,
                        (struct multifield*)sublist[i].value, 0,
                        (long)theSegment->multifieldLength - 1, FALSE);
      }
    }
